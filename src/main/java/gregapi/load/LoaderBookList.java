...
public class LoaderBookList implements Runnable {
    ...
    public void run() {
        ...
        if (MD.ARS.mLoaded) {
            ...
            for (int i = 0; i < 17; i++) BooksGT.BOOK_REGISTER.put(MD.ARS, "rune", i, (byte) 15);
        ...
        if (MD.HBM.mLoaded) {
            ...
            BooksGT.BOOK_REGISTER.put(MD.HBM, "item.bobmazon_materials", W, (byte) 48); // Technically Beige instead of
                                                                                        // Orange
            BooksGT.BOOK_REGISTER.put(MD.HBM, "item.bobmazon_machines", W, (byte) 5);
            BooksGT.BOOK_REGISTER.put(MD.HBM, "item.bobmazon_weapons", W, (byte) 3); // Technically Gray instead of
                                                                                     // Black
