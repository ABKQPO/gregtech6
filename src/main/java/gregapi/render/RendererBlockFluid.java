...
public class RendererBlockFluid implements ISimpleBlockRenderingHandler {
    ...
    private static final float MAX_FLUID_HEIGHT = 0.8888889F, LIGHT_Y_NEG = 0.5F, LIGHT_Y_POS = 1.0F,
        LIGHT_XZ_NEG = 0.8F, LIGHT_XZ_POS = 0.6F;
    ...
    public float getFluidHeightForRender(IBlockAccess aWorld, int aX, int aY, int aZ, BlockFluidBase aFluidBlock,
        ...
        if (aBlock == aFluidBlock) {
            ...
            if (tBlockAbove.getMaterial()
                .isLiquid() || tBlockAbove instanceof IFluidBlock) return 1;
            return UT.Code.bindF(aFluidBlock.getQuantaPercentage(aWorld, aX, aY, aZ)) * MAX_FLUID_HEIGHT;
        }
        return !aBlock.getMaterial()
            .isSolid() && aWorld.getBlock(aX, aY - FL.dir(aFluidBlock), aZ) == aFluidBlock ? 1
                : UT.Code.bindF(aFluidBlock.getQuantaPercentage(aWorld, aX, aY, aZ)) * MAX_FLUID_HEIGHT;
    }
    ...
    public boolean renderWorldBlock(IBlockAccess aWorld, int aX, int aY, int aZ, Block aBlock, int aModelID,
        ...
        boolean renderBottom = aFluid.shouldSideBeRendered(aWorld, aX, aY + aDir, aZ, 0)
            && aWorld.getBlock(aX, aY + aDir, aZ) != aFluid;

        boolean[] renderSides = new boolean[] { aFluid.shouldSideBeRendered(aWorld, aX, aY, aZ - 1, 2),
