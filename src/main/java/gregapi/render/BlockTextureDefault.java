...
public class BlockTextureDefault implements ITexture {
    ...
    public static BlockTextureDefault get(OreDictMaterial aMaterial, OreDictPrefix aPrefix, short[] aRGBa,
        boolean aGlow, boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(aMaterial, aPrefix, aRGBa, aGlow, aEnableAO)
            : null;
    }

    public static BlockTextureDefault get(OreDictMaterial aMaterial, int aTextureSetIndex, int aState, boolean aGlow,
        boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aMaterial, aTextureSetIndex, aState, aGlow, aEnableAO)
            : null;
    }
    ...
    public static BlockTextureDefault get(OreDictMaterial aMaterial, int aTextureSetIndex, short[] aRGBa, boolean aGlow,
        boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aMaterial, aTextureSetIndex, aRGBa, aGlow, aEnableAO)
            : null;
    }
    ...
    public static BlockTextureDefault get(OreDictMaterial aMaterial, int aTextureSetIndex, int aState, boolean aGlow) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(aMaterial, aTextureSetIndex, aState, aGlow)
            : null;
    }

    public static BlockTextureDefault get(OreDictMaterial aMaterial, int aTextureSetIndex, boolean aGlow, int aRGBa) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(aMaterial, aTextureSetIndex, aGlow, aRGBa)
            : null;
    }

    public static BlockTextureDefault get(OreDictMaterial aMaterial, int aTextureSetIndex, short[] aRGBa,
        boolean aGlow) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(aMaterial, aTextureSetIndex, aRGBa, aGlow)
            : null;
    }
    ...
    public static BlockTextureDefault get(IIconContainer aIcon, int aRGBa, boolean aAllowAlpha,
        boolean aUseMaxBrightness, boolean aUseConstantBrightness, boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aIcon, aRGBa, aAllowAlpha, aUseMaxBrightness, aUseConstantBrightness, aEnableAO)
            : null;
    }

    public static BlockTextureDefault get(IIconContainer aIcon, short[] aRGBa, boolean aAllowAlpha,
        boolean aUseMaxBrightness, boolean aUseConstantBrightness, boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aIcon, aRGBa, aAllowAlpha, aUseMaxBrightness, aUseConstantBrightness, aEnableAO)
            : null;
    ...
    public static BlockTextureDefault get(String aIcon, int aRGBa, boolean aGlow) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(new Textures.BlockIcons.CustomIcon(aIcon), aRGBa, aGlow)
            : null;
    }

    public static BlockTextureDefault get(String aIcon, boolean aGlow) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(new Textures.BlockIcons.CustomIcon(aIcon), aGlow)
            : null;
    }

    public static BlockTextureDefault get(String aIcon, int aRGBa) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(new Textures.BlockIcons.CustomIcon(aIcon), aRGBa)
            : null;
    }
    ...
    public static BlockTextureDefault get(String aIcon, int aRGBa, boolean aAllowAlpha, boolean aUseMaxBrightness,
        boolean aUseConstantBrightness, boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(
                new Textures.BlockIcons.CustomIcon(aIcon),
                aRGBa,
                aAllowAlpha,
                aUseMaxBrightness,
    ...
    public static BlockTextureDefault get(String aIcon, short[] aRGBa, boolean aAllowAlpha, boolean aUseMaxBrightness,
        boolean aUseConstantBrightness, boolean aEnableAO) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(
                new Textures.BlockIcons.CustomIcon(aIcon),
    ...
    public static BlockTextureDefault get(String aIcon) {
        return CODE_CLIENT || CODE_UNCHECKED ? new BlockTextureDefault(new Textures.BlockIcons.CustomIcon(aIcon))
            : null;
    }
    ...
    public static BlockTextureDefault get(IIconContainer aIcon, int aRGBa, int aBrightness) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aIcon, UT.Code.getRGBaArray(aRGBa), aBrightness, F)
            : null;
    }

    public static BlockTextureDefault get(IIconContainer aIcon, int aRGBa, int aBrightness, boolean aAllowAlpha) {
        return CODE_CLIENT || CODE_UNCHECKED
            ? new BlockTextureDefault(aIcon, UT.Code.getRGBaArray(aRGBa), aBrightness, aAllowAlpha)
            : null;
    }
    ...
    public BlockTextureDefault(IIconContainer aIcon, int aRGBa, boolean aAllowAlpha, boolean aUseMaxBrightness,
        boolean aUseConstantBrightness, boolean aEnableAO) {
        this(aIcon, UT.Code.getRGBaArray(aRGBa), aAllowAlpha, aUseMaxBrightness, aUseConstantBrightness, aEnableAO);
    }

    public BlockTextureDefault(IIconContainer aIcon, short[] aRGBa, boolean aAllowAlpha, boolean aUseMaxBrightness,
        boolean aUseConstantBrightness, boolean aEnableAO) {
    ...
    public void renderXPos(RenderBlocks aRenderer, Block aBlock, int aX, int aY, int aZ, int aBrightness,
        ...
        for (int i = 0, j = mIconContainer.getIconPasses(); i < j; i++) ITexture.Util.renderSide(
            SIDE_X_POS,
            mIconContainer.getIcon(i),
            mIconContainer.isUsingColorModulation(i) ? fRGBa : mIconContainer.getIconColor(i),
            mAllowAlpha,
            mUseConstantBrightness,
            mEnableAO,
            aRenderer,
            aBlock,
            aX,
            aY,
            aZ,
            mUseOwnBrightness ? mBrightness : aBrightness,
            aChangedBlockBounds);
    }
