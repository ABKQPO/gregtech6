...
public class BlockTextureFluid implements ITexture {
    ...
    public BlockTextureFluid(FluidStack aFluid, boolean aAllowAlpha) {
        if (aFluid == null || aFluid.getFluid() == null) {
            ...
            mLuminosity = aFluid.getFluid()
                .getLuminosity(aFluid) * 16;
            Block tBlock = aFluid.getFluid()
                .getBlock();
            if (tBlock != null && tBlock != NB) {
                mIcon = tBlock.getIcon(0, 0);
                mRGBa = UT.Code.getRGBaArray(tBlock.getRenderColor(0));
            } else {
                mIcon = aFluid.getFluid()
                    .getStillIcon();
                mRGBa = UT.Code.getRGBaArray(
                    aFluid.getFluid()
                        .getColor(aFluid));
            }
        }
    ...
    public void renderXPos(RenderBlocks aRenderer, Block aBlock, int aX, int aY, int aZ, int aBrightness,
        ...
        ITexture.Util.renderSide(
            ...
            aY,
            aZ,
            Math.max(mLuminosity, aBrightness),
            aChangedBlockBounds);
        // aRenderer.flipTexture = !aRenderer.flipTexture;
    }
