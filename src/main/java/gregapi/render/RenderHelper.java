...
public class RenderHelper {
    ...
    public static void renderItemIntoGUI(FontRenderer aFontRenderer, TextureManager aTextureManager, ItemStack aStack,
        int aX, int aY, boolean aEffect) {
        glAlphaFunc(GL_GREATER, 0.1F);
        glEnable(GL_ALPHA_TEST);
        if (aStack.getItemSpriteNumber() == 0 && RenderBlocks.renderItemIn3d(
            Block.getBlockFromItem(aStack.getItem())
                .getRenderType())) {
            ...
            int tMaxRenderPasses = Math.max(
                1,
                aStack.getItem()
                    .getRenderPasses(ST.meta_(aStack)));
            for (int i = 0; i < tMaxRenderPasses; i++) {
                IIcon tIcon = aStack.getItem()
                    .getIcon(aStack, i);
                if (tIcon == null) tIcon = ((TextureMap) Minecraft.getMinecraft()
                    .getTextureManager()
                    .getTexture(tResource)).getAtlasSprite("missingno");
                glDisable(GL_LIGHTING);
                glEnable(GL_BLEND);
                OpenGlHelper.glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, 1, 0);
                aTextureManager.bindTexture(tResource);
                int tColor = aStack.getItem()
                    .getColorFromItemStack(aStack, i);
                glColor4f((tColor >> 16 & 255) / 255F, (tColor >> 8 & 255) / 255F, (tColor & 255) / 255F, 1);
    ...
    public static void drawWrenchOverlay(EntityPlayer aPlayer, int aX, int aY, int aZ, byte aConnections, byte aSide,
        float aPartialTicks) {
        try {
        ...
        GL11.glPushMatrix();
        GL11.glTranslated(
            -(aPlayer.lastTickPosX + (aPlayer.posX - aPlayer.lastTickPosX) * aPartialTicks),
            -(aPlayer.lastTickPosY + (aPlayer.posY - aPlayer.lastTickPosY) * aPartialTicks),
            -(aPlayer.lastTickPosZ + (aPlayer.posZ - aPlayer.lastTickPosZ) * aPartialTicks));
        ...
        double tColor = (CLIENT_TIME % 42 < 21 ? 0.25 + ((CLIENT_TIME % 21) / 40.0)
            : 0.75 - ((CLIENT_TIME % 21) / 40.0));
