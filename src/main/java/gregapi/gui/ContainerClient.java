...
public class ContainerClient extends GuiContainer {
        ...
        try {
            ...
            for (int i = 0; i < inventorySlots.inventorySlots.size(); ++i) {
                ...
                if (ST.invalid(tSlot.getStack()) && isMouseOverSlot(tSlot, aX, aY) && tSlot instanceof Slot_Base) {
                    drawHoveringText(
                        ((Slot_Base) tSlot).getTooltip(mc.thePlayer, mc.gameSettings.advancedItemTooltips),
                        aX,
                        aY,
                        fontRendererObj);
                }
    ...
    protected boolean isMouseOverSlot(Slot aSlot, int aX, int aY) {
        return func_146978_c(aSlot.xDisplayPosition, aSlot.yDisplayPosition, 16, 16, aX, aY);
    }
