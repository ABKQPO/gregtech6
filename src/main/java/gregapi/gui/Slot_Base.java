...
public class Slot_Base extends Slot {
    ...
    public ItemStack getStack() {
        ...
        return ST.meta(rStack) != W || ST.isGT(rStack) ? rStack
            : ST.name(ST.copyMeta(0, rStack), ST.regName(rStack) + ":Wildcard");
    }
    ...
    public Slot_Base setTooltips(String[] aTooltips, String[] aToolTipColors) {
        mToolTips = aTooltips;
        mToolTipColors = (aToolTipColors.length < mToolTips.length ? mToolTipColors = new String[mToolTips.length]
            : aToolTipColors);
        return this;
    }
    ...
    public boolean isItemValid(ItemStack aStack) {
        return mCanPut && mInventory.isItemValidForSlotGUI(mIndex, aStack);
    }

    @Override
    public boolean canTakeStack(EntityPlayer aPlayer) {
        return mInventory.canTakeOutOfSlotGUI(mIndex)
            && (UT.Entities.isCreative(aPlayer) || (mCanTake && !ST.debug(getStack())));
    }

    @Override
    public boolean isSlotInInventory(IInventory aInventory, int aIndex) {
        return aInventory == mInventory && aIndex == mIndex;
    }

    @Override
    public int getSlotStackLimit() {
        return mInventory.getInventoryStackLimitGUI(mIndex);
    }
    ...
    @Override
    public ItemStack decrStackSize(int aAmount) {
        return mInventory.decrStackSizeGUI(mIndex, aAmount);
    }

    @Override
    public void onSlotChanged() {
        mInventory.markDirtyGUI();
    }
    ...
    @Override
    public void onSlotChange(ItemStack aStack, ItemStack aStack2) {
        if (ST.equal(aStack, aStack2, T)) {
            int tDifference = aStack2.stackSize - aStack.stackSize;
            if (tDifference > 0) onCrafting(aStack, tDifference);
        }
    }

    @Override
    protected void onCrafting(ItemStack aStack, int aDifference) {/**/}

    @Override
    protected void onCrafting(ItemStack aStack) {/**/}

    @Override
    public void onPickupFromSlot(EntityPlayer aPlayer, ItemStack aStack) {
        onSlotChanged();
    }
