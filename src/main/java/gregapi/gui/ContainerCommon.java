...
public class ContainerCommon extends Container {
    ...
    public ContainerCommon(InventoryPlayer aInventoryPlayer, ITileEntityInventoryGUI aTileEntity, int aOffset,
        int aSlotCount) {
    ...
    public int getSlotCount() {
        return useDefaultSlots() ? mSlotCount : 0;
    }
    ...
    protected final int getAllSlotCount() {
        return inventorySlots != null ? doesBindPlayerInventory() ? inventorySlots.size() - 36 : inventorySlots.size()
            : getSlotCount();
    }
    ...
    public boolean canInteractWith(EntityPlayer aPlayer) {
        return mTileEntity.isUseableByPlayerGUI(aPlayer);
    }
    ...
    public ItemStack slotClick(int aIndex, int aMouse, int aShift, EntityPlayer aPlayer) {
        ...
        try {
            if (aSlot instanceof Slot_Base && mTileEntity.interceptClick(
                mGUIID,
                (Slot_Base) aSlot,
                aIndex,
                aSlot.getSlotIndex(),
                ...
                aMouse,
                aShift)) {
                ItemStack rStack = mTileEntity.slotClick(
                    mGUIID,
                    (Slot_Base) aSlot,
                    aIndex,
                    aSlot.getSlotIndex(),
        ...
        if (aIndex >= 0) {
            ...
            if (!(aSlot instanceof Slot_Armor)) if (aIndex < getAllSlotCount())
                if (aIndex < getStartIndex() || aIndex >= getStartIndex() + getSlotCount()) return null;
        }
                ...
                if (aSlot != null) {
                    ...
                    if (tTempStack == null) {
                        ...
                        if (tHeldStack == null) {
                            ...
                            if (tTempStack.getItem() == tHeldStack.getItem()
                                && tTempStack.getItemDamage() == tHeldStack.getItemDamage()
                                && ItemStack.areItemStackTagsEqual(tTempStack, tHeldStack)) {
                                tTempStackSize = (aMouse == 0 ? tHeldStack.stackSize : 1);
                        ...
                        } else if (tTempStack.getItem() == tHeldStack.getItem() && tHeldStack.getMaxStackSize() > 1
                            ...
                            && ItemStack.areItemStackTagsEqual(tTempStack, tHeldStack)) {
                                ...
                                if (tTempStackSize > 0
                                    && tTempStackSize + tHeldStack.stackSize <= tHeldStack.getMaxStackSize()) {
        ...
        } else if (aShift == 3 && UT.Entities.hasInfiniteItems(aPlayer)
        ...
        if (getSlotCount() > 0 && tSlot != null && tSlot.getHasStack() && !(tSlot instanceof Slot_Holo)) {
            ...
            if (aIndex < getAllSlotCount()) {
                if (doesBindPlayerInventory()
                    && !mergeItemStack(tStack, getAllSlotCount(), getAllSlotCount() + 36, T)) {
                    return null;
                }
                // Player -> TileEntity
            ...
            if (tStack.stackSize == 0) tSlot.putStack(null);
            else tSlot.onSlotChanged();
        }
        return rStack;
    }
    ...
    protected boolean mergeItemStack(ItemStack aStack, int aStartIndex, int aSlotCount, boolean aReverse) {
        ...
        if (aStack.stackSize > 0) {
            if (aReverse) tIndex = aSlotCount - 1;
            else tIndex = aStartIndex;
            while (aReverse ? tIndex >= aStartIndex : tIndex < aSlotCount) {
                Slot tSlot = (Slot) inventorySlots.get(tIndex);
                if (!(tSlot instanceof Slot_Holo) && tSlot.isItemValid(aStack)) {
                    if (tSlot.getStack() == null) {
                        ItemStack tStack = ST.amount(
                            Math.min(aStack.stackSize, Math.min(aStack.getMaxStackSize(), tSlot.getSlotStackLimit())),
                            aStack);
            ...
            if (mTileEntity.needsToSyncEverything()) {
                for (int i = 0; i < inventorySlots.size(); ++i) {
                    ...
                    for (int j = 0; j < crafters.size(); ++j)
                        ((ICrafting) crafters.get(j)).sendSlotContents(this, i, tStack);
                }
            } else {
                for (int i = 0; i < inventorySlots.size(); ++i) {
