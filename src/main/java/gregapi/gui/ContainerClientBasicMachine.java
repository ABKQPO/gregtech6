...
public class ContainerClientBasicMachine extends ContainerClient {
    ...
    public ContainerClientBasicMachine(InventoryPlayer aInventoryPlayer, ITileEntityInventoryGUI aTileEntity,
        RecipeMap aRecipes, int aGUIID, String aGUITexture) {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        fontRendererObj.drawString(
            mContainer.mTileEntity.hasCustomInventoryNameGUI() ? mContainer.mTileEntity.getInventoryNameGUI()
    ...
    protected void drawGuiContainerBackgroundLayer2(float par1, int par2, int par3) {
        ...
        if (mContainer instanceof ContainerCommonBasicMachine) {
            if (((ContainerCommonBasicMachine) mContainer).mProgressBar >= 0) {
                int tSize = (mRecipes.mProgressBarDirection % 4 < 2 ? 20 : 18), tProgress = (int) UT.Code.scale(
                    ((ContainerCommonBasicMachine) mContainer).mProgressBar,
                    0,
                    Short.MAX_VALUE,
                    tSize * UT.Code.unsignB(mRecipes.mProgressBarAmount),
                    F) % (tSize + 1);
