...
public class CompatCC extends CompatBase implements ICompatCC, IPeripheralProvider {
    ...
    public IPeripheral getPeripheral(World aWorld, int aX, int aY, int aZ, int aSide) {
        ...
        if (SIDES_VALID[aDelegator.mSideOfTileEntity] && aDelegator.mTileEntity instanceof ITileEntityCoverable) {
            ...
            if (tData != null && tData.mBehaviours[aDelegator.mSideOfTileEntity] instanceof ICoverComputerizable)
                return new ComputerizablePeripheral(
                    (ICoverComputerizable) tData.mBehaviours[aDelegator.mSideOfTileEntity],
                    aDelegator);
        }
        return aDelegator.mTileEntity instanceof ITileEntityComputerizable
            ? new ComputerizablePeripheral((ITileEntityComputerizable) aDelegator.mTileEntity, aDelegator)
            : null;
    }
    ...
    public static class ComputerizablePeripheral implements IPeripheral {
        ...
        public String[] getMethodNames() {
            return mComputerizable.allComputerizableMethods(mDelegator);
        }

        @Override
        public Object[] callMethod(IComputerAccess aComputer, ILuaContext aContext, int aFunctionIndex,
            Object[] aArguments) throws LuaException, InterruptedException {
            return mComputerizable.callComputerizableMethod(mDelegator, aFunctionIndex, aArguments);
        }
        ...
        public boolean equals(IPeripheral aOther) {
            return aOther == this || (aOther instanceof ComputerizablePeripheral
                && ((ComputerizablePeripheral) aOther).mComputerizable == mComputerizable
                && mDelegator.equalSideTileEntityAndCoords(((ComputerizablePeripheral) aOther).mDelegator));
        }
    }
}
