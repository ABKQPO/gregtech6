...
public class CompatIC2 extends CompatBase implements ICompatIC2 {
    ...
    public void onPostLoad(FMLPostInitializationEvent aEvent) {
        ...
        if (mToBlacklist != null) {
            for (ItemStackContainer tStack : mToBlacklist)
                ic2.api.recipe.Recipes.recyclerBlacklist.add((IRecipeInput) makeInput(tStack.toStack()));
            mToBlacklist.clear();
            mToBlacklist = null;
        }

        if (MD.AA.mLoaded) Crops.instance.registerBaseSeed(
            ST.make(MD.AA, "itemCoffeeBeans", 1, W),
            Crops.instance.getCropCard("IC2", "Coffee"),
            1,
            1,
            1,
            1);
    ...
    public void onRetextureEvent(RetextureEvent aEvent) {
        ...
        if (tTileEntity instanceof ITileEntityCoverable) {
            ...
            if (tData != null && tData.mBehaviours[aEvent.side] instanceof CoverTextureCanvas) {
                ...
                tData.mBehaviours[aEvent.side]
                    .onCoverPlaced((byte) aEvent.side, tData, null, tData.getCoverItem((byte) aEvent.side));
                aEvent.applied = T;
            }
        }
    }
    ...
    public boolean valuable(Block aBlock, int aMeta, int aValue) {
        ...
        ic2.core.IC2
            .addValuableOre((IRecipeInput) makeInput(ST.make(aBlock, 1, UT.Code.bind4(aMeta))), Math.max(1, aValue));
        return T;
    }
    ...
    public ItemStack recycler(ItemStack aInput, int aScrapChance) {
        if (aInput == null || aScrapChance != 0 || ic2.api.recipe.Recipes.recyclerBlacklist.contains(aInput))
            return null;
        return ic2.api.recipe.Recipes.recyclerWhitelist.isEmpty()
            || ic2.api.recipe.Recipes.recyclerWhitelist.contains(aInput) ? IL.IC2_Scrap.get(1) : null;
    }
    ...
    public boolean blacklist(ItemStack aBlacklisted) {
        ...
        if (ic2.api.recipe.Recipes.recyclerBlacklist == null) mToBlacklist.add(aBlacklisted);
        else ic2.api.recipe.Recipes.recyclerBlacklist.add((IRecipeInput) makeInput(aBlacklisted));
        return T;
    }
    ...
    public Object makeInput(String aOreDict, long aAmount) {
        return new RecipeInputOreDict(aOreDict, UT.Code.bindStack(aAmount));
    }
    ...
    public boolean isReactorItem(ItemStack aStack) {
        try {
            return ST.valid(aStack) && aStack.getItem() instanceof ic2.api.reactor.IReactorComponent;
