...
public class CompatIC2EUItem extends CompatBase implements ICompatIC2EUItem {
    ...
    public long charge(ItemStack aStack, long aAmount, int aTier, boolean aIgnoreLimit, boolean aDoCharge) {
        ...
        if (tTier < 0 || tTier == aTier || aTier == Integer.MAX_VALUE) {
            if (!aIgnoreLimit && tTier >= 0)
                aAmount = (int) Math.min(aAmount, V[Math.max(0, Math.min(V.length - 1, tTier))]);
            if (aAmount > 0) {
                long rAmount = (long) Math
                    .max(0, ic2.api.item.ElectricItem.manager.charge(aStack, aAmount, tTier, T, !aDoCharge));
                return rAmount;
            }
        }
        return 0;
    ...
    @Override
    public long decharge(ItemStack aStack, long aAmount, int aTier, boolean aIgnoreLimit, boolean aDoDecharge,
        ...
        if (tTier < 0 || tTier == aTier || aTier == Integer.MAX_VALUE) {
            if (!aIgnoreLimit && tTier >= 0)
                aAmount = (int) Math.min(aAmount, V[Math.max(0, Math.min(V.length - 1, tTier))]);
            if (aAmount > 0) {
                long rAmount = (long) Math.max(
                    0,
                    ic2.api.item.ElectricItem.manager.discharge(
                        aStack,
                        aAmount + (aAmount * 4 > aTier ? aTier : 0),
                        tTier,
                        T,
    ...
    public long charge(ItemStack aStack, long aAmount, boolean aDoCharge) {
        return aAmount > 0 && ic2.api.item.ElectricItem.manager != null
            ? (long) Math
                .max(0, ic2.api.item.ElectricItem.manager.charge(aStack, aAmount, Integer.MAX_VALUE, T, !aDoCharge))
            : 0;
    }

    @Override
    public long decharge(ItemStack aStack, long aAmount, boolean aDoDecharge) {
        return aAmount > 0 && ic2.api.item.ElectricItem.manager != null ? (long) Math
            .max(0, ic2.api.item.ElectricItem.manager.discharge(aStack, aAmount, Integer.MAX_VALUE, T, F, !aDoDecharge))
            : 0;
    }

    @Override
    public byte tier(ItemStack aStack) {
        return UT.Code.bind4(((ic2.api.item.IElectricItem) aStack.getItem()).getTier(aStack));
    }

    @Override
    public boolean insidevolt(ItemStack aStack, long aMinVolt, long aMaxVolt) {
        byte tTier = tier(aStack);
        return UT.Code.inside(aMinVolt, aMaxVolt, VMIN[tTier]) || UT.Code.inside(aMinVolt, aMaxVolt, VMAX[tTier]);
    }
    ...
    public boolean is(ItemStack aStack) {
        return ST.valid(aStack) && aStack.getItem() instanceof ic2.api.item.IElectricItem
    ...
    public long stored(ItemStack aStack) {
        return ic2.api.item.ElectricItem.manager != null
            ? (long) Math
                .max(0, ic2.api.item.ElectricItem.manager.discharge(aStack, Long.MAX_VALUE, Integer.MAX_VALUE, T, F, T))
            : 0;
    }

    @Override
    public long capacity(ItemStack aStack) {
        return (long) ((ic2.api.item.IElectricItem) aStack.getItem()).getMaxCharge(aStack);
    }
}
