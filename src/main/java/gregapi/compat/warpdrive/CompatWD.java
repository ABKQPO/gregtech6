...
public class CompatWD extends CompatBase implements ICompatWD, IBlockTransformer {
    ...
    public int rotate(Block aBlock, int aMeta, NBTTagCompound aNBT, ITransformation aTransformation) {
        ...
        CoverData tCovers = aNBT.hasKey(NBT_COVERS) ? CoverRegistry.coverdata(null, aNBT.getCompoundTag(NBT_COVERS))
            : null;
        ...
        switch (aTransformation.getRotationSteps() % 4) {
            case 1:
                ...
                UT.NBT.setNumber(
                    aNBT,
                    NBT_CONNECTION,
                    (tConnections & ~60) | (FACE_CONNECTED[SIDE_NORTH][tConnections] ? SBIT_E : 0)
            ...
            case 2:
                ...
                UT.NBT.setNumber(
                    aNBT,
                    NBT_CONNECTION,
                    (tConnections & ~60) | (FACE_CONNECTED[SIDE_NORTH][tConnections] ? SBIT_S : 0)
            ...
            case 3:
                ...
                UT.NBT.setNumber(
                    aNBT,
                    NBT_CONNECTION,
                    (tConnections & ~60) | (FACE_CONNECTED[SIDE_NORTH][tConnections] ? SBIT_W : 0)
    ...
    @Override
    public boolean isJumpReady(Block aBlock, int aMeta, TileEntity aTileEntity, StringBuilder aStringBuilder) {
        return T;
    }

    @Override
    public NBTBase saveExternals(World world, int aX, int aY, int aZ, Block aBlock, int aMeta, TileEntity aTileEntity) {
        return null;
    }

    @Override
    public void removeExternals(World world, int aX, int aY, int aZ, Block aBlock, int aMeta,
        TileEntity aTileEntity) {/**/}

    @Override
    public void restoreExternals(World world, int aX, int aY, int aZ, Block aBlock, int aMeta, TileEntity aTileEntity,
        ITransformation aTransformation, NBTBase aNBT) {/**/}
}
