...
public class CompatFR extends CompatBase implements ICompatFR, IFarmable {
    ...
    public void addToBackpacks(String aType, ItemStack aStack) {
        if (MD.FR.mLoaded) try {
            BackpackManager.definitions.get(aType)
                .addValidItem(aStack);
        } catch (Throwable e) {
    ...
    public ICrop getCropAt(World aWorld, int aX, int aY, int aZ) {
        ...
        return aBlock.isWood(aWorld, aX, aY, aZ)
            ? new CropBlock(aWorld, aBlock, aWorld.getBlockMetadata(aX, aY, aZ), new Vect(aX, aY, aZ))
            : null;
    }

    @Override
    public boolean isGermling(ItemStack aStack) {
        return aStack.getItem() instanceof ItemBlockBase
            && ((ItemBlockBase) aStack.getItem()).mPlaceable instanceof BlockBaseSapling;
    }
    ...
    public boolean plantSaplingAt(EntityPlayer aPlayer, ItemStack aSeed, World aWorld, int aX, int aY, int aZ) {
        return aSeed.copy()
            .tryPlaceItemIntoWorld(aPlayer, aWorld, aX, aY - 1, aZ, SIDE_UP, 0, 0, 0);
    }
}
