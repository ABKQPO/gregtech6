...
public class ItemFluidDisplay extends Item implements IFluidContainerItem, IItemUpdatable, IItemGT {
    ...
    public boolean onItemUseFirst(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ,
        ...
        if (!aWorld.isRemote && UT.Entities.hasInfiniteItems(aPlayer)) for (byte tSide : ALL_SIDES_VALID)
            if (FL.fill(WD.te(aWorld, aX, aY, aZ, tSide, T), FL.make(FL.fluid(ST.meta_(aStack)), Integer.MAX_VALUE), T)
                > 0) return T;
    ...
    public void addInformation(ItemStack aStack, EntityPlayer aPlayer, @SuppressWarnings("rawtypes") List aList,
        ...
        if (aFluid == null) {
            ...
            if (FL.exists(FluidsGT.FLUID_RENAMINGS.get(aName)) || FluidsGT.NONSTANDARD.contains(aName))
                    ...
                    if (tMatAmount > 0) {
                        ...
                        aList.add(
                            LH.Chat.BLUE + "Worth: "
                                ...
                                + "."
                                + (tDigits < 1 ? "000"
                                    : tDigits < 10 ? "00" + tDigits : tDigits < 100 ? "0" + tDigits : tDigits)
                                + " Units of "
                                + tMaterial.mMaterial.getLocal());
                    }
                }
            ...
            if (FL.plasma(tFluid)) {
                aList.add(
                    LH.Chat.GREEN + "State: "
                        + LH.Chat.YELLOW
                        + "Plasma"
                        + (!aFluid.isGaseous(tFluid)
                            ? LH.Chat.RED + " (Warning: Considered a Liquid by Mods other than GT!)"
                            : LH.Chat.ORANGE + " (Note: Considered a Gas by Mods other than GT!)"));
            } else if (tGas) {
                aList.add(
                    LH.Chat.GREEN + "State: "
                        + LH.Chat.CYAN
                ...
                aList.add(
                    LH.Chat.GREEN + "State: "
                        + LH.Chat.BLUE
                        + "Liquid"
                        + (tMaterial != null && ST.valid(OP.ingot.mat(tMaterial.mMaterial, 1))
                            ? LH.Chat.CYAN + " (Might able to cast into Molds)"
                            : ""));
                ...
                for (Recipe.RecipeMap tMap : Recipe.RecipeMap.FUEL_MAP_LIST) {
                    ...
                    if (tRecipes != null && !tRecipes.isEmpty()) {
                        ...
                        for (Recipe tRecipe : tRecipes)
                            if (tRecipe.mEnabled && tRecipe.mFluidInputs[0] != null) tFuelValue = Math.max(
                                tFuelValue,
                                (tRecipe.getAbsoluteTotalPower() * U) / tRecipe.mFluidInputs[0].amount);
                        if (tFuelValue > 0) {
                            if (tAmount > 1) {
                                aList.add(
                                    LH.Chat.RED + LH.get(tMap.mNameInternal)
                                        + ": "
                                        + LH.Chat.WHITE
                                        + UT.Code.makeString(tFuelValue / U)
                                        + LH.Chat.YELLOW
                                        + " GU/L; "
                                        + LH.Chat.WHITE
                                        + UT.Code.makeString((tFuelValue * tAmount) / U)
                                        + LH.Chat.YELLOW
                                        + " GU total");
                            } else {
                                aList.add(
    ...
    public void registerIcons(IIconRegister aIconRegister) {
        ...
        if (Abstract_Mod.sFinalized >= Abstract_Mod.sModCountUsingGTAPI) {
            ...
            if (MD.IC2.mLoaded) {
                try {
                    for (gregapi.old.GT_BaseCrop tCrop : gregapi.old.GT_BaseCrop.sCropList)
    ...
    public void updateItemStack(ItemStack aStack) {
        ...
        Fluid tFluid = FL.fluid(ST.meta_(aStack));
        if (tFluid == null) ST.meta_(aStack, W);
        else {
            aStack.setTagCompound(UT.NBT.makeString("f", tFluid.getName()));
        }
    }
