...
public class PrefixItem extends Item implements Runnable, IItemUpdatable, IPrefixItem, IItemGT, IItemNoGTOverride {
    ...
    public void getSubItems(Item var1, CreativeTabs aCreativeTab, @SuppressWarnings("rawtypes") List aList) {
        if ((SHOW_HIDDEN_PREFIXES || !mPrefix.contains(TD.Creative.HIDDEN)))
            for (int i = 0; i < mMaterialList.length; i++) if (mPrefix.isGeneratingItem(mMaterialList[i]))
                if (SHOW_HIDDEN_MATERIALS || !mMaterialList[i].mHidden) {
    ...
    public IIcon getIconIndex(ItemStack aStack) {
        return getIconFromDamageForRenderPass(ST.meta_(aStack), 0);
    }
    ...
    public IIcon getIconFromDamageForRenderPass(int aMetaData, int aRenderPass) {
        if (mPrefix.mIconIndexItem >= 0) {
            if (UT.Code.exists(aMetaData, mMaterialList) && mMaterialList[aMetaData].mTextureSetsItems != null)
                return mMaterialList[aMetaData].mTextureSetsItems.get(mPrefix.mIconIndexItem)
                    .getIcon(aRenderPass);
            return MT.NULL.mTextureSetsItems.get(mPrefix.mIconIndexItem)
                .getIcon(aRenderPass);
        }
        return null;
    }
    ...
    public int getColorFromItemStack(ItemStack aStack, int aRenderPass) {
        if (aRenderPass == 0) {
            ...
            if (UT.Code.exists(aMetaData, mMaterialList))
                return UT.Code.getRGBInt(mMaterialList[aMetaData].mRGBa[mPrefix.mState]);
        }
        return 16777215;
    }
    ...
    public final String getUnlocalizedName(ItemStack aStack) {
        ...
        if (UT.Code.exists(aMetaData, mMaterialList)) return "oredict." + mPrefix.dat(mMaterialList[aMetaData])
            .toString();
    ...
    public ItemStack getContainerItem(ItemStack aStack) {
        ...
        return mContainerItem != null ? ST.amount(1, mContainerItem)
            : mPrefix.mContainerItem != null ? ST.amount(1, mContainerItem = mPrefix.mContainerItem) : null;
    }

    @Override
    public boolean isBeaconPayment(ItemStack aStack) {
        if (mPrefix.mAmount >= U
            && (mPrefix.contains(TD.Prefix.GEM_BASED) || mPrefix.contains(TD.Prefix.INGOT_BASED))) {
            short aMetaData = ST.meta_(aStack);
            return UT.Code.exists(aMetaData, mMaterialList)
                && (mMaterialList[aMetaData].contains(TD.Properties.VALUABLE)
                    || ANY.Iron.mToThis.contains(mMaterialList[aMetaData]));
        }
        return F;
    }
    ...
    public void updateItemStack(ItemStack aStack) {
        ...
        if (UT.Code.exists(aMeta, mMaterialList)) {
            ...
            if (!mPrefix.isGeneratingItem(aMaterial.mTargetRegistration))
                ST.set(aStack, mPrefix.mat(aMaterial.mTargetRegistration, 1), F, F);
        }
    }
    ...
    public int getItemStackLimit(ItemStack aStack) {
        return mPrefix.mDefaultStackSize;
    }

    @Override
    public OreDictItemData getOreDictItemData(ItemStack aStack) {
        return UT.Code.exists(ST.meta_(aStack), mMaterialList)
            ? new OreDictItemData(mPrefix, mMaterialList[ST.meta_(aStack)])
            : null;
    }
     ...
     * if (aMaterial != null && (mPrefix.mAmount >= U || mPrefix.mAmount < 0)) for (TC_AspectStack tAspect :
     * aMaterial.mAspects) tAspect.addToAspectList(rAspects);
