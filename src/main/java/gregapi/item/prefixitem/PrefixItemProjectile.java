...
public class PrefixItemProjectile extends PrefixItem implements IItemProjectile {
    ...
    public PrefixItemProjectile(ModData aMod, String aNameInternal, OreDictPrefix aPrefix, TagData aProjectileType,
        Class<? extends EntityProjectile> aEntityClass, float aSpeedMultiplier, float aPrecision, boolean aDispensable,
        boolean aStabbing, boolean aIsBullet, OreDictMaterial... aMaterialList) {
        this(
            aMod.mID,
            aMod.mID,
            aNameInternal,
    ...
    public PrefixItemProjectile(String aModIDOwner, String aModIDTextures, String aNameInternal, OreDictPrefix aPrefix,
        TagData aProjectileType, Class<? extends EntityProjectile> aEntityClass, float aSpeedMultiplier,
        float aPrecision, boolean aDispensable, boolean aStabbing, boolean aIsBullet,
    ...
    public void addInformation(ItemStack aStack, EntityPlayer aPlayer, List aList, boolean aF3_H) {
        if (mIsBullet) {
            ...
            int tDamage = (int) ((tMat == null ? 1.0 : tMat.getWeight(getPrefix(ST.meta(aStack)).mAmount) / 50.0) * 2.0F
                * TFC_DAMAGE_MULTIPLIER) + 1;
            aList.add(
                LH.Chat.WHITE + "Bullet Damage: "
                    + LH.Chat.RED
                    + tDamage / 2.0F
                    + (TFC_DAMAGE_MULTIPLIER > 1 ? "" : " Hearts"));
        }
    ...
    public boolean hasProjectile(TagData aProjectileType, ItemStack aStack) {
        return (mProjectileType == aProjectileType || aProjectileType == null)
            && UT.Code.exists(ST.meta_(aStack), mMaterialList)
            && mMaterialList[ST.meta_(aStack)] != MT.Empty;
    }
    ...
    public EntityProjectile getProjectile(TagData aProjectileType, ItemStack aStack, World aWorld, double aX, double aY,
        ...
        try {
            EntityProjectile tProjectile = mEntityClass
                .getConstructor(World.class, Double.TYPE, Double.TYPE, Double.TYPE)
                .newInstance(aWorld, aX, aY, aZ);
            tProjectile.setProjectileStack(ST.amount(1, aStack));
            return tProjectile;
        } catch (Throwable e) {
            FMLLog.severe("Problems with '%s'", mEntityClass.getName());
    ...
    public EntityProjectile getProjectile(TagData aProjectileType, ItemStack aStack, World aWorld,
        ...
        if (!hasProjectile(aProjectileType, aStack)) return null;
        try {
            EntityProjectile tProjectile = mEntityClass.getConstructor(World.class, EntityLivingBase.class, Float.TYPE)
                .newInstance(aWorld, aEntity, mSpeedMultiplier * aSpeed);
            tProjectile.setProjectileStack(ST.amount(1, aStack));
            return tProjectile;
        } catch (Throwable e) {
            FMLLog.severe("Problems with '%s'", mEntityClass.getName());
            ...
            if (!tNBT.getBoolean("gt.u")) {
                ...
                for (ObjectStack<Enchantment> tEnchantment : mMaterialList[aMetaData].mEnchantmentAmmo) {
                    UT.NBT.addEnchantment(
                        aStack,
                        tEnchantment.mObject,
                        tEnchantment.mObject == Enchantment.looting ? tEnchantment.mAmount * mLootingMultiplier
    ...
    public ItemStack onDispense(IBlockSource aSource, ItemStack aStack) {
        ...
        EntityProjectile tProjectile = getProjectile(
            mProjectileType,
            aStack,
            aWorld,
            tPosition.getX(),
    ...
    public static class MetaItemDispense extends BehaviorProjectileDispense {
        ...
        public ItemStack dispenseStack(IBlockSource aSource, ItemStack aStack) {
            return ((PrefixItemProjectile) aStack.getItem()).onDispense(aSource, aStack);
        }
