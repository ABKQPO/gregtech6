...
public interface IItemBumbleBee {
    ...
    /**
     * @return The Coordinates of where the Bumbles fly to for this product, or the Coords of the passed Hive, if the
     *         Environment allows the production of this specific Product. This is usually for checking for specific
     *         Flowers for this Product.
     */
    ...
    /**
     * @return The Coordinates of where the Bumbles fly to for this product, or the Coords of the passed Hive, if the
     *         Environment allows the Bumblebee Species to be active at this point in time. This is used for the Main
     *         checks.
     */
    ...
    /**
     * @return 0 = Drone, 1 = Princess, 2 = Queen, 4 = Dead, 5 = Scanned Drone, 6 = Scanned Princess, 7 = Scanned Queen,
     *         9 = Dead Scanned
     */
    ...
    /**
     * @return ItemStack with a mutated variant of this Bumblebee. Used for random Mutations, when two Bumblebees of the
     *         same Species breed.
     */
    public ItemStack bumbleMutate(ItemStack aBumbleBee, short aMetaData, Random aRandom);

    /**
     * This is only called in quarter of all cases for aBumbleBeeA, another quarter goes to aBumbleBeeB, the remaining
     * half goes to selecting the same species as one of the Parents.
    ...
    /**
     * @return if this Bumblebee was able to attack the Target, if it has an effect. You have to implement the damaging
     *         Entity Part fully yourself, since I don't know if you want to damage or heal someone for example.
     */
    ...
    public static class Util {
        ...
        public static ItemStack setBumbleTag(ItemStack aBumbleBee, NBTTagCompound aBumbleTag) {
            UT.NBT.getOrCreate(aBumbleBee)
                .setTag("gt.bumble", aBumbleTag);
            return aBumbleBee;
        }

        public static NBTTagCompound getBumbleGenes(ItemStack aBumblePrincess, ItemStack aBumbleDrone, Random aRandom) {
            NBTTagCompound rBumbleTag = UT.NBT.make(), tBumbleTagA = getBumbleTag(aBumblePrincess),
                tBumbleTagB = getBumbleTag(aBumbleDrone);
            ...
            setDayActive(
                rBumbleTag,
                getDayActive(aRandom.nextBoolean() ? tBumbleTagA : tBumbleTagB) || !getNightActive(rBumbleTag));
            setInsideActive(rBumbleTag, getInsideActive(aRandom.nextBoolean() ? tBumbleTagA : tBumbleTagB));
            setOutsideActive(
                rBumbleTag,
                getOutsideActive(aRandom.nextBoolean() ? tBumbleTagA : tBumbleTagB) || !getInsideActive(rBumbleTag));
            return rBumbleTag;
        }

        public static NBTTagCompound getBumbleGenes(Random aRandom) {
            return getBumbleGenes(WD.envTemp(BiomeGenBase.plains), BiomeGenBase.plains, T, aRandom);
        }

        public static NBTTagCompound getBumbleGenes(long aTemperature, BiomeGenBase aBiome, boolean aHasSky,
            Random aRandom) {
            return getBumbleGenes(
                aTemperature,
                aBiome,
                aHasSky,
                !(BIOMES_DESERT.contains(aBiome.biomeName) || BIOMES_MESA.contains(aBiome.biomeName)),
                BIOMES_DESERT.contains(aBiome.biomeName) || BIOMES_MESA.contains(aBiome.biomeName),
                aRandom);
        }

        public static NBTTagCompound getBumbleGenes(long aTemperature, BiomeGenBase aBiome, boolean aHasSky,
            boolean aDay, boolean aNight, Random aRandom) {
        ...
        public static void setHumidityMin(NBTTagCompound aBumbleTag, float aHumidity) {
            aBumbleTag.setFloat("minhum", aHumidity < 0.01F ? 0 : aHumidity);
        }
        ...
        public static void setAggressiveness(NBTTagCompound aBumbleTag, long aAggressiveness) {
            UT.NBT.setNumber(aBumbleTag, "aggro", UT.Code.bind(1, 10000, aAggressiveness));
        }

        public static void setWorkForce(NBTTagCompound aBumbleTag, long aWorkForce) {
            UT.NBT.setNumber(aBumbleTag, "work", UT.Code.bind(1, 10000, aWorkForce));
        }

        public static void setLifeSpan(NBTTagCompound aBumbleTag, long aLifeSpan) {
            UT.NBT.setNumber(aBumbleTag, "life", UT.Code.bind(1200, 144000, aLifeSpan));
        }
        ...
        public static float getHumidityMin(NBTTagCompound aBumbleTag) {
            return Math.max(0, aBumbleTag.getFloat("minhum"));
        }
        ...
        public static long getAggressiveness(NBTTagCompound aBumbleTag) {
            return UT.Code.bind(100, 10000, aBumbleTag.getLong("aggro"));
        }

        public static long getWorkForce(NBTTagCompound aBumbleTag) {
            return UT.Code.bind(1, 10000, aBumbleTag.getLong("work"));
        }

        public static long getLifeSpan(NBTTagCompound aBumbleTag) {
            return UT.Code.bind(1200, 144000, aBumbleTag.getLong("life"));
        }
