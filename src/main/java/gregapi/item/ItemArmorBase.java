...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "ic2.api.item.IMetalArmor", modid = ModIDs.IC2),
        @Optional.Interface(iface = "forestry.api.apiculture.IArmorApiarist", modid = ModIDs.FR) })
public class ItemArmorBase extends ItemArmor
    implements IItemUpdatable, IItemGT, IItemNoGTOverride, ISpecialArmor, IMetalArmor, IArmorApiarist {
     ...
     */
    public ItemArmorBase(String aModID, String aUnlocalized, String aEnglish, String aEnglishTooltip, String aArmorName,
        int aSlot, int[] aShields, int aDurability, int aDamageReduction, int aEnchantability, boolean aMetalArmor,
        boolean aBeeArmor, Object... aRecipe) {
        super(
            EnumHelper.addArmorMaterial("armor." + aUnlocalized, aDurability, aShields, aEnchantability),
            GT_API.api_proxy.addArmor(TEX_DIR_ARMOR + aArmorName),
            aSlot);
        ...
        if (UT.Code.stringValid(aEnglishTooltip)) LH.add(mTooltip = mName + ".tooltip_main", aEnglishTooltip);
        else mTooltip = null;
    ...
    @Override
    public ArmorProperties getProperties(EntityLivingBase aPlayer, ItemStack aStack, DamageSource aSource,
        double aDamage, int aSlot) {
        return aSource.isUnblockable() ? new ArmorProperties(0, 0, 0)
            : new ArmorProperties(0, damageReduceAmount / 25.0, getMaxDamage() + 1 - aStack.getItemDamage());
    }

    @Override
    public int getArmorDisplay(EntityPlayer aPlayer, ItemStack aStack, int aSlot) {
        return getArmorMaterial().getDamageReductionAmount(aSlot);
    }
    ...
    public String getUnlocalizedName(ItemStack aStack) {
        return getHasSubtypes() ? mName + "." + ST.meta_(aStack) : mName;
    }
    ...
    public void registerIcons(IIconRegister aIconRegister) {
        mIcon = aIconRegister.registerIcon(mModID + ":" + "armor/" + mArmorName + "/" + armorType);
    }
