...
public interface IItemEnergy {
    ...
    public static class Utility {
        ...
        public static long inject(TagData aEnergyType, ItemStack aStack, long aSize, long aAmount,
            IInventory aInventory, World aWorld, int aX, int aY, int aZ, boolean aDoInject) {
            if (ST.invalid(aStack)) return 0;
            if (aStack.getItem() instanceof IItemEnergy) return ((IItemEnergy) aStack.getItem())
                .doEnergyInjection(aEnergyType, aStack, aSize, aAmount, aInventory, aWorld, aX, aY, aZ, aDoInject);
            if (aEnergyType == TD.Energy.EU && COMPAT_EU_ITEM != null
                && COMPAT_EU_ITEM.is(aStack)
                && !IL.IC2_EnergyCrystal.equal(aStack, T, T)
                && !IL.IC2_LapotronCrystal.equal(aStack, T, T)
                && COMPAT_EU_ITEM.insidevolt(aStack, UT.Code.voltMin(aSize), UT.Code.voltMax(aSize))
                && COMPAT_EU_ITEM.charge(aStack, aSize, aDoInject) > 0) return 1;
            return 0;
        }

        public static long extract(TagData aEnergyType, ItemStack aStack, long aSize, long aAmount,
            IInventory aInventory, World aWorld, int aX, int aY, int aZ, boolean aDoExtract) {
            if (ST.invalid(aStack)) return 0;
            if (aStack.getItem() instanceof IItemEnergy) return ((IItemEnergy) aStack.getItem())
            ...
            if (aEnergyType == TD.Energy.EU && COMPAT_EU_ITEM != null
                ...
                && COMPAT_EU_ITEM.insidevolt(aStack, UT.Code.voltMin(aSize), UT.Code.voltMax(aSize))
                && COMPAT_EU_ITEM.decharge(aStack, aSize, aDoExtract) >= aSize) return 1;
            return 0;
        }
        ...
        public static boolean full(ItemStack aStack, boolean aDefault) {
            ...
            if (aStack.getItem() instanceof IItemEnergy)
                for (TagData tEnergyType : ((IItemEnergy) aStack.getItem()).getEnergyTypes(aStack))
                    if (((IItemEnergy) aStack.getItem()).getEnergyStored(tEnergyType, aStack)
                        < ((IItemEnergy) aStack.getItem()).getEnergyCapacity(tEnergyType, aStack)) return F;
            if (COMPAT_EU_ITEM != null && COMPAT_EU_ITEM.is(aStack)
                && COMPAT_EU_ITEM.stored(aStack) < COMPAT_EU_ITEM.capacity(aStack)) return F;
            return aDefault;
        }
        ...
        public static boolean empty(ItemStack aStack, boolean aDefault) {
            ...
            if (aStack.getItem() instanceof IItemEnergy)
                for (TagData tEnergyType : ((IItemEnergy) aStack.getItem()).getEnergyTypes(aStack))
                    if (((IItemEnergy) aStack.getItem()).getEnergyStored(tEnergyType, aStack) > 0) return F;
