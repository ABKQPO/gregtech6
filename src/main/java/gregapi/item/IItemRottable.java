    ...
    public static class RottingUtil {
        ...
        public static ItemStack rotting(ItemStack aStack) {
            if (aStack.getItem() == Items.milk_bucket)
                return IL.ENVM_Spoiled_Milk_Bucket.exists() ? IL.ENVM_Spoiled_Milk_Bucket.get(aStack.stackSize)
                    : ST.make(Items.bucket, aStack.stackSize, 0);
            ...
            if (aStack.getItem() instanceof IFluidContainerItem)
                return rotting(aStack, (IFluidContainerItem) aStack.getItem());
            return aStack;
        }
        ...
        public static ItemStack rotting(ItemStack aStack, IFluidContainerItem aItem) {
            ...
            if (tFluid != null) {
                if (FL.Milk_Spoiled.is(tFluid) || FL.Rotten_Drink.is(tFluid)
                    || FL.Dirty_Water.is(tFluid)
                    || FL.Swampwater.is(tFluid)
                    || FL.Stagnant_Water.is(tFluid)) {
                    //
                } else if (FluidsGT.WATER.contains(
                    tFluid.getFluid()
                        .getName())) {
                            aItem.drain(aStack, Integer.MAX_VALUE, T);
                            aItem.fill(aStack, FL.Dirty_Water.make(tFluid.amount), T);
                        } else
                    if (FluidsGT.MILK.contains(
                        tFluid.getFluid()
                            .getName())) {
                                aItem.drain(aStack, Integer.MAX_VALUE, T);
                                aItem.fill(aStack, FL.Milk_Spoiled.make(tFluid.amount), T);
                            } else
                        if (FluidsGT.FOOD.contains(
                            tFluid.getFluid()
                                .getName())) {
                                    aItem.drain(aStack, Integer.MAX_VALUE, T);
