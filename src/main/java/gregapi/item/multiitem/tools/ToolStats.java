...
public abstract class ToolStats implements IToolStats {
        ...
        return DamageSources.getCombatDamage(
            ...
            aEntity instanceof EntityLivingBase
                ? getDeathMessage(
                    ...
                    aPlayer == null ? "Someone" : UT.Code.stringValidate(aPlayer.getCommandSenderName(), "Someone"),
                    UT.Code.stringValidate(aEntity.getCommandSenderName(), "Someone"))
                : null,
            canBehead());
    }
    ...
    public IChatComponent getDeathMessage(EntityLivingBase aPlayer, EntityLivingBase aEntity, String aNamePlayer,
        String aNameEntity) {
        return DamageSources.getDeathMessage(aPlayer, aEntity, aNamePlayer, aNameEntity, getDeathMessage());
    }
    ...
    public boolean harvestGrass(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        long aAvailableDurability, int aX, int aY, int aZ, byte aMetaData, int aFortune, boolean aSilkTouch,
        BlockEvent.HarvestDropsEvent aEvent) {
        ...
        if (aBlock == Blocks.double_plant) {
            switch (aMetaData & 7) {
                case 2:
                case 3:
                    aDrops.add(IL.Grass.get(2 + RNGSUS.nextInt(1 + aFortune) + RNGSUS.nextInt(1 + aFortune)));
                    return T;
            }
            return F;
        }
        ...
        if (MD.BoP.mLoaded) {
            if (aBlock == ST.block(MD.BoP, "foliage")) {
                switch (aMetaData) {
                    case 1:
                        if (RNGSUS.nextInt(4) <= aFortune) aDrops.add(IL.Grass.get(1));
                        return T;
                    case 2:
                        if (RNGSUS.nextInt(2) <= aFortune) aDrops.add(IL.Grass.get(1));
                        return T;
                    case 10:
                        aDrops.add(IL.Grass.get(1 + RNGSUS.nextInt(1 + aFortune)));
                        return T;
    ...
    public boolean harvestStick(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        long aAvailableDurability, int aX, int aY, int aZ, byte aMetaData, int aFortune, boolean aSilkTouch,
        BlockEvent.HarvestDropsEvent aEvent) {
        if (aBlock == Blocks.tallgrass) {
            switch (aMetaData) {
        ...
        if (IL.TF_Tall_Grass.equal(aBlock)) {
            switch (aMetaData) {
                case 11:
                    aDrops.add(IL.Stick.get(1 + RNGSUS.nextInt(2 + aFortune)));
                    return T;
            }
            return F;
        }
    ...
    public ItemStack getBrokenItem(ItemStack aStack) {
        return mMaterialAmount < U4 ? null
            : OP.scrapGt
                .mat(MultiItemTool.getPrimaryMaterial(aStack), 1 + RNGSUS.nextInt(1 + (int) (4 * mMaterialAmount / U)));
    }
    ...
    public void afterDealingDamage(float aNormalDamage, float aMagicDamage, int aFireAspect, boolean aCriticalHit,
        ...
        int tKnockback = (aPlayer.isSprinting() ? 1 : 0) + (aEntity instanceof EntityLivingBase
            ? EnchantmentHelper.getKnockbackModifier(aPlayer, (EntityLivingBase) aEntity)
            : 0);
        if (tKnockback > 0) {
            aEntity.addVelocity(
                -MathHelper.sin((float) (aPlayer.rotationYaw * Math.PI / 180)) * tKnockback * 0.5,
                0.1,
                MathHelper.cos((float) (aPlayer.rotationYaw * Math.PI / 180)) * tKnockback * 0.5);
            aPlayer.motionX *= 0.6;
            aPlayer.motionZ *= 0.6;
        ...
        if (aEntity instanceof EntityLivingBase)
            aPlayer.addStat(StatList.damageDealtStat, Math.round((aNormalDamage + aMagicDamage) * 10));
    ...
    public IIcon getIcon(ItemStack aStack, int aRenderPass) {
        switch (aRenderPass) {
            case 0:
                return getIcon(F, aStack).getIcon(0);
            case 1:
                return getIcon(F, aStack).getIcon(1);
            case 2:
                return getIcon(T, aStack).getIcon(0);
            case 3:
                return getIcon(T, aStack).getIcon(1);
        }
        return null;
    }
    ...
    public short[] getRGBa(ItemStack aStack, int aRenderPass) {
        switch (aRenderPass) {
            case 0:
                return getRGBa(F, aStack);
            case 1:
                return UNCOLOURED;
            case 2:
                return getRGBa(T, aStack);
            case 3:
                return UNCOLOURED;
        }
        return UNCOLOURED;
