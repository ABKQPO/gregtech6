...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "thaumcraft.api.IWarpingGear", modid = ModIDs.TC),
...
public class MultiItemToolWithCompat extends MultiItemTool implements IWarpingGear, IToolGrafter, IToolCrowbar,
    IToolWrench, IBoxable, ISpecialElectricItem, IElectricItemManager, IItemElectric {
    ...
    public int getWarp(ItemStack aStack, EntityPlayer aPlayer) {
        return getPrimaryMaterial(aStack).contains(TD.Properties.WARPING)
            || getSecondaryMaterial(aStack).contains(TD.Properties.WARPING) ? 1 : 0;
    }
    ...
    public float getSaplingModifier(ItemStack aStack, World aWorld, EntityPlayer aPlayer, int aX, int aY, int aZ) {
        ...
        return tStats != null && tStats.isGrafter()
            ? Math.min(100.0F, (1 + UT.Code.bind4(getHarvestLevel(aStack, ""))) * 20.0F)
            : 0.0F;
    }
