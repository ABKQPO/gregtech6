...
public class MultiItemTool extends MultiItem implements IItemGTHandTool, IItemGTContainerTool {
    ...
    public MultiItemTool(String aModID, String aUnlocalized) {
         ...
         * UT.Reflection.callPublicMethod(Class.forName("mods.battlegear2.api.weapons.WeaponRegistry"), "addTwoHanded",
         * make(0));
         * UT.Reflection.callPublicMethod(Class.forName("mods.battlegear2.api.weapons.WeaponRegistry"), "addTwoHanded",
         * make(W));
         * UT.Reflection.callPublicMethod(Class.forName("mods.battlegear2.api.weapons.WeaponRegistry"), "setWeapon",
         * "MainHand", make(0));
         * UT.Reflection.callPublicMethod(Class.forName("mods.battlegear2.api.weapons.WeaponRegistry"), "setWeapon",
         * "MainHand", make(W));
         * } catch(Throwable e) {
     ...
     * 
     * @param aID               The Id of the assigned Tool Class [0 - 32765] (only even Numbers allowed! Uneven ID's
     *                          are empty electric Items)
    ...
    public final ItemStack addTool(int aID, String aEnglish, String aToolTip, IToolStats aToolStats,
        ...
        if (aID >= 0 && aID < 32766 && isUsableMeta((short) aID)) {
            ...
            for (Object tRandomParameter : aRandomParameters) {
                ...
                else OM.reg(tRandomParameter, rStack);
            }
    ...
    public final ItemStack getToolWithStats(int aToolID, OreDictMaterial aPrimaryMaterial,
        OreDictMaterial aSecondaryMaterial) {
     ...
     * @param aSecondaryMaterial Secondary (Rod/Handle) Material of this Tool
     */
    public final ItemStack getToolWithStats(int aToolID, int aAmount, OreDictMaterial aPrimaryMaterial,
        OreDictMaterial aSecondaryMaterial) {
     ...
     */
    public final ItemStack getToolWithStats(int aToolID, int aAmount, OreDictMaterial aPrimaryMaterial,
        OreDictMaterial aSecondaryMaterial, long aMaxCharge, long aVoltage) {
     ...
     */
    public final ItemStack getToolWithStats(int aToolID, int aAmount, OreDictMaterial aPrimaryMaterial,
        OreDictMaterial aSecondaryMaterial, long aMaxCharge, long aVoltage, long aCharge) {
        ...
        if (tToolStats != null) {
            ...
            if (aPrimaryMaterial != null) {
                ...
                UT.NBT.setNumber(
                    tToolNBT,
                    "j",
                    (long) ((aPrimaryMaterial.mToolDurability * 100L) * tToolStats.getMaxDurabilityMultiplier()));
            }
            if (aSecondaryMaterial != null) {
                if (aSecondaryMaterial.mID > 0) tToolNBT.setShort("c", aSecondaryMaterial.mID);
                else tToolNBT.setString("d", aSecondaryMaterial.toString());
            }
            if (aMaxCharge > 0) {
            ...
            if (aCharge > 0 && aMaxCharge > 0) for (TagData tEnergyType : getEnergyTypes(rStack))
                setEnergyStored(tEnergyType, rStack, Math.min(aCharge, aMaxCharge));
        }
    ...
    public void onHarvestBlockEvent(ArrayList<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        int aX, int aY, int aZ, byte aMeta, int aFortune, boolean aSilkTouch, BlockEvent.HarvestDropsEvent aEvent) {
        IToolStats tStats = getToolStats(aStack);
        if (tStats == null || ST.instaharvest(aBlock, aMeta)
            || !isItemStackUsable(aStack)
            || getDigSpeed(aStack, aBlock, aMeta) <= 0) {
            doDamage(aStack, 0, aPlayer, T);
            return;
        }
        long tDamage = tStats.convertBlockDrops(
            aDrops,
            aStack,
            aPlayer,
            aBlock,
            (getToolMaxDamage(aStack) - getToolDamage(aStack)) / tStats.getToolDamagePerDropConversion(),
            aX,
            aY,
            aZ,
            aMeta,
        ...
        if (WD.dimBTL(aPlayer.worldObj) && !getPrimaryMaterial(aStack).contains(TD.Properties.BETWEENLANDS))
            tDamage *= 4;
    ...
    public boolean canCollectDropsDirectly(ItemStack aStack) {
        IToolStats tStats = getToolStats(aStack);
        return (tStats.canCollect() || getPrimaryMaterial(aStack).contains(TD.Properties.AUTO_COLLECTING)
            || getSecondaryMaterial(aStack).contains(TD.Properties.AUTO_COLLECTING)) && isItemStackUsable(aStack);
    }
    ...
    public float onBlockBreakSpeedEvent(float aDefault, ItemStack aStack, EntityPlayer aPlayer, Block aBlock, int aX,
        int aY, int aZ, byte aMeta, PlayerEvent.BreakSpeed aEvent) {
        ...
        if (OD.obsidian.is(ST.make(aBlock, 1, aMeta)))
            aDefault *= Math.max(1, getPrimaryMaterial(aStack).mToolQuality - 2);
        // and now the basic Tool Stats.
        IToolStats tStats = getToolStats(aStack);
        return tStats == null ? aDefault
            : tStats.getMiningSpeed(aBlock, aMeta, aDefault, aPlayer, aPlayer.worldObj, aX, aY, aZ);
    }
    ...
    public boolean onLeftClickEntity(ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        ...
        if (aEntity.canAttackWithItem()) {
            ...
            if (aEntity.hitByEntity(aPlayer)) {
                ...
                if (tImplosion > 0 && UT.Entities.isExplosiveCreature(aEntity)
                    && !EntityCreeper.class.isInstance(aEntity)) tMagicDamage += 1.5F * tImplosion;

                if (tDamage + tMagicDamage > 0) {
                    boolean tRealHit = (!aEntity.worldObj.isRemote || aEntity.hurtResistantTime <= 0);
                    boolean tCriticalHit = aPlayer.fallDistance > 0 && !aPlayer.onGround
                        && !aPlayer.isOnLadder()
                        && !aPlayer.isInWater()
                    ...
                    if (MD.BTL.mLoaded && aEntity.getClass()
                        .getName()
                        .startsWith("thebetweenlands")
                        && getPrimaryMaterial(aStack).contains(TD.Properties.BETWEENLANDS)) {
                        float tDamageToDeal = tFullDamage;
                        while (tDamageToDeal > 0
                            && aEntity.attackEntityFrom(tSource, Math.min(tDamageToDeal, 12) / 0.3F)) {
                            tDamageToDeal -= 12;
    ...
    public void addAdditionalToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (tMaxDamage > 0 && tStats != null) {
            if (tMat1 == MT.NULL) {
                ...
                aList.add(
                    LH.Chat.WHITE + "Melee Damage: +"
                        + LH.Chat.BLUE
                        + (tCombat * TFC_DAMAGE_MULTIPLIER)
                        + LH.Chat.RED
                        + " (= "
                        + (TFC_DAMAGE_MULTIPLIER > 1 ? ((tCombat + 1) * (TFC_DAMAGE_MULTIPLIER / 2.0)) + ")"
                            : ((tCombat + 1) / 2) + " Hearts)"));
                ...
                aList.add(
                    LH.Chat.WHITE + "Durability: "
                        + LH.Chat.GREEN
                        + UT.Code.makeString(tMaxDamage - tDamage)
                        + " / "
                        + UT.Code.makeString(tMaxDamage));
                aList.add(
                    LH.Chat.WHITE + tMat1.getLocal()
                        + LH.Chat.YELLOW
                        + " Level: "
                        + (tStats.getBaseQuality() + tMat1.mToolQuality));
                float tCombat = getToolCombatDamage(aStack);
                aList.add(
                    LH.Chat.WHITE + "Melee Damage: "
                        + LH.Chat.BLUE
                        ...
                        + (TFC_DAMAGE_MULTIPLIER > 1 ? ((tCombat + 1) * (TFC_DAMAGE_MULTIPLIER / 2.0)) + ")"
                            : ((tCombat + 1) / 2) + " Hearts)"));
                aList.add(
                    LH.Chat.WHITE + "Mining Speed: "
                        + LH.Chat.PINK
                        + Math.max(Float.MIN_NORMAL, tStats.getSpeedMultiplier() * tMat1.mToolSpeed));
                aList.add(
                    LH.Chat.WHITE + "Crafting Uses: "
                        + LH.Chat.GREEN
                        + UT.Code.divup(
                            getEnergyStats(aStack) == null ? tMaxDamage - tDamage
                                : Math.min(
                                    getEnergyStored(TD.Energy.EU, aStack),
                                    getEnergyCapacity(TD.Energy.EU, aStack)),
                            tStats.getToolDamagePerContainerCraft()));
                if (MD.BTL.mLoaded && tMat1.contains(TD.Properties.BETWEENLANDS))
                    aList.add(LH.Chat.GREEN + LH.get(LH.TOOLTIP_BETWEENLANDS_RESISTANCE));
                if (MD.TF.mLoaded && tMat1.contains(TD.Properties.MAZEBREAKER)) {
                    if (canHarvestBlock(IL.TF_Mazestone.block(), aStack))
                        aList.add(LH.Chat.PINK + LH.get(LH.TOOLTIP_TWILIGHT_MAZE_STONE_BREAKING));
                    if (canHarvestBlock(IL.TF_Mazehedge.block(), aStack))
                        aList.add(LH.Chat.PINK + LH.get(LH.TOOLTIP_TWILIGHT_MAZE_HEDGE_BREAKING));
                    if (canHarvestBlock(IL.TF_Towerwood.block(), aStack))
                        aList.add(LH.Chat.PINK + LH.get(LH.TOOLTIP_TWILIGHT_TOWER_WOOD_BREAKING));
                }
                if (tMat1.contains(TD.Properties.UNBURNABLE) || tMat2.contains(TD.Properties.UNBURNABLE))
                    aList.add(LH.Chat.GREEN + LH.get(LH.TOOLTIP_UNBURNABLE));
                if (tStats.canCollect() || tMat1.contains(TD.Properties.AUTO_COLLECTING)
                    || tMat2.contains(TD.Properties.AUTO_COLLECTING))
                    aList.add(LH.Chat.DGRAY + LH.get(LH.TOOLTIP_AUTOCOLLECT));
                if (tStats.canPenetrate()) aList.add(LH.Chat.DGRAY + LH.get(LH.TOOLTIP_ARMOR_PENETRATING));
            }
        }
    }
    ...
    public IItemEnergy getEnergyStats(ItemStack aStack) {
        ...
        if (aNBT != null) {
            aNBT = aNBT.getCompoundTag("GT.ToolStats");
            if (aNBT != null) {
                if (aNBT.getBoolean("e")) return EnergyStat.makeTool(
                    TD.Energy.EU,
                    aNBT.getLong("f"),
                    aNBT.getLong("g"),
                    64,
                    ST.make(this, 1, getUnusableMeta(aStack)),
                    ST.make(this, 1, getUsableMeta(aStack)),
                    ST.make(this, 1, getUsableMeta(aStack)));
            }
        }
        return null;
            ...
            if (aAllowBreaking && tNewDamage >= getToolMaxDamage(aStack)) {
                ...
                if (tStats == null) {
                    ...
                    if (TOOL_SOUNDS) {
                        if (aPlayer != null) {
                            UT.Sounds.send(
                                getPrimaryMaterial(aStack) == MT.NULL ? tStats.getCraftingSound()
                                    : tStats.getBreakingSound(),
                                aPlayer);
                        } else {
                            UT.Sounds.play(
                                getPrimaryMaterial(aStack) == MT.NULL ? tStats.getCraftingSound()
                                ...
                                100,
                                1,
                                LAST_TOOL_COORDS_BEFORE_DAMAGE);
                        }
                    }
    ...
    public float getDigSpeed(ItemStack aStack, Block aBlock, int aMeta) {
        ...
        // Required because a combination of Twilight Forest and Block Metadata Extenders can fuck this up and give me
        // values like 49 for vanilla Blocks.
        ...
        if ((IL.TF_Mazestone.equal(aBlock) || IL.TF_Mazehedge.equal(aBlock) || IL.TF_Towerwood.equal(aBlock))
            && tMaterial.contains(TD.Properties.MAZEBREAKER)) tMultiplier *= 40;
        IToolStats tStats = getToolStats(aStack);
        if (tStats == null
            || tStats.getBaseQuality() + tMaterial.mToolQuality < UT.Code.bind4(aBlock.getHarvestLevel(aMeta)))
            return 0;
        return tStats.getMiningSpeed(aBlock, (byte) aMeta)
            * Math.max(Float.MIN_NORMAL, tStats.getSpeedMultiplier() * tMultiplier * tMaterial.mToolSpeed);
    }
    ...
    public boolean onBlockDestroyed(ItemStack aStack, World aWorld, Block aBlock, int aX, int aY, int aZ,
        ...
        if (MD.TFC.owns(aRegName) || MD.TFCP.owns(aRegName)) {
            ...
            if (IL.TF_Mazestone.equal(aBlock)) if (aMat1.contains(TD.Properties.MAZEBREAKER)) tDamage /= 40;
            else tDamage *= 16;
            if (IL.TF_Mazehedge.equal(aBlock)) {
                if (aMat1.contains(TD.Properties.MAZEBREAKER)) tDamage /= 40;
                else tDamage *= 16;
    ...
    public boolean isItemStackUsable(ItemStack aStack) {
        ...
        if (tStats.isMiningTool()) for (ObjectStack<Enchantment> tEnchantment : aMaterial.mEnchantmentTools)
            tEnchantments.add(new ObjectStack<>(tEnchantment.mObject, tEnchantment.mAmount));
        if (tStats.isWeapon()) for (ObjectStack<Enchantment> tEnchantment : aMaterial.mEnchantmentWeapons)
            tEnchantments.add(new ObjectStack<>(tEnchantment.mObject, tEnchantment.mAmount));
        if (tStats.isRangedWeapon()) for (ObjectStack<Enchantment> tEnchantment : aMaterial.mEnchantmentRanged)
            tEnchantments.add(new ObjectStack<>(tEnchantment.mObject, tEnchantment.mAmount));
        ...
        for (ObjectStack<Enchantment> tEnchantment : tEnchantments)
            UT.NBT.addEnchantment(aStack, tEnchantment.mObject, tEnchantment.amountShort());
        return T;
    }
    ...
    public IIcon getIcon(ItemStack aStack, int aRenderPass, EntityPlayer aPlayer, ItemStack aUsedStack,
        ...
        if (aPlayer == null) {
            ...
            if (aRenderPass == 0) {
                ...
                return Textures.ItemIcons.DURABILITY_BAR[(int) Math
                    .max(0, Math.min(7, ((tMaxDamage - tDamage) * 8L) / tMaxDamage))].getIcon(0);
            }
            if (aRenderPass == 1) {
                IItemEnergy tElectric = getEnergyStats(aStack);
                if (tElectric != null) {
                    long tStored = tElectric.getEnergyStored(TD.Energy.EU, aStack),
                        tCapacity = tElectric.getEnergyCapacity(TD.Energy.EU, aStack);
                    ...
                    return Textures.ItemIcons.ENERGY_BAR[7
                        - (int) Math.max(0, Math.min(6, ((tCapacity - tStored) * 7L) / tCapacity))].getIcon(0);
                }
            }
        }
