...
public abstract class MultiItemRandom extends MultiItem implements Runnable {
     ...
     * This gets called after all the PrefixItems and PrefixBlocks have been registered to the OreDict, what is during
     * the @Init-Phase of the regular API.
     */
    ...
    public ItemStack onItemRightClick(ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        ...
        if (tStat != null && (UT.Entities.isCreative(aPlayer) || aPlayer.getFoodStats()
            .needFood() || tStat.alwaysEdible(this, aStack, aPlayer)))
    ...
    public ItemStack last(int aAmount) {
        return ST.make(this, aAmount, mLastID);
    }
    ...
    public ItemStack prev(int aAmount) {
        return ST.make(this, aAmount, mLastID - 1);
    }
            ...
            for (Object tRandomData : aRandomData) if (tRandomData != null) {
                ...
                if (tRandomData instanceof IFoodStat) {
                    ...
                    if (IL.IC2_Food_Can_Empty.exists() && IL.IC2_Food_Can_Filled.exists()
                        && getContainerItem(aStack) == null) {
                        int tFoodValue = ((IFoodStat) tRandomData).getFoodLevel(this, aStack, null);
                        if (tFoodValue > 0) RM.Canner.addRecipe2(
                            T,
                            16,
                            tFoodValue * 16,
                            aStack,
                            IL.IC2_Food_Can_Empty.get(tFoodValue),
                            ((IFoodStat) tRandomData).isRotten(this, aStack, null)
                                ? IL.IC2_Food_Can_Spoiled.get(tFoodValue, IL.IC2_Food_Can_Filled.get(tFoodValue))
                                : IL.IC2_Food_Can_Filled.get(tFoodValue));
                    }
                    tUseOreDict = F;
                }
                    ...
                    if (((OreDictItemData) tRandomData).validData()) {
                        ...
                        if (ST.invalid(tStack) || tStack.getItem() instanceof PrefixItem) {
                            OreDictManager.INSTANCE.setTarget_(
                                ((OreDictItemData) tRandomData).mPrefix,
                                ((OreDictItemData) tRandomData).mMaterial.mMaterial,
                                aStack);
                            continue;
                        }
                    }
                ...
                if (tRandomData instanceof FluidStack) {
                    tRandomData = new FluidContainerData(
                        (FluidStack) tRandomData,
                        ST.copy_(aStack),
                        getContainerItem(aStack),
                        T);
                    // if (((FluidContainerData)tRandomData).emptyContainer != null)
                    // RM.Canner.addRecipe1(T, 16, Math.max(((FluidContainerData)tRandomData).fluid.amount / 64, 16),
                    // ((FluidContainerData)tRandomData).emptyContainer, ((FluidContainerData)tRandomData).fluid, NF,
                    // ((FluidContainerData)tRandomData).filledContainer);
                    // RM.Canner.addRecipe1(T, 16, Math.max(((FluidContainerData)tRandomData).fluid.amount / 64, 16),
                    // ((FluidContainerData)tRandomData).filledContainer, NF, ((FluidContainerData)tRandomData).fluid,
                    // ST.container(((FluidContainerData)tRandomData).filledContainer, F));
    ...
    public MultiItemRandom setElectricStats(int aMetaValue, IItemEnergy aStats) {
        if (aMetaValue < 0 || aMetaValue >= W) return this;
        if (aStats == null) mElectricStats.remove((short) aMetaValue);
        else {
            mElectricStats.put((short) aMetaValue, aStats);
            if (!(aStats instanceof EnergyStatDebug))
                mIconList[aMetaValue] = Arrays.copyOf(mIconList[aMetaValue], Math.max(9, mIconList[aMetaValue].length));
        }
        return this;
    }
                    ...
                    aPlayer.getFoodStats()
                        .func_151686_a(
                            (ItemFood) UT.Reflection
                                .callConstructor("squeek.applecore.api.food.ItemFoodProxy", 0, null, T, this),
                            aStack);
                } else {
                    aPlayer.getFoodStats()
                        .addStats(tFoodLevel, tSaturationLevel);
                }
            }

            if (!aWorld.isRemote && MD.ENVM.mLoaded) {
                try {
                    float tTemperature = tStat.getTemperature(this, aStack, aPlayer) - C,
                        tHydration = tStat.getHydration(this, aStack, aPlayer);
                    Object tTracker = EM_StatusManager.lookupTracker(aPlayer);
                    if (tTracker != null && ((EnviroDataTracker) tTracker).bodyTemp >= 0) {
                        ((EnviroDataTracker) tTracker).bodyTemp += (tTemperature
                            - ((EnviroDataTracker) tTracker).bodyTemp)
                            * tStat.getTemperatureEffect(this, aStack, aPlayer);
    ...
    public void getSubItems(Item aItem, CreativeTabs aCreativeTab, @SuppressWarnings("rawtypes") List aList) {
        if (aItem == this) for (int i = 0, j = mEnabledItems.length(); i < j; i++)
            if (mVisibleItems.get(i) || (SHOW_HIDDEN_ITEMS && mEnabledItems.get(i))) {
    ...
    public void registerIcons(IIconRegister aIconRegister) {
        for (short aMeta = 0, tMaxMeta = (short) mEnabledItems.length(); aMeta < tMaxMeta; aMeta++)
            if (mEnabledItems.get(aMeta)) {
                for (byte k = 1; k < mIconList[aMeta].length; k++) {
                    mIconList[aMeta][k] = aIconRegister
                        .registerIcon(mModID + ":" + getUnlocalizedName() + "/" + aMeta + "/" + k);
                }
    ...
    public IIcon getIconIndex(ItemStack aStack) {
        ...
        if (tStats != null && mIconList[aMetaData].length > 1) {
            TagData tEnergyType = tStats.getEnergyTypes(aStack)
                .iterator()
                .next();
            long tStored = tStats.getEnergyStored(tEnergyType, aStack),
                tCapacity = tStats.getEnergyCapacity(tEnergyType, aStack);
    ...
    public IIcon getIconFromDamage(int aMetaData) {
        return UT.Code.exists(aMetaData, mIconList) ? mIconList[aMetaData][0]
        ...
        if (OM.is(OD_USB_STICKS[2], aStack)) {
            ...
            aStack.getTagCompound()
                .setTag(
                    NBT_USB_DATA,
                    UT.NBT.makeString(
                        UT.NBT.makeString(NBT_REACTOR_SETUP_NAME, "" + aSetup.hashCode()),
                        NBT_REACTOR_SETUP,
                        aSetup));
            aStack.getTagCompound()
                .setByte(NBT_USB_TIER, (byte) 2);
            return T;
        }
        return F;
    }
    ...
    public void setPlanName(ItemStack aStack, String aName) {
        aStack.getTagCompound()
            ...
            .setString(NBT_REACTOR_SETUP_NAME, aName);
    }

    public boolean hasSetup(ItemStack aStack) {
        return OM.is(OD_USB_STICKS[2], aStack) && aStack.hasTagCompound()
            && aStack.getTagCompound()
                .getCompoundTag(NBT_USB_DATA)
                .hasKey(NBT_REACTOR_SETUP);
    }

    public String getSetup(ItemStack aStack) {
        return aStack.getTagCompound()
            .getCompoundTag(NBT_USB_DATA)
            .getString(NBT_REACTOR_SETUP);
    }
}
