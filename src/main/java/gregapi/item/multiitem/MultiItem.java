...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "ic2.api.item.ISpecialElectricItem", modid = ModIDs.IC2),
...
public abstract class MultiItem extends ItemBase implements IItemEnergy {
    ...
    public boolean hasProjectile(TagData aProjectileType, ItemStack aStack) {
        ...
        if (tList != null) for (IBehavior<MultiItem> tBehavior : tList)
            if (tBehavior.hasProjectile(this, aProjectileType, aStack)) return T;
        return super.hasProjectile(aProjectileType, aStack);
    }

    @Override
    public EntityProjectile getProjectile(TagData aProjectileType, ItemStack aStack, World aWorld, double aX, double aY,
        double aZ) {
        ArrayList<IBehavior<MultiItem>> tList = mItemBehaviors.get(ST.meta_(aStack));
        if (tList != null) for (IBehavior<MultiItem> tBehavior : tList) {
    ...
    @Override
    public EntityProjectile getProjectile(TagData aProjectileType, ItemStack aStack, World aWorld,
    ...
    public boolean isItemStackUsable(ItemStack aStack) {
        ArrayList<IBehavior<MultiItem>> tList = mItemBehaviors.get(ST.meta_(aStack));
        if (tList != null)
            for (IBehavior<MultiItem> tBehavior : tList) if (!tBehavior.isItemStackUsable(this, aStack)) return F;
    ...
    public boolean onItemUseFirst(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ,
        ...
        if (tList != null) for (IBehavior<MultiItem> tBehavior : tList) try {
            if (tBehavior
                .onItemUseFirst(this, aStack, aPlayer, aWorld, aX, aY, aZ, UT.Code.side(aSide), hitX, hitY, hitZ)) {
                destroyCheck(aStack, aPlayer);
                return T;
            }
            if (destroyCheck(aStack, aPlayer)) {
    ...
    public final void addInformation(ItemStack aStack, EntityPlayer aPlayer, @SuppressWarnings("rawtypes") List aList,
        boolean aF3_H) {
        try {
                ...
                if (tEnergyStats instanceof EnergyStatDebug) {
                    ...
                    for (TagData tEnergyType : tEnergyStats.getEnergyTypes(aStack)) {
                        ...
                        aList.add(
                            LH.Chat.WHITE
                                + UT.Code
                                    .makeString(Math.min(tCapacity, tEnergyStats.getEnergyStored(tEnergyType, aStack)))
                                + " / "
                                + UT.Code.makeString(tCapacity)
                                + " "
                                + tEnergyType.getLocalisedChatNameShort()
                                + LH.Chat.WHITE
                                + " - Size: "
                                + tEnergyStats.getEnergySizeInputRecommended(tEnergyType, aStack));
                    }
                }
            }
    ...
    public int fill(ItemStack aStack, FluidStack aFluid, boolean doFill) {
        ...
        if (tStats == null || tStats[0] <= 0
            || aFluid == null
            || aFluid.amount <= 0
            || !isAllowedToFill(aStack, aFluid)) return 0;
    ...
    public void chargeFromArmor(ItemStack aStack,
        EntityLivingBase aPlayer) {/* No longer in this Part of the Code, its in the EnergyStats now. */}
