        ...
        if (!aWorld.isRemote) {
            if (aPlayer != null) {
                ...
                if (COMPAT_TC != null) {
                    ...
                    // Notify the GT Log File that someone started using this Item. Just in case someone abuses this to
                    // Lag a Server.
                    OUT.println(
                        aPlayer.getCommandSenderName() + " has used the Item '"
                            + ST.make(aStack, (NBTTagCompound) null)
                                .getDisplayName()
                            + "', which may or may not be lagging for a few minutes");
                    // Tell the User that this is gonna Lag.
                    UT.Entities.sendchat(
                        aPlayer,
                        "Unlocking this many Aspects will lag for a few minutes, if done for the first time");
                    ...
                    for (OreDictMaterial tMat : OreDictMaterial.MATERIAL_ARRAY) if (tMat != null) {
                        for (ModData tMod : mModIDs) if (tMod == tMat.mOriginalMod) {
                            for (ItemStackContainer tStack : tMat.mRegisteredItems)
                                tScannedAnything |= COMPAT_TC.scan(aPlayer, tStack.toStack());
                            break;
                        }
                    }
                    // Prevent 16 Bit Integer Overflows because some Thaumcraft UIs use short instead of int...
                    ...
                    // Notify the GT Log File that someone used this Item. Just in case someone abuses this to Lag a
                    // Server.
                    OUT.println(
                        aPlayer.getCommandSenderName() + " is done using the Item '"
                            + ST.make(aStack, (NBTTagCompound) null)
                                .getDisplayName()
                            + "', the related Lag which may or may not have happened, is definitely over now");
                }
            }
            return T;
        }
