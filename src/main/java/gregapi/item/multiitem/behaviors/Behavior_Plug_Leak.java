    ...
    public boolean onItemUse(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        for (byte tSide : ALL_SIDES) {
            ...
            if (!WD.liquid(
                WD.block(
                    aWorld,
                    aX + OFFX[aSide] + OFFX[tSide],
                    aY + OFFY[aSide] + OFFY[tSide],
                    aZ + OFFZ[aSide] + OFFZ[tSide])))
                continue;
            ...
            for (int i = 0; i < aPlayer.inventory.mainInventory.length; i++) {
                ...
                if (tBlock instanceof IPrefixBlock || tBlock instanceof ITileEntityProvider
                    || tBlock instanceof BlockSilverfish) continue;
                // Only use Blocks that are typically mined.
                if (tBlock.getMaterial() != Material.rock && tBlock.getMaterial() != Material.ground
                    && tBlock.getMaterial() != Material.sand
                    && tBlock.getMaterial() != Material.clay) continue;
