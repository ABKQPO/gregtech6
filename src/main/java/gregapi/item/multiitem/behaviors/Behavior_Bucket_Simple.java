...
public class Behavior_Bucket_Simple extends AbstractBehaviorDefault {
    ...
    public ItemStack onDispense(MultiItem aItem, IBlockSource aSource, ItemStack aStack) {
        ...
        int aX = aSource.getXInt() + aFacing.getFrontOffsetX(), aY = aSource.getYInt() + aFacing.getFrontOffsetY(),
            aZ = aSource.getZInt() + aFacing.getFrontOffsetZ();
        ...
        if (mFluid == null) {
            ...
            if (ST.valid(mDefaultFullBucket)) {
                ...
                if (ST.invalid(tBucket = FL.fill(mFluid, tBucket, F, T, F, T)))
                    return super.onDispense(aItem, aSource, aStack);
            }
            if (ST.item_(tBucket) instanceof ItemBucket
                && ((ItemBucket) ST.item_(tBucket)).tryPlaceContainedLiquid(aWorld, aX, aY, aZ)) {
    ...
    public ItemStack onItemRightClick(MultiItem aItem, ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        ...
        if (mFluid == null) {
            ...
            if (tFluidBlock == Blocks.lava || tFluidBlock == Blocks.flowing_lava
                || tFluidBlock == Blocks.water
                || tFluidBlock == Blocks.flowing_water) {
                if (aWorld.getBlockMetadata(aX, aY, aZ) == 0) tBucket = tBucket.getItem()
                    .onItemRightClick(tBucket, aWorld, aPlayer);
            } else if (tFluidBlock instanceof IFluidBlock) {
                FluidStack tFluid = ((IFluidBlock) tFluidBlock).drain(aWorld, aX, aY, aZ, F);
                if (tFluid != null) {
                    if (ST.valid(FL.fill(tFluid, aStack, F, T, F, T))) tBucket = tBucket.getItem()
                        .onItemRightClick(tBucket, aWorld, aPlayer);
    ...
    public boolean onRightClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        if (FL.getFluid(aStack, T) == null && aEntity instanceof EntityLivingBase
            && !((EntityLivingBase) aEntity).isChild()) {
    ...
    protected ItemStack processBucket(ItemStack aBucket, ItemStack aStack, boolean aWasFull) {
        ...
        if (aWasFull) {
            if (aBucket.getItem() == Items.bucket) {
                ...
                if (aBucket == null) aStack.stackSize = 0;
                else aStack = aBucket;
                return aStack;
            }
        } else {
