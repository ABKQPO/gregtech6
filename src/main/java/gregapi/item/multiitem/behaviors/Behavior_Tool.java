...
public class Behavior_Tool extends AbstractBehaviorDefault {
    ...
    public Behavior_Tool(String aToolName) {
        this(aToolName, null, 0, T, 1.0F);
    }

    public Behavior_Tool(String aToolName, boolean aOnItemUseReturn) {
        this(aToolName, null, 0, aOnItemUseReturn, 1.0F);
    }
    ...
    public Behavior_Tool(String aToolName, String aSoundName, long aDamage, boolean aOnItemUseReturn,
        boolean aRandomPitch) {
        this(aToolName, aSoundName, aDamage, aOnItemUseReturn, aRandomPitch ? SFX.RANDOM_PITCH : 1.0F);
    }
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        // if (aPlayer != null && SIDES_VALID[aSide] && !(aPlayer instanceof FakePlayer) &&
        // UT.Worlds.isSideObstructed(aWorld, aX, aY, aZ, aSide)) return !aWorld.isRemote;
        List<String> tChatReturn = new ArrayListNoNulls<>();
        long tDamage = IBlockToolable.Util.onToolClick(
            mToolName,
            Long.MAX_VALUE,
            (aItem instanceof MultiItemTool ? ((MultiItemTool) aItem).getHarvestLevel(aStack, mToolName) : 1),
            aPlayer,
            tChatReturn,
        ...
        if (tDamage > 0) {
            if (mDamage > 0)
                ((MultiItemTool) aItem).doDamage(aStack, UT.Code.units(tDamage, 10000, mDamage, T), aPlayer, F);
    ...
    public List<String> getAdditionalToolTips(MultiItem aItem, List<String> aList, ItemStack aStack) {
        aList.add(
            LH.Chat.DGRAY + LH.get(TOOL_LOCALISER_PREFIX + mToolName, "Unknown")
                + "   "
                + LH.Chat.GRAY
                + LH.get(TOOL_TOOLTIP_PREFIX + mToolName, ""));
        return aList;
    }
}
