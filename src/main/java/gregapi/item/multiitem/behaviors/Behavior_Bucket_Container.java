...
public class Behavior_Bucket_Container extends AbstractBehaviorDefault {
    ...
    public ItemStack onItemRightClick(MultiItem aItem, ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        ...
        if (tBlock == Blocks.water || tBlock == Blocks.flowing_water) {
            if (aWorld.getBlockMetadata(tPosition.blockX, tPosition.blockY, tPosition.blockZ) == 0
                && aItem.fill(aStack, FL.Water.make(1000), F) == 1000) {
        ...
        if (tBlock instanceof IFluidBlock) {
            FluidStack tDrained = ((IFluidBlock) tBlock)
                .drain(aWorld, tPosition.blockX, tPosition.blockY, tPosition.blockZ, F);
            if (tDrained != null && tDrained.amount > 0 && aItem.fill(aStack, tDrained, F) == tDrained.amount) {
                // Forge fucked up the Fluid Draining Function, meaning if you insert true for doDrain it will ALWAYS
                // return a null Fluid for the finite Fluid Blocks. That's why I take the result from the simulation
                // instead of the actual draining.
