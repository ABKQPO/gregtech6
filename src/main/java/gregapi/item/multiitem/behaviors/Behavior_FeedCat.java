...
public class Behavior_FeedCat extends AbstractBehaviorDefault {
    ...
    public boolean onRightClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        if (aEntity instanceof EntityOcelot) {
            for (Object tTask : ((EntityOcelot) aEntity).tasks.taskEntries)
                if (((EntityAITaskEntry) tTask).action instanceof EntityAITempt
                    && ((EntityAITempt) ((EntityAITaskEntry) tTask).action).isRunning()) {
                        if (aPlayer.getDistanceSqToEntity(aEntity) < 9.0D) {
                            ...
                            if (!aPlayer.worldObj.isRemote) {
                                if (RNGSUS.nextInt(3) == 0) {
                                    ...
                                    ((EntityOcelot) aEntity).func_152115_b(
                                        aPlayer.getUniqueID()
                                            .toString());
                                    for (int i = 0; i < 7; ++i) aEntity.worldObj.spawnParticle(
                                        "heart",
                                        aEntity.posX + (RNGSUS.nextFloat() * aEntity.width * 2.0F) - aEntity.width,
                                        aEntity.posY + 0.5D + (RNGSUS.nextFloat() * aEntity.height),
                                        aEntity.posZ + (RNGSUS.nextFloat() * aEntity.width * 2.0F) - aEntity.width,
                                        RNGSUS.nextGaussian() * 0.02D,
                                        RNGSUS.nextGaussian() * 0.02D,
                                        RNGSUS.nextGaussian() * 0.02D);
            ...
            if (((EntityOcelot) aEntity).isTamed() && ((EntityOcelot) aEntity).getGrowingAge() == 0
                && !((EntityOcelot) aEntity).isInLove()) {
                UT.Entities.consumeCurrentItem(aPlayer);
