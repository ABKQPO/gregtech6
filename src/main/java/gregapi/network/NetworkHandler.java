...
public final class NetworkHandler extends MessageToMessageCodec<FMLProxyPacket, IPacket> implements INetworkHandler {
     ...
     * @param aChannelName Name of your Channel (use 4 Characters or less, we don't want to Lag out the Connection), the
     *                     GT Channel is called "GREG" and the API Channel is called "GAPI".
     * @param aPacketTypes An Array of your Packet Types (an empty instance of every Packet you want to use for
     *                     decoding). Remember that "getPacketID" must return a for your Handler individual Number. All
     *                     256 Byte Values are possible. Yes I mean the negative ones.
     */
    public NetworkHandler(String aModID, String aChannelName, IPacket... aPacketTypes) {
        ...
        mChannel = NetworkRegistry.INSTANCE.newChannel(
            aChannelName,
            this,
            FMLCommonHandler.instance()
                .getSide() == Side.CLIENT ? new HandlerClient(this) : new HandlerServer(this));
        mPacketTypes = new IPacket[256];
        for (int i = 0; i < aPacketTypes.length; i++) {
            int tID = UT.Code.unsignB(aPacketTypes[i].getPacketID());
            if (mPacketTypes[tID] == null) mPacketTypes[tID] = aPacketTypes[i];
            else throw new IllegalArgumentException("Duplicate Packet ID! " + tID);
        }
    }
    ...
    protected void decode(ChannelHandlerContext aContext, FMLProxyPacket aPacket, List<Object> aOutput)
        ...
        if (mPacketTypes[aID] == null) {
            FMLLog.warning(
                "Your Version of '" + mModID
                    + "' definetly does not match the Version installed on the Server you joined! Do not report this as a Bug! You failed to install/update the proper Version of '"
                    + mModID
                    + "' all by yourself!");
        } else {
    ...
    public void sendToServer(IPacket aPacket) {
        ...
        tChannel.attr(FMLOutboundHandler.FML_MESSAGETARGET)
    ...
    public void sendToPlayer(IPacket aPacket, EntityPlayerMP aPlayer) {
        ...
        tChannel.attr(FMLOutboundHandler.FML_MESSAGETARGETARGS)
            .set(aPlayer);
        tChannel.writeAndFlush(aPacket);
    }
    ...
    public void sendToAllPlayersInRange(IPacket aPacket, World aWorld, ChunkCoordinates aCoords) {
        sendToAllPlayersInRange(aPacket, aWorld, aCoords.posX, aCoords.posZ);
    }
        ...
        if (aWorld != null && !aWorld.isRemote) for (Object tObject : aWorld.playerEntities) {
            if (tObject instanceof EntityPlayerMP) {
                ...
                if (tPlayer.getServerForPlayer()
                    .getPlayerManager()
                    .isPlayerWatchingChunk(tPlayer, tChunk.xPosition, tChunk.zPosition)) sendToPlayer(aPacket, tPlayer);
            } else return;
        }
    }
    ...
    public void sendToPlayerIfInRange(IPacket aPacket, UUID aPlayer, World aWorld, ChunkCoordinates aCoords) {
        sendToPlayerIfInRange(aPacket, aPlayer, aWorld, aCoords.posX, aCoords.posZ);
    }
    ...
    public void sendToAllPlayersInRangeExcept(IPacket aPacket, UUID aPlayer, World aWorld, ChunkCoordinates aCoords) {
        sendToAllPlayersInRangeExcept(aPacket, aPlayer, aWorld, aCoords.posX, aCoords.posZ);
    }
    ...
    static final class HandlerClient extends SimpleChannelInboundHandler<IPacket> {
        ...
        protected void channelRead0(ChannelHandlerContext ctx, IPacket aPacket) throws Exception {
            aPacket.process(
                Minecraft.getMinecraft().thePlayer == null ? null : Minecraft.getMinecraft().thePlayer.worldObj,
                mNetworkHandler);
            // DEB.println(aPacket.getClass().getName());
            // if (aPacket instanceof PacketCoordinates) DEB.println(" X: " + ((PacketCoordinates)aPacket).mX + " - Y: "
            // + ((PacketCoordinates)aPacket).mY + " - Z: " + ((PacketCoordinates)aPacket).mZ);
        }
    }
