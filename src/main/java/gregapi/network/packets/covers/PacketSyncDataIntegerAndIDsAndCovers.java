...
public class PacketSyncDataIntegerAndIDsAndCovers extends PacketSyncDataIntegerAndIDs {
    ...
    public PacketSyncDataIntegerAndIDsAndCovers(int aX, int aY, int aZ, short aID1, short aID2, int aData,
        CoverData aCoverData) {
    ...
    public PacketSyncDataIntegerAndIDsAndCovers(ChunkCoordinates aCoords, short aID1, short aID2, int aData,
        CoverData aCoverData) {
    ...
    private PacketSyncDataIntegerAndIDsAndCovers(int aX, int aY, int aZ, short aID1, short aID2, int aData,
        ByteArrayDataInput aCovers) {
        super(aX, aY, aZ, aID1, aID2, aData);
        mCoverIDs = new short[] { 0, 0, 0, 0, 0, 0 };
        mCoverMetas = new short[] { 0, 0, 0, 0, 0, 0 };
        mCoverVisuals = new short[] { 0, 0, 0, 0, 0, 0 };
        for (byte i = 0, j = aCovers.readByte(); i < 6; i++) if ((j & (1 << i)) != 0) {
            mCoverIDs[i] = aCovers.readShort();
            mCoverMetas[i] = aCovers.readShort();
            mCoverVisuals[i] = aCovers.readShort();
        }
    ...
    public ByteArrayDataOutput encode2(ByteArrayDataOutput aData) {
        ...
        if (mCoverIDs != null) for (byte i = 0; i < 6; i++) if (mCoverIDs[i] > 0) {
            aData.writeShort(mCoverIDs[i]);
            aData.writeShort(mCoverMetas[i]);
            aData.writeShort(mCoverVisuals[i]);
        }
        return aData;
    }
    ...
    public PacketCoordinates decode2(int aX, int aY, int aZ, ByteArrayDataInput aData) {
        return new PacketSyncDataIntegerAndIDsAndCovers(
            aX,
            aY,
            aZ,
            aData.readShort(),
    ...
    public void process(IBlockAccess aWorld, INetworkHandler aNetworkHandler) {
        if (aWorld != null) {
            ...
            if (tBlock instanceof IBlockSyncDataAndCoversAndIDs)
                ((IBlockSyncDataAndCoversAndIDs) tBlock).receiveDataInteger(
                    aWorld,
                    mX,
                    mY,
                    mZ,
