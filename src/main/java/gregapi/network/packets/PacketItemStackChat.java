...
public class PacketItemStackChat implements IPacket {
    ...
    public ByteArrayDataOutput encode() {
        ...
        if (tNBT == null) aData.writeShort(-1);
        else {
            try {
    ...
    public IPacket decode(ByteArrayDataInput aData) {
        return new PacketItemStackChat(
            ST.make(aData.readShort(), aData.readByte(), aData.readShort(), readNBTTagCompoundFromBuffer(aData)));
    }
    ...
    public NBTTagCompound readNBTTagCompoundFromBuffer(ByteArrayDataInput aData) {
        ...
        try {
            return CompressedStreamTools.func_152457_a(tData, new NBTSizeTracker(2097152L));
        } catch (IOException e) {
            e.printStackTrace(ERR);
        }
        return null;
