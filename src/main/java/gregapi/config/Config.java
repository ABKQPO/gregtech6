...
public class Config implements Runnable {
    ...
    public Config(String aFileName) {
        this(DirectoriesGT.CONFIG_GT, aFileName);
    }
    ...
    public Config(File aDirPath, String aFileName) {
        File tPathUppercase = new File(aDirPath, aFileName),
            tPathLowercase = new File(aDirPath, aFileName.toLowerCase()), tPathUsed = tPathUppercase;
        ...
        try {
            ...
            if (aFileName.equals(aFileName.toLowerCase())) {
                ...
                if (tPathUppercase.exists()) {
                    if (tPathLowercase.exists()
                        && !Files.isSameFile(tPathLowercase.toPath(), tPathUppercase.toPath())) {
                        ...
                        // This is likely on Windows Systems. And in that case it would also happen regardless if the
                        // Name is already Lowercased or not.
                        // Try to rename the File if possible, otherwise just leave it be.
                        if (tPathUsed.renameTo(tPathLowercase)) tPathUsed = tPathLowercase;
                    }
                } else {
                    // Nothing to actually do here, the File either doesn't exist at all due to being a new install, or
    ...
    public boolean get(Object aCategory, String aName, boolean aDefault) {
        ...
        Property tProperty = mConfig.get(
            aCategory.toString()
                .replaceAll("\\|", "_"),
            (aName + (mUsesDefaultsInNames ? "_" + aDefault : "")).replaceAll("\\|", "_"),
            aDefault);
        boolean rResult = tProperty.getBoolean(aDefault);
        if (Abstract_Mod.sFinalized >= Abstract_Mod.sModCountUsingGTAPI && mSaveOnEdit && !tProperty.wasRead())
            mConfig.save();
        return rResult;
    }
    ...
    public int get(Object aCategory, String aName, long aDefault) {
        ...
        Property tProperty = mConfig.get(
            aCategory.toString()
                .replaceAll("\\|", "_"),
            (aName + (mUsesDefaultsInNames ? "_" + UT.Code.bindInt(aDefault) : "")).replaceAll("\\|", "_"),
            UT.Code.bindInt(aDefault));
    ...
    public OreDictMaterial get(Object aCategory, String aName, OreDictMaterial aDefault) {
        ...
        Property tProperty = mConfig.get(
            aCategory.toString()
                .replaceAll("\\|", "_"),
            (aName + (mUsesDefaultsInNames ? "_" + aDefault.mNameInternal : "")).replaceAll("\\|", "_"),
            aDefault.mNameInternal);
    ...
    public String toString() {
        return "Config File: " + mConfig.getConfigFile()
            .getAbsolutePath();
    }
    ...
    public static int addIDConfig(Object aCategory, String aName, int aDefault) {
        ...
        if (Abstract_Mod.sFinalized >= Abstract_Mod.sModCountUsingGTAPI && !tProperty.wasRead()) sConfigFileIDs.save();
        else sIDConfigNeedsSaving = T;
        return rResult;
    }
}
