 ...
 */
public abstract class TileEntityBase04Covers extends TileEntityBase03MultiTileEntities
    implements ITileEntityCoverable, ISidedInventory, ITileEntitySurface, ITileEntityOnDrawBlockHighlight,
    IMTE_ShouldCheckWeakPower, IMTE_OnNeighborBlockChange, IMTE_IsSideSolid, IMTE_SyncDataCovers, IMTE_OnToolClick,
    IMTE_AddCollisionBoxesToList, IMTE_IsProvidingWeakPower, IMTE_IsProvidingStrongPower {
    ...
    public boolean receiveDataCovers(short[] aCoverVisuals, boolean[] aVisualsToSync, INetworkHandler aNetworkHandler) {
        ...
        for (byte i = 0; i < mCovers.mVisuals.length; i++)
            if (aVisualsToSync[i]) mCovers.mVisuals[i] = aCoverVisuals[i];
        return T;
    }
    ...
    public final boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (SIDES_INVALID[aSide] || aPlayer == null || !allowInteraction(aPlayer))
        ...
        if (hasCovers()) {
            if (mCovers.mBehaviours[aSide] != null) {
                if (mCovers.mBehaviours[aSide]
                    .onCoverClickedRight(aSide, mCovers, aPlayer, aSide, aHitX, aHitY, aHitZ)) {
                    checkCoverValidity();
                ...
                if (mCovers.mBehaviours[aSide]
                    .interceptClickRight(aSide, mCovers, aPlayer, aSide, aHitX, aHitY, aHitZ)) {
                    checkCoverValidity();
                    if (mCovers.requiresSync()) {
                        sendClientData(F, null);
                        mCovers.resetSync();
                ...
                if (mCovers.mBehaviours[tSide]
                    .onCoverClickedRight(tSide, mCovers, aPlayer, aSide, aHitX, aHitY, aHitZ)) {
                    checkCoverValidity();
                    if (mCovers.requiresSync()) {
                        sendClientData(F, null);
                        mCovers.resetSync();
                ...
                if (mCovers.mBehaviours[tSide]
                    .interceptClickRight(tSide, mCovers, aPlayer, aSide, aHitX, aHitY, aHitZ)) {
                    checkCoverValidity();
                    if (mCovers.requiresSync()) {
                        sendClientData(F, null);
                        mCovers.resetSync();
    ...
    public final long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (checkObstruction(
            aPlayer instanceof EntityPlayer ? (EntityPlayer) aPlayer : null,
            aSide,
            aHitX,
            aHitY,
            aHitZ)) return 0;
        ...
        if (SIDES_VALID[aSide] && hasCovers()) {
            byte tSide = usePipePlacementMode(aSide) && mCovers.mIDs[aSide] == 0
                ? UT.Code.getSideWrenching(aSide, aHitX, aHitY, aHitZ)
                : aSide;
            if (aTool.equals(TOOL_crowbar) && isServerSide()) {
                ...
                if (tStack != null && setCoverItem(tSide, null, aPlayer, F, T)) {
                    if (!ST.add(aPlayer, tStack, F)) ST.place(
                        worldObj,
                        getOffsetX(aSide) + 0.5,
                        getOffsetY(aSide) + 0.5,
                        getOffsetZ(aSide) + 0.5,
                        tStack);
            ...
            if (mCovers.mBehaviours[tSide] != null) {
                ...
                rDamage = mCovers.mBehaviours[tSide].onToolClick(
                    tSide,
                    mCovers,
                    aTool,
                    aRemainingDurability,
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        return aSendAll
            ? hasCovers()
                ? new PacketSyncDataIDsAndCovers(
                    getCoords(),
                    getMultiTileEntityRegistryID(),
                    getMultiTileEntityID(),
                    mCovers)
                : new PacketSyncDataIDs(getCoords(), getMultiTileEntityRegistryID(), getMultiTileEntityID())
            : hasCovers() && mCovers.requiresSync() ? new PacketSyncDataCoverVisuals(getCoords(), mCovers) : null;
    }

    @Override
    public IPacket getClientDataPacketByte(boolean aSendAll, byte aByte) {
        return aSendAll ? hasCovers()
            ? new PacketSyncDataByteAndIDsAndCovers(
                getCoords(),
                getMultiTileEntityRegistryID(),
                getMultiTileEntityID(),
                aByte,
                mCovers)
            : new PacketSyncDataByteAndIDs(getCoords(), getMultiTileEntityRegistryID(), getMultiTileEntityID(), aByte)
            : hasCovers() && mCovers.requiresSync() ? new PacketSyncDataByteAndCoverVisuals(getCoords(), aByte, mCovers)
                : new PacketSyncDataByte(getCoords(), aByte);
    }

    @Override
    public IPacket getClientDataPacketShort(boolean aSendAll, short aShort) {
        return aSendAll ? hasCovers()
            ? new PacketSyncDataShortAndIDsAndCovers(
                getCoords(),
                getMultiTileEntityRegistryID(),
                getMultiTileEntityID(),
                aShort,
                mCovers)
            : new PacketSyncDataShortAndIDs(getCoords(), getMultiTileEntityRegistryID(), getMultiTileEntityID(), aShort)
            : hasCovers() && mCovers.requiresSync()
                ? new PacketSyncDataShortAndCoverVisuals(getCoords(), aShort, mCovers)
                : new PacketSyncDataShort(getCoords(), aShort);
    }
    ...
    public IPacket getClientDataPacketInteger(boolean aSendAll, int aInteger) {
        return aSendAll
            ? hasCovers()
                ? new PacketSyncDataIntegerAndIDsAndCovers(
                    getCoords(),
                    getMultiTileEntityRegistryID(),
                    getMultiTileEntityID(),
                    aInteger,
                    mCovers)
                : new PacketSyncDataIntegerAndIDs(
                    getCoords(),
                    getMultiTileEntityRegistryID(),
                    getMultiTileEntityID(),
                    aInteger)
            : hasCovers() && mCovers.requiresSync()
                ? new PacketSyncDataIntegerAndCoverVisuals(getCoords(), aInteger, mCovers)
                : new PacketSyncDataInteger(getCoords(), aInteger);
    }

    @Override
    public IPacket getClientDataPacketLong(boolean aSendAll, long aLong) {
        return aSendAll ? hasCovers()
            ? new PacketSyncDataLongAndIDsAndCovers(
                getCoords(),
                getMultiTileEntityRegistryID(),
                getMultiTileEntityID(),
                aLong,
                mCovers)
            : new PacketSyncDataLongAndIDs(getCoords(), getMultiTileEntityRegistryID(), getMultiTileEntityID(), aLong)
            : hasCovers() && mCovers.requiresSync() ? new PacketSyncDataLongAndCoverVisuals(getCoords(), aLong, mCovers)
    ...
    public IPacket getClientDataPacketByteArray(boolean aSendAll, byte... aByteArray) {
        return aSendAll
            ? hasCovers()
                ? new PacketSyncDataByteArrayAndIDsAndCovers(
                    getCoords(),
                    getMultiTileEntityRegistryID(),
                    getMultiTileEntityID(),
                    aByteArray,
                    mCovers)
                : new PacketSyncDataByteArrayAndIDs(
                    getCoords(),
                    getMultiTileEntityRegistryID(),
                    getMultiTileEntityID(),
                    aByteArray)
            : hasCovers() && mCovers.requiresSync()
                ? new PacketSyncDataByteArrayAndCoverVisuals(getCoords(), aByteArray, mCovers)
                : new PacketSyncDataByteArray(getCoords(), aByteArray);
    }

    public void checkCoverValidity() {
        if (worldObj != null && isServerSide() && hasCovers())
            for (byte tSide : ALL_SIDES_VALID) if (!allowCovers(tSide)) {
                ItemStack tStack = getCoverItem(tSide);
                if (tStack != null && setCoverItem(tSide, null, null, T, T)) {
                    ST.place(
                        worldObj,
                        getOffsetX(tSide) + 0.5,
                        getOffsetY(tSide) + 0.5,
                        getOffsetZ(tSide) + 0.5,
                        tStack);
    ...
    public boolean isFireProof(byte aSide) {
        return hasCovers() && mCovers.mBehaviours[aSide] != null && mCovers.mBehaviours[aSide].isOpaque(aSide, mCovers);
    }
    ...
    public boolean setCoverItem(byte aSide, ItemStack aStack, Entity aPlayer, boolean aForce, boolean aBlockUpdate) {
        ...
        if (!aForce) {
            if (aStack == null) {
                if (mCovers.mBehaviours[aSide] != null
                    && mCovers.mBehaviours[aSide].interceptCoverRemoval(aSide, mCovers, aPlayer))
                ...
                if (tCover == null || !allowCover(aSide, tCover)
                    || tCover.interceptCoverPlacement(aSide, mCovers, aPlayer))
                    return !checkIfCoversEmptyAndDeleteIfNeeded();
            }
        }
    ...
    public boolean isUseableByPlayer(EntityPlayer aPlayer) {
        return !isDead() && allowInteraction(aPlayer)
            && aPlayer.getDistanceSq(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D) <= 64D;
    }
    ...
    public final int[] getAccessibleSlotsFromSide(int aSide) {
        if (hasCovers() && SIDES_VALID[aSide]
            && mCovers.mBehaviours[aSide] != null
            && mCovers.mBehaviours[aSide]
                .getAccessibleSlotsFromSideOverride(UT.Code.side(aSide), mCovers, UT.Code.side(aSide)))
            return mCovers.mBehaviours[aSide].getAccessibleSlotsFromSide(
                UT.Code.side(aSide),
                mCovers,
                UT.Code.side(aSide),
                getAccessibleSlotsFromSide2(UT.Code.side(aSide)));
    ...
    public final boolean canInsertItem(int aSlot, ItemStack aStack, int aSide) {
        if (hasCovers() && SIDES_VALID[aSide] && mCovers.mBehaviours[aSide] != null) {
            if (mCovers.mBehaviours[aSide]
                .interceptItemInsert(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide))) return F;
            if (mCovers.mBehaviours[aSide]
                .canInsertItemOverride(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide)))
                return mCovers.mBehaviours[aSide]
                    .canInsertItem(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide))
                    && canInsertItem2(aSlot, aStack, UT.Code.side(aSide));
        }
    ...
    public final boolean canExtractItem(int aSlot, ItemStack aStack, int aSide) {
        if (hasCovers() && SIDES_VALID[aSide] && mCovers.mBehaviours[aSide] != null) {
            if (mCovers.mBehaviours[aSide]
                .interceptItemExtract(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide))) return F;
            if (mCovers.mBehaviours[aSide]
                .canExtractItemOverride(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide)))
                return mCovers.mBehaviours[aSide]
                    .canExtractItem(UT.Code.side(aSide), mCovers, aSlot, aStack, UT.Code.side(aSide))
                    && canExtractItem2(aSlot, aStack, UT.Code.side(aSide));
        }
    ...
    protected final IFluidTank getFluidTankFillable(byte aSide, FluidStack aFluidToFill) {
        if (hasCovers() && SIDES_VALID[aSide] && mCovers.mBehaviours[aSide] != null) {
            if (mCovers.mBehaviours[aSide].getFluidTankFillableOverride(aSide, mCovers, aSide, aFluidToFill))
                return mCovers.mBehaviours[aSide].getFluidTankFillable(
                    aSide,
                    mCovers,
                    aSide,
                    aFluidToFill,
                    getFluidTankFillable2(aSide, aFluidToFill));
    ...
    protected final IFluidTank getFluidTankDrainable(byte aSide, FluidStack aFluidToDrain) {
        if (hasCovers() && SIDES_VALID[aSide] && mCovers.mBehaviours[aSide] != null) {
            if (mCovers.mBehaviours[aSide].getFluidTankDrainableOverride(aSide, mCovers, aSide, aFluidToDrain))
                return mCovers.mBehaviours[aSide].getFluidTankDrainable(
                    aSide,
                    mCovers,
                    aSide,
                    aFluidToDrain,
                    getFluidTankDrainable2(aSide, aFluidToDrain));
            if (aFluidToDrain == null) {
                IFluidTank rTank = getFluidTankDrainable2(aSide, null);
                if (rTank == null) return null;
                if (mCovers.mBehaviours[aSide].interceptFluidDrain(aSide, mCovers, aSide, FL.mul(rTank.getFluid(), 1)))
                    return null;
                return rTank;
            }
    ...
    protected final IFluidTank[] getFluidTanks(byte aSide) {
        if (hasCovers() && SIDES_VALID[aSide]
            && mCovers.mBehaviours[aSide] != null
            && mCovers.mBehaviours[aSide].getFluidTanksOverride(aSide, mCovers, aSide))
            return mCovers.mBehaviours[aSide].getFluidTanks(aSide, mCovers, aSide, getFluidTanks2(aSide));
    ...
    public void onNeighborBlockChange(World aWorld, Block aBlock) {
        if (hasCovers()) mCovers.onBlockUpdate();
    }
    ...
    public final byte getRedstoneIncoming(byte aSide) {
            ...
            for (byte tSide : ALL_SIDES_VALID) {
                if (mCovers.mBehaviours[tSide] == null) {
                    rRedstone = (byte) Math.max(
                        rRedstone,
                        worldObj
                            .getIndirectPowerLevelTo(getOffsetX(tSide), getOffsetY(tSide), getOffsetZ(tSide), tSide));
                } else {
        ...
        return mCovers.mBehaviours[aSide] == null
            ? UT.Code
                .bind4(worldObj.getIndirectPowerLevelTo(getOffsetX(aSide), getOffsetY(aSide), getOffsetZ(aSide), aSide))
            : mCovers.mBehaviours[aSide].getRedstoneIn(aSide, mCovers);
    }

    @Override
    public final int isProvidingWeakPower(byte aOppositeSide) {
        byte tActualSide = OPOS[aOppositeSide];
        if (hasCovers() && SIDES_VALID[tActualSide] && mCovers.mBehaviours[tActualSide] != null)
            return mCovers.mBehaviours[tActualSide]
                .getRedstoneOutWeak(tActualSide, mCovers, isProvidingWeakPower2(aOppositeSide));
    ...
    public final int isProvidingStrongPower(byte aOppositeSide) {
        ...
        if (hasCovers() && SIDES_VALID[tActualSide] && mCovers.mBehaviours[tActualSide] != null)
            return mCovers.mBehaviours[tActualSide]
                .getRedstoneOutStrong(tActualSide, mCovers, isProvidingStrongPower2(aOppositeSide));
    ...
    public final boolean isSideSolid(byte aSide) {
        return (hasCovers() && mCovers.mBehaviours[aSide] != null && mCovers.mBehaviours[aSide].isSolid(aSide, mCovers))
            || isSideSolid2(aSide);
    }

    @Override
    public final boolean isSurfaceOpaque(byte aSide) {
        return (hasCovers() && mCovers.mBehaviours[aSide] != null
            && mCovers.mBehaviours[aSide].isOpaque(aSide, mCovers)) || isSurfaceOpaque2(aSide);
    }

    public boolean isSideSolid2(byte aSide) {
        return isSurfaceSolid(aSide);
    }
    ...
    public final boolean usesRenderPass(int aRenderPass, boolean[] aShouldSideBeRendered) {
        return aRenderPass < mInternalRenderPasses ? usesRenderPass2(aRenderPass, aShouldSideBeRendered)
            : mCovers.mBehaviours[(aRenderPass - mInternalRenderPasses) / 2] != null
                && ((aRenderPass - mInternalRenderPasses) % 2 != 0
                    || allowCoverHolders((byte) ((aRenderPass - mInternalRenderPasses) / 2)))
                && !isCoverSurface((byte) ((aRenderPass - mInternalRenderPasses) / 2));
    }
    ...
    public final boolean setBlockBounds(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        ...
        return box(
            aBlock,
            tRenderPass % 2 == 0
                ? mCovers.mBehaviours[tRenderPass / 2].getHolderBounds((byte) (tRenderPass / 2), mCovers)
                : mCovers.mBehaviours[tRenderPass / 2].getCoverBounds((byte) (tRenderPass / 2), mCovers));
    }
    ...
    public final ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass < mInternalRenderPasses) {
            if (hasCovers() && mCovers.mBehaviours[aSide] != null && isCoverSurface(aSide, aRenderPass))
                return BlockTextureMulti.get(
                    getTexture2(aBlock, aRenderPass, aSide, aShouldSideBeRendered),
                    mCovers.mBehaviours[aSide].getCoverTextureSurface(aSide, mCovers));
            return getTexture2(aBlock, aRenderPass, aSide, aShouldSideBeRendered);
        }
        int tRenderPass = aRenderPass - mInternalRenderPasses;
        if (tRenderPass % 2 == 0) {
            tRenderPass /= 2;
            return ALONG_AXIS[aSide][tRenderPass] ? null
                : mCovers.mBehaviours[tRenderPass].getCoverTextureHolder((byte) tRenderPass, mCovers, aSide);
        }
        tRenderPass /= 2;
        return !ALONG_AXIS[aSide][tRenderPass]
            && ((mCovers.mBehaviours[aSide] != null && mCovers.mBehaviours[aSide].isFullTexture(aSide, mCovers))
                || isCoverSurface(aSide, aRenderPass))
                    ? null
                    : aSide == OPOS[tRenderPass] || aShouldSideBeRendered[aSide]
                        ? mCovers.mBehaviours[tRenderPass].getCoverTextureAttachment((byte) tRenderPass, mCovers, aSide)
                        : null;
    }
    ...
    public final void addCollisionBoxesToList(AxisAlignedBB aAABB, List<AxisAlignedBB> aList, Entity aEntity) {
        ...
        if (hasCovers()) for (byte tSide : ALL_SIDES_VALID) if (mCovers.mBehaviours[tSide] != null)
            mCovers.mBehaviours[tSide].getCollisions(tSide, mCovers, aAABB, aList, aEntity);
    ...
    public final Object getGUIClient(int aGUIID, EntityPlayer aPlayer) {
        return aGUIID <= -1 && aGUIID >= -6 ? hasCovers() && mCovers.mBehaviours[-aGUIID - 1] != null
            ? mCovers.mBehaviours[-aGUIID - 1].getGUIServer((byte) (-aGUIID - 1), mCovers, aPlayer)
            : null : getGUIClient2(aGUIID, aPlayer);
    }

    @Override
    public final Object getGUIServer(int aGUIID, EntityPlayer aPlayer) {
        return aGUIID <= -1 && aGUIID >= -6 ? hasCovers() && mCovers.mBehaviours[-aGUIID - 1] != null
            ? mCovers.mBehaviours[-aGUIID - 1].getGUIClient((byte) (-aGUIID - 1), mCovers, aPlayer)
            : null : getGUIServer2(aGUIID, aPlayer);
    }
