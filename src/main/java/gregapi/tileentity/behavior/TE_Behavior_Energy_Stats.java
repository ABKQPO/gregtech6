...
public class TE_Behavior_Energy_Stats extends TE_Behavior_Energy {
    ...
    public TE_Behavior_Energy_Stats(TileEntity aTileEntity, NBTTagCompound aNBT, TagData aEnergyType,
        TE_Behavior_Energy_Capacitor aStorage, long aSizeMin, long aSizeRec, long aSizeMax) {
        super(aTileEntity, aNBT, aEnergyType);
        mStorage = aStorage;
        mMin = Math.abs(aSizeMin);
        mRec = Math.abs(aSizeRec);
        mMax = Math.abs(aSizeMax);
    }

    public boolean isType(TagData aEnergyType) {
        return mType == aEnergyType;
    }

    public long sizeMin(TagData aEnergyType) {
        return mType != aEnergyType ? 0 : mMin;
    }

    public long sizeRec(TagData aEnergyType) {
        return mType != aEnergyType ? 0 : mRec;
    }

    public long sizeMax(TagData aEnergyType) {
        return mType != aEnergyType ? 0 : mMax;
    }

    public Collection<TagData> getTypes() {
        return mType.AS_LIST;
    }

    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H, String aSides, boolean aEmitting) {
        aList.add(
            (aEmitting ? (Chat.RED + LH.get(LH.ENERGY_OUTPUT)) : (Chat.GREEN + LH.get(LH.ENERGY_INPUT))) + ": "
                + Chat.WHITE
                + mRec
                + " "
                ...
                + Chat.WHITE
                + "/t ("
                + (mMin > 1 ? mMin + " to " : "up to ")
                + mMax
                + (UT.Code.stringInvalid(aSides) ? "" : ", " + aSides)
                + ")");
    ...
    public long doInject(long aSize, long aAmount, boolean aDoInject) {
        ...
        long tInput = Math.min(mStorage.mCapacity - mStorage.mEnergy, aSize * aAmount),
