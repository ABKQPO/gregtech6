 ...
 */
public abstract class TileEntityBase08Barrel extends TileEntityBase07Paintable
    implements IMTE_AddToolTips, IMTE_GetMaxStackSize, ITileEntityFunnelAccessible, ITileEntityTapAccessible,
    ITileEntityProgress, ITileEntityConnectedTank, IFluidHandler, IFluidContainerItem, IItemRottable {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_CAPACITY_HU)) mMeltingPoint = aNBT.getLong(NBT_CAPACITY_HU);
        else mMeltingPoint = (long) (mMaterial.mMeltingPoint * 1.25);
        mMode = aNBT.getByte(NBT_MODE);
        mSealedTime = aNBT.getLong(NBT_PROGRESS);
        mTank.setPreventDraining(keepsFilter())
            .setCapacity(aNBT.getLong(NBT_TANK_CAPACITY))
            .readFromNBT(aNBT, NBT_TANK);
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_wrench) || aTool.equals(TOOL_monkeywrench)) {
            mMode ^= B[0];
            aChatReturn.add(
                (mMode & B[0]) == 0 ? "Won't fill vertically adjacent Tanks"
        ...
        if (aTool.equals(TOOL_thermometer)) {
            if (aChatReturn != null) aChatReturn.add("Temperature: " + FL.temperature(mTank) + "K");
            return 10000;
        }
        ...
        if (aIsServerSide) {
            ...
            if (tFluid != null && tFluid.amount > 0) {
                ...
                if (!mPlasmaProof && FL.plasma(tFluid)) {
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                    GarbageGT.trash(mTank);
                } else if (!mGasProof && FL.gas(tFluid)) {
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                    GarbageGT.trash(mTank);
                } else if (!allowFluid(tFluid)) {
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                    GarbageGT.trash(mTank);
                } else {
                    if ((mMode & B[1]) != 0) {
                        if (mMaxSealedTime <= 0 || mRecipe == null) {
                            mRecipe = RM.Fermenter.findRecipe(
                                this,
                                mRecipe,
                                T,
                                Long.MAX_VALUE,
                                NI,
                                FL.array(mTank.getFluid()),
                                ST.tag(0));
                            if (mRecipe != null && mRecipe.mFluidInputs.length > 0
                                && mRecipe.mFluidOutputs.length > 0
                                && !FL.gas(mRecipe.mFluidInputs[0])
                                && !FL.gas(mRecipe.mFluidOutputs[0])) {
                                mMaxSealedTime = UT.Code.divup(
                                    Math.max(1, mRecipe.getAbsoluteTotalPower()) * Math.max(1, mTank.amount()),
                                    Math.max(1, mRecipe.mFluidInputs[0].amount));
                            } else {
                                mMaxSealedTime = 0;
                                mSealedTime = 0;
                                mRecipe = null;
                            ...
                            if (mSealedTime < mMaxSealedTime) {
                                ...
                                mTank.setFluid(
                                    FL.mul(
                                        mRecipe.mFluidOutputs[0],
                                        mTank.amount(),
                                        mRecipe.mFluidInputs[0].amount,
                                        F));
                                mMaxSealedTime = 0;
                                mSealedTime = 0;
                                mRecipe = null;
                        ...
                        if ((mMode & B[0]) != 0) {
                            byte[] tSides = ZL_BYTE;
                            if (FL.gas(tFluid)) tSides = ALL_SIDES_VERTICAL;
                            else if (FL.lighter(tFluid)) tSides = ALL_SIDES_TOP;
                            else tSides = ALL_SIDES_BOTTOM;
                            for (byte tSide : tSides)
                                if (mTank.has() && FL.move(mTank, getAdjacentTank(tSide)) > 0) updateInventory();
                        }
                    }
                }
            }
    ...
    public ItemStack getRotten(ItemStack aStack) {
        return mMaterial.contains(TD.Properties.BETWEENLANDS) ? aStack
            : IItemRottable.RottingUtil.rotting(aStack, (IFluidContainerItem) aStack.getItem());
    }
    ...
    public int removeFluidFromConnectedTank(byte aSide, FluidStack aFluid, boolean aOnlyRemoveIfItCanRemoveAllAtOnce) {
        if ((mMode & B[1]) == 0 && mTank.contains(aFluid)
            && mTank.has(aOnlyRemoveIfItCanRemoveAllAtOnce ? aFluid.amount : 1))
