 ...
 */
public abstract class TileEntityBase08FluidContainer extends TileEntityBase07Paintable
    implements ITileEntityConnectedTank, IMTE_GetMaxStackSize, IMTE_OnlyPlaceableWhenSneaking, IMTE_OnItemRightClick,
    IMTE_OnItemUseFirst, IMTE_AddToolTips, IFluidContainerItem {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_TEMPERATURE)) mTemperatureMax = aNBT.getLong(NBT_TEMPERATURE);
        else mTemperatureMax = mMaterial.mMeltingPoint - 50;
    ...
    public NBTTagCompound writeItemNBT2(NBTTagCompound aNBT) {
        mTank.writeToNBT(aNBT, NBT_TANK);
        if (isClientSide() && !mTank.isEmpty())
            aNBT.setTag("display", UT.NBT.makeString(aNBT.getCompoundTag("display"), "Name", FL.name(mTank, T)));
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide && canFillWithRain()
            && SERVER_TIME % 600 == 10
            && worldObj.isRaining()
            && getRainOffset(0, 1, 0)) {
            ...
            if (tBiome.rainfall > 0 && tBiome.temperature >= 0.2) {
                ...
                if (!WD.liquid(tInFront)
                    ...
                    mTank.fill(
                        FL.Water.make((long) Math.max(1, tBiome.rainfall * 100) * (worldObj.isThundering() ? 2 : 1)),
                        T);
                }
            }
        }
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (isDrinkable() && canDrinkFromSide(aSide)) {
            ...
            if (UT.Entities.isCreative(aPlayer) || aPlayer.getFoodStats()
                .needFood() || FoodStatFluid.INSTANCE.alwaysEdible(aStack.getItem(), aStack, aPlayer)) {
                switch (FoodStatFluid.INSTANCE.getFoodAction(aStack.getItem(), aStack)) {
                    case eat:
                        UT.Sounds.send(SFX.MC_EAT, this, F);
                        break;
                    default:
                        UT.Sounds.send(SFX.MC_DRINK, this, F);
                        break;
                }
                mTank.remove(250);
                aStack.getItem()
                    .onEaten(aStack, worldObj, aPlayer);
            }
        }
        return T;
    }
            ...
            if (FL.water(mFluid)) {
                ...
                try {
                    if (tTileEntity instanceof TileCrucible) {
                        if (FL.water(mFluid) && FL.nonzero(
                            aItem.drain(
                                aStack,
                                (int) FL.fill((IFluidHandler) tTileEntity, SIDE_TOP, FL.Water.make(mFluid.amount), T),
                                T))) {
                            UT.Sounds.send(SFX.MC_LIQUID_WATER, aWorld, aX, aY, aZ);
                        }
                        return T;
    ...
    public ItemStack onItemRightClick(MultiTileEntityItemInternal aItem, ItemStack aStack, World aWorld,
        ...
        if (canPickUpFluids() && aStack.stackSize == 1) {
            ...
            if (tTarget != null && tTarget.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK
                && aWorld.canMineBlock(aPlayer, tTarget.blockX, tTarget.blockY, tTarget.blockZ)) {
                ...
                if (tBlock == Blocks.lava || tBlock == Blocks.flowing_lava) {
                    if (aWorld.getBlockMetadata(tTarget.blockX, tTarget.blockY, tTarget.blockZ) == 0
                        && aItem.fill(aStack, FL.Lava.make(1000), F) == 1000) {
                        aWorld.setBlockToAir(tTarget.blockX, tTarget.blockY, tTarget.blockZ);
                ...
                if (tBlock instanceof IFluidBlock) {
                    FluidStack tDrained = ((IFluidBlock) tBlock)
                        .drain(aWorld, tTarget.blockX, tTarget.blockY, tTarget.blockZ, F);
                    if (tDrained != null && tDrained.amount > 0 && aItem.fill(aStack, tDrained, F) == tDrained.amount) {
                        // Forge fucked up the Fluid Draining Function, meaning if you insert true for doDrain it will
                        // ALWAYS return a null Fluid for the finite Fluid Blocks. That's why I take the result from the
                        // simulation instead of the actual draining.
        ...
        if (isDrinkable() && aStack.stackSize == 1
            && (UT.Entities.isCreative(aPlayer) || aPlayer.getFoodStats()
                .needFood() || FoodStatFluid.INSTANCE.alwaysEdible(aStack.getItem(), aStack, aPlayer))) {
            aPlayer.setItemInUse(
                aStack,
                Math.max(FoodStatFluid.INSTANCE.getFoodLevel(aStack.getItem(), aStack, null) * 8, 32));
            return aStack;
        }
        return aStack;
    }
    ...
    public int getMaxItemUseDuration(MultiTileEntityItemInternal aItem, ItemStack aStack) {
        return isDrinkable() && aStack.stackSize == 1
            ? Math.max(FoodStatFluid.INSTANCE.getFoodLevel(aStack.getItem(), aStack, null) * 8, 32)
            : 0;
    }

    public EnumAction getItemUseAction(MultiTileEntityItemInternal aItem, ItemStack aStack) {
        return isDrinkable() && aStack.stackSize == 1 ? FoodStatFluid.INSTANCE.getFoodAction(aStack.getItem(), aStack)
            : EnumAction.none;
    }
        ...
        if (tFoodLevel > 0) {
            if (FoodStatFluid.INSTANCE.useAppleCoreFunctionality(aStack.getItem(), aStack, aPlayer)) {
                aPlayer.getFoodStats()
                    .func_151686_a(
                        (ItemFood) UT.Reflection
                            .callConstructor("squeek.applecore.api.food.ItemFoodProxy", 0, null, T, aStack.getItem()),
                        aStack);
            } else {
                aPlayer.getFoodStats()
                    .addStats(tFoodLevel, FoodStatFluid.INSTANCE.getSaturation(aStack.getItem(), aStack, aPlayer));
            }
        }

        if (!aWorld.isRemote && MD.ENVM.mLoaded) {
            try {
                float tTemperature = FoodStatFluid.INSTANCE.getTemperature(aStack.getItem(), aStack, aPlayer) - C,
                    tHydration = FoodStatFluid.INSTANCE.getHydration(aStack.getItem(), aStack, aPlayer);
                Object tTracker = EM_StatusManager.lookupTracker(aPlayer);
                if (tTracker != null && ((EnviroDataTracker) tTracker).bodyTemp >= 0) {
                    ((EnviroDataTracker) tTracker).bodyTemp += (tTemperature - ((EnviroDataTracker) tTracker).bodyTemp)
                        * FoodStatFluid.INSTANCE.getTemperatureEffect(aStack.getItem(), aStack, aPlayer);
                    if (tHydration > 0) ((EnviroDataTracker) tTracker).hydrate(tHydration);
                    else if (tHydration < 0) ((EnviroDataTracker) tTracker).dehydrate(-tHydration);
                }
            } catch (Throwable e) {
    ...
    public FoodValues getFoodValues(MultiTileEntityItemInternal aItem, ItemStack aStack) {
        ...
        return tFoodLevel > 0 && isDrinkable() ? new squeek.applecore.api.food.FoodValues(
            tFoodLevel,
            FoodStatFluid.INSTANCE.getSaturation(aStack.getItem(), aStack, null)) : null;
    }
    ...
    public int addFluidToConnectedTank(byte aSide, FluidStack aFluid,
        ...
        if (aFluid == NF || !isFluidAllowed(aFluid)
            || (mTank.isEmpty() && aOnlyAddIfItAlreadyHasFluidsOfThatTypeOrIsDedicated)) return 0;
    ...
    public int removeFluidFromConnectedTank(byte aSide, FluidStack aFluid, boolean aOnlyRemoveIfItCanRemoveAllAtOnce) {
        if (mTank.contains(aFluid) && mTank.has(aOnlyRemoveIfItCanRemoveAllAtOnce ? aFluid.amount : 1))
    ...
    public boolean isFluidAllowed(FluidStack aFluid) {
        return aFluid != null && !FL.powerconducting(aFluid)
            && (FL.gas(aFluid) ? mGasProof : mLiquidProof)
            && (mAcidProof || !FL.acid(aFluid))
            && (mPlasmaProof || !FL.plasma(aFluid))
            && (mMagicProof || !FL.magic(aFluid))
            && FL.temperature(aFluid) <= mTemperatureMax;
    }

    @Override
    public byte getMaxStackSize(ItemStack aStack, byte aDefault) {
        return mTank.has() ? 1 : aDefault;
    }
