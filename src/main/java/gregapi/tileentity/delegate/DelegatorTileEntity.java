...
public final class DelegatorTileEntity<T> extends WorldAndCoords {
    ...
    public AxisAlignedBB box(double aMinX, double aMinY, double aMinZ, double aMaxX, double aMaxY, double aMaxZ) {
        return AxisAlignedBB.getBoundingBox(mX + aMinX, mY + aMinY, mZ + aMinZ, mX + aMaxX, mY + aMaxY, mZ + aMaxZ);
    }

    public AxisAlignedBB box(double[] aBox) {
        return AxisAlignedBB
            .getBoundingBox(mX + aBox[0], mY + aBox[1], mZ + aBox[2], mX + aBox[3], mY + aBox[4], mZ + aBox[5]);
    }
    ...
    public AxisAlignedBB box() {
        return AxisAlignedBB.getBoundingBox(mX, mY, mZ, mX + 1, mY + 1, mZ + 1);
    }

    public ForgeDirection getForgeSideOfTileEntity() {
        return FORGE_DIR[mSideOfTileEntity];
    }

    public Block getBlock() {
        return mWorld.getBlock(mX, mY, mZ);
    }

    public byte getMetaData() {
        return UT.Code.bind4(mWorld.getBlockMetadata(mX, mY, mZ));
    }

    public boolean setBlock(Block aBlock) {
        return mWorld.setBlock(mX, mY, mZ, aBlock, 0, 3);
    }

    public boolean setBlock(Block aBlock, int aMetaData) {
        return mWorld.setBlock(mX, mY, mZ, aBlock, UT.Code.bind4(aMetaData), 3);
    }

    public boolean setBlock(Block aBlock, int aMetaData, int aFlags) {
        return mWorld.setBlock(mX, mY, mZ, aBlock, UT.Code.bind4(aMetaData), aFlags);
    }

    public boolean setMetaData(int aMetaData) {
        return mWorld.setBlockMetadataWithNotify(mX, mY, mZ, UT.Code.bind4(aMetaData), 3);
    }

    public boolean setMetaData(int aMetaData, int aFlags) {
        return mWorld.setBlockMetadataWithNotify(mX, mY, mZ, UT.Code.bind4(aMetaData), aFlags);
    }

    public boolean hasCollisionBox() {
        return mWorld != null && WD.hasCollide(mWorld, mX, mY, mZ);
    }

    public boolean equalCoords(DelegatorTileEntity<?> aOther) {
        return aOther.mX == mX && aOther.mY == mY && aOther.mZ == mZ;
    }

    public boolean equalSideAndCoords(DelegatorTileEntity<?> aOther) {
        return aOther.mSideOfTileEntity == mSideOfTileEntity && equalCoords(aOther);
    }

    public boolean equalSideWorldAndCoords(DelegatorTileEntity<?> aOther) {
        return aOther.mWorld == mWorld && equalSideAndCoords(aOther);
    }

    public boolean equalSideTileEntityAndCoords(DelegatorTileEntity<?> aOther) {
        return aOther.mTileEntity == mTileEntity && equalSideAndCoords(aOther);
    }

    public boolean exists() {
        return mTileEntity instanceof ITileEntityUnloadable ? !((ITileEntityUnloadable) mTileEntity).isDead()
            : mTileEntity != null && !((TileEntity) mTileEntity).isInvalid()
                && mWorld != null
                && mWorld.blockExists(mX, mY, mZ);
    }
