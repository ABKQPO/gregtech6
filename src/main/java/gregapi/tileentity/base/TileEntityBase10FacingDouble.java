...
public abstract class TileEntityBase10FacingDouble extends TileEntityBase09FacingSingle {
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.DGRAY + LH.get(LH.TOOL_TO_SET_FACING2_PRE)
                + LH.get(TOOL_LOCALISER_PREFIX + TOOL_monkeywrench, "Monkey Wrench")
                + LH.get(LH.TOOL_TO_SET_FACING2_POST));
    }
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        mFacing = (useInversePlacementRotation(aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)
        ...
        mSecondFacing = (useInversePlacementRotation(aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)
            ? UT.Code.getSideForPlayerPlacing(aPlayer, mSecondFacing, getValidSecondSides())
            : UT.Code.getOppositeSideForPlayerPlacing(aPlayer, mSecondFacing, getValidSecondSides()));
    ...
    public void onFacingChange(byte aPreviousFacing, byte aPreviousSecondFacing) {
        onFacingChange(aPreviousFacing);
    }

    @Override
    public byte getDirectionData() {
        return (byte) (mFacing | (mSecondFacing << 3));
    }

    @Override
    public void setDirectionData(byte aData) {
        mFacing = (byte) (aData & 7);
        mSecondFacing = (byte) ((aData >>> 3) & 7);
    }
    ...
    public boolean isConnectedWrenchingOverlay(ItemStack aStack, byte aSide) {
        return aSide == mFacing || aSide == mSecondFacing;
    }

    // Stuff to Override
    public byte getDefaultSecondSide() {
        return OPOS[getDefaultSide()];
    }
