 ...
 */
public abstract class TileEntityBase09FacingSingle extends TileEntityBase08Directional
    implements IMTE_OnPlaced, IMTE_AddToolTips {
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        if (getFacingTool() != null) aList.add(
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (getFacingTool() != null && aTool.equals(getFacingTool())) {
            ...
            if (getValidSides()[aTargetSide]) {
                byte oFacing = mFacing;
                mFacing = aTargetSide;
                updateClientData();
                causeBlockUpdate();
                onFacingChange(oFacing);
                return 10000;
            }
        }
        return 0;
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        mFacing = useSidePlacementRotation(aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)
            ? useInversePlacementRotation(aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)
                ? getValidSides()[OPOS[aSide]] ? OPOS[aSide] : getDefaultSide()
                : getValidSides()[aSide] ? aSide : getDefaultSide()
            : (useInversePlacementRotation(aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)
    ...
    public void setFacing(short aFacing) {
        setPrimaryFacing(UT.Code.side(aFacing));
    }

    @Override
    public boolean wrenchCanSetFacing(EntityPlayer aPlayer, int aSide) {
        return TOOL_wrench.equals(getFacingTool()) && getValidSides()[aSide]
            && (aPlayer == null || aPlayer.getHeldItem() == null
                || !ItemsGT.SPECIAL_CASE_TOOLS.contains(aPlayer.getHeldItem(), T));
    }
    ...
    public void setPrimaryFacing(byte aFacing) {
        if (isClientSide() || aFacing == mFacing) return;
        byte oFacing = mFacing;
        mFacing = aFacing;
        updateClientData();
        causeBlockUpdate();
    ...
    public boolean useSidePlacementRotation(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        return useSidePlacementRotation();
    }
    ...
    public boolean useInversePlacementRotation(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        int aZ, byte aSide, float aHitX, float aHitY, float aHitZ) {
        return useInversePlacementRotation();
    }
}
