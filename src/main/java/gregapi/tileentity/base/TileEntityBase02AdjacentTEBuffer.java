 ...
 * 
 *         Improved TileEntity with less Lag in regards of getting adjacent TileEntities, a Timer and a Coordinate Check
 *         in case it gets moved by Frames or something.
 */
public abstract class TileEntityBase02AdjacentTEBuffer extends TileEntityBase01Root implements ITileEntityNeedsSaving {
    ...
    public TileEntity getTileEntityAtSideAndDistance(byte aSide, int aDistance) {
        ...
        if (tChunksCrossed && (!(mBufferedTileEntities[aSide] instanceof ITileEntityUnloadable)
            || ((ITileEntityUnloadable) mBufferedTileEntities[aSide]).isDead())) {
        ...
        if (mBufferedTileEntities[aSide] == null) {
            ...
            if (tTileEntity.xCoord == tCoords.posX && tTileEntity.yCoord == tCoords.posY
                && tTileEntity.zCoord == tCoords.posZ) return mBufferedTileEntities[aSide] = tTileEntity;
            mBufferedTileEntities[aSide] = null;
            return tTileEntity;
        }
        if (mBufferedTileEntities[aSide].xCoord != tCoords.posX || mBufferedTileEntities[aSide].yCoord != tCoords.posY
            || mBufferedTileEntities[aSide].zCoord != tCoords.posZ) {
            mBufferedTileEntities[aSide] = null;
