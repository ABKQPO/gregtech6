 ...
 */
public abstract class TileEntityBase05Inventories extends TileEntityBase04MultiTileEntities
    implements IInventory, ITileEntityInventoryGUI, IMTE_OnBlockExploded, IMTE_BreakBlock {
    ...
    public void writeToNBT2(NBTTagCompound aNBT) {
        if (mInventory != null && mInventory.length > 0) {
            ...
            for (short tSlot = 0; tSlot < mInventory.length; tSlot++) if (ST.valid(mInventory[tSlot]) && canSave(tSlot))
                tList.appendTag(UT.NBT.makeShort(ST.save(mInventory[tSlot]), "s", tSlot));
    ...
    public NBTTagCompound writeItemNBT(NBTTagCompound aNBT) {
        ...
        if (mInventory != null && mInventory.length > 0) {
            NBTTagList tList = new NBTTagList();
            for (short tSlot = 0; tSlot < mInventory.length; tSlot++)
                if (ST.valid(mInventory[tSlot]) && keepSlot(tSlot))
                    tList.appendTag(UT.NBT.makeShort(ST.save(mInventory[tSlot]), "s", tSlot));
            if (tList.tagCount() > 0) aNBT.setTag(NBT_INV_LIST, tList);
        }
        return aNBT;
    ...
    public final ItemStack slot(int aIndex, ItemStack aStack) {
        return mInventory[aIndex] = aStack;
    }
    ...
    public final ItemStack slotTake(int aIndex) {
        ItemStack rStack = mInventory[aIndex];
    ...
    public final boolean slotNull(int aIndex) {
        if (mInventory[aIndex] != null && mInventory[aIndex].stackSize <= 0) return slotKill(aIndex);
        return F;
    }
    ...
    public final boolean invempty() {
        for (int i = 0; i < mInventory.length; i++) if (mInventory[i] != null) return F;
        return T;
    }
    ...
    public final NBTTagCompound slotNBT(int aIndex) {
        return mInventory[aIndex] != null ? mInventory[aIndex].getTagCompound() : null;
    }
    ...
    public ItemStack decrStackSize(int aSlot, int aDecrement) {
        updateInventory();
        if (mInventory[aSlot] == null || aDecrement <= 0) return NI;
        if (mInventory[aSlot].stackSize <= aDecrement) {
            ItemStack tStack = ST.copy(mInventory[aSlot]);
            if (allowZeroStacks(aSlot)) mInventory[aSlot].stackSize = 0;
            else mInventory[aSlot] = NI;
            return tStack;
        }
        ItemStack rStack = mInventory[aSlot].splitStack(aDecrement);
        if (mInventory[aSlot].stackSize <= 0 && !allowZeroStacks(aSlot)) mInventory[aSlot] = NI;
        return rStack;
    }

    @Override
    public ItemStack getStackInSlotOnClosing(int aSlot) {
        ItemStack rStack = mInventory[aSlot];
        mInventory[aSlot] = null;
        return rStack;
    }

    @Override
    public ItemStack getStackInSlot(int aSlot) {
        return mInventory[aSlot];
    }
    ...
    public int getSizeInventory() {
        return mInventory == null ? 0 : mInventory.length;
    }

    @Override
    public void setInventorySlotContents(int aSlot, ItemStack aStack) {
        updateInventory();
        mInventory[aSlot] = OM.get(aStack);
    }
    ...
    public void removeAllDroppableNullStacks() {
        for (int i = 0; i < mInventory.length; i++)
            if (canDrop(i) && mInventory[i] != null && mInventory[i].stackSize <= 0) mInventory[i] = NI;
    }
    ...
    /**
     * Returns a Stack to be put into that Slot in case of a Mod being uninstalled causing a Loading Error for the
     * original ItemStack. For example Shelves just replace the missing Item with a normal Book instead.
     */
    ...
    public boolean breakBlock() {
        if (isServerSide()) for (short i = 0; i < mInventory.length; i++)
            if (mInventory[i] != null && canDrop(i) && !ST.debug(mInventory[i]) && breakDrop(i)) {
                ItemStack tDumpedStack = ST.amount(
                    UT.Code.bind_(0, 512L * Math.max(1, mInventory[i].getMaxStackSize()), mInventory[i].stackSize),
                    mInventory[i]);
    ...
    public boolean addStackToSlot(int aIndex, ItemStack aStack) {
        ...
        if (ST.equal(tStack, aStack) && tStack.stackSize + aStack.stackSize
            <= Math.min(Math.max(1, tStack.getMaxStackSize()), getInventoryStackLimit())) {
