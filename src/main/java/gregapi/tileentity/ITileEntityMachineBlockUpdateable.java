...
public interface ITileEntityMachineBlockUpdateable {
    ...
    /**
     * Called whenever a Machine Block adjacent to this Block got updated. The Coords are the Coords of the Block that
     * changed.
     */
    ...
    public static class Util {
        ...
        public static boolean causeMachineUpdate(IHasWorldAndCoords aTileEntity, boolean aRemoved) {
            if (aTileEntity.isServerSide()) new Thread(
                new MachineBlockUpdateRunnable(
                    aTileEntity.getWorld(),
                    aTileEntity.getCoords(),
                    aTileEntity.getBlockOffset(0, 0, 0),
                    aTileEntity.getMetaDataOffset(0, 0, 0),
                    aRemoved),
                "Machine Block Updating").start();
            return T;
        }
         ...
         * You should call this Function in @Block.breakBlock and in @Block.onBlockAdded of your Machine.
         */
        public static boolean causeMachineUpdate(World aWorld, int aX, int aY, int aZ, Block aBlock, byte aMeta,
            boolean aRemoved) {
            if (!aWorld.isRemote) new Thread(
                new MachineBlockUpdateRunnable(aWorld, new ChunkCoordinates(aX, aY, aZ), aBlock, aMeta, aRemoved),
                "Machine Block Updating").start();
            return T;
        }
         ...
         * You should call this Function in @Block.breakBlock and in @Block.onBlockAdded of your Machine.
         */
        public static boolean causeMachineUpdate(World aWorld, ChunkCoordinates aCoords, Block aBlock, byte aMeta,
            boolean aRemoved) {
            if (!aWorld.isRemote) new Thread(
                new MachineBlockUpdateRunnable(aWorld, aCoords, aBlock, aMeta, aRemoved),
                "Machine Block Updating").start();
            return T;
        }
        ...
        private static class MachineBlockUpdateRunnable implements Runnable {
            ...
            public MachineBlockUpdateRunnable(World aWorld, ChunkCoordinates aCoords, Block aBlock, byte aMeta,
                boolean aRemoved) {
                mWorld = aWorld;
                mCoords = aCoords;
                mBlock = aBlock;
            ...
            public void run() {
                try {
                    stepToUpdateMachine(mWorld, mCoords, new HashSetNoNulls<>(F, mCoords));
                } catch (Throwable e) {/**/} finally {
                    if (TICK_LOCK.isHeldByCurrentThread()) TICK_LOCK.unlock();
                }
            }

            private void stepToUpdateMachine(World aWorld, ChunkCoordinates aCoords,
                HashSetNoNulls<ChunkCoordinates> aSet) {
                // Wait for the updateEntities Thread to be done because fucking Mojang and race conditions in loading
                // Chunks.
                TICK_LOCK.lock();
                TileEntity tTileEntity = WD.te(aWorld, aCoords, T);
                if (tTileEntity instanceof ITileEntityMachineBlockUpdateable)
                    ((ITileEntityMachineBlockUpdateable) tTileEntity)
                        .onMachineBlockUpdate(mCoords, mBlock, mMeta, mRemoved);
                if (aSet.size() < 5 || tTileEntity instanceof ITileEntityMachineBlockUpdateable
                    || isMachineBlock(
                        aWorld.getBlock(aCoords.posX, aCoords.posY, aCoords.posZ),
                        aWorld.getBlockMetadata(aCoords.posX, aCoords.posY, aCoords.posZ))) {
                    TICK_LOCK.unlock();
                    ChunkCoordinates tCoords;
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX + 1, aCoords.posY, aCoords.posZ)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX - 1, aCoords.posY, aCoords.posZ)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY + 1, aCoords.posZ)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY - 1, aCoords.posZ)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY, aCoords.posZ + 1)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                    if (aSet.add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY, aCoords.posZ - 1)))
                        stepToUpdateMachine(aWorld, tCoords, aSet);
                } else {
