 ...
 */
public abstract class MultiTileEntityTreeHole extends TileEntityBase09FacingSingle
    implements ITileEntityTreeHole, IMTE_IsWood {
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (isServerSide() && hasResin(aSide)) {
            ItemStack tResin = getResinItem(aSide),
                tStack = FL.fill(getResinFluid(aSide), ST.amount(1, aStack), T, T, T, T);
    ...
    public byte getDirectionData() {
        return (byte) ((mFacing & 7) | (mHasResin ? 8 : 0));
    }
    ...
    public boolean hasResin(byte aSide) {
        return aSide == mFacing && mHasResin;
    }

    @Override
    public boolean extractResin(byte aSide) {
        if (!hasResin(aSide)) return F;
        mHasResin = F;
        updateClientData();
        return T;
    }
