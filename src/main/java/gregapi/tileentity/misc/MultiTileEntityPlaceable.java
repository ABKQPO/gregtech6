 ...
 */
public abstract class MultiTileEntityPlaceable extends TileEntityBase03MultiTileEntities
    implements IMTE_SyncDataByteArray, IMTE_CanEntityDestroy, IMTE_GetBlockHardness, IMTE_IsSideSolid,
    IMTE_GetLightOpacity, IMTE_GetExplosionResistance, ITileEntityQuickObstructionCheck,
    ...
    public boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (ST.give(aPlayer, ST.amount(1, mStack), T, worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5)) {
            ...
            for (int i = 1; i < 255; i++) {
                ...
                if (tTileEntity instanceof MultiTileEntityPlaceable
                    && ST.equal(mStack, ((MultiTileEntityPlaceable) tTileEntity).mStack)) {
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        return getClientDataPacketByteArray(
            aSendAll,
            UT.Code.toByteS(mMaterial.mID, 0),
            UT.Code.toByteS(mMaterial.mID, 1),
            mSize);
    }
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return SIDES_HORIZONTAL[aSide] ? mTextureSides : mTextureTop;
    }
