 ...
 * 
 *         In order to create a Basic Machine (Steel Shredder in this example), I use the following NBT Parameters in my
 *         MultiTileEntity System:
 * 
 ...
 */
@Optional.InterfaceList(value = { @Optional.Interface(iface = "buildcraft.api.tiles.IHasWork", modid = ModIDs.BC) })
public class MultiTileEntityBasicMachine extends TileEntityBase09FacingSingle implements IHasWork,
    ITileEntityFunnelAccessible, ITileEntityTapAccessible, ITileEntitySwitchableOnOff, ITileEntityRunningSuccessfully,
    ITileEntityAdjacentInventoryUpdatable, ITileEntityEnergy, ITileEntityProgress, ITileEntityGibbl, IFluidHandler {

    public boolean mSpecialIsStartEnergy = F, mNoConstantEnergy = F, mCheapOverclocking = F, mCouldUseRecipe = F,
        mStopped = F, oActive = F, oRunning = F, mStateNew = F, mStateOld = F, mDisabledItemInput = F,
        mDisabledItemOutput = F, mDisabledFluidInput = F, mDisabledFluidOutput = F, mRequiresIgnition = F,
        mParallelDuration = F, mCanUseOutputTanks = F;
    public byte mEnergyInputs = 127, mEnergyOutput = SIDE_UNDEFINED, mOutputBlocked = 0, mMode = 0, mIgnited = 0;
    public byte mItemInputs = 127, mItemOutputs = 127, mItemAutoInput = SIDE_UNDEFINED,
        mItemAutoOutput = SIDE_UNDEFINED;
    public byte mFluidInputs = 127, mFluidOutputs = 127, mFluidAutoInput = SIDE_UNDEFINED,
        mFluidAutoOutput = SIDE_UNDEFINED;
    public short mEfficiency = 10000;
    public int mParallel = 1;
    public long mEnergy = 0, mInputMin = 16, mInput = 32, mInputMax = 64, mMinEnergy = 0, mOutputEnergy = 0,
        mChargeRequirement = 0;
    public TagData mEnergyTypeAccepted = TD.Energy.TU, mEnergyTypeEmitted = TD.Energy.QU,
        mEnergyTypeCharged = TD.Energy.TU;
    ...
    public IIconContainer[] mTexturesMaterial = L6_IICONCONTAINER, mTexturesInactive = L6_IICONCONTAINER,
        mTexturesActive = L6_IICONCONTAINER, mTexturesRunning = L6_IICONCONTAINER;
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_SPECIAL_IS_START_ENERGY))
            mSpecialIsStartEnergy = aNBT.getBoolean(NBT_SPECIAL_IS_START_ENERGY);
        ...
        if (aNBT.hasKey(NBT_INPUT_MIN)) {
            mInputMin = aNBT.getLong(NBT_INPUT_MIN);
        }
        if (aNBT.hasKey(NBT_INPUT_MAX)) {
            mInputMax = aNBT.getLong(NBT_INPUT_MAX);
        }
        if (aNBT.hasKey(NBT_MINENERGY)) {
            mMinEnergy = aNBT.getLong(NBT_MINENERGY);
        }
        if (aNBT.hasKey(NBT_PARALLEL)) {
            mParallel = Math.max(1, aNBT.getInteger(NBT_PARALLEL));
        }
        ...
        if (aNBT.hasKey(NBT_PROGRESS)) {
            mProgress = aNBT.getLong(NBT_PROGRESS);
        }
        if (aNBT.hasKey(NBT_MAXPROGRESS)) {
        ...
        if (aNBT.hasKey(NBT_ENERGY_ACCEPTED))
            mEnergyTypeAccepted = TagData.createTagData(aNBT.getString(NBT_ENERGY_ACCEPTED));
        if (aNBT.hasKey(NBT_ENERGY_ACCEPTED_2))
            mEnergyTypeCharged = TagData.createTagData(aNBT.getString(NBT_ENERGY_ACCEPTED_2));
        if (aNBT.hasKey(NBT_ENERGY_ACCEPTED_SIDES))
            mEnergyInputs = (byte) (aNBT.getByte(NBT_ENERGY_ACCEPTED_SIDES) | SBIT_A);
        if (aNBT.hasKey(NBT_ENERGY_EMITTED))
            mEnergyTypeEmitted = TagData.createTagData(aNBT.getString(NBT_ENERGY_EMITTED));
        ...
        for (int i = 0; i < mTanksInput.length; i++)
            mTanksInput[i] = new FluidTankGT(tCapacity).setCapacity(mRecipes, mParallel * 2L)
                .readFromNBT(aNBT, NBT_TANK + ".in." + i);
        mTanksOutput = new FluidTankGT[mRecipes.mOutputFluidCount];
        for (int i = 0; i < mTanksOutput.length; i++)
            mTanksOutput[i] = new FluidTankGT().readFromNBT(aNBT, NBT_TANK + ".out." + i);
            ...
            if (GT_API.sBlockIcons == null && aNBT.hasKey(NBT_TEXTURE)) {
                ...
                mTexturesActive = new IIconContainer[] {
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/bottom"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/top"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/left"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/front"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/right"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_active/back") };
                mTexturesRunning = new IIconContainer[] {
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/bottom"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/top"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/left"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/front"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/right"),
                    new Textures.BlockIcons.CustomIcon(
                        "machines/basicmachines/" + tTextureName + "/overlay_running/back") };
            } else {
                TileEntity tCanonicalTileEntity = MultiTileEntityRegistry
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        aList.add(
            Chat.CYAN + LH.get(LH.RECIPES)
                + ": "
                + Chat.WHITE
                + LH.get(mRecipes.mNameInternal)
                + (mParallel > 1 ? " (up to " + mParallel + "x processed per run)" : ""));

        if (mCheapOverclocking) aList.add(Chat.YELLOW + LH.get(LH.CHEAP_OVERCLOCKING));
        if (mEfficiency != 10000) aList.add(LH.getToolTipEfficiency(mEfficiency));
    ...
    public void addToolTipsSided(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (mEnergyTypeAccepted != TD.Energy.TU) {
            if (mEnergyInputs != 127) {
                for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mEnergyInputs]) {
                    tSideNames += (UT.Code.stringValid(tSideNames) ? ", " : "") + LH.get(LH.FACES[tSide]);
                }
            }
        ...
        if (mRecipes.mInputItemsCount > 0) {
            if (mItemInputs != 127) {
                for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mItemInputs]) {
                    tSideNames += (UT.Code.stringValid(tSideNames) ? ", " : "") + LH.get(LH.FACES[tSide])
                        + (tSide == mItemAutoInput ? " (auto)" : "");
                }
                if (UT.Code.stringValid(tSideNames))
                    aList.add(Chat.GREEN + LH.get(LH.ITEM_INPUT) + ": " + Chat.WHITE + tSideNames);
                tSideNames = "";
            } else if (SIDES_VALID[mItemAutoInput]) {
                aList.add(
                    Chat.GREEN + LH.get(LH.ITEM_INPUT)
                        + ": "
                        + Chat.WHITE
                        + LH.get(LH.FACES[mItemAutoInput])
                        + " (auto, otherwise any)");
            } else {
        ...
        if (mRecipes.mOutputItemsCount > 0) {
            if (mItemOutputs != 127) {
                for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mItemOutputs]) {
                    tSideNames += (UT.Code.stringValid(tSideNames) ? ", " : "") + LH.get(LH.FACES[tSide])
                        + (tSide == mItemAutoOutput ? " (auto)" : "");
                }
                if (UT.Code.stringValid(tSideNames))
                    aList.add(Chat.RED + LH.get(LH.ITEM_OUTPUT) + ": " + Chat.WHITE + tSideNames);
                tSideNames = "";
            } else if (SIDES_VALID[mItemAutoOutput]) {
                aList.add(
                    Chat.RED + LH.get(LH.ITEM_OUTPUT)
                        + ": "
                        + Chat.WHITE
                        + LH.get(LH.FACES[mItemAutoOutput])
                        + " (auto, otherwise any)");
            } else {
        ...
        if (mRecipes.mInputFluidCount > 0) {
            if (mFluidInputs != 127) {
                for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mFluidInputs]) {
                    tSideNames += (UT.Code.stringValid(tSideNames) ? ", " : "") + LH.get(LH.FACES[tSide])
                        + (tSide == mFluidAutoInput ? " (auto)" : "");
                }
                if (UT.Code.stringValid(tSideNames))
                    aList.add(Chat.GREEN + LH.get(LH.FLUID_INPUT) + ": " + Chat.WHITE + tSideNames);
                tSideNames = "";
            } else if (SIDES_VALID[mFluidAutoInput]) {
                aList.add(
                    Chat.GREEN + LH.get(LH.FLUID_INPUT)
                        + ": "
                        + Chat.WHITE
                        + LH.get(LH.FACES[mFluidAutoInput])
                        + " (auto, otherwise any)");
            } else {
        ...
        if (mRecipes.mOutputFluidCount > 0) {
            if (mFluidOutputs != 127) {
                for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mFluidOutputs]) {
                    tSideNames += (UT.Code.stringValid(tSideNames) ? ", " : "") + LH.get(LH.FACES[tSide])
                        + (tSide == mFluidAutoOutput ? " (auto)" : "");
                }
                if (UT.Code.stringValid(tSideNames))
                    aList.add(Chat.RED + LH.get(LH.FLUID_OUTPUT) + ": " + Chat.WHITE + tSideNames);
            } else if (SIDES_VALID[mFluidAutoOutput]) {
                aList.add(
                    Chat.RED + LH.get(LH.FLUID_OUTPUT)
                        + ": "
                        + Chat.WHITE
                        + LH.get(LH.FACES[mFluidAutoOutput])
                        + " (auto, otherwise any)");
            } else {
    ...
    public long onToolClick3(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        List<String> aChatReturn, IInventory aPlayerInventory, boolean aSneaking, ItemStack aStack, byte aSide,
        float aHitX, float aHitY, float aHitZ, ChunkCoordinates aFrom) {
        return onToolClick2(
            aTool,
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        long rReturn = super.onToolClick2(
            aTool,
            aRemainingDurability,
            aQuality,
            aPlayer,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            ...
            aChatReturn.add("========================================");
            aChatReturn.add(
                (mMode & 1) != 0 ? "Only produce when Output is completely empty" : "Produce whenever there is space");
            aChatReturn
                .add((mMode & 2) != 0 ? "Only accept Input on empty Input Slots" : "Accept Input on all Input Slots");
            updateInventory();
            return 10000;
        }
    ...
    public void onMagnifyingGlass(List<String> aChatReturn) {
        aChatReturn
            .add((mMode & 1) != 0 ? "Only produce when Output is completely empty" : "Produce whenever there is space");
        aChatReturn
            .add((mMode & 2) != 0 ? "Only accept Input on empty Input Slots" : "Accept Input on all Input Slots");
        if (SIDES_VALID[mItemAutoInput])
            aChatReturn.add(mDisabledItemInput ? "Auto Item Input Disabled" : "Auto Item Input Enabled");
        if (SIDES_VALID[mItemAutoOutput])
            aChatReturn.add(mDisabledItemOutput ? "Auto Item Output Disabled" : "Auto Item Output Enabled");
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            ...
            for (int i = 0; i < mTanksOutput.length; i++) slot(
                mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount + 1 + i + mTanksInput.length,
                FL.display(mTanksOutput[i], T, T));
        }
    }
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        ...
        if (aEnergyType == mEnergyTypeAccepted) {
            ...
            long tInput = Math.min(mInputMax - mEnergy, aSize * aAmount),
                tConsumed = Math.min(aAmount, (tInput / aSize) + (tInput % aSize != 0 ? 1 : 0));
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return aEmitting ? aEnergyType == mEnergyTypeEmitted
            : aEnergyType == mEnergyTypeAccepted || aEnergyType == mEnergyTypeCharged;
    }

    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mStopped) && FACE_CONNECTED[FACING_ROTATIONS[mFacing][aSide]][mEnergyInputs]
            && super.isEnergyAcceptingFrom(aEnergyType, aSide, aTheoretical);
    }

    @Override
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mStopped)
            && (SIDES_INVALID[mEnergyOutput] || FACING_ROTATIONS[mFacing][aSide] == mEnergyOutput)
            && super.isEnergyEmittingTo(aEnergyType, aSide, aTheoretical);
    }
    ...
    public ItemStack[] getDefaultInventory(NBTTagCompound aNBT) {
        ...
        return new ItemStack[mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount
            + 1
            + mRecipes.mInputFluidCount
            + mRecipes.mOutputFluidCount];
    }
    ...
    public void updateAccessibleSlots() {
        for (byte i = 0; i < ACCESSIBLE.length; i++) {
            if (FACE_CONNECTED[FACING_ROTATIONS[mFacing][i]][mItemInputs]) {
                if (FACE_CONNECTED[FACING_ROTATIONS[mFacing][i]][mItemOutputs]) ACCESSIBLE[i] = ACCESSIBLE_SLOTS;
                else ACCESSIBLE[i] = ACCESSIBLE_INPUTS;
            } else {
                if (FACE_CONNECTED[FACING_ROTATIONS[mFacing][i]][mItemOutputs]) ACCESSIBLE[i] = ACCESSIBLE_OUTPUTS;
                else ACCESSIBLE[i] = ZL_INTEGER;
            }
        }
    }
    ...
    public int[] getAccessibleSlotsFromSide2(byte aSide) {
        return ACCESSIBLE[aSide];
    }

    @Override
    public boolean canDrop(int aInventorySlot) {
        return aInventorySlot < mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount + 1;
    }
    ...
    public IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        if (!mDisabledFluidOutput && SIDES_VALID[mFluidAutoOutput]
            && FACING_TO_SIDE[mFacing][mFluidAutoOutput] == aSide) return null;
        ...
        if (!mRecipes.containsInput(aFluidToFill, this, slot(mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount)))
            return null;
    ...
    public IFluidTank getFluidTankDrainable2(byte aSide, FluidStack aFluidToDrain) {
        ...
        if (aFluidToDrain == null) {
            for (int i = 0, j; i < mTanksOutput.length; i++)
                if (mTanksOutput[j = ((int) (SERVER_TIME / 20) + i) % mTanksOutput.length].has())
                    return mTanksOutput[j];
        } else {
            for (int i = 0; i < mTanksOutput.length; i++)
                if (mTanksOutput[i].contains(aFluidToDrain)) return mTanksOutput[i];
        }
        return null;
    }
    ...
    public int canOutput(Recipe aRecipe) {
        ...
        if (mParallelDuration) {
            // Ugh, I do not feel like Maths right now, but the previous incarnation of this seemed a tiny bit wrong, so
            // I will make sure it works properly.
            while (rMaxTimes > 1 && aRecipe.getAbsoluteTotalPower() * rMaxTimes > mInputMax * 600) rMaxTimes--;
        }

        for (int i = 0, j = mRecipes.mInputItemsCount; i < mRecipes.mOutputItemsCount
            && i < aRecipe.mOutputs.length; i++, j++) if (ST.valid(aRecipe.mOutputs[i])) {
                if (slotHas(j)) {
                    ...
                    rMaxTimes = Math.min(
                        rMaxTimes,
                        (slot(j).getMaxStackSize() - slot(j).stackSize) / aRecipe.mOutputs[i].stackSize);
                    if (rMaxTimes <= 0) {
                        mOutputBlocked++;
                        return 0;
                    }
        ...
        if (aRecipe.mFluidOutputs.length > 0) {
            ...
            for (int i = 0; i < mTanksOutput.length; i++) if (mTanksOutput[i].isEmpty()) tEmptyOutputTanks++;
            else if (aRecipe.mNeedsEmptyOutput || (mMode & 1) != 0) return 0;
            // This optimisation would not work! The Tanks would not have an Output Amount Limiter if this was in the
            // Code!
            // if (tRequiredEmptyTanks <= tEmptyOutputTanks) {
            for (int j = 0; j < aRecipe.mFluidOutputs.length; j++) {
                if (aRecipe.mFluidOutputs[j] == null) {
                    tRequiredEmptyTanks--;
                } else for (int i = 0; i < mTanksOutput.length; i++)
                    if (mTanksOutput[i].contains(aRecipe.mFluidOutputs[j])) {
                        if (mTanksOutput[i].has(Math.max(16000, 1 + aRecipe.mFluidOutputs[j].amount * mParallel))
                            && !FluidsGT.VOID_OVERFLOW.contains(
                                aRecipe.mFluidOutputs[j].getFluid()
                                    .getName()))
                            return 0;
                        tRequiredEmptyTanks--;
                        break;
    ...
    public static final int DID_NOT_FIND_RECIPE = 0, FOUND_RECIPE_BUT_DID_NOT_MEET_REQUIREMENTS = 1,
        FOUND_AND_SUCCESSFULLY_USED_RECIPE = 2, FOUND_AND_COULD_HAVE_USED_RECIPE = 3;
    ...
    public int checkRecipe(boolean aApplyRecipe, boolean aUseAutoIO) {
        ...
        if (aUseAutoIO && !mDisabledFluidInput && SIDES_VALID[tAutoInput]) {
            ...
            if (tTileEntity != null && tTileEntity.mTileEntity != null) {
                ...
                if (tInfos != null) for (FluidTankInfo tInfo : tInfos) if (tInfo != null && tInfo.fluid != null
                    && tInfo.fluid.amount > 0
                    && getFluidTankFillable(SIDE_ANY, tInfo.fluid) != null) {
        ...
        Recipe tRecipe = mRecipes.findRecipe(
            this,
            mLastRecipe,
            F,
            mEnergyTypeAccepted == TD.Energy.RF ? mInputMax / RF_PER_EU : mInputMax,
            slot(mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount),
            mTanksInput,
            tInputs);
        ...
        if (tRecipe == null) {
            ...
            if (tMaxProcessCount > 1) {
                if (!mParallelDuration && mEnergyTypeAccepted != TD.Energy.TU) tMaxProcessCount = (int) UT.Code.bind(
                    1,
                    tMaxProcessCount,
                    mInput
                        / Math.max(1, (mEnergyTypeAccepted == TD.Energy.RF ? tRecipe.mEUt * RF_PER_EU : tRecipe.mEUt)));
        ...
        for (byte tSide : ALL_SIDES_VALID_FIRST[FACING_TO_SIDE[mFacing][mItemAutoInput]])
            if (FACE_CONNECTED[FACING_ROTATIONS[mFacing][tSide]][mItemInputs]) {
                ...
                if (tDelegator != null && tDelegator.mTileEntity instanceof ITileEntityAdjacentInventoryUpdatable) {
                    ((ITileEntityAdjacentInventoryUpdatable) tDelegator.mTileEntity)
                        .adjacentInventoryUpdated(tDelegator.mSideOfTileEntity, this);
                }
            }
        ...
        if (tRecipe.mEUt < 0) {
            ...
            if (mParallelDuration) {
                mMinEnergy = Math
                    .max(1, (mEnergyTypeAccepted == TD.Energy.RF ? tRecipe.mEUt * RF_PER_EU : tRecipe.mEUt));
                mMaxProgress = Math.max(
                    1,
                    UT.Code
                        .units(mMinEnergy * Math.max(1, tRecipe.mDuration) * tMaxProcessCount, mEfficiency, 10000, T));
            } else {
                mMinEnergy = Math.max(
                    1,
                    (mEnergyTypeAccepted == TD.Energy.RF ? tRecipe.mEUt * RF_PER_EU * tMaxProcessCount
                        : mEnergyTypeAccepted == TD.Energy.TU ? tRecipe.mEUt : tRecipe.mEUt * tMaxProcessCount));
                mMaxProgress = Math
                    .max(1, UT.Code.units(mMinEnergy * Math.max(1, tRecipe.mDuration), mEfficiency, 10000, T));
            }
            if (!mCheapOverclocking) while (mMinEnergy < mInputMin && mMinEnergy * 4 <= mInputMax) {
                mMinEnergy *= 4;
                mMaxProgress *= 2;
            }
        }
    ...
    public boolean doActive(long aTimer, long aEnergy) {
        ...
        if (mMaxProgress <= 0) {
            // Successfully produced something or just got ignited || Some Inventory Stuff changes || The Machine has
            // just been turned ON || Check once every Minute
            if ((mIgnited > 0 || mInventoryChanged || !mRunning || aTimer % 1200 == 5)
                && checkRecipe(!mStopped, T) == FOUND_AND_SUCCESSFULLY_USED_RECIPE) {
                onProcessStarted();
            } else {
                mProgress = 0;
            }
        ...
        if (mMaxProgress > 0 && !(mSpecialIsStartEnergy && mChargeRequirement > 0)) {
            ...
            if (mProgress >= mMaxProgress
                && (mStateOld && !mStateNew || !TD.Energy.ALL_ALTERNATING.contains(mEnergyTypeAccepted))) {
                for (int i = 0; i < mOutputItems.length; i++) if (mOutputItems[i] != null
                    && addStackToSlot(mRecipes.mInputItemsCount + (i % mRecipes.mOutputItemsCount), mOutputItems[i])) {
                        mSuccessful = T;
                        mIgnited = 40;
                        mOutputItems[i] = null;
                        continue;
                ...
                for (int i = 0; i < mOutputFluids.length; i++)
                    if (mOutputFluids[i] != null) for (int j = 0; j < mTanksOutput.length; j++) {
                ...
                if (UT.Code.containsSomething(mOutputItems) || UT.Code.containsSomething(mOutputFluids)) {
                    ...
                    mProgress -= mMaxProgress; // this way the leftover energy can be used on the next processed thing,
                                               // unless it gets stuck on an output.
                    mMinEnergy = 0;
                    mMaxProgress = 0;
                    mOutputEnergy = 0;
                    mChargeRequirement = 0;
                    ...
                    for (byte tSide : ALL_SIDES_VALID_FIRST[FACING_TO_SIDE[mFacing][mItemAutoOutput]])
                        if (FACE_CONNECTED[FACING_ROTATIONS[mFacing][tSide]][mItemOutputs]) {
                            ...
                            if (tDelegator != null
                                && tDelegator.mTileEntity instanceof ITileEntityAdjacentInventoryUpdatable) {
                                ((ITileEntityAdjacentInventoryUpdatable) tDelegator.mTileEntity)
                                    .adjacentInventoryUpdated(tDelegator.mSideOfTileEntity, this);
                            }
                        }
        ...
        if (!mDisabledItemOutput && SIDES_VALID[mItemAutoOutput]) {
            ...
            for (int i = mRecipes.mInputItemsCount, j = i + mRecipes.mOutputItemsCount; i < j; i++) if (slotHas(i)) {
                tOutputEmpty = F;
                break;
            }

            // Output not Empty && (Successfully produced something or just got ignited || Some Inventory Stuff changes
            // || The Machine has just been turned ON || Output has been blocked since 256 active ticks || Check once
            // every 10 Seconds)
            if (!tOutputEmpty
                && (mIgnited > 0 || mInventoryChanged || !mRunning || mOutputBlocked == 1 || aTimer % 200 == 5)) {
                boolean tInventoryChanged = mInventoryChanged;
                mInventoryChanged = F;
                doOutputItems();
                if (mInventoryChanged) mOutputBlocked = 0;
                else mInventoryChanged |= tInventoryChanged;
            }
    ...
    public int funnelFill(byte aSide, FluidStack aFluid, boolean aDoFill) {
        ...
        if (!mRecipes.containsInput(aFluid, this, slot(mRecipes.mInputItemsCount + mRecipes.mOutputItemsCount)))
            return 0;
    ...
    public boolean doSoundInterrupt() {
        return UT.Sounds.send(
            mRequiresIgnition ? SFX.MC_FIZZ : mNoConstantEnergy ? SFX.IC_MACHINE_INTERRUPT : SFX.MC_CLICK,
            this,
            F);
    }
    ...
    public void doOutputFluids() {
        for (FluidTankGT tCheck : mTanksOutput) if (tCheck.has()) {
            if (FL.move(tCheck, getFluidOutputTarget(FACING_TO_SIDE[mFacing][mFluidAutoOutput], tCheck.fluid())) > 0)
                updateInventory();
        }
    }

    public void doOutputEnergy() {
        ITileEntityEnergy.Util
            .emitEnergyToSide(mEnergyTypeEmitted, FACING_TO_SIDE[mFacing][mEnergyOutput], mOutputEnergy, 1, this);
    }
    ...
    public Object getGUIClient2(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerClientBasicMachine(aPlayer.inventory, this, mRecipes, aGUIID, mGUITexture);
    }

    @Override
    public Object getGUIServer2(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerCommonBasicMachine(aPlayer.inventory, this, mRecipes, aGUIID);
    }

    @Override
    public byte getVisualData() {
        return (byte) ((mActive ? 1 : 0) | (mRunning ? 2 : 0));
    }
    ...
    public boolean[] getValidSides() {
        return mActive ? SIDES_THIS[mFacing] : SIDES_HORIZONTAL;
    }

    @Override
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? BlockTextureMulti.get(
            BlockTextureDefault.get(mTexturesMaterial[FACING_ROTATIONS[mFacing][aSide]], mRGBa),
            BlockTextureDefault.get(
                (mActive || worldObj == null ? mTexturesActive
                    : mRunning ? mTexturesRunning : mTexturesInactive)[FACING_ROTATIONS[mFacing][aSide]]))
            : null;
    }

    @Override
    public boolean canSave(int aSlot) {
        return !IL.Display_Fluid.equal(slot(aSlot), T, T);
    }

    @Override
    public boolean hasWork() {
        return mMaxProgress > 0 || mChargeRequirement > 0;
    }

    @Override
    public long getProgressValue(byte aSide) {
        return mSuccessful ? getProgressMax(aSide) : mMinEnergy < 1 ? mProgress : UT.Code.divup(mProgress, mMinEnergy);
    }

    @Override
    public long getProgressMax(byte aSide) {
        return Math.max(1, mMinEnergy < 1 ? mMaxProgress : UT.Code.divup(mMaxProgress, mMinEnergy));
    }

    @Override
    public long getGibblValue(byte aSide) {
        long rGibbl = 0;
        for (int i = 0; i < mTanksInput.length; i++) rGibbl += mTanksInput[i].amount();
        return rGibbl;
    }
    ...
    public long getGibblMax(byte aSide) {
        long rGibbl = 0;
        for (int i = 0; i < mTanksInput.length; i++) rGibbl += mTanksInput[i].capacity();
        return rGibbl;
    }

    @Override
    public boolean getStateRunningPossible() {
        return mCouldUseRecipe || mActive
            || mMaxProgress > 0
            || mChargeRequirement > 0
            || (mIgnited > 0 && !mDisabledItemOutput && mOutputBlocked != 0);
    }
