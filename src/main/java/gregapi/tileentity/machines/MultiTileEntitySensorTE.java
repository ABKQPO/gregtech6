...
public abstract class MultiTileEntitySensorTE extends MultiTileEntitySensor
    implements ITileEntityComputerizable, ITileEntityServerTickPost {

    public static final byte MODE_COUNT = 8

        , MODE_DISPLAY = 0, MODE_PERCENT = 1, MODE_GREATER = 2, MODE_EQUAL = 3, MODE_SMALLER = 4, MODE_SCALE = 5,
        MODE_FULL = 6, MODE_NOT_FULL = 7;
        ...
        if (getTickRate() < 2 || SERVER_TIME % getTickRate() == 0) {
            ...
            if (tDelegator.mTileEntity instanceof MultiTileEntityMultiBlockPart) {
                if (((MultiTileEntityMultiBlockPart) tDelegator.mTileEntity).mTarget != null) {
                    tDelegator = new DelegatorTileEntity<>(
                        (TileEntity) ((MultiTileEntityMultiBlockPart) tDelegator.mTileEntity).mTarget,
            ...
            switch (mMode & 127) {
                case MODE_DISPLAY:
                    mDisplayedNumber = UT.Code.bindInt(mCurrentValue);
                    break;
                case MODE_GREATER:
                    tRedstone = (byte) (mCurrentValue > mSetNumber ? 15 : 0);
                    break;
                case MODE_EQUAL:
                    tRedstone = (byte) (mCurrentValue == mSetNumber ? 15 : 0);
                    break;
                case MODE_SMALLER:
                    ...
                    break;
                case MODE_SCALE:
                    tRedstone = (byte) UT.Code.scale(mCurrentValue, mSetNumber, 15, F);
                    break;
                case MODE_PERCENT:
                    tRedstone = (byte) UT.Code.scale(
                        mDisplayedNumber = (mCurrentMax > 0 ? UT.Code.bindInt((mCurrentValue * 100L) / mCurrentMax)
                            : 0),
                        100L,
                        15,
                        F);
                    break;
                case MODE_FULL:
                    tRedstone = (byte) (mCurrentValue >= mCurrentMax ? 15 : 0);
                    break;
                case MODE_NOT_FULL:
                    tRedstone = (byte) (mCurrentValue >= mCurrentMax ? 0 : 15);
                    break;
            }
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (aSide == mFacing) {
            if (isServerSide()) {
                if ((mMode & 127) != MODE_DISPLAY && (mMode & 127) != MODE_PERCENT
                    && (mMode & 127) != MODE_FULL
                    && (mMode & 127) != MODE_NOT_FULL) {
                    float[] tCoords = UT.Code.getFacingCoordsClicked(aSide, aHitX, aHitY, aHitZ);
                    if (mMode < 0) {
                        if (tCoords[1] >= PX_P[6] && tCoords[1] <= PX_P[8]) {
                            if (tCoords[0] >= PX_P[9] && tCoords[0] <= PX_P[11]) {
                                oDisplayedNumber = Short.MIN_VALUE;
                                mSetNumber -= 256;
                                playClick();
                                mSetNumber = UT.Code.bind16(mSetNumber);
                                return T;
                            }
                            if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {
                                oDisplayedNumber = Short.MIN_VALUE;
                                mSetNumber += 256;
                                playClick();
                                mSetNumber = UT.Code.bind16(mSetNumber);
                        ...
                        if (tCoords[1] >= PX_P[6] && tCoords[1] <= PX_P[8]) {
                            if (tCoords[0] >= PX_P[9] && tCoords[0] <= PX_P[11]) {
                                ...
                                mSetNumber -= 100;
                                playClick();
                                mSetNumber = (int) UT.Code.bind(0, 9999, mSetNumber);
                                return T;
                            }
                            if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {
                                oDisplayedNumber = Short.MIN_VALUE;
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        long tDamage = super.onToolClick2(
            aTool,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            ...
            if (tCoords[1] >= PX_P[6] && tCoords[1] <= PX_P[8]) {
                if (tCoords[0] >= PX_P[9] && tCoords[0] <= PX_P[11]) {
                    if (mMode < 0)
                        mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 256)];
                    else mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 100)];
                    playClick();
                    if (aChatReturn != null) aChatReturn.add("Averaging over: " + mValues.length + " Values");
                    return 1000;
                }
                if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {
                    if (mMode < 0)
            ...
            if (tCoords[1] >= PX_P[9] && tCoords[1] <= PX_P[11]) {
                if (tCoords[0] >= PX_P[9] && tCoords[0] <= PX_P[11]) {
                    if (mMode < 0) mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 16)];
                    else mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 10)];
                    playClick();
                    if (aChatReturn != null) aChatReturn.add("Averaging over: " + mValues.length + " Values");
                    return 1000;
                }
            ...
            if (tCoords[1] >= PX_P[12] && tCoords[1] <= PX_P[14]) {
                if (tCoords[0] >= PX_P[9] && tCoords[0] <= PX_P[11]) {
                    mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length - 1)];
                    playClick();
                    if (aChatReturn != null) aChatReturn.add(
                        mValues.length < 2 ? "Averaging Disabled" : "Averaging over: " + mValues.length + " Values");
                    return 1000;
                }
                if (tCoords[0] >= PX_P[12] && tCoords[0] <= PX_P[14]) {
                    mValues = new int[(int) UT.Code.bind_(1, MAX_AVERAGING_VALUES, mValues.length + 1)];
    ...
    public IIconContainer getCharacterIcon(int aIndex) {
        if (aIndex == 0) {
            switch (mMode & 127) {
                ...
                default:
                    return mMode < 0 ? BI.CHAR_HEX : BI.decimalDigit(mDisplayedNumber, 4);
            }
        }
        ...
        return mMode < 0 ? BI.hexadecimalDigit(mDisplayedNumber, 4 - aIndex)
            : BI.decimalDigit(mDisplayedNumber, 4 - aIndex);
    }

    @Override
    public short[] getCharacterColor(int aIndex) {
        return aIndex == 5 && (mMode & 127) != MODE_PERCENT ? getSymbolColor()
            : (mMode & 127) == MODE_FULL || (mMode & 127) == MODE_NOT_FULL ? CA_RED_192 : CA_WHITE;
    }
    ...
    public static final String[] METHODS = { "getval", "getmax" }, ARGS = { "void", "void" },
        HELPS = { "gets the value the sensor is reading for the object it is connected to",
            "gets the maximum value the sensor has for the object it is connected to" };
    ...
    @Override
    public Object[] callComputerizableMethod(DelegatorTileEntity<TileEntity> aDelegator, int aFunctionIndex,
        Object[] aArguments) {
