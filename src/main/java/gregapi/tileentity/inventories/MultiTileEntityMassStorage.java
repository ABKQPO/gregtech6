 ...
 */
public abstract class MultiTileEntityMassStorage extends TileEntityBase09FacingSingle
    implements ITileEntityConnectedInventory, ITileEntityLogisticsSemiFilteredItem, ITileEntityProgress,
    ITileEntityAdjacentInventoryUpdatable, IMTE_GetMaxStackSize, IMTE_SyncDataInteger, IMTE_SyncDataShort,
    IMTE_OnRegistrationFirstClient {
    ...
    public NBTTagCompound writeItemNBT2(NBTTagCompound aNBT) {
        if (isClientSide() && slotHas(1))
            aNBT.setTag("display", UT.NBT.makeString(aNBT.getCompoundTag("display"), "Name", slot(1).getDisplayName()));
        UT.NBT.setNumber(aNBT, NBT_MODE, mMode);
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        if (slotHas(1))
            aList.add(Chat.YELLOW + slot(1).getDisplayName() + Chat.GRAY + ": " + Chat.WHITE + slot(1).stackSize);
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_softhammer)) {
            ...
            if (slotHas(0)) {
                ST.place(
                    worldObj,
                    xCoord + OFFX[mFacing] + 0.5,
                    yCoord + OFFY[mFacing] + 0.5,
                    zCoord + OFFZ[mFacing] + 0.5,
                    ST.copy(slot(0)));
                slotKill(0);
                updateInventory();
                return 10000;
            ...
            if (slotHas(1)) {
                for (int i = 0; i < 128 && slot(1).stackSize > Math.max(1, slot(1).getMaxStackSize()); i++) {
                    ST.place(
                        ...
                        yCoord + OFFY[mFacing] + 0.5,
                        zCoord + OFFZ[mFacing] + 0.5,
                        ST.amount(Math.max(1, slot(1).getMaxStackSize()), slot(1)));
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) {
                ...
                aChatReturn.add((mMode & B[1]) == 0 ? "Filter stays when empty" : "Filter resets when empty");
                aChatReturn
                    .add((mMode & B[2]) == 0 ? "Won't emit Overflow" : "Will emit Overflow to Inventories below");
        ...
        if (isServerSide() && aPlayer != null) {
            ...
            if (slotHas(1)) {
                ...
                if (tCoords[1] >= PX_P[6] && tCoords[1] <= PX_P[8]) {
                    if (tCoords[0] >= PX_P[1] && tCoords[0] <= PX_P[3]) {
                        tAmount = 8;
                    }
                    if (tCoords[0] >= PX_N[3] && tCoords[0] <= PX_N[1]) {
                        tAmount = 64;
                    }
                }
                ...
                if (tCoords[1] >= PX_P[6]) {
                    if (tCoords[0] >= PX_P[4] && tCoords[0] <= PX_N[4]) {
                        tAmount = -1;
                    }
                }
                if (tAmount > 0) {
                    ...
                    if (tAmount > 0) {
                        ...
                        while (tAmount > 0) {
                            ItemStack tStack = ST
                                .amount(Math.min(tAmount, Math.max(1, slot(1).getMaxStackSize())), slot(1));
                            tAmount -= tStack.stackSize;
                            ST.place(
                                worldObj,
                                getOffsetX(mFacing) + 0.5,
                                getOffsetY(mFacing) + 0.5,
                                getOffsetZ(mFacing) + 0.5,
                                tStack);
                        }
                        updateInventory();
                        playCollect();
                        ...
                        if (tAmount == -1) {
                            ...
                            for (int i = 0; i < aPlayer.inventory.mainInventory.length; i++)
                                if (aPlayer.inventory.mainInventory[i] != null
                                    && allowInsertion(aPlayer.inventory.mainInventory[i])) {
        ...
        if (aIsServerSide && (mMode & B[3]) == 0) {
            ...
            if (mInventoryChanged || aTimer % 100 == 0) {
                if (slotHas(1)) {
                    ...
                    } else // else, because if it already tried to emit normally, then it doesn't need to check a second
                           // time.
    ...
    public boolean onTickCheck(long aTimer) {
        return super.onTickCheck(aTimer) || (isFaceVisible() && (slotHas(1)
            ? slot(1).stackSize != oStacksize
                && (Math.abs(slot(1).stackSize - oStacksize) > 64 ? SERVER_TIME % 5 == 0 : SYNC_SECOND)
            : oStacksize != 0));
    }
    ...
    public void setInventorySlotContents(int aSlot, ItemStack aStack) {
        ...
        if (aSlot == 1 && slotHas(aSlot))
            decrStackSize(aSlot, aStack == null ? slot(aSlot).stackSize : slot(aSlot).stackSize - aStack.stackSize);
    }
    ...
    public void emitOverflow() {
        ...
        while (slotHas(1) && slot(1).stackSize > mMaxStorage) {
            ...
            if (ST.move(delegator(SIDE_BOTTOM), tTileEntity, null, F, F, F, T, tToBeMoved, 1, tToBeMoved, 1) <= 0)
                break;
        }
    }
    ...
    public ItemStackSet<ItemStackContainer> getLogisticsFilter(byte aSide) {
        ...
        if (tData != null && tData.validData()) {
            if (tData.mPrefix.contains(TD.Prefix.DUST_BASED)) {
                for (ItemStack tStack : OreDictManager.getOres(OP.blockDust, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.dust, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.dustSmall, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.dustTiny, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.dustDiv72, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix.contains(TD.Prefix.INGOT_BASED)) {
                for (ItemStack tStack : OreDictManager.getOres(OP.blockIngot, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.ingot, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.billet, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.chunkGt, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.nugget, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix.contains(TD.Prefix.WIRE_BASED)) {
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt01, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt02, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt03, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt04, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt05, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt06, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt07, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt08, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt09, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt10, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt11, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt12, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt13, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt14, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt15, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.wireGt16, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                ...
                for (ItemStack tStack : OreDictManager.getOres(OP.blockGem, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.plate || tData.mPrefix == OP.blockPlate) {
                for (ItemStack tStack : OreDictManager.getOres(OP.plate, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.blockPlate, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.plateGem || tData.mPrefix == OP.blockPlateGem) {
                for (ItemStack tStack : OreDictManager.getOres(OP.plateGem, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.blockPlateGem, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.crushed || tData.mPrefix == OP.crushedTiny) {
                for (ItemStack tStack : OreDictManager.getOres(OP.crushed, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.crushedTiny, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.crushedPurified || tData.mPrefix == OP.crushedPurifiedTiny) {
                for (ItemStack tStack : OreDictManager.getOres(OP.crushedPurified, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.crushedPurifiedTiny, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.crushedCentrifuged || tData.mPrefix == OP.crushedCentrifugedTiny) {
                for (ItemStack tStack : OreDictManager.getOres(OP.crushedCentrifuged, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.crushedCentrifugedTiny, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            } else if (tData.mPrefix == OP.oreRaw || tData.mPrefix == OP.blockRaw) {
                for (ItemStack tStack : OreDictManager.getOres(OP.oreRaw, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
                for (ItemStack tStack : OreDictManager.getOres(OP.blockRaw, tData.mMaterial, F))
                    mLogisticsCache.add(tStack);
            }
        }
    ...
    public boolean isFaceVisible() {
        return SIDES_HORIZONTAL[mFacing] && (mMode & B[3]) == 0
            && (!hasCovers() || mCovers.mBehaviours[mFacing] == null
                || !mCovers.mBehaviours[mFacing].isOpaque(mFacing, mCovers));
    }
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        ...
        return aSendAll
            ? getClientDataPacketByteArray(
                ...
                getDirectionData(),
                mMode,
                UT.Code.toByteS(tID, 0),
                UT.Code.toByteS(tID, 1),
                UT.Code.toByteS(tMeta, 0),
                UT.Code.toByteS(tMeta, 1),
                UT.Code.toByteI(tStacksize, 0),
                UT.Code.toByteI(tStacksize, 1),
                UT.Code.toByteI(tStacksize, 2),
                UT.Code.toByteI(tStacksize, 3))
            : tStacksize <= Short.MAX_VALUE ? getClientDataPacketShort(aSendAll, (short) tStacksize)
                : getClientDataPacketInteger(aSendAll, tStacksize);
    }

    @Override
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        mRGBa = UT.Code
            .getRGBInt(new short[] { UT.Code.unsignB(aData[0]), UT.Code.unsignB(aData[1]), UT.Code.unsignB(aData[2]) });
        setDirectionData(aData[3]);
        mMode = aData[4];
        slot(
            1,
            ST.make(
                UT.Code.combine(aData[5], aData[6]),
                UT.Code.combine(aData[9], aData[10], aData[11], aData[12]),
                UT.Code.combine(aData[7], aData[8])));
        return T;
    }

    @Override
    public boolean receiveDataInteger(int aData, INetworkHandler aNetworkHandler) {
        if (aData < 0) slotKill(1);
        else if (slotHas(1)) slot(1).stackSize = aData;
        return T;
    }
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return aSlot == 0 && (mMode & B[3]) == 0
            && (!SIDES_BOTTOM[aSide] || (mMode & B[0]) == 0)
            && (!slotHas(1) || (slot(1).stackSize < getMaxContent() && allowInsertion(aStack)));
    }

    @Override
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        return aSlot == 0 || (slotHas(1) && slot(1).stackSize > 0 && (mMode & B[3]) == 0);
    }

    @Override
    public boolean allowZeroStacks(int aSlot) {
        return aSlot == 1 && ((mMode & B[1]) == 0 || mPartialUnits > 0);
    }

    @Override
    public void adjacentInventoryUpdated(byte aSide, IInventory aTileEntity) {
        if (SIDES_BOTTOM[aSide]) updateInventory();
    }

    @Override
    public long getAmountOfItemsInConnectedInventory(byte aSide, ItemStack aStack, long aStopCountingAtThisNumber) {
        return slotHas(1) && ST.equal(slot(1), aStack) ? slot(1).stackSize : 0;
    }

    @Override
    public long getProgressValue(byte aSide) {
        return slotHas(1) ? slot(1).stackSize : 0;
    }
    ...
    public ItemStack getPartialStack() {
        ...
        if (mData.mPrefix.contains(TD.Prefix.WIRE_BASED))
            return OP.wireGt01.mat(mData.mMaterial.mMaterial, mPartialUnits / OP.wireGt01.mAmount);
        ...
        if (mData.mPrefix == OP.crushedPurified || mData.mPrefix == OP.crushedPurifiedTiny) {
            return OP.crushedPurifiedTiny
                .mat(mData.mMaterial.mMaterial, mPartialUnits / OP.crushedPurifiedTiny.mAmount);
        }
        if (mData.mPrefix == OP.crushedCentrifuged || mData.mPrefix == OP.crushedCentrifugedTiny) {
            return OP.crushedCentrifugedTiny
                .mat(mData.mMaterial.mMaterial, mPartialUnits / OP.crushedCentrifugedTiny.mAmount);
        }
    ...
    public long getUnitAmount(ItemStack aStack) {
        ...
        if (mData != null && aData != null
            && mData.validData()
            && aData.validData()
            && mData.mMaterial.mMaterial == aData.mMaterial.mMaterial
            && (!mData.mBlackListed || (mData.mMaterial.mMaterial != MT.Glass && MD.MC.owns(slot(1))))
            && mPartialUnits < getUnitAmount(mData.mPrefix)) {
            ...
            if (mData.mPrefix == OP.crushedPurified || mData.mPrefix == OP.crushedPurifiedTiny) {
                return aData.mPrefix == OP.crushedPurified || aData.mPrefix == OP.crushedPurifiedTiny
                    ? aData.mPrefix.mAmount
                    : 0;
            }
            if (mData.mPrefix == OP.crushedCentrifuged || mData.mPrefix == OP.crushedCentrifugedTiny) {
                return aData.mPrefix == OP.crushedCentrifuged || aData.mPrefix == OP.crushedCentrifugedTiny
                    ? aData.mPrefix.mAmount
                    : 0;
            }
            if (mData.mPrefix == OP.oreRaw || mData.mPrefix == OP.blockRaw) {
    ...
    public static class MultiTileEntityRendererMassStorage extends TileEntitySpecialRenderer {
        ...
        public void renderTileEntityAt(TileEntity aTileEntity, double aX, double aY, double aZ, float aPartialTick) {
            if (aTileEntity instanceof MultiTileEntityMassStorage
                && ((MultiTileEntityMassStorage) aTileEntity).slotHas(1)
                && ((MultiTileEntityMassStorage) aTileEntity).isFaceVisible()) {
                ...
                glTranslated(
                    aX + 0.5 + OFFX[tTileEntity.mFacing] * 0.502 - OFFZ[tTileEntity.mFacing] * 0.25,
                    aY + 0.625,
                    aZ + 0.5 + OFFZ[tTileEntity.mFacing] * 0.502 + OFFX[tTileEntity.mFacing] * 0.25);
                glRotatef(180, 0, 0, 1);
                ...
                try {
                    if (!ForgeHooksClient.renderInventoryItem(
                        ...
                        0,
                        0)) {
                        RenderHelper.renderItemIntoGUI(
