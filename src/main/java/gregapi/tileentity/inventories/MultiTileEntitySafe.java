 ...
 */
public abstract class MultiTileEntitySafe extends TileEntityBase09FacingSingle
    implements IMTE_GetPlayerRelativeBlockHardness {
    ...
    protected void generateDungeonLoot() {
        if (isServerSide() && UT.Code.stringValid(mDungeonLootName)) {
            for (int i = 0, j = getSizeInventory(); i < j; i++)
                if (!slotHas(i)) slot(i, ChestGenHooks.getOneItem(mDungeonLootName, RNGSUS));
            mDungeonLootName = "";
        }
    }
    ...
    public boolean getSubItems(MultiTileEntityBlockInternal aBlock, Item aItem, CreativeTabs aTab,
        ...
        if (showInCreative()) {
            if (D1) for (String tLoot : new String[] { "mineshaftCorridor", "pyramidDesertyChest", "pyramidJungleChest",
                "pyramidJungleDispenser", "strongholdCorridor", "strongholdLibrary", "strongholdCrossing",
                "villageBlacksmith", "bonusChest", "dungeonChest" })
