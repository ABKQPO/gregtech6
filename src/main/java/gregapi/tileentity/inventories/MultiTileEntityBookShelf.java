 ...
 */
public class MultiTileEntityBookShelf extends TileEntityBase09FacingSingle implements ITileEntityBookShelf,
    IMTE_GetEnchantPowerBonus, IMTE_GetSelectedBoundingBoxFromPool, IMTE_SetBlockBoundsBasedOnState {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (mShelfIcon == null || mShelfIcon == Textures.BlockIcons.RENDERING_ERROR)
            mShelfIcon = mMaterial.mTextureSetsBlock.get(OP.casingMachine.mIconIndexBlock);
        for (int i = 0; i < mDisplay.length; i++) {
            Byte tID = (slotHas(i) ? BooksGT.BOOK_REGISTER.get(new ItemStackContainer(slot(i))) : null);
            if ((tID == null || tID == 0) && slotHas(i))
                tID = BooksGT.BOOK_REGISTER.get(new ItemStackContainer(slot(i), W));
                    ...
                    for (ItemStack tStack : tDummyInventory.mInventory) {
                        ...
                        if (!slotHas(tSlot)) {
                            if (BooksGT.BOOK_REGISTER.containsKey(tStack, T)) {
                                ...
                                slot(
                                    tSlot,
                                    rng(4) != 0 ? ST.make(Items.book, 1, 0)
                                        : ST.make(MD.LOSTBOOKS, "randomBook", 1, 0, Items.book));
                            }
                        }
                    }
    ...
    public float getEnchantPowerBonus() {
        ...
        for (int i = 0; i < mDisplay.length; i++) if (slotHas(i)) {
            if (BooksGT.BOOKS_ENCHANTED.contains(slot(i), T)) {
                tPoints += 2;
                continue;
            }
            if (BooksGT.BOOKS_NORMAL.contains(slot(i), T)) {
                tPoints += 1;
                continue;
            }
        }
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            if (aTimer % 300 == 0
                && (UT.Code.stringValid(mDungeonLootNameFront) || UT.Code.stringValid(mDungeonLootNameBack))
                && !worldObj.getEntitiesWithinAABB(EntityPlayer.class, box(-32, -32, -32, +33, +33, +33))
                    .isEmpty())
                generateDungeonLoot();
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_magnifyingglass) && aPlayer instanceof EntityPlayerMP) {
            ...
            if (tCoords[0] >= PX_P[1] && tCoords[0] <= PX_N[1] && tCoords[1] >= PX_P[1] && tCoords[1] <= PX_N[1]) {
                int tIndex = -1;
                if (aSide == mFacing) tIndex = (tCoords[1] < PX_P[8] ? 6 : 13)
                    - (int) UT.Code.bind_(0, 6, (long) (8 * (tCoords[0] - PX_P[1])));
                if (aSide == OPOS[mFacing]) tIndex = (tCoords[1] < PX_P[8] ? 20 : 27)
                    - (int) UT.Code.bind_(0, 6, (long) (8 * (tCoords[0] - PX_P[1])));
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (tCoords[0] >= PX_P[1] && tCoords[0] <= PX_N[1] && tCoords[1] >= PX_P[1] && tCoords[1] <= PX_N[1]) {
            if (aSide == mFacing) {
                if (isServerSide()) switchBooks(
                    aPlayer,
                    (tCoords[1] < PX_P[8] ? 6 : 13) - (int) UT.Code.bind_(0, 6, (long) (8 * (tCoords[0] - PX_P[1]))));
                return T;
            }
            if (aSide == OPOS[mFacing]) {
                if (isServerSide()) switchBooks(
                    aPlayer,
                    (tCoords[1] < PX_P[8] ? 20 : 27) - (int) UT.Code.bind_(0, 6, (long) (8 * (tCoords[0] - PX_P[1]))));
                return T;
            }
        }
        return F;
        ...
        if (slotHas(aSlot)) {
            if (!aPlayer.isSneaking()) {
                ...
                if (OD.lever.is(slot(aSlot)) || ST.equal(slot(aSlot), Blocks.redstone_torch)) {
                    if (mRedstoneDelay == 0) mRedstoneDelay = -1;
                    else mRedstoneDelay = 0;
                    causeBlockUpdate();
                    playClick();
                    return T;
                }
    ...
    public boolean usesRenderPass2(int aRenderPass, boolean[] aShouldSideBeRendered) {
        return aRenderPass < 7
            || (mDisplay[aRenderPass - 7] != 0 && aShouldSideBeRendered[aRenderPass < 21 ? mFacing : OPOS[mFacing]]);
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                return F;
            case 1:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[15], PX_N[0]);
            case 2:
                return box(aBlock, PX_P[0], PX_P[15], PX_P[0], PX_N[0], PX_N[0], PX_N[0]);
            case 3:
                return box(
                    aBlock,
                    PX_P[0],
                    PX_P[1],
                    PX_P[0],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 15 : 0],
                    PX_N[1],
                    PX_N[SIDES_AXIS_X[mFacing] ? 15 : 0]);
            case 4:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_Z[mFacing] ? 15 : 0],
                    PX_P[1],
            ...
            case 6:
                return box(aBlock, PX_P[1], PX_P[7], PX_P[1], PX_N[1], PX_N[7], PX_N[1]);
            default:
                aRenderPass -= 7;
                switch (mFacing) {
                    case SIDE_X_POS:
                        if (aRenderPass < 7) {
                            return box(
                                ...
                                PX_P[9],
                                PX_P[9],
                                PX_N[15 - aRenderPass * 2],
                                PX_N[2],
                                PX_N[1],
                                PX_N[13 - aRenderPass * 2]);
                        }
                        if (aRenderPass < 14) {
                            aRenderPass %= 7;
                            return box(
                        ...
                        if (aRenderPass < 21) {
                            ...
                            return box(
                                ...
                                PX_P[2],
                                PX_P[9],
                                PX_N[3 + aRenderPass * 2],
                                PX_N[9],
                                PX_N[1],
                                PX_N[1 + aRenderPass * 2]);
                        }
                        aRenderPass %= 7;
                        return box(
                            aBlock,
                    ...
                    case SIDE_X_NEG:
                        if (aRenderPass < 7) {
                            return box(
                                ...
                                PX_P[2],
                                PX_P[9],
                                PX_P[13 - aRenderPass * 2],
                                PX_N[9],
                                PX_N[1],
                                PX_P[15 - aRenderPass * 2]);
                        }
                        if (aRenderPass < 14) {
                            aRenderPass %= 7;
                            return box(
                        ...
                        if (aRenderPass < 21) {
                            ...
                            return box(
                                ...
                                PX_P[9],
                                PX_P[9],
                                PX_P[1 + aRenderPass * 2],
                                PX_N[2],
                                PX_N[1],
                                PX_P[3 + aRenderPass * 2]);
                        }
                        aRenderPass %= 7;
                        return box(
                            aBlock,
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass < 7) {
            ...
            switch (aRenderPass) {
                case 0:
                    return ALONG_AXIS[aSide][mFacing] || !aShouldSideBeRendered[aSide] ? null
                        : BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING));
                case 1:
                    return ALONG_AXIS[aSide][mFacing] || SIDES_TOP[aSide]
                        ? BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING))
                        : null;
                case 2:
                    return ALONG_AXIS[aSide][mFacing] || SIDES_BOTTOM[aSide]
                        ? BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING))
                        : null;
                case 3:
                    return aSide == (SIDES_AXIS_Z[mFacing] ? SIDE_X_NEG : SIDE_Z_NEG) ? null
                        : BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING));
                case 4:
                    return aSide == (SIDES_AXIS_Z[mFacing] ? SIDE_X_POS : SIDE_Z_POS) ? null
                        : BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING));
                case 5:
                    return ALONG_AXIS[aSide][mFacing]
                        ...
                        : null;
                case 6:
                    return ALONG_AXIS[aSide][mFacing] || SIDES_VERTICAL[aSide]
                        ? BlockTextureDefault.get(mShelfIcon, mRGBa, mMaterial.contains(TD.Properties.GLOWING))
                        : null;
            }
        }
        ...
        return (ALONG_AXIS[aSide][mFacing] ? BooksGT.BOOK_TEXTURES_BACK : BooksGT.BOOK_TEXTURES_SIDE)[UT.Code
            .unsignB(mDisplay[aRenderPass - 7])];
    }

    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool() {
        return box(
            PX_P[SIDES_AXIS_X[mFacing] ? 2 : 0],
            0,
            PX_P[SIDES_AXIS_Z[mFacing] ? 2 : 0],
            PX_N[SIDES_AXIS_X[mFacing] ? 2 : 0],
            1,
            PX_N[SIDES_AXIS_Z[mFacing] ? 2 : 0]);
    }
    ...
    public ItemStack getDefaultStack(int aSlot) {
        return ST.make(Items.book, 1, 0);
    }

    @Override
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        return slotHas(aSlot) && !ST.equal(slot(aSlot), Blocks.cobblestone)
            && !ST.equal(slot(aSlot), Blocks.redstone_torch)
            && !OD.lever.is(slot(aSlot))
            && !OD.button.is(slot(aSlot));
    }

    @Override
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return !slotHas(aSlot) && BooksGT.BOOK_REGISTER.containsKey(aStack, T)
            && !ST.equal(slot(aSlot), Blocks.cobblestone)
            && !ST.equal(slot(aSlot), Blocks.redstone_torch)
            && !OD.lever.is(slot(aSlot))
            && !OD.button.is(slot(aSlot));
    }
