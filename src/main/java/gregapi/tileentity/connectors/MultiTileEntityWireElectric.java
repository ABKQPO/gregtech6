 ...
 */
public class MultiTileEntityWireElectric extends TileEntityBase10ConnectorRendered
    implements ITileEntityQuickObstructionCheck, ITileEntityEnergy, ITileEntityEnergyDataConductor, ITileEntityProgress,
    IMTE_GetDebugInfo, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {

    public long mTransferredAmperes = 0, mTransferredWattage = 0, mWattageLast = 0, mLoss = 1, mAmperage = 1,
        mVoltage = 32;
     ...
     */
    public static void addElectricWires(int aID, int aCreativeTabID, long aVoltage, long aAmperage, long aLossWire,
        long aLossCable, boolean aContactDamageWire, boolean aContactDamageCable, boolean aCable,
        MultiTileEntityRegistry aRegistry, MultiTileEntityBlock aBlock, Class<? extends TileEntity> aClass,
        OreDictMaterial aMat) {
        OreDictManager.INSTANCE.setTarget_(
            OP.wireGt01,
            aMat,
            aRegistry.add(
                "1x " + aMat.getLocal() + " Wire",
                "Electric Wires",
                aID,
                aCreativeTabID,
                aClass,
        ...
        if (aCable) {
            ...
            for (OreDictMaterial tMat : ANY.Rubber.mToThis) {
                RM.Laminator
                    .addRecipe2(T, 16, 16, OP.plate.mat(tMat, 1), aRegistry.getItem(aID), aRegistry.getItem(aID + 16));
                RM.Laminator.addRecipe2(
                    T,
                    16,
                    16,
                    OP.plate.mat(tMat, 1),
                    aRegistry.getItem(aID + 1),
                    aRegistry.getItem(aID + 16 + 1));
                RM.Laminator.addRecipe2(
                    T,
                    16,
                    32,
                    OP.plate.mat(tMat, 2),
                    aRegistry.getItem(aID + 3),
                    aRegistry.getItem(aID + 16 + 3));
                RM.Laminator.addRecipe2(
                    T,
                    16,
                    48,
                    OP.plate.mat(tMat, 3),
                    aRegistry.getItem(aID + 7),
                    aRegistry.getItem(aID + 16 + 7));
                RM.Laminator.addRecipe2(
                    T,
                    16,
                    64,
                    OP.plate.mat(tMat, 4),
                    aRegistry.getItem(aID + 11),
                    aRegistry.getItem(aID + 16 + 11));

                RM.Laminator
                    .addRecipe2(T, 16, 16, OP.foil.mat(tMat, 4), aRegistry.getItem(aID), aRegistry.getItem(aID + 16));
                RM.Laminator.addRecipe2(
                    T,
                    16,
                    16,
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        aList.add(
            Chat.CYAN + LH.get(LH.WIRE_STATS_VOLTAGE)
                + mVoltage
                + " "
                + TD.Energy.EU.getLocalisedNameShort()
                + " ("
                + VN[UT.Code.tierMin(mVoltage)]
                + ")");
            ...
            if (mBurnCounter >= 16) {
                ...
                if (EnergyCompat.IC_ENERGY) for (byte tSide : ALL_SIDES_VALID) if (canAcceptEnergyFrom(tSide)) {
                    ...
                    if (!(tDelegator.mTileEntity instanceof gregapi.tileentity.ITileEntityEnergy)) {
                        TileEntity tEmitter = tDelegator.mTileEntity instanceof IEnergyTile
                            || EnergyNet.instance == null ? tDelegator.mTileEntity
                                : EnergyNet.instance
                                    .getTileEntity(tDelegator.mWorld, tDelegator.mX, tDelegator.mY, tDelegator.mZ);
                        if (tEmitter instanceof IEnergySource
                            && ((IEnergySource) tEmitter).emitsEnergyTo(this, tDelegator.getForgeSideOfTileEntity())) {
                            long tEU = (long) ((IEnergySource) tEmitter).getOfferedEnergy();
                            if (transferElectricity(tSide, tEU, 1, -1, new HashSetNoNulls<TileEntity>(F, this)) > 0)
    ...
    public long transferElectricity(byte aSide, long aVoltage, long aAmperage, long aChannel,
        HashSetNoNulls<TileEntity> aAlreadyPassed) {
        if (mTimer < 1 || Math.abs(aVoltage) <= mLoss) return 0;
        if (aVoltage > 0) aVoltage -= mLoss;
        else aVoltage += mLoss;
        ...
        for (byte tSide : ALL_SIDES_VALID_BUT[aSide]) if (canEmitEnergyTo(tSide)) {
            ...
            if (aAlreadyPassed.add(tDelegator.mTileEntity)) {
                if (tDelegator.mTileEntity instanceof MultiTileEntityWireElectric) {
                    if (((MultiTileEntityWireElectric) tDelegator.mTileEntity)
                        .isEnergyAcceptingFrom(TD.Energy.EU, tDelegator.mSideOfTileEntity, F)) {
                        rUsedAmperes += ((MultiTileEntityWireElectric) tDelegator.mTileEntity).transferElectricity(
                    ...
                    rUsedAmperes += ITileEntityEnergy.Util
                        .insertEnergyInto(TD.Energy.EU, aVoltage, aAmperage - rUsedAmperes, this, tDelegator);
                }
            }
        }
    ...
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mContactDamage && !mFoamDried) UT.Entities.applyElectricityDamage(aEntity, mWattageLast);
    }
    ...
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return isEnergyType(aEnergyType, aSide, T) && canEmitEnergyTo(aSide);
    }

    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return isEnergyType(aEnergyType, aSide, F) && canAcceptEnergyFrom(aSide);
    }
    ...
    public synchronized long doEnergyInjection(TagData aEnergyType, byte aSide, long aSize, long aAmount,
        ...
        return aSize != 0 && isEnergyAcceptingFrom(aEnergyType, aSide, F)
            ? aDoInject ? transferElectricity(aSide, aSize, aAmount, -1, new HashSetNoNulls<TileEntity>(F, this))
                : aAmount
            : 0;
    }
    ...
    public long getEnergyMaxSize(TagData aEnergyType) {
        return aEnergyType == TD.Energy.EU ? mVoltage : 0;
    }

    @Override
    public long getEnergyMaxPackets(TagData aEnergyType) {
        return aEnergyType == TD.Energy.EU ? mAmperage : 0;
    }

    @Override
    public long getEnergyLossPerMeter(TagData aEnergyType) {
        return aEnergyType == TD.Energy.EU ? mLoss : 0;
    }
    ...
    public ArrayList<String> getDebugInfo(int aScanLevel) {
        return aScanLevel > 0 ? new ArrayListNoNulls<>(F, "Transferred Power: " + mWattageLast) : null;
    }

    @Override
    public ITexture getTextureSide(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return mRenderType == 1 || mRenderType == 2
            ? BlockTextureDefault
                .get(Textures.BlockIcons.INSULATION_FULL, isPainted() ? mRGBa : UT.Code.getRGBInt(64, 64, 64))
            : BlockTextureDefault
                .get(mMaterial, getIconIndexSide(aSide, aConnections, aDiameter, aRenderPass), F, mRGBa);
    }
    ...
    public ITexture getTextureConnected(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return mRenderType == 1 || mRenderType == 2
            ? BlockTextureMulti.get(
                BlockTextureDefault
                    .get(mMaterial, getIconIndexConnected(aSide, aConnections, aDiameter, aRenderPass), mIsGlowing),
                BlockTextureDefault.get(
                    mRenderType == 2 ? Textures.BlockIcons.INSULATION_BUNDLED
                        : aDiameter < 0.37F ? Textures.BlockIcons.INSULATION_TINY
                            : aDiameter < 0.49F ? Textures.BlockIcons.INSULATION_SMALL
                                : aDiameter < 0.74F ? Textures.BlockIcons.INSULATION_MEDIUM
                                    : aDiameter < 0.99F ? Textures.BlockIcons.INSULATION_LARGE
                                        : Textures.BlockIcons.INSULATION_HUGE,
                    isPainted() ? mRGBa : UT.Code.getRGBInt(64, 64, 64)))
            : BlockTextureDefault
