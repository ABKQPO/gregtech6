...
public interface ITileEntityItemPipe extends ITileEntityConnector {
    ...
    public static class Util {
         ...
         */
        public static Map<ITileEntityItemPipe, Long> scanPipes(ITileEntityItemPipe aPipe,
            Map<ITileEntityItemPipe, Long> aMap, long aStep, boolean aSuckItems, boolean aIgnoreCapacity) {
                ...
                for (byte aSide : ALL_SIDES_VALID) {
                    if (aSuckItems) {
                        if (aPipe.canAcceptItemsFrom(aSide, null)) {
                            ...
                            if (tDelegator.mTileEntity instanceof ITileEntityItemPipe
                                && UT.Code.haveOneCommonElement(
                                    aPipe.getConnectorTypes(aSide),
                                    ((ITileEntityItemPipe) tDelegator.mTileEntity)
                                        .getConnectorTypes(tDelegator.mSideOfTileEntity))
                                && ((ITileEntityItemPipe) tDelegator.mTileEntity)
                                    .canEmitItemsTo(tDelegator.mSideOfTileEntity, null)) {
                                scanPipes(
                                    (ITileEntityItemPipe) tDelegator.mTileEntity,
                                    aMap,
                                    aStep,
                            ...
                            if (tDelegator.mTileEntity instanceof ITileEntityItemPipe
                                && UT.Code.haveOneCommonElement(
                                    ...
                                    ((ITileEntityItemPipe) tDelegator.mTileEntity)
                                        .getConnectorTypes(tDelegator.mSideOfTileEntity))
                                && ((ITileEntityItemPipe) tDelegator.mTileEntity)
                                    .canAcceptItemsFrom(tDelegator.mSideOfTileEntity, null)) {
                                scanPipes(
                                    (ITileEntityItemPipe) tDelegator.mTileEntity,
                                    aMap,
                                    aStep,
