...
public interface ITileEntityRedstoneWire extends ITileEntityConnector {
    ...
    public static class Util {
        ...
        public static void doRedstoneUpdate(ITileEntityRedstoneWire aTileEntity, int aRedstoneID) {
            HashSetNoNulls<ITileEntityRedstoneWire> tSetUpdating = new HashSetNoNulls<>(F, aTileEntity),
                tSetNext = new HashSetNoNulls<>();
            while (!tSetUpdating.isEmpty()) {
                for (ITileEntityRedstoneWire tTileEntity : tSetUpdating)
                    for (byte tSide : ALL_SIDES_VALID) if (tTileEntity.canEmitRedstoneToWire(tSide, aRedstoneID)) {
                        ...
                        if (tDelegator.mTileEntity instanceof ITileEntityRedstoneWire
                            && ((ITileEntityRedstoneWire) tDelegator.mTileEntity)
                                .canAcceptRedstoneFromWire(tDelegator.mSideOfTileEntity, aRedstoneID)
                            && ((ITileEntityRedstoneWire) tDelegator.mTileEntity).updateRedstone(aRedstoneID)) {
