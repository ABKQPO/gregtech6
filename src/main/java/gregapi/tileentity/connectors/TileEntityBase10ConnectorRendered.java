...
public abstract class TileEntityBase10ConnectorRendered extends TileEntityBase09Connector
    implements ITileEntityFoamable, IMTE_GetPlayerRelativeBlockHardness, IMTE_IgnorePlayerCollisionWhenPlacing,
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_DIAMETER))
            mDiameter = Math.max(PX_P[2], Math.min(PX_N[0], (float) aNBT.getDouble(NBT_DIAMETER)));
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0) {
            ...
            box(
                ...
                (1.0F - mDiameter) / 2.0F,
                (1.0F - mDiameter) / 2.0F,
                1 - (1.0F - mDiameter) / 2.0F,
                1 - (1.0F - mDiameter) / 2.0F,
                1 - (1.0F - mDiameter) / 2.0F);
            return T;
        }
        // TODO: I need to add the old optimizations back somehow.
        // Even though this Version is way more modular and can adjust to stuff much easier, it does also look bad when
        // rendered with ambient occlusion.
        if (aRenderPass <= 6) {
            DelegatorTileEntity<TileEntity> tDelegator = getAdjacentTileEntity((byte) (aRenderPass - 1), F, F);
            float tDiameter = getConnectorDiameter((byte) (aRenderPass - 1), tDelegator),
                tLength = getConnectorLength((byte) (aRenderPass - 1), tDelegator);
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 7)
            return aShouldSideBeRendered[aSide] ? getTextureCFoam(aSide, mConnections, mDiameter, aRenderPass) : null;
        if (aRenderPass == 0) return mFoamDried
            ? aShouldSideBeRendered[aSide] ? getTextureCFoamDry(aSide, mConnections, mDiameter, aRenderPass) : null
            : mConnections == 0 || (mDiameter >= 1.0F && connected(aSide))
                ? getTextureConnected(aSide, mConnections, mDiameter, aRenderPass)
                : getTextureSide(aSide, mConnections, mDiameter, aRenderPass);
        return aSide == OPOS[aRenderPass - 1] ? null
            : aSide == aRenderPass - 1
                ? aShouldSideBeRendered[aSide] ? getTextureConnected(aSide, mConnections, mDiameter, aRenderPass) : null
                : getTextureSide(aSide, mConnections, mDiameter, aRenderPass);
    }

    @Override
    public boolean usesRenderPass2(int aRenderPass, boolean[] aShouldSideBeRendered) {
        return aRenderPass == 0 || aRenderPass == 7 || connected((byte) (aRenderPass - 1));
    }

    @Override
    public int getLightOpacity() {
        return mFoamDried ? LIGHT_OPACITY_MAX
            : mTransparent
                ? mDiameter >= 1.0F ? LIGHT_OPACITY_WATER : mDiameter > 0.5F ? LIGHT_OPACITY_LEAVES : LIGHT_OPACITY_NONE
                : mDiameter >= 1.0F ? LIGHT_OPACITY_MAX : mDiameter > 0.5F ? LIGHT_OPACITY_WATER : LIGHT_OPACITY_LEAVES;
    }
    ...
    public float getExplosionResistance2() {
        return Math.max(
            mFoam ? (mFoamDried ? BlocksGT.CFoam : BlocksGT.CFoamFresh).getExplosionResistance(null) : 0,
            super.getExplosionResistance2());
    }

    @Override
    public byte getDirectionData() {
        return (byte) (((byte) (mConnections & 63)) | ((byte) ((mFoamDried ? mOwnable : mFoam) ? 64 : 0))
            | ((byte) (mFoamDried ? 128 : 0)));
    }
    ...
    public int getFireSpreadSpeed(byte aSide, boolean aDefault) {
        return mFoam ? 0 : super.getFireSpreadSpeed(aSide, aDefault);
    }

    @Override
    public int getFlammability(byte aSide, boolean aDefault) {
        return mFoam ? 0 : super.getFlammability(aSide, aDefault);
    }

    @Override
    public float getSurfaceSize(byte aSide) {
        return mFoamDried ? 1.0F : mDiameter;
    }
    ...
    public float getSurfaceDistance(byte aSide) {
        return mFoamDried || connected(aSide) ? 0.0F : (1.0F - mDiameter) / 2.0F;
    }

    @Override
    public boolean isSurfaceSolid(byte aSide) {
        return mFoamDried || mDiameter == 1.0F;
    }

    @Override
    public boolean isSurfaceOpaque2(byte aSide) {
        return mFoamDried || (mDiameter == 1.0F && !mTransparent);
    }
    ...
    public boolean addDefaultCollisionBoxToList() {
        return mDiameter >= 1.0F || mFoamDried;
    }

    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return mContactDamage && !mFoamDried ? box(PX_P[2], PX_P[2], PX_P[2], PX_N[2], PX_N[2], PX_N[2])
    ...
    public void addCollisionBoxesToList2(AxisAlignedBB aAABB, List<AxisAlignedBB> aList, Entity aEntity) {
        if (!addDefaultCollisionBoxToList()) {
            ...
            box(
                ...
                1 - (1.0F - mDiameter) / 2.0F,
                1 - (1.0F - mDiameter) / 2.0F);
            if (connected(tSide = SIDE_X_NEG)) {
                DelegatorTileEntity<TileEntity> tDelegator = getAdjacentTileEntity(tSide, F, F);
                float tDiameter = getConnectorDiameter(tSide, tDelegator), tLength = mContactDamage ? -PX_P[2] : 0;
                box(
                    aAABB,
                    aList,
                    0 - tLength,
    ...
    public float[] shrunkBox() {
        ...
        return new float[] { connected(SIDE_X_NEG) ? 0 : tDiameter, connected(SIDE_Y_NEG) ? 0 : tDiameter,
            connected(SIDE_Z_NEG) ? 0 : tDiameter, connected(SIDE_X_POS) ? 1 : 1 - tDiameter,
            connected(SIDE_Y_POS) ? 1 : 1 - tDiameter, connected(SIDE_Z_POS) ? 1 : 1 - tDiameter };
    }

    public float getConnectorLength(byte aConnectorSide, DelegatorTileEntity<TileEntity> aDelegator) {
        float rLength = (mDiameter != 1.0F && hasCovers() && mCovers.mBehaviours[aConnectorSide] != null
            ? mCovers.mBehaviours[aConnectorSide].showsConnectorFront(aConnectorSide, mCovers) ? +0.001F : -0.001F
            : 0);
        if (aDelegator.mTileEntity instanceof ITileEntitySurface) {
            float tDistance = ((ITileEntitySurface) aDelegator.mTileEntity)
                .getSurfaceDistance(aDelegator.mSideOfTileEntity);
    ...
    public float getConnectorDiameter(byte aConnectorSide, DelegatorTileEntity<TileEntity> aDelegator) {
        ...
        if (aDelegator.mTileEntity instanceof ITileEntitySurface)
            rDiameter = ((ITileEntitySurface) aDelegator.mTileEntity)
                .getSurfaceSizeAttachable(aDelegator.mSideOfTileEntity);
        // Connect with normal Size when the target cannot connect and ensure that minimum and maximum size are properly
        // done.
        if (rDiameter <= 0.0F || rDiameter > mDiameter) rDiameter = mDiameter;
        else if (rDiameter < PX_P[2]) rDiameter = PX_P[2];
        return rDiameter;
    }
    ...
    public ITexture getTextureCFoam(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return BlockTextureDefault
            .get(mOwnable ? Textures.BlockIcons.CFOAM_FRESH_OWNED : Textures.BlockIcons.CFOAM_FRESH, mRGBa);
    }

    public ITexture getTextureCFoamDry(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return BlockTextureDefault
            .get(mOwnable ? Textures.BlockIcons.CFOAM_HARDENED_OWNED : Textures.BlockIcons.CFOAM_HARDENED, mRGBa);
    }
    ...
    public int getIconIndexConnected(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return aDiameter < 0.37F ? OP.pipeTiny.mIconIndexBlock
