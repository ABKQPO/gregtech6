 ...
 */
public class MultiTileEntityPipeItem extends TileEntityBase10ConnectorRendered
    implements ITileEntityQuickObstructionCheck, ITileEntityProgress, ITileEntityItemPipe, ITileEntityServerTickPre {

    public long mTransferredItems = 0, mStepSize = 1;
    public byte mLastReceivedFrom = SIDE_UNDEFINED, oLastReceivedFrom = SIDE_UNDEFINED, mRenderType = 0,
        mDisabledOutputs = 0, mDisabledInputs = 0;
     ...
     */
    public static void addItemPipes(int aID, int aCreativeTabID, long aStepSize, int aInvSize, boolean aRecipe,
        ...
        OreDictManager.INSTANCE.setTarget_(
            ...
            T);
        OreDictManager.INSTANCE.setTarget_(
            OP.pipeRestrictiveMedium,
            aMat,
            aRegistry.add(
                "Restrictive " + aMat.getLocal() + " Item Pipe",
                "Item Pipes",
                aID + 5,
                aCreativeTabID,
                aClass,
                ...
                UT.NBT.make(
                    ...
                    aBlocking),
                aRecipe
                    ? new Object[] { " h ", "RPR", " R ", 'P', OP.pipeMedium.dat(aMat), 'R', OP.ring.dat(ANY.Steel) }
                    : ZL),
            T,
            F,
            T);
        OreDictManager.INSTANCE.setTarget_(
            OP.pipeRestrictiveLarge,
            aMat,
            aRegistry.add(
                "Restrictive Large " + aMat.getLocal() + " Item Pipe",
                "Item Pipes",
                aID + 6,
                aCreativeTabID,
                aClass,
                ...
                UT.NBT.make(
                    ...
                    NBT_OPAQUE,
                    aBlocking),
                aRecipe ? new Object[] { "hR ", "RPR", " R ", 'P', OP.pipeLarge.dat(aMat), 'R', OP.ring.dat(ANY.Steel) }
                    : ZL),
            T,
            F,
            T);
        OreDictManager.INSTANCE.setTarget_(
            OP.pipeRestrictiveHuge,
            aMat,
            aRegistry.add(
                "Restrictive Huge " + aMat.getLocal() + " Item Pipe",
                "Item Pipes",
                aID + 7,
                aCreativeTabID,
                aClass,
                ...
                UT.NBT.make(
                    ...
                    NBT_OPAQUE,
                    aBlocking),
                aRecipe ? new Object[] { " h ", "RPR", "RRR", 'P', OP.pipeHuge.dat(aMat), 'R', OP.ring.dat(ANY.Steel) }
                    : ZL),
            T,
            F,
            T);
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.CYAN + LH.get(LH.PIPE_STATS_BANDWIDTH) + Chat.WHITE + UT.Code.makeString(getPipeCapacity()) + "/s");
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_monkeywrench)) {
            ...
            if (aChatReturn != null) {
                aChatReturn.add(
                    FACE_CONNECTED[aTargetSide][mDisabledInputs] ? "Accepting from selected Side disabled"
                        : "Accepting from selected Side enabled");
                aChatReturn.add(
                    FACE_CONNECTED[aTargetSide][mDisabledOutputs] ? "Emitting to selected Side disabled"
                        : "Emitting to selected Side enabled");
            }
            return 2500;
        }
        ...
        if (aFirst) {
            ...
            if (oLastReceivedFrom == mLastReceivedFrom) {
                ...
                for (boolean temp = T; temp && UT.Code.containsSomething(getInventory()) && pipeCapacityCheck();) {
                    temp = F;
                    tPipeList.clear();
                    for (ITileEntityItemPipe tTileEntity : UT.Code
                        .sortByValuesAcending(
                            ITileEntityItemPipe.Util.scanPipes(this, new HashMap<ITileEntityItemPipe, Long>(), 0, F, F))
                        .keySet()) {
                        if (temp) break;
    ...
    public boolean insertItemStackIntoTileEntity(Object aSender, byte aSide) {
        if (!FACE_CONNECTED[aSide][mDisabledOutputs] && canEmitItemsTo(aSide, aSender)) {
            ...
            if (ST.canConnect(tDelegator) && !(tDelegator.mTileEntity instanceof TileEntityBase09Connector)) {
                if (!(tDelegator.mTileEntity instanceof TileEntityHopper
                    || tDelegator.mTileEntity instanceof TileEntityDispenser)
                    || getMetaDataAtSide(aSide) != tDelegator.mSideOfTileEntity) {
                    // special cases for the win...
                    CoverData tCovers = getCoverData();
                    if (tCovers != null && tCovers.mBehaviours[aSide] instanceof CoverFilterItem
                        && tCovers.mNBTs[aSide] != null) {
                        ItemStack tStack = ST.load(tCovers.mNBTs[aSide], "gt.filter.item");
                        return ST.valid(tStack) && ST.move(
                            new DelegatorTileEntity<>((TileEntity) aSender, SIDE_ANY),
                            tDelegator,
                            ST.hashset(tStack),
                            F,
                            F,
                            tCovers.mVisuals[aSide] != 0,
                            T,
                            64,
                            1,
                            64,
    ...
    public boolean sendItemStack(Object aSender) {
        if (pipeCapacityCheck()) for (byte i = 0, j = (byte) rng(6); i < 6; i++)
            if (insertItemStackIntoTileEntity(aSender, (byte) ((i + j) % 6))) return T;
        return F;
    }
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        if (!connected(aSide) || FACE_CONNECTED[aSide][mDisabledInputs]) return F;
        if (!UT.Code.containsSomething(getInventory())) mLastReceivedFrom = aSide;
        return mLastReceivedFrom == aSide && !slotHas(aSlot);
    }

    @Override
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        return SIDES_INVALID[aSide] || connected(aSide);
    }

    @Override
    public ItemStack[] getDefaultInventory(NBTTagCompound aNBT) {
        ItemStack[] rStack = super.getDefaultInventory(aNBT);
        ACCESSIBLE_SLOTS = new int[rStack.length];
    ...
    public boolean canConnect(byte aSide, DelegatorTileEntity<TileEntity> aDelegator) {
        return aDelegator.mTileEntity instanceof ISidedInventory
            ? aDelegator.mTileEntity instanceof ITileEntityCanDelegate || ((ISidedInventory) aDelegator.mTileEntity)
                .getAccessibleSlotsFromSide(aDelegator.mSideOfTileEntity).length > 0
            : ST.canConnect(aDelegator);
    }
    ...
    public boolean isUsingWrenchingOverlay(ItemStack aStack, byte aSide) {
        return super.isUsingWrenchingOverlay(aStack, aSide) || ToolsGT.contains(TOOL_monkeywrench, aStack);
    }
