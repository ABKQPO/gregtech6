 ...
 */
public abstract class TileEntityBase09Connector extends TileEntityBase08Directional
    implements ITileEntityConnector, IMTE_OnPlaced, IMTE_AddToolTips {
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        aList.add(
            Chat.DGRAY + LH.get(LH.TOOL_TO_SET_CONNECTIONS_PRE)
                + LH.get(TOOL_LOCALISER_PREFIX + getFacingTool(), "Unknown")
                + LH.get(LH.TOOL_TO_SET_CONNECTIONS_POST));
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(getFacingTool())) {
            ...
            if (tDelegator.mTileEntity instanceof ITileEntity
                && !((ITileEntity) tDelegator.mTileEntity).allowInteraction(aPlayer)) return 0;
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        if (isServerSide()) {
            ...
            if (tDelegator.mTileEntity instanceof ITileEntity
                && !((ITileEntity) tDelegator.mTileEntity).allowInteraction(aPlayer)) return T;
            connect(aSide, T);
            for (byte tSide : ALL_SIDES_VALID) {
                ...
                if (tDelegator.mTileEntity instanceof ITileEntityConnector && SIDES_VALID[tDelegator.mSideOfTileEntity]
                    && UT.Code.haveOneCommonElement(
                        ...
                        getConnectorTypes(tSide))) {
                    if (((ITileEntityConnector) tDelegator.mTileEntity).connected(tDelegator.mSideOfTileEntity))
                        connect(tSide, T);
                }
            }
        }
    ...
    public byte getDirectionData() {
        return (byte) (mConnections & (byte) 63);
    }
    ...
    public boolean connect(byte aSide, boolean aNotify) {
        ...
        if (hasCovers() && mCovers.mBehaviours[aSide] != null
            && mCovers.mBehaviours[aSide].interceptConnect(aSide, mCovers)) return F;
        DelegatorTileEntity<TileEntity> tDelegator = getAdjacentTileEntity(aSide);
        if (tDelegator.mTileEntity instanceof ITileEntityConnector) {
            if (tDelegator.mTileEntity instanceof ITileEntityCoverable
                && ((ITileEntityCoverable) tDelegator.mTileEntity).getCoverData() != null
                && ((ITileEntityCoverable) tDelegator.mTileEntity)
                    .getCoverData().mBehaviours[tDelegator.mSideOfTileEntity] != null
                && ((ITileEntityCoverable) tDelegator.mTileEntity)
                    .getCoverData().mBehaviours[tDelegator.mSideOfTileEntity].interceptConnect(
                        tDelegator.mSideOfTileEntity,
                        ((ITileEntityCoverable) tDelegator.mTileEntity).getCoverData()))
                return F;
            if (SIDES_VALID[tDelegator.mSideOfTileEntity] && UT.Code.haveOneCommonElement(
                ((ITileEntityConnector) tDelegator.mTileEntity).getConnectorTypes(tDelegator.mSideOfTileEntity),
                getConnectorTypes(aSide))) {
                ...
                if (hasMultiBlockMachineRelevantData())
                    ITileEntityMachineBlockUpdateable.Util.causeMachineUpdate(this, F);
                return T;
            }
        ...
        } else if (WD.air(tDelegator.mWorld, tDelegator.mX, tDelegator.mY, tDelegator.mZ)
            || WD.liquid(tDelegator.mWorld, tDelegator.mX, tDelegator.mY, tDelegator.mZ)
            || canConnect(aSide, tDelegator)) {
                byte oConnections = mConnections;
                mConnections |= SBIT[aSide];
                updateClientData();
                causeBlockUpdate();
    ...
    public boolean disconnect(byte aSide, boolean aNotify) {
        ...
        DelegatorTileEntity<TileEntity> tDelegator = getAdjacentTileEntity(aSide);
        if (hasCovers() && mCovers.mBehaviours[aSide] != null
            && mCovers.mBehaviours[aSide].interceptDisconnect(aSide, mCovers)) return F;
        if (tDelegator.mTileEntity instanceof ITileEntityCoverable
            && ((ITileEntityCoverable) tDelegator.mTileEntity).getCoverData() != null
            && ((ITileEntityCoverable) tDelegator.mTileEntity).getCoverData().mBehaviours[tDelegator.mSideOfTileEntity]
                != null
            && ((ITileEntityCoverable) tDelegator.mTileEntity).getCoverData().mBehaviours[tDelegator.mSideOfTileEntity]
                .interceptDisconnect(tDelegator.mSideOfTileEntity, mCovers))
            return F;
        byte oConnections = mConnections;
        mConnections &= ~SBIT[aSide];
        updateClientData();
        ...
        if (aNotify && tDelegator.mTileEntity instanceof ITileEntityConnector
            && SIDES_VALID[tDelegator.mSideOfTileEntity]
            && UT.Code.haveOneCommonElement(
                ((ITileEntityConnector) tDelegator.mTileEntity).getConnectorTypes(tDelegator.mSideOfTileEntity),
                getConnectorTypes(aSide)))
            ((ITileEntityConnector) tDelegator.mTileEntity).disconnect(tDelegator.mSideOfTileEntity, F);
        return T;
    }
