 ...
 */
public class MultiTileEntityWireRedstoneInsulated extends TileEntityBase10ConnectorRendered
    implements ITileEntityQuickObstructionCheck, ITileEntityRedstoneWire, ITileEntityProgress,
    ITileEntitySwitchableMode, IMTE_GetComparatorInputOverride {
    ...
    public long getRedstoneAtSide(byte aSide) {
        ...
        if (tDelegator.mTileEntity instanceof ITileEntityRedstoneWire)
            return canAcceptRedstoneFromWire(aSide, REDSTONE_ID) && ((ITileEntityRedstoneWire) tDelegator.mTileEntity)
                .canEmitRedstoneToWire(tDelegator.mSideOfTileEntity, REDSTONE_ID)
                    ? ((ITileEntityRedstoneWire) tDelegator.mTileEntity)
                        .getRedstoneMinusLoss(tDelegator.mSideOfTileEntity, REDSTONE_ID)
                    : 0;
        ...
        // Making sure the Glowstone Illuminators from Thermal Expansion are not flickering due to their weird Redstone
        // emitting Mechanics.
        if (MD.TE.mLoaded && tDelegator.mTileEntity != null
            && tDelegator.mTileEntity.getClass()
                .getName()
                .startsWith("cofh.thermalexpansion.block.light"))
            return 0;
    ...
    public boolean updateRedstone(int aRedstoneID) {
        ...
        for (byte tSide : ALL_SIDES_VALID_BUT[oReceived]) if ((tRedstone = getRedstoneAtSide(tSide)) > mRedstone) {
            mRedstone = tRedstone;
            mReceived = tSide;
        }
        if (mRedstone != oRedstone) {
            if (mConnectedToNonWire) causeBlockUpdate();
            return T;
        }
        return F;
    }
    ...
    public void updateConnectionStatus() {
        ...
        for (byte tSide : ALL_SIDES_VALID) if (canAcceptRedstoneFromVanilla(tSide)
            && !(getAdjacentTileEntity(tSide).mTileEntity instanceof ITileEntityRedstoneWire)) mConnectedToNonWire = T;
    }
    ...
    public byte isProvidingWeakPower2(byte aSide) {
        ...
        return UT.Code.bind4(
            UT.Code.divup(mRedstone, MAX_RANGE) - (tBlock instanceof BlockRedstoneWire
                || tBlock.isNormalCube(worldObj, xCoord + OFFX[aSide], yCoord + OFFY[aSide], zCoord + OFFZ[aSide]) ? 1
                    : 0));
    }
    ...
    public boolean canEmitRedstoneToVanilla(byte aSide) {
        return aSide != mReceived && connected(aSide)
            && !(getAdjacentTileEntity(aSide).mTileEntity instanceof ITileEntityRedstoneWire);
    }
    ...
    public boolean canEmitRedstoneToWire(byte aSide, int aRedstoneID) {
        return aRedstoneID == REDSTONE_ID && connected(aSide);
    }
    ...
    public long getRedstoneLoss(int aRedstoneID) {
        return aRedstoneID == REDSTONE_ID ? mLoss : MAX_RANGE;
    }
    ...
    public long getProgressValue(byte aSide) {
        return (1000 * mRedstone) / MAX_RANGE;
    }
    ...
    public ITexture getTextureSide(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return BlockTextureDefault
            .get(Textures.BlockIcons.INSULATION_FULL, isPainted() ? mRGBa : UT.Code.getRGBInt(96, 64, 64));
    }
    ...
    public ITexture getTextureConnected(byte aSide, byte aConnections, float aDiameter, int aRenderPass) {
        return BlockTextureMulti.get(
            ...
            BlockTextureDefault.get(
                ...
                isPainted() ? mRGBa : UT.Code.getRGBInt(96, 64, 64)));
    }
