 ...
 */
public abstract class TileEntityBase08Battery extends TileEntityBase07Paintable
    implements ITileEntityQuickObstructionCheck, IMTE_SetBlockBoundsBasedOnState, IMTE_GetCollisionBoundingBoxFromPool,
    IMTE_GetSelectedBoundingBoxFromPool, IItemEnergy, IMTE_GetMaxStackSize, IMTE_OnlyPlaceableWhenSneaking,
    IMTE_AddToolTips {
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        if (mCapacity > 0) aList.add(
            LH.Chat.WHITE + UT.Code.makeString(Math.min(mCapacity, mEnergy))
                + " / "
                + UT.Code.makeString(mCapacity)
                + " "
                + mType.getLocalisedChatNameShort()
                + LH.Chat.WHITE
                + (mSizeMin <= 1 ? " - Size: up to " + mSizeMax : " - Size: " + mSizeRec));
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) aChatReturn.add(
                UT.Code.makeString(mEnergy) + " of "
                    + UT.Code.makeString(mCapacity)
                    + " "
                    + mType.getLocalisedNameShort());
            return 1;
        }
        return 0;
    }
    ...
    public byte getMaxStackSize(ItemStack aStack, byte aDefault) {
        return mEnergy > 0 ? 1 : aDefault;
    }

    @Override
    public byte getVisualData() {
        return mDisplayedEnergy;
    }
    ...
    public ItemStack rechargeFromPlayer(TagData aEnergyType, ItemStack aStack, EntityLivingBase aPlayer,
        ...
        if (COMPAT_EU_ITEM == null || aPlayer == null
    ...
    public long getEnergyStored(TagData aEnergyType, ItemStack aStack) {
        return aEnergyType == mType || aEnergyType == null ? mEnergy : 0;
    }
    ...
    public long getEnergySizeInputMin(TagData aEnergyType, ItemStack aStack) {
        return aEnergyType == mType || aEnergyType == null ? mSizeMin : 0;
    }
    ...
    public long getEnergySizeInputRecommended(TagData aEnergyType, ItemStack aStack) {
        return aEnergyType == mType || aEnergyType == null ? mSizeRec : 0;
    }
    ...
    public long getEnergySizeInputMax(TagData aEnergyType, ItemStack aStack) {
        return aEnergyType == mType || aEnergyType == null ? mSizeMax : 0;
    }
    ...
    public boolean isEnergyType(TagData aEnergyType, ItemStack aStack, boolean aEmitting) {
        return (aEnergyType == mType || aEnergyType == null);
    }

    @Override
    public boolean canEnergyInjection(TagData aEnergyType, ItemStack aStack, long aSize) {
        return (aEnergyType == mType || aEnergyType == null) && aStack.stackSize == 1
