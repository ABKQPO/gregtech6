 ...
 */
public abstract class TileEntityBase10EnergyBatBox extends TileEntityBase09FacingSingle
    implements ITileEntityEnergy, ITileEntityEnergyDataCapacitor, ITileEntityRunningActively,
    ITileEntitySwitchableOnOff, ITileEntitySwitchableMode, ITileEntityProgress {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_ENERGY_EMITTED))
            mEnergyType = mEnergyTypeOut = TagData.createTagData(aNBT.getString(NBT_ENERGY_EMITTED));
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            ...
            if (SERVER_TIME % 20 == 1) {
                if (COMPAT_EU_ITEM == null || mEnergyType != TD.Energy.EU) {
                    switch (UT.Code.bind3(mEnergy / (mInput * 40 * invsize()))) {
                        case 0:
                            for (ItemStack tStack : getInventory())
                                if (ST.valid(tStack) && tStack.getItem() instanceof IItemEnergy)
                                    mEnergy += mOutput * ((IItemEnergy) tStack.getItem()).doEnergyExtraction(
                                        mEnergyType,
                                        tStack,
                                        mOutput,
                                        40,
                        ...
                        case 6:
                            for (ItemStack tStack : getInventory())
                                if (ST.valid(tStack) && tStack.getItem() instanceof IItemEnergy)
                                    mEnergy -= mInput * ((IItemEnergy) tStack.getItem()).doEnergyInjection(
                                        mEnergyType,
                                        tStack,
                                        mInput,
                                        20,
                            ...
                            for (ItemStack tStack : getInventory()) if (ST.valid(tStack)) {
                                if (tStack.getItem() instanceof IItemEnergy)
                                    mEnergy += mOutput * ((IItemEnergy) tStack.getItem()).doEnergyExtraction(
                                        ...
                                        zCoord,
                                        T);
                                else if (COMPAT_EU_ITEM.is(tStack) && COMPAT_EU_ITEM.provider(tStack)
                                    && !IL.IC2_EnergyCrystal.equal(tStack, T, T)
                                    && !IL.IC2_LapotronCrystal.equal(tStack, T, T)
                                    && COMPAT_EU_ITEM.insidevolt(tStack, mOutput / 2, mOutput * 2))
                                    mEnergy += COMPAT_EU_ITEM.decharge(tStack, mOutput * 40, T);
                            }
                            break;
                        case 1:
                            for (ItemStack tStack : getInventory()) if (ST.valid(tStack)) {
                                if (tStack.getItem() instanceof IItemEnergy)
                                    ...
                                    && !IL.IC2_LapotronCrystal.equal(tStack, T, T)
                                    && COMPAT_EU_ITEM.insidevolt(tStack, mOutput / 2, mOutput * 2))
                                    mEnergy += COMPAT_EU_ITEM.decharge(tStack, mOutput * 20, T);
                            }
                            break;
                        case 6:
                            for (ItemStack tStack : getInventory()) if (ST.valid(tStack)) {
                                if (tStack.getItem() instanceof IItemEnergy)
                                    mEnergy -= mInput * ((IItemEnergy) tStack.getItem()).doEnergyInjection(
                                        ...
                                        zCoord,
                                        T);
                                else if (COMPAT_EU_ITEM.is(tStack) && !IL.IC2_EnergyCrystal.equal(tStack, T, T)
                                    && !IL.IC2_LapotronCrystal.equal(tStack, T, T)
                                    && COMPAT_EU_ITEM.insidevolt(tStack, mInput / 2, mInput * 2))
                                    mEnergy -= COMPAT_EU_ITEM.charge(tStack, mInput * 20, T);
                            }
                            break;
                        case 7:
                            for (ItemStack tStack : getInventory()) if (ST.valid(tStack)) {
                                if (tStack.getItem() instanceof IItemEnergy)
                                    ...
                                    && !IL.IC2_LapotronCrystal.equal(tStack, T, T)
                                    && COMPAT_EU_ITEM.insidevolt(tStack, mInput / 2, mInput * 2))
                                    mEnergy -= COMPAT_EU_ITEM.charge(tStack, mInput * 40, T);
                            }
                            break;
                    }
                }
            ...
            if (mBatteryCount < 0 || mChargeableCount < 0 || mInventoryChanged) {
                ...
                for (ItemStack tStack : getInventory()) if (ST.valid(tStack)) {
                    if (tStack.getItem() instanceof IItemEnergy) {
                        if (((IItemEnergy) tStack.getItem()).canEnergyInjection(mEnergyType, tStack, mInput))
                            mChargeableCount++;
                        if (((IItemEnergy) tStack.getItem()).canEnergyExtraction(mEnergyType, tStack, mOutput))
                            mBatteryCount++;
                    } else if (COMPAT_EU_ITEM != null && mEnergyType == TD.Energy.EU
                        && COMPAT_EU_ITEM.is(tStack)
                        && !IL.IC2_EnergyCrystal.equal(tStack, T, T)
                        && !IL.IC2_LapotronCrystal.equal(tStack, T, T)) {
                            mChargeableCount++;
            ...
            if (mActive) {
                if (!mStopped) {
                    ...
                    if (tOutput > 0) {
                        long tEmittedPackets = ITileEntityEnergy.Util
                            .emitEnergyToNetwork(mEnergyTypeOut, mOutput, tOutput, this);
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        ...
        long tInput = Math.min(mInput * 320 * invsize() - mEnergy, aSize * aAmount),
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return aStack != null && aStack.getItem() instanceof IItemEnergy
            && (((IItemEnergy) aStack.getItem()).isEnergyType(mEnergyType, aStack, F)
                || ((IItemEnergy) aStack.getItem()).isEnergyType(mEnergyType, aStack, T));
    }
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return aEnergyType == (aEmitting ? mEnergyTypeOut : mEnergyType);
    }

    @Override
    public boolean isEnergyCapacitorType(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyType;
    }

    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (SIDES_INVALID[aSide] || isInput(aSide))
            && super.isEnergyAcceptingFrom(aEnergyType, aSide, aTheoretical);
    }

    @Override
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mStopped) && (SIDES_INVALID[aSide] || isOutput(aSide))
    ...
    public long getEnergyStored(TagData aEnergyType, byte aSide) {
        long rAmount = 0;
        for (ItemStack tStack : getInventory()) if (tStack != null && tStack.getItem() instanceof IItemEnergy)
    ...
    public Collection<TagData> getEnergyTypes(byte aSide) {
        return new ArrayListNoNulls<>(F, mEnergyType, mEnergyTypeOut);
    }

    @Override
    public Collection<TagData> getEnergyCapacitorTypes(byte aSide) {
        return mEnergyType.AS_LIST;
    }
