...
public abstract class TileEntityBase11Twotypes extends TileEntityBase10EnergyConverter
    ...
    public void readEnergyBehavior(NBTTagCompound aNBT) {
        ...
        mEnergyOUT2 = new TE_Behavior_Energy_Stats(
            this,
            aNBT,
            aNBT.hasKey(NBT_ENERGY_EMITTED_2) ? TagData.createTagData(aNBT.getString(NBT_ENERGY_EMITTED_2))
                : TD.Energy.QU,
            mStorage,
            aNBT.getLong(NBT_OUTPUT) / 2,
            aNBT.getLong(NBT_OUTPUT),
            aNBT.getLong(NBT_OUTPUT) * 2);
    }
    ...
    public void addToolTipsEfficiency(List<String> aList, ItemStack aStack, boolean aF3_H) {
        LH.addToolTipsEfficiency(
            aList,
            aStack,
            aF3_H,
            mConverter.mEnergyIN,
            mConverter.mEnergyOUT,
            mEnergyOUT2,
            mConverter.mMultiplier);
    }
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return aEmitting ? mConverter.mEnergyOUT.isType(aEnergyType) || mEnergyOUT2.isType(aEnergyType)
            : mConverter.mEnergyIN.isType(aEnergyType);
    }

    @Override
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return isEnergyType(aEnergyType, aSide, T) && (SIDES_INVALID[aSide]
            || (mConverter.mEnergyOUT.isType(aEnergyType) ? isOutput(aSide) : isOutput2(aSide)));
    }

    @Override
    public Collection<TagData> getEnergyTypes(byte aSide) {
        return new ArrayListNoNulls<>(F, mConverter.mEnergyIN.mType, mConverter.mEnergyOUT.mType, mEnergyOUT2.mType);
    }
    ...
    public boolean isOutput2(byte aSide) {
