 ...
 */
public abstract class TileEntityBase10EnergyConverter extends TileEntityBase09FacingSingle
    implements ITileEntityEnergy, ITileEntityRunningActively {
    ...
    public void readEnergyBehavior(NBTTagCompound aNBT) {
        ...
        mEnergyIN = new TE_Behavior_Energy_Stats(
            this,
            aNBT,
            aNBT.hasKey(NBT_ENERGY_ACCEPTED) ? TagData.createTagData(aNBT.getString(NBT_ENERGY_ACCEPTED))
                : TD.Energy.QU,
            mStorage,
            takesAnyLowerSize() || tInput <= 16 ? 1 : tInput / 2,
            tInput,
            tInput * 2);
        mEnergyOUT = new TE_Behavior_Energy_Stats(
            this,
            aNBT,
            aNBT.hasKey(NBT_ENERGY_EMITTED) ? TagData.createTagData(aNBT.getString(NBT_ENERGY_EMITTED))
                : mEnergyIN.mType,
            mStorage,
            emitsAnyLowerSize() ? 1 : tOutput / 2,
            tOutput,
            tOutput * 2);
    }
    ...
    public void readEnergyConverter(NBTTagCompound aNBT) {
        mConverter = new TE_Behavior_Energy_Converter(
            ...
            mEnergyIN,
            mEnergyOUT,
            aNBT.hasKey(NBT_MULTIPLIER) ? aNBT.getLong(NBT_MULTIPLIER) : 1,
            aNBT.getBoolean(NBT_WASTE_ENERGY),
            F,
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            doConversion(aTimer);
            if (mTimer % 600 == 5) if (mActivity.mActive) doDefaultStructuralChecks();
            else if (mExplosionPrevention > 0) mExplosionPrevention--;
        } else {
            if (mActivity.mState != 0 && (mEnergyIN.mType == TD.Energy.RU || mEnergyOUT.mType == TD.Energy.RU)
                && WD.random(this, 20, CLIENT_TIME)) UT.Sounds.play(SFX.MC_MINECART, 1, 0.2F, getCoords());
        }
    }

    @Override
    public boolean onTickCheck(long aTimer) {
        return mActivity.check(mStopped) || mNegativeInput != oNegativeInput || super.onTickCheck(aTimer);
    }

    @Override
    public void onTickResetChecks(long aTimer, boolean aIsServerSide) {
        super.onTickResetChecks(aTimer, aIsServerSide);
        oNegativeInput = mNegativeInput;
    }

    @Override
    public void setVisualData(byte aData) {
        mActivity.mState = (byte) (aData & 127);
        mNegativeInput = (aData < 0);
    }

    @Override
    public byte getVisualData() {
        return (byte) (mActivity.mState | (byte) (mNegativeInput ? B[7] : 0));
    }

    public void doConversion(long aTimer) {
        mActivity.mActive = mConverter.doConversion(
            aTimer,
            this,
            SIDE_ANY,
            mMode,
            mNegativeInput && TD.Energy.ALL_NEGATIVE_ALLOWED.contains(mEnergyIN.mType)
                && TD.Energy.ALL_NEGATIVE_ALLOWED.contains(mEnergyOUT.mType));
    ...
    public void overload(long aSize, TagData aEnergyType) {
        if (mExplosionPrevention < 100) {
            if (mTimer < 100)
                DEB.println("Machine overloaded on startup with: " + aSize + " " + aEnergyType.getLocalisedNameLong());
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return (aEmitting ? mConverter.mEnergyOUT : mConverter.mEnergyIN).isType(aEnergyType);
    }

    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical
            || (!mStopped && (mConverter.mWasteEnergy || (mConverter.mEmitsEnergy == mConverter.mCanEmitEnergy))))
            && (SIDES_INVALID[aSide] || isInput(aSide))
    ...
    public long getEnergySizeOutputMin(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyOUT.sizeMin(aEnergyType);
    }

    @Override
    public long getEnergySizeOutputRecommended(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyOUT.sizeRec(aEnergyType);
    }

    @Override
    public long getEnergySizeOutputMax(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyOUT.sizeMax(aEnergyType);
    }

    @Override
    public long getEnergySizeInputMin(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyIN.sizeMin(aEnergyType);
    }

    @Override
    public long getEnergySizeInputRecommended(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyIN.sizeRec(aEnergyType);
    }

    @Override
    public long getEnergySizeInputMax(TagData aEnergyType, byte aSide) {
        return mConverter.mEnergyIN.sizeMax(aEnergyType);
    }

    @Override
    public Collection<TagData> getEnergyTypes(byte aSide) {
        return new ArrayListNoNulls<>(F, mConverter.mEnergyIN.mType, mConverter.mEnergyOUT.mType);
    }
    ...
    public boolean getStateRunningActively() {
        return mConverter.mEmitsEnergy;
    }

    public boolean setAdjacentOnOff(boolean aOnOff) {
        if (mConverter.mWasteEnergy) mStopped = !aOnOff;
        return !mStopped;
    }
