 ...
 *         Client Side Worlds (worldObj.isRemote == true) should never access any of these Functions nor expect anything
 *         from calling them.
 * 
...
public interface ITileEntityEnergy extends gregapi.tileentity.ITileEntityEnergy {
     ...
     * @param aSide       0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (don't do Side
     *                    checks for this Side)
     * @return if this TileEntity has anything to do with this Type of Energy, depending on insert or extract request.
     *         The returning Value must be constant for this TileEntity.
     */
    @Override
     ...
     * 
     * @param aSide 0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (should return all
     *              related Types of Energy)
     * @return any Type of Energy that is related to this TileEntity. This is especially useful for Data Displays and
     *         Redstone Conditions, where people can select the Energy Type via GUI or something.
     */
    @Override
     ...
     * @param aSide        0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (don't do Side
     *                     checks for this Side)
     * @param aTheoretical true if this is only checking for Conductor Connections. Use it in case things like Redstone
     *                     or Covers for example can toggle the connectivity of this Side for this Energy Type, but you
     *                     still want to visually connect to it, even if it temporarily wouldn't accept Energy
     *                     Interactions from that Side right now. Basically, so that the Conductor doesn't visually
     *                     toggle on/off all the time causing Sync Lag.
     */
    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical);

    /**
     * This Method may be called by a different Thread. Take that into account when implementing it. As in do not make a
     * direct call to set Blocks or explode things, just set a flag to do it in the next tick.
     * 
     * @param aEnergyType  The Type of Energy
     * @param aSide        0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (don't do Side
     *                     checks for this Side)
     ...
     * 
     * Note: The IMPLEMENTOR of this Function has to check for isEnergyAcceptingFrom, when implementing this Function,
     * NOT the one injecting the Energy, because the Network won't check for that by itself.
     * 
     * @param aEnergyType The Type of Energy
     ...
     * @param aSide       0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (don't do Side
     *                    checks for this Side)
     * @param aSize       the Energy Packet Size to be demanded. getEnergySizeInputRecommended is the recommended Power
     *                    Level.
     ...
     * 
     * Note: The IMPLEMENTOR of this Function has to check for isEnergyEmittingTo, when implementing this Function, NOT
     * the one extracting the Energy, because the Network won't check for that by itself.
     * 
     * @param aEnergyType The Type of Energy
     ...
     * @param aSide       0 - 5 = Vanilla Directions of the Implementors Block. 6 = No specific Side (don't do Side
     *                    checks for this Side)
     * @param aSize       the Energy Packet Size to be taken. getEnergySizeOutputRecommended is the recommended Power
     *                    Level.
    ...
    // Parameters for the maximum Packet Sizes before bad shit happens. Anything greater than this Value would cause an
    // explosion in a GregTech Machine. Just saying.
    ...
    public static class Util {
         ...
         * @param aSize       The Minimum Transfer Rate of Energy (like Voltage for example). This can be negative too
         *                    in case it has a direction for example (clockwise/counterclockwise)
         ...
         */
        public static final long emitEnergyToNetwork(TagData aEnergyType, long aSize, long aAmount,
            gregapi.tileentity.ITileEntityEnergy aEmitter) {
            long rUsedAmount = 0;
            for (byte tSide : ALL_SIDES_VALID) if (aEmitter.isEnergyEmittingTo(aEnergyType, tSide, F)) {
                rUsedAmount += emitEnergyToSide(
                    aEnergyType,
                    tSide,
                    aSize,
                    aAmount - rUsedAmount,
         ...
         * @return the amount of used Energy Packets.
         */
        public static final long emitEnergyToSide(TagData aEnergyType, byte aSideOutOf, long aSize, long aAmount,
            TileEntity aEmitter) {
            DelegatorTileEntity<TileEntity> tDelegator = aEmitter instanceof IHasWorldAndCoords
                ? ((IHasWorldAndCoords) aEmitter).getAdjacentTileEntity(aSideOutOf)
                : WD.te(
                    aEmitter.getWorldObj(),
                    aEmitter.xCoord + OFFX[aSideOutOf],
                    aEmitter.yCoord + OFFY[aSideOutOf],
                    aEmitter.zCoord + OFFZ[aSideOutOf],
                    OPOS[aSideOutOf],
                    F);
         ...
         * @return the amount of used Energy Packets.
         */
        public static final long insertEnergyInto(TagData aEnergyType, byte aSideInto, long aSize, long aAmount,
            Object aEmitter, TileEntity aReceiver) {
            return aReceiver instanceof ITileEntityEnergy
                ? ((ITileEntityEnergy) aReceiver).doEnergyInjection(aEnergyType, aSideInto, aSize, aAmount, T)
                : EnergyCompat.insertEnergyInto(aEnergyType, aSideInto, aSize, aAmount, aEmitter, aReceiver);
        }
         ...
         * @return the amount of used Energy Packets.
         */
        public static final long insertEnergyInto(TagData aEnergyType, long aSize, long aAmount, Object aEmitter,
            DelegatorTileEntity<TileEntity> aReceiver) {
            return insertEnergyInto(
                aEnergyType,
                aReceiver.mSideOfTileEntity,
                aSize,
                aAmount,
                aEmitter,
                aReceiver.mTileEntity);
        }
    }
}
