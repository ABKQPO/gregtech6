...
public class StoneLayerOres {
    ...
    @SuppressWarnings("rawtypes")
    public StoneLayerOres(OreDictMaterial aMaterial, long aChance, int aMinY, int aMaxY, Block aBlock,
        Collection... aBiomes) {
        this(aMaterial, T, aChance, aMinY, aMaxY, aBlock, 0, aBiomes);
    }

    @SuppressWarnings("rawtypes")
    public StoneLayerOres(OreDictMaterial aMaterial, long aChance, int aMinY, int aMaxY, Block aBlock, long aMeta,
        Collection... aBiomes) {
        this(aMaterial, T, aChance, aMinY, aMaxY, aBlock, aMeta, aBiomes);
    }

    @SuppressWarnings("rawtypes")
    public StoneLayerOres(OreDictMaterial aMaterial, boolean aGenerateIndicators, long aChance, int aMinY, int aMaxY,
        Collection... aBiomes) {
    ...
    public boolean check(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome,
        int aRandomNumber) {
        return aY >= mMinY && aY <= mMaxY
            && aRandomNumber < mChance
            && (mTargetBiomes.isEmpty() || mTargetBiomes.contains(aBiome));
    }

    @SuppressWarnings("unlikely-arg-type")
    public boolean check(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome, Random aRandom) {
        return aY >= mMinY && aY <= mMaxY
            && aRandom.nextInt((int) U) < mChance
            && (mTargetBiomes.isEmpty() || mTargetBiomes.contains(aBiome));
    }
    ...
    public boolean set(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome, Random aRandom) {
        ...
        return aY == mMinY || aY == mMaxY || aRandom.nextBoolean() ? small(aLayer, aWorld, aX, aY, aZ, aBiome)
            : normal(aLayer, aWorld, aX, aY, aZ, aBiome);
    }

    public boolean set(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome) {
        if (mBlock != null) return aWorld.setBlock(aX, aY, aZ, mBlock, mMeta, 0);
        return aY == mMinY || aY == mMaxY || RNGSUS.nextBoolean() ? small(aLayer, aWorld, aX, aY, aZ, aBiome)
            : normal(aLayer, aWorld, aX, aY, aZ, aBiome);
    }

    public boolean normal(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome) {
        if (mBlock != null) return aWorld.setBlock(aX, aY, aZ, mBlock, mMeta, 0);
        return aLayer.mOre != null
            && aLayer.mOre.placeBlock(aWorld, aX, aY, aZ, SIDE_UNKNOWN, mMaterial.mID, null, F, T);
    }

    public boolean small(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome) {
        if (mBlock != null) return aWorld.setBlock(aX, aY, aZ, mBlock, mMeta, 0);
        return aLayer.mOreSmall != null
            && aLayer.mOreSmall.placeBlock(aWorld, aX, aY, aZ, SIDE_UNKNOWN, mMaterial.mID, null, F, T);
    }

    public boolean broken(StoneLayer aLayer, World aWorld, int aX, int aY, int aZ, BiomeGenBase aBiome) {
        if (mBlock != null) return aWorld.setBlock(aX, aY, aZ, mBlock, mMeta, 0);
        return aLayer.mOreBroken != null
            && aLayer.mOreBroken.placeBlock(aWorld, aX, aY, aZ, SIDE_UNKNOWN, mMaterial.mID, null, F, T);
    }
}
