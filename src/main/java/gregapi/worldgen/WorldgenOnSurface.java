...
public abstract class WorldgenOnSurface extends WorldgenObject {
    ...
    @SafeVarargs
    public WorldgenOnSurface(String aName, boolean aDefault, int aAmount, int aProbability,
    ...
    public boolean generate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        for (int i = 0; i < 16; i++)
            for (int j = 0; j < 16; j++) if (tTargets[i][j]) if (aRandom.nextInt(mProbability) == 0) {
    ...
    public int canGenerate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        Random aRandom, BiomeGenBase[][] aBiomes, Set<String> aBiomeNames) {
        return checkForMajorWorldgen(aWorld, aMinX, aMinZ, aMaxX, aMaxZ) ? 0 : mAmount;
    }
