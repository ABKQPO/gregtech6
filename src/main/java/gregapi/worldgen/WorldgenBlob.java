...
public abstract class WorldgenBlob extends WorldgenObject {
    ...
    @SafeVarargs
    public WorldgenBlob(String aName, boolean aDefault, Block aBlock, int aBlockMeta, int aAmount, int aSize,
        int aProbability, int aMinY, int aMaxY, Collection<String> aBiomeList, boolean aAllowToGenerateinVoid,
    ...
    public boolean generate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        if (mBiomeList != null) {
            boolean temp = T;
            for (String tName : aBiomeNames) if (mBiomeList.contains(tName)) {
                temp = F;
                break;
            }
            if (temp) return F;
        ...
        if (aRandom.nextInt(mProbability) == 0) {
            for (int i = 0; i < mAmount; i++) {
                int tX = aMinX + aRandom.nextInt(16), tY = mMinY + aRandom.nextInt(mMaxY - mMinY),
                    tZ = aMinZ + aRandom.nextInt(16);
                if (mAllowToGenerateinVoid || !aWorld.getBlock(tX, tY, tZ)
                    ...
                    int bMinX = Integer.MAX_VALUE, bMinY = Integer.MAX_VALUE, bMinZ = Integer.MAX_VALUE,
                        bMaxX = Integer.MIN_VALUE, bMaxY = Integer.MIN_VALUE, bMaxZ = Integer.MIN_VALUE;
                    ...
                    for (int j = 0; j <= mSize; ++j) {
                        double bX = aX1 + (aX2 - aX1) * j / mSize, bY = aY1 + (aY2 - aY1) * j / mSize,
                            bZ = aZ1 + (aZ2 - aZ1) * j / mSize,
                            b = ((MathHelper.sin(j * (float) Math.PI / mSize) + 1) * tRandoms[j] + 1) / 2;
                    ...
                    boolean[][][] tCheck = new boolean[Math.max(4, bMaxX - bMinX + 1)][Math
                        .max(4, bMaxY - bMinY + 1)][Math.max(4, bMaxZ - bMinZ + 1)];
                    ...
                    for (int a = 0; a < tCheck.length; a++) for (int b = 0; b < tCheck[a].length; b++)
                        for (int c = 0; c < tCheck[a][b].length; c++) if (tCheck[a][b][c]) {
