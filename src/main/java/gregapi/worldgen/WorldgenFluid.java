...
public class WorldgenFluid extends WorldgenBlob {
    ...
    @SafeVarargs
    public WorldgenFluid(String aName, boolean aDefault, Block aBlock, int aBlockMeta, int aAmount, int aSize,
    ...
    public boolean tryPlaceStuff(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (WD.bedrock(aWorld, aX, aY, aZ, tTargetedBlock)) {
            return aY >= 1 && aY <= 4 ? aWorld.setBlock(aX, aY, aZ, mBlock, mBlockMeta, 0)
                : doBedrockStuff(aWorld, aX, aY, aZ, aRandom);
        }
        ...
        if (tTargetedBlock == Blocks.dirt || tTargetedBlock == Blocks.soul_sand
            || WD.ore_stone(tTargetedBlock, (byte) aWorld.getBlockMetadata(aX, aY, aZ))) {
