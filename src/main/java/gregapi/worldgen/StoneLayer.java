...
public class StoneLayer {
    ...
    public StoneLayer(BlockBase aStoneGT, StoneLayerOres... aOreChances) {
        this(
            ST.invalid(aStoneGT) ? Blocks.stone : aStoneGT,
    ...
    public StoneLayer(BlockBase aStoneGT, OreDictMaterial aMaterial, Block aStone, StoneLayerOres... aOreChances) {
        this(
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? Blocks.stone : aStoneGT : aStone,
    ...
    public StoneLayer(BlockBase aStoneGT, OreDictMaterial aMaterial, Block aStone, long aMetaStone,
        StoneLayerOres... aOreChances) {
        this(
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? Blocks.stone : aStoneGT : aStone,
    ...
    public StoneLayer(BlockBase aStoneGT, OreDictMaterial aMaterial, Block aStone, long aMetaStone, Block aCobble,
        long aMetaCobble, StoneLayerOres... aOreChances) {
        this(
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? Blocks.stone : aStoneGT : aStone,
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? 0 : 0 : aMetaStone,
    ...
    public StoneLayer(BlockBase aStoneGT, OreDictMaterial aMaterial, Block aStone, long aMetaStone, Block aCobble,
        long aMetaCobble, Block aMossy, long aMetaMossy, StoneLayerOres... aOreChances) {
        this(
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? Blocks.stone : aStoneGT : aStone,
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? 0 : 0 : aMetaStone,
            ST.invalid(aStone) ? ST.invalid(aStoneGT) ? Blocks.cobblestone : aStoneGT : aCobble,
    ...
    public StoneLayer(Block aStone, long aMetaStone, Block aCobble, long aMetaCobble, OreDictMaterial aMaterial,
        StoneLayerOres... aOreChances) {
        this(aStone, aMetaStone, aCobble, aMetaCobble, aCobble, aMetaCobble, aMaterial, aOreChances);
    }

    public StoneLayer(Block aStone, long aMetaStone, Block aCobble, long aMetaCobble, Block aMossy, long aMetaMossy,
        OreDictMaterial aMaterial, StoneLayerOres... aOreChances) {
        ...
        mMaterialSurface = (aStone == Blocks.stone ? MT.Stone
            : aStone instanceof BlockStones ? ((BlockStones) aStone).mMaterial : mMaterial);
        ...
        for (StoneLayerOres tOre : aOreChances) if (tOre != null && tOre.mMaterial != MT.Empty
            && ConfigsGT.WORLDGEN.get("stonelayers." + mMaterial.mNameInternal, tOre.mMaterial.mNameInternal, T))
            mOres.add(tOre);
    }
    ...
    public static final Set<Block> REPLACEABLE_BLOCKS = new HashSetNoNulls<>(
        F,
        Blocks.stone,
        Blocks.coal_ore,
        Blocks.iron_ore,
    ...
    /**
     * Whenever two Rock Types hit each other in WorldGen an Ore from the returned List will spawn. The first ones
     * mentioned inside the List can override the chances for others by spawning before, so insert the lowest chances
     * first and then the high chances.
     */
    ...
    public static boolean bothsides(OreDictMaterial aMaterialA, OreDictMaterial aMaterialB,
        StoneLayerOres... aOreChances) {
    ...
    public static boolean topbottom(OreDictMaterial aTop, OreDictMaterial aBottom, StoneLayerOres... aOreChances) {
        ...
        for (StoneLayerOres tOre : aOreChances) if (tOre != null && tOre.mMaterial != MT.Empty
            && ConfigsGT.WORLDGEN.get(
                "doublelayers." + aTop.mNameInternal + "." + aBottom.mNameInternal,
                tOre.mMaterial.mNameInternal,
                T))
            tList.add(tOre);
        return T;
    }
