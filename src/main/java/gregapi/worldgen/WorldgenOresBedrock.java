...
public class WorldgenOresBedrock extends WorldgenObject {
    ...
    @SafeVarargs
    public WorldgenOresBedrock(String aName, boolean aDefault, int aProbability, OreDictMaterial aPrimary,
        List<WorldgenObject>... aLists) {
        this(aName, aDefault, T, aProbability, aPrimary, aLists);
    }

    @SafeVarargs
    public WorldgenOresBedrock(String aName, boolean aDefault, boolean aIndicatorRocks, int aProbability,
        OreDictMaterial aPrimary, List<WorldgenObject>... aLists) {
        this(aName, aDefault, aIndicatorRocks, aProbability, aPrimary, NB, 0, aLists);
    }

    @SafeVarargs
    public WorldgenOresBedrock(String aName, boolean aDefault, boolean aIndicatorRocks, int aProbability,
        OreDictMaterial aPrimary, Object aFlower, long aFlowerMeta, List<WorldgenObject>... aLists) {
        ...
        if (mMaterial == ANY.Hexorium) {
            ...
            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, MT.HexoriumWhite.mID)),
                tOres,
                null,
                tChances,
            ...
            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, MT.HexoriumBlack.mID)),
                tOres,
                null,
                tChances,
                FL.array(FL.lube(100)),
            ...
            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, MT.HexoriumRed.mID)),
                tOres,
                null,
                tChances,
                FL.array(FL.lube(100)),
            ...
            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, MT.HexoriumGreen.mID)),
                tOres,
                null,
                tChances,
                FL.array(FL.lube(100)),
            ...
            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, MT.HexoriumBlue.mID)),
                tOres,
                null,
                tChances,
                FL.array(FL.lube(100)),
            ...
            for (int i = 0, j = mMaterial.mByProducts.size(); i < j; i++) {
                ...
                tOres[i + 1] = ST
                    .make((Block) BlocksGT.oreBroken, 1, (tByProduct.mID > 0 ? tByProduct : mMaterial).mID);
                tChances[i + 1] = UT.Code.divup(10000, (32 * (tChances.length - 2)));
            }

            RM.BedrockOreList.addFakeRecipe(
                F,
                ST.array(ST.make((Block) BlocksGT.oreBedrock, 1, mMaterial.mID)),
                tOres,
                null,
                tChances,
                FL.array(FL.lube(100)),
    ...
    @Override
    public void reset(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
    ...
    public boolean generate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        if ((mIndicatorRocks || mIndicatorFlowers) && (!GENERATE_STREETS || aWorld.provider.dimensionId != 0
            || (Math.abs(aMinX) >= 64 && Math.abs(aMaxX) >= 64 && Math.abs(aMinZ) >= 64 && Math.abs(aMaxZ) >= 64))) {
            MultiTileEntityRegistry tRegistry = (mIndicatorRocks
                ? MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                : null);
            ItemStack tRock = (tRegistry == null ? null
                : OP.oreRaw.mat(
                    mMaterial == ANY.Hexorium
                        ? UT.Code.select(MT.HexoriumBlack, ANY.Hexorium.mToThis.toArray(ZL_MATERIAL))
                        : mMaterial,
                    1));
            boolean tFlowers = (mIndicatorFlowers && !BIOMES_WASTELANDS.contains(aBiomes[8][8].biomeName)),
                tRocks = ST.valid(tRock);

            int tMinHeight = Math.min(aWorld.getHeight() - 2, WD.waterLevel(aWorld) - 1),
                tMaxHeight = Math.min(aWorld.getHeight() - 1, tMinHeight * 2 + 16);
            // Generate first an 8x8 of 4, then a 16x16 of 8, and at the end a 32x32 of 16 Rocks/Flowers. That way the
            // Pattern gets denser in the middle, and Chunk Boundary Issues of GalactiCraft wont be as terrible.
            for (int tD = 4; tD <= 16; tD *= 2) try {
                for (int i = 0; i < tD; i++) {
                    ...
                    for (int tY = tMaxHeight; tY > tMinHeight; tY--) {
                        ...
                        if (tContact.getMaterial()
                            .isLiquid() || tContact == Blocks.farmland) break;
                        if (!tContact.isOpaqueCube() || tContact.isWood(aWorld, tX, tY, tZ)
                            || tContact.isLeaves(aWorld, tX, tY, tZ)) continue;
                        ...
                        if (tRocks
                            && (tContact.getMaterial() == Material.grass || tContact.getMaterial() == Material.ground
                                || tContact.getMaterial() == Material.sand
                                || tContact.getMaterial() == Material.rock)) {
    ...
    public static boolean generateVein(OreDictMaterial aMaterial, World aWorld, int aDimType, int aMinX, int aMinZ,
        Random aRandom) {
        try {
            ...
            for (int tX = 5; tX < 11; tX++) for (int tZ = 5; tZ < 11; tZ++) {
                switch (aRandom.nextInt(6)) {
                    case 0:
                        BlocksGT.oreBedrock.placeBlock(
                            aWorld,
                            aMinX + tX,
                            0,
                            aMinZ + tZ,
                            SIDE_UNKNOWN,
                            (aMaterial == ANY.Hexorium
                                ? UT.Code.select(MT.HexoriumBlack, ANY.Hexorium.mToThis.toArray(ZL_MATERIAL))
                                : aMaterial).mID,
                            null,
                            F,
                    ...
                    case 1:
                    case 2:
                        BlocksGT.oreSmallBedrock.placeBlock(
                            aWorld,
                            aMinX + tX,
                            0,
                            aMinZ + tZ,
            ...
            BlocksGT.oreBedrock.placeBlock(
                aWorld,
                aMinX + 6 + aRandom.nextInt(4),
                0,
                aMinZ + 6 + aRandom.nextInt(4),
                SIDE_UNKNOWN,
                (aMaterial == ANY.Hexorium ? UT.Code.select(MT.HexoriumBlack, ANY.Hexorium.mToThis.toArray(ZL_MATERIAL))
                    : aMaterial).mID,
                null,
                F,
                T);
            // Use Deepslate if available, except in the Nether.
            tStone = (aDimType == DIM_NETHER ? Blocks.netherrack
                : StoneLayer.DEEPSLATE == null ? NB : StoneLayer.DEEPSLATE.mStone);
            ...
            for (int tY = 1; tY < tD1.length; tY++)
                for (int tX = tD1[tY]; tX < tD2[tY]; tX++) for (int tZ = tD1[tY]; tZ < tD2[tY]; tZ++) {
