...
public class DungeonChunkRoomMiningBedrock extends DungeonChunkRoomEmpty {
    ...
    public boolean generate(DungeonData aData) {
        ...
        OreDictMaterial tMaterial = UT.Code.select(
            MT.Redstone,
            MT.Redstone,
            MT.S,
            MT.Fe2O3,
        ...
        if (!WorldgenOresBedrock.generateVein(
            tMaterial,
            aData.mWorld,
            aData.mWorld.provider.dimensionId,
            aData.mX,
            aData.mZ,
            aData.mRandom)) return F;
        ...
        } else if (aData.mRoomLayout[aData.mRoomX][aData.mRoomZ + 1] != 0
        ...
        for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) {
            for (int i = tStart[a]; i <= tEnd[a]; i++) for (int j = tStart[b]; j <= tEnd[b]; j++) {
                if (aData.next3in4()) {
                    aData.set(BlocksGT.blockRaw, i, 5 - aData.mY, j, tMaterial.mID);
                    if (aData.next2in3()) {
                        aData.set(BlocksGT.blockRaw, i, 6 - aData.mY, j, tMaterial.mID);
                        if (aData.next1in2()) {
                            aData.set(BlocksGT.blockRaw, i, 7 - aData.mY, j, tMaterial.mID);
                        }
                    }
                }
            }
