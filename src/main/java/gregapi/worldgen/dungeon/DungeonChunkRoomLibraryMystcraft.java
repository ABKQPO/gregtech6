...
public class DungeonChunkRoomLibraryMystcraft extends DungeonChunkRoomLibrary {
    ...
    public boolean generate(DungeonData aData) {
        if (!MD.MYST.mLoaded || !aData.mTags.contains(WorldgenDungeonGT.TAG_LIBRARY_MYST) || !super.generate(aData))
            return F;
        aData.mTags.add(WorldgenDungeonGT.TAG_LIBRARY_MYST);

        Block tWritingTable = IL.Myst_Desk_Block.block(), tBookBinder = IL.Myst_Book_Binder.block(),
            tInkMixer = IL.Myst_Ink_Mixer.block(), tLectern = IL.Myst_Lectern.block(),
            tStand = IL.Myst_Bookstand.block();
        boolean temp = T;
        if (aData.mRoomLayout[aData.mRoomX + 1][aData.mRoomZ] == 0) {
            if (temp) {
                ...
                aData.set(
                    ...
                    6,
                    32705,
                    FluidTankGT.writeToNBT(NBT_TANK + ".out.0", FL.InkMyst.make(1000 + 1000 * aData.next(8))));
