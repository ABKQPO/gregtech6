...
public class DungeonChunkEntrance extends DungeonChunkPillar {
    ...
    public boolean generate(DungeonData aData) {
        ...
        } // The Pillar is not important enough to fail the entire Entrance.
        ...
        for (int eY = aData.mWorld.getHeight() - 32, tAirAmount = 0; tHeight < eY && tAirAmount < 100; tHeight++) {
            tAirAmount = 0;
            for (int tX = 3 + aData.mX, eX = tX + 10; tX < eX; tX++)
                for (int tZ = 3 + aData.mZ, eZ = tZ + 10; tZ < eZ; tZ++) {
                    Block tBlock = WD.block(aData.mWorld, tX, tHeight, tZ);
                    if (WD.easyRep(aData.mWorld, tX, tHeight, tZ, tBlock)
                        || tBlock.isWood(aData.mWorld, tX, tHeight, tZ)) tAirAmount++;
                }
        }
        tHeight -= aData.mY;
        ...
        for (int tY = Math.max(8, tHeight - 2); tY <= tHeight + 2; tY++)
