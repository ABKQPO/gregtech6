 ...
 *         It is a good Idea to just extend this Class and do a super Call in order to have a Pillar generated before
 *         placing your Exterior Design.
 */
public class DungeonChunkPillar implements IDungeonChunk {
    ...
    public boolean generate(DungeonData aData) {
        ...
        for (int tX = 6; tX <= 9 && temp; tX++) for (int tZ = 6; tZ <= 9 && temp; tZ++)
            if (WD.opq(aData.mWorld, aData.mX + tX, aData.mY - 1, aData.mZ + tZ, T, F)) temp = F;
        ...
        for (int tY = -3; aData.mY + tY >= 2 && temp; tY--) {
            ...
            for (int tX = 6; tX <= 9 && !temp; tX++) for (int tZ = 6; tZ <= 9 && !temp; tZ++) {
                ...
                if (tBlock instanceof BlockFalling || !tBlock.isOpaqueCube()
                    || tBlock == BlocksGT.Sands
                    || tBlock == BlocksGT.Diggables) temp = T;
            }
            if (temp) {
                ...
                for (int tX = 5; tX <= 10; tX++) for (int tZ = 5; tZ <= 10; tZ++) {
                    ...
                    if (tY > 2 || aData.mWorld.getBlock(aData.mX + tX, 0, aData.mZ + tZ)
                        .getBlockHardness(aData.mWorld, aData.mX + tX, 0, aData.mZ + tZ) >= 0)
