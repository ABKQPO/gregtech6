...
public class GT6WorldGenerator {
    ...
    public static class WorldGenContainer implements Runnable {
        ...
        public WorldGenContainer(List<WorldgenObject> aGenNormal, List<WorldgenObject> aGenLargeOres, int aDimType,
        ...
        public void run() {
            if (!mGenNormal.isEmpty()) {
                ...
                for (int i = 0; i < 16; i++) for (int j = 0; j < 16; j++) {
                    ...
                    if (tBiomes[i][j] == null) {
                        tBiomes[i][j] = (mDimType == DIM_NETHER ? BiomeGenBase.hell
                            : mDimType == DIM_END ? BiomeGenBase.sky : BiomeGenBase.plains);
                    } else {
                ...
                for (WorldgenObject tWorldGen : mGenNormal) tWorldGen
                    .reset(mWorld, tChunk, mDimType, mMinX, mMinZ, mMaxX, mMaxZ, mRandom, tBiomes, tBiomeNames);
                for (WorldgenObject tWorldGen : mGenNormal) if (tWorldGen.enabled(mWorld, mDimType)) {
                    ...
                    if (tChunk.lastSaveTime == Long.MAX_VALUE) {
                        ...
                        throw new RuntimeException(
                            "A corrupted Chunk was found while generating at (X: " + mMinX
                                + " Z: "
                                + mMinZ
                                + "), please try loading the World again to see if this specific Chunk consistently corrupts, and >>>ONLY<<< if it does so, please report this to Greg.");
                    }
                }
                ...
                if (mGenLargeOres != null && !mGenLargeOres.isEmpty()) {
                    ...
                    for (WorldgenObject tWorldGen : mGenLargeOres) if (tWorldGen.enabled(mWorld, mDimType)) {
                ...
                for (EntityItem tEntity : (List<EntityItem>) mWorld.getEntitiesWithinAABB(
                    EntityItem.class,
                    AxisAlignedBB.getBoundingBox(mMinX - 32, 0, mMinZ - 32, mMinX + 48, 256, mMinZ + 48)))
                    tEntity.setDead();
    ...
    public static void generate(World aWorld, int aX, int aZ, boolean aGalactiCraft) {
            ...
            case DIM_OVERWORLD:
                generate(
                    new WorldGenContainer(
                        TFC ? GEN_TFC : PFAA ? GEN_PFAA : GENERATE_STONE ? GEN_GT : GEN_OVERWORLD,
                        TFC ? ORE_TFC : PFAA ? ORE_PFAA : GENERATE_STONE ? null : ORE_OVERWORLD,
                        DIM_OVERWORLD,
                        aWorld,
                        aX,
                        aZ));
        ...
        if (WD.dimENVM(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_ENVM_GT : GEN_ENVM,
                    GENERATE_STONE ? null : ORE_ENVM,
                    DIM_ENVM,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimA97(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_A97_GT : GEN_A97,
                    GENERATE_STONE ? null : ORE_A97,
                    DIM_A97,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCW2AquaCavern(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_CW2_AquaCavern_GT : GEN_CW2_AquaCavern,
                    GENERATE_STONE ? null : ORE_CW2_AquaCavern,
                    DIM_CW2_AquaCavern,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCW2Caveland(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_CW2_Caveland_GT : GEN_CW2_Caveland,
                    GENERATE_STONE ? null : ORE_CW2_Caveland,
                    DIM_CW2_Caveland,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCW2Cavenia(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_CW2_Cavenia_GT : GEN_CW2_Cavenia,
                    GENERATE_STONE ? null : ORE_CW2_Cavenia,
                    DIM_CW2_Cavenia,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCW2Cavern(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_CW2_Cavern_GT : GEN_CW2_Cavern,
                    GENERATE_STONE ? null : ORE_CW2_Cavern,
                    DIM_CW2_Cavern,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCW2Caveworld(aWorld)) {
            generate(
                new WorldGenContainer(
                    GENERATE_STONE ? GEN_CW2_Caveworld_GT : GEN_CW2_Caveworld,
                    GENERATE_STONE ? null : ORE_CW2_Caveworld,
                    DIM_CW2_Caveworld,
                    aWorld,
                    aX,
                    aZ));
        ...
        if (WD.dimCANDY(aWorld)) {
            generate(new WorldGenContainer(GEN_CANDY, ORE_CANDY, DIM_CANDY, aWorld, aX, aZ));
            return;
        }
