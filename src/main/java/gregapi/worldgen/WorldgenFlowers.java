...
public class WorldgenFlowers extends WorldgenOnSurface {
    ...
    @SafeVarargs
    public WorldgenFlowers(String aName, boolean aDefault, Block aBlock, int aBlockMeta, int aAmount, int aProbability,
        Collection<String> aBiomeList, List<WorldgenObject>... aLists) {
    ...
    public boolean tryPlaceStuff(World aWorld, int aX, int aY, int aZ, Random aRandom, Block aContact) {
        return WD.easyRep(aWorld, aX, aY + 1, aZ) && mBlock.canBlockStay(aWorld, aX, aY + 1, aZ)
            && WD.set(aWorld, aX, aY + 1, aZ, mBlock, mBlockMeta, 2);
    }
}
