...
public class MD {
    ...
    public static final ModData MC = new ModData(ModIDs.MC, "Minecraft").setLoaded(T)
        ...
        , GT = new ModData(ModIDs.GT, "GregTech6"), GAPI = new ModData(ModIDs.GAPI, "Greg-API"),
        GAPI_POST = new ModData(ModIDs.GAPI_POST, "Greg-API-Post"),
        GT5U = new ModData(ModIDs.GT5U, "GregTech 5 Unofficial"), GT6U = new ModData(ModIDs.GT, "GregTech 6 Unofficial")
        ...
        , NC = new ModData(ModIDs.NC, "Nuclear Control"), IHL = new ModData(ModIDs.IHL, "IHL")

        , FMB = new ModData(ModIDs.FMB, "Forge Microblocks"),
        TRANSLOCATOR = new ModData(ModIDs.TRANSLOCATOR, "Translocator"),
        FUNK = new ModData(ModIDs.FUNK, "Funky Locomotion"), BAUBLES = new ModData(ModIDs.BAUBLES, "Baubles")

        , TC = new ModData(ModIDs.TC, "Thaumcraft"), TCTE = new ModData(ModIDs.TCTE, "Thaumcraft Extras"),
        TCFM = new ModData(ModIDs.TCFM, "Forbidden Magic"), TECHNOM = new ModData(ModIDs.TECHNOM, "Technomancy"),
        BOTA = new ModData(ModIDs.BOTA, "Botania"), ALF = new ModData(ModIDs.ALF, "Alfheim"),
        PE = new ModData(ModIDs.PE, "Project E"), WTCH = new ModData(ModIDs.WTCH, "Witchery"),
        HOWL = new ModData(ModIDs.HOWL, "Howling Moon"), TF = new ModData(ModIDs.TF, "Twilight Forest"),
        ERE = new ModData(ModIDs.ERE, "Erebus"), ATUM = new ModData(ModIDs.ATUM, "Atum"),
        BTL = new ModData(ModIDs.BTL, "The Betweenlands"), AETHER = new ModData(ModIDs.AETHER, "The Aether"),
        AETHEL = new ModData(ModIDs.AETHEL, "The Aether"), TROPIC = new ModData(ModIDs.TROPIC, "Tropicraft"),
        MYST = new ModData(ModIDs.MYST, "Mystcraft"), WARPBOOK = new ModData(ModIDs.WARPBOOK, "Warpbook"),
        ARS = new ModData(ModIDs.ARS, "Ars Magica"), CANDY = new ModData(ModIDs.CANDY, "CandyCraft"),
        ...
        , AE = new ModData(ModIDs.AE, "Applied Energistics"), MO = new ModData(ModIDs.MO, "Matter Overdrive")

        , TFC = new ModData(ModIDs.TFC, "TerraFirmaCraft"), TFCP = new ModData(ModIDs.TFCP, "TerraFirmaCraft Plus")

        , MET = new ModData("Metallurgy", "Metallurgy")

        , Streams = new ModData("streams", "Streams")

        , ZTONES = new ModData(ModIDs.ZTONES, "Ztones"), CHSL = new ModData(ModIDs.CHSL, "Chisel")

        , NePl = new ModData(ModIDs.NePl, "Netherite Plus"), NeLi = new ModData(ModIDs.NeLi, "Netherlicious"),
        EnLi = new ModData(ModIDs.EnLi, "Enderlicious"), EtFu = new ModData(ModIDs.EtFu, "Et Futurum"),
        BB = new ModData(ModIDs.BB, "Better Beginnings"),
        DYNAMIC_TREES = new ModData(ModIDs.DYNAMIC_TREES, "Dynamic Trees")

        , BbLC = new ModData(ModIDs.BbLC, "BiblioCraft"), CARP = new ModData(ModIDs.CARP, "Carpenters Blocks"),
        ...
        LOSTBOOKS = new ModData(ModIDs.LOSTBOOKS, "Lost Books"), LOOTBAGS = new ModData(ModIDs.LOOTBAGS, "Lootbags"),
        EUREKA = new ModData(ModIDs.EUREKA, "Eureka"), VN4 = new ModData(ModIDs.VN4, "Village Names")

        , UB = new ModData(ModIDs.UB, "Underground Biomes"), COG = new ModData(ModIDs.COG, "Custom Ore Generation"),
        PFAA = new ModData(ModIDs.PFAA, "Per Fabrica Ad Astra"), MIN = new ModData(ModIDs.MIN, "Mineralogy"),
        RH = new ModData(ModIDs.RH, "Rockhounding")

        , FR = new ModData(ModIDs.FR, "Forestry"), FRMB = new ModData(ModIDs.FRMB, "Magic Bees"),
        ...
        , MFR = new ModData(ModIDs.MFR, "MineFactory Reloaded"), PnC = new ModData(ModIDs.PnC, "PneumaticCraft"),
        FSP = new ModData(ModIDs.FSP, "Flaxbeard's Steam Power"), SC2 = new ModData(ModIDs.SC2, "Steamcraft 2"),
        ExU = new ModData(ModIDs.ExU, "Extra Utilities"), ExS = new ModData(ModIDs.ExS, "Extra Simple"),
        EIO = new ModData(ModIDs.EIO, "Ender IO"), RT = new ModData(ModIDs.RT, "Random Things"),
        AA = new ModData(ModIDs.AA, "Actually Additions")

        , SD = new ModData(ModIDs.SD, "Storage Drawers"), BTRS = new ModData(ModIDs.BTRS, "Better Storage"),
        JABBA = new ModData(ModIDs.JABBA, "JABBA")

        , MgC = new ModData(ModIDs.MgC, "Magneticraft"), BR = new ModData(ModIDs.BR, "Big Reactors"),
        HBM = new ModData(ModIDs.HBM, "HBM's Nuclear Tech Mod"), ELN = new ModData(ModIDs.ELN, "Electrical Age")

        , DRGN = new ModData(ModIDs.DRGN, "Dragon API"), RoC = new ModData(ModIDs.RoC, "RotaryCraft"),
        ReC = new ModData(ModIDs.ReC, "ReactorCraft"), ElC = new ModData(ModIDs.ElC, "ElectriCraft"),
        CrC = new ModData(ModIDs.CrC, "ChromatiCraft")

        , VOLTZ = new ModData(ModIDs.VOLTZ, "Voltz Engine"),
        MFFS = new ModData(ModIDs.MFFS, "Modular Force Field System"), ICBM = new ModData(ModIDs.ICBM, "ICBM"),
        ATSCI = new ModData(ModIDs.ATSCI, "Atomic Science")

        , Mek = new ModData(ModIDs.Mek, "Mekanism"), Mek_Tools = new ModData(ModIDs.Mek_Tools, "Mekanism Tools"),
        Mek_Generators = new ModData(ModIDs.Mek_Generators, "Mekanism Generators")

        , OC = new ModData(ModIDs.OC, "Open Computers"), CC = new ModData(ModIDs.CC, "ComputerCraft")

        , TreeCap = new ModData(ModIDs.TreeCap, "Treecapitator"), HaC = new ModData(ModIDs.HaC, "HarvestCraft"),
        Salt = new ModData(ModIDs.Salt, "Salty Mod"), CookBook = new ModData(ModIDs.CookBook, "Cooking for Blockheads"),
        APC = new ModData(ModIDs.APC, "AppleCore"), HO = new ModData(ModIDs.HO, "Hunger Overhaul"),
        ENVM = new ModData(ModIDs.ENVM, "Enviromine"), MaCr = new ModData(ModIDs.MaCr, "Magical Crops"),
        MaCu = new ModData(ModIDs.MaCu, "Mariculture"), MoCr = new ModData(ModIDs.MoCr, "Mo'Creatures"),
        WiMo = new ModData(ModIDs.WiMo, "Wild Mobs"), Birb = new ModData(ModIDs.Birb, "Exotic Birbs"),
        ChocoCraft = new ModData(ModIDs.ChocoCraft, "ChocoCraft (Plus)"),
        GoG = new ModData(ModIDs.GoG, "Grimoire of Gaia"), DRPG = new ModData(ModIDs.DRPG, "Divine RPG"),
        PdC = new ModData(ModIDs.PdC, "Psychedelicraft") // Wait, why did I add some compat to this Drug Mod again?
        , Bamboo = new ModData(ModIDs.Bamboo, "Bamboo Mod"), PMP = new ModData(ModIDs.PMP, "Plant Mega Pack"),
        Fossil = new ModData(ModIDs.Fossil, "Fossils and Archeology"), GrC = new ModData(ModIDs.GrC, "Growthcraft"),
        ...
        , CrGu = new ModData(ModIDs.CrGu, "Craft Guide"), SmAc = new ModData(ModIDs.SmAc, "Simple Achievements"),
        HQM = new ModData(ModIDs.HQM, "Hardcore Questing Mode")

        , HEX = new ModData(ModIDs.HEX, "HEXCraft"), DE = new ModData(ModIDs.DE, "Draconic Evolution"),
        AV = new ModData(ModIDs.AV, "Avaritia")

        , EB = new ModData(ModIDs.EB, "Enhanced Biomes"), EBXL = new ModData(ModIDs.EBXL, "Extra Biomes XL"),
        BoP = new ModData(ModIDs.BoP, "Biomes O' Plenty"), HiL = new ModData(ModIDs.HiL, "Highlands")
        ...
        , GaSu = new ModData(ModIDs.GaSu, "Ganys Surface"), GaNe = new ModData(ModIDs.GaNe, "Ganys Nether"),
        GaEn = new ModData(ModIDs.GaEn, "Ganys End"), WdSt = new ModData(ModIDs.WdSt, "Ganys Wood Stuff")
        ...
        , BC = new ModData(ModIDs.BC, "BuildCraft"), BC_SILICON = new ModData(ModIDs.BC_SILICON, "BuildCraft Silicon"),
        ...
        , RP = new ModData(ModIDs.RP, "Redpower"), BP = new ModData(ModIDs.BP, "Blue Power"),
        ...
        , OB = new ModData(ModIDs.OB, "Open Blocks"), PA = new ModData(ModIDs.PA, "Progressive Automation"),
        MNTL = new ModData(ModIDs.MNTL, "Mantle"), TiC = new ModData(ModIDs.TiC, "Tinkers Construct"),
        Natura = new ModData(ModIDs.Natura, "Natura"), MF2 = new ModData(ModIDs.MF2, "MineFantasy II"),
        FZ = new ModData(ModIDs.FZ, "Factorization"), BWM = new ModData(ModIDs.BWM, "Balkon's Weapon Mod"),
        BG2 = new ModData(ModIDs.BG2, "Battlegear 2"), OMT = new ModData(ModIDs.OMT, "Open Modular Turrets"),
        TG = new ModData(ModIDs.TG, "Tech Guns")

        , WARPDRIVE = new ModData(ModIDs.WARPDRIVE, "Warp Drive"), FM = new ModData(ModIDs.FM, "Falling Meteors"),
        ...
        VULPES = new ModData(ModIDs.VULPES, "Lib Vulpes"), MD8 = new ModData(ModIDs.MD8, "Micdoodle8 Core")
