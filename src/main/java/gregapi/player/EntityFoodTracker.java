...
public class EntityFoodTracker implements IExtendedEntityProperties {
    ...
    public void saveNBTData(NBTTagCompound aNBT) {
        ...
        if (tNBT.hasNoTags()) aNBT.removeTag("gt.props.food");
        else aNBT.setTag("gt.props.food", tNBT);
    }
        ...
        if (SERVER_TIME % 50 == 0) for (int i = 0; i < TICK_LIST.size(); i++) {
            ...
            if (tTracker.mRadiation >= 100) {
                UT.Entities.applyPotion(
                    tTracker.mEntity,
                    PotionsGT.ID_RADIATION >= 0 ? PotionsGT.ID_RADIATION : Potion.wither.id,
                    100,
                    0,
                    F);
                ...
                UT.Entities.applyPotion(
                    tTracker.mEntity,
                    PotionsGT.ID_RADIATION >= 0 ? PotionsGT.ID_RADIATION : Potion.poison.id,
                    100,
                    0,
                    F);
            ...
            if (NUTRITION_SYSTEM) {
                ...
                if (tTracker.mDehydration >= 100) {
                    if (FOOD_OVERDOSE_DEATH || tTracker.mEntity.getHealth() >= 2) tTracker.mEntity
                        .attackEntityFrom(DamageSources.getDehydrationDamage(), FOOD_OVERDOSE_DEATH ? 2 : 1);
                    UT.Entities.applyPotion(
                        tTracker.mEntity,
                        PotionsGT.ID_DEHYDRATION >= 0 ? PotionsGT.ID_DEHYDRATION : Potion.hunger.id,
                        1200,
                        3,
                        F);
            ...
            if (SERVER_TIME % 100 == 0) {
                ...
                // if (tTracker.mRadiation > 0) tTracker.mRadiation--; // The only one that does not decrease, so you
                // will have to deal with it until you either die or get a Radaway,
            }
        }
    }
