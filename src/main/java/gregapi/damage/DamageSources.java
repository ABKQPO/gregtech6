...
public class DamageSources {
    ...
    public static DamageSource getCombatDamage(String aType, EntityLivingBase aPlayer, IChatComponent aDeathMessage) {
        return getCombatDamage(aType, aPlayer, aDeathMessage, F);
    }
    ...
    public static IChatComponent getDeathMessage(EntityLivingBase aPlayer, Entity aEntity, String aMessage) {
        return getDeathMessage(
            aPlayer,
            aEntity,
            UT.Code.stringValidate(aPlayer.getCommandSenderName(), "Someone"),
            UT.Code.stringValidate(aEntity.getCommandSenderName(), "Someone"),
            aMessage);
    }

    public static IChatComponent getDeathMessage(EntityLivingBase aPlayer, Entity aEntity, String aNamePlayer,
        String aNameEntity, String aMessage) {
        ...
        if (aNamePlayer.equalsIgnoreCase("CrazyJ84") || aNamePlayer.equalsIgnoreCase("CrazyJ1984")) {
            if (aNameEntity.equalsIgnoreCase("Bear989jr")) return new ChatComponentText(
                "<" + EnumChatFormatting.LIGHT_PURPLE
                    ...
                    + "Junior"
                    + EnumChatFormatting.WHITE);
            if (aNameEntity.equalsIgnoreCase("Bear989Sr")) return new ChatComponentText(
                "<" + EnumChatFormatting.LIGHT_PURPLE
                    + "Mrs. Crazy"
                    + EnumChatFormatting.WHITE
                    + "> Hush it!, "
        ...
        if (UT.Code.stringValid(aMessage)) {
            return new ChatComponentText(
                aMessage.replace("[KILLER]", EnumChatFormatting.GREEN + aNamePlayer + EnumChatFormatting.WHITE)
                    .replace("[VICTIM]", EnumChatFormatting.RED + aNameEntity + EnumChatFormatting.WHITE));
            ...
            return new EntityDamageSource(aPlayer instanceof EntityPlayer ? "player" : "mob", aPlayer)
                .func_151519_b((EntityLivingBase) aEntity);
        } else if (aEntity instanceof EntityDragonPart) {
            return new EntityDamageSource(aPlayer instanceof EntityPlayer ? "player" : "mob", aPlayer)
                .func_151519_b((EntityLivingBase) ((EntityDragonPart) aEntity).entityDragonObj);
        }
        return new ChatComponentText(
            EnumChatFormatting.GREEN + aNamePlayer
                + EnumChatFormatting.WHITE
                + " has killed "
                + EnumChatFormatting.RED
                + aNameEntity
