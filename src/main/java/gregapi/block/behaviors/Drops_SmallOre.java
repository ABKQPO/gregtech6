    ...
    public ArrayList<ItemStack> getDrops(PrefixBlock aBlock, World aWorld, int aX, int aY, int aZ, short aMetaData,
        ...
        if (aWorld.isRemote) {
            ...
            if (aMaterial == MT.STONES.Gneiss || aMaterial == MT.PetrifiedWood) {
                ...
                for (int i = 0, j = Math.max(
                    1,
                    aMaterial.mOreMultiplier * aMaterial.mOreProcessingMultiplier
                        + (aFortune > 0
                            ? (tRandom.nextInt(
                                (1 + aFortune) * aMaterial.mOreMultiplier * aMaterial.mOreProcessingMultiplier))
                            : 0) / 2
                        + tRandom.nextInt(2)); i < j; i++) {
                ...
                if (tStack != null && tRandom.nextInt(aSilkTouch ? 5000 : 10000) <= aFortune) {
                    ...
                    if (tStack != null) {
                        for (int i = 0, j = (aSilkTouch ? 10 : 5); i < j; i++) tSelector.add(tStack);
                        tStack = OP.crushed.mat(aMaterial, 1);
                        if (tStack != null)
                            for (int i = 0, j = (aSilkTouch ? 5 : 10); i < j; i++) tSelector.add(tStack);
                    } else {
                        tStack = OP.crushed.mat(aMaterial, 1);
                    ...
                    if (tSelector.size() > 0) {
                        for (int i = 0, j = Math.max(
                            1,
                            aMaterial.mOreMultiplier * aMaterial.mOreProcessingMultiplier + (aFortune > 0
                                ? (tRandom.nextInt(
                                    (1 + aFortune) * aMaterial.mOreMultiplier * aMaterial.mOreProcessingMultiplier))
                                : 0) / 2); i < j; i++) {
            ...
            if (mSecondaryDrop != null && tRandom.nextInt(3 + aFortune) > 1)
                rList.add(ST.update(OP.dust.mat(mSecondaryDrop.mTargetCrushing.mMaterial, 1)));
        }
        return rList;
    }
}
