...
public class BlockBaseFluid extends BlockFluidFinite implements IBlock, IItemGT, IBlockOnHeadInside {
    ...
    public BlockBaseFluid(String aNameInternal, Fluid aFluid, int aFlammability) {
        this(
            ...
            aFluid,
            aFlammability,
            aFluid.isGaseous() ? MaterialGas.instance : aFluid.getTemperature() > 500 ? Material.lava : Material.water);
    }
    ...
    public BlockBaseFluid(String aNameInternal, Fluid aFluid, int aAmountPerQuanta, int aFlammability,
        Material aMaterial) {
        ...
        // Speaking of retarded, only allowing one type of Block per Fluid is retarded too! So I guess I gotta override
        // all pre-existing Fluids with my Version to make sure shit works.
    ...
    public FluidStack drain(World aWorld, int aX, int aY, int aZ, boolean aDoDrain) {
        // Forge royally fucked up again. You check for MetaData FIRST and do the set Block to Air SECOND, like I
        // demonstrate here!!!
    ...
    public void onNeighborBlockChange(World aWorld, int aX, int aY, int aZ, Block aUselessBlock) {
        ...
        for (byte tSide : ALL_SIDES_VALID) {
            ...
            if (tBlock.getMaterial() == getMaterial() && WD.liquid_borken(tBlock)) {
                ...
                if (WD.meta(aWorld, aX, aY, aZ, tSide, F) != 0
                    && WD.set(aWorld, aX + OFFX[tSide], aY + OFFY[tSide], aZ + OFFZ[tSide], NB, 0, 2)) {
    ...
    public void updateFluidBlocks(World aWorld, int aX, int aY, int aZ, boolean aAll) {
        for (int j = mDensityDir > 0 ? -1 : 0; j < (mDensityDir > 0 ? 1 : 2); j++)
            if (UT.Code.inside(0, aWorld.getHeight(), aY + j))
                for (int i = -4; i <= 4; i++) for (int k = -4; k <= 4; k++) if (i != 0 || j != 0 || k != 0) {
                    if (aWorld.getBlock(aX + i, aY + j, aZ + k) == this
                        && (aAll || aWorld.getBlockMetadata(aX + i, aY + j, aZ + k) > (j == 0 ? Math.abs(i) : 0))) {
    ...
    public void updateTick(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (mFlammability > 0) for (byte tSide : ALL_SIDES_VALID) {
            ...
            if (tBlock != this && (tBlock.getMaterial() == Material.lava || tBlock.getMaterial() == Material.fire)) {
                ...
                WD.burn(
                    aWorld,
                    aX - 4 + aRandom.nextInt(9),
                    aY - 4 + aRandom.nextInt(9),
                    aZ - 4 + aRandom.nextInt(9),
                    F,
                    F);
                WD.burn(
                    aWorld,
        ...
        if (aY <= 0 || aY + 1 >= aWorld.getHeight()) {
            if (WD.set(aWorld, aX, aY, aZ, NB, 0, FLUID_UPDATE_FLAGS | 1))
                GarbageGT.trash(FL.mul(mQuanta, tRemainingQuanta));
            return;
        }
        ...
        if (aWorld.blockExists(aX, aY, aZ - 1) && displaceIfPossible(aWorld, aX, aY, aZ - 1))
            WD.setIfDiff(aWorld, aX, aY, aZ - 1, NB, 0, FLUID_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX, aY, aZ + 1) && displaceIfPossible(aWorld, aX, aY, aZ + 1))
            WD.setIfDiff(aWorld, aX, aY, aZ + 1, NB, 0, FLUID_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX - 1, aY, aZ) && displaceIfPossible(aWorld, aX - 1, aY, aZ))
            WD.setIfDiff(aWorld, aX - 1, aY, aZ, NB, 0, FLUID_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX + 1, aY, aZ) && displaceIfPossible(aWorld, aX + 1, aY, aZ))
            WD.setIfDiff(aWorld, aX + 1, aY, aZ, NB, 0, FLUID_UPDATE_FLAGS | 1);
        ...
        if (north >= 0) {
            ...
            tCount--;
            if (tNew != north) if (tNew > 0) {
                if (set(aWorld, aX, aY, aZ - 1, tNew - 1, F))
                    aWorld.scheduleBlockUpdate(aX, aY, aZ - 1, this, tickRate);
            } else WD.setIfDiff(aWorld, aX, aY, aZ - 1, NB, 0, FLUID_UPDATE_FLAGS | 1);
        }
        if (south >= 0) {
            int tNew = tSpread;
            if (tRemainder == tCount || tRemainder > 1 && aRandom.nextInt(tCount - tRemainder) != 0) {
        ...
        if (west >= 0) {
            ...
            tCount--;
            if (tNew != west) if (tNew > 0) {
                if (set(aWorld, aX - 1, aY, aZ, tNew - 1, F))
                    aWorld.scheduleBlockUpdate(aX - 1, aY, aZ, this, tickRate);
            } else WD.setIfDiff(aWorld, aX - 1, aY, aZ, NB, 0, FLUID_UPDATE_FLAGS | 1);
        }
        if (east >= 0) {
            int tNew = tSpread;
            if (tRemainder == tCount || tRemainder > 1 && aRandom.nextInt(tCount - tRemainder) != 0) {
    ...
    public int tryToFlowVerticallyInto(World aWorld, int aX, int aY, int aZ, int aAmount) {
        ...
        if (tBlock instanceof BlockFluidBase) {
            if (mDensityDir > 0 ? getDensity(aWorld, aX, tY, aZ) > density : getDensity(aWorld, aX, tY, aZ) < density) {
                ...
                // Even worse I noticed that the Block Update caused by the second setBlock will schedule the update for
                // the Block ANYWAYS!!!
    ...
    public boolean shouldSideBeRendered(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        ...
        if (tTileEntity instanceof ITileEntitySurface)
            return !((ITileEntitySurface) tTileEntity).isSurfaceOpaque(OPOS[aSide]);
        return T;
    }
    ...
    public final String getUnlocalizedName() {
        return mFluid.getUnlocalizedName();
    }

    @Override
    public String getLocalizedName() {
        return LH.get(mFluid.getUnlocalizedName());
    }
    ...
    public IIcon getIcon(int aSide, int aMeta) {
        return SIDES_VERTICAL[aSide] ? mFluid.getStillIcon() : mFluid.getFlowingIcon();
    }

    @Override
    public int getRenderColor(int aMeta) {
        return mFluid.getColor();
    }
    ...
    public boolean canDisplace(IBlockAccess aWorld, int aX, int aY, int aZ) {
        return !aWorld.getBlock(aX, aY, aZ)
            .getMaterial()
            .isLiquid() && super.canDisplace(aWorld, aX, aY, aZ);
    }
    ...
    public boolean displaceIfPossible(World aWorld, int aX, int aY, int aZ) {
        return !aWorld.getBlock(aX, aY, aZ)
            .getMaterial()
            .isLiquid() && super.displaceIfPossible(aWorld, aX, aY, aZ);
    }
    ...
    public boolean getBlocksMovement(IBlockAccess aWorld, int aX, int aY, int aZ) {
        return mActLikeWeb || !mEffectsBathing.isEmpty() || !mEffectsBreathing.isEmpty();
    }
    ...
    public boolean set(World aWorld, int aX, int aY, int aZ, int aMeta, boolean aBlockUpdate) {
        ...
        return aMeta == tMeta || WD.set(
            ...
            this,
            aMeta,
            aMeta >= 7 && tMeta >= 7 ? aBlockUpdate ? 5 : 4 : aBlockUpdate ? 3 : 2);
    }
    ...
    public void onEntityCollidedWithBlock(World aWorld, int aX, int aY, int aZ, Entity aEntity) {
        ...
        if (!aWorld.isRemote && !mEffectsBathing.isEmpty()
            && aEntity instanceof EntityLivingBase
            && !UT.Entities.isWearingFullChemHazmat((EntityLivingBase) aEntity)) {
            for (int[] tEffects : mEffectsBathing)
                UT.Entities.applyPotion(aEntity, tEffects[0], tEffects[1], tEffects[2], F);
        }
    }
    ...
    public void onHeadInside(EntityLivingBase aEntity, World aWorld, int aX, int aY, int aZ) {
        if (!aWorld.isRemote && !mEffectsBreathing.isEmpty() && !UT.Entities.isImmuneToBreathingGases(aEntity)) {
            for (int[] tEffects : mEffectsBreathing)
                UT.Entities.applyPotion(aEntity, tEffects[0], tEffects[1], tEffects[2], F);
