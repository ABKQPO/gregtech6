...
public class ToolCompat {
    ...
    public static long onToolClick(Block aBlock, String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        try {
            ...
            if (aTool.equals(TOOL_hoe)
                && (aEntityPlayer == null || aEntityPlayer.canPlayerEdit(aX, aY, aZ, aSide, aStack))) {
                if (!MinecraftForge.EVENT_BUS.post(new UseHoeEvent(aEntityPlayer, aStack, aWorld, aX, aY, aZ))) {
                    if (SIDES_TOP_HORIZONTAL[aSide] && !WD.hasCollide(aWorld, aX, aY + 1, aZ)
                        && (aBlock == Blocks.grass || aBlock == Blocks.dirt
                            || aBlock == BlocksGT.Grass
                            || IL.EtFu_Path.equal(aBlock)
                            || IL.BoP_Grass_Origin.equal(aBlock)
                            || IL.BoP_Grass_Long.equal(aBlock))) {
                        aWorld.playSoundEffect(
                            aX + 0.5F,
                            aY + 0.5F,
                            aZ + 0.5F,
                            Blocks.farmland.stepSound.getStepResourcePath(),
                            (Blocks.farmland.stepSound.getVolume() + 1.0F) * 0.5F,
            ...
            boolean aCanCollect = (ST.item(aStack) instanceof MultiItemTool
                && ((MultiItemTool) ST.item_(aStack)).canCollectDropsDirectly(aStack));
            ...
            if (aTool.equals(TOOL_axe) || aTool.equals(TOOL_saw) || aTool.equals(TOOL_knife)) {
                ...
                if (!rReturn && BlocksGT.BeamB != null) {
                    if (IL.HaC_Log_Cinnamon.equal(aBlock)) {
                        ...
                        if (rReturn)
                            tBark = IL.HaC_Cinnamon.get(2, IL.Food_Cinnamon.get(2, OM.dust(MT.Cinnamon, U * 2)));
                    }
                }
                if (!rReturn && MD.NeLi.mLoaded && (aMeta & 1) == 0) {
                    if (IL.NeLi_Stem_Crimson.equal(aBlock) || IL.NeLi_Stem_FoxFire.equal(aBlock)
                        || IL.NeLi_Hyphae_Crimson.equal(aBlock)) {
                ...
                if (!rReturn && BlocksGT.Beam1 != null) {
                    if (aBlock == Blocks.log || IL.EtFu_Bark_Oak.equal(aBlock)) {
                        ...
                        rReturn = aWorld
                            .setBlock(aX, aY, aZ, BlocksGT.Beam1, (aMeta & 12) | ((aMeta & 2) == 0 ? 1 : 2), 3);
                ...
                if (!rReturn && BlocksGT.Beam3 != null) {
                    if (IL.TC_Greatwood_Log.equal(aBlock)) {
                        if ((aMeta & 3) < 2) rReturn = aWorld.setBlock(aX, aY, aZ, BlocksGT.Beam3, aMeta, 3);
                ...
                if (!rReturn && BlocksGT.Beam2 != null) {
                    if (aBlock == Blocks.log2 || IL.EtFu_Bark_Acacia.equal(aBlock)) {
                        ...
                        rReturn = aWorld
                            .setBlock(aX, aY, aZ, BlocksGT.Beam2, (aMeta & 12) | ((aMeta & 2) == 0 ? 0 : 1), 3);
            ...
            if (aTool.equals(TOOL_sense) || aTool.equals(TOOL_scythe)) {
                if (IC_CROPTILE && aTileEntity instanceof ICropTile) {
                    ...
                    for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++)
                        if ((aTileEntity = WD.te(aWorld, aX + i, aY + j, aZ + k, T)) instanceof ICropTile
                            && ((ICropTile) aTileEntity).harvest(T)) {
                                UT.Sounds.send(
                                    SFX.MC_COLLECT,
                                    0.2F,
                    ...
                    if (aCanCollect) for (ItemStack tDrop : WD.suckAll(aWorld, aX - 1.5, aY - 0.5, aZ - 1.5, 4, 2, 4))
                        ST.give(aEntityPlayer, tDrop, aWorld, aX, aY, aZ);
                    return tDamage;
                }
                if (aBlock instanceof IGrowable) {
                    int tDamage = 0;
                    for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++)
                        for (int k = -1; k < 2; k++) if (aWorld.getBlockMetadata(aX + i, aY + j, aZ + k) == 7) {
                            byte tMeta = WD.meta(aWorld, aX + i, aY + j, aZ + k);
                            Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k);
                            if (tBlock.getClass() == aBlock.getClass()
                                && !((IGrowable) tBlock).func_149851_a(aWorld, aX + i, aY + j, aZ + k, F)) {
                                tBlock.onBlockActivated(
                                    aWorld,
                                    aX + i,
                                    aY + j,
                                    aZ + k,
                            ...
                            if (tMeta != WD.meta(aWorld, aX + i, aY + j, aZ + k)
            ...
            if (aTool.equals(TOOL_igniter) && ST.item(aStack) != Items.flint_and_steel) {
                ...
                if (!IL.TF_Lamp_of_Cinders.equal(aStack, T, T)) {
                    if (aEntityPlayer == null || aEntityPlayer
                        .canPlayerEdit(aX + OFFX[aSide], aY + OFFY[aSide], aZ + OFFZ[aSide], aSide, aStack)) {
                        if (aWorld.isAirBlock(aX + OFFX[aSide], aY + OFFY[aSide], aZ + OFFZ[aSide])) {
                            if (WD.oxygen(aWorld, aX, aY, aZ))
                                aWorld.setBlock(aX + OFFX[aSide], aY + OFFY[aSide], aZ + OFFZ[aSide], Blocks.fire);
                            return 10000;
                        }
                    }
                }
            ...
            if (aTool.equals(TOOL_chisel) && !aSneaking) {
                ...
                if (tChiseledBlock != null) {
                    Recipe tRecipe = RM.Chisel
                        .findRecipe(null, null, T, Integer.MAX_VALUE, null, ZL_FS, tChiseledBlock);
                    if (tRecipe != null && tRecipe.blockINblockOUT()
                        && ST.equal(tRecipe.mInputs[0], tChiseledBlock)
            ...
            if (aTool.equals(TOOL_rotator)) {
                ...
                if (aBlock instanceof BlockPumpkin || aBlock instanceof BlockFurnace
                    || aBlock instanceof BlockChest
                    || aBlock instanceof BlockEnderChest) {
                    if (aWorld.setBlockMetadataWithNotify(aX, aY, aZ, ((aMeta - 1) % 4) + 2, 3)) return 2500;
                }
                if (aBlock instanceof BlockHopper) {
                    if (aWorld.setBlockMetadataWithNotify(aX, aY, aZ, (aMeta + 1) % 6 == 1 ? (aMeta + 1) % 6 : 2, 3))
                        return 2500;
                }
            ...
            if (aTool.equals(TOOL_screwdriver)) {
                if (aBlock instanceof BlockRedstoneDiode) {
                    if (aWorld.setBlockMetadataWithNotify(aX, aY, aZ, (aMeta / 4) * 4 + (((aMeta % 4) + 1) % 4), 3))
                        return 10000;
                }
            }
            if (aTool.equals(TOOL_crowbar)) {
                if (aBlock instanceof BlockRailBase
                    && (!MD.RC.mLoaded || !(MD.MC.owns(aBlock) || MD.RC.owns(aBlock)))) {
                    aWorld.isRemote = T;
                    // Why the fuck are the two Coordinate Parameters in isFlexibleRail switched? And then it is used
                    // like x y z instead of using the broken namings.
                    boolean tResult = aWorld.setBlock(
                        aX,
                        aY,
                        aZ,
                        aBlock,
                        ((BlockRailBase) aBlock).isFlexibleRail(aWorld, aX, aY, aZ) ? (aMeta + 1) % 10
                            : ((aMeta / 8) * 8) + (((aMeta % 8) + 1) % 6),
                        0);
                    aWorld.isRemote = F;
            ...
            if (aTool.equals(TOOL_wrench) || aTool.equals(TOOL_monkeywrench)) {
                if (GC_BLOCKADVANCED && aBlock instanceof BlockAdvanced) {
                    return (aSneaking
                        ? ((BlockAdvanced) aBlock)
                            .onSneakUseWrench(aWorld, aX, aY, aZ, aEntityPlayer, aSide, aHitX, aHitY, aHitZ)
                        : ((BlockAdvanced) aBlock)
                            .onUseWrench(aWorld, aX, aY, aZ, aEntityPlayer, aSide, aHitX, aHitY, aHitZ)) ? 2500 : 0;
                }
                ...
                if (aMeta == aTargetSide) {
                    if (aBlock instanceof BlockPumpkin || aBlock instanceof BlockPistonBase
                        || aBlock instanceof BlockDispenser
                        || aBlock instanceof BlockFurnace
                        || aBlock instanceof BlockChest
                        || aBlock instanceof BlockHopper
                        || aBlock instanceof BlockEnderChest) {
                        if (aWorld.setBlockToAir(aX, aY, aZ)) {
                    ...
                    if (aBlock instanceof BlockPumpkin || aBlock instanceof BlockFurnace
                        || aBlock instanceof BlockChest
                        || aBlock instanceof BlockEnderChest) {
                        if (SIDES_HORIZONTAL[aTargetSide]
                            && aWorld.setBlockMetadataWithNotify(aX, aY, aZ, aTargetSide, 3)) return 10000;
                    }
                    if (aBlock instanceof BlockHopper) {
                        if (SIDES_BOTTOM_HORIZONTAL[aTargetSide]
                            && aWorld.setBlockMetadataWithNotify(aX, aY, aZ, aTargetSide, 3)) return 10000;
                    }
                }
                if (aBlock instanceof BlockRailBase || aBlock instanceof BlockRedstoneDiode
                    || aBlock instanceof BlockPistonExtension
                    || aBlock instanceof BlockPistonBase) {
            ...
            FMLLog.severe(
                "Exception occured when ToolCompat was used at the Coordinates: [%d;%d;%d] at '%s' with TileEntity '%s' using the Tool '%s' %s",
                aX,
                aY,
                aZ,
    ...
    public static boolean prospectOre(Block aBlock, byte aMeta, List<String> aChatReturn, World aWorld, int aX, int aY,
        int aZ) {
        ...
        if (tAssotiation != null && tAssotiation.mPrefix.contains(TD.Prefix.ORE)) {
            if (aChatReturn != null) aChatReturn
                .add(LanguageHandler.getLocalName(tAssotiation.mPrefix, tAssotiation.mMaterial.mMaterial) + "!");
            return T;
        }
        return F;
    }

    public static boolean prospectStone(Block aBlock, byte aMeta, long aQuality, List<String> aChatReturn, World aWorld,
        byte aSide, int aX, int aY, int aZ) {
        Block tBlock;
        ...
        for (int i = 0, j = 1 + 2 * tQuality, k = tQuality * tQuality; i < k; i++) {
            ...
            if (tBlock != NB && tBlock != Blocks.obsidian && tBlock != BlocksGT.RockOres) {
                OreDictItemData tAssotiation = OM.anyassociation(
                    (tBlock instanceof IBlockRetrievable
                        ? ((IBlockRetrievable) tBlock).getItemStackFromBlock(aWorld, tX, tY, tZ, SIDE_INVALID)
                        : ST.make(tBlock, 1, aWorld.getBlockMetadata(tX, tY, tZ))));
                if (tAssotiation != null
                    && tAssotiation.mPrefix.containsAny(TD.Prefix.STANDARD_ORE, TD.Prefix.DENSE_ORE)) {
                    if (aChatReturn != null)
