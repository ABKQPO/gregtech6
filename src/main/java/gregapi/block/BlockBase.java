...
public abstract class BlockBase extends Block implements IBlockBase {
    ...
    public BlockBase(Class<? extends ItemBlock> aItemClass, String aNameInternal, Material aMaterial,
    ...
    public boolean isSideSolid(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aDirection) {
        return isSideSolid(WD.meta(aWorld, aX, aY, aZ), UT.Code.side(aDirection));
    }

    @Override
    public boolean shouldSideBeRendered(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return isOpaqueCube() ? !WD.visOpq(aWorld.getBlock(aX, aY, aZ))
    ...
    public boolean canCreatureSpawn(EnumCreatureType type, IBlockAccess aWorld, int aX, int aY, int aZ) {
        ...
        return canCreatureSpawn(aMeta) && isSideSolid(aMeta, SIDE_TOP);
    }

    @Override
    public boolean isFireSource(World aWorld, int aX, int aY, int aZ, ForgeDirection aSide) {
        return isFireSource(WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public boolean isFlammable(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aSide) {
        return isFlammable(WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public int getFlammability(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aSide) {
        return getFlammability(WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public int getFireSpreadSpeed(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aSide) {
        return getFireSpreadSpeed(WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public float getExplosionResistance(Entity aEntity, World aWorld, int aX, int aY, int aZ, double eX, double eY,
        double eZ) {
        return getExplosionResistance(WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public float getExplosionResistance(Entity aEntity) {
        return getExplosionResistance((byte) 0);
    }
    ...
    public final void onNeighborBlockChange(World aWorld, int aX, int aY, int aZ, Block aBlock) {
        if (useGravity(WD.meta(aWorld, aX, aY, aZ))) aWorld.scheduleBlockUpdate(aX, aY, aZ, this, 2);
        onNeighborBlockChange2(aWorld, aX, aY, aZ, aBlock);
    }

    @Override
    public final void onBlockAdded(World aWorld, int aX, int aY, int aZ) {
        if (useGravity(WD.meta(aWorld, aX, aY, aZ))) aWorld.scheduleBlockUpdate(aX, aY, aZ, this, 2);
        onBlockAdded2(aWorld, aX, aY, aZ);
    }

    @Override
    public IIcon getIcon(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return getIcon(aSide, WD.meta(aWorld, aX, aY, aZ));
    }

    @Override
    public String name(byte aMeta) {
        return aMeta == W ? mNameInternal : mNameInternal + "." + aMeta;
    }
    ...
    public boolean isSealable(byte aMeta, byte aSide) {
        return isSideSolid(aMeta, aSide);
    }
    ...
    @Override
    public void addInformation(ItemStack aStack, byte aMeta, EntityPlayer aPlayer, List<String> aList,
        boolean aF3_H) {/**/}
    ...
    public boolean checkNoEntityCollision(World aWorld, int aX, int aY, int aZ, byte aMeta, Entity aExceptThisOne) {
        return aWorld
            .checkNoEntityCollision(AxisAlignedBB.getBoundingBox(aX, aY, aZ, aX + 1, aY + 1, aZ + 1), aExceptThisOne);
    }
    ...
    public boolean checkGravity(World aWorld, int aX, int aY, int aZ) {
        ...
        if (aY > 0 && useGravity(aMeta) && BlockFalling.func_149831_e(aWorld, aX, aY - 1, aZ)) {
            if (!BlockFalling.fallInstantly
                ...
                if (!aWorld.isRemote) aWorld
                    .spawnEntityInWorld(new EntityFallingBlock(aWorld, aX + 0.5, aY + 0.5, aZ + 0.5, this, aMeta));
            } else {
    ...
    public boolean onItemUse(ItemBlockBase aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (!aWorld.getBlock(aX, aY, aZ)
            .isReplaceable(aWorld, aX, aY, aZ)) return F;
        ...
        if (aItem.placeBlockAt(
            ...
            aHitY,
            aHitZ,
            onBlockPlaced(aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ, aMeta))) {
    ...
    public final int quantityDropped(Random aRandom) {
        return quantityDropped(0, 0, aRandom);
    }
}
