...
public abstract class BlockBaseLeaves extends BlockBaseTree implements IShearable, IOxygenReliantBlock {
    ...
    public BlockBaseLeaves(Class<? extends ItemBlock> aItemClass, String aNameInternal, Material aMaterial,
        SoundType aSoundType, long aMaxMeta, IIconContainer[] aIcons, Block aSaplings, Block[] aLogs,
        byte[] aLogMetas) {
    ...
    public Item getItemDropped(int aMeta, Random aRandom, int aFortune) {
        return Item.getItemFromBlock(mSaplings);
    }
    ...
    public IIcon getIcon(int aSide, int aMeta) {
        return mIcons[(aMeta & 7) | (Blocks.leaves.isOpaqueCube() ? 8 : 0)].getIcon(0);
    }

    @Override
    public ArrayList<ItemStack> onSheared(ItemStack aItem, IBlockAccess aWorld, int aX, int aY, int aZ, int aFortune) {
        return ST.arraylist(ST.make(this, 1, WD.meta(aWorld, aX, aY, aZ) & 7));
    }
    ...
    public void onOxygenRemoved(World aWorld, int aX, int aY, int aZ) {
        if (!aWorld.isRemote) {
            aWorld.scheduleBlockUpdate(aX, aY, aZ, this, 201 + RNGSUS.nextInt(100));
            return;
        }
    }
    ...
    public void updateTick2(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (aMeta < 8) return;
        int tRangeSide = getLeavesRangeSide(aMeta), tRangeYNeg = getLeavesRangeYNeg(aMeta),
            tRangeYPos = getLeavesRangeYPos(aMeta);
        ...
        if (!(MD.TFC.mLoaded || MD.TFCP.mLoaded) || aRandom.nextInt(4) == 0)
            dropBlockAsItem(aWorld, aX, aY, aZ, aMeta, 0);
        aWorld.setBlockToAir(aX, aY, aZ);
    }
    ...
    public int colorMultiplier(IBlockAccess aWorld, int aX, int aY, int aZ) {
        ...
        for (int k1 = -1; k1 <= 1; ++k1) for (int l1 = -1; l1 <= 1; ++l1) {
            int i2 = aWorld.getBiomeGenForCoords(aX + l1, aZ + k1)
                .getBiomeFoliageColor(aX + l1, aY, aZ + k1);
            l += (i2 & 16711680) >> 16;
            i1 += (i2 & 65280) >> 8;
            j1 += i2 & 255;
        }
