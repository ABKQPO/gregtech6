...
public abstract class BlockBaseSapling extends BlockBaseMeta implements IPlantable, IGrowable, IOxygenReliantBlock {
    ...
    public BlockBaseSapling(Class<? extends ItemBlock> aItemClass, String aNameInternal, Material aMaterial,
    ...
    public int getDamageValue(World aWorld, int aX, int aY, int aZ) {
        return WD.meta(aWorld, aX, aY, aZ) & 7;
    }

    @Override
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        return Blocks.sapling.getBlockHardness(aWorld, aX, aY, aZ);
    }

    @Override
    public float getExplosionResistance(byte aMeta) {
        return Blocks.sapling.getExplosionResistance(null);
    }
    ...
    public IIcon getIcon(int aSide, int aMeta) {
        return mIcons[aMeta & 15].getIcon(0);
    }

    @Override
    public boolean canBlockStay(World aWorld, int aX, int aY, int aZ) {
        return aWorld.getBlock(aX, aY - 1, aZ)
            .canSustainPlant(aWorld, aX, aY - 1, aZ, ForgeDirection.UP, (IPlantable) Blocks.sapling);
    }
    ...
    public void updateTick2(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (aWorld.isRemote || checkAndDropBlock(aWorld, aX, aY, aZ)
            || aWorld.getBlockLightValue(aX, aY + 1, aZ) < 9
            || aRandom.nextInt(7) != 0) return;
    ...
    public int getMaxHeight(World aWorld, int aX, int aY, int aZ, int aMaxTreeHeight) {
        ...
        while (rMaxHeight++ < aMaxTreeHeight)
            if (aY + rMaxHeight >= aWorld.getHeight() || !canPlaceTree(aWorld, aX, aY + rMaxHeight, aZ))
                return rMaxHeight - 1;
        return rMaxHeight;
    }
    ...
    public boolean canPlaceTree(World aWorld, int aX, int aY, int aZ) {
        ...
        return tBlock == this || tBlock instanceof BlockTallGrass
            || tBlock instanceof BlockSnow
            || tBlock instanceof BlockLeavesBase
            || tBlock.canBeReplacedByLeaves(aWorld, aX, aY, aZ);
    }
