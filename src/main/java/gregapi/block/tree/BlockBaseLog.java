...
public abstract class BlockBaseLog extends BlockBaseTree {
    ...
    public BlockBaseLog(Class<? extends ItemBlock> aItemClass, String aNameInternal, Material aMaterial,
        ...
        for (byte tMeta = 0; tMeta < maxMeta(); tMeta++) {
            ...
            CR.shapeless(
                ST.make(this, 4, tMeta | 12),
                CR.DEF_NCC,
                new Object[] { OP.dust.dat(MT.Bark), ST.make(this, 1, tMeta), ST.make(this, 1, tMeta),
                    ST.make(this, 1, tMeta), ST.make(this, 1, tMeta) });
            CR.shapeless(
                ST.make(this, 3, tMeta | 12),
                CR.DEF_NCC,
                new Object[] { OP.dust.dat(MT.Bark), ST.make(this, 1, tMeta), ST.make(this, 1, tMeta),
                    ST.make(this, 1, tMeta) });
            CR.shapeless(
                ST.make(this, 2, tMeta | 12),
                CR.DEF_NCC,
                new Object[] { OP.dust.dat(MT.Bark), ST.make(this, 1, tMeta), ST.make(this, 1, tMeta) });
            CR.shapeless(
                ST.make(this, 1, tMeta | 12),
                CR.DEF_NCC,
                new Object[] { OP.dust.dat(MT.Bark), ST.make(this, 1, tMeta) });
            CR.shapeless(
                ST.make(this, 1, tMeta | 12),
                CR.DEF_NCC,
                new Object[] { OP.dustSmall.dat(MT.Bark), ST.make(this, 1, tMeta) });
        }
    }
    ...
    public int damageDropped(int aMeta) {
        return aMeta & PILLAR_DATA;
    }

    @Override
    public int getDamageValue(World aWorld, int aX, int aY, int aZ) {
        return WD.meta(aWorld, aX, aY, aZ) & PILLAR_DATA;
    }
    ...
    public int onBlockPlaced(World aWorld, int aX, int aY, int aZ, int aSide, float aHitX, float aHitY, float aHitZ,
        int aMeta) {
        return PILLAR_DATA_SIDE[aMeta][aSide];
    }

    @Override
    public IIcon getIcon(int aSide, int aMeta) {
        return mIcons[2 * (aMeta & PILLAR_DATA) + (PILLAR_TO_AXIS[aMeta][aSide] ? 0 : 1)].getIcon(0);
    }
    ...
    public void getSubBlocks(Item aItem, CreativeTabs aTab, @SuppressWarnings("rawtypes") List aList) {
        for (int i = 0; i < maxMeta(); i++) {
            aList.add(ST.make(aItem, 1, i));
            aList.add(ST.make(aItem, 1, i | 12));
        }
    }
}
