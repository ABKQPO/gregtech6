...
public class ItemBlockBase extends ItemBlock implements IBlock, IItemGT {
    ...
    public void addInformation(ItemStack aStack, EntityPlayer aPlayer, @SuppressWarnings("rawtypes") List aList,
        ...
        if (field_150939_a.getCollisionBoundingBoxFromPool(aPlayer.worldObj, 0, 0, 0) != null) {
            ...
            if (mPlaceable.canCreatureSpawn(aMeta)) {
                if (ITexture.Util.OPTIFINE_LOADED && aMeta != 0 && !mPlaceable.canCreatureSpawn((byte) 0)) {
                    aList.add(
                        LH.Chat.BLINKING_RED + LH.get(
                            Minecraft.getMinecraft()
                                .isSingleplayer() ? LH.TOOLTIP_SPAWNPROOF_SP_BUG : LH.TOOLTIP_SPAWNPROOF_MP_BUG));
                ...
                if (ITexture.Util.OPTIFINE_LOADED && aMeta != 0 && mPlaceable.canCreatureSpawn((byte) 0)) {
                    aList.add(
                        LH.Chat.BLINKING_RED + LH.get(
                            Minecraft.getMinecraft()
                                .isSingleplayer() ? LH.TOOLTIP_SPAWNPROOF_SP_BROKEN : LH.TOOLTIP_SPAWNPROOF_MP_BROKEN));
                    aList.add(LH.Chat.BLINKING_RED + LH.get(LH.TOOLTIP_SPAWNPROOF_OPTIFINE));
                } else {
        ...
        aList.add(
            LH.getToolTipHarvest(
                field_150939_a.getMaterial(),
                field_150939_a.getHarvestTool(aMeta),
                field_150939_a.getHarvestLevel(aMeta)));
    ...
    public boolean onItemUseFirst(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ,
        int aSide, float aHitX, float aHitY, float aHitZ) {
        return mPlaceable.onItemUseFirst(this, aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ);
    }
    ...
    public boolean onItemUse(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, int aSide,
        ...
        return mPlaceable.onItemUse(this, aStack, aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ);
    }

    @Override
    public IIcon getIconFromDamage(int aMeta) {
        return field_150939_a.getIcon(SIDE_TOP, aMeta);
    }
    ...
    public String getUnlocalizedName(ItemStack aStack) {
        return mPlaceable.name(UT.Code.bind4(getDamage(aStack)));
    }
    ...
    public boolean placeBlockAt(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, int aSide,
        float aHitX, float aHitY, float aHitZ, int aMetaData) {
        return aWorld.setBlock(aX, aY, aZ, field_150939_a, aMetaData, 3);
    }

    @Override
    public int getItemStackLimit(ItemStack aStack) {
        return mPlaceable.getItemStackLimit(aStack);
    }
    ...
    public ItemStack onItemRightClick(ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        return mPlaceable.onItemRightClick(aStack, aWorld, aPlayer);
    }
}
