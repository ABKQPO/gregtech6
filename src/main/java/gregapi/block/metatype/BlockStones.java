...
public class BlockStones extends BlockMetaType
    implements IOreDictListenerEvent, IBlockToolable, IBlockOnWalkOver, IGrowable, Runnable {

    public static final boolean[] MOSSY = { F, F, T, F, F, T, F, F, F, F, F, F, F, F, F, F },
    ...
    public static final byte STONE = 0, SMOTH = 7, COBBL = 1, MCOBL = 2, BRICK = 3, CRACK = 4, MBRIK = 5, SBRIK = 12,
        QBRIK = 15, CHISL = 6, WINDA = 13, WINDB = 14, RNFBR = 8, RSTBR = 9, TILES = 10, STILE = 11;

    public static final byte[]
    // NO_MAPPINGS = {STONE, COBBL, MCOBL, BRICK, CRACK, MBRIK, CHISL, SMOTH, RNFBR, RSTBR, TILES, STILE, SBRIK, WINDA,
    // WINDB, QBRIK}
    CHISEL_MAPPINGS = { SMOTH, COBBL, MCOBL, CRACK, COBBL, MCOBL, CHISL, CHISL, RNFBR, RSTBR, STILE, STILE, STILE,
        WINDB, WINDA, STILE },
        FILE_MAPPINGS = { SMOTH, COBBL, MCOBL, SBRIK, CRACK, MBRIK, CHISL, SMOTH, RNFBR, RSTBR, STILE, STILE, STILE,
            WINDB, WINDA, STILE },
        HAMMER_MAPPINGS = { STONE, COBBL, MCOBL, CRACK, COBBL, CRACK, CRACK, COBBL, RNFBR, RSTBR, CRACK, CRACK, CRACK,
            CRACK, CRACK, CRACK } // Has to Map Stone to itself for prospecting Compatibility
        , MOSS_MAPPINGS = { STONE, MCOBL, MCOBL, MBRIK, MBRIK, MBRIK, CHISL, SMOTH, RNFBR, RSTBR, TILES, STILE, SBRIK,
            WINDA, WINDB, QBRIK };

    public final OreDictMaterial mMaterial;
    @SuppressWarnings("rawtypes")
    public final ItemStackSet[] mEqualBlocks = { ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(),
        ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(), ST.hashset(),
        ST.hashset(), ST.hashset(), ST.hashset() };

    public BlockStones(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aVanillaSoundType,
        String aName, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier,
        float aHardnessMultiplier, int aHarvestLevel, IIconContainer[] aIcons) {
        super(
            aItemClass,
            aVanillaMaterial == null ? Material.rock : aVanillaMaterial,
            aVanillaSoundType == null ? soundTypeStone : aVanillaSoundType,
            aName,
            aDefaultLocalised,
            aMaterial,
            ...
            16,
            aIcons == null
                ? new IIconContainer[] { new Textures.BlockIcons.CustomIcon("stones/" + aName + "/STONE"),
        ...
        if (COMPAT_FR != null) for (int i = 0; i < maxMeta(); i++)
            COMPAT_FR.addToBackpacks(SPAWNABLE[i] ? "digger" : "builder", ST.make(this, 1, i));
    }
    ...
    protected BlockStones(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aVanillaSoundType,
        String aName, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier,
        float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons, byte aSlabType,
        BlockMetaType aBlock) {
        super(
            aItemClass,
            aVanillaMaterial == null ? Material.rock : aVanillaMaterial,
            aVanillaSoundType == null ? soundTypeStone : aVanillaSoundType,
    ...
    public void run() {
        if ((!MD.NePl.mLoaded && !MD.NeLi.mLoaded)
            || (mMaterial != MT.STONES.Basalt && mMaterial != MT.STONES.Blackstone)) {
        ...
        CR.shaped(ST.make(Blocks.stone_stairs, 1, 0), CR.DEF_MIR, " X", "XX", 'X', OP.rockGt.dat(mMaterial)); // TODO
                                                                                                              // Stairs
        ...
        for (int i = 0; i < maxMeta(); i++) if (JUSTSTONE[i]) {
            RM.sawing(
                ...
                F,
                50,
                ST.make(mSlabs[0], 1, i),
                OP.plate.mat(mMaterial, 4),
                OP.dustSmall.mat(mMaterial, 2));
        }
        RM.sawing(
            16,
            16,
            F,
            50,
            ST.make(mSlabs[0], 1, RSTBR),
            OP.plate.mat(mMaterial, 4),
            OP.dustSmall.mat(mMaterial, 2),
            OP.dustSmall.mat(MT.Redstone, 2));
        ...
        for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[STONE]) {
            ...
            if (IL.TF_Pick_Giant.exists()) RM.Boxinator.addRecipe2(
                T,
                128,
                128,
                ST.amount(64, tStack.toStack()),
                IL.TF_Pick_Giant.getWildcard(0),
                IL.TF_Giant_Cobble.get(1));
            RM.Extruder.addRecipe2(
                ...
                16,
                32,
                ST.amount(1, tStack.toStack()),
                IL.Shape_Extruder_Plate.get(0),
                OP.plate.mat(mMaterial, 9));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.plateCurved.mat(mMaterial, 9));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
            ...
            RM.Extruder.addRecipe2(
                ...
                OP.toolHeadRawShovel.mat(mMaterial, 9));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.toolHeadRawSword.mat(mMaterial, 4));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.toolHeadRawHoe.mat(mMaterial, 4));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.toolHeadRawPickaxe.mat(mMaterial, 3));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.toolHeadRawAxe.mat(mMaterial, 3));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
            ...
            RM.Extruder.addRecipe2(
                ...
                OP.gearGtSmall.mat(mMaterial, 9));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
                ...
                OP.toolHeadHammer.mat(mMaterial, 1));
            RM.Extruder.addRecipe2(
                F,
                F,
                F,
        ...
        for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[COBBL]) {
            ...
            CR.shaped(ST.make(Blocks.cobblestone_wall, 6, 0), CR.DEF_MIR, "XXX", "XXX", 'X', tStack.toStack()); // TODO
                                                                                                                // Walls
            if (IL.TF_Pick_Giant.exists()) RM.Boxinator.addRecipe2(
                T,
                128,
        ...
        for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[RNFBR]) {
            ...
            RM.Crusher.addRecipe1(
                ...
                64 + mHarvestLevel * 64,
                tStack.toStack(),
                ST.make(this, 1, COBBL),
                OM.dust(MT.Fe, OP.stick.mAmount));
            RM.Shredder.addRecipe1(
                T,
                16,
                64 + mHarvestLevel * 64,
                tStack.toStack(),
                OP.blockDust.mat(mMaterial, 1),
                OM.dust(MT.Fe, OP.stick.mAmount));
        }
    ...
    public void onOreRegistration(OreDictRegistrationContainer aEvent) {
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_White])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 64, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, CHISL));
            }
        }
        ...
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Red])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, TILES));
            }
        }
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Green])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, STILE));
            }
        }
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Blue])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, WINDA));
            }
        }
        ...
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Cyan])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, BRICK));
            }
        }
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Magenta])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, SBRIK));
            }
        }
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Yellow])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, WINDB));
            }
        }
        ...
        if (aEvent.mOreDictName.equals(DYE_OREDICTS_LENS[DYE_INDEX_Pink])) {
            for (ItemStackContainer tStack : (ItemStackSet<ItemStackContainer>) mEqualBlocks[SMOTH]) {
                RM.LaserEngraver
                    .addRecipe2(T, 16, 16, tStack.toStack(), ST.amount(0, aEvent.mStack), ST.make(this, 1, QBRIK));
            }
        }
    }
    ...
    public int getItemStackLimit(ItemStack aStack) {
        switch (ST.meta_(aStack)) {
            case 0:
                return UT.Code.bindStack(OP.stone.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 1:
                return UT.Code.bindStack(OP.stoneCobble.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 2:
                return UT.Code.bindStack(OP.stoneMossy.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 4:
                return UT.Code.bindStack(OP.stoneCracked.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 5:
                return UT.Code.bindStack(OP.stoneMossyBricks.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 6:
                return UT.Code.bindStack(OP.stoneChiseled.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            case 7:
                return UT.Code.bindStack(OP.stonePolished.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
            default:
                return UT.Code.bindStack(OP.stoneBricks.mDefaultStackSize * (mBlock.mBlock == mBlock ? 1 : 2));
        }
    }
    ...
    public long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        byte aMeta = WD.meta(aWorld, aX, aY, aZ);
        if (aTool.equals(TOOL_prospector)) return aMeta == STONE
            && ToolCompat.prospectStone(this, aMeta, aQuality, aChatReturn, aWorld, aSide, aX, aY, aZ) ? 10000 : 0;
    ...
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        switch (WD.meta(aWorld, aX, aY, aZ)) {
            case RNFBR:
                return Blocks.stone.getBlockHardness(aWorld, aX, aY, aZ) * mHardnessMultiplier * 2;
            default:
                return Blocks.stone.getBlockHardness(aWorld, aX, aY, aZ) * mHardnessMultiplier;
        }
    }
    ...
    public float getExplosionResistance(byte aMeta) {
        switch (aMeta) {
            case RNFBR:
                return Blocks.stone.getExplosionResistance(null) * mResistanceMultiplier * 2;
            default:
                return Blocks.stone.getExplosionResistance(null) * mResistanceMultiplier;
        }
    }
    ...
    public void updateTick2(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        if (!aWorld.isRemote && WD.burning(aWorld, aX, aY, aZ)) switch (WD.meta(aWorld, aX, aY, aZ)) {
            case MCOBL:
                aWorld.setBlock(aX, aY, aZ, this, COBBL, 3);
                break;
            case MBRIK:
                aWorld.setBlock(aX, aY, aZ, this, BRICK, 3);
                break;
        }
    }

    @Override
    public boolean canSustainPlant(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aSide,
        IPlantable aPlant) {
        return PLANTABLE[WD.meta(aWorld, aX, aY, aZ)]
            && aPlant.getPlantType(aWorld, aX + aSide.offsetX, aY + aSide.offsetY, aZ + aSide.offsetZ)
                == EnumPlantType.Cave;
    }

    @Override
    public boolean func_149851_a(World aWorld, int aX, int aY, int aZ, boolean aIsRemote) {
        return MOSSY[WD.meta(aWorld, aX, aY, aZ)];
    }
    ...
    public void func_149853_b(World aWorld, Random aRandom, int aX, int aY, int aZ) {
        for (byte[] tOffs : CUBE_3) {
            ...
            if (tBlock == Blocks.cobblestone
                && WD.set(aWorld, aX + tOffs[0], aY + tOffs[1], aZ + tOffs[2], Blocks.mossy_cobblestone, 0, 3)) return;
            byte tMeta = WD.meta(aWorld, aX + tOffs[0], aY + tOffs[1], aZ + tOffs[2]);
            if (tBlock == Blocks.stonebrick && (tMeta == 0 || tMeta == 2)
                && WD.set(aWorld, aX + tOffs[0], aY + tOffs[1], aZ + tOffs[2], Blocks.stonebrick, 1, 3)) return;
            if (tBlock instanceof BlockStones && MOSSABLE[tMeta]
                && WD.set(aWorld, aX + tOffs[0], aY + tOffs[1], aZ + tOffs[2], tBlock, MOSS_MAPPINGS[tMeta], 3)) return;
        }
    }
        ...
        if (!aEntity.isInWater() && !aEntity.isSneaking() && WD.meta(aWorld, aX, aY, aZ) == MCOBL) {
            ...
            if (Math.abs(aEntity.motionX) < tSpeed) {
                if (!WD.opq(aWorld, aX + tAddX, aY, aZ, F, F) && !WD.hasCollide(aWorld, aX + tAddX, aY + 1, aZ)) {
                    ...
                    if (!WD.opq(aWorld, aX - tAddX, aY, aZ, F, F) && !WD.hasCollide(aWorld, aX - tAddX, aY + 1, aZ)) {
                        aEntity.motionX = -tAddX * tSpeed;
                    } else if (!WD.opq(aWorld, aX + tAddX * 2, aY, aZ, F, F)
                        && !WD.hasCollide(aWorld, aX + tAddX * 2, aY + 1, aZ)) {
                            aEntity.motionX = +tAddX * tSpeed;
                        } else if (!WD.opq(aWorld, aX - tAddX * 2, aY, aZ, F, F)
                            && !WD.hasCollide(aWorld, aX - tAddX * 2, aY + 1, aZ)) {
                                aEntity.motionX = -tAddX * tSpeed;
            ...
            if (Math.abs(aEntity.motionZ) < tSpeed) {
                if (!WD.opq(aWorld, aX, aY, aZ + tAddZ, F, F) && !WD.hasCollide(aWorld, aX, aY + 1, aZ + tAddZ)) {
                    ...
                    if (!WD.opq(aWorld, aX, aY, aZ - tAddZ, F, F) && !WD.hasCollide(aWorld, aX, aY + 1, aZ - tAddZ)) {
                        aEntity.motionZ = -tAddZ * tSpeed;
                    } else if (!WD.opq(aWorld, aX, aY, aZ + tAddZ * 2, F, F)
                        && !WD.hasCollide(aWorld, aX, aY + 1, aZ + tAddZ * 2)) {
                            aEntity.motionZ = +tAddZ * tSpeed;
                        } else if (!WD.opq(aWorld, aX, aY, aZ - tAddZ * 2, F, F)
                            && !WD.hasCollide(aWorld, aX, aY + 1, aZ - tAddZ * 2)) {
                                aEntity.motionZ = -tAddZ * tSpeed;
            ...
            if (Math.abs(aEntity.motionX) < tSpeed && Math.abs(aEntity.motionZ) < tSpeed) {
                if (!WD.opq(aWorld, aX + tAddX, aY, aZ + tAddZ, F, F)
                    && !WD.hasCollide(aWorld, aX + tAddX, aY + 1, aZ + tAddZ)) {
                    aEntity.motionX = +tAddX * tSpeed;
                    aEntity.motionZ = +tAddZ * tSpeed;
                } else if (!WD.opq(aWorld, aX - tAddX, aY, aZ - tAddZ, F, F)
                    && !WD.hasCollide(aWorld, aX - tAddX, aY + 1, aZ - tAddZ)) {
                        aEntity.motionX = -tAddX * tSpeed;
                        aEntity.motionZ = -tAddZ * tSpeed;
                    } else if (!WD.opq(aWorld, aX + tAddX * 2, aY, aZ + tAddZ * 2, F, F)
                        && !WD.hasCollide(aWorld, aX + tAddX * 2, aY + 1, aZ + tAddZ * 2)) {
                            aEntity.motionX = +tAddX * tSpeed;
                            aEntity.motionZ = +tAddZ * tSpeed;
                        } else if (!WD.opq(aWorld, aX - tAddX * 2, aY, aZ - tAddZ * 2, F, F)
                            && !WD.hasCollide(aWorld, aX - tAddX * 2, aY + 1, aZ - tAddZ * 2)) {
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        return ST.arraylist(ST.make(this, 1, mBlock == this && aMeta == STONE ? COBBL : aMeta));
    }

    @Override
    public boolean isSealable(byte aMeta, byte aSide) {
        return SEALABLE[aMeta] && super.isSealable(aMeta, aSide);
    }

    @Override
    public int isProvidingWeakPower(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return WD.meta(aWorld, aX, aY, aZ) == RSTBR ? 15 : 0;
    }

    @Override
    public boolean shouldCheckWeakPower(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return mBlock == this && WD.meta(aWorld, aX, aY, aZ) != RSTBR;
    }

    @Override
    public void onNeighborBlockChange2(World aWorld, int aX, int aY, int aZ, Block aBlock) {
        if (MOSSY[WD.meta(aWorld, aX, aY, aZ)] && WD.burning(aWorld, aX, aY, aZ))
            aWorld.scheduleBlockUpdate(aX, aY, aZ, this, tickRate(aWorld));
    }
    ...
    public boolean canCreatureSpawn(byte aMeta) {
        return mBlock == this && SPAWNABLE[aMeta];
    }
    ...
    public int getFireSpreadSpeed(byte aMeta) {
        return MOSSY[aMeta] ? 3000 : 0;
    }

    @Override
    public boolean isReplaceableOreGen(World aWorld, int aX, int aY, int aZ, Block aTarget) {
        return aTarget == this && WD.meta(aWorld, aX, aY, aZ) == STONE;
    }// No longer pretend to be Vanilla Stone at Y<=6, as all the special cases (Draconium) have been resolved.
}
