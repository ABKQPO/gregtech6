...
public class BlockMetaType extends BlockBaseMeta {
    ...
    public BlockMetaType(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aSoundType,
        String aNameInternal, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier,
        float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons) {
        super(
            aItemClass == null ? ItemBlockMetaType.class : aItemClass,
            aNameInternal,
            aVanillaMaterial,
            aSoundType,
            aCount,
        ...
        mSlabs = new BlockMetaType[] {
            makeSlab(
                ...
                aDefaultLocalised,
                aMaterial,
                aResistanceMultiplier / 2,
                aHardnessMultiplier / 2,
                aHarvestLevel,
                maxMeta(),
                aIcons,
                SIDE_DOWN,
    ...
    protected BlockMetaType(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aSoundType,
    ...
    public void onBlockCreation(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aSoundType,
        String aName, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier,
        float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons) {
        //
    }

    public void onSlabCreation(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial, SoundType aSoundType,
        String aName, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier,
        float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons, byte aSlabType,
        BlockMetaType aBlock) {
        //
    ...
    public boolean onBlockActivated(World aWorld, int aX, int aY, int aZ, EntityPlayer aPlayer, int aSide, float aHitX,
        ...
        if (mBlock == this || aSide != OPOS[mSide]
            || (mBlock.getCollisionBoundingBoxFromPool(aWorld, aX, aY, aZ) != null
                && !aWorld.checkNoEntityCollision(mBlock.getCollisionBoundingBoxFromPool(aWorld, aX, aY, aZ))))
            return F;
        ...
        if (ST.equal(aStack, mBlock.mSlabs[0], aMetaData)) {
            ...
            aWorld.playSoundEffect(
                ...
                aY + 0.5F,
                aZ + 0.5F,
                mBlock.stepSound.func_150496_b(),
                (mBlock.stepSound.getVolume() + 1.0F) / 2.0F,
                mBlock.stepSound.getPitch() * 0.8F);
    ...
    public boolean shouldSideBeRendered(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        ...
        if (aSide != mSide && SIDES_VALID[mSide]) {
            ...
            if (aBlock instanceof BlockMetaType && ((BlockMetaType) aBlock).mSide == mSide)
    ...
    public boolean isSideSolid(int aMeta, byte aSide) {
        return mBlock == this || mSide == aSide;
    }
    ...
    public int getLightOpacity() {
        return mBlock == this ? LIGHT_OPACITY_MAX : LIGHT_OPACITY_WATER;
    }
    ...
    public Item getItemDropped(int par1, Random par2Random, int par3) {
        return Item.getItemFromBlock(mIsSlab ? mBlock.mSlabs[0] : mBlock);
    }

    @Override
    public void getSubBlocks(Item aItem, CreativeTabs aTab, @SuppressWarnings("rawtypes") List aList) {
        if (mIsPrimary) super.getSubBlocks(aItem, aTab, aList);
    }
