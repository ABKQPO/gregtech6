...
public class PrefixBlockFallingEntity extends EntityFallingBlock {
    ...
    public PrefixBlockFallingEntity(World aWorld, double aX, double aY, double aZ, IBlockPlacable aBlock,
        ItemStack aStack) {
        ...
        if (!worldObj.isRemote) {
            ...
            if (onGround) {
                ...
                if (worldObj.getBlock(aX, aY, aZ) != Blocks.piston_extension) {
                    setDead();
                    if (!worldObj.canPlaceEntityOnSide(super.func_145805_f(), aX, aY, aZ, T, 1, null, mStack)
                        || BlockFalling.func_149831_e(worldObj, aX, aY - 1, aZ)
                        || !mBlock.placeBlock(
                            worldObj,
                            aX,
                            aY,
                            ...
                            (byte) 1,
                            ST.meta_(mStack),
                            mStack.getTagCompound(),
                            T,
                            T)) {
                        if (field_145813_c) if (mBlock instanceof PrefixBlock) {
                            for (ItemStack tStack : ((PrefixBlock) mBlock).mDrops
                                .getDrops((PrefixBlock) mBlock, worldObj, aX, aY, aZ, ST.meta_(mStack), null, 0, F))
                                entityDropItem(tStack, 0.0F);
                        } else entityDropItem(mStack, 0.0F);
                    }
                }
    ...
    protected void fall(float p_70069_1_) {
        ...
        if (i > 0) for (Entity tEntity : new ArrayListNoNulls<Entity>(
            worldObj.getEntitiesWithinAABBExcludingEntity(this, boundingBox))) {
                if (tEntity instanceof EntityLivingBase) tEntity.attackEntityFrom(
                    DamageSource.fallingBlock,
                    TFC_DAMAGE_MULTIPLIER * Math.min(MathHelper.floor_float((float) i * 2), 40));
            }
    }
