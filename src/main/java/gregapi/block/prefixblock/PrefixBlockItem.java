...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "vazkii.botania.api.item.IFlowerPlaceable", modid = ModIDs.BOTA) })
public class PrefixBlockItem extends ItemBlock
    implements IItemUpdatable, IPrefixItem, IItemGT, IItemNoGTOverride, IFlowerPlaceable {
    ...
    public PrefixBlockItem(Block aBlock) {
        ...
        if ((SHOW_HIDDEN_PREFIXES || !mBlock.mPrefix.contains(TD.Creative.HIDDEN))
            && (SHOW_ORE_BLOCK_PREFIXES || "gt.meta.ore.normal.default".equalsIgnoreCase(mBlock.mNameInternal)
                || !mBlock.mPrefix.contains(TD.Prefix.ORE)
                || mBlock.mPrefix.contains(TD.Prefix.STORAGE_BASED))) {
            if (mBlock.mPrefix.mCreativeTab == null) mBlock.mPrefix.mCreativeTab = new CreativeTab(
                mBlock.mPrefix.mNameInternal,
                mBlock.mPrefix.mNameCategory,
                this,
                W);
    ...
    public void getSubItems(Item var1, CreativeTabs aCreativeTab, @SuppressWarnings("rawtypes") List aList) {
        if (!mBlock.mHidden && (SHOW_HIDDEN_PREFIXES || !mBlock.mPrefix.contains(TD.Creative.HIDDEN))
            && (SHOW_ORE_BLOCK_PREFIXES || mBlock == BlocksGT.ore
                || !mBlock.mPrefix.contains(TD.Prefix.ORE)
                || mBlock.mPrefix.contains(TD.Prefix.STORAGE_BASED)))
            for (int i = 0; i < mBlock.mMaterialList.length; i++)
                if (mBlock.mPrefix.isGeneratingItem(mBlock.mMaterialList[i]))
                    if (SHOW_HIDDEN_MATERIALS || !mBlock.mMaterialList[i].mHidden) {
    ...
    @Override
    public boolean placeBlockAt(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, int aSide,
    ...
    public int getColorFromItemStack(ItemStack aStack, int aRenderPass) {
        if (aRenderPass == 0) {
            ...
            if (UT.Code.exists(aMetaData, mBlock.mMaterialList))
                return UT.Code.getRGBInt(mBlock.mMaterialList[aMetaData].mRGBa[mBlock.mPrefix.mState]);
        }
        return UNCOLORED;
    }
    ...
    public final String getUnlocalizedName(ItemStack aStack) {
        ...
        if (UT.Code.exists(aMetaData, mBlock.mMaterialList))
            return "oredict." + mBlock.mPrefix.dat(mBlock.mMaterialList[aMetaData])
                .toString();
    ...
    public void addInformation(ItemStack aStack, EntityPlayer aPlayer, @SuppressWarnings("rawtypes") List aList,
        ...
        aList.add(
            LH.getToolTipBlastResistance(
                field_150939_a,
                mBlock.mBaseResistance * (1 + mBlock.getHarvestLevel(aMaterial == null ? 0 : aMaterial.mToolQuality))));
        while (aList.remove(null));
    }

    @Override
    public OreDictItemData getOreDictItemData(ItemStack aStack) {
        if (mBlock.mPrefix != null && UT.Code.exists(ST.meta_(aStack), mBlock.mMaterialList))
            return new OreDictItemData(mBlock.mPrefix, mBlock.mMaterialList[ST.meta_(aStack)]);
        return null;
    }
    ...
    public int getItemStackLimit(ItemStack aStack) {
        return mBlock.mPrefix.mDefaultStackSize;
    }

    @Override
    public OreDictMaterial getMaterial(int aMetaData) {
        return UT.Code.exists(aMetaData, mBlock.mMaterialList) ? mBlock.mMaterialList[aMetaData] : null;
    }

    @Override
    public OreDictPrefix getPrefix(int aMetaData) {
        return mBlock.mPrefix;
    }
    /*
     ...
     * OreDictMaterial aMaterial = (UT.Code.exists(aStack), mBlock.mMaterialList) ?
     * mBlock.mMaterialList[ST.meta(aStack)] : null);
     * if (aMaterial != null && (mBlock.mPrefix.mAmount >= U || mBlock.mPrefix.mAmount < 0)) for (TC_AspectStack tAspect
