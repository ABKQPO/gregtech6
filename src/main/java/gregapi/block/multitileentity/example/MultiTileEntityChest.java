...
public class MultiTileEntityChest extends TileEntityBase05Inventories
    implements IMultiTileEntity.IMTE_IsProvidingWeakPower, IMultiTileEntity.IMTE_IsProvidingStrongPower,
    IItemColorableRGB, ITileEntityDecolorable, ITileEntitySurface, IMultiTileEntity.IMTE_OnRegistrationClient,
    IMultiTileEntity.IMTE_OnRegistrationFirstClient, IMultiTileEntity.IMTE_SyncDataByte,
    IMultiTileEntity.IMTE_AddToolTips, IMultiTileEntity.IMTE_SetBlockBoundsBasedOnState,
    IMultiTileEntity.IMTE_GetSubItems, IMultiTileEntity.IMTE_SyncDataByteArray,
    IMultiTileEntity.IMTE_GetExplosionResistance, IMultiTileEntity.IMTE_GetBlockHardness,
    IMultiTileEntity.IMTE_GetComparatorInputOverride, IMultiTileEntity.IMTE_GetSelectedBoundingBoxFromPool,
    IMultiTileEntity.IMTE_GetCollisionBoundingBoxFromPool, IMultiTileEntity.IMTE_OnPlaced,
    ...
    public void onTick(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (mUsingPlayers > 0) {
                ...
                if (mLidAngle > 0.1F && oLidAngle <= 0.1F)
                    UT.Sounds.play("random.chestopen", 2, 0.5F, RNGSUS.nextFloat() * 0.1F + 0.9F, getCoords());
            } else {
                mLidAngle = Math.max(0, mLidAngle - 0.1F);
                if (mLidAngle < 0.5F && oLidAngle >= 0.5F)
                    UT.Sounds.play("random.chestclosed", 2, 0.5F, RNGSUS.nextFloat() * 0.1F + 0.9F, getCoords());
            }
        }
    }
    ...
    public boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide() && !worldObj.isSideSolid(xCoord, yCoord + 1, zCoord, FORGE_DIR[SIDE_BOTTOM])
            && isUseableByPlayerGUI(aPlayer)) {
            generateDungeonLoot();
            openGUI(aPlayer);
        }
        return T;
    ...
    // // Only auto-generate Loot if a second has passed since its original placement. Prevents Item spillage during
    // Worldgen in most cases.
    ...
    public void openInventoryGUI() {
        mUsingPlayers++;
        if (mIsTrapped) causeBlockUpdate();
    }
    ...
    protected void generateDungeonLoot() {
        if (isServerSide() && UT.Code.stringValid(mDungeonLootName)
            && ST.generateLoot(RNGSUS, mDungeonLootName, this)) {
            worldObj.spawnEntityInWorld(
                new EntityXPOrb(
                    ...
                    yCoord + 1.25,
                    zCoord + 0.4,
                    5 + RNGSUS.nextInt(5) + RNGSUS.nextInt(5)));
            worldObj.spawnEntityInWorld(
                new EntityXPOrb(
                    worldObj,
                    xCoord + 0.4,
    ...
    public boolean getSubItems(MultiTileEntityBlockInternal aBlock, Item aItem, CreativeTabs aTab,
        ...
        if (D1 || "lootchest".equalsIgnoreCase(mTextureName)) for (String tLoot : ST.LOOT_TABLES)
            aList.add(aBlock.mMultiTileEntityRegistry.getItem(aID, UT.NBT.makeString("gt.dungeonloot", tLoot)));
        return T;
    }

    @Override
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        if (UT.Code.stringValid(mDungeonLootName))
            aList.add(LH.Chat.BLINKING_CYAN + "Contains Loot of " + LH.Chat.WHITE + LH.get("loot." + mDungeonLootName));
    ...
    public boolean unpaint() {
        if (mIsPainted) {
            mIsPainted = F;
            mRGBa = UT.Code.getRGBInt(mMaterial.fRGBaSolid);
            updateClientData();
            return T;
        }
        return F;
    ...
    public boolean isPainted() {
        return mIsPainted || (worldObj != null && isClientSide() && UT.Code.getRGBInt(mMaterial.fRGBaSolid) != mRGBa);
    }
    ...
    public boolean recolorItem(ItemStack aStack, int aRGB) {
        if (paint((isPainted() ? UT.Code.mixRGBInt(aRGB, getPaint()) : aRGB) & ALL_NON_ALPHA_COLOR)) {
    ...
    public int isProvidingWeakPower(byte aOppositeSide) {
        return mIsTrapped && mUsingPlayers > 0 ? 15 : 0;
    }
    ...
    public Object getGUIClient(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerClientChest(aPlayer.inventory, this, aGUIID);
    }

    @Override
    public Object getGUIServer(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerCommonChest(aPlayer.inventory, this, aGUIID);
    }
    ...
    public static class MultiTileEntityRendererChest extends TileEntitySpecialRenderer {
        ...
        public void renderTileEntityAt(TileEntity aTileEntity, double aX, double aY, double aZ, float aPartialTick) {
            if (aTileEntity instanceof MultiTileEntityChest) {
                double tLidAngle = 1 - (((MultiTileEntityChest) aTileEntity).oLidAngle
                    + (((MultiTileEntityChest) aTileEntity).mLidAngle - ((MultiTileEntityChest) aTileEntity).oLidAngle)
                        * aPartialTick);
