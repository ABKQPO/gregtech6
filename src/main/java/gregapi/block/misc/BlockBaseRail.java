...
public class BlockBaseRail extends BlockRailBase implements IBlockBase, IBlockSealable, IBlockToolable {
    ...
    public BlockBaseRail(Class<? extends ItemBlockBase> aItemClass, String aNameInternal, String aLocalName,
        boolean aPowerRail, boolean aDetectorRail, float aSpeed, float aExplosionResistance, int aHarvestLevel,
        IIconContainer aIconPrimary, IIconContainer aIconSecondary) {
    ...
    public void addInformation(ItemStack aStack, byte aMeta, EntityPlayer aPlayer,
        @SuppressWarnings("rawtypes") List aList, boolean aF3_H) {
        aList.add(
            LH.Chat.CYAN + LH.get(LH.TOOLTIP_RAILSPEED)
                + LH.Chat.GREEN
                + Math.min(MD.RC.mLoaded ? 3 : 10, mSpeed / 0.4F)
                + "x");
    }
    ...
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        return Blocks.rail.getBlockHardness(aWorld, aX, aY, aZ);
    }

    @Override
    public float getExplosionResistance(Entity aEntity, World aWorld, int aX, int aY, int aZ, double eX, double eY,
        double eZ) {
    ...
    public boolean canSilkHarvest(World aWorld, EntityPlayer aPlayer, int aX, int aY, int aZ, int aMeta) {
        return canSilkHarvest(UT.Code.bind4(aMeta));
    }
    ...
    public boolean canCreatureSpawn(EnumCreatureType type, IBlockAccess aWorld, int aX, int aY, int aZ) {
        return canCreatureSpawn(WD.meta(aWorld, aX, aY, aZ));
    }
    ...
    public IIcon getIcon(int aSide, int aMeta) {
        return ((mPowerRail || mDetectorRail ? (aMeta & 8) != 0 : aMeta >= 6) ? mIconSecondary : mIconPrimary)
            .getIcon(0);
    }
    ...
    public boolean isFlammable(byte aMeta) {
        return getFlammability(aMeta) > 0;
    }
        ...
        if (!aWorld.isRemote) {
            ...
            if (aTool.equals(TOOL_crowbar)) {
                ...
                boolean tResult = aWorld.setBlock(
                    ...
                    aZ,
                    this,
                    isPowered() ? (aMeta + 1) % 10 : ((aMeta / 8) * 8) + (((aMeta % 8) + 1) % 6),
                    0);
                aWorld.isRemote = F;
    ...
    protected boolean func_150058_a(World aWorld, int aX, int aY, int aZ, int p_150058_5_, boolean p_150058_6_,
        int p_150058_7_) {
        ...
        return func_150057_a(aWorld, aX, aY, aZ, p_150058_6_, p_150058_7_, j1)
            || (flag1 && func_150057_a(aWorld, aX, aY - 1, aZ, p_150058_6_, p_150058_7_, j1));
    }

    protected boolean func_150057_a(World aWorld, int aX, int aY, int aZ, boolean p_150057_5_, int p_150057_6_,
        int p_150057_7_) {
    ...
    protected void func_150048_a(World aWorld, int aX, int aY, int aZ, int aMeta, int aData, Block aBlock) {
        if (mPowerRail) {
            ...
            flag = flag || func_150058_a(aWorld, aX, aY, aZ, aMeta, T, 0)
                || func_150058_a(aWorld, aX, aY, aZ, aMeta, F, 0);
            boolean flag1 = F;
            if (flag && (aMeta & 8) == 0) {
    ...
    public int isProvidingWeakPower(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return mDetectorRail ? (WD.meta(aWorld, aX, aY, aZ) & 8) != 0 ? 15 : 0 : 0;
    }

    @Override
    public int isProvidingStrongPower(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        return mDetectorRail ? (WD.meta(aWorld, aX, aY, aZ) & 8) == 0 ? 0 : (aSide == 1 ? 15 : 0) : 0;
    }
    ...
    private void func_150054_a(World aWorld, int aX, int aY, int aZ, int aMetaData) {
        ...
        boolean flag1 = F;
        @SuppressWarnings("unchecked")
        List<EntityMinecart> list = aWorld.getEntitiesWithinAABB(
            EntityMinecart.class,
            AxisAlignedBB.getBoundingBox(aX + 0.125, aY, aZ + 0.125, aX + 0.875, aY + 0.875, aZ + 0.875));
    ...
    public void onBlockAdded(World p_149726_1_, int p_149726_2_, int p_149726_3_, int p_149726_4_) {
        ...
        if (mDetectorRail) func_150054_a(
            p_149726_1_,
            p_149726_2_,
            p_149726_3_,
            p_149726_4_,
    ...
    public int getComparatorInputOverride(World aWorld, int aX, int aY, int aZ, int aSide) {
        if (mDetectorRail && (WD.meta(aWorld, aX, aY, aZ) & 8) > 0) {
            @SuppressWarnings("unchecked")
            List<EntityMinecartCommandBlock> list = aWorld.getEntitiesWithinAABB(
            ...
            if (list.size() > 0) return list.get(0)
                ...
                .func_145760_g();
            @SuppressWarnings("unchecked")
            List<EntityMinecart> list1 = aWorld.selectEntitiesWithinAABB(
                EntityMinecart.class,
                AxisAlignedBB.getBoundingBox(aX + 0.125, aY, aZ + 0.125, aX + 0.875, aY + 0.875, aZ + 0.875),
    ...
    public float getRailMaxSpeed(World aWorld, EntityMinecart aCart, int aX, int aY, int aZ) {
        switch (WD.meta(aWorld, aX, aY, aZ) & 7) {
            case 0:
                if (WD.block(aWorld, aX, aY, aZ + 1) instanceof BlockBaseRail
                    && (WD.meta(aWorld, aX, aY, aZ + 1) & 7) == 0
                    && WD.block(aWorld, aX, aY, aZ - 1) instanceof BlockBaseRail
                    && (WD.meta(aWorld, aX, aY, aZ - 1) & 7) == 0)
                    return aWorld.doChunksNearChunkExist(aX, aY, aZ, 17) ? mSpeed : Math.min(mSpeed, 1.0F);
            case 1:
                if (WD.block(aWorld, aX + 1, aY, aZ) instanceof BlockBaseRail
                    && (WD.meta(aWorld, aX + 1, aY, aZ) & 7) == 1
                    && WD.block(aWorld, aX - 1, aY, aZ) instanceof BlockBaseRail
                    && (WD.meta(aWorld, aX - 1, aY, aZ) & 7) == 1)
                    return aWorld.doChunksNearChunkExist(aX, aY, aZ, 17) ? mSpeed : Math.min(mSpeed, 1.0F);
            default:
                ...
                if (tMotion > 0.01) {
                    ...
                    if (tRailMeta == 1) {
                        if (aWorld.getBlock(aX - 1, aY, aZ)
                            .isNormalCube(aWorld, aX - 1, aY, aZ)) aCart.motionX = +0.02;
                        else if (aWorld.getBlock(aX + 1, aY, aZ)
                            .isNormalCube(aWorld, aX + 1, aY, aZ)) aCart.motionX = -0.02;
                    } else if (tRailMeta == 0) {
                        if (aWorld.getBlock(aX, aY, aZ - 1)
                            .isNormalCube(aWorld, aX, aY, aZ - 1)) aCart.motionZ = +0.02;
                        else if (aWorld.getBlock(aX, aY, aZ + 1)
                            .isNormalCube(aWorld, aX, aY, aZ + 1)) aCart.motionZ = -0.02;
                    }
                }
            } else {
    ...
    public boolean onItemUse(ItemBlockBase aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (tBlock == Blocks.snow_layer && (WD.meta(aWorld, aX, aY, aZ) & 7) < 1) {
            aSide = SIDE_UP;
        } else if (tBlock != Blocks.vine && tBlock != Blocks.tallgrass
            && tBlock != Blocks.deadbush
            && !tBlock.isReplaceable(aWorld, aX, aY, aZ)) {
                aX += OFFX[aSide];
                aY += OFFY[aSide];
                aZ += OFFZ[aSide];
            }

        if (!aPlayer.canPlayerEdit(aX, aY, aZ, aSide, aStack) || (aY == 255 && getMaterial().isSolid())
            || !aWorld.canPlaceEntityOnSide(this, aX, aY, aZ, F, aSide, aPlayer, aStack)) return F;
        ...
        if (aItem.placeBlockAt(
            ...
            aHitY,
            aHitZ,
            SIDES_AXIS_X[UT.Code.getHorizontalForPlayerPlacing(aPlayer)] ? 1 : 0)) {
