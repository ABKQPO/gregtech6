...
public class BlockBaseLilyPad extends BlockBaseMeta implements IPlantable, IRenderedBlock {
    ...
    public BlockBaseLilyPad(Class<? extends ItemBlock> aItemClass, String aNameInternal, Material aMaterial,
    ...
    public void addCollisionBoxesToList(World aWorld, int aX, int aY, int aZ, AxisAlignedBB aAABB,
        ...
        if (!(aEntity instanceof EntityBoat)) super.addCollisionBoxesToList(aWorld, aX, aY, aZ, aAABB, aList, aEntity);
    }

    @Override
    public boolean canBlockStay(World aWorld, int aX, int aY, int aZ) {
        return aY >= 0 && aY < 256
            && aWorld.getBlock(aX, aY - 1, aZ)
                .getMaterial() == Material.water
            && aWorld.getBlockMetadata(aX, aY - 1, aZ) == 0;
    }

    @Override
    public boolean canPlaceBlockAt(World aWorld, int aX, int aY, int aZ) {
        return super.canPlaceBlockAt(aWorld, aX, aY, aZ) && canBlockStay(aWorld, aX, aY, aZ);
    }
    ...
    public ItemStack onItemRightClick(ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        ...
        if (aWorld.getBlock(aX, aY, aZ)
            .getMaterial() == Material.water && WD.meta(aWorld, aX, aY, aZ) == 0
            && aWorld.isAirBlock(aX, aY + 1, aZ)) {
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World aWorld, int aX, int aY, int aZ) {
        return AxisAlignedBB.getBoundingBox(aX, aY, aZ, aX + 1, aY + 0.015625F, aZ + 1);
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess aWorld, int aX, int aY, int aZ) {
        setBlockBounds(0, 0, 0, 1, 0.015625F, 1);
    }
    ...
    public ITexture getTexture(int aRenderPass, byte aSide, ItemStack aStack) {
        return SIDES_VERTICAL[aSide] ? BlockTextureDefault.get(mIcons[ST.meta_(aStack) % mIcons.length]) : null;
    }

    @Override
    public ITexture getTexture(int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered, IBlockAccess aWorld,
        int aX, int aY, int aZ) {
        return SIDES_VERTICAL[aSide] ? BlockTextureDefault.get(mIcons[WD.meta(aWorld, aX, aY, aZ) % mIcons.length])
            : null;
    }
