...
public class PlankEntry {
    ...
    public PlankEntry(ItemStack aPlank, OreDictMaterial aMaterialPlank, int aPlankIndex) {
        this(
            ...
            aMaterialPlank,
            aPlankIndex,
            aMaterialPlank == MT.Wood ? IL.Stick.get(1) : OP.stick.mat(aMaterialPlank, 1));
    }
    ...
    public PlankEntry(ItemStack aPlank, OreDictMaterial aMaterialPlank, int aPlankIndex, ItemStack aStick,
        int aStickCountHand, int aStickCountSaw, int aStickCountLathe) {
        this(
            aPlank,
            IL.Plank_Slab.get(1, ST.make(Blocks.wooden_slab, 1, 0)),
            aMaterialPlank,
    ...
    public PlankEntry(ItemStack aPlank, ItemStack aSlab, OreDictMaterial aMaterialPlank, int aPlankIndex,
        ItemStack aStick, int aStickCountHand, int aStickCountSaw, int aStickCountLathe) {
        this(
            aPlank,
            aSlab,
            IL.Plank_Stairs.get(1, ST.make(Blocks.oak_stairs, 1, 0)),
            aMaterialPlank,
            aPlankIndex,
            aStick,
            aStickCountHand,
    ...
    public PlankEntry(ItemStack aPlank, ItemStack aSlab, ItemStack aStair, OreDictMaterial aMaterialPlank,
        int aPlankIndex, ItemStack aStick) {
        this(aPlank, aSlab, aStair, aMaterialPlank, aPlankIndex, aStick, 1, 2, 2);
    }

    public PlankEntry(ItemStack aPlank, ItemStack aSlab, ItemStack aStair, OreDictMaterial aMaterialPlank,
        int aPlankIndex, ItemStack aStick, int aStickCountHand, int aStickCountSaw, int aStickCountLathe) {
        ...
        if (ST.valid(mPlank) && !WoodDictionary.PLANKS.containsKey(new ItemStackContainer(mPlank))) {
            if (ST.meta(mPlank) == W) for (int i = 0; i < 16; i++) {
                ...
                if (OM.materialcontained(tPlank, T, MT.Wood, MT.WoodRubber, ANY.Wood))
                    OreDictManager.INSTANCE.setItemData_(tPlank, new OreDictItemData(mMaterialPlank, U));
            }
            if (OM.materialcontained(mPlank, T, MT.Wood, MT.WoodRubber, ANY.Wood))
        ...
        if (ST.valid(mStair) && !WoodDictionary.STAIRS.containsKey(new ItemStackContainer(mStair))) {
            if (ST.meta(mStair) == W) for (int i = 0; i < 16; i++) {
                ...
                if (OM.materialcontained(tStair, T, MT.Wood, MT.WoodRubber, ANY.Wood))
                    OreDictManager.INSTANCE.setItemData_(tStair, new OreDictItemData(mMaterialPlank, U4 * 3));
            }
            if (OM.materialcontained(mStair, T, MT.Wood, MT.WoodRubber, ANY.Wood))
        ...
        if (ST.valid(mSlab) && !WoodDictionary.SLABS.containsKey(new ItemStackContainer(mSlab))) {
            if (ST.meta(mSlab) == W) for (int i = 0; i < 16; i++) {
                ...
                if (OM.materialcontained(tSlab, T, MT.Wood, MT.WoodRubber, ANY.Wood))
                    OreDictManager.INSTANCE.setItemData_(tSlab, new OreDictItemData(mMaterialPlank, U2));
            }
            if (OM.materialcontained(mSlab, T, MT.Wood, MT.WoodRubber, ANY.Wood))
