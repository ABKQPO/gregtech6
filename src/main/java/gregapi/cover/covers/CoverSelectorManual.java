...
public class CoverSelectorManual extends AbstractCoverAttachmentSelector {
    ...
    public void onCoverPlaced(byte aSide, CoverData aData, Entity aPlayer, ItemStack aCover) {
        ...
        if (aData.mTileEntity instanceof ITileEntitySwitchableMode)
            aData.visual(aSide, UT.Code.bind4(((ITileEntitySwitchableMode) aData.mTileEntity).getStateMode()));
    }
        ...
        if (!aData.mStopped && aSide == aSideClicked && aData.mTileEntity instanceof ITileEntitySwitchableMode) {
            ...
            if (tCoords[1] >= PX_P[1] && tCoords[1] <= PX_P[4]) {
                if (tCoords[0] >= PX_P[1] && tCoords[0] <= PX_P[4]) {
                    ...
                    if (tMode < 0) tMode = 15;
                    rReturn = T;
                } else if (tCoords[0] >= PX_N[4] && tCoords[0] <= PX_N[1]) {
                    tMode++;
                    if (tMode > 15) tMode = 0;
                    rReturn = T;
                }
            } else if (tCoords[1] >= PX_N[7] && tCoords[1] <= PX_N[4]) {
                if (tCoords[0] >= PX_P[2] && tCoords[0] <= PX_N[2]) {
                    if (tCoords[0] <= PX_P[5]) {
                        tMode ^= 8;
                    } else if (tCoords[0] <= PX_P[8]) {
                        tMode ^= 4;
                    } else if (tCoords[0] <= PX_N[5]) {
                        tMode ^= 2;
                    } else {
                        tMode ^= 1;
                    }
            ...
            if (aData.mTileEntity.isServerSide())
                aData.visual(aSide, ((ITileEntitySwitchableMode) aData.mTileEntity).setStateMode(tMode));
            return rReturn;
        }
        return F;
    }
