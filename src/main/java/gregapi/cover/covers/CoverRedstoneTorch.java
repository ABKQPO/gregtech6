...
public class CoverRedstoneTorch extends AbstractCoverAttachmentTorch {
    ...
    public ITexture getCoverTextureSurface(byte aSide, CoverData aData) {
        return aData.mVisuals[aSide] == 0 ? sTextureFrontON : sTextureFrontOFF;
    }

    @Override
    public ITexture getCoverTextureAttachment(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide == aTextureSide ? aData.mVisuals[aSide] == 0 ? sTextureFrontON : sTextureFrontOFF : null;
    }

    @Override
    public ITexture getCoverTextureHolder(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide == aTextureSide ? null : aData.mVisuals[aSide] == 0 ? sTextureSideON : sTextureSideOFF;
    }

    public static final ITexture sTextureFrontON = BlockTextureDefault.get("machines/covers/redstonetorch/on/front"),
    ...
    @Override
    public boolean condition(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide,
