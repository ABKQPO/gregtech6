...
public class CoverSelectorButtonPanel extends AbstractCoverAttachmentSelector {
    ...
    public void onCoverPlaced(byte aSide, CoverData aData, Entity aPlayer, ItemStack aCover) {
        ...
        if (aData.mTileEntity instanceof ITileEntitySwitchableMode) aData.visual(
            aSide,
            (short) ((aData.mVisuals[aSide] & ~15)
                | UT.Code.bind4(((ITileEntitySwitchableMode) aData.mTileEntity).getStateMode())));
    }
    ...
    public void onCoverLoaded(byte aSide, CoverData aData) {
        ...
        if (aData.mTileEntity instanceof ITileEntitySwitchableMode)
            ((ITileEntitySwitchableMode) aData.mTileEntity).setStateMode(UT.Code.bind4(aData.mVisuals[aSide] & 15));
    }

    @Override
    public void onBlockUpdate(byte aSide, CoverData aData) {
        if (!aData.mStopped && aData.mTileEntity instanceof ITileEntitySwitchableMode) aData.visual(
            aSide,
            (short) ((aData.mVisuals[aSide] & ~15)
                | UT.Code.bind4(((ITileEntitySwitchableMode) aData.mTileEntity).getStateMode())));
    }
    ...
    public boolean onCoverClickedRight(byte aSide, CoverData aData, Entity aPlayer, byte aSideClicked, float aHitX,
        ...
        if (!aData.mStopped && aSide == aSideClicked && aData.mTileEntity instanceof ITileEntitySwitchableMode) {
            ...
            if (aData.mTileEntity.isServerSide()) aData.visual(
                aSide,
                (short) ((aData.mVisuals[aSide] & ~15)
                    | UT.Code.bind4(((ITileEntitySwitchableMode) aData.mTileEntity).setStateMode(tMode))));
    ...
    public ITexture getCoverTextureSurface(byte aSide, CoverData aData) {
        return BlockTextureMulti
            .get(sTexturesBase[(aData.mVisuals[aSide] >> 4) & 7], sTextures[UT.Code.bind4(aData.mVisuals[aSide] & 15)]);
    }

    @Override
    public ITexture getCoverTextureAttachment(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide != aTextureSide ? sTexturesBase[0]
            : BlockTextureMulti.get(sTexturesBase[0], getCoverTextureSurface(aSide, aData));
    }
