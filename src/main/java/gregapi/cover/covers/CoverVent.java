...
public class CoverVent extends AbstractCoverAttachment {
    ...
    public void onTickPre(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide, boolean aReceivedBlockUpdate,
        ...
        if (aIsServerSide && !aData.mStopped
            ...
            && SERVER_TIME % 360 == 30 + (60 * aSide)) {
            if (WD.collectable_air(
                aData.mTileEntity.getWorld(),
                aData.mTileEntity.getOffsetX(aSide),
                aData.mTileEntity.getOffsetY(aSide),
                aData.mTileEntity.getOffsetZ(aSide))) {
                switch (aData.mTileEntity.getWorld().provider.dimensionId) {
                    case DIM_OVERWORLD:
                        FL.fill_(
                            (IFluidHandler) aData.mTileEntity,
                            ALL_SIDES_THIS_AND_ANY[aSide],
                            FL.Air.make(256000),
                            T);
                        return;
                    case DIM_NETHER:
                    ...
                    case DIM_END:
                        FL.fill_(
                            (IFluidHandler) aData.mTileEntity,
                            ALL_SIDES_THIS_AND_ANY[aSide],
                            FL.Air_End.make(256000),
                            T);
                        return;
                }
                String tBiome = aData.mTileEntity
                    .getBiome(aData.mTileEntity.getOffsetX(aSide), aData.mTileEntity.getOffsetZ(aSide)).biomeName;
