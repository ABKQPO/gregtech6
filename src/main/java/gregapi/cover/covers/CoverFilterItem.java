...
public class CoverFilterItem extends AbstractCoverAttachment {
                    ...
                    if (ST.invalid(tStack)) {
                        ...
                        aChatReturn.add(
                            "Filters for: " + LH.Chat.CYAN
                                ...
                                + LH.Chat.GRAY
                                + " ; "
                                + (ST.meta_(tStack) == W ? LH.Chat.GREEN + "Wildcard"
                                    : LH.Chat.CYAN + ST.meta_(tStack)));
                    }
                }
            }
            return 1;
            ...
            if (ST.valid(tStack)) {
                ...
                if (ST.invalid(tFilter)) {
                    ...
                    if (ST.meta_(tFilter) == W) {
                        aData.mNBTs[aCoverSide] = ST
                            .save("gt.filter.item", ST.make(ST.item_(tStack), 1, ST.meta_(tStack)));
                        UT.Sounds.send(SFX.MC_CLICK, aData.mTileEntity);
                        UT.Entities.sendchat(
                            aPlayer,
                            "Filters for: " + LH.Chat.CYAN
    ...
    public boolean interceptItemInsert(byte aCoverSide, CoverData aData, int aSlot, ItemStack aStack, byte aSide) {
        ...
        return (aData.mVisuals[aCoverSide] == 0)
            != ST.equal(ST.load(aData.mNBTs[aCoverSide], "gt.filter.item"), aStack, T);
    }
    ...
    public boolean interceptConnect(byte aCoverSide, CoverData aData) {
        boolean rIntercept = aData.mTileEntity instanceof ITileEntityItemPipe
            && aData.mTileEntity.getAdjacentTileEntity(aCoverSide).mTileEntity instanceof ITileEntityItemPipe;
    ...
    public boolean interceptCoverPlacement(byte aCoverSide, CoverData aData, Entity aPlayer) {
        return aData.mTileEntity instanceof ITileEntityItemPipe
            && aData.mTileEntity.getAdjacentTileEntity(aCoverSide).mTileEntity instanceof ITileEntityItemPipe;
    }
