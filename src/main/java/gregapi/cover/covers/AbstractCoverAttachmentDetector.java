...
public abstract class AbstractCoverAttachmentDetector extends AbstractCoverAttachment {
    ...
    public long onToolClick(byte aSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            ...
            if (aChatReturn != null) aChatReturn
                .add((aData.mVisuals[aSide] & B[1]) != 0 ? "Emits when Condition FALSE" : "Emits when Condition TRUE");
            return 1000;
        }
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) {
                aChatReturn.add((aData.mVisuals[aSide] & B[0]) != 0 ? "Emits strong Redstone" : "Emits weak Redstone");
                aChatReturn.add(
                    (aData.mVisuals[aSide] & B[1]) != 0 ? "Emits when Condition FALSE" : "Emits when Condition TRUE");
            }
            return 1;
        }
    ...
    public byte getRedstoneOutWeak(byte aCoverSide, CoverData aData, byte aDefaultRedstone) {
        return (aData.mVisuals[aCoverSide] & B[1]) != 0 ? UT.Code.bind4(15 - aData.mValues[aCoverSide])
            : UT.Code.bind4(aData.mValues[aCoverSide]);
    }
