...
public abstract class AbstractCoverAttachmentLogistics extends AbstractCoverAttachment {
    ...
    public boolean interceptCoverPlacement(byte aCoverSide, CoverData aData, Entity aPlayer) {
        return !(aData.mTileEntity instanceof ITileEntityLogistics
            && ((ITileEntityLogistics) aData.mTileEntity).canLogistics(SIDE_ANY));
    }
    ...
    public void onCoverPlaced(byte aSide, CoverData aData, Entity aPlayer, ItemStack aCover) {
        if (aData.mTileEntity instanceof ITileEntityConnector
            && ((ITileEntityConnector) aData.mTileEntity).connected(aSide))
            ((ITileEntityConnector) aData.mTileEntity).disconnect(aSide, T);
    ...
    public long onToolClick(byte aCoverSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_cutter) && useTargetStackSize()) {
            aData.value(
                aCoverSide,
                (short) ((aData.mValues[aCoverSide] & 3) | (((((aData.mValues[aCoverSide] >> 2) + 1) % 65) << 2))));
            if (aChatReturn != null) {
    ...
    public ITexture getCoverTextureAttachment(byte aCoverSide, CoverData aData, byte aTextureSide) {
        return aCoverSide == aTextureSide ? getCoverTextureSurface(aCoverSide, aData) : sTextureBase;
    }
