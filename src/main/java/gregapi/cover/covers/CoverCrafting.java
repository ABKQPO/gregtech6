...
public class CoverCrafting extends CoverTextureMulti {
    ...
    public boolean onCoverClickedRight(byte aSide, CoverData aData, Entity aPlayer, byte aSideClicked, float aHitX,
        ...
        if (aPlayer instanceof EntityPlayerMP) {
            ...
            ((EntityPlayerMP) aPlayer).playerNetServerHandler
                .sendPacket(new S2DPacketOpenWindow(((EntityPlayerMP) aPlayer).currentWindowId, 1, "Crafting", 9, T));
            ((EntityPlayerMP) aPlayer).openContainer = new ContainerWorkbench(
                ...
                aData.mTileEntity.getX(),
                aData.mTileEntity.getY(),
                aData.mTileEntity.getZ()) {
