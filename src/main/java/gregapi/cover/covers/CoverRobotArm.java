...
public class CoverRobotArm extends AbstractCoverAttachment {
    ...
    public long onToolClick(byte aSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            aData.value(
                ...
                (short) UT.Code.bind(
                    Short.MIN_VALUE,
                    aData.mTileEntity instanceof MultiTileEntityPipeItem ? -1 : Short.MAX_VALUE,
                    aData.mValues[aSide] + (aSneaking ? -1 : +1)));
            if (aChatReturn != null) aChatReturn.add(
                aData.mValues[aSide] < 0 ? "Takes from Slot: " + (-1 - aData.mValues[aSide])
                    : "Puts into Slot: " + aData.mValues[aSide]);
            return 200;
        }
        ...
        if (aIsServerSide && SERVER_TIME % mTiming == 0 && !aData.mStopped && aData.mTileEntity instanceof IInventory) {
            if (aData.mValues[aSide] < 0) {
                if (aData.mVisuals[aSide] == 0) {
                    ST.moveFrom(
                        aData.delegator(aSide),
                        aData.mTileEntity.getAdjacentTileEntity(aSide),
                        -1 - aData.mValues[aSide],
                        null,
                        T,
                        F,
                        F,
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            LH.Chat.CYAN + "Transfers a Stack every "
                + (mTiming == 1 ? "Tick from/to a specific Slot" : mTiming + " Ticks from/to a specific Slot"));
    ...
    public ITexture getCoverTextureAttachment(byte aCoverSide, CoverData aData, byte aTextureSide) {
        return aCoverSide == aTextureSide
            ? BlockTextureMulti.get(BACKGROUND_COVER, aData.mVisuals[aCoverSide] == 0 ? sTextureOut : sTextureIn)
            : aCoverSide == OPOS[aTextureSide]
                ? BlockTextureMulti.get(BACKGROUND_COVER, aData.mVisuals[aCoverSide] != 0 ? sTextureOut : sTextureIn)
                : BACKGROUND_COVER;
    }
