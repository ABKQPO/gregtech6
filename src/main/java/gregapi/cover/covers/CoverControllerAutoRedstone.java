...
public class CoverControllerAutoRedstone extends AbstractCoverAttachmentController {
    ...
    public long onToolClick(byte aSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            ...
            if (aChatReturn != null) aChatReturn
                .add((aData.mValues[aSide] & B[0]) != 0 ? "Runs when Input is OFF" : "Runs when Input is ON");
            return 1000;
        }
    ...
    public static final ITexture sTextureForeground = BlockTextureDefault
        .get("machines/covers/autoredstoneswitch/circuit");

    @Override
    public boolean getStateOnOff(byte aSide, CoverData aData) {
        return ((aData.mTileEntity instanceof ITileEntityRunningActively
            && ((ITileEntityRunningActively) aData.mTileEntity).getStateRunningActively())
            && !(aData.mTileEntity instanceof ITileEntityRunningSuccessfully
                && ((ITileEntityRunningSuccessfully) aData.mTileEntity).getStateRunningSuccessfully()))
            || UT.Code.bind1(aData.mTileEntity.getRedstoneIncoming(aSide)) != (aData.mValues[aSide] & B[0]);
    }
}
