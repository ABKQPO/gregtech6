...
public class CoverControllerDisplay extends AbstractCoverAttachmentController {
    ...
    public boolean interceptCoverPlacement(byte aCoverSide, CoverData aData, Entity aPlayer) {
        return !(aData.mTileEntity instanceof ITileEntityRunningPossible
            || aData.mTileEntity instanceof ITileEntitySwitchableOnOff);
    }
    ...
    public long onToolClick(byte aSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_chisel) && sTexturesBase.length > 1) {
            aData.visual(
                aSide,
                (short) ((aData.mVisuals[aSide] & 1023)
                    | ((((aData.mVisuals[aSide] >>> 10) + 1) % sTexturesBase.length) << 10)));
            return 100;
        }
    ...
    public void onTickPost(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide,
        ...
        if (aIsServerSide) {
            ...
            if (aData.mTileEntity instanceof ITileEntityRunningPossible) {
                rVisuals |= B[5];
                if (((ITileEntityRunningPossible) aData.mTileEntity).getStateRunningPossible()) rVisuals |= B[0];
            }
            if (aData.mTileEntity instanceof ITileEntityRunningPassively) {
                rVisuals |= B[6];
                if (((ITileEntityRunningPassively) aData.mTileEntity).getStateRunningPassively()) rVisuals |= B[1];
            }
            if (aData.mTileEntity instanceof ITileEntityRunningActively) {
                rVisuals |= B[7];
                if (((ITileEntityRunningActively) aData.mTileEntity).getStateRunningActively()) rVisuals |= B[2];
            }
            if (aData.mTileEntity instanceof ITileEntitySwitchableOnOff) {
                rVisuals |= B[8];
                if (((ITileEntitySwitchableOnOff) aData.mTileEntity).getStateOnOff()) rVisuals |= B[3];
            }
    ...
    public boolean onCoverClickedRight(byte aSide, CoverData aData, Entity aPlayer, byte aSideClicked, float aHitX,
        ...
        if (aSide == aSideClicked && aData.mTileEntity instanceof ITileEntitySwitchableOnOff) {
            ...
            switch ((short) ((aData.mVisuals[aSide] >>> 10) % sTexturesBase.length)) {
                case 0:
                    if (tCoords[0] >= PX_N[6] && tCoords[1] >= PX_N[4]) {
                        if (aData.mTileEntity.isServerSide()) ((ITileEntitySwitchableOnOff) aData.mTileEntity)
                            .setStateOnOff(!((ITileEntitySwitchableOnOff) aData.mTileEntity).getStateOnOff());
                        return T;
                    }
                    break;
                case 1:
                    if (tCoords[0] >= PX_N[6] && tCoords[1] <= PX_P[4]) {
                        if (aData.mTileEntity.isServerSide()) ((ITileEntitySwitchableOnOff) aData.mTileEntity)
                            .setStateOnOff(!((ITileEntitySwitchableOnOff) aData.mTileEntity).getStateOnOff());
                        return T;
                    }
    ...
    public ITexture getCoverTextureSurface(byte aSide, CoverData aData) {
        short tVisuals = aData.mVisuals[aSide], tStyle = (short) ((tVisuals >>> 10) % sTexturesBase.length);
        return BlockTextureMulti.get(
            sTexturesBase[tStyle],
            (tVisuals & B[5]) != 0 ? sTextures[tStyle][(tVisuals & B[0]) != 0 ? 1 : 0] : null,
            (tVisuals & B[6]) != 0 ? sTextures[tStyle][(tVisuals & B[1]) != 0 ? 3 : 2] : null,
            (tVisuals & B[7]) != 0 ? sTextures[tStyle][(tVisuals & B[2]) != 0 ? 5 : 4] : null,
            (tVisuals & B[8]) != 0 ? sTextures[tStyle][(tVisuals & B[3]) != 0 ? 7 : 6] : null,
            (tVisuals & B[9]) != 0 ? sTextures[tStyle][(tVisuals & B[4]) != 0 ? 9 : 8] : null);
    }

    @Override
    public ITexture getCoverTextureAttachment(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide != aTextureSide ? BACKGROUND_COVER
            : BlockTextureMulti.get(BACKGROUND_COVER, getCoverTextureSurface(aSide, aData));
    }
    ...
    public static final ITexture[][] sTextures = new ITexture[][] {
        { BlockTextureDefault.get("machines/covers/statusdisplay/bottom/1_off", T),
            ...
            BlockTextureDefault.get("machines/covers/statusdisplay/bottom/4_on", F), null, null },
        { BlockTextureDefault.get("machines/covers/statusdisplay/top/1_off", T),
            ...
            BlockTextureDefault.get("machines/covers/statusdisplay/top/4_on", F), null, null } };
