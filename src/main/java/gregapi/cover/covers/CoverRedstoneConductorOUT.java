...
public class CoverRedstoneConductorOUT extends AbstractCoverAttachment {
    ...
    public void onBlockUpdate(byte aCoverSide, CoverData aData) {
        byte tEmitted = 0;
        for (byte tSide : ALL_SIDES_VALID) if (aData.mBehaviours[tSide] instanceof CoverRedstoneConductorIN)
        ...
        if (aData.mValues[aCoverSide] != tEmitted) {
            ...
            if (!GT_API_Proxy.DELAYED_BLOCK_UPDATES.contains(aData.mTileEntity))
                GT_API_Proxy.DELAYED_BLOCK_UPDATES.add(aData.mTileEntity);
        }
    }
