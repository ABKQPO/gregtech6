...
public class CoverRetrieverItem extends AbstractCoverAttachment {
    ...
    public void onTickPre(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide, boolean aReceivedBlockUpdate,
        ...
        if (aIsServerSide && !aData.mStopped
            && (SERVER_TIME % 20 == 15 || aData.mValues[aSide] != 0)
            && aData.mTileEntity instanceof ITileEntityItemPipe
            && ((ITileEntityItemPipe) aData.mTileEntity).pipeCapacityCheck()) {
            ...
            for (ITileEntityItemPipe tPipe : UT.Code
                .sortByValuesAcending(
                    ITileEntityItemPipe.Util.scanPipes(
                        (ITileEntityItemPipe) aData.mTileEntity,
                        new HashMap<ITileEntityItemPipe, Long>(),
                        0,
                        T,
                        F))
                .keySet()) {
                if (tUsedPipes.add(tPipe))
                    for (byte tSide : ALL_SIDES_VALID) if (tPipe.canAcceptItemsFrom(tSide, aData.mTileEntity)
                        && (tSide != aSide || tPipe != aData.mTileEntity)) {
                            DelegatorTileEntity<IInventory> tDelegator = tPipe.getAdjacentInventory(tSide);
                            if (!(tDelegator.mTileEntity instanceof ITileEntityItemPipe) && ST
                                .move(tDelegator, tTarget, tFilter, F, F, aData.mVisuals[aSide] != 0, T, 64, 1, 64, 1)
                                > 0) {
    ...
    public ITexture getCoverTextureAttachment(byte aCoverSide, CoverData aData, byte aTextureSide) {
        return ALONG_AXIS[aCoverSide][aTextureSide]
            ? BlockTextureMulti
                .get(BACKGROUND_COVER, aData.mVisuals[aCoverSide] == 0 ? sTextureNormal : sTextureInverted)
            : BACKGROUND_COVER;
    }
