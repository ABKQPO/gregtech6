...
public class CoverScaleProgress extends AbstractCoverAttachmentScale {
    ...
    public void onTickPost(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide,
        ...
        if (aIsServerSide && aData.mTileEntity instanceof ITileEntityProgress) {
            long tProgress = ((ITileEntityProgress) aData.mTileEntity).getProgressValue(aSide),
                tMax = ((ITileEntityProgress) aData.mTileEntity).getProgressMax(aSide);
            aData.value(
                aSide,
                UT.Code.bind4(
                    tProgress <= 0 || tMax <= 0 ? 0
                        : tProgress >= tMax ? 15
                            : 14 - (int) Math.max(0, Math.min(13, ((tMax - tProgress) * 14L) / tMax))),
                T);
        }
    }
    ...
    public static final ITexture sTextureForeground = BlockTextureDefault
        .get("machines/covers/progressredstone/circuit");
}
