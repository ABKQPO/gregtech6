...
public class CoverPressureValve extends AbstractCoverAttachment {
    ...
    public boolean interceptCoverPlacement(byte aCoverSide, CoverData aData, Entity aPlayer) {
        return !(aData.mTileEntity instanceof MultiTileEntityPipeFluid)
            || ((MultiTileEntityPipeFluid) aData.mTileEntity).mTanks.length != 1
            || aData.mTileEntity.getAdjacentTileEntity(aCoverSide).mTileEntity instanceof MultiTileEntityPipeFluid;
    }

    @Override
    public boolean interceptConnect(byte aCoverSide, CoverData aData) {
        return aData.mTileEntity.getAdjacentTileEntity(aCoverSide).mTileEntity instanceof MultiTileEntityPipeFluid;
    }
        ...
        if (aIsServerSide && !aData.mStopped && aTimer > 2 && aData.mTileEntity instanceof MultiTileEntityPipeFluid) {
            ...
            if (tTank.isFull()) {
                ...
                if (tDelegator.mTileEntity != null) {
                    ...
                    try {
                        for (Entity tEntity : (ArrayList<Entity>) aData.mTileEntity.getWorld()
                            .getEntitiesWithinAABB(Entity.class, aData.box(-2, -2, -2, +3, +3, +3)))
    ...
    public static final float[][] BOXES_VALVES = new float[][] {
        { PX_P[6], PX_P[0], PX_P[6], PX_N[6], PX_N[8], PX_N[6] },
        { PX_P[6], PX_P[8], PX_P[6], PX_N[6], PX_N[0], PX_N[6] },
        { PX_P[6], PX_P[6], PX_P[0], PX_N[6], PX_N[6], PX_N[8] },
        { PX_P[6], PX_P[6], PX_P[8], PX_N[6], PX_N[6], PX_N[0] },
        { PX_P[0], PX_P[6], PX_P[6], PX_N[8], PX_N[6], PX_N[6] },
        { PX_P[8], PX_P[6], PX_P[6], PX_N[0], PX_N[6], PX_N[6] } };
