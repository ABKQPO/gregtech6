...
public class CoverDisplayEnergy extends AbstractCoverAttachmentDisplay {
    ...
    public void onTickPost(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide,
        ...
        if (aIsServerSide && aData.mTileEntity instanceof ITileEntityEnergyDataCapacitor
            && !((ITileEntityEnergyDataCapacitor) aData.mTileEntity).getEnergyCapacitorTypes(aSide)
                .isEmpty()) {
            TagData tEnergyType = ((ITileEntityEnergyDataCapacitor) aData.mTileEntity).getEnergyCapacitorTypes(aSide)
                .iterator()
                .next();
            long tStored = ((ITileEntityEnergyDataCapacitor) aData.mTileEntity).getEnergyStored(tEnergyType, aSide),
                tCapacity = ((ITileEntityEnergyDataCapacitor) aData.mTileEntity).getEnergyCapacity(tEnergyType, aSide);
            aData.visual(
                aSide,
                (short) (tStored <= 0 || tCapacity <= 0 ? 0
                    : tStored >= tCapacity ? 10
                        : 9 - (int) Math.max(0, Math.min(8, ((tCapacity - tStored) * 9L) / tCapacity))));
        }
    }
