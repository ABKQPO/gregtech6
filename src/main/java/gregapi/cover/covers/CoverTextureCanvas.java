...
public class CoverTextureCanvas extends AbstractCoverDefault {
    ...
    public void onCoverPlaced(byte aSide, CoverData aData, Entity aPlayer, ItemStack aCover) {
        if (aCover != null && aCover.hasTagCompound()) aData.visual(
            aSide,
            (short) ((aCover.getTagCompound()
                .getInteger(NBT_CANVAS_BLOCK) << 4)
                | (aCover.getTagCompound()
                    .getInteger(NBT_CANVAS_META) & 15)));
    ...
    public void onAfterCrowbar(ITileEntityCoverable aTileEntity) {
        UT.Sounds.send(SFX.MC_DIG_CLOTH, 1.0F, -1.0F, aTileEntity);
    }
    ...
    public ITexture getCoverTextureSurface(byte aSide, CoverData aData) {
        return aData.mVisuals[aSide] == 0 ? null
            : BlockTextureCopied
                .get(Block.getBlockById((aData.mVisuals[aSide] >>> 4) & 4095), SIDE_ANY, aData.mVisuals[aSide] & 15);
    }

    @Override
    public ITexture getCoverTextureAttachment(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide != aTextureSide ? mTexture : BlockTextureMulti.get(mTexture, getCoverTextureSurface(aSide, aData));
    }
