...
public class CoverPump extends AbstractCoverAttachment {
    ...
    public long onToolClick(byte aSide, CoverData aData, String aTool, long aRemainingDurability, long aQuality,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            aData.visual(
                aSide,
                (short) (aData.mVisuals[aSide] == 0 || aData.mTileEntity instanceof MultiTileEntityPipeFluid ? 1 : 0));
            return 1000;
        }
        return 0;
    }
    ...
    public void onTickPre(byte aSide, CoverData aData, long aTimer, boolean aIsServerSide, boolean aReceivedBlockUpdate,
        ...
        if (aIsServerSide && !aData.mStopped && SERVER_TIME % 20 == 5 && aData.mTileEntity instanceof IFluidHandler) {
            ...
            if (aData.mVisuals[aSide] == 0) {
                while (tMoved > 0 && tThroughput > 0) tThroughput -= (tMoved = FL
                    .move(aData.delegator(aSide), aData.mTileEntity.getAdjacentTank(aSide), tThroughput));
            } else {
                while (tMoved > 0 && tThroughput > 0) tThroughput -= (tMoved = FL
                    .move(aData.mTileEntity.getAdjacentTank(aSide), aData.delegator(aSide), tThroughput));
            }
        }
    }
    ...
    public ITexture getCoverTextureAttachment(byte aSide, CoverData aData, byte aTextureSide) {
        return aSide == aTextureSide
            ? BlockTextureMulti.get(BACKGROUND_COVER, aData.mVisuals[aSide] == 0 ? sTextureOut : sTextureIn)
            : aSide == OPOS[aTextureSide]
                ? BlockTextureMulti.get(BACKGROUND_COVER, aData.mVisuals[aSide] != 0 ? sTextureOut : sTextureIn)
                : BACKGROUND_COVER;
    }
