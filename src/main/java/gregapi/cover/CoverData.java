...
public class CoverData {
    ...
    public CoverData(ITileEntityCoverable aTileEntity) {
        ...
        mValues = new short[6];
        mNBTs = new NBTTagCompound[6];
        mTileEntity = aTileEntity;
    }

    public CoverData(short[] aIDs, short[] aMetas, short[] aVisuals, short[] aValues, NBTTagCompound[] aNBTs,
        boolean aStopped, ITileEntityCoverable aTileEntity) {
        mVisuals = aVisuals;
        mValues = aValues;
        mNBTs = aNBTs;
        ...
        mStopped = aStopped;
        mTileEntity = aTileEntity;
        if (mTileEntity != null) try {
            for (byte tSide : ALL_SIDES_VALID)
                if (mBehaviours[tSide] != null) mBehaviours[tSide].onCoverLoaded(tSide, this);
        } catch (Throwable e) {
            e.printStackTrace(ERR);
            mTileEntity.setError("Cover Loaded:" + e);
        }
    }

    public CoverData(ITileEntityCoverable aTileEntity, NBTTagCompound aNBT) {
        this(
            new short[] { aNBT.getShort("a"), aNBT.getShort("b"), aNBT.getShort("c"), aNBT.getShort("d"),
                aNBT.getShort("e"), aNBT.getShort("f") },
            new short[] { aNBT.getShort("g"), aNBT.getShort("h"), aNBT.getShort("i"), aNBT.getShort("j"),
                aNBT.getShort("k"), aNBT.getShort("l") },
            new short[] { aNBT.getShort("m"), aNBT.getShort("n"), aNBT.getShort("o"), aNBT.getShort("p"),
                aNBT.getShort("q"), aNBT.getShort("r") },
            new short[] { aNBT.getShort("0"), aNBT.getShort("1"), aNBT.getShort("2"), aNBT.getShort("3"),
                aNBT.getShort("4"), aNBT.getShort("5") },
            new NBTTagCompound[] { aNBT.getCompoundTag("s"), aNBT.getCompoundTag("t"), aNBT.getCompoundTag("u"),
                aNBT.getCompoundTag("v"), aNBT.getCompoundTag("w"), aNBT.getCompoundTag("x") },
            aNBT.getBoolean("y"),
            aTileEntity);
    }

    public NBTTagCompound writeToNBT() {
        return writeToNBT(UT.NBT.make(), T);
    }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound aNBT, boolean aIncludeVisuals) {
        ...
        if (mIDs[i] != 0) {
            ...
            if (mVisuals[i] != 0
                && (aIncludeVisuals || (mBehaviours[i] != null && mBehaviours[i].needsVisualsSaved(i, this))))
                aNBT.setShort("m", mVisuals[i]);
        }
        if (mIDs[++i] != 0) {
    ...
    public CoverData setIDs(short[] aIDs, short[] aMetas) {
        mIDs = aIDs;
        mMetas = aMetas;
        for (byte tSide : ALL_SIDES_VALID) if (mIDs[tSide] == 0) mBehaviours[tSide] = null;
        else mBehaviours[tSide] = CoverRegistry.get(mIDs[tSide], mMetas[tSide]);
        return this;
    }

    public CoverData set(byte aSide, ItemStack aStack) {
        return aStack == null ? set(aSide, (short) 0, (short) 0, null)
            : set(aSide, ST.id(aStack), ST.meta_(aStack), aStack.getTagCompound());
    }

    public CoverData set(byte aSide, short aID, short aMeta, NBTTagCompound aNBT) {
        mIDs[aSide] = aID;
        mMetas[aSide] = aMeta;
        if (aID == 0) mBehaviours[aSide] = null;
        else mBehaviours[aSide] = CoverRegistry.get(aID, aMeta);
    ...
    public boolean setStopped(boolean aStopped) {
        ...
        mStopped = aStopped;
        for (byte tSide : ALL_SIDES_VALID)
            if (mBehaviours[tSide] != null) mBehaviours[tSide].onStoppedUpdate(tSide, this, mStopped);
        return T;
    }

    public boolean onBlockUpdate() {
        for (byte tSide : ALL_SIDES_VALID)
            if (mBehaviours[tSide] != null) mBehaviours[tSide].onBlockUpdate(tSide, this);
        return T;
    }

    public ItemStack getCoverItem(byte aSide) {
        return mIDs[aSide] == 0 ? null
            : mBehaviours[aSide] == null
                ? ST.make(
                    mIDs[aSide],
                    1,
                    mMetas[aSide],
                    mNBTs[aSide] == null || mNBTs[aSide].hasNoTags() ? null : mNBTs[aSide])
                : mBehaviours[aSide].getCoverItem(aSide, this);
    }
    ...
    public void tickPre(long aTimer, boolean aIsServerSide, boolean aReceivedBlockUpdate,
        boolean aReceivedInventoryUpdate) {
        try {
            for (byte tSide : ALL_SIDES_VALID) if (mBehaviours[tSide] != null) mBehaviours[tSide]
                .onTickPre(tSide, this, aTimer, aIsServerSide, aReceivedBlockUpdate, aReceivedInventoryUpdate);
        } catch (Throwable e) {
            e.printStackTrace(ERR);
            mTileEntity.setError("Cover Pre Tick - " + (aIsServerSide ? "Serverside: " : "Clientside: ") + e);
        }
    }

    public void tickPost(long aTimer, boolean aIsServerSide, boolean aReceivedBlockUpdate,
        boolean aReceivedInventoryUpdate) {
        try {
            for (byte tSide : ALL_SIDES_VALID) if (mBehaviours[tSide] != null) mBehaviours[tSide]
                .onTickPost(tSide, this, aTimer, aIsServerSide, aReceivedBlockUpdate, aReceivedInventoryUpdate);
        } catch (Throwable e) {
            e.printStackTrace(ERR);
            mTileEntity.setError("Cover Post Tick - " + (aIsServerSide ? "Serverside: " : "Clientside: ") + e);
        }
    }

    public AxisAlignedBB box(double aMinX, double aMinY, double aMinZ, double aMaxX, double aMaxY, double aMaxZ) {
        return AxisAlignedBB.getBoundingBox(
            mTileEntity.getX() + aMinX,
            mTileEntity.getY() + aMinY,
            mTileEntity.getZ() + aMinZ,
            mTileEntity.getX() + aMaxX,
            mTileEntity.getY() + aMaxY,
            mTileEntity.getZ() + aMaxZ);
    }

    public AxisAlignedBB box(double[] aBox) {
        return AxisAlignedBB.getBoundingBox(
            mTileEntity.getX() + aBox[0],
            mTileEntity.getY() + aBox[1],
            mTileEntity.getZ() + aBox[2],
            mTileEntity.getX() + aBox[3],
            mTileEntity.getY() + aBox[4],
            mTileEntity.getZ() + aBox[5]);
    }
