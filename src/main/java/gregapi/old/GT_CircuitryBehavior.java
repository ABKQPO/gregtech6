...
public abstract class GT_CircuitryBehavior {
     ...
     */
    public static final boolean getAnyRedstone(
        IRedstoneCircuitBlock aRedstoneCircuitBlock) {/*
                                                       * for (byte i = 0; i < 6; i++) {
                                                       * if (i != aRedstoneCircuitBlock.getOutputFacing() &&
                                                       * aRedstoneCircuitBlock.getCover(i).letsRedstoneGoIn(i,
     ...
     */
    public static final boolean getAllRedstone(
        IRedstoneCircuitBlock aRedstoneCircuitBlock) {/*
                                                       * for (byte i = 0; i < 6; i++) {
                                                       * if (i != aRedstoneCircuitBlock.getOutputFacing() &&
                                                       * aRedstoneCircuitBlock.getCover(i).letsRedstoneGoIn(i,
    ...
    public static final boolean getOneRedstone(IRedstoneCircuitBlock aRedstoneCircuitBlock) {
        int tRedstoneAmount = 0;/*
                                 ...
                                 * if (i != aRedstoneCircuitBlock.getOutputFacing() &&
                                 * aRedstoneCircuitBlock.getCover(i).letsRedstoneGoIn(i,
                                 * aRedstoneCircuitBlock.getCoverID(i), aRedstoneCircuitBlock.getCoverVariable(i),
                                 * aRedstoneCircuitBlock.getOwnTileEntity())) {
                                 * if (aRedstoneCircuitBlock.getInputRedstone(i) > 0) {
                                 * tRedstoneAmount++;
                                 * }
    ...
    public static final byte getStrongestRedstone(IRedstoneCircuitBlock aRedstoneCircuitBlock) {
        byte tRedstoneAmount = 0;/*
                                  ...
                                  * aRedstoneCircuitBlock.getCoverID(i), aRedstoneCircuitBlock.getCoverVariable(i),
                                  * aRedstoneCircuitBlock.getOwnTileEntity())) {
                                  * tRedstoneAmount = (byte)Math.max(tRedstoneAmount,
                                  * aRedstoneCircuitBlock.getInputRedstone(i));
                                  * }
                                  * }
                                  */
    ...
    public static final byte getWeakestNonZeroRedstone(IRedstoneCircuitBlock aRedstoneCircuitBlock) {
        ...
        byte tRedstoneAmount = 15;/*
                                   ...
                                   * aRedstoneCircuitBlock.getOwnTileEntity())) {
                                   * if (aRedstoneCircuitBlock.getInputRedstone(i) > 0)
                                   * tRedstoneAmount = (byte)Math.min(tRedstoneAmount,
                                   * aRedstoneCircuitBlock.getInputRedstone(i));
                                   * }
                                   * }
                                   */
