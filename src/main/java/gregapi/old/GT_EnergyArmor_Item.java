...
public class GT_EnergyArmor_Item extends ItemArmor /* implements ISpecialArmor */ {
    ...
    @Override
    public void onArmorTick(World aWorld, EntityPlayer aPlayer,
        ItemStack aStack) {/*
                            ...
                            * aPlayer.isPotionActive(Potion.poison)) {
                            * UT.Entities.removePotion(aPlayer, Potion.poison.id);
                            * GT_ModHandler.useElectricItem(aStack, 10000, aPlayer);
                            * }
                            * if (GT_ModHandler.canUseElectricItem(aStack, 100000) &&
                            * aPlayer.isPotionActive(Potion.wither)) {
                            ...
                            * float var6 = jumpChargeMap.containsKey(aPlayer) ?
                            * ((Float)jumpChargeMap.get(aPlayer)).floatValue() : 1.0F;
                            ...
                            * (aPlayer.onGround && Math.abs(aPlayer.motionX) + Math.abs(aPlayer.motionZ) >
                            * 0.10000000149011612D || aPlayer.isInWater())) {
                            ...
                            * if (GT_ModHandler.chargeElectricItem(tTargetChargeItem, 1, Integer.MAX_VALUE, true, true)
                            * < 1) {
                            * tTargetChargeItem = aPlayer.inventory.armorInventory[2];
                            * }
                            * if (GT_ModHandler.dischargeElectricItem(tTargetDechargeItem, 10, Integer.MAX_VALUE, true,
                            * true, true) < 10) {
                            ...
                            * }
                            * if (tTargetDechargeItem == null || !GT_ModHandler.isElectricItem(tTargetChargeItem) ||
                            * !(aStack == tTargetDechargeItem || GT_ModHandler.isChargerItem(tTargetDechargeItem))) {
                            * tTargetDechargeItem = null;
                            * }
                            * if (aPlayer.worldObj.isDaytime() &&
                            * aPlayer.worldObj.canBlockSeeTheSky(MathHelper.floor_double(aPlayer.posX),
                            * MathHelper.floor_double(aPlayer.posY+1), MathHelper.floor_double(aPlayer.posZ))) {
                            ...
                            * }
                            * } else {
                            * if ((mSpecials & 16) != 0 && tTargetDechargeItem != null &&
                            * GT_ModHandler.canUseElectricItem(tTargetDechargeItem, 10)) {
                            * if (aPlayer.worldObj.getBlock ((int)aPlayer.posX, (int)aPlayer.posY+1, (int)aPlayer.posZ)
                            * == Blocks.air)
                            * aPlayer.worldObj.setBlock ((int)aPlayer.posX, (int)aPlayer.posY+1, (int)aPlayer.posZ,
                            * GregTech_API.sBlockList[3]);
    ...
    // @ForgeSubscribe
    public void onEntityLivingFallEvent(
        LivingFallEvent var1) {/*
                                ...
                                * if (var5 <= GT_ModHandler.dischargeElectricItem(var3, Integer.MAX_VALUE,
    ...
    /*
     * @Override
     * public ISpecialArmor.ArmorProperties getProperties(EntityLivingBase var1, ItemStack var2, DamageSource var3,
     * double var4, int var6) {
     * return new ISpecialArmor.ArmorProperties((var3 == DamageSource.fall && (mSpecials & 2) != 0)?10:0,
     * getBaseAbsorptionRatio() * mArmorAbsorbtionPercentage, mDamageEnergyCost > 0 ? 25 *
     * GT_ModHandler.dischargeElectricItem(var2, Integer.MAX_VALUE, Integer.MAX_VALUE, true, true, true) /
     * mDamageEnergyCost : 0);
     * }
     * @Override
