...
public class GT_BaseCrop extends CropCard {
     ...
     */
    public GT_BaseCrop(String aCropName, String aDiscoveredBy, ItemStack aDrop, ItemStack[] aSpecialDrops,
        ItemStack aBaseSeed, int aTier, int aMaxSize, int aGrowthSpeed, int aAfterHarvestSize, int aHarvestSize,
        int aStatChemical, int aStatFood, int aStatDefensive, int aStatColor, int aStatWeed, String[] aAttributes) {
        mName = aCropName.toLowerCase()
            .replaceAll(" ", "");
    ...
    public ItemStack getGain(ICropTile aCrop) {
        int tDrop = 0;
        if (mSpecialDrops != null && (tDrop = RNGSUS.nextInt(mSpecialDrops.length + 4)) < mSpecialDrops.length
    ...
    public void registerSprites(IIconRegister iconRegister) {
        ...
        for (int i = 1; i <= textures.length; i++)
            textures[i - 1] = iconRegister.registerIcon(RES_PATH_BLOCK + "crop/" + name() + "/" + i);
    }
}
