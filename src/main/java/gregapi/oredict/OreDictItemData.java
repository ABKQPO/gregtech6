...
public class OreDictItemData {
    ...
    /**
     * The OreDictMaterialStack containing the remaining Byproduct Materials of this Item. The Amount is in Material
     * Units (U).
     */
    ...
    public OreDictItemData(OreDictMaterialStack aMaterial, OreDictMaterialStack... aByProducts) {
        ...
        if (aByProducts == null) {
            ...
            OreDictMaterialStack[] tByProducts = aByProducts.length < 1 ? ZL_MS
                : new OreDictMaterialStack[aByProducts.length];
            int j = 0;
            for (int i = 0; i < aByProducts.length; i++)
                if (aByProducts[i] != null) tByProducts[j++] = aByProducts[i].clone();
    ...
    public OreDictItemData(OreDictMaterial aMaterial, OreDictPrefix aAmount, OreDictMaterial aByProduct,
        OreDictPrefix aByProductAmount) {
    ...
    public OreDictItemData(OreDictMaterial aMaterial, long aAmount, OreDictMaterial aByProduct,
        OreDictPrefix aByProductAmount) {
        this(OM.stack(aMaterial, aAmount), OM.stack(aByProduct, aByProductAmount));
    }
    ...
    public OreDictItemData(OreDictMaterial aMaterial, long aAmount, OreDictMaterial aByProduct, long aByProductAmount,
        OreDictMaterial aByProduct2, long aByProductAmount2) {
        this(
            OM.stack(aMaterial, aAmount),
            OM.stack(aByProduct, aByProductAmount),
            OM.stack(aByProduct2, aByProductAmount2));
    }
    ...
    public OreDictItemData(OreDictItemData... aData) {
        ...
        for (OreDictItemData tData : aData) if (tData != null) {
            if (tData.validMaterial() && tData.mMaterial.mAmount > 0)
                aList.add(OM.stack(tData.mMaterial.mMaterial.mTargetReversing, tData.mMaterial.mAmount));
            for (OreDictMaterialStack tMaterial : tData.mByProducts)
                if (tMaterial.mAmount > 0) aList.add(OM.stack(tMaterial.mMaterial.mTargetReversing, tMaterial.mAmount));
        }
    ...
    public boolean fullData() {
        return validPrefix() && fullMaterial();
    }

    public boolean listedData() {
        return validPrefix() && listedMaterial();
    }

    public boolean nonemptyData() {
        return validPrefix() && nonemptyMaterial();
    }

    public boolean validData() {
        return validPrefix() && validMaterial();
    }

    public boolean fullMaterial() {
        return nonemptyMaterial() && mMaterial.mAmount > 0;
    }

    public boolean listedMaterial() {
        return validMaterial() && mMaterial.mMaterial.mID >= 0;
    }// If it has a positive ID then it is in the List.

    public boolean nonemptyMaterial() {
        return validMaterial() && mMaterial.mMaterial.mID > 0;
    }// 0 happens to be the "Empty" Material
    ...
    /**
     * Utility Function for getting a List containing both, the Main Material and all the Byproduct Materials. The
     * Amount is in Material Units (U).
     */
    ...
    /**
     * Utility Function for getting a Byproduct Material at a certain Index, if it exists. The Amount is in Material
     * Units (U).
     */
    ...
    public ItemStack getStack(long aAmount) {
        return mUnificationTarget == null ? mPrefix.mat(mMaterial.mMaterial, aAmount)
            : ST.amount(aAmount, mUnificationTarget);
    }

    public OreDictItemData copy() {
        OreDictItemData rData = mPrefix == null ? new OreDictItemData(mMaterial, mByProducts)
            : new OreDictItemData(mPrefix, mMaterial.mMaterial);
