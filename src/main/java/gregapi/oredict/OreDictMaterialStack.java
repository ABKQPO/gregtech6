...
public final class OreDictMaterialStack implements Cloneable {
    ...
    public boolean equals(Object aObject) {
        ...
        if (aObject instanceof OreDictMaterialStack) return ((OreDictMaterialStack) aObject).mMaterial == mMaterial
            && (mAmount < 0 || ((OreDictMaterialStack) aObject).mAmount < 0
                || ((OreDictMaterialStack) aObject).mAmount == mAmount);
        return F;
    }
    ...
    public List<OreDictMaterialStack> addToList(List<OreDictMaterialStack> aList) {
        ...
        for (OreDictMaterialStack tMaterial : aList) if (tMaterial.mMaterial == mMaterial) {
            tMaterial.mAmount += mAmount;
            return aList;
        }
        aList.add(clone());
        return aList;
    ...
    public static OreDictMaterialStack load(NBTTagCompound aNBT) {
        if (aNBT.hasKey("i"))
            return new OreDictMaterialStack(OreDictMaterial.MATERIAL_ARRAY[aNBT.getShort("i")], aNBT.getLong("a"));
