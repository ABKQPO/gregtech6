 ...
 * 
 *         This Class is for filtering out the Private and Invalid OreDictionary Events and passing the proper ones to
 *         any interested User.
...
public final class OreDictManager {
    ...
    /**
     * Assign Item Data to certain OreDict Strings for recycling and similar. Used for plankWood and slabWood for
     * example.
     */
    ...
    private final Set<String> mKnownNames = new HashSetNoNulls<>(
        F,
        ...
        "prbackpack"
        // , "stonebrick", "stoneAndesiteBricks", "stoneDioriteChiseled", "stoneAndesiteChiseled",
        // "stoneDioritePolished", "stoneAndesitePolished", "stoneDioriteBricks", "stoneBasaltChiseled",
        // "stoneGraniteChiseled", "stoneBasaltBricks", "stoneBasaltPolished", "stoneGraniteBricks",
        // "stoneGranitePolished"
        // , "stoneBrown", "stonePink", "stoneGreen", "stoneWhite", "stoneMagenta", "stoneBlue", "stoneOrange",
        // "stonePurple", "stoneGray", "stoneYellow", "stoneLime", "stoneCyan", "stoneLightBlue", "stoneLightGray"
        ,
        "diamondNugget",
        "UUMatter",
        "gaiaIngot",
    ...
    private final Set<String> mIgnoredNames = new HashSetNoNulls<>(
        F,
        "rawMeat",
        "cookedMeat",
        "rawRubber",
        ...
        "halfSheetCylinderMetal",
        "halfSheetMetal",
        "quarterSheetMetal",
        "sheetCurvedFourMetal",
        "sheetCurvedOneMetal",
        "sheetCurvedThreeMetal",
        "sheetCurvedTwoMetal",
        "sheetCylinderMetal",
        "sheetMediumConeMetal",
        "sheetMetal",
        "sheetMicroConeMetal",
        "sheetMicroFinMetal",
        "sheetSmallConeMetal",
        "sheetSmallCylinderMetal",
        "sheetSmallFinMetal",
        "shirtSheetMetal",
        "rivetSheetMetal",
        "triangleSheetMetal");
    /**
     * Lists all Names which have already been registered. Used for the OreDict Listeners, so that certain Recipes (such
     * as Crafting) don't get registered twice.
     */
    ...
    private OreDictManager() {
        ...
        for (String tOreName : OreDictionary.getOreNames())
            for (ItemStack tOreStack : OreDictionary.getOres(tOreName, F))
                onOreRegistration1(new OreRegisterEvent(tOreName, tOreStack));
        for (FluidContainerData tData : FluidContainerRegistry.getRegisteredFluidContainerData())
            onFluidContainerRegistration(new FluidContainerRegisterEvent(tData));
     ...
     * Unlike the Forge Event System, which replaced Eloraams better System, this Interface ensures, like the great old
     * System, that also all the older Events, which have been called before your Listener-Registration, are getting
     * passed onto your Listener.
     */
    public void addListener(IOreDictListenerEvent aListener) {
        if (GAPI.mStartedPostInit) addListenerInternal(aListener);
        else mBufferedListeners1.add(aListener);
    }
    ...
    private void addListenerInternal(IOreDictListenerEvent aListener) {
        if (mGlobalOreDictListeners.add(aListener))
            for (OreDictRegistrationContainer tEvent : mGlobalRegistrations) aListener.onOreRegistration(tEvent);
    }

    /**
     * This Function adds a global Listener for Recycling Events. These Events let you create Reversal Recipes inside
     * your Machines, and contain all the Components, which can be recycled out of the passed ItemStack.
     * Unlike the Forge Event System, which replaced Eloraams better System, this Interface ensures, like the great old
     * System, that also all the older Events, which have been called before your Listener-Registration, are getting
     * passed onto your Listener.
     */
    public void addListener(IOreDictListenerRecyclable aListener) {
        if (GAPI.mStartedPostInit) addListenerInternal(aListener);
        else mBufferedListeners2.add(aListener);
    }
    ...
    private void addListenerInternal(IOreDictListenerRecyclable aListener) {
        if (mRecyclableOreDictListeners.add(aListener))
            for (OreDictRecyclingContainer tEvent : mRecyclableRegistrations)
                aListener.onRecycleableRegistration(new OreDictRecyclingContainer(tEvent));
    }
    ...
    public boolean addReRegistrationWithReversal(Object aOreToReRegister, Object aAdditionalName) {
        // Why that Variable you say? Because the OR would kill the second call in its attempt to optimise the return
        // value, and I don't want that second call to be killed.
    ...
    public boolean addAutoBlackListingForMod(Object aAutoBlackListedMod) {
        mAutoBlackListedMods.add(
            aAutoBlackListedMod.toString()
                .toLowerCase());
        mAutoBlackListedMods.add(
            aAutoBlackListedMod.toString()
                .toUpperCase());
    ...
    public boolean addReRegistration(Object aOreToReRegister, Object aAdditionalName) {
        ...
        if (aOreToReRegister.toString()
            .equals(aAdditionalName.toString())) return F;
        ...
        if (tNames.add(aAdditionalName.toString())) {
            // You thought I would not have this Line, to make it possible to add Re-Registrations for already
            // registered things, didn't you?
            for (ItemStack tOreStack : OreDictionary.getOres(aOreToReRegister.toString(), F))
                registerOreSafe(aAdditionalName, tOreStack);
            return T;
        }
        return F;
    }
    ...
    public boolean addVisibilityTrigger(Object aName, OreDictMaterial aMaterial) {
        ...
        if (tNames.add(aMaterial)) {
            for (ItemStack tOreStack : OreDictionary.getOres(aName.toString(), F))
                if (!ST.isGT(tOreStack)) aMaterial.mHidden = F;
            return T;
        }
        return F;
    }
    ...
    public void enableRegistrations() {
        if (GAPI.mStartedInit) {
            ...
            // Yep, I used a Sampler to see why the hell this part lags so much, figures Mariculture eats most of the
            // CPU in that.
            UT.LoadingBar.start(
                MD.MaCu.mLoaded ? "(Lags because Mariculture) OreDict" : "OreDict",
                tBufferedRegistrations.size());
    ...
    public void onFluidContainerRegistration(FluidContainerRegisterEvent aFluidEvent) {
        if (aFluidEvent.data.filledContainer.getItem() == Items.potionitem
            && ST.meta_(aFluidEvent.data.filledContainer) == 0) aFluidEvent.data.fluid.amount = 0;
    ...
    public void onOreRegistration1(OreRegisterEvent aEvent) {
        ...
        if (aEvent.Ore == null) {
            ERR.println(
                "ERROR: A NULL STACK from the Mod '" + aModID
                    + "' has been registered to the OreDict as: "
                    + aEvent.Name);
            return;
        }
        // I am very sure the OreDict actually checks for these cases, so I do not think this will ever trigger.
        if (aEvent.Ore.getItem() == null) {
            ERR.println(
                "ERROR: A NULL ITEM from the Mod '" + aModID
                    + "' has been registered to the OreDict as: "
                    + aEvent.Name);
            return;
        }
        ...
        // Yeah this definitely can happen, and I want to see it if any Mod fucks that one up, so I can potentially fix
        // that..
        if (UT.Code.stringInvalid(aRegName)) {
            ERR.println(
                "ERROR: " + aEvent.Ore.getItem()
                    .getClass()
                    + " from the Mod '"
                    + aModID
                    + "' has been registered to the OreDict before being registered as an Item/Block with: "
                    + aEvent.Name);
            return;
        }
        ...
        if (GT != null && isRegisteringOre != 1) {
            ...
            if (MD.TE_FOUNDATION == aMod && MD.TE_FOUNDATION.owns(aRegName, "material")
                && UT.Code.inside(1024, 1029, ST.meta_(aEvent.Ore))) return;
            // In order to fix a ThaumCraft Bug I have to ignore this registration under all circumstances. I registered
            // it under the proper Name manually.
            ...
            // This Red/Redstone Alloy is violating two OreDict Materials at the same time with its Name and
            // Composition, so I'm gonna keep it out of my System. And some other things need to be ignored too.
            if (MD.HBM.owns(aRegName) && (tLowerCase.endsWith("desh") || tLowerCase.endsWith("redalloy")
                || tLowerCase.endsWith("redstonealloy")
                || tLowerCase.startsWith("platedense"))) return;
            // Ignore Congealed Slime being registered as Rubber. TODO Whenever I decide to add Materials for Slime,
            // this needs to be revisited.
            if (MD.SC2.owns(aRegName) && tLowerCase.endsWith("rubber")) return;
            // OreDictPrefix Conflict caused by Galacticraft fixing its OreDict Registrations a little bit late to use
            // Plates instead of Compressed Stuff now.
            // Note: This can be removed in later MC Versions too, since Galacticraft either does not update or since it
            // has already fixed itself by now.
            ...
            // I had to do this instead of convincing Reika, because it is only a GT balance Issue, that wouldn't exist
            // without GT.
            // Basically those two things are outputted 4 times too much (1 Plank = 1 Pulp and not 4) and therefore
            // would be small Piles of Dust and not regular ones.
            if (MD.RoC.owns(aRegName)
                && (tLowerCase.equals("pulpwood") || tLowerCase.equals("dustwood") || tLowerCase.equals("dustwheat"))) {
                ItemStack tTargetStack = null, tFoundStack = null;
                // This iteration works btw only because RotaryCraft registers Stuff in PostInit instead of PreInit like
                // it is supposed to.
                for (ItemStack tStack : OreDictionary.getOres(aEvent.Name, F))
                    if (ST.equal(tStack, aEvent.Ore)) tFoundStack = tStack;
                    else tTargetStack = tStack;
        ...
        // Fixing Thaumcraft checking for the wrong OreDict when chopping Wood with Golems. Oh and it doesn't check
        // Wildcard either, so I'm gonna need to split that too.
        // Also there is a huge Issue within Thaumcraft itself that makes the whole OreDict check impossible, I fixed
        // that in CompatTC.
        if (aEvent.Name.startsWith("log") && ST.block(aEvent.Ore) != NB) if (ST.meta_(aEvent.Ore) == W)
            for (int i = 0; i < 16; i++) registerOreSafe("woodLog", ST.copyMeta(i, aEvent.Ore));
        else registerOreSafe("woodLog", aEvent.Ore);
        ...
        if (!ST.isGT(aEvent.Ore) && aMod != MD.EtFu) {
            ...
            // Et Futurum Requiem adds Deepslate Ores and Ore Blocks for Compat, those should not count for this.
            // Luckily its native Materials are never hidden.
        ...
        if (aEvent.Name.contains(" ")) {
            ...
            if (!(mIgnoredNames.contains(aEvent.Name) || aEvent.Name.contains("|")
    ...
    public void onOreRegistration2(String aModID, ModData aMod, String aRegName, OreRegisterEvent aEvent) {
        ...
        if (aEvent.Ore.getItem() instanceof IOreDictOptimizedParsingItem) {
            ...
            if (aPrefix == null || aMaterial == null
                || !aEvent.Name.equals(aPrefix.mNameInternal + aMaterial.mNameInternal)) {
                aPrefix = null;
                aMaterial = null;
            }
        }

        if (mAutoBlackListedNames.contains(aEvent.Name) || mAutoBlackListedMods.contains(aModID))
            addToBlacklist_(aEvent.Ore);
        ...
        if (aPrefix == null) {
            ...
            if (aPrefix == OP.nugget)
                registerOreSafe(aEvent.Name.replaceFirst(aPrefix.mNameInternal, OP.tiny.mNameInternal), aEvent.Ore);
            if (aPrefix != aPrefix.mTargetRegistration) {
                registerOreSafe(
                    aEvent.Name.replaceFirst(aPrefix.mNameInternal, aPrefix.mTargetRegistration.mNameInternal),
                    aEvent.Ore);
                return;
            }
            ...
            if (tName.length() > 0) {
                // This is a vanilla Forge Rule. I just make sure it gets applied. All Dyes no matter of which Colour
                // have to be registered under the Name "dye" too.
                ...
                if (aPrefix == OP.ore || aPrefix.contains(TD.Prefix.MATERIAL_BASED)) {
                    ...
                    if (aMaterial.contains(TD.Properties.AUTO_MATERIAL) && addKnownName(aEvent.Name))
                        mUnknownMaterials.add(aEvent.Name);
                    if (aMaterial != aMaterial.mTargetRegistration) registerOreSafe(
                        aPrefix.mNameInternal + aMaterial.mTargetRegistration.mNameInternal,
                        aEvent.Ore);
                    ...
                    for (OreDictMaterial tReRegisteredMaterial : aMaterial.mReRegistrations)
                        registerOreSafe(aPrefix.mNameInternal + tReRegisteredMaterial.mNameInternal, aEvent.Ore);
                    if (!aMaterial.contains(TD.Properties.INVALID_MATERIAL)) {
                        if (aPrefix == OP.rockGt && aMaterial.contains(TD.Properties.STONE))
                            registerOreSafe(OD.itemRock, aEvent.Ore);
                        if (aPrefix != OP.ore && aPrefix.contains(TD.Prefix.STANDARD_ORE)
                            && aMaterial.contains(TD.Properties.COMMON_ORE))
                        ...
                        } else if (aPrefix == OP.gem && aMod == MD.RH
                            && (!MD.ReC.mLoaded || aMaterial == MT.FluoriteBlack
                                || !ANY.CaF2.mToThis.contains(aMaterial))) {
                                    setTarget_(aPrefix, aMaterial, aEvent.Ore, T, T);
                                } else
                            if (aPrefix == OP.billet && aMod == MD.HBM) {
                                setTarget_(aPrefix, aMaterial, aEvent.Ore, T, T);
                            } else if (aPrefix == OP.ore) {
                                addItemData_(aEvent.Ore, aPrefix.dat(aMaterial));
                            } else if (aPrefix == OP.plateSteamcraft) {
                                // Skip these, the only valid ones are the ones from Flaxbeard's Steam Power itself, and
                                // I manually added those to Unification.
        ...
        OreDictRegistrationContainer tRegistration = new OreDictRegistrationContainer(
            aPrefix,
            aMaterial,
            aEvent.Name,
            aEvent.Ore,
            ...
            aModID,
            aRegName,
            aNotAlreadyRegisteredName);

        // Global Listeners. Those are usually direct Name->Recipe Systems, meaning they should have priority over
        // Prefix based Stuff.
    ...
    public boolean setTarget(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ModData aMod, Object aName, long aMeta,
        ...
        if (aMod.mLoaded && aMod2.mLoaded && aStack == null) ERR
            .println("Item does not exist for Unification Target despite Mod being loaded: " + aMod.mID + ":" + aName);
        return setTarget(aPrefix, aMaterial, aStack, T, F, T);
    }

    public boolean setTarget(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ModData aMod, Object aName, long aMeta) {
        ItemStack aStack = ST.make(aMod, aName.toString(), 1, aMeta);
        if (aMod.mLoaded && aStack == null) ERR
            .println("Item does not exist for Unification Target despite Mod being loaded: " + aMod.mID + ":" + aName);
        return setTarget(aPrefix, aMaterial, aStack, T, F, T);
    }
    ...
    public boolean setTarget(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, boolean aOverwrite,
        boolean aAlreadyRegistered) {
        return setTarget(aPrefix, aMaterial, aStack, aOverwrite, aAlreadyRegistered, T);
    }

    public boolean setTarget_(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, boolean aOverwrite,
        boolean aAlreadyRegistered) {
        return setTarget_(aPrefix, aMaterial, aStack, aOverwrite, aAlreadyRegistered, T);
    }

    public boolean setTarget(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, boolean aOverwrite,
        boolean aAlreadyRegistered, boolean aIgnoreBlacklist) {
    ...
    public boolean setTarget_(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, boolean aOverwrite,
        ...
        if ((aIgnoreBlacklist || !isBlacklisted(aStack))
            && (aOverwrite || ST.invalid(sName2StackMap.get(aPrefix.mNameInternal + aMaterial.mNameInternal))))
            sName2StackMap.put(aPrefix.mNameInternal + aMaterial.mNameInternal, aStack);
        isAddingOre--;
        return T;
    }
    ...
    public ItemStack getStack(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aReplacement, long aAmount) {
        if (GAPI.mStartedInit)
            return getStack(aPrefix.mNameInternal + aMaterial.mNameInternal, aReplacement, aAmount, F, F);
        ItemStack rStack = getStack(aPrefix.mNameInternal + aMaterial.mNameInternal, aReplacement, aAmount, F, F);
        if (rStack == null && aMaterial.mID >= 0
            && aPrefix.isGeneratingItem(aMaterial)
            && !aPrefix.mRegisteredPrefixItems.isEmpty()) {
            return ST.make((Item) aPrefix.mRegisteredPrefixItems.get(0), aAmount, aMaterial.mID);
        }
        return rStack;
    }

    public ItemStack getStack(Object aName, ItemStack aReplacement, long aAmount, boolean aMentionPossibleTypos,
        ...
        if (!sName2StackMap.containsKey(aName.toString()) && aMentionPossibleTypos) {
            ...
            for (StackTraceElement tElement : new Exception().getStackTrace())
                if (i++ < 1 && i < 11) ERR.println("\tat " + tElement);
                else break;
        }
    ...
    public ItemStack getStack_(boolean aUseBlackList, ItemStack aStack) {
        ...
        if (tAssociation.mUnificationTarget == null)
            tAssociation.mUnificationTarget = sName2StackMap.get(tAssociation.toString());
    ...
    public boolean equal_(boolean aUseBlackList, ItemStack aUnificatableStack, ItemStack aStackToCheckAgainst,
        ...
        if (tAssociation.mUnificationTarget == null)
            tAssociation.mUnificationTarget = sName2StackMap.get(tAssociation.toString());
        return ST.valid(tAssociation.mUnificationTarget)
            && ST.equal(tAssociation.mUnificationTarget, aStackToCheckAgainst, aIgnoreNBT);
    }
    ...
    public boolean setItemData_(ItemStack aStack, OreDictItemData aData) {
        ...
        if (!aData.mBlocked) aData.mBlocked = (aData.mBlackListed || ST.block(aStack) != NB
            || FL.getFluid(aStack, T) != null
        ...
        if (aData.validMaterial()) {
            long tValidMaterialAmount = aData.mMaterial.mMaterial.contains(TD.Processing.UNRECYCLABLE) ? 0
                : aData.mMaterial.mAmount >= 0 ? aData.mMaterial.mAmount : U;
            for (OreDictMaterialStack tMaterial : aData.mByProducts)
                tValidMaterialAmount += tMaterial.mMaterial.contains(TD.Processing.UNRECYCLABLE) ? 0
                    : tMaterial.mAmount >= 0 ? tMaterial.mAmount : U;
        ...
        if (!aData.validPrefix() || aData.mPrefix.contains(TD.Prefix.RECYCLABLE)) {
            ...
            for (IOreDictListenerRecyclable tListener : mRecyclableOreDictListeners)
    ...
    public OreDictItemData getItemData_(ItemStack aStack, boolean aAllowOverride) {
        ...
        if (rData != null) return rData;
        if (aAllowOverride && aStack.getItem()
            .isDamageable()) {
            ...
            if (rData != null && rData.mUseVanillaDamage) {
                ...
                for (int i = 0; i < tByProducts.length; i++) tByProducts[i] = OM.stack(
                    rData.mByProducts[i].mMaterial,
                    UT.Code.units(
                        aStack.getMaxDamage() - aStack.getItemDamage(),
                        aStack.getMaxDamage(),
                        rData.mByProducts[i].mAmount,
                        F));
                rData = new OreDictItemData(
                    OM.stack(
                        rData.mMaterial.mMaterial,
                        UT.Code.units(
                            aStack.getMaxDamage() - aStack.getItemDamage(),
                            aStack.getMaxDamage(),
                            rData.mMaterial.mAmount,
                            F)),
                    tByProducts);
            }
        }
    ...
    public boolean addAssociation_(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack) {
        if (ST.meta_(aStack) == W) for (byte i = 0; i < 16; i++)
            setItemData_(ST.copyAmountAndMeta(1, i, aStack), new OreDictItemData(aPrefix, aMaterial));
    ...
    public boolean registerOre_(Object aName, ItemStack aStack) {
        if (CR.DELATE == aName) {
            if (MD.GT.mLoaded) CR.delate(aStack);
            return MD.GT.mLoaded;
        }
        if (Abstract_Mod.sStartedPostInit > 0) throw new IllegalStateException(
            "Late OreDict Registration using GT OreDict Utility. Only @Init and @PreInit are allowed for this when you use this Function instead of the Forge one.");
    ...
    private void registerUnificationEntries() {
        ...
        for (OreDictRegistrationContainer tEvent : mGlobalRegistrations) {
            if (tEvent.mPrefix != null && tEvent.mMaterial != null
                && tEvent.mPrefix.contains(TD.Prefix.UNIFICATABLE)
                && !tEvent.mMaterial.contains(TD.Properties.INVALID_MATERIAL)) {
                addAssociation_(tEvent.mPrefix, tEvent.mMaterial, tEvent.mStack);
                setTarget_(
                    tEvent.mPrefix,
                    tEvent.mMaterial,
                    tEvent.mStack,
                    !(tEvent.mStack.getItem() instanceof IPrefixItem) && !tEvent.mModName.equals(MD.GAPI_POST.mID)
                        && mUnificationConfig
                            .get("specialunificationtargets." + tEvent.mModName, tEvent.mEvent.Name, F),
                    T);
            }
        }
        Recipe.reInit();
    ...
    public static List<ItemStack> getOres(OreDictPrefix aPrefix, OreDictMaterial aMaterial,
        boolean aTransformWildcardBlocksTo16) {
        return getOres(aPrefix.mNameInternal + aMaterial.mNameInternal, aTransformWildcardBlocksTo16);
    }

    /** @return a Copy of the OreDictionary.getOres() List */
    public static List<ItemStack> getOres(OreDictPrefix aPrefix, OreDictMaterialStack aMaterial,
        boolean aTransformWildcardBlocksTo16) {
