...
public final class OreDictPrefix
    implements IOreDictListenerEvent, ITagDataContainer<OreDictPrefix>, ICondition<ITagDataContainer<?>> {
    ...
    /**
     * This is to prevent smaller Prefixes from breaking larger ones by just starting with the same few Characters the
     * larger one starts with.
     */
    ...
    private final Set<OreDictMaterial> mItemGeneratorBlackList = new HashSetNoNulls<>(),
        mIgnoredRegistrations = new HashSetNoNulls<>(), mItemGeneratorForced = new HashSetNoNulls<>();
    ...
    /**
     * List of Prefixes which are familiar to this Prefix. Like "dust" having "dustSmall" and "dustTiny" and vice versa.
     * Note that this per Default also contains the Prefix itself inside this Set.
     */
    ...
    public static OreDictPrefix createPrefix(String aName) {
        ...
        String tName = aName.replaceAll(" ", "")
            .replaceAll("-", ""); // Auto-Replace all Spaces and Minuses.
    ...
    private OreDictPrefix(String aNameInternal, String aNameLocal) {
        ...
        if (mNameInternal.contains("|") || mNameInternal.contains("*")
            || mNameInternal.contains(":")
            || mNameInternal.contains(".")
            || mNameInternal.contains("$"))
            throw new IllegalArgumentException("The Prefix Name contains invalid Characters!");
        if (mNameInternal.length() < 3) throw new IllegalArgumentException(
            "A Prefix must have at least 3 Characters, otherwise it would break other Prefixes way to easily.");
        VALUES.add(this);
    ...
    public OreDictPrefix applyStackSizes() {
        ...
        if (this != OP.block && this != OP.stone && this != OP.scrapGt) addListener(new IOreDictListenerEvent() {

            @Override
            public void onOreRegistration(OreDictRegistrationContainer aEvent) {
                if (!aEvent.mStack.getItem()
                    .isDamageable() && aEvent.mStack.getMaxStackSize() > 1 && !ST.isGT_(aEvent.mStack))
                    aEvent.mStack.getItem()
                        .setMaxStackSize(aEvent.mPrefix.mDefaultStackSize);
            }
        });
        ...
        Item.getItemFromBlock(Blocks.packed_ice)
            .setMaxStackSize(OP.block.mDefaultStackSize);
        ...
        Item.getItemFromBlock(Blocks.sandstone)
            .setMaxStackSize(OP.stone.mDefaultStackSize);
        ...
        Item.getItemFromBlock(Blocks.stone_slab)
            .setMaxStackSize(OP.slab.mDefaultStackSize);
        ...
        Item.getItemFromBlock(Blocks.stone_brick_stairs)
            .setMaxStackSize(OP.stair.mDefaultStackSize);
        ...
        Item.getItemFromBlock(Blocks.glass)
            .setMaxStackSize(OP.glass.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.stained_glass)
            .setMaxStackSize(OP.glass.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.glass_pane)
            .setMaxStackSize(OP.paneGlass.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.stained_glass_pane)
            .setMaxStackSize(OP.paneGlass.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.hardened_clay)
            .setMaxStackSize(OP.stainedClay.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.stained_hardened_clay)
            .setMaxStackSize(OP.stainedClay.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.cobblestone)
            .setMaxStackSize(OP.stoneCobble.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.fence)
            .setMaxStackSize(OP.fence.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.fence_gate)
            .setMaxStackSize(OP.fence.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.cobblestone_wall)
            .setMaxStackSize(OP.fence.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.nether_brick_fence)
            .setMaxStackSize(OP.fence.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.mossy_cobblestone)
            .setMaxStackSize(OP.stoneMossy.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.stonebrick)
            .setMaxStackSize(OP.stoneBricks.mDefaultStackSize);
        ...
        Item.getItemFromBlock(Blocks.sapling)
            .setMaxStackSize(OP.treeSapling.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.leaves)
            .setMaxStackSize(OP.treeLeaves.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.leaves2)
            .setMaxStackSize(OP.treeLeaves.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.log)
            .setMaxStackSize(OP.log.mDefaultStackSize);
        Item.getItemFromBlock(Blocks.log2)
            .setMaxStackSize(OP.log.mDefaultStackSize);

        Item.getItemFromBlock(Blocks.planks)
            .setMaxStackSize(OP.plank.mDefaultStackSize);
    ...
    public OreDictPrefix ignoreMaterials(OreDictMaterial... aMaterials) {
        if (aMaterials != null) for (OreDictMaterial aMaterial : aMaterials)
            if (aMaterial != null && !mIgnoredRegistrations.contains(aMaterial)) mIgnoredRegistrations.add(aMaterial);
        return this;
    }

    public OreDictPrefix disableItemGeneration(OreDictMaterial... aMaterials) {
        if (aMaterials != null) for (OreDictMaterial aMaterial : aMaterials)
            if (aMaterial != null && !mItemGeneratorBlackList.contains(aMaterial))
                mItemGeneratorBlackList.add(aMaterial);
        return this;
    }

    public OreDictPrefix forceItemGeneration(OreDictMaterial... aMaterials) {
        if (aMaterials != null) for (OreDictMaterial aMaterial : aMaterials)
            if (aMaterial != null && !mItemGeneratorForced.contains(aMaterial)) mItemGeneratorForced.add(aMaterial);
        return this;
    }

    @SuppressWarnings("unchecked")
    public boolean isGeneratingItem(OreDictMaterial aMaterial) {
        return aMaterial != null && (mItemGeneratorForced.contains(aMaterial)
            || (!mItemGeneratorBlackList.contains(aMaterial) && mCondition.isTrue(aMaterial)));
    }
    ...
    public OreDictPrefix addListener(IOreDictListenerEvent aListener) {
        ...
        else mBufferedListeners.add(aListener);
        return this;
    }
    ...
    private void addListenerInternal(IOreDictListenerEvent aListener) {
        if (mListenersOre.add(aListener))
            for (OreDictRegistrationContainer tEvent : mRegistrations) aListener.onOreRegistration(tEvent);
    }

    public void onPostLoad() {
        if (GAPI.mStartedPostInit)
            for (IOreDictListenerEvent aListener : mBufferedListeners) addListenerInternal(aListener);
    }
    ...
    /**
     * Adds the internal Name of this Prefix as Icon Name to all Texture Sets and sets its Icon Index to the Index of
     * the Set.
     * 
     * @param aModID the Mod responsible for adding the Icons (which are located in either
     *               assets\aModID.toLowerCase()\textures\items\materialicons\ or
     *               assets\aModID.toLowerCase()\textures\blocks\materialicons\).
     */
    public OreDictPrefix addTextureSet(String aModID, boolean aIsItem) {
        if (aIsItem) mIconIndexItem = TextureSet.addToAll(aModID, aIsItem, mNameTextureSet);
        else mIconIndexBlock = TextureSet.addToAll(aModID, aIsItem, mNameTextureSet);
        return this;
    }
    ...
    public OreDictPrefix addTextureSet(ModData aModID, boolean aIsItem) {
        if (aIsItem) mIconIndexItem = TextureSet.addToAll(aModID.mID, aIsItem, mNameTextureSet);
        else mIconIndexBlock = TextureSet.addToAll(aModID.mID, aIsItem, mNameTextureSet);
        return this;
    }
    ...
    public void onOreRegistration(OreDictRegistrationContainer aEvent) {
        if (aEvent.mMaterial == MT.NULL) {
            if (!contains(MATERIAL_BASED)) {
                if (COMPAT_TC != null && !(aEvent.mStack.getItem() instanceof IEssentiaContainerItem)
                    && !(aEvent.mStack.getItem() instanceof MultiItemRandom)
                    && !MD.MC.owns(aEvent.mRegName)
                    && !MD.TC.owns(aEvent.mRegName)) {
                ...
                for (IOreDictListenerEvent tListener : mListenersOre) {
                    if (D2) ORD.println(
                        "Processing '" + aEvent.mOreDictName
                            + "' with the Prefix '"
                            + mNameInternal
                            + "' and without Material at "
                            + UT.Reflection.getClassName(tListener));
                    ...
                    if (COMPAT_TC != null && !(aEvent.mStack.getItem() instanceof IEssentiaContainerItem)
                        ...
                        for (TC_AspectStack tAspect : mAspects) {
                            if (tAspect.mAspect == TC.METALLUM && !aEvent.mMaterial.mHasMetallum) {
                                ...
                                TC.stack(TC.ORDO, tAspect.mAmount)
                    ...
                    for (IOreDictListenerEvent tListener : mListenersOre) {
                        if (D2) ORD.println(
                            "Processing '" + aEvent.mOreDictName
                                ...
                                + mNameInternal
                                + "' and the Material '"
                                + aEvent.mMaterial.mNameInternal
                                + "' at "
                                + UT.Reflection.getClassName(tListener));
                        tListener.onOreRegistration(aEvent);
                    }
