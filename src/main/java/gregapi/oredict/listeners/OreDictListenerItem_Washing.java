...
public class OreDictListenerItem_Washing extends OreDictListenerItem {
     ...
     * @param aByProductPrefixes the Prefixes it can select from, if Items of those Prefixes exist. You can add the same
     *                           Prefix multiple times to weight it.
     */
    ...
    public ItemStack onTickWorld(OreDictPrefix aPrefix, OreDictMaterial aMaterial, ItemStack aStack, EntityItem aItem) {
        if (aMaterial != null) {
            int tX = UT.Code.roundDown(aItem.posX), tY = UT.Code.roundDown(aItem.posY - 0.25),
                tZ = UT.Code.roundDown(aItem.posZ);
            ...
            if (tBlock instanceof BlockCauldron && tMetaData > 0) {
                ...
                if (tStack != null) {
                    ...
                    if (mByProductPrefixes.length > 0 && RNGSUS.nextInt(mChance) > 0) {
                        ...
                        for (OreDictPrefix tPrefix : mByProductPrefixes)
                            tStacks.add(tPrefix.mat(UT.Code.select(aMaterial, aMaterial.mByProducts), 1));
                        if (tStacks.size() > 0) ST.drop(
                            aItem.worldObj,
                            aItem.posX,
                            aItem.posY,
                            aItem.posZ,
                            tStacks.get(RNGSUS.nextInt(tStacks.size())));
                    }
