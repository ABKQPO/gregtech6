...
public interface IMath<O> {
    ...
    /**
     * @param aObject the Object to check the Maths on.
     * @return the result of the Maths done by this Object as double. This also means that the calculation CAN be more
     *         precise than the Long variant.
     */
    ...
    public static class Neg<O> implements IMath<O> {
        ...
        @Override
        public long getLong(O aObject) {
            return -mValue.getLong(aObject);
        }

        @Override
        public double getDouble(O aObject) {
            return -mValue.getDouble(aObject);
        }
    }
    ...
    public static class Ter<O> implements IMath<O> {
        ...
        public Ter(ICondition<O> aCondition, IMath<O> aValueTrue, IMath<O> aValueFalse) {
            ...
            mValueTrue = aValueTrue;
            mValueFalse = aValueFalse;
        }

        @Override
        public long getLong(O aObject) {
            return mCondition.isTrue(aObject) ? mValueTrue.getLong(aObject) : mValueFalse.getLong(aObject);
        }

        @Override
        public double getDouble(O aObject) {
            return mCondition.isTrue(aObject) ? mValueTrue.getDouble(aObject) : mValueFalse.getDouble(aObject);
        }
    }
