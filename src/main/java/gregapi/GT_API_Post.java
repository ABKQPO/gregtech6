 ...
 */
@Mod(
    modid = ModIDs.GAPI_POST,
    name = "Greg-API-Post",
    version = "GT6-MC1710",
    dependencies = "required-after:" + ModIDs.GAPI
        + "; after:"
        + ModIDs.MD8
        + "; after:"
        + ModIDs.IC2
...
public class GT_API_Post extends Abstract_Mod {
    ...
    public void onModPreInit2(FMLPreInitializationEvent aEvent) {
        try {
            ...
            for (short i = 0; i < tModList.size(); i++) {
                ...
                if (tMod.getModId()
                    .equalsIgnoreCase(MD.GAPI_POST.mID)) tGregTech = tMod;
        ...
        for (Item tItem : new Item[] {
            ST.item(MD.GrC_Grapes, "grc.grapes"), ST.item(MD.FR, "letters"), ST.item(MD.FZ, "acid") })
            if (tItem != null) tItem.setMaxDamage(0)
                .setHasSubtypes(T);
        ...
        if (MD.TiC.mLoaded) {
            ...
            MT.OREMATS.Galena.addOreByProducts(MT.Ardite); // I found Ardaite on Wikipedia, which is a Galena Type of
                                                           // Material.
        ...
        if (MD.AE.mLoaded) {
            // Because Applied Energistics would not create its own Items if I also added them, which leads to it
            // getting Issues with its own shit.
    ...
    public void onModInit2(FMLInitializationEvent aEvent) {
        ...
        if (MD.PR.mLoaded) {
            ...
            for (int i = 0; i < 16; i++) {
                OM.reg(
                    DYE_OREDICTS_MIXABLE[i],
                    ST.make(MD.PR_EXPLORATION, "projectred.exploration.lilyseed", 1, 15 - i));
            ...
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Ruby, MD.PR_EXPLORATION, "projectred.exploration.ore", 0);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.BlueSapphire, MD.PR_EXPLORATION, "projectred.exploration.ore", 1);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Peridot, MD.PR_EXPLORATION, "projectred.exploration.ore", 2);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Cu, MD.PR_EXPLORATION, "projectred.exploration.ore", 3);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Sn, MD.PR_EXPLORATION, "projectred.exploration.ore", 4);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Ag, MD.PR_EXPLORATION, "projectred.exploration.ore", 5);
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.Nikolite, MD.PR_EXPLORATION, "projectred.exploration.ore", 6);
        }
        ...
        if (MD.MO.mLoaded) {
            ...
            OreDictManager.INSTANCE
                .setTarget(OP.oreVanillastone, MT.OREMATS.TritaniumDioxide, MD.MO, "tritanium_ore", 0);
        ...
        if (MD.ARS.mLoaded) {
            ...
            new SaplingEntry(
                ST.make(MD.ARS, "saplingWitchwood", 1, 0),
                new WoodEntry(
                    ST.make(MD.ARS, "WitchwoodLog", 1, 0),
                    new PlankEntry(
                        ST.make(MD.ARS, "planksWitchwood", 1, 0),
                        ST.make(MD.ARS, "witchwoodSingleSlab", 1, 0),
                        ST.make(MD.ARS, "stairsWitchwood", 1, 0),
                        MT.WOODS.Witchwood,
                        260),
                    1,
                    500),
                ST.make(MD.ARS, "WitchwoodLeaves", 1, 0));
            CR.shaped(
                ST.make(MD.ARS, "planksWitchwood", 1, 0),
                CR.DEF_NCC,
                "S",
        ...
        // Aether Legacy is fucking this up as well, and the best part is, it once WAS working fine, but they fucking
        // decided to break it!
        if (MD.AETHEL.mLoaded) {
            ...
            IL.AETHER_Skyroot_Planks
                .set(ST.make(MD.AETHEL, "skyroot_planks", 1, 0), new OreDictItemData(MT.Skyroot, U), OD.plankSkyroot);
            IL.AETHER_Skyroot_Log
                .set(ST.make(MD.AETHEL, "skyroot_log", 1, 0), new OreDictItemData(MT.Skyroot, U * 8, MT.Bark, U));
            IL.AETHER_Skyroot_Log_Gold.set(
                ST.make(MD.AETHEL, "golden_oak_log", 1, 2),
                new OreDictItemData(MT.Skyroot, U * 8, MT.AmberGolden, U));
            OM.reg(IL.AETHER_Skyroot_Log.wild(1), OD.logWood);
            IL.AETHER_Bucket_Empty
                .set(ST.make(MD.AETHEL, "skyroot_bucket", 1, 0), new OreDictItemData(MT.Skyroot, U * 3));
            IL.AETHER_Bucket_Water.set(
                ST.make(MD.AETHEL, "skyroot_bucket", 1, 1),
                new OreDictItemData(MT.Skyroot, U * 3),
                OD.container1000water);
            IL.AETHER_Bucket_Poison.set(
                ST.make(MD.AETHEL, "skyroot_bucket", 1, 2),
                new OreDictItemData(MT.Skyroot, U * 3),
                OD.container1000poison);
            IL.AETHER_Bucket_Remedy
                .set(ST.make(MD.AETHEL, "skyroot_bucket", 1, 3), new OreDictItemData(MT.Skyroot, U * 3));
            IL.AETHER_Bucket_Milk.set(
                ST.make(MD.AETHEL, "skyroot_bucket", 1, 4),
                new OreDictItemData(MT.Skyroot, U * 3),
                OD.container1000milk);
            IL.AETHER_Torch_Ambrosium.set(
                ST.make(MD.AETHEL, "ambrosium_torch", 1, 0),
            ...
            IL.AETHER_Skyroot_Leaves_Dark
                .set(ST.make(MD.AETHER, "decorated_holiday_leaves", 1, 1), null, OP.treeLeaves);
            ...
            BeamEntry tSkyrootBeam = new BeamEntry(
                ...
                new PlankEntry(
                    ...
                    ST.make(MD.AETHEL, "skyroot_slab", 1, 0),
                    ST.make(MD.AETHEL, "skyroot_stairs", 1, 0),
                    MT.Skyroot,
                    124),
                1,
                200);
            ...
            // new SaplingEntry(IL.AETHER_Skyroot_Sapling_Blue .wild(1), tSkyrootWood, IL.AETHER_Skyroot_Leaves_Blue
            // .wild(1));
            // new SaplingEntry(IL.AETHER_Skyroot_Sapling_Dark .wild(1), tSkyrootWood, IL.AETHER_Skyroot_Leaves_Dark
            // .wild(1));
            new SaplingEntry(
                IL.AETHER_Skyroot_Sapling_Green.wild(1),
                new WoodEntry(IL.AETHER_Skyroot_Log.wild(1), tSkyrootBeam, 1, 300),
                IL.AETHER_Skyroot_Leaves_Green.wild(1));
            new SaplingEntry(
                IL.AETHER_Skyroot_Sapling_Gold.wild(1),
                new WoodEntry(
                    IL.AETHER_Skyroot_Log_Gold.wild(1),
                    tSkyrootBeam,
                    2,
                    500,
            ...
            // new SaplingEntry(IL.AETHER_Skyroot_Sapling_Purple .wild(1), tSkyrootWood, IL.AETHER_Skyroot_Leaves_Purple
            // .wild(1));
        }
    ...
    public void onModPostInit2(FMLPostInitializationEvent aEvent) {
        ...
        if (MD.AE.mLoaded) AEApi.instance()
            .registries()
            .grinder()
            .getRecipes()
            .clear();

        // Well Netherite Plus is very special with its Compat Items... This is WAY too late in the loading Cycle! (I am
        // aware that the Ancient Dust got removed in later Versions)
        if (MD.NePl.mLoaded) {
            ...
            OreDictManager
                .registerOreSafe(OP.compressed.dat(MT.Netherite), ST.make(MD.NePl, "CompressedNetherite", 1, 0));
            OreDictManager.INSTANCE
                .setTarget(OP.dust, MT.AncientDebris, ST.make(MD.NePl, "AncientDust", 1, 0), T, T, T);
            OreDictManager.INSTANCE
                .setTarget(OP.compressed, MT.Netherite, ST.make(MD.NePl, "CompressedNetherite", 1, 0), T, T, T);
        }
        ...
        for (Enchantment tEnchant : Enchantment.enchantmentsList) if (tEnchant != null) {
            if ("enchantment.Magnetization".equalsIgnoreCase(tEnchant.getName())) {
                for (OreDictMaterial tMaterial : MT.ALL_MATERIALS_REGISTERED_HERE) {
                    if (tMaterial == MT.NeodymiumMagnetic) {
                        tMaterial.addEnchantmentForTools(tEnchant, 3)
                            .addEnchantmentForWeapons(tEnchant, 3)
                            .addEnchantmentForArmors(tEnchant, 3);
                    } else if (tMaterial == MT.SteelMagnetic || tMaterial == MT.MeteoricSteel
                        || tMaterial == MT.MeteoricBlackSteel
                        || tMaterial == MT.MeteoricBlueSteel
                        || tMaterial == MT.MeteoricRedSteel
                        || tMaterial == MT.MeteoflameSteel
                        || tMaterial == MT.MeteoflameBlackSteel
                        || tMaterial == MT.MeteoflameBlueSteel
                        || tMaterial == MT.MeteoflameRedSteel
                        || tMaterial == MT.Meteorite) {
                            tMaterial.addEnchantmentForTools(tEnchant, 2)
                                .addEnchantmentForWeapons(tEnchant, 2)
                                .addEnchantmentForArmors(tEnchant, 2);
                        } else
                        if (tMaterial.containsAny(TD.Properties.MAGNETIC_ACTIVE, TD.Properties.AUTO_COLLECTING)) {
                            tMaterial.addEnchantmentForTools(tEnchant, 1)
                                .addEnchantmentForWeapons(tEnchant, 1)
                                .addEnchantmentForArmors(tEnchant, 1);
                        }
                }
            }
            if ("enchantment.swift_sneak".equalsIgnoreCase(tEnchant.getName())) {
                if (MD.TF.mLoaded) {
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.tower_library, "ultrarare"))
                        .addEnchantedBook(tEnchant, 2);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.labyrinth_vault, "rare"))
                        .addEnchantedBook(tEnchant, 2);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.stronghold_room, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.darktower_cache, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.aurora_cache, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.aurora_room, "uncommon"))
                        .addEnchantedBook(tEnchant, 1);
                }
            }
            if ("enchantment.mending".equalsIgnoreCase(tEnchant.getName())) {
                if (MD.TF.mLoaded) {
                    ...
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.labyrinth_vault, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.darktower_key, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.aurora_room, "rare"))
                        .addEnchantedBook(tEnchant, 1);
                    ((TFTreasureTable) UT.Reflection.getFieldContent(TFTreasure.troll_vault, "uncommon"))
                        .addEnchantedBook(tEnchant, 1);
                }
            }
            ...
            if ("enchantment.railcraft.crowbar.implosion".equalsIgnoreCase(tEnchant.getName())) {
                ...
                for (OreDictMaterial tMat : ANY.TigerEye.mToThis) tMat.addEnchantmentForWeapons(tEnchant, 4)
                    .addEnchantmentForAmmo(tEnchant, 6);
                for (OreDictMaterial tMat : ANY.Aventurine.mToThis) tMat.addEnchantmentForWeapons(tEnchant, 3)
                    .addEnchantmentForAmmo(tEnchant, 5);
                for (OreDictMaterial tMat : ANY.Phosphorus.mToThis) tMat.addEnchantmentForWeapons(tEnchant, 5)
                    .addEnchantmentForAmmo(tEnchant, 7);
                MT.PO4.addEnchantmentForWeapons(tEnchant, 5)
                    .addEnchantmentForAmmo(tEnchant, 7);
                MT.Phosphorite.addEnchantmentForWeapons(tEnchant, 5)
                    .addEnchantmentForAmmo(tEnchant, 7);
                ...
                MT.BlueTopaz.addEnchantmentForWeapons(tEnchant, 5)
                    .addEnchantmentForAmmo(tEnchant, 7);
                MT.Zanite.addEnchantmentForWeapons(tEnchant, 4)
                    .addEnchantmentForAmmo(tEnchant, 6);
                ...
                MT.Opal.addEnchantmentForWeapons(tEnchant, 4)
                    .addEnchantmentForAmmo(tEnchant, 6);
                MT.Peridot.addEnchantmentForWeapons(tEnchant, 2)
                    .addEnchantmentForAmmo(tEnchant, 4);
                ...
                MT.Craponite.addEnchantmentForWeapons(tEnchant, 1)
                    .addEnchantmentForAmmo(tEnchant, 3);
        ...
        for (OreDictMaterial tMaterial : OreDictMaterial.MATERIAL_MAP.values()) {
            // Diatomic Elements get a Subscript 2 appended to their ToolTip after PostInit. That way the ToolTip
            // Calculation works properly until PostInit happens.
            ...
            if (tMaterial.contains(TD.Processing.CRUCIBLE_ALLOY))
                if (tMaterial.mComponents != null) tMaterial.addAlloyingRecipe(tMaterial.mComponents);
                else ERR.println(
                    "ERROR: Alloying Recipe for " + tMaterial.mNameLocal
                        + " cannot be added due to lack of Component Information");
        }

        // Polyatomic Elements get a Subscript 4 or 8 appended to their ToolTip after PostInit. That way the ToolTip
        // Calculation works properly until PostInit happens.
        ...
        for (OreDictPrefix tPrefix : OreDictPrefix.VALUES)
            if (tPrefix.containsAny(TD.Prefix.STANDARD_ORE, TD.Prefix.DUST_ORE, TD.Prefix.DENSE_ORE))
                OP.ore.mRegisteredItems.addAll(tPrefix.mRegisteredItems);

        // And now to stop CoFH-Core from crashing and lagging the Game in Singleplayer with the FMLProxyPacket race
        // condition Bug.
        // I hate having to do this, but there is no other proper way to actually fix this Issue...
        // It would be so much easier to just say to not use this Mod, but ofcourse a lot of Stuff depends on it
        // "existing" for no good reasons.
        // This Bug has caused many people way too much wasted time to just leave it alone. So Baseball Bat instead of
        // Surgical Precision it is!

        // Replaced by ASM Stuff.
        // if (CODE_CLIENT && MD.COFH_CORE.mLoaded && ConfigsGT.CLIENT.get(ConfigCategories.general,
        // "BrutallyFixCoFHCoreInSinglePlayer", T)) try {
