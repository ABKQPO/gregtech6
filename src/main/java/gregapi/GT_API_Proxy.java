...
public abstract class GT_API_Proxy extends Abstract_Proxy implements IGuiHandler, IFuelHandler, IWorldGenerator {
    ...
    public Object getServerGuiElement(int aGUIID, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ) {
        ...
        return tTileEntity instanceof ITileEntityGUI ? ((ITileEntityGUI) tTileEntity).getGUIServer(aGUIID, aPlayer)
            : null;
    }
    ...
    public Object getClientGuiElement(int aGUIID, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ) {
        TileEntity tTileEntity = WD.te(aWorld, aX, aY, aZ, T);
        return tTileEntity instanceof ITileEntityGUI ? ((ITileEntityGUI) tTileEntity).getGUIClient(aGUIID, aPlayer)
            : null;
    }
    ...
    public boolean checkSaveLocation(File aSaveLocation, boolean aForceSave) {
        ...
        // Did Save Files swap secretly? Can happen in Singleplayer with the popular Forge Monopoly Bug: "Go directly to
        // the Main Menu. Do not enter your World. Do not collect 200 Blocks."
        ...
        if (tSave && mSaveLocation != null) {
            ...
            if (aSaveLocation == null) OUT.println("Saving  World! " + mSaveLocation.getName());// else
                                                                                                // DEB.println("Autosave!
                                                                                                // " +
                                                                                                // mSaveLocation.getName());
    ...
    // @SubscribeEvent(priority = EventPriority.LOWEST) public void onWorldUnload(WorldEvent.Unload aEvent)
    // {checkSaveLocation(DimensionManager.getCurrentSaveRootDirectory(), F);}
    // @SubscribeEvent(priority = EventPriority.LOWEST) public void onWorldSave (WorldEvent.Save aEvent)
    // {checkSaveLocation(DimensionManager.getCurrentSaveRootDirectory(), F);}

    public static final List<ITileEntityServerTickPre> SERVER_TICK_PRE = new ArrayListNoNulls<>(),
        SERVER_TICK_PR2 = new ArrayListNoNulls<>();
    public static final List<ITileEntityServerTickPost> SERVER_TICK_POST = new ArrayListNoNulls<>(),
        SERVER_TICK_PO2T = new ArrayListNoNulls<>();
    ...
    public void onServerTick(ServerTickEvent aEvent) {
        ...
        if (aEvent.side.isServer()) {
            // Try acquiring the Lock within 10 Milliseconds. Otherwise fuck anyone who locks it up for too long, or any
            // other faulty reason MC doesn't work.
            try {
            ...
            if (aEvent.phase == Phase.START) {
                ...
                if (SERVER_TIME++ == 0) {
                    ...
                    if (MD.IC2.mLoaded) try {
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.cannerBottle.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.centrifuge.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.compressor.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.extractor.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.macerator.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.metalformerCutting
                            .getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.metalformerExtruding
                            .getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.metalformerRolling
                            .getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.matterAmplifier.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                        for (ic2.api.recipe.RecipeOutput tRecipe : ic2.api.recipe.Recipes.oreWashing.getRecipes()
                            .values()) for (ItemStack tStack : tRecipe.items) tStacks.add(tStack);
                    } catch (Throwable e) {
                        e.printStackTrace(ERR);
                    }
                    ...
                    if (MD.RC.mLoaded) {
                        try {
                            for (Object tRecipe : mods.railcraft.api.crafting.RailcraftCraftingManager.blastFurnace
                                .getRecipes())
                                tStacks.add((ItemStack) UT.Reflection.getFieldContent(tRecipe, "output"));
                        } catch (Throwable e) {
                            e.printStackTrace(ERR);
                        }
                        try {
                            for (Object tRecipe : mods.railcraft.api.crafting.RailcraftCraftingManager.cokeOven
                                .getRecipes())
                                tStacks.add((ItemStack) UT.Reflection.getFieldContent(tRecipe, "output"));
                        } catch (Throwable e) {
                            e.printStackTrace(ERR);
                        ...
                        try {
                            for (Object tRecipe : mods.railcraft.api.crafting.RailcraftCraftingManager.rockCrusher
                                .getRecipes())
                                for (Map.Entry<ItemStack, Float> tEntry : (List<Map.Entry<ItemStack, Float>>) UT.Reflection
                                    .getFieldContent(tRecipe, "outputs")) tStacks.add(tEntry.getKey());
                        } catch (Throwable e) {
                            e.printStackTrace(ERR);
                        }
                        try {
                            for (IRecipe tRecipe : mods.railcraft.api.crafting.RailcraftCraftingManager.rollingMachine
                                .getRecipeList()) if (tRecipe != null) tStacks.add(tRecipe.getRecipeOutput());
                        } catch (Throwable e) {
                            e.printStackTrace(ERR);
                        }
                    }
                    ...
                    if (MD.TE.mLoaded && ALWAYS_FALSE) {
                        ...
                        for (String tClassName : new String[] { "cofh.thermalexpansion.util.crafting.InsolatorManager",
                            "cofh.thermalexpansion.util.crafting.ChargerManager",
                            "cofh.thermalexpansion.util.crafting.ExtruderManager",
                            "cofh.thermalexpansion.util.crafting.PrecipitatorManager",
                            "cofh.thermalexpansion.util.crafting.TransposerManager",
                            "cofh.thermalexpansion.util.crafting.CrucibleManager",
                            "cofh.thermalexpansion.util.crafting.SmelterManager",
                            "cofh.thermalexpansion.util.crafting.SawmillManager",
                            "cofh.thermalexpansion.util.crafting.PulverizerManager",
                            "cofh.thermalexpansion.util.crafting.FurnaceManager" }) {
                            try {
                        ...
                        for (Map tMap : tMaps) {
                            try {
                                for (Object tCompStack : tMap.keySet()) if (tCompStack instanceof ComparableItem) {
                                    ItemStack tStack = OM.get(
                                        ST.make(
                                            ((ComparableItem) tCompStack).item,
                                            1,
                                            ((ComparableItem) tCompStack).metadata));
                                    if (ST.valid(tStack)) {
                    ...
                    for (String tLootList : ST.LOOT_TABLES)
                        for (WeightedRandomChestContent tContent : ChestGenHooks.getInfo(tLootList)
                            .getItems(RNGSUS)) tStacks.add(tContent.theItemId);

                    if (MD.IE.mLoaded) try {
                        for (WeightedRandomChestContent tContent : ((ChestGenHooks) UT.Reflection.getFieldContent(
                            "blusunrize.immersiveengineering.common.world.VillageEngineersHouse",
                            "crateContents")).getItems(RNGSUS)) {
                    ...
                    for (Object tStack : FurnaceRecipes.smelting()
                        .getSmeltingList()
                        .values()) tStacks.add((ItemStack) tStack);

                    if (MD.EtFu.mLoaded) {
                        boolean tSuccess = F;

                        if (!tSuccess) try {
                            Map tMap = ((Map) UT.Reflection
                                .getFieldContent(SmokerRecipes.smelting(), "smeltingList", T, D1));
                            if (tMap != null) {
                                for (Object tStack : tMap.values()) tSuccess |= tStacks.add((ItemStack) tStack);
                            }
                            tMap = ((Map) UT.Reflection
                                .getFieldContent(BlastFurnaceRecipes.smelting(), "smeltingList", T, D1));
                            if (tMap != null) {
                                for (Object tStack : tMap.values()) tSuccess |= tStacks.add((ItemStack) tStack);
                            }
                        } catch (Throwable e) {
                    ...
                    for (ItemStack tOutput : tStacks) {
                        if (OreDictManager.INSTANCE.isOreDictItem(tOutput)) {
                            ERR.println(
                                "GT-ERR-01: @ " + tOutput.getUnlocalizedName() + "   " + tOutput.getDisplayName());
                            FMLLog.severe(
                                "GT-ERR-01: @ " + tOutput.getUnlocalizedName() + "   " + tOutput.getDisplayName());
                            if (CS.CODE_CLIENT) {
                                FMLLog.severe(
                                ...
                                FMLLog.severe(
                                    "Also it is a Ban Reason on the IC2-Forums to seriously post this Text. We all know about its existence.");
                        ...
                        if (tRecipe instanceof ShapedOreRecipe) {
                            ...
                            for (int j = 0; j < tInput.length; j++) {
                                if (tInput[j] instanceof List && ((List<?>) tInput[j]).isEmpty()) {
                                    // DEB.println("Removed Recipe for " +
                                    // ((ShapedOreRecipe)tRecipe).getRecipeOutput().getDisplayName() + " because
                                    // Ingredient Nr. " + j + " is missing");
                                    tList.remove(i--);
                                    break;
                                }
                            }
                            ...
                            for (int j = 0; j < tInput.size(); j++) {
                                if (tInput.get(j) instanceof List && ((List<?>) tInput.get(j)).isEmpty()) {
                                    // DEB.println("Removed Recipe for " +
                                    // ((ShapelessOreRecipe)tRecipe).getRecipeOutput().getDisplayName() + " because
                                    // Ingredient Nr. " + j + " is missing");
                                    tList.remove(i--);
                                    break;
                                }
                ...
                for (IHasWorldAndCoords tTileEntity : DELAYED_BLOCK_UPDATES_2) {
                    try {
                        tTileEntity.getWorld()
                            .notifyBlocksOfNeighborChange(
                                tTileEntity.getX(),
                                tTileEntity.getY(),
                                tTileEntity.getZ(),
                                tTileEntity.getBlock(tTileEntity.getCoords()));
                    } catch (Throwable e) {
                ...
                if (SERVER_TIME > 10) {
                    for (ITileEntityScheduledUpdate tTileEntity : SCHEDULED_TILEENTITY_UPDATES_2)
                        if (!tTileEntity.isDead()) {
                            try {
                    ...
                    while (!mNewPlayers.isEmpty()) {
                        ...
                        for (OreDictPrefix tPrefix : OreDictPrefix.VALUES)
                            if (!tPrefix.contains(TD.Prefix.PREFIX_UNUSED))
    ...
    public void onLivingUpdate(LivingUpdateEvent aEvent) {
        int tX = UT.Code.roundDown(aEvent.entityLiving.posX),
        ...
        if (tBlock instanceof IBlockOnHeadInside)
            ((IBlockOnHeadInside) tBlock).onHeadInside(aEvent.entityLiving, aEvent.entityLiving.worldObj, tX, tY, tZ);
            ...
            if (BlocksGT.Paths != null && !aEvent.entityLiving.worldObj.isRemote) {
                ...
                if (ST.valid(tPath))
                    for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++) {
                        if (tPath == aEvent.entityLiving.worldObj.getBlock(tX + i, tY + k, tZ + j)) WD.replaceAll(
                            aEvent.entityLiving.worldObj,
                            tX + i,
                            tY + k,
                            tZ + j,
                            tPath,
        ...
        if (aEvent.entityLiving.onGround) {
            ...
            if (!WD.hasCollide(aEvent.entityLiving.worldObj, tX, tY, tZ, tBlock)) {
                int tAddX = (aEvent.entityLiving.posX >= tX + 0.5 ? +1 : -1),
                    tAddZ = (aEvent.entityLiving.posZ >= tZ + 0.5 ? +1 : -1);
            ...
            if (!aEvent.entityLiving.worldObj.isRemote) {
                ...
                if (aEvent.entityLiving instanceof EntityPig || aEvent.entityLiving instanceof EntitySheep
                    || aEvent.entityLiving instanceof EntityCow
                    || aEvent.entityLiving instanceof EntityHorse) {
                ...
                if (aEvent.entityLiving.hurtResistantTime > 0) {
                    ...
                    if (MD.TF.mLoaded && aEvent.entityLiving instanceof EntityTFMinoshroom) {
                        ...
                        for (int iX = tX - 15, eX = tX + 15; iX <= eX; iX++)
                            for (int iZ = tZ - 15, eZ = tZ + 15; iZ <= eZ; iZ++)
                                for (int iY = tY + 1, eY = tY + 3; iY <= eY; iY++) {
    ...
    public void onWorldTick(WorldTickEvent aEvent) {
        ...
        if (aEvent.side.isServer() && aEvent.phase == Phase.END) {
            ArrayListNoNulls<EntityXPOrb> tOrbs = (XP_ORB_COMBINING && SERVER_TIME % 40 == 31
                ? new ArrayListNoNulls<EntityXPOrb>(128)
                : null);
                ...
                if (aEntity instanceof EntityXPOrb) {
                    ...
                    if (ST.valid(aStack)) {
                        ...
                        if (rStack.getItem() instanceof MultiItemTool) {
                            if (MultiItemTool.getPrimaryMaterial(aStack)
                                .contains(TD.Properties.UNBURNABLE)) tFireProof = T;
                            if (MultiItemTool.getSecondaryMaterial(aStack)
                                .contains(TD.Properties.UNBURNABLE)) tFireProof = T;
                        }

                        OreDictItemData aData = OM.anydata_(rStack);
                        if (aData != null) {
                            if (aData.validPrefix())
                                for (IOreDictListenerItem tListener : aData.mPrefix.mListenersItem) {
                                    rStack = tListener.onTickWorld(
                                        aData.mPrefix,
                    ...
                    if (ENTITY_CRAMMING > 0 && SERVER_TIME % 50 == 0
                        && !(aEntity instanceof EntityPlayer)
                        && ((EntityLivingBase) aEntity).canBePushed()
                        && ((EntityLivingBase) aEntity).getHealth() > 0) {
                        List<?> tList = aEntity.worldObj
                            .getEntitiesWithinAABBExcludingEntity(aEntity, aEntity.boundingBox.expand(0.2, 0.0, 0.2));
                        Class<? extends Entity> tClass = aEntity.getClass();
                        int aEntityCount = 1;
                        if (tList != null)
                            for (int j = 0; j < tList.size(); j++) if (tList.get(j) != null && tList.get(j)
                                .getClass() == tClass) aEntityCount++;
                        if (aEntityCount > ENTITY_CRAMMING) aEntity.attackEntityFrom(
                            DamageSource.inWall,
                            (aEntityCount - ENTITY_CRAMMING) * TFC_DAMAGE_MULTIPLIER);
                    }
                }
            }
            ...
            if (tOrbs != null && tOrbs.size() > 32) for (EntityXPOrb aOrb : tOrbs) {
                ...
                for (EntityXPOrb tOrb : tOrbs) if (aOrb != tOrb && !tOrb.isDead
                    && tOrb.xpValue > 0
                    && tOrb.xpValue < Short.MAX_VALUE
                    && aOrb.getDistanceSqToEntity(tOrb) <= 3) {
    ...
    public void onPlayerTickEvent(PlayerTickEvent aEvent) {
        if (!aEvent.player.isDead && aEvent.phase == Phase.END) {
            ...
            //// Object tChunkProvider =
            //// ((WorldProviderTwilightForest)aEvent.player.worldObj.provider).getChunkProvider();
            //// if (tChunkProvider != null) {
            //// DEB.println("TRYING TO REMOVE SPAMMED TWILIGHT STRUCTURES");
            //// Iterator<Map.Entry<Long, Object>> titerator =
            //// ((MapGenTFMajorFeature)UT.Reflection.getFieldContent(tChunkProvider,
            //// "majorFeatureGenerator")).structureMap.entrySet().iterator();
            //// while (titerator.hasNext()) {
            //// Map.Entry<Long, Object> tEntry = titerator.next();
            //// ChunkCoordIntPair tCoords = new ChunkCoordIntPair((int)((tEntry.getKey()) & 4294967295L),
            //// (int)((tEntry.getKey() >> 32) & 4294967295L));
            //// if (Math.abs(tCoords.chunkXPos) > 666 || Math.abs(tCoords.chunkZPos) > 666) {
            //// DEB.println("REMOVED A SUPERFLUOUS STRUCTURE AT: " + tCoords.chunkXPos*16 + "; " +
            //// tCoords.chunkZPos*16);
            //// titerator.remove();
            //// }
            //// }
            //// }
            ...
            if (aEvent.side.isServer()) {
                 ...
                 * DEB.println(tEntity.getClass() + " " + tEntity.getAge() + " " + tEntity.ticksExisted + " GOT CLOSE TO
                 * PLAYER");
                 * tIterator.remove();
                 * } else {
                 * DEB.println(tEntity.getClass() + " " + tEntity.getAge() + " " + tEntity.ticksExisted);
                 ...
                 * }
                 * DEB.println("====01====");
                 * DEB.println("List Changed: " +
                 * mMobsToFastDespawn.removeAll(aEvent.player.worldObj.getEntitiesWithinAABBExcludingEntity(aEvent.player,
                 * AxisAlignedBB.getBoundingBox(aEvent.player.posX-32, aEvent.player.posY-32, aEvent.player.posZ-32,
                 * aEvent.player.posX+32, aEvent.player.posY+32, aEvent.player.posZ+32))));
                 * DEB.println("====02====");
                 * tIterator = mMobsToFastDespawn.iterator();
                 * while (tIterator.hasNext()) {
                 * EntityLiving tEntity = tIterator.next();
                 ...
                 * }
                 */
                if (SURVIVAL_INTO_ADVENTURE_MODE && aEvent.player.ticksExisted % 200 == 0
                    && aEvent.player.capabilities.allowEdit
                    && !UT.Entities.isCreative(aEvent.player)) {
                    aEvent.player.setGameType(WorldSettings.GameType.ADVENTURE);
                    aEvent.player.capabilities.allowEdit = F;
                    if (ADVENTURE_MODE_KIT) {
                        if (MD.GT.mLoaded) {
                            UT.Entities.sendchat(
                                aEvent.player,
                                CHAT_GREG + "Thank you for choosing the GregTech-6 Adventure Mode Starter Kit.");

                            MultiTileEntityRegistry tRegistry = MultiTileEntityRegistry
                                .getRegistry("gt.multitileentity");
                            ST.drop(
                                aEvent.player,
                                tRegistry == null ? IL.Bottle_Purple_Drink.get(6)
                                    : tRegistry.getItem(
                                        8762,
                                        1,
                                        UT.NBT.make(
                                            NBT_INV_LIST,
                                            UT.NBT.makeInv(
                                                IL.Bottle_Purple_Drink.get(1),
                                                IL.Bottle_Empty.get(1),
                                                IL.Bottle_Purple_Drink.get(1),
                                                IL.Bottle_Purple_Drink.get(1),
                                                IL.Bottle_Empty.get(1),
                                                IL.Bottle_Purple_Drink.get(1),
                ...
                final boolean tHungerEffect = (HUNGER_BY_INVENTORY_WEIGHT && aEvent.player.ticksExisted % 2400 == 1200),
                    tBetweenlands = WD.dimBTL(aEvent.player.worldObj.provider);// , tCrazyJ1984 =
                                                                               // "CrazyJ1984".equalsIgnoreCase(aEvent.player.getCommandSenderName());
                if (aEvent.player.ticksExisted % 120 == 0) {
                        ...
                        if (ST.valid(tStack = aEvent.player.inventory.getStackInSlot(i))) {
                            ...
                            if (tBetweenlands) {
                                if (tStack.getItem() == Items.potionitem) {
                                    ...
                                    if (tFluid != null && !FL.Potion_Tainted.is(tFluid)
                                        && FluidsGT.POTION.contains(
                                            tFluid.getFluid()
                                                .getName())) {
                                        ((IFluidContainerItem) tStack.getItem()).drain(tStack, Integer.MAX_VALUE, T);
                                        ((IFluidContainerItem) tStack.getItem())
                                            .fill(tStack, FL.Potion_Tainted.make(tFluid.amount), T);
                                    }
                                }
                                ItemStack tRotten = RottingUtil.rotting(
                                    tStack,
                                    aEvent.player.worldObj,
                                    UT.Code.roundDown(aEvent.player.posX),
                                    UT.Code.roundDown(aEvent.player.posY),
                                    UT.Code.roundDown(aEvent.player.posZ));
                            ...
                            // You can't detect properly when you pick things up out of a Chest, so part of the
                            // Inventory scan it is!
                            ...
                            if (!UT.Entities.isInvincible(aEvent.player)) {
                                UT.Entities.applyRadioactivity(
                                    aEvent.player,
                                    UT.Entities.getRadioactivityLevel(tStack),
                                    tStack.stackSize);
                                float tHeat = UT.Entities.getHeatDamageFromItem(tStack);
                                if (tHeat != 0.0F) if (tHeat > 0) UT.Entities.applyHeatDamage(aEvent.player, tHeat);
                                else UT.Entities.applyFrostDamage(aEvent.player, -tHeat);
                            }
                            // Data based checks.
                            OreDictItemData tData = OM.anydata_(tStack);
                            if (tData != null && tData.validMaterial()) {
                                if ((tData.mMaterial.mMaterial == MT.Bedrockium
                                    || tData.mMaterial.mMaterial == MT.Neutronium)
                                    && (tData.validPrefix() || tData.mByProducts.length <= 0)) {
                                    PotionEffect tEffect = null;
                                    UT.Entities.applyPotion(
                                        aEvent.player,
                                        Potion.moveSlowdown.id,
                                        Math.max(
                                            140,
                                            ((tEffect = aEvent.player.getActivePotionEffect(Potion.moveSlowdown))
                                ...
                                if (tData.mMaterial.mMaterial == MT.Firestone && tData.validPrefix()
                                    && !MD.RC.owns(tStack))
                                    for (int j = (int) UT.Code.divup(tData.mMaterial.mAmount * tStack.stackSize, U); j
                                        > 0; j--) {
                                            WD.fire(
                                                aEvent.player.worldObj,
                                                UT.Code.roundDown(aEvent.player.posX) - 5 + RNGSUS.nextInt(11),
                                                UT.Code.roundDown(aEvent.player.posY) - 5 + RNGSUS.nextInt(11),
                                                UT.Code.roundDown(aEvent.player.posZ) - 5 + RNGSUS.nextInt(11),
                                                RNGSUS.nextInt(8) != 0);
                                        }
                            }
                            if (tHungerEffect)
                            ...
                            if (tStack.hasTagCompound() && tStack.getTagCompound()
                                .hasNoTags()) tStack.setTagCompound(null);
                        }
                    }

                    // This Code is to tell Bear and all the people around him that he should clean up his always
                    // cluttered Inventory.
                                ...
                                if ("Bear989Sr".equalsIgnoreCase(tPlayer.getCommandSenderName())) {
                                    if (tPlayer.posY < 30) {
                                        UT.Entities.chat(
                                            ...
                                            new ChatComponentText(
                                                CHAT_GREG + "Stop making Holes in the Ground, Bear!"));
                                    } else {
                                        ...
                                        switch (tEmptySlots) {
                                            case 0:
                                                UT.Entities.chat(
                                                    ...
                                                    new ChatComponentText(
                                                        CHAT_GREG
                                                            + "Alright Buttercup, your Inventory is full, time to go home."));
                                                break;
                                            case 1:
                                                UT.Entities.chat(
                                                    tPlayer,
                                                    new ChatComponentText(
                                                        CHAT_GREG
                                                            + "Your Inventory is starting to get full, Buttercup"));
                                                break;
                                            case 2:
                                                UT.Entities.chat(
                                                    tPlayer,
                                                    new ChatComponentText(
                                                        CHAT_GREG
                                                            + "Your Inventory is starting to get full, Bean989Sr"));
                                                break;
                                            case 3:
                                                UT.Entities.chat(
                                                    tPlayer,
                                                    new ChatComponentText(
                                                        CHAT_GREG
                                                            + "Your Inventory is starting to get full, Mr. Bear"));
                                                break;
                                        }
                                    }
                                    ...
                                    if (ST.valid(tArrow)) {
                                        ...
                                        UT.Entities.chat(
                                            ...
                                            new ChatComponentText(
                                                CHAT_GREG
                                                    + "I'm not trying to tell you what to do, but please don't hurt Bear with this."));
                                    } else {
                                        UT.Entities.chat(
                                            tPlayer,
                                            new ChatComponentText(
                                                CHAT_GREG
                                                    + "I'm not trying to tell you what to do, but please don't hurt Bear."));
                                    }
                                    ...
                                    if (ST.valid(tArrow)) {
                                        ...
                                        UT.Entities.chat(
                                            ...
                                            new ChatComponentText(
                                                CHAT_GREG
                                                    + "People around Bear always seem to suffer a severe case of Craponite Arrow in Inventory, I don't know why."));
                                    } else {
                                        UT.Entities.chat(
                                            tPlayer,
                                            new ChatComponentText(
                                                CHAT_GREG + "Aaaaand Bears Inventory is full again isn't it..."));
                                    }
                                    ...
                                    UT.Entities.chat(
                                        ...
                                        new ChatComponentText(
                                            CHAT_GREG
                                                + "Could you tell Bear989Sr very gently, that his Inventory is a fucking mess again?"));
                                } else if ("Shadowkn1ght18".equalsIgnoreCase(tPlayer.getCommandSenderName())) {
                                    UT.Entities.chat(
                                        tPlayer,
                                        new ChatComponentText(
                                            CHAT_GREG
                                                + "Here is your special Message to make you tell Bear989Sr to clean his Inventory."));
                                } else if ("e99999".equalsIgnoreCase(tPlayer.getCommandSenderName())) {
                                    UT.Entities.chat(
                                        tPlayer,
                                        new ChatComponentText(
                                            LH.Chat.DGRAY
                                                + "You get the sneaking suspicion that Bears Inventory may or may not be full right now."));
                                } else {
                                    UT.Entities.chat(
                                        tPlayer,
                                        new ChatComponentText(
                                            CHAT_GREG
                                                + "There is this fella called Bear-Nine-Eight-Nine, needing be reminded of his Inventory being a major Pine."));
                                }
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < 4; i++) if (ST.valid(tStack = aEvent.player.inventory.armorInventory[i])) {
                        // The Better Storage Backpack would dupe Items when destroyed while worn, so this will prevent
                        // that.
                        // A Backpack already is hindrance enough if you want full Armor, so Durability should not
                        // matter here anyways.
                        // I also like this Backpack implementation, so I cant just leave the dupe exploit easy to pull
                        // off.
                        if (MD.BTRS.mLoaded
                            && (IL.BTRS_Backpack.equal(tStack, T, T) || IL.BTRS_Thaumpack.equal(tStack, T, T)
                                || IL.BTRS_Enderpack.equal(tStack, T, T))) {
                            ST.meta(tStack, 0);
                        }
    ...
    public void onChunkWatchEvent(ChunkWatchEvent.Watch aEvent) {
        ...
        if (tChunk != null && tChunk.isTerrainPopulated
            && tChunk.chunkTileEntityMap != null
            && tChunk.chunkTileEntityMap.size() > 0) {
            byte tIterations = 8;
            HashSetNoNulls<Object> tSet = new HashSetNoNulls<>();
            while (tIterations-- > 0) try {
                for (Object tTileEntity : tChunk.chunkTileEntityMap.values())
                    if (tTileEntity instanceof ITileEntitySynchronising) if (tSet.add(tTileEntity))
    ...
    public void onPlayerDestroyItem(PlayerDestroyItemEvent aEvent) {
        ...
        if (TOOL_BREAK_FATIQUE) {
            ...
            } else if (!ItemsGT.NO_TOOL_FATIQUE.contains(aEvent.original, T)
                && (ST.item_(aEvent.original) instanceof ItemSword || ST.item_(aEvent.original) instanceof ItemTool)) {
            ...
            if (ST.equal(aEvent.original, tInv[tSlot + 27], T)) {
                if (ST.equal(aEvent.original, tInv[tSlot + 18], T)) {
                    if (ST.equal(aEvent.original, tInv[tSlot + 9], T)) {
                        tInv[tSlot] = tInv[tSlot + 9];
                        tInv[tSlot + 9] = null;
                        ST.update(aEvent.entityPlayer);
                        return;
                    }
                    tInv[tSlot] = tInv[tSlot + 18];
                    tInv[tSlot + 18] = null;
                    ST.update(aEvent.entityPlayer);
                    return;
                }
                tInv[tSlot] = tInv[tSlot + 27];
                tInv[tSlot + 27] = null;
                ST.update(aEvent.entityPlayer);
                return;
            }
    ...
    public void onItemUseFinish(PlayerUseItemEvent.Finish aEvent) {
        ...
        if (tNBT != null && tNBT.hasKey(NBT_EFFECTS)) {
            ...
            if (RNGSUS.nextInt(100) < tNBT.getInteger("chance")) UT.Entities.applyPotion(
                aEvent.entityPlayer,
                tNBT.getInteger("id"),
                tNBT.getInteger("time"),
                tNBT.getInteger("lvl"),
                F);
        }
        ...
        if (aEvent.item.getItem() == Items.apple) {
            if (IL.GrC_Applecore.exists()) {
                if (aEvent.result == null) aEvent.result = IL.GrC_Applecore.get(1);
                else ST.give(aEvent.entityPlayer, IL.GrC_Applecore.get(1), F);
            } else if (IL.Food_Apple_Red_Core.exists()) {
                if (aEvent.result == null) aEvent.result = IL.Food_Apple_Red_Core.get(1);
                else ST.give(aEvent.entityPlayer, IL.Food_Apple_Red_Core.get(1), F);
            }
        }
    }
    ...
    public void onPlayerInteraction(PlayerInteractEvent aEvent) {
        if (aEvent.entityPlayer == null || aEvent.entityPlayer.worldObj == null
            || aEvent.action == null
            || aEvent.world.provider == null) return;
        ...
        if (aEvent.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
            ...
            if (IL.TF_Trophy.equal(aBlock)) {
                ST.check(
                    aEvent.entityPlayer,
                    ST.make(
                        aBlock.getItemDropped(0, RNGSUS, 0),
                        1,
                        aBlock.getDamageValue(aEvent.world, aEvent.x, aEvent.y, aEvent.z)));
                return;
            }
            ...
            if (aEvent.entityPlayer.worldObj.isRemote && !aEvent.entityPlayer.isSneaking()) {
                if (aTileEntity instanceof PrefixBlockTileEntity) {
                    ...
                    if (aBlock == BlocksGT.oreBedrock || aBlock == BlocksGT.oreSmallBedrock) {
                        ...
                        // RM.BedrockOreList.guiUsesNEI(ST.make((Block)BlocksGT.oreBedrock, 1,
                        // ((PrefixBlockTileEntity)aTileEntity).mMetaData));
                    }
                }
            }
            if (ST.valid(aStack)) {
                // Preventing a Railcraft Crash with Fluid Container Items.
                if (aStack.getItem() instanceof IFluidContainerItem && !aEvent.entityPlayer.isSneaking()
                    && aTileEntity != null
                    && aTileEntity.getClass()
                        .getName()
                        .startsWith("mods.railcraft.common")) {
                    aEvent.setCanceled(T);
                    return;
                }
                /*
                 * I think this was for fixing some Adventure Mode related thing. Probably placing Scaffolds with
                 * Leftclick was broken, but I ended up fixing it another way.
                 * if (MD.IC2.mLoaded && SIDES_HORIZONTAL[aEvent.face] && !aEvent.entityPlayer.isSneaking() &&
                 * !aEvent.entityPlayer.capabilities.allowEdit && !aEvent.world.canPlaceEntityOnSide(aBlock,
                 * aEvent.x+OFFSETS_X[aEvent.face], aEvent.y, aEvent.z+OFFSETS_Z[aEvent.face], F, aEvent.face,
                 * aEvent.entityPlayer, aStack)) {
                ...
                if (!(aStack.getItem() instanceof IItemNoGTOverride)) {
                    // Dollies won't work on GT6 TileEntities, so to prevent a Crash and deleted Resources, I just
                    ...
                    if (IL.JABBA_Dolly.equal(aStack, T, T) || IL.JABBA_Dolly_Diamond.equal(aStack, T, T)) {
                        if (aTileEntity instanceof ITileEntitySpecificPlacementBehavior) {
                            UT.Entities.chat(
                                aEvent.entityPlayer,
                                CHAT_GREG + "The Dolly Code is sadly not smart enough to move this TileEntity.",
                                CHAT_GREG + "It would crash if it actually did, so be glad I prevented your mistake.",
                                CHAT_GREG + "Would be great if it did work though...");
                            aEvent.setCanceled(T);
                        }
                        return;
                    }
                    // Instant breaking for those Wrenches.
                    if (IL.BC_Wrench.equal(aStack, T, T) || IL.FR_Wrench.equal(aStack, T, T)
                        || IL.SC2_Wrench.equal(aStack, T, T)
                        || IL.AE_Wrench_Certus.equal(aStack, T, T)
                        || IL.AE_Wrench_Quartz.equal(aStack, T, T)
                        || IL.TE_Wrench.equal(aStack, T, T)
                        || IL.TE_Wrench_Battle.equal(aStack, T, T)) {
                        List<String> tChatReturn = new ArrayListNoNulls<>();
                        long tDamage = IBlockToolable.Util.onToolClickWithoutCompat(
                            TOOL_wrench,
                            Long.MAX_VALUE,
                            3,
                            aEvent.entityPlayer,
                            ...
                            aEvent.entityPlayer.isSneaking(),
                            aStack,
                    ...
                    if (IL.IE_Hammer.equal(aStack, F, T) || IL.A97_Hammer.equal(aStack, T, T)
                        || IL.SC2_Hammer.equal(aStack, T, T)
                        || IL.SC2_Hammer_Gilded.equal(aStack, T, T)) {
                        List<String> tChatReturn = new ArrayListNoNulls<>();
                        long tDamage = IBlockToolable.Util.onToolClickWithoutCompat(
                            TOOL_hammer,
                            Long.MAX_VALUE,
                    ...
                    if (IL.RC_Crowbar_Iron.equal(aStack, T, T) || IL.RC_Crowbar_Steel.equal(aStack, T, T)
                        || IL.RC_Crowbar_Thaumium.equal(aStack, T, T)
                        || IL.RC_Crowbar_Voidmetal.equal(aStack, T, T)) {
                        List<String> tChatReturn = new ArrayListNoNulls<>();
                        long tDamage = IBlockToolable.Util.onToolClickWithoutCompat(
                            TOOL_crowbar,
                            Long.MAX_VALUE,
                    ...
                    // Should be a good reward for getting to it.
                        ...
                        if (aTileEntity instanceof TileEntityMobSpawner) {
                            ...
                            if (UT.Code.stringValid(tResult)) {
                                if (ST.use(aEvent.entityPlayer, aStack, 16)) {
                                    ...
                                    UT.Entities.sendchat(
                                        aEvent.entityPlayer,
                                        "You need 16 Bags of Transformation Powder to convert this!");
                                }
                            } else {
                                UT.Entities.sendchat(
                                    aEvent.entityPlayer,
                                    "This Spawner does not have a Counterpart to convert into!");
                            }
                            aEvent.setCanceled(T);
                            return;
                        }
        ...
        if (aEvent.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK
            ...
            if (ST.valid(aStack)) {
                ...
                if (aTileEntity instanceof ITileEntityBookShelf
                    && ((ITileEntityBookShelf) aTileEntity).isShelfFace((byte) aEvent.face)) {
                ...
                if (!aEvent.entityPlayer.worldObj.isRemote) {
                    for (int i = 0; i < aEvent.entityPlayer.inventory.mainInventory.length; i++) {
                        ...
                        if (ST.item(tStack) instanceof MultiItem) {
                            List<IBehavior<MultiItem>> tList = ((MultiItem) ST.item_(tStack)).mItemBehaviors
                                .get(ST.meta_(tStack));
    ...
    public void onUseHoeEvent(net.minecraftforge.event.entity.player.UseHoeEvent aEvent) {
        if (aEvent.world.getBlock(aEvent.x, aEvent.y, aEvent.z) == Blocks.dirt
            && aEvent.world.getBlockMetadata(aEvent.x, aEvent.y, aEvent.z) != 0) aEvent.setCanceled(T);
    }
        ...
        if (aEvent.newSpeed > 0) {
            if (aEvent.entityPlayer != null) {
                ...
                if (aStack != null && aStack.getItem() instanceof MultiItemTool) {
                    ...
                    UT.Reflection.setField(
                        aEvent,
                        "originalSpeed",
                        aEvent.newSpeed = ((MultiItemTool) aStack.getItem()).onBlockBreakSpeedEvent(
                            aEvent.newSpeed,
                            aStack,
                            aEvent.entityPlayer,
                            aEvent.block,
            ...
            if (BlocksGT.stoneToBrokenOres.containsKey(tBlock) || BlocksGT.stoneToNormalOres.containsKey(tBlock)
                || BlocksGT.stoneToSmallOres.containsKey(tBlock)) {
                if (aEvent.block.getMaterial() == Material.sand || aEvent.block.getMaterial() == Material.clay
                    || aEvent.block.getMaterial() == Material.grass
                    || aEvent.block.getMaterial() == Material.ground) {
                    // Aether does something stupid so Reflection it is
            ...
            if (aEvent.block instanceof IBlockPlacable) {
                if (BlocksGT.stoneToBrokenOres.containsValue(aEvent.block)
                    || BlocksGT.stoneToNormalOres.containsValue(aEvent.block)
                    || BlocksGT.stoneToSmallOres.containsValue(aEvent.block)) {
                    // Aether does something stupid so Reflection it is
                    UT.Reflection.setField(aEvent, "originalSpeed", aEvent.newSpeed /= HARDNESS_MULTIPLIER_ORES);
                    return;
                }
    ...
    public void onBlockBreakingEvent(BlockEvent.BreakEvent aEvent) {
        if (aEvent.block instanceof IPrefixBlock && EnchantmentHelper.getSilkTouchModifier(aEvent.getPlayer()))
            aEvent.setExpToDrop(0);
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onBlockHarvestingEvent(BlockEvent.HarvestDropsEvent aEvent) {
        Iterator<ItemStack> aDrops = aEvent.drops.iterator();
        Block aBlock = (aEvent.block == Blocks.lit_redstone_ore ? Blocks.redstone_ore
            : aEvent.block == Blocks.lit_redstone_lamp ? Blocks.redstone_lamp
                : aEvent.block == BlocksGT.EtFu_Deepslate_Lit_Redstone_Ore ? BlocksGT.EtFu_Deepslate_Redstone_Ore
                    : aEvent.block);

        while (aDrops.hasNext()) {
            ItemStack aDrop = aDrops.next();
            if (ST.invalid(aDrop) || ItemsGT.ILLEGAL_DROPS.contains(aDrop, T)) {
                aDrops.remove();
                continue;
            }
            if (ST.item_(aDrop) == Items.gold_nugget) ST.meta_(aDrop, 0);
            if (FORCE_GRAVEL_NO_FLINT && aBlock == Blocks.gravel && ST.item_(aDrop) == Items.flint)
                ST.set(aDrop, ST.make(Blocks.gravel, 1, 0), T, F);
        }

        if (aBlock == null) return;

        if (aBlock == Blocks.dirt && aEvent.blockMetadata == 1)
            for (int i = 0, j = aEvent.drops.size(); i < j; i++) if (ST.block(aEvent.drops.get(0)) == Blocks.dirt) {
        ...
        if (IL.TF_Mushgloom_Huge.equal(aBlock)) {
            aEvent.drops.clear();
            aEvent.drops.add(
                aEvent.isSilkTouching ? IL.TF_Mushgloom_Huge.get(1)
                    : IL.TF_Mushgloom
                        .get(UT.Code.bind(1, 4, RNGSUS.nextInt(3) + RNGSUS.nextInt(1 + aEvent.fortuneLevel))));
        }
        ...
        if (aEvent.harvester != null) {
            ...
            if (aTool != null) {
                boolean tFireAspect = (UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, aTool) >= 3),
                    tCanCollect = (ST.item_(aTool) instanceof MultiItemTool && ((MultiItemTool) ST.item_(aTool))
                        .canCollectDropsDirectly(aTool, aBlock, (byte) aEvent.blockMetadata));

                if (ST.item_(aTool) instanceof MultiItemTool) {
                    ((MultiItemTool) ST.item_(aTool)).onHarvestBlockEvent(
                        aEvent.drops,
                        aTool,
                        aEvent.harvester,
                        aBlock,
                ...
                for (ItemStack tDrop : aEvent.drops) {
                    ItemStack tTarget = (aEvent.isSilkTouching ? BlocksGT.blockToSilk : BlocksGT.blockToDrop)
                        .get(tDrop);
                    ...
                    if (ST.valid(tTarget)) {
                        ...
                        if (tWoodEntry != null && tWoodEntry.mCharcoalCount > 0) {
                            ...
                            if (tBeamEntry != null && tBeamEntry.mCharcoalCount > 0) {
                                ST.set(
                                    tDrop,
                                    OP.gem.mat(MT.Charcoal, tBeamEntry.mCharcoalCount * tDrop.stackSize),
                                    T,
                                    F);
                            }
                        }
                    ...
                    while (aDrops.hasNext()) {
                        ...
                        if (ST.valid(aDrop)) {
                            ...
                            if (tEntity != null) {
                                ...
                                if (tEvent.getResult() == Result.ALLOW || tEntity.isDead
                                    || aDrop.stackSize <= 0
                                    || ST.invalid(aDrop)) {
                                    aDrops.remove();
                                    ...
                                    if (aCollectSound) {
                                        UT.Sounds.send(
                                            SFX.MC_COLLECT,
                                            0.2F,
                                            ((RNGSUS.nextFloat() - RNGSUS.nextFloat()) * 0.7F + 1.0F) * 2.0F,
                                            aEvent.harvester);
                                        aCollectSound = F;
                                    }
                                }
    ...
    public void onEntitySpawningEvent(EntityJoinWorldEvent aEvent) {
        if (aEvent.entity instanceof EntityItem && !aEvent.entity.worldObj.isRemote) {
            ...
            if (ST.valid(aStack) && aStack.stackSize > 0) {
                ...
                try {
                    if (null != aEvent.entity.worldObj.findNearestEntityWithinAABB(
                        (Class<? extends Entity>) Class.forName("drzhark.mocreatures.entity.IMoCEntity"),
                        aEvent.entity.boundingBox.expand(0.5, 1.0, 0.5),
                        aEvent.entity)) {
                        // Replace stupid Wooden and Stone Tools that clutter up Mob Farms for no reason, but only if
                        // nonplayerkill.
                        if (aItem == Items.wooden_sword || aItem == Items.wooden_pickaxe
                            || aItem == Items.wooden_shovel
                            || aItem == Items.wooden_axe
                            || aItem == Items.wooden_hoe) {
                            ST.set(aStack, IL.Stick.get(1));
                        } else if (aItem == Items.stone_sword || aItem == Items.stone_pickaxe
                            || aItem == Items.stone_shovel
                            || aItem == Items.stone_axe
                            || aItem == Items.stone_hoe) {
                ...
                if (((EntityItem) aEvent.entity).lifespan > 1200) {
                    if (ST.item_(aStack) == Items.egg || ST.item_(aStack) == Items.feather
                        || ST.item_(aStack) == Items.apple) {
    ...
    public void onLivingDeath(LivingDeathEvent aEvent) {
        if (aEvent.entityLiving instanceof EntityPlayerMP) NW_API.sendToPlayer(
            new PacketDeathPoint(
                UT.Code.roundDown(aEvent.entityLiving.posX),
                UT.Code.roundDown(aEvent.entityLiving.posY),
                UT.Code.roundDown(aEvent.entityLiving.posZ)),
            (EntityPlayerMP) aEvent.entityLiving);
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onLoginEvent(cpw.mods.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent aEvent) {
        if (DISABLE_ALL_IC2_COMPRESSOR_RECIPES) ic2.api.recipe.Recipes.compressor.getRecipes()
            .clear();
        if (DISABLE_ALL_IC2_EXTRACTOR_RECIPES) ic2.api.recipe.Recipes.extractor.getRecipes()
            .clear();
        if (DISABLE_ALL_IC2_MACERATOR_RECIPES) ic2.api.recipe.Recipes.macerator.getRecipes()
            .clear();
        if (DISABLE_ALL_IC2_OREWASHER_RECIPES) ic2.api.recipe.Recipes.oreWashing.getRecipes()
            .clear();
        if (DISABLE_ALL_IC2_CENTRIFUGE_RECIPES) ic2.api.recipe.Recipes.centrifuge.getRecipes()
            .clear();
    ...
    @Override
    public void generate(Random aRandom, int aChunkX, int aChunkZ, World aWorld, IChunkProvider aChunkGenerator,
    ...
    public void onItemExpireEvent(ItemExpireEvent aEvent) {
        ...
        int aX = UT.Code.roundDown(aEvent.entity.posX), aY = UT.Code.roundDown(aEvent.entity.posY),
            aZ = UT.Code.roundDown(aEvent.entity.posZ);
        if (ST.valid(aStack)) {
            ...
            if (tRegistry != null) {
                ...
                if (tData != null) {
                    if (tData.mPrefix == OP.rockGt || tData.mPrefix == OP.oreRaw) for (byte[] tOff : CUBE_3)
                        if (WD.irrelevant(aEvent.entity.worldObj, aX + tOff[0], aY + tOff[1], aZ + tOff[2])
                            && tRegistry.mBlock.placeBlock(
                    ...
                    if (tData.mPrefix == OP.ingot) for (byte[] tOff : CUBE_3)
                        if (WD.irrelevant(aEvent.entity.worldObj, aX + tOff[0], aY + tOff[1], aZ + tOff[2])
                            && tRegistry.mBlock.placeBlock(
                                aEvent.entity.worldObj,
                                aX + tOff[0],
                    ...
                    if (tData.mPrefix == OP.plate) for (byte[] tOff : CUBE_3)
                        if (WD.irrelevant(aEvent.entity.worldObj, aX + tOff[0], aY + tOff[1], aZ + tOff[2])
                            && tRegistry.mBlock.placeBlock(
                                aEvent.entity.worldObj,
                                aX + tOff[0],
                    ...
                    if (tData.mPrefix == OP.plateGem) for (byte[] tOff : CUBE_3)
                        if (WD.irrelevant(aEvent.entity.worldObj, aX + tOff[0], aY + tOff[1], aZ + tOff[2])
                            && tRegistry.mBlock.placeBlock(
                                aEvent.entity.worldObj,
                                aX + tOff[0],
                    ...
                    if (tData.mPrefix == OP.scrapGt) for (byte[] tOff : CUBE_3)
                        if (WD.irrelevant(aEvent.entity.worldObj, aX + tOff[0], aY + tOff[1], aZ + tOff[2])
                            && tRegistry.mBlock.placeBlock(
                                aEvent.entity.worldObj,
                                aX + tOff[0],
    ...
    public void onCheckSpawnEvent(LivingSpawnEvent.CheckSpawn aEvent) {
        ...
        int aX = UT.Code.roundDown(aEvent.x),
            aY = (int) UT.Code.bind(0, aWorld.getHeight(), UT.Code.roundDown(aEvent.y)),
            aZ = UT.Code.roundDown(aEvent.z);

        if (SPAWN_NO_BATS && aMobClass == EntityBat.class
            && aWorld.getBlock(aX, aY - 2, aZ) != Blocks.stone
            && aWorld.getBlock(aX, aY + 2, aZ) != Blocks.stone) {
        ...
        if (SPAWN_HOSTILES_ONLY_IN_DARKNESS && WD.dimOverworldLike(aWorld)) try {
            ...
            if (tChunk != null && tChunk.getBlockStorageArray() != null
                && tChunk.getBlockStorageArray()[aY >> 4] != null
                && tChunk.getBlockStorageArray()[aY >> 4].getExtBlocklightValue(aX & 15, aY & 15, aZ & 15) > 0) {
                // Vanilla Mobs only, just in case.
                if (aMobClass == EntityCreeper.class || aMobClass == EntityEnderman.class
                    || aMobClass == EntitySkeleton.class
                    || aMobClass == EntityZombie.class
                    || aMobClass == EntitySpider.class
                    || aMobClass == EntityWitch.class
                    || aMobClass == EntityBat.class) {
                    aEvent.setResult(Result.DENY);
                    return;
                }
                // Well, that Zombie is kindof like Vanilla, so it counts.
                if (MD.TC.mLoaded) if (aEvent.entityLiving instanceof EntityBrainyZombie) {
                    aEvent.setResult(Result.DENY);
                    return;
                }
                // TODO Add Drowned and other Et Futurum Requiem Mobs once they are released.
                if (MD.EtFu.mLoaded) if (aEvent.entityLiving instanceof EntityZombieVillager
                    || aEvent.entityLiving instanceof EntityStray
                    || aEvent.entityLiving instanceof EntityHusk) {
                        aEvent.setResult(Result.DENY);
                        return;
                    }
            }
        ...
        if (aWorld.provider.dimensionId == 0 && aY >= WD.waterLevel(aWorld) - 16) {
            if (GENERATE_BIOMES) {
                if (UT.Code.inside(-96, 95, aX) && UT.Code.inside(-96, 95, aZ)) {
                    aEvent.setResult(Result.DENY);
                    return;
                }
            } else if (GENERATE_NEXUS) {
                if (UT.Code.inside(0, 48, aX) && UT.Code.inside(-64, -16, aZ)) {
                    aEvent.setResult(Result.DENY);
                    return;
                }
            }
            if (GENERATE_STREETS && (UT.Code.inside(-48, 48, aX) || UT.Code.inside(-48, 48, aZ))) {
                aEvent.setResult(Result.DENY);
                return;
            }
            if (SPAWN_ZONE_MOB_PROTECTION && UT.Code.inside(
                -144,
                144,
                aX - aWorld.getWorldInfo()
                    .getSpawnX())
                && UT.Code.inside(
                    -144,
                    144,
                    aZ - aWorld.getWorldInfo()
                        .getSpawnZ())
                && WD.opq(aWorld, aX, 0, aZ, F, F)) {
                aEvent.setResult(Result.DENY);
                return;
            }
        }

        // if (aEvent.entity instanceof EntityMob && !(aEvent.entity instanceof IBossDisplayData) &&
        // ((EntityMob)aEvent.entity).getCanSpawnHere()) mMobsToFastDespawn.add((EntityLiving)aEvent.entityLiving);
        ...
        for (int i = 0; i < MOB_SPAWN_INHIBITORS.size(); i++) {
            ...
            if (tTileEntity.isDead()) {
                ...
                if (tTileEntity.inhibitMobSpawn(aEvent, aWorld, aX, aY, aZ)) {
                    aEvent.setResult(Result.DENY);
                    return;
                }
            } catch (Throwable e) {
    ...
    public void onEntityConstructingEvent(EntityConstructing aEvent) {
        if (Abstract_Mod.sFinalized >= Abstract_Mod.sModCountUsingGTAPI && aEvent.entity instanceof EntityPlayer)
            EntityFoodTracker.add((EntityPlayer) aEvent.entity);
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public void onArrowNockEvent(ArrowNockEvent aEvent) {
        if (!aEvent.isCanceled() && ST.valid(aEvent.result)
            && ST.projectile(aEvent.entityPlayer.inventory, TD.Projectiles.ARROW) != null) {
    ...
    public void onArrowLooseEvent(ArrowLooseEvent aEvent) {
        ...
        if (!aEvent.isCanceled() && ST.valid(aEvent.bow) && aArrow != null && aEvent.bow.getItem() instanceof ItemBow) {
            ...
            EntityProjectile tArrowEntity = ((IItemProjectile) aArrow.getItem()).getProjectile(
                TD.Projectiles.ARROW,
                aArrow,
                aEvent.entityPlayer.worldObj,
                aEvent.entityPlayer,
                tSpeed * 2.0F);
            ...
            int tLevel = UT.NBT.getEnchantmentLevel(Enchantment.power, aEvent.bow);
            ...
            aEvent.bow.getItem();
            aEvent.entityPlayer.worldObj.playSoundAtEntity(
                aEvent.entityPlayer,
                "random.bow",
                1.0F,
                1.0F / (RNGSUS.nextFloat() * 0.4F + 1.2F) + tSpeed * 0.5F);
    ...
    public int getBurnTime(ItemStack aFuel) {
        ...
        if (aBlock instanceof BlockRailBase) return 0; // Needed so Railcrafts Tunnel Bore works properly and doesn't
                                                       // try to burn its Rails while laying them.
        ...
        long rFuelValue = UT.NBT.getNBT(aFuel)
            .getLong(NBT_FUEL_VALUE);
        ...
        if (tData != null && (tData.mFurnaceFuel || rFuelValue != 0)) {
            ...
            if (tData.mPrefix == null) {
                for (OreDictMaterialStack tMaterial : tData.getAllMaterialStacks())
                    tBurnTime += UT.Code.units(tMaterial.mMaterial.mFurnaceBurnTime, U, tMaterial.mAmount, F);
                ...
                if (tData.mPrefix == OP.blockPlate && ANY.Wood.mToThis.contains(tData.mMaterial.mMaterial))
                    return (int) UT.Code.bind(0, 32000, Math.max((TICKS_PER_SMELT * 27L) / 2, tBurnTime));
                if (tData.mPrefix == OP.crateGtPlate && ANY.Wood.mToThis.contains(tData.mMaterial.mMaterial))
                    return (int) UT.Code.bind(0, 32000, Math.max((TICKS_PER_SMELT * 51L) / 2, tBurnTime));
                if (tData.mPrefix == OP.crateGt64Plate && ANY.Wood.mToThis.contains(tData.mMaterial.mMaterial))
                    return (int) UT.Code.bind(0, 32000, Math.max((TICKS_PER_SMELT * 195L) / 2, tBurnTime));
            }
