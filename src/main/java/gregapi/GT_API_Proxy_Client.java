...
public class GT_API_Proxy_Client extends GT_API_Proxy {
    ...
    public GT_API_Proxy_Client() {
        ...
        for (int i = 0; i < 4; i++) {
            sPosR.addAll(
                Arrays.asList(
                    MT.ChargedCertusQuartz.mRGBa[i],
                    MT.Enderium.mRGBa[i],
                    MT.Vinteum.mRGBa[i],
                    MT.U_235.mRGBa[i],
                    MT.Am_241.mRGBa[i],
                    ...
                    MT.Nq_528.mRGBa[i],
                    MT.Nq_522.mRGBa[i],
                    MT.InfusedOrder.mRGBa[i],
                    MT.Force.mRGBa[i],
                    MT.Pyrotheum.mRGBa[i],
                    MT.Sunnarium.mRGBa[i],
                    MT.Mcg.mRGBa[i],
                    MT.Thaumium.mRGBa[i],
                    MT.InfusedVis.mRGBa[i],
                    MT.InfusedAir.mRGBa[i],
                    MT.InfusedFire.mRGBa[i],
                    MT.FierySteel.mRGBa[i],
                    MT.Fireleaf.mRGBa[i],
                    MT.Firestone.mRGBa[i],
                    MT.ArcaneAsh.mRGBa[i]));
            sPosG.addAll(
                Arrays.asList(
                    MT.ChargedCertusQuartz.mRGBa[i],
                    MT.Enderium.mRGBa[i],
            ...
            sPosB.addAll(
                Arrays.asList(
                    ...
                    MT.Mcg.mRGBa[i],
                    MT.InfusedVis.mRGBa[i],
                    MT.InfusedWater.mRGBa[i],
                    MT.Thaumium.mRGBa[i],
                    MT.Co_60.mRGBa[i],
                    MT.Lumium.mRGBa[i],
                    MT.VinteumPurified.mRGBa[i],
                    MT.ArcaneAsh.mRGBa[i]));
            ...
            sRainbow.addAll(
                Arrays.asList(
                    MT.GaiaSpirit.mRGBa[i],
                    MT.GaiaSpirit.mRGBa[i],
                    MT.Shimmerwood.mRGBa[i],
                    MT.Shimmerwood.mRGBa[i],
                    MT.Chimerite.mRGBa[i]));
    ...
    public void onProxyAfterPreInit(Abstract_Mod aMod, FMLPreInitializationEvent aEvent) {
        ...
        ITexture.Util.OPTIFINE_LOADED = FMLClientHandler.instance()
            .hasOptifine();
        ...
        switch (tDate.getMonth() + 1) {// Not going to use Calendar, because it fucking crashes with Missing Resource
                                       // Exception...
            case 1:
    ...
    public void onProxyAfterPostInit(Abstract_Mod aMod, FMLPostInitializationEvent aEvent) {
        ...
        for (int i = 0; i < PlankData.PLANKS.length; i++) {
            ...
            if (tBlock != null && tBlock != NB)
                PlankData.PLANK_ICONS[i] = new IconContainerCopied(tBlock, ST.meta_(PlankData.PLANKS[i]), SIDE_ANY);
        }
    }

    public static final List<short[]> sRainbow = new ArrayListNoNulls<>(), sRainbowFast = new ArrayListNoNulls<>(),
        sPosR = new ArrayListNoNulls<>(), sPosG = new ArrayListNoNulls<>(), sPosB = new ArrayListNoNulls<>(),
        sPosA = new ArrayListNoNulls<>(), sNegR = new ArrayListNoNulls<>(), sNegG = new ArrayListNoNulls<>(),
        sNegB = new ArrayListNoNulls<>(), sNegA = new ArrayListNoNulls<>();

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onTextureStitchedPre(TextureStitchEvent.Pre aEvent) {
        // You should thank me for fixing this Fluid Bug. Seriously, some people just don't set the Icons of their
        // registered Fluids...
        for (Fluid aFluid : FluidRegistry.getRegisteredFluids()
            .values()) {
            // Check if it is whitelisted first, because those are not actually broken, they just behave like that early
            // on.
            if (!FluidsGT.BORKEN.contains(aFluid.getName())) {
                ...
                if (aFluid.getIcon() == null || FluidsGT.BROKEN.contains(aFluid.getName())) try {
                    ...
                    try {
                        aFluid.setIcons(Blocks.water.getIcon(0, 0));
                    } catch (Throwable f) {
    ...
    public void onItemTooltip(ItemTooltipEvent aEvent) {
        ...
        try {
            if (UT.NBT.getNBT(aEvent.itemStack)
                .getBoolean("gt.err.oredict.output")) {
                aEvent.toolTip.clear();
                aEvent.toolTip.add(
                    0,
                    LH.Chat.BLINKING_RED
                        + "A Recipe used an OreDict Item as Output directly, without copying it before!");
                aEvent.toolTip.add(
                    1,
                    LH.Chat.BLINKING_RED
                        + "This is a typical CallByReference/CallByValue Error of the Modder doing it.");
                aEvent.toolTip.add(
                    2,
                    LH.Chat.BLINKING_RED
            ...
            if (aEvent.itemStack.getTagCompound() == null) {
                if (aBlock == Blocks.dirt && aBlockMeta == 1) {
                    aEvent.toolTip.set(
                        0,
                        aEvent.toolTip.get(0)
                            .replaceAll("Dirt", "Coarse Dirt"));
                }
                if (MD.RC.mLoaded && "Railcraft:part.plate".equalsIgnoreCase(aRegName)) {
                    switch (aMeta) {
                        case 0:
                            aEvent.toolTip.set(0, LH.Chat.WHITE + LH.get("oredict.plateIron"));
                            break;
                        case 1:
                            aEvent.toolTip.set(0, LH.Chat.WHITE + LH.get("oredict.plateSteel"));
                            break;
                        case 2:
                            aEvent.toolTip.set(0, LH.Chat.WHITE + LH.get("oredict.plateTinAlloy"));
                            break;
                        case 3:
                            aEvent.toolTip.set(0, LH.Chat.WHITE + LH.get("oredict.plateCopper"));
                            break;
                        case 4:
                            aEvent.toolTip.set(0, LH.Chat.WHITE + LH.get("oredict.plateLead"));
                            break;
                    }
                }
            }
            ...
            if (MD.Mek.owns(aRegName)) aEvent.toolTip.set(
                0,
                aEvent.toolTip.get(0)
                    .replaceAll("Osmium", MT.Ge.mNameLocal));
            if (MD.BP.owns(aRegName)) aEvent.toolTip.set(
                0,
                aEvent.toolTip.get(0)
                    .replaceAll("Infused Teslatite", MT.PurpleAlloy.mNameLocal)
                    .replaceAll("Teslatite", MT.Nikolite.mNameLocal));
            if (MD.BP.mLoaded) aEvent.toolTip.set(
                0,
                aEvent.toolTip.get(0)
                    .replaceAll("Teslatite", MT.Nikolite.mNameLocal));
            ...
            if (!(aItem instanceof ItemFluidDisplay) && SHOW_INTERNAL_NAMES) {
                if (tData != null && tData.validData()) {
                    if (tData.mBlackListed) {
                        if (ST.isGT(aItem)) aEvent.toolTip.add(1, LH.Chat.ORANGE + tData.toString());
                        else aEvent.toolTip.add(
                            1,
                            LH.Chat.DCYAN + aRegName
                                + LH.Chat.WHITE
                                + " - "
                                + LH.Chat.CYAN
                                + aMeta
                        ...
                        if (ST.isGT(aItem)) aEvent.toolTip.add(1, LH.Chat.GREEN + tData.toString());
                        else aEvent.toolTip.add(
                            1,
                            LH.Chat.DCYAN + aRegName
                                + LH.Chat.WHITE
            ...
            if (ItemsGT.RECIPE_REMOVED_USE_TRASH_BIN_INSTEAD.contains(aEvent.itemStack, T)) {
                aEvent.toolTip
                    .add(LH.Chat.BLINKING_RED + "Recipe has been removed in favour of the GregTech Ender Garbage Bin");
            }
            ...
            if (aBlock != NB) {
                if (IL.TC_Warded_Glass.equal(aEvent.itemStack, F, T)) {
                    ...
                    aEvent.toolTip.add(
                        LH.getToolTipHarvest(
                            aBlock.getMaterial(),
                            aBlock.getHarvestTool(aBlockMeta),
                            aBlock.getHarvestLevel(aBlockMeta)));
                }
            ...
            if (tBurnValue > 0) aEvent.toolTip.add(
                LH.Chat.RED + LH.get(LH.TOOLTIP_FURNACE_FUEL)
                    + LH.Chat.WHITE
                    + tBurnValue
                    + " ("
                    + (tBurnValue * EU_PER_FURNACE_TICK)
                    + LH.Chat._RED
                    + "HU"
                    + LH.Chat.WHITE
                    + ")");
            ...
            if (tData != null) {
                if (tData.validPrefix()) {
                    for (IOreDictListenerItem tListener : tData.mPrefix.mListenersItem) {
                        String tToolTip = tListener
                            .getListenerToolTip(tData.mPrefix, tData.mMaterial.mMaterial, aEvent.itemStack);
                        if (tToolTip != null) aEvent.toolTip.add(tToolTip);
                    }
                } else {
                    if (IL.RC_Firestone_Refined.equal(aEvent.itemStack, T, T)) aEvent.toolTip.add(
                        LH.Chat.CYAN + "GT6 Burning Boxes: "
                            + LH.Chat.WHITE
                            + (800 * EU_PER_LAVA)
                            + LH.Chat._RED
                            + "HU"
                            + LH.Chat._CYAN
                            + "per Lava Block");
                    else if (IL.RC_Firestone_Cracked.equal(aEvent.itemStack, T, T)) aEvent.toolTip.add(
                        LH.Chat.CYAN + "GT6 Burning Boxes: "
                            + LH.Chat.WHITE
                            + (600 * EU_PER_LAVA)
                            + LH.Chat._RED
                        ...
                        aEvent.toolTip.add(LH.Chat.CYAN + "Repairable with Knightmetal Ingots on the Vanilla Anvil");
                    else if (IL.TF_Sword_Giant.equal(aEvent.itemStack, T, T))
                        aEvent.toolTip.add(LH.Chat.CYAN + "Repairable with Ironwood Ingots on the Vanilla Anvil");
                ...
                if (tData.validMaterial()) {
                    ...
                    if (tData.mMaterial.mMaterial.mToolTypes > 0 && (tData.mPrefix != null
                        || (aEvent.itemStack.getMaxStackSize() > 1 && tData.mByProducts.length == 0
                            && tData.mMaterial.mAmount <= U))) {
                        aEvent.toolTip.add(
                            LH.Chat.BLUE + "Q: "
                                + tData.mMaterial.mMaterial.mToolQuality
                                + " - S: "
                                + tData.mMaterial.mMaterial.mToolSpeed
                                + " - D: "
                                + tData.mMaterial.mMaterial.mToolDurability);
                    }
                    if (SHOW_CHEM_FORMULAS && UT.Code.stringValid(tData.mMaterial.mMaterial.mTooltipChemical)
                        && (tData.mPrefix == null ? tData.mByProducts.length == 0
                            : tData.mPrefix.contains(TD.Prefix.TOOLTIP_MATERIAL))) {
                    ...
                    if (tData.mMaterial.mMaterial == MT.Nikolite) {
                        aEvent.toolTip.set(
                            0,
                            aEvent.toolTip.get(0)
                                .replaceAll("(Teslatite|Electrotine)", MT.Nikolite.mNameLocal));
                    }
                    ...
                    if (tData.validPrefix()) {
                        if (!ST.isGT(aItem) && tData.mPrefix == OP.dustTiny
                            && ANY.Blaze.mToThis.contains(tData.mMaterial.mMaterial)) {
                            aEvent.toolTip.set(
                                0,
                                aEvent.toolTip.get(0)
                                    .replaceAll(
                                        tData.mMaterial.mMaterial.mNameLocal,
                                        OP.dustTiny.mMaterialPre + tData.mMaterial.mMaterial.mNameLocal));
                        }
                        if (tData.mPrefix.contains(TD.Prefix.NEEDS_SHARPENING))
                            aEvent.toolTip.add(LH.Chat.CYAN + LH.get(LH.TOOLTIP_NEEDS_SHARPENING));
                        if (tData.mPrefix.contains(TD.Prefix.NEEDS_HANDLE)) aEvent.toolTip.add(
                            LH.Chat.CYAN + LH.get(LH.TOOLTIP_NEEDS_HANDLE)
                                + LH.Chat.WHITE
                                + tData.mMaterial.mMaterial.mHandleMaterial.getLocal());

                        if (!tData.mMaterial.mMaterial.mSourceOf.isEmpty()
                            && tData.mPrefix.containsAny(TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_DIRTY)) {
                            StringBuilder tToolTip = null;
                            for (OreDictMaterial tMaterial : tData.mMaterial.mMaterial.mSourceOf) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.CYAN).append("Source of: ")
                                    .append(LH.Chat.WHITE);
                                else tToolTip.append(", ");
                        ...
                        for (AdvancedCrafting1ToY tHandler : tData.mPrefix.mShapelessManagersSingle)
                            if (tHandler.hasOutputFor(tData.mMaterial.mMaterial)) tShapelessAmounts.add(1);
                        for (AdvancedCraftingXToY tHandler : tData.mPrefix.mShapelessManagers)
                            if (tHandler.hasOutputFor(tData.mMaterial.mMaterial))
                                tShapelessAmounts.add(tHandler.mInputCount);
                        if (!tShapelessAmounts.isEmpty()) {
                            Collections.sort(tShapelessAmounts);
                            aEvent.toolTip.add(
                                LH.Chat.CYAN + LH.get(LH.TOOLTIP_SHAPELESS_CRAFT) + LH.Chat.WHITE + tShapelessAmounts);
                        }
                        if (tData.mPrefix.contains(TD.Prefix.TOOLTIP_ENCHANTS)) {
                            ...
                            for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentTools) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                    .append(LH.get(LH.TOOLTIP_POSSIBLE_TOOL_ENCHANTS))
                                    .append(LH.Chat.PINK);
                                else tToolTip.append(", ");
                                tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                                if (tEnchantment.mObject == Enchantment.fireAspect && tEnchantment.mAmount >= 3)
                                    tToolTip.append(" (Autosmelt)");
                            }
                            if (tToolTip != null) aEvent.toolTip.add(tToolTip.toString());
                            tToolTip = null;
                            for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentWeapons) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                    .append(LH.get(LH.TOOLTIP_POSSIBLE_WEAPON_ENCHANTS))
                                    .append(LH.Chat.PINK);
                                else tToolTip.append(", ");
                                tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                            }
                            ...
                            for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentAmmo) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                    .append(LH.get(LH.TOOLTIP_POSSIBLE_AMMO_ENCHANTS))
                                    .append(LH.Chat.PINK);
                                else tToolTip.append(", ");
                                tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                            }
                            ...
                            for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentRanged) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                    .append(LH.get(LH.TOOLTIP_POSSIBLE_RANGED_ENCHANTS))
                                    .append(LH.Chat.PINK);
                                else tToolTip.append(", ");
                                tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                            }
                            ...
                            for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentFishing) {
                                if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                    .append(LH.get(LH.TOOLTIP_POSSIBLE_FISHING_ENCHANTS))
                                    .append(LH.Chat.PINK);
                                else tToolTip.append(", ");
                                tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                            }
                            ...
                            if (!tData.mPrefix.containsAny(
                                TD.Prefix.TOOL_HEAD,
                                TD.Prefix.WEAPON_ALIKE,
                                TD.Prefix.AMMO_ALIKE,
                                TD.Prefix.TOOL_ALIKE)) {
                                tToolTip = null;
                                for (ObjectStack<Enchantment> tEnchantment : tData.mMaterial.mMaterial.mEnchantmentArmors) {
                                    if (tToolTip == null) tToolTip = new StringBuilder(LH.Chat.PURPLE)
                                        .append(LH.get(LH.TOOLTIP_POSSIBLE_ARMOR_ENCHANTS))
                                        .append(LH.Chat.PINK);
                                    else tToolTip.append(", ");
                                    tToolTip.append(tEnchantment.mObject.getTranslatedName((int) tEnchantment.mAmount));
                                }
                        ...
                        if (aBlock == NB
                            || !(aBlock instanceof MultiTileEntityBlockInternal || aBlock instanceof IBlockBase)) {
                ...
                if (aEvent.showAdvancedItemTooltips) {
                    boolean temp = T;
                    for (OreDictMaterialStack tMaterial : tData.getAllMaterialWeights()) if (tMaterial.mAmount != 0
                        && !tMaterial.mMaterial.contains(TD.Properties.DONT_SHOW_THIS_COMPONENT)) {
                            if (temp) {
                            ...
                            tString.append(LH.Chat.WHITE)
                                .append(UT.Code.displayUnits(tMaterial.mAmount))
                                .append(" ");
                            tString.append(LH.Chat.YELLOW)
                                .append(tMaterial.mMaterial.getLocal());
                            tString.append(LH.Chat.WHITE)
                                .append(" (");
                            tString.append(LH.Chat.CYAN)
                                .append("M: ");
                            tString.append(LH.Chat.WHITE)
                                .append(tMaterial.mMaterial.mMeltingPoint);
                            tString.append(LH.Chat.RED)
                                .append("K ");
                            tString.append(LH.Chat.CYAN)
                                .append(" B: ");
                            tString.append(LH.Chat.WHITE)
                                .append(tMaterial.mMaterial.mBoilingPoint);
                            tString.append(LH.Chat.RED)
                                .append("K ");
                            tString.append(LH.Chat.CYAN)
                                .append(" W: ");
                            tString.append(LH.Chat.WHITE)
                                ...
                                .append(
                                    tWeight < 1 ? "000"
                                        : tWeight < 10 ? "00" + tWeight : tWeight < 100 ? "0" + tWeight : tWeight);
                            tString.append(LH.Chat.YELLOW)
                                .append("kg");
                            tString.append(LH.Chat.WHITE)
                                .append(")");
                ...
                if (tData.validData()) {
                    if (ST.isGT(aItem)) {
                        if (tData.mMaterial.mMaterial.mOriginalMod == null) {
                            ...
                            aEvent.toolTip
                                .add(LH.Chat.BLUE + "Material from " + tData.mMaterial.mMaterial.mOriginalMod.mName);
                        }
                    } else {
                        if ((tData.mMaterial.mMaterial == MT.Fe || tData.mMaterial.mMaterial == MT.Fe2O3)
                            && tData.mPrefix.containsAny(TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_BASED)
                            && !aEvent.toolTip.get(0)
                                .contains("Native")) {
                            aEvent.toolTip.set(
                                0,
                                aEvent.toolTip.get(0)
                                    .replaceAll("Banded Iron", MT.Fe2O3.mNameLocal)
                                    .replaceAll("Iron", MT.Fe2O3.mNameLocal));
                        }
                        if (tData.mMaterial.mMaterial == MT.Au
                            && tData.mPrefix.containsAny(TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_BASED)
                            && !aEvent.toolTip.get(0)
                                .contains("Native")) {
                            aEvent.toolTip.set(
                                0,
                                aEvent.toolTip.get(0)
                                    .replaceAll("Gold", "Native Gold"));
                        }
                        if (tData.mMaterial.mMaterial == MT.Cu
                            && tData.mPrefix.containsAny(TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_BASED)
                            && !aEvent.toolTip.get(0)
                                .contains("Native")) {
                            aEvent.toolTip.set(
                                0,
                                aEvent.toolTip.get(0)
                                    .replaceAll("Copper", "Native Copper"));
                        }
                    }
                }
            }
            ...
            for (int i = 1, j = aEvent.toolTip.size(); i < j; i++) {
                ...
                if (tTooltip == null || LH.Chat.BASICALLY_EMPTY_STRINGS.contains(tTooltip)) {
                    aEvent.toolTip.remove(i--);
                    j--;
                } else aEvent.toolTip.set(i, tTooltip + LH.Chat.RESET_TOOLTIP);
            }
        } catch (Throwable e) {
            e.printStackTrace(ERR);
        }
    ...
    public void onClientTickEvent(ClientTickEvent aEvent) {
        if (aEvent.phase == Phase.END) {
            if (CLIENT_TIME == 10) {
                ...
                if (FL.XP.exists()) for (Object tObject : FurnaceRecipes.smelting()
                    .getSmeltingList()
                    .keySet()) if (tObject instanceof ItemStack) {
                        RM.Furnace.addFakeRecipe(
                            F,
                            RM.Furnace
                                .findRecipe(null, null, F, Long.MAX_VALUE, NI, ZL_FS, ST.array((ItemStack) tObject)));
                    }
                // Now for hiding stuff from NEI that should have never been there in the first place.
                if (!SHOW_MICROBLOCKS && NEI)
                    for (Item aItem : new Item[] { ST.item(MD.FMB, "microblock"), ST.item(MD.ExU, "microblocks"),
                        ST.item(MD.ExS, "microblocks"), ST.item(MD.AE, "item.ItemFacade") }) if (aItem != null) {
                            ST.hide(aItem);
            ...
            for (int i = 0; i < UT.Sounds.sPlayedSounds.size(); i++)
                if (UT.Sounds.sPlayedSounds.get(i).mTimer-- < 0) UT.Sounds.sPlayedSounds.remove(i--);
            ...
            // Regardless of whether all the Sounds actually played, clear the List, we don't want any randomly delayed
            // junk showing up.
            ...
            boolean tNR = UT.Code.inside(0, 99, (CLIENT_TIME / 2) % 300),
                tNG = UT.Code.inside(50, 149, (CLIENT_TIME / 2) % 300),
                tNB = UT.Code.inside(100, 199, (CLIENT_TIME / 2) % 300),
                tPR = UT.Code.inside(100, 199, (CLIENT_TIME / 2) % 300),
                tPG = UT.Code.inside(150, 249, (CLIENT_TIME / 2) % 300),
                tPB = UT.Code.inside(200, 299, (CLIENT_TIME / 2) % 300);
    ...
    public void onDrawBlockHighlight(DrawBlockHighlightEvent aEvent) {
        Block aBlock = ST.block(aEvent.player.getCurrentEquippedItem());
        ...
        TileEntity aTileEntity = WD
            .te(aEvent.player.worldObj, aEvent.target.blockX, aEvent.target.blockY, aEvent.target.blockZ, T);
        if (!(aTileEntity instanceof ITileEntityOnDrawBlockHighlight)
            || !((ITileEntityOnDrawBlockHighlight) aTileEntity).onDrawBlockHighlight(aEvent)) {
            if ((ROTATABLE_VANILLA_BLOCKS.contains(aBlock)
                || (ToolCompat.IC_WRENCHABLE && aTileEntity instanceof ic2.api.tile.IWrenchable))
                && ST.valid(aEvent.currentItem)
                && ToolsGT.contains(TOOL_wrench, aEvent.currentItem)) {
                RenderHelper.drawWrenchOverlay(
                    aEvent.player,
                    aEvent.target.blockX,
                    aEvent.target.blockY,
    ...
    private static List<Block> ROTATABLE_VANILLA_BLOCKS = Arrays.asList(
        Blocks.piston,
