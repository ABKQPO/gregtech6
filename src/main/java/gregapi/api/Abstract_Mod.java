...
public abstract class Abstract_Mod {
    ...
    public static int sStartedPreInit = 0, sFinishedPreInit = 0, sStartedInit = 0, sFinishedInit = 0,
        sStartedPostInit = 0, sFinishedPostInit = 0, sFinalized = 0;
    ...
    public boolean mStartedPreInit = F, mFinishedPreInit = F, mStartedInit = F, mFinishedInit = F, mStartedPostInit = F,
        mFinishedPostInit = F, mFinalized = F;

    /** Contains the amount of Server Start/Stop Phases the Mod has already ran through. */
    public int mStartedServerStarting = 0, mFinishedServerStarting = 0, mStartedServerStarted = 0,
        mFinishedServerStarted = 0, mStartedServerStopping = 0, mFinishedServerStopping = 0, mStartedServerStopped = 0,
        mFinishedServerStopped = 0;

    /**
     * Event Lists where you can hook into the loading order of the Code, without having to care much about regular Mod
     * load order. Note, that these Lists will be cleared and then set to null, right after they got executed once, in
     * order to clean up some RAM.
     */
    public List<Runnable> mBeforePreInit = new ArrayListNoNulls<>(), mAfterPreInit = new ArrayListNoNulls<>(),
    ...
    public final List<Runnable> mBeforeServerStarting = new ArrayListNoNulls<>(),
        mAfterServerStarting = new ArrayListNoNulls<>(), mBeforeServerStarted = new ArrayListNoNulls<>(),
        mAfterServerStarted = new ArrayListNoNulls<>(), mBeforeServerStopping = new ArrayListNoNulls<>(),
        mAfterServerStopping = new ArrayListNoNulls<>(), mBeforeServerStopped = new ArrayListNoNulls<>(),
        mAfterServerStopped = new ArrayListNoNulls<>();
    ...
    public void loadRunnables(List<Runnable> aList) {
        for (Runnable tRunnable : aList) try {
            tRunnable.run();
        } catch (Throwable e) {
            e.printStackTrace(ERR);
        }
    ...
    public void onModPreInit(FMLPreInitializationEvent aEvent) {
        ...
        try {
            ...
            if (sFinishedPreInit >= sModCountUsingGTAPI) for (Abstract_Mod tMod : MODS_USING_GT_API) try {
    ...
    public void onModInit(FMLInitializationEvent aEvent) {
        ...
        try {
            ...
            loadRunnables("Before Init", mBeforeInit);
            mBeforeInit.clear();
            mBeforeInit = null;
            ...
            loadRunnables("After Init", mAfterInit);
            mAfterInit.clear();
            mAfterInit = null;

            loadRunnables("Saving Configs", sConfigs);

            if (sFinishedInit >= sModCountUsingGTAPI) for (Abstract_Mod tMod : MODS_USING_GT_API) try {
    ...
    public void onModPostInit(FMLPostInitializationEvent aEvent) {
        ...
        try {
            ...
            if (sFinishedPostInit >= sModCountUsingGTAPI) for (Abstract_Mod tMod : MODS_USING_GT_API) try {
