...
public class NEI_GT_API_Config implements codechicken.nei.api.IConfigureNEI, Runnable {
    ...
    public void run() {
        ...
        if (!NEI_NH) {
            ...
            for (Iterator tIterator : new Iterator[] {
                /* GuiCraftingRecipe.craftinghandlers.iterator(), */GuiUsageRecipe.usagehandlers.iterator() }) {
                while (tIterator.hasNext()) {
                    Object tObject = tIterator.next();
                    if (tObject == null || tObject.getClass()
                        .getName()
                        .contains("codechicken.nei.recipe.FuelRecipeHandler") // This one is just annoying clutter and
                                                                              // badly designed too.
                        || tObject.getClass()
                            .getName()
                            .contains("codechicken.nei.recipe.FurnaceRecipeHandler") // There is a more detailed GT6
                                                                                     // Handler for this.
