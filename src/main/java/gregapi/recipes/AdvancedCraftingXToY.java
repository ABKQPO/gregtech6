...
public class AdvancedCraftingXToY implements ICraftingRecipeGT {
    ...
    public AdvancedCraftingXToY(OreDictPrefix aInput, int aInputCount, OreDictPrefix aOutput, int aOutputCount,
        boolean aAutoCraftable) {
        this(aInput, aInputCount, aOutput, aOutputCount, aAutoCraftable, MT.NULL.NOT);
    }

    public AdvancedCraftingXToY(OreDictPrefix aInput, int aInputCount, OreDictPrefix aOutput, int aOutputCount,
        boolean aAutoCraftable, ICondition aCondition) {
        ...
        try {
            for (int i = 0; i < tRecipeList.size(); i++) {
                ...
                if (tRecipeList.get(i) instanceof ICraftingRecipeGT) {
                    ...
                    if (tInputs != null && tInputs.length >= mInputCount
                        && (mInputCount == 9 || (mInputCount == 4 && tInputs.length == 9
                            && tInputs[2] == null
                            && tInputs[5] == null
                            && tInputs[6] == null
                            && tInputs[7] == null
                            && tInputs[8] == null)))
                        for (Object tObject : tInputs) if (tObject != null) {
                            ...
                            if (tObject instanceof ItemStack) {
                                ...
                                if (tData == null || tData.mPrefix != mInput
                                    ...
                                    || !mCondition.isTrue(tData.mMaterial.mMaterial)) {
                                    tCount = 0;
                                    break;
                                } else if (tCount == 1) {
                    ...
                    if (tInputs != null && tInputs.size() == mInputCount)
                        for (Object tObject : tInputs) if (tObject != null) {
                            tCount++;
                            if (tObject instanceof ItemStack) {
                                OreDictItemData tData = OM.data((ItemStack) tObject);
                    ...
                    if (tInputs != null && tInputs.length >= mInputCount
                        && (mInputCount == 9 || (mInputCount == 4 && tInputs.length == 9
                            ...
                            && tInputs[7] == null
                            && tInputs[8] == null)))
                        for (ItemStack tObject : tInputs) if (tObject != null) {
                            if (++tCount > mInputCount) {
                                tCount = 0;
                                break;
                            }
    ...
    public boolean matches(InventoryCrafting aGrid, World aWorld) {
        ...
        for (int i = 0; i < tInventorySize; i++) {
            ...
            if (ST.valid(tStack)) {
                ...
                if (rMaterial == null) rMaterial = tData.mMaterial.mMaterial;
                else if (rMaterial != tData.mMaterial.mMaterial) return F;
                tCounter++;
                continue;
            }
