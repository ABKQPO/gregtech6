...
public class AdvancedCraftingShapeless extends ShapelessOreRecipe implements ICraftingRecipeGT {
    ...
    public AdvancedCraftingShapeless(ItemStack aResult, boolean aDismantleAble, boolean aRemovableByGT,
        ...
        if (mKeepingNBT) {
            ...
            for (int i = 0; i < aGrid.getSizeInventory(); i++) {
                if (aGrid.getStackInSlot(i) != null) {
                    if (tMainInput == null) {
                        if (getInput().get(0) instanceof Iterable)
                            for (Object tObject : (Iterable) getInput().get(0)) if (tObject instanceof ItemStack) {
    ...
    public ItemStack getCraftingResult(InventoryCrafting aGrid) {
        ...
        if (rStack != null) {
            ...
            // Keeping NBT
            if (mKeepingNBT) {
                ItemStack tMainInput = ((getInput().get(0) instanceof ItemStack) ? (ItemStack) getInput().get(0)
                    : null);
                for (int i = 0; i < aGrid.getSizeInventory(); i++) {
                    if (aGrid.getStackInSlot(i) != null && aGrid.getStackInSlot(i)
                        .hasTagCompound()
                        && (tMainInput == null || ST.equal_(aGrid.getStackInSlot(i), tMainInput, T))) {
                        UT.NBT.set(
                            rStack,
            ...
            if (mDismantleable) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    if (tStack != null && ST.container(tStack, true) == null
