...
public class RecipeMapHandlerMaterial extends RecipeMapHandler {
    ...
    public RecipeMapHandlerMaterial(OreDictMaterial aInputMaterial, FluidStack aFluidInputPerUnit, long aEUt,
        long aDuration, FluidStack aFluidOutputPerUnit, OreDictMaterial aOutputMaterial, ItemStack aAdditionalInput,
        boolean aAllowToGenerateAllRecipesAtOnce, ICondition aCondition) {
    ...
    public boolean addRecipesUsing(RecipeMap aMap, boolean aNEI, ItemStack aStack, OreDictItemData aData) {
        ...
        return aData != null && aData.validData()
            && aData.mMaterial.mMaterial == mInputMaterial
            && addRecipeForPrefix(aMap, aData.mPrefix);
    }
    ...
    public boolean addRecipesProducing(RecipeMap aMap, boolean aNEI, ItemStack aStack, OreDictItemData aData) {
        if (isDone()) return F;
        return aData != null && aData.validData()
            && aData.mMaterial.mMaterial == mOutputMaterial
            && addRecipeForPrefix(aMap, aData.mPrefix);
    }
    ...
    public boolean addRecipeForPrefix(RecipeMap aMap, OreDictPrefix aPrefix) {
        ...
        return aMap.addRecipeX(
            ...
            T,
            mEUt,
            Math.max(1, getCosts(aPrefix)),
            tInputs,
            FL.mul(mFluidInputPerUnit, aPrefix.mAmount, U, T),
            FL.mul(mFluidOutputPerUnit, aPrefix.mAmount, U, F),
            tOutput) != null;
    }
