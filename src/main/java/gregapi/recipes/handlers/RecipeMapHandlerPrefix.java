...
public class RecipeMapHandlerPrefix extends RecipeMapHandler {
    ...
    public RecipeMapHandlerPrefix(OreDictPrefix aInputPrefix, long aInputAmount, FluidStack aFluidInputPerUnit,
        long aEUt, long aDuration, long aMultiplier, FluidStack aFluidOutputPerUnit, OreDictPrefix aOutputPrefix,
        long aOutputAmount, ItemStack aAdditionalInput, ItemStack aAdditionalOutput,
        boolean aAllowToGenerateAllRecipesAtOnce, boolean aOutputPulverizedRemains, boolean aFlatFluidCosts,
        ...
        tUnitsProcessed = 0;
        for (int i = 0; i < mOutputPrefixes.length; i++)
            tUnitsProcessed += mOutputPrefixes[i].mAmount * mOutputAmounts[i];
        mUnitsOutputted = tUnitsProcessed;
        mOutputPulverizedRemains = (aOutputPulverizedRemains
            && mUnitsInputted - mUnitsOutputted >= OP.dustDiv72.mAmount);
    }

    public RecipeMapHandlerPrefix(OreDictPrefix aInputPrefix1, long aInputAmount1, OreDictPrefix aInputPrefix2,
        long aInputAmount2, FluidStack aFluidInputPerUnit, long aEUt, long aDuration, long aMultiplier,
        FluidStack aFluidOutputPerUnit, OreDictPrefix aOutputPrefix1, long aOutputAmount1, OreDictPrefix aOutputPrefix2,
        long aOutputAmount2, ItemStack aAdditionalInput, ItemStack aAdditionalOutput,
        boolean aAllowToGenerateAllRecipesAtOnce, boolean aOutputPulverizedRemains, boolean aFlatFluidCosts,
        ICondition aCondition) {
        mAllowToGenerateAllRecipesAtOnce = aAllowToGenerateAllRecipesAtOnce;
        mFlatFluidCosts = aFlatFluidCosts;
        ...
        mFluidOutputPerUnit = aFluidOutputPerUnit;
        mCondition = (aCondition == null ? ICondition.TRUE : aCondition);
        mInputPrefixes = aInputPrefix2 == null ? new OreDictPrefix[] { aInputPrefix1 }
            : new OreDictPrefix[] { aInputPrefix1, aInputPrefix2 };
        mInputAmounts = aInputPrefix2 == null ? new byte[] { UT.Code.bindStack(aInputAmount1) }
            : new byte[] { UT.Code.bindStack(aInputAmount1), UT.Code.bindStack(aInputAmount2) };
        mOutputPrefixes = aOutputPrefix2 == null
            ? aOutputPrefix1 == null ? ZL_OREDICTPREFIX : new OreDictPrefix[] { aOutputPrefix1 }
            : new OreDictPrefix[] { aOutputPrefix1, aOutputPrefix2 };
        mOutputAmounts = aOutputPrefix2 == null ? new byte[] { UT.Code.bindStack(aOutputAmount1) }
            : new byte[] { UT.Code.bindStack(aOutputAmount1), UT.Code.bindStack(aOutputAmount2) };
        mAdditionalOutput = aAdditionalOutput;
        mAdditionalInput = aAdditionalInput;
        mMultiplier = aMultiplier;
        mDuration = aDuration;
    ...
    public RecipeMapHandlerPrefix(OreDictPrefix[] aInputPrefixes, long[] aInputAmount, FluidStack aFluidInputPerUnit,
        long aEUt, long aDuration, long aMultiplier, FluidStack aFluidOutputPerUnit, OreDictPrefix[] aOutputPrefixes,
        long[] aOutputAmount, ItemStack aAdditionalInput, ItemStack aAdditionalOutput,
        boolean aAllowToGenerateAllRecipesAtOnce, boolean aOutputPulverizedRemains, boolean aFlatFluidCosts,
        ICondition aCondition) {
        mAllowToGenerateAllRecipesAtOnce = aAllowToGenerateAllRecipesAtOnce;
    ...
    public boolean addRecipesUsing(RecipeMap aMap, boolean aNEI, ItemStack aStack, OreDictItemData aData) {
        if (isDone()) return F;
        if (ST.equal(aStack, mAdditionalInput))
            return aNEI && mAllowToGenerateAllRecipesAtOnce && addAllRecipesInternal(aMap);
        return aData != null && aData.validData()
            && UT.Code.contains(aData.mPrefix, mInputPrefixes)
            && addRecipeForMaterial(aMap, aData.mMaterial.mMaterial);
    }

    @Override
    public boolean addRecipesProducing(RecipeMap aMap, boolean aNEI, ItemStack aStack, OreDictItemData aData) {
        if (isDone()) return F;
        if (ST.equal(aStack, mAdditionalOutput))
            return aNEI && mAllowToGenerateAllRecipesAtOnce && addAllRecipesInternal(aMap);
        return aData != null && aData.validData()
            && (UT.Code.contains(aData.mPrefix, mOutputPrefixes)
                || (mOutputPulverizedRemains && aData.mPrefix == OP.dust))
            && addRecipeForMaterial(aMap, aData.mMaterial.mMaterial);
    }
    ...
    public boolean addRecipesUsing(RecipeMap aMap, boolean aNEI, Fluid aFluid) {
        if (isDone()) return F;
        return aNEI && mAllowToGenerateAllRecipesAtOnce
            && mFluidInputPerUnit != null
            && mFluidInputPerUnit.getFluid() == aFluid
            && addAllRecipesInternal(aMap);
    }
    ...
    public boolean addRecipesProducing(RecipeMap aMap, boolean aNEI, Fluid aFluid) {
        if (isDone()) return F;
        return aNEI && mAllowToGenerateAllRecipesAtOnce
            && mFluidOutputPerUnit != null
            && mFluidOutputPerUnit.getFluid() == aFluid
            && addAllRecipesInternal(aMap);
    }
    ...
    public boolean onAddedToMap(RecipeMap aMap) {
        if (mFluidInputPerUnit != null) {
            aMap.mMaxFluidInputSize = Math.max(
                mFlatFluidCosts ? mFluidInputPerUnit.amount : mFluidInputPerUnit.amount * 4,
                aMap.mMaxFluidInputSize);
            Long tSize = aMap.mMinInputTankSizes.get(
                mFluidInputPerUnit.getFluid()
                    .getName());
            if (tSize == null || tSize < mFluidInputPerUnit.amount) aMap.mMinInputTankSizes.put(
                mFluidInputPerUnit.getFluid()
                    .getName(),
                (long) mFluidInputPerUnit.amount);
        }
    ...
    public boolean addRecipeForMaterial(RecipeMap aMap, OreDictMaterial aMaterial) {
        ...
        for (int i = 0; i < mInputPrefixes.length; i++)
            if (ST.invalid(tInputs[i] = mInputPrefixes[i].mat(aMaterial, mInputAmounts[i]))) return F;

        OreDictMaterial tMaterial = getOutputMaterial(aMaterial);

        ItemStack[] tOutputs = new ItemStack[mOutputPrefixes.length + (mOutputPulverizedRemains ? 1 : 0)
            + (ST.valid(mAdditionalOutput) ? 1 : 0)];
        for (int i = 0; i < mOutputPrefixes.length; i++)
            if (ST.invalid(tOutputs[i] = mOutputPrefixes[i].mat(tMaterial, mOutputAmounts[i]))) return F;
        if (mOutputPulverizedRemains)
            tOutputs[mOutputPrefixes.length] = OM.pulverize(aMaterial, mUnitsInputted - mUnitsOutputted);
        ...
        return aMap.addRecipeX(
            ...
            F,
            F,
            !aMaterial.contains(TD.Properties.UNUSED_MATERIAL),
            mEUt,
            mDuration <= 0 ? Math.max(1, getCosts(aMaterial)) : mDuration,
            mChances,
            tInputs,
            mFlatFluidCosts ? mFluidInputPerUnit : FL.mul(mFluidInputPerUnit, aMaterial.mToolQuality + 1),
            mFlatFluidCosts ? mFluidOutputPerUnit : FL.mul(mFluidOutputPerUnit, aMaterial.mToolQuality + 1),
            tOutputs) != null;
    }
    ...
    public long getCosts(OreDictMaterial aMaterial) {
        return UT.Code
            .units(Math.max(mUnitsInputted, mUnitsOutputted), U, mMultiplier + mMultiplier * aMaterial.mToolQuality, T);
    }
}
