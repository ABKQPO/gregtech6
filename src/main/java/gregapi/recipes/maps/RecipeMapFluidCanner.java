...
public class RecipeMapFluidCanner extends RecipeMap {
    ...
    public RecipeMapFluidCanner(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (aInputs == null || aInputs.length <= 0 || rRecipe != null || GAPI_POST.mFinishedServerStarted <= 0)
            return rRecipe;
        for (ItemStack tInput : aInputs) if (ST.valid(tInput)) {
            FluidStack tFluid = FL.getFluid(tInput, T);
            if (tFluid != null) {
                return FL.invalid(tFluid) ? null
                    : new Recipe(
                        F,
                        F,
                        F,
                        ST.array(ST.amount(1, tInput)),
                        ST.array(ST.container(tInput, T)),
                        null,
                        null,
                        ZL_FS,
                        FL.array(tFluid),
                ...
                if (MD.GC.mLoaded || MD.GC_GALAXYSPACE.mLoaded) {
                    if (FluidsGT.LIQUID_OXYGEN.contains(FL.regName(aFluids[0]))) {
                        if (IL.GC_OxyTank_1.equal(tInput, T, T) || IL.GC_OxyTank_2.equal(tInput, T, T)
                            || IL.GC_OxyTank_3.equal(tInput, T, T)
                            || IL.GC_OxyTank_4.equal(tInput, T, T)
                            || IL.GC_OxyTank_5.equal(tInput, T, T)
                            || IL.GC_OxyTank_6.equal(tInput, T, T)
                            || IL.GC_OxyTank_7.equal(tInput, T, T)
                            || IL.GC_OxyTank_Env.equal(tInput, T, T)) {
                            ...
                            return new Recipe(
                                ...
                                F,
                                ST.array(ST.amount(1, tInput)),
                                ST.array(ST.copyAmountAndMeta(1, 0, tInput)),
                                null,
                                null,
                                FL.array(tMeta <= 0 ? NF : FL.amount(aFluids[0], UT.Code.units(tMeta, 2700, 250, T))),
                                ZL_FS,
                                tMeta <= 0 ? 1 : 64,
                                16,
                                0);
                ...
                if (tFluid != null) return new Recipe(
                    F,
                    F,
                    F,
                    ST.array(ST.amount(1, tInput)),
    ...
    public boolean containsInput(ItemStack aStack, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return aStack != null && (super.containsInput(aStack, aTileEntity, aSpecialSlot)
