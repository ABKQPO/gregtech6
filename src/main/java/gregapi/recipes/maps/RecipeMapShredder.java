...
public class RecipeMapShredder extends RecipeMapSpecialSingleInput {
    ...
    public RecipeMapShredder(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe getRecipeFor(ItemStack aInput) {
        ...
        if (aData == null || (aData.mPrefix != null && (!aData.mPrefix.contains(TD.Prefix.RECYCLABLE) || aData.mPrefix
            .containsAny(TD.Prefix.DUST_BASED, TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_BASED, TD.Prefix.IS_CONTAINER)))
            || (aData.mMaterial != null && aData.mMaterial.mMaterial.contains(TD.Atomic.ANTIMATTER))
            || FL.getFluid(aInput, T) != null) return null;
        List<OreDictMaterialStack> tList = new ArrayListNoNulls<>();
        for (OreDictMaterialStack tMaterial : aData.getAllMaterialStacks())
            if (tMaterial.mMaterial.mTargetPulver.mAmount > 0) OM
                .stack(
                    UT.Code.units(tMaterial.mAmount, U, tMaterial.mMaterial.mTargetPulver.mAmount, F),
                    tMaterial.mMaterial.mTargetPulver.mMaterial)
                .addToList(tList);
        ...
        for (OreDictMaterialStack tMaterial : tList) {
            tDuration += UT.Code.units(
                tMaterial.mAmount,
                U,
                (tMaterial.mMaterial.containsAny(TD.Properties.QUARTZ) ? 64
                    : tMaterial.mMaterial.containsAny(
                        TD.Properties.BRITTLE,
                        TD.Properties.STRETCHY,
                        TD.Properties.BOUNCY,
                        TD.Properties.STONE,
                        TD.Properties.WOOD,
                        TD.Properties.FOOD,
                        TD.Properties.MEAT,
                        TD.Properties.ROTTEN) ? 2 : 256)
                    * Math.max(1, tMaterial.mMaterial.mToolQuality + 1),
                T);
