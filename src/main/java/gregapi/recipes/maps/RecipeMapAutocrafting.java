...
public class RecipeMapAutocrafting extends RecipeMap {
    ...
    public RecipeMapAutocrafting(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (rRecipe != null || aSpecialSlot == null
            || aInputs == null
            || aInputs.length < 1
        ...
        if (ALLOWED_RECIPES.isEmpty()) {
            for (Object tCraftingRecipe : CR.list()) if (tCraftingRecipe instanceof IRecipe) {
                if (!(tCraftingRecipe instanceof ICraftingRecipeGT)
                    || ((ICraftingRecipeGT) tCraftingRecipe).isAutocraftableByGT()) {
        ...
        for (int i = 0, j = RECENT_RECIPES.size(); i < j; i++) {
            if (RECENT_RECIPES.get(i)
                .matches(tCraftInv, aTileEntity == null ? DW : aTileEntity.getWorld())) {
        ...
        return new Recipe(
            ...
            F,
            T,
            tInputs.toArray(ZL_IS),
            tOutputs.toArray(ZL_IS),
            null,
            null,
            null,
            null,
    ...
    public ItemStack[] getBlueprint(IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        ...
        if (IL.Paper_Blueprint_Used.equal(aSpecialSlot, F, T)) {
            ...
            NBTTagCompound tData = aSpecialSlot.getTagCompound()
            ...
            for (byte tSide : ALL_SIDES_VALID_ONLY[aSpecialSlot.hasTagCompound() && aSpecialSlot.getTagCompound()
                .hasKey(NBT_USB_DIRECTION) ? aSpecialSlot.getTagCompound()
                ...
                if (tDelegator.mTileEntity instanceof ITileEntityUSBPort) {
                    NBTTagCompound tData = ((ITileEntityUSBPort) tDelegator.mTileEntity)
    ...
    public Slot_Base getSpecialSlot(ITileEntityInventoryGUI aInventory, int aIndex, int aX, int aY) {
        return new Slot_Whitelist(
            aInventory,
            aIndex,
            aX,
            aY,
            OreDictManager.getOres("gt:autocrafterblueprintitem", F)
                .toArray(ZL_IS)).setTooltip(LH.AUTOCRAFTING_INSERT_BLUEPRINT, LH.Chat.WHITE);
    }
}
