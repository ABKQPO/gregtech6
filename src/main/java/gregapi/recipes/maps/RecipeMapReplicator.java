...
public class RecipeMapReplicator extends RecipeMap {
    ...
    public RecipeMapReplicator(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (rRecipe != null || aInputs == null || aFluids == null || GAPI_POST.mFinishedServerStarted <= 0)
            return rRecipe;
                    ...
                    for (byte tSide : ALL_SIDES_VALID_ONLY[tUSB.hasTagCompound() && tUSB.getTagCompound()
                        ...
                        if (tDelegator.mTileEntity instanceof ITileEntityUSBPort) {
                            tData = ((ITileEntityUSBPort) tDelegator.mTileEntity)
                                .getUSBData(tDelegator.mSideOfTileEntity, 3);
        ...
        if (tUSB != null && tData.hasKey(NBT_REPLICATOR_DATA)) {
            ...
            if (tID > 0 && UT.Code.exists(tID, OreDictMaterial.MATERIAL_ARRAY))
                return getReplicatorRecipe(OreDictMaterial.MATERIAL_ARRAY[tID], tUSB);
        }
        return rRecipe;
    }
    ...
    public static Recipe getReplicatorRecipe(OreDictMaterial aMaterial, ItemStack aUSB) {
        if (aMaterial.contains(TD.Processing.UUM) && !aMaterial.contains(TD.Atomic.ANTIMATTER)) {
            FluidStack[] tMatters = FL.array(
                aMaterial.mNeutrons <= 0 ? NF : FL.MatterNeutral.make(aMaterial.mNeutrons),
                aMaterial.mProtons <= 0 ? NF : FL.MatterCharged.make(aMaterial.mProtons));
            ...
            if (aMaterial.mMeltingPoint <= DEF_ENV_TEMP) {
                ...
                if (FL.nonzero(tFluidOutput)) return new Recipe(
                    F,
                    F,
                    T,
                    ST.array(ST.amount(0, aUSB)),
                    ZL_IS,
                    null,
                    null,
                    tMatters,
                    FL.array(tFluidOutput),
                    tPower,
                    1,
                    0).setNoBuffering();
            }
            ...
            if (ST.invalid(tOutput)) {
                if (ST.invalid(tOutput = OP.gem.mat(aMaterial, 1))
                    && ST.invalid(tOutput = OP.plateGem.mat(aMaterial, 1))
                    && ST.invalid(tOutput = OP.ingot.mat(aMaterial, 1))
                    && ST.invalid(tOutput = OP.plate.mat(aMaterial, 1))
                    && ST.invalid(tOutput = OP.nugget.mat(aMaterial, 9))
                    && ST.invalid(tOutput = OP.chunkGt.mat(aMaterial, 4))
                    && ST.invalid(tOutput = OP.dust.mat(aMaterial, 1))
                    && ST.invalid(tOutput = OP.dustTiny.mat(aMaterial, 9))
                    && ST.invalid(tOutput = OP.dustSmall.mat(aMaterial, 4))
                    && ST.invalid(tOutput = OP.stick.mat(aMaterial, 2))) {
    ...
    public boolean containsInput(FluidStack aFluid, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return super.containsInput(aFluid, aTileEntity, aSpecialSlot)
            || FL.is(aFluid, "neutralmatter", "chargedmatter");
    }
