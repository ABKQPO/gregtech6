...
public class RecipeMapFurnace extends RecipeMapNonGTRecipes {
    ...
    public RecipeMapFurnace(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal, String aNameNEI,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        FluidStack tCookingOil = (aFluids != null && aFluids.length > 0
            && aFluids[0] != null
            && FluidsGT.COOKING_OIL.contains(
                aFluids[0].getFluid()
                    .getName()) ? aFluids[0] : NF),
            rXP = NF;
        if (FL.valid(tCookingOil) && tCookingOil.amount >= 50
            && !OD.listAllmeatsubstitute.is(aInputs[0])
            && !OD.listAllmeatsubstitute.is(tOutput)
            && (OD.listAllmeatraw.is(aInputs[0]) || OD.listAllmeatcooked.is(tOutput))) {
            tOutput.stackSize++;
        ...
        if (FL.XP.exists()) {
            ...
            if (!OM.prefixcontainsany(tData, TD.Prefix.DUST_BASED, TD.Prefix.INGOT_BASED, TD.Prefix.GEM_BASED)) {
                if (tOutput.getItem() == Items.brick || tOutput.getItem() == Items.netherbrick
                    || tOutput.getItem() == Items.clay_ball
                    || tOutput.getItem() == Items.dye) {
                    ...
                    // Glass is 0.05 XP, yes I know it can be made from Stone, but this is enough effort to warrant at
                    // least some XP.
                    rXP = FL.XP.make(tOutput.stackSize);
                } else {
                    Block tBlock = ST.block(tOutput);
                    if (tBlock == Blocks.cobblestone || tBlock == Blocks.stone
                        || tBlock == Blocks.stonebrick
                        || tBlock instanceof BlockStones) {
                        ...
                        if (tData != null && tData.validData()) {
                            if (tData.mPrefix.containsAny(TD.Prefix.ORE, TD.Prefix.ORE_PROCESSING_BASED)) {
                                ...
                                if (tData != null && tData.validData()) {
                                    ...
                                    // Valuable Tag is for Gold and certain Gems and happens to double the XP you can
                                    // get from this.
                                    ...
                                    if (tData.mPrefix.mAmount > 0) {
                                        ...
                                        // This is probably an Ore Block since those have a Value of -1, or something
                                        // else that doesn't have a Unit Amount.
                                        rXP = FL.XP.make(tXP);
                                    }
                                } else {
                                ...
                                // No XP from this case! This is likely either a Recycling Recipe or a Dust to Ingot
                                // Recipe!
                            }
                        } else {
                            ...
                            if (MD.RoC.owns(aInputs[0], "extracts")) {
                                ...
                                rXP = FL.XP.make(
                                    UT.Code.bind(
                                        1,
                                        tOutput.stackSize * 20L,
                                        UT.Code.roundUp(
                                            tOutput.stackSize * 20
                                                * FurnaceRecipes.smelting()
                                                    .func_151398_b(tOutput))));
                            }
                        }
                    }
                }
        ...
        return new Recipe(
            ...
            null,
            null,
            tCookingOil != null && tCookingOil.amount > 0 ? FL.array(tCookingOil) : ZL_FS,
            rXP != null && rXP.amount > 0 ? FL.array(rXP) : ZL_FS,
            16,
            16,
            0);
    }
    ...
    public boolean containsInput(FluidStack aFluid, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return FluidsGT.COOKING_OIL.contains(
            aFluid.getFluid()
                .getName());
    }
    ...
    public boolean containsInput(Fluid aFluid, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return FluidsGT.COOKING_OIL.contains(aFluid.getName());
    }
}
