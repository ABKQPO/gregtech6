...
public class RecipeMapPrinter extends RecipeMap {
    ...
    public RecipeMapPrinter(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal, String aNameNEI,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        for (ItemStack aInput : aInputs) if (aInput != null) {
            if (tData == null) {
                if (OM.is_(OD_USB_STICKS[1], aInput)) {
                    ...
                    tUSB = aInput;
                    tData = tUSB.getTagCompound()
                        .getCompoundTag(NBT_USB_DATA);
                } else if (OM.is_(OD_USB_CABLES[1], aInput)) {
                    if (aTileEntity == null) return rRecipe;
                    tUSB = aInput;
                    for (byte tSide : ALL_SIDES_VALID_ONLY[tUSB.hasTagCompound() && tUSB.getTagCompound()
                        .hasKey(NBT_USB_DIRECTION) ? tUSB.getTagCompound()
                            .getByte(NBT_USB_DIRECTION) : SIDE_ANY]) {
                        ...
                        if (tDelegator.mTileEntity instanceof ITileEntityUSBPort) {
                            tData = ((ITileEntityUSBPort) tDelegator.mTileEntity)
                                .getUSBData(tDelegator.mSideOfTileEntity, 1);
        ...
        if (tPaper != null && tUSB != null) {
            if (OM.is_("gt:canvas", tPaper)) {
                if (tData.hasKey(NBT_CANVAS_BLOCK) && (!tPaper.hasTagCompound() || !tPaper.getTagCompound()
                    .hasKey(NBT_CANVAS_BLOCK))) {
                    rRecipe = new Recipe(
                        F,
                        F,
                        F,
                        ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                        ST.array(ST.amount(1, tPaper)),
                        null,
                        null,
                        FL.array(
            ...
            if (IL.Paper_Blueprint_Empty.equal(tPaper, F, T)) {
                ...
                if (tBlueprint != ZL_IS) {
                    ...
                    return new Recipe(
                        ...
                        ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                        ST.array(
                            IL.Paper_Blueprint_Used.getWithNameAndNBT(
                                1,
                                tCrafted == null ? null : tCrafted.getDisplayName(),
                                UT.NBT.setBlueprintCrafting(UT.NBT.make(), tBlueprint))),
                        null,
                        null,
            ...
            if (IL.Paper_Punch_Card_Empty.equal(tPaper, F, T)) {
                if (UT.Code.stringValid(UT.NBT.getPunchCardData(tData))) {
                    rRecipe = new Recipe(
                        ...
                        F,
                        ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                        ST.array(IL.Paper_Punch_Card_Encoded.get(1)),
                        null,
                        null,
                        FL.array(FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, 9, T)),
                        null,
                        32,
                        16,
                        0);
            ...
            if (OD.paperEmpty.is_(tPaper)) {
                if (IL.GC_Schematic_1.exists() && tData.hasKey("gc_schematics_1")) return new Recipe(
                    F,
                    F,
                    F,
                    ST.array(ST.amount(8, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.GC_Schematic_1.getWithMeta(1, tData.getShort("gc_schematics_1"))),
                    null,
                    null,
                    ...
                    16,
                    0);
                if (IL.GC_Schematic_2.exists() && tData.hasKey("gc_schematics_2")) return new Recipe(
                    F,
                    F,
                    F,
                    ST.array(ST.amount(8, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.GC_Schematic_2.getWithMeta(1, tData.getShort("gc_schematics_2"))),
                    null,
                    null,
                    FL.array(FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 4, 1, T)),
                    null,
                    ...
                    16,
                    0);
                if (IL.GC_Schematic_3.exists() && tData.hasKey("gc_schematics_3")) return new Recipe(
                    F,
                    F,
                    F,
                    ST.array(ST.amount(8, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.GC_Schematic_3.getWithMeta(1, tData.getShort("gc_schematics_3"))),
                    null,
                    null,
                    FL.array(FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 4, 1, T)),
                    null,
                    ...
                    16,
                    0);
                if (IL.IE_Blueprint_Projectiles_Common.exists() && tData.hasKey("ie_blueprint")) return new Recipe(
                    F,
                    F,
                    F,
                    ST.array(ST.amount(3, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.IE_Blueprint_Projectiles_Common.getWithMeta(1, tData.getShort("ie_blueprint"))),
                    null,
                    null,
                ...
                if (tBlueprint != ZL_IS) {
                    ...
                    return new Recipe(
                        ...
                        null,
                        null,
                        FL.array(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Blue]),
                        null,
                        128,
                        16,
                        0);
                }
                if (UT.Code.stringValid(UT.NBT.getBookTitle(tData))
                    && UT.Code.stringValid(UT.NBT.getBookAuthor(tData))) {
                    ...
                    if (tPages == null || tPages.tagCount() < 1) {
                        ...
                        if (UT.Code.stringValid(aMapping)) {
                            ...
                            if (tBook != null && tBook.hasTagCompound()) tPages = tBook.getTagCompound()
                                .getTagList("pages", 8);
                        }
                    }
                    ...
                    rRecipe = new Recipe(
                        ...
                        F,
                        F,
                        ST.array(ST.amount(tUseManyPages ? 6 : 3, tPaper), ST.amount(0, tUSB)),
                        ST.array(tUseManyPages ? IL.Paper_Printed_Pages_Many.get(1) : IL.Paper_Printed_Pages.get(1)),
                        null,
                        null,
                        FL.array(FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, tUseManyPages ? 1 : 2, T)),
                        null,
                        tUseManyPages ? 1024 : 512,
                        16,
                        0);
                ...
                if (tID > 0 && UT.Code.exists(tID, OreDictMaterial.MATERIAL_ARRAY)) {
                    ...
                    if (ST.valid(tMaterial.mDictionaryBook)) {
                        ...
                        rRecipe = new Recipe(
                            ...
                            ST.array(ST.amount(tUseManyPages ? 6 : 3, tPaper), ST.amount(0, tUSB)),
                            ST.array(
                                tUseManyPages ? IL.Paper_Printed_Pages_Many.get(1) : IL.Paper_Printed_Pages.get(1)),
                            null,
                            null,
                            FL.array(FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, tUseManyPages ? 1 : 2, T)),
                            null,
            ...
            if (ST.item_(tPaper) == Items.map) {
                ...
                return new Recipe(
                    ...
                    F,
                    ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                    ST.array(ST.make(Items.filled_map, 1, tMapID)),
                    null,
                    null,
                    FL.array(
                        FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, 9, T),
            ...
            if (IL.TF_Magic_Map_Empty.equal(tPaper, T, T)) {
                ...
                return new Recipe(
                    ...
                    F,
                    ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.TF_Magic_Map.getWithMeta(1, tMapID)),
                    null,
                    null,
                    FL.array(
                        FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, 9, T),
            ...
            if (IL.TF_Ore_Map_Empty.equal(tPaper, T, T)) {
                ...
                return new Recipe(
                    ...
                    F,
                    ST.array(ST.amount(1, tPaper), ST.amount(0, tUSB)),
                    ST.array(IL.TF_Ore_Map.getWithMeta(1, tMapID)),
                    null,
                    null,
                    FL.array(
                        FL.mul(DYE_FLUIDS_CHEMICAL[DYE_INDEX_Black], 1, 9, T),
         ...
         * if (rRecipe == null) {
         * ItemStack
         * tOutput = GT_ModHandler.getAllRecipeOutput(aTileEntity==null?null:aTileEntity.getWorld(), aInputs[0],
         * aInputs[0], aInputs[0], aInputs[0], IL.DYE_ONLY_ITEMS[aDye.mIndex].get(1), aInputs[0], aInputs[0],
         * aInputs[0], aInputs[0]);
         * if (tOutput != null) return addRecipe(new Recipe(T, ST.array(ST.amount_(8, aInputs[0])}, ST.array(tOutput},
         * null, null, FL.array(new FluidStack(aFluids[0].getFluid(), (int)L)}, null, 256, 2, 0), F, F, T);
         * tOutput = GT_ModHandler.getAllRecipeOutput(aTileEntity==null?null:aTileEntity.getWorld(), aInputs[0],
         * IL.DYE_ONLY_ITEMS[aDye.mIndex].get(1));
         * if (tOutput != null) return addRecipe(new Recipe(T, ST.array(ST.amount_(1, aInputs[0])}, ST.array(tOutput},
         * null, null, FL.array(new FluidStack(aFluids[0].getFluid(), (int)L)}, null, 32, 2, 0), F, F, T);
         * }
         */
        return rRecipe;
    }
