...
public class RecipeMapFurnaceFuel extends RecipeMap {
    ...
    public RecipeMapFurnaceFuel(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (aInputs == null || aInputs.length < 1 || ST.invalid(aInputs[0]) || GAPI_POST.mFinishedServerStarted <= 0)
            return rRecipe;
        if (rRecipe == null) {
            long tFuelValue = ST.fuel(aInputs[0]);
            OreDictItemData tData = OM.anydata_(aInputs[0]);
            if (tFuelValue > 0 && !FL.contains(aInputs[0], FL.Lava.make(0), T)
                && !OM.materialcontains(tData, TD.Properties.EXPLOSIVE)) {
                ...
                if (tContainer == null) {
                    ...
                    if (tData != null) {
                        if (tData.mPrefix == OP.oreRaw) {
                            tMaterial = OM.stack(
                                tData.mMaterial.mMaterial.mTargetBurning.mMaterial,
                                tData.mMaterial.mMaterial.mTargetBurning.mAmount
                                    * tData.mMaterial.mMaterial.mOreMultiplier
                                    * tData.mMaterial.mMaterial.mOreProcessingMultiplier
                                    * 2);
                            ...
                            if (tMaterial == null || tMaterial.mAmount <= 0) {
                                tMaterial = OM.stack(
                                    aMaterial.mMaterial.mTargetBurning.mMaterial,
                                    UT.Code.units(aMaterial.mAmount, U, aMaterial.mMaterial.mTargetBurning.mAmount, F));
                            } else if (tMaterial.mMaterial == aMaterial.mMaterial.mTargetBurning.mMaterial) {
                                tMaterial.mAmount += UT.Code
                                    .units(aMaterial.mAmount, U, aMaterial.mMaterial.mTargetBurning.mAmount, F);
                            }
                        }
                    }
                    ...
                    if (tMaterial == null || tMaterial.mAmount <= 0) {
                        rRecipe = new Recipe(
                            ...
                            null,
                            null,
                            tFuelValue * EU_PER_FURNACE_TICK,
                            -1,
                            0);
                    } else {
                        for (int i = 1; i < 64; i++) {
                            ...
                            if (ST.valid(tAshes)) {
                                rRecipe = new Recipe(
                                    ...
                                    F,
                                    T,
                                    ST.array(ST.amount(i, aInputs[0])),
                                    ST.array(tAshes),
                                    null,
                                    null,
                                    null,
                                    null,
                                    tFuelValue * i * EU_PER_FURNACE_TICK,
                                    -1,
                                    0);
                                break;
                            }
    ...
    public boolean containsInput(ItemStack aStack, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return null != findRecipe(null, null, T, Long.MAX_VALUE, null, ZL_FS, aStack);
    }
}
