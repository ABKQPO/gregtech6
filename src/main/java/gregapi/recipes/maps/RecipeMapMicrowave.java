...
public class RecipeMapMicrowave extends RecipeMapNonGTRecipes {
    ...
    public RecipeMapMicrowave(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (ST.equal(aInputs[0], Items.book)) {
            return new Recipe(
                ...
                ST.array(ST.book("Manual_Microwave")),
                null,
                null,
                null,
                null,
        ...
        for (ItemStack tStack : ST.array(ST.container(aInputs[0], T), aInputs[0], ST.container(tOutput, T), tOutput))
            if (ST.valid(tStack)) {
                if (ST.equal(tStack, Blocks.netherrack) || ST.equal(tStack, Blocks.tnt)
                    ...
                    if (aTileEntity instanceof TileEntityBase01Root)
                        ((TileEntityBase01Root) aTileEntity).overcharge(aSize * 4, TD.Energy.EU);
                    return null;
                }
