...
public class RecipeMapScannerMolecular extends RecipeMap {
    ...
    public RecipeMapScannerMolecular(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        for (ItemStack aInput : aInputs) if (ST.valid(aInput)) {
            if (ST.invalid(tUSB) && OM.is_(OD_USB_STICKS[3], aInput)) tUSB = aInput;
            ...
            if (ST.valid(tUSB) && ST.valid(tScanned)) {
                ...
                if (aData.mPrefix != null && aData.mMaterial != null
                    && aData.mMaterial.mMaterial.mID > 0
                    && aData.mPrefix.contains(TD.Prefix.SCANNABLE)) {
                    rRecipe = new Recipe(
                        F,
                        F,
                        F,
                        ST.array(ST.amount(1, tScanned), ST.amount(1, tUSB)),
                        ST.array(ST.amount(1, tUSB)),
                        null,
                        null,
                        null,
                        null,
                        (aData.mMaterial.mMaterial.mProtons + aData.mMaterial.mMaterial.mNeutrons) * 512,
                        512,
                        0);
                    if (!rRecipe.mOutputs[0].hasTagCompound()) rRecipe.mOutputs[0].setTagCompound(UT.NBT.make());
                    rRecipe.mOutputs[0].getTagCompound()
                        .setTag(NBT_USB_DATA, UT.NBT.makeShort(NBT_REPLICATOR_DATA, aData.mMaterial.mMaterial.mID));
                    rRecipe.mOutputs[0].getTagCompound()
                        .setByte(NBT_USB_TIER, (byte) 3);
                    return rRecipe;
                }
                return rRecipe;
            }
    ...
    public boolean containsInput(ItemStack aStack, IHasWorldAndCoords aTileEntity, ItemStack aSpecialSlot) {
        return super.containsInput(aStack, aTileEntity, aSpecialSlot) || OM.is(OD_USB_STICKS[3], aStack)
            || OM.prefixcontains(aStack, TD.Prefix.SCANNABLE);
    }
}
