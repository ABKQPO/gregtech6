...
public class RecipeMapUnboxinator extends RecipeMap {
    ...
    public RecipeMapUnboxinator(Collection<Recipe> aRecipeList, String aUnlocalizedName, String aNameLocal,
    ...
    public Recipe findRecipe(IHasWorldAndCoords aTileEntity, Recipe aRecipe, boolean aNotUnificated, long aSize,
        ...
        if (IL.Crate_Loot.equal(aInputs[0], F, T)) {
            // Due to the randomness it is not good if there are Items in the Output Slot, because those Items could
            ...
            return new Recipe(
                ...
                ST.array(ST.generateOneVanillaLoot(), IL.Crate.get(1)),
                null,
                null,
                null,
                null,
        ...
        // For any GT6 Grabbag Items, so I don't have to add to this over and over. Still gotta manually add Fake
        // Recipes for the Item Filter though.
        if (ST.item(aInputs[0]) instanceof MultiItem) {
            List<IBehavior<MultiItem>> tList = ((MultiItem) ST.item(aInputs[0])).mItemBehaviors
                .get(ST.meta(aInputs[0]));
            if (tList != null)
                for (IBehavior<MultiItem> tBehavior : tList) if (tBehavior instanceof Behavior_Drop_Loot) {
                    ItemStack[] tOutputs = ST.array(((Behavior_Drop_Loot) tBehavior).mLoots.length);
                    for (int i = 0; i < tOutputs.length; i++)
                        tOutputs[i] = ChestGenHooks.getOneItem(((Behavior_Drop_Loot) tBehavior).mLoots[i], RNGSUS);
                    // If somehow no Loot, return the whole stack of Lootbags, don't waste any time or items.
                    if (!ST.hasValid(tOutputs))
                        return new Recipe(F, F, F, aInputs, aInputs, null, null, null, null, 1, 1, 0);
                    // Due to the randomness it is not good if there are Items in the Output Slot, because those Items
        ...
        if (COMPAT_TC != null && IL.TC_Loot_Common.equal(aInputs[0], T, T)) {
            ...
            // Due to the randomness it is not good if there are Items in the Output Slot, because those Items could
            // manipulate the outcome.
            return new Recipe(F, F, F, ST.array(ST.amount(1, aInputs[0])), tOutputs, null, null, null, null, 16, 16, 0)
                .setNeedEmptyOut();
        }
        if (IL.LOOTBAGS_Bag_0.equal(aInputs[0], T, T)) {
            ItemStack tBag = ST.amount(1, aInputs[0]);
            UT.Reflection.callPrivateMethod(
                tBag.getItem()
                    .getClass(),
                "generateInventory",
                tBag);
            ...
            return new Recipe(
                ...
                F,
                ST.array(ST.amount(1, aInputs[0])),
                tBag.hasTagCompound() ? (ItemStack[]) UT.Reflection.callPublicMethod(
                    tBag.getItem()
                        .getClass(),
                    "getInventory",
                    tBag) : ZL_IS,
