...
import gregtech6.entities.ai.EntityAIBetterAttackOnCollide;
...
public abstract class GT_Proxy extends Abstract_Proxy {
    ...
    public void onProxyBeforePreInit(Abstract_Mod aMod, FMLPreInitializationEvent aEvent) {
        ...
        try {
            ...
            while (tScanner.hasNextLine()) mSupporterListSilver.add(
                tScanner.nextLine()
                    .toLowerCase());
            tScanner.close();
        } catch (Throwable e) {
        ...
        try {
            ...
            while (tScanner.hasNextLine()) mSupporterListGold.add(
                tScanner.nextLine()
                    .toLowerCase());
            tScanner.close();
        } catch (Throwable e) {
    ...
    public void onEndermanTeleportEvent(EnderTeleportEvent aEvent) {
        if (aEvent.entityLiving instanceof EntityEnderman
            && aEvent.entityLiving.getActivePotionEffect(Potion.weakness) != null) aEvent.setCanceled(T);
    }

    private static final EnumSet<EventType> PREVENTED_ORES = EnumSet.of(
        EventType.COAL,
        EventType.IRON,
        EventType.GOLD,
        EventType.DIAMOND,
    ...
    public void onOreGenEvent(GenerateMinable aEvent) {
        if (mDisableVanillaOres && !WD.dimTF(aEvent.world) && PREVENTED_ORES.contains(aEvent.type))
    ...
    public void onTerrainGenEvent(DecorateBiomeEvent.Decorate aEvent) {
        if (aEvent.world.provider.dimensionId == 0) {
            if (MD.RTG.mLoaded) {
                ...
                if ("WorldProviderSurfaceRTG".equalsIgnoreCase(tClassName)
                    || "WorldTypeRTG".equalsIgnoreCase(tClassName)) return;
            }
            if (GENERATE_STREETS
                && (UT.Code.inside(-48, 47, aEvent.chunkX) || UT.Code.inside(-48, 47, aEvent.chunkZ))) {
                aEvent.setResult(Result.DENY);
                return;
            }
            if (GENERATE_BIOMES && (UT.Code.inside(-96, 95, aEvent.chunkX) && UT.Code.inside(-96, 95, aEvent.chunkZ))) {
                aEvent.setResult(Result.DENY);
                return;
            }
        }
    ...
    public void onTerrainGenEvent(PopulateChunkEvent.Populate aEvent) {
        if (aEvent.world.provider.dimensionId == 0) {
            if (mDisableVanillaLakes
                && (aEvent.type == Populate.EventType.LAKE || aEvent.type == Populate.EventType.LAVA)) {
                aEvent.setResult(Result.DENY);
                return;
            }
            if (MD.RTG.mLoaded) {
    ...
    public void onGetVillageBlockIDEvent(BiomeEvent.GetVillageBlockID aEvent) {
        if (aEvent.original == Blocks.cobblestone) {
            aEvent.replacement = (aEvent.biome == null ? BlocksGT.Andesite
                : BlocksGT.stones[(aEvent.biome.biomeID + 6) % BlocksGT.stones.length]);
            aEvent.setResult(Result.DENY);
        }
    }
    ...
    public void onPlayerInteraction(PlayerInteractEvent aEvent) {
        ...
        if (!aEvent.world.isRemote && CHECKED_PLAYERS.add(aName)) {
            if (mSupporterListSilver.contains(
                aEvent.entityPlayer.getUniqueID()
                    .toString())
                || mSupporterListGold.contains(
                    aEvent.entityPlayer.getUniqueID()
                        .toString())
                || mSupporterListSilver.contains(aNameLowercase)
                || mSupporterListGold.contains(aNameLowercase)) {
                if (!MultiTileEntityCertificate.ALREADY_RECEIVED.contains(aNameLowercase)) {
                    if (ST.give(aEvent.entityPlayer, MultiTileEntityCertificate.getCertificate(1, aName), F)) {
                        ...
                        UT.Entities.sendchat(
                            ...
                            CHAT_GREG + "Thank you, "
                                + aName
                                + ", for Supporting GregTech! Here, have a Certificate. ;)");
                    }
                }
            }
        }
        ...
        if (aStack != null && aStack.stackSize > 0) {
            if (aEvent.action == PlayerInteractEvent.Action.RIGHT_CLICK_AIR) {
                if (aStack.getItem() == Items.glass_bottle) {
                    ...
                    if (tTarget == null || tTarget.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK
                        || !aEvent.world
                            .canMineBlock(aEvent.entityPlayer, tTarget.blockX, tTarget.blockY, tTarget.blockZ)
                        || !aEvent.entityPlayer
                            .canPlayerEdit(tTarget.blockX, tTarget.blockY, tTarget.blockZ, tTarget.sideHit, aStack))
                        return;
                    ...
                    if (tBlock == Blocks.water || tBlock == Blocks.flowing_water) {
                        ...
                        for (int i = 0; i < 3 && aStack.stackSize > 0; i++) {
                            if (aStack.stackSize == 1) {
                                aEvent.entityPlayer.inventory.mainInventory[aEvent.entityPlayer.inventory.currentItem] = ST
                                    .make(Items.potionitem, 1, 0);
                            } else {
            ...
            if (aEvent.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
                if (IL.ERE_Spray_Repellant.equal(aStack, T, T)) {
                    if (!aEvent.world.isRemote && aStack.getItem()
                        .onItemUse(
                            aStack,
                            aEvent.entityPlayer,
                            aEvent.world,
                    ...
                    if (!aEvent.world.isRemote && !UT.Entities.hasInfiniteItems(aEvent.entityPlayer)
                        && RNGSUS.nextInt(100) >= mFlintChance) {
                        aEvent.setCanceled(T);
                    ...
                    if (tDamage > 0) {
                        ...
                        if (!UT.Entities.hasInfiniteItems(aEvent.entityPlayer)) {
                            aStack
                                .damageItem(UT.Code.bindInt(UT.Code.units(tDamage, 10000, 1, T)), aEvent.entityPlayer);
                            if (aStack.getItemDamage() >= aStack.getMaxDamage()) ST.use(aEvent.entityPlayer, aStack);
                        }
                        return;
                    }
                    ...
                    if (!aEvent.world.isRemote && aEvent.entityPlayer.isSneaking()
                        && MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                            .getItem(32105, ST.save("sandwich.0", ST.amount(tUsed, aStack)))
                    ...
                    || OM.is("stickAnyNormalWood", aStack)) {
                        if (!aEvent.world.isRemote && aEvent.entityPlayer.isSneaking()
                            && MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                .getItem(32073)
                                .tryPlaceItemIntoWorld(
                        ...
                        if (!aEvent.world.isRemote && aEvent.entityPlayer.isSneaking()
                            && MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                .getItem(32074, ST.save(NBT_VALUE, ST.amount(1, aStack)))
                                .tryPlaceItemIntoWorld(
                                    aEvent.entityPlayer,
                                    aEvent.world,
                                    aEvent.x,
                        ...
                        if (!aEvent.world.isRemote && aEvent.entityPlayer.isSneaking() && ST.block(aStack) == NB) {
                            ...
                            if (tData != null) {
                                if (tData.mPrefix == OP.rockGt || tData.mPrefix == OP.oreRaw) {
                                    if (MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                        .getItem(32074, ST.save(NBT_VALUE, ST.amount(1, aStack)))
                                        .tryPlaceItemIntoWorld(
                                            aEvent.entityPlayer,
                                            aEvent.world,
                                ...
                                if (tData.mPrefix == OP.ingot)
                                    if (!MD.BOTA.mLoaded || tData.mMaterial.mMaterial.mOriginalMod != MD.BOTA
                                        || Blocks.beacon != aEvent.world.getBlock(aEvent.x, aEvent.y, aEvent.z)) {
                                            if (MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                                .getItem(32084, ST.save(NBT_VALUE, aStack))
                                                .tryPlaceItemIntoWorld(
                                                    aEvent.entityPlayer,
                                                    aEvent.world,
                                                    aEvent.x,
                                ...
                                if (tData.mPrefix == OP.plate) {
                                    if (MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                        .getItem(32085, ST.save(NBT_VALUE, aStack))
                                        .tryPlaceItemIntoWorld(
                                            aEvent.entityPlayer,
                                            aEvent.world,
                                            aEvent.x,
                                ...
                                if (tData.mPrefix == OP.plateGem) {
                                    if (MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                        .getItem(32086, ST.save(NBT_VALUE, aStack))
                                        .tryPlaceItemIntoWorld(
                                            aEvent.entityPlayer,
                                            aEvent.world,
                                            aEvent.x,
                                ...
                                if (tData.mPrefix == OP.scrapGt) {
                                    if (MultiTileEntityRegistry.getRegistry("gt.multitileentity")
                                        .getItem(32103, ST.save(NBT_VALUE, aStack))
                                        .tryPlaceItemIntoWorld(
                                            aEvent.entityPlayer,
                                            aEvent.world,
                                            aEvent.x,
    ...
    public void onEntitySpawningEvent(EntityJoinWorldEvent aEvent) {
        ...
        if (aEvent.entity instanceof EntityLiving) {
            ...
            if (tTasks != null) {
                ...
                if (aEvent.entity instanceof EntityOcelot) {
                    if (ItemsGT.CANS != null)
                        tTasks.addTask(3, new EntityAITempt((EntityCreature) aEvent.entity, 0.6D, ItemsGT.CANS, T));
                }
                if (aEvent.entity instanceof EntityZombie) {
                    for (int i = 0; i < tTasks.taskEntries.size(); i++) {
                        EntityAITasks.EntityAITaskEntry tEntry = (EntityAITasks.EntityAITaskEntry) tTasks.taskEntries
                            .get(i);
            ...
            if (!aEvent.entity.worldObj.isRemote && !aEvent.entity.getEntityData()
                .hasKey("gt.spawned")) {
                if (aEvent.entity instanceof EntityZombie && !((EntityZombie) aEvent.entity).isChild()
                    && ST.invalid(((EntityZombie) aEvent.entity).getEquipmentInSlot(0))) {
                    if (ZOMBIES_HOLD_TNT && RNGSUS.nextInt(250) == 0) {
                        ((EntityZombie) aEvent.entity)
                            .setCurrentItemOrArmor(0, ST.make(Blocks.tnt, 1 + RNGSUS.nextInt(2), 0));
                    } else if (ZOMBIES_HOLD_PICKAXES && RNGSUS.nextInt(100) == 0) {
                        ((EntityZombie) aEvent.entity).setCurrentItemOrArmor(
                            0,
                            ST.make(
                                Items.iron_pickaxe,
                                1,
                                Items.iron_pickaxe.getMaxDamage() < 5 ? 0
                                    : 1 + RNGSUS.nextInt(Items.iron_pickaxe.getMaxDamage() - 2)));
                    }
                }
                // Mark Entity as has been spawned
                aEvent.entity.getEntityData()
                    .setBoolean("gt.spawned", T);
            }
            return;
        }
        ...
        if (mSkeletonsShootGTArrows > 0 && aEvent.entity.getClass() == EntityArrow.class
            && RNGSUS.nextInt(mSkeletonsShootGTArrows) == 0) {
            if (((EntityArrow) aEvent.entity).shootingEntity instanceof EntitySkeleton) {
                ...
                switch (RNGSUS.nextInt(10)) {
                    ...
                    case 2:
                        ...
                        break; // Disjunction 5 and Werebane 5
                    case 3:
                        tMaterial = MT.BismuthBronze;
                        break; // Bane of Arthropods 4
                    case 4:
                        tMaterial = MT.Pt;
                        break; // Smite 5
                    case 5:
                ...
                if (ST.valid(tArrow)) {
                    aEvent.entity.worldObj
                        .spawnEntityInWorld(new EntityArrow_Material((EntityArrow) aEvent.entity, tArrow));
    ...
    public void onEntityLivingDropsEventEvent(LivingDropsEvent aEvent) {
        ...
        Override_Drops.handleDrops(
            aEvent.entityLiving,
            UT.Reflection.getLowercaseClass(aEvent.entityLiving),
            aEvent.drops,
            aEvent.source,
            aEvent.lootingLevel,
            aEvent.entityLiving.isBurning(),
            aEvent.recentlyHit);
    }
    ...
    public void onEntityLivingFallEvent(LivingFallEvent aEvent) {
        if (!aEvent.entity.worldObj.isRemote && aEvent.entity instanceof EntityPlayer) {
            if (ST.equal(((EntityPlayer) aEvent.entity).getCurrentEquippedItem(), ToolsGT.sMetaTool, ToolsGT.SCISSORS)
                || ST.equal(
                    ((EntityPlayer) aEvent.entity).getCurrentEquippedItem(),
                    ToolsGT.sMetaTool,
    ...
    @SafeVarargs
    public final Fluid addAutogeneratedLiquid(OreDictMaterial aMaterial, IIconContainer aTexture,
    ...
    @SafeVarargs
    public final Fluid addAutogeneratedGas(OreDictMaterial aMaterial, IIconContainer aTexture,
        Set<String>... aFluidList) {
    ...
    @SafeVarargs
    public final Fluid addAutogeneratedMolten(OreDictMaterial aMaterial, IIconContainer aTexture,
        Set<String>... aFluidList) {
    ...
    @SafeVarargs
    public final Fluid addAutogeneratedVaporized(OreDictMaterial aMaterial, IIconContainer aTexture,
        Set<String>... aFluidList) {
    ...
    @SafeVarargs
    public final Fluid addAutogeneratedPlasma(OreDictMaterial aMaterial, IIconContainer aTexture,
        Set<String>... aFluidList) {
        return FL.createPlasma(aMaterial, aTexture, aFluidList);
    }

    @SafeVarargs
    public final Fluid addFluid(String aName, String aLocalized, OreDictMaterial aMaterial, int aState,
    ...
    @SafeVarargs
    public final Fluid addFluid(String aName, IIconContainer aTexture, String aLocalized, OreDictMaterial aMaterial,
