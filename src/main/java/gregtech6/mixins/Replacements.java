...
public class Replacements {
    ...
    public static void EntityZombie_onKillEntity(Object aZombie, Object aVictim) {
        ...
        if (aVictim instanceof EntityVillager) {
            ...
            // Converting Villager Zombies back to Villagers would make it impossible to retrieve the Items, so don't
            ...
            // With Sound ofcourse!
            aWorld.playAuxSFXAtEntity(
                null,
                1016,
                (int) tZombieVillager.posX,
                (int) tZombieVillager.posY,
                (int) tZombieVillager.posZ,
                0);
    ...
    public static void BlockStaticLiquid_updateTick(BlockStaticLiquid self, World world, int x, int y, int z,
        Random rand) {
        if (self.getMaterial() == Material.lava) {
            ...
            for (i1 = 0; i1 < l; ++i1) {
                ...
                if (block.getMaterial() == Material.air) {
                    if (BlockStaticLiquid_isFlammable(world, x - 1, y, z, ForgeDirection.EAST)
            ...
            if (l == 0) {
                ...
                for (int j1 = 0; j1 < 3; ++j1) {
                    ...
                    if (world.isAirBlock(x, y + 1, z)
                        && BlockStaticLiquid_isFlammable(world, x, y, z, ForgeDirection.UP)) {
    ...
    public static boolean EntityAICreeperSwell_shouldExecute(EntityCreeper swellingCreeper) {
        ...
        double facing = Vec3.createVectorHelper(target.posX, target.posY, target.posZ)
