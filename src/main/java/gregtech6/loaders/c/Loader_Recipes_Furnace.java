...
public class Loader_Recipes_Furnace implements Runnable {
    ...
    public void run() {
        Map<ItemStack, ItemStack>[] tMaps = new Map[] { FurnaceRecipes.smelting()
            .getSmeltingList(), null, null };
        ...
        for (Map<ItemStack, ItemStack> tMap : tMaps) if (tMap != null) {
            Iterator<Entry<ItemStack, ItemStack>> tIterator = tMap.entrySet()
                .iterator();
            while (tIterator.hasNext()) {
                ...
                if (tFurnace && MD.RoC.owns(tReg1, "extracts")) {
                    if (tData2 != null && tData2.nonemptyData()) {
                        ItemStack tDust = OM.dust(
                            tData2.mMaterial.mMaterial.mTargetCrushing.mMaterial,
                            UT.Code.units(
                                tData2.mMaterial.mAmount * tEntry.getValue().stackSize,
                                U,
                                tData2.mMaterial.mMaterial.mTargetCrushing.mAmount,
                                F));
                        ...
                        if (tData2.mMaterial.mMaterial.mTargetCrushing.mMaterial != tData2.mMaterial.mMaterial)
                            tEntry.setValue(tDust);
                ...
                if (tData2 != null && tData2.nonemptyData()) {
                    if (tData1 != null && tData1.nonemptyData()) {
                        // Just outright remove all Furnace Recipes, that both Input and Output valid OreDicted Stuff at
                        // the same time, we add the proper ones below anyways.
                        tIterator.remove();
                        if (MD.EtFu.mLoaded) try {
                    ...
                    if (tData2.mMaterial.mMaterial.contains(TD.Processing.NEVER_FURNACE)) {
                        ...
                        tEntry.setValue(
                            OP.scrapGt.mat(
                                tData2.mMaterial,
                                (tData2.mMaterial.mAmount * tEntry.getValue().stackSize) / OP.scrapGt.mAmount));
                        if (MD.EtFu.mLoaded) try {
                            SmokerRecipes.smelting().smeltingBlacklist.add(tEntry.getKey());
                            BlastFurnaceRecipes.smelting().smeltingBlacklist.add(tEntry.getKey());
                        } catch (Throwable e) {/**/}
    ...
    public static class Listener_Furnace_Smelting implements IOreDictListenerEvent {
        ...
        public void onOreRegistration(OreDictRegistrationContainer aEvent) {
            if (aEvent.mMaterial.contains(TD.Processing.FURNACE)
                && !aEvent.mMaterial.contains(TD.Properties.UNUSED_MATERIAL)) {
                long aTargetAmount = UT.Code.units(
                    UT.Code.units(
                        aEvent.mMaterial.mTargetSmelting.mAmount,
                        U,
                        aEvent.mMaterial.mTargetSmelting.mMaterial.mTargetSolidifying.mAmount,
                        F),
                    U,
                    mTargetAmount < 0 ? aEvent.mPrefix.mAmount : mTargetAmount,
                    F);
                if (aEvent.mMaterial.contains(TD.Properties.FOOD)) {
                    RM.add_smelting(
                        aEvent.mStack,
                        OM.ingot(
                            aEvent.mMaterial.mTargetSmelting.mMaterial.mTargetSolidifying.mMaterial,
                            aTargetAmount),
                        mExp ? UT.Code.units(aTargetAmount, U, aEvent.mMaterial.mToolQuality + 1, T) : 0,
                        !RUNNING,
                        T,
                        F);
                } else {
