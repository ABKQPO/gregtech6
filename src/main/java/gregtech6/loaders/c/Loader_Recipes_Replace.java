...
public class Loader_Recipes_Replace implements Runnable {
    ...
    public static final Set<String> sNonReplaceableNames = new HashSetNoNulls<>(
        F,
        "ic2.itemwoodrotor",
        "ic2.itemironrotor",
        "ic2.itemsteelrotor",
    ...
    public void run() {
        ...
        for (int l = 0; l < tRecipeList.size(); l++) {
            ...
            for (int i = 0; i < tRecipeInputs.length; i++) {
                ...
                if (tObject instanceof ItemStack) {
                    ...
                    if (OD.stickWood.toString()
                        .equals(((ic2.api.recipe.RecipeInputOreDict) tObject).input)) {
                        tRod = ANY.Wood;
                        continue;
                    }
                    if (OD.stickAnyWood.toString()
                        .equals(((ic2.api.recipe.RecipeInputOreDict) tObject).input)) {
                        tRod = ANY.Wood;
                        continue;
                    }
                        ...
                        default:
                            for (Object tContent : ((List) tObject)) if (tContent instanceof ItemStack) {
                                if (tData == null) {
                                    ...
                                    if (tAssociation == null || tAssociation.mPrefix != tData.mPrefix
                                        || tAssociation.mMaterial.mMaterial != tData.mMaterial.mMaterial) {
                                        temp = F;
                                        break;
                                    }
                                }
        ...
        for (RecipeReplacement aRecipe : tList) {
            ...
            for (RecipeReplacer aReplacer : tReplacer) {
                ...
                for (ItemStack tStack : aReplacer.mRecipe) if (tStack == INGT) tMatCount++;
                else if (tStack == STCK) tRodCount++;
                OM.dat2(
                    tCrafted,
                    aRecipe.mMat,
                    aRecipe.mPrefix.mAmount * tMatCount,
                    OM.stack(aRecipe.mRod, OP.stick.mAmount * tRodCount));
                ...
                if (!ConfigsGT.RECIPES
                    .get(ConfigCategories.Recipes.recipereplacements, aRecipe.mMat + "." + aReplacer.mName, T))
                    continue;
                if (!tRecipeList.remove(aRecipe.mRecipe)) continue;
                Object tNamePlate = OreDictionary.getOres(
                    OP.plate.dat(aRecipe.mMat)
                        .toString())
                    .isEmpty() ? OP.ingot.dat(aRecipe.mMat) : OP.plate.dat(aRecipe.mMat),
                    tNameCurve = OreDictionary.getOres(
                        OP.plateCurved.dat(aRecipe.mMat)
                            .toString())
                        .isEmpty() ? tNamePlate : OP.plateCurved.dat(aRecipe.mMat);
                switch (aReplacer.mShape.length) {
                    case 1:
                        CR.shaped(
                            tCrafted,
                            CR.DEF,
                            new Object[] { aReplacer.mShape[0], PLT.charAt(0), tNamePlate, CRV.charAt(0), tNameCurve,
                                ROD.charAt(0), OP.stick.dat(aRecipe.mRod == null ? aRecipe.mMat : aRecipe.mRod),
                                NGT.charAt(0), OP.ingot.dat(aRecipe.mMat) });
                        break;
                    case 2:
                        CR.shaped(
                            tCrafted,
                            CR.DEF,
                            new Object[] { aReplacer.mShape[0], aReplacer.mShape[1], PLT.charAt(0), tNamePlate,
                                CRV.charAt(0), tNameCurve, ROD.charAt(0),
                                OP.stick.dat(aRecipe.mRod == null ? aRecipe.mMat : aRecipe.mRod), NGT.charAt(0),
                                OP.ingot.dat(aRecipe.mMat) });
                        break;
                    default:
                        CR.shaped(
                            tCrafted,
                            CR.DEF,
                            new Object[] { aReplacer.mShape[0], aReplacer.mShape[1], aReplacer.mShape[2], PLT.charAt(0),
                                tNamePlate, CRV.charAt(0), tNameCurve, ROD.charAt(0),
                                OP.stick.dat(aRecipe.mRod == null ? aRecipe.mMat : aRecipe.mRod), NGT.charAt(0),
                                OP.ingot.dat(aRecipe.mMat) });
                        break;
                }
    ...
    public static class RecipeReplacement {
        ...
        public RecipeReplacement(IRecipe aRecipe, OreDictPrefix aPrefix, OreDictMaterial aMat, OreDictMaterial aRod) {
            mRecipe = aRecipe;
            mPrefix = aPrefix;
            mMat = aMat;
            mRod = aRod;
    ...
    public static class RecipeReplacer {
        ...
        public RecipeReplacer(ItemStack[] aRecipe, String aName, String... aShape) {
            mRecipe = aRecipe;
            mName = aName;
            mShape = aShape;
        }

        public RecipeReplacer(ItemStack[] aRecipe) {
            mRecipe = aRecipe;
            mName = "null";
            mShape = ZL_STRING;
        }
    ...
    public static final RecipeReplacer[] sRecipesMat = {
        new RecipeReplacer(ST.array(INGT, null, INGT, INGT, INGT, INGT, null, INGT, null)),
        new RecipeReplacer(ST.array(INGT, null, INGT, INGT, null, INGT, INGT, INGT, INGT)),
        new RecipeReplacer(ST.array(null, INGT, null, INGT, INGT, INGT, INGT, null, INGT)),
        new RecipeReplacer(
            ST.array(INGT, INGT, INGT, INGT, null, INGT, null, null, null),
            "Helmet",
            PLT + PLT + PLT,
            CRV + HAM + CRV),
        new RecipeReplacer(
            ST.array(INGT, null, INGT, INGT, INGT, INGT, INGT, INGT, INGT),
            "ChestPlate",
            PLT + HAM + PLT,
            CRV + PLT + CRV,
            CRV + PLT + CRV),
        new RecipeReplacer(
            ST.array(INGT, INGT, INGT, INGT, null, INGT, INGT, null, INGT),
            "Pants",
            PLT + CRV + PLT,
            CRV + HAM + CRV,
            CRV + ___ + CRV),
        new RecipeReplacer(
            ST.array(null, null, null, INGT, null, INGT, INGT, null, INGT),
            "Boots",
            PLT + ___ + PLT,
            CRV + HAM + CRV),
        new RecipeReplacer(
            ST.array(INGT, null, null, null, INGT, null, null, null, null),
            "Shears",
            HAM + PLT,
            PLT + FIL),
        new RecipeReplacer(
            ST.array(null, INGT, null, INGT, null, null, null, null, null),
            "Shears",
            HAM + PLT,
            PLT + FIL) };
    ...
    public static final RecipeReplacer[] sRecipesRod = {
        new RecipeReplacer(
            ST.array(null, INGT, null, null, INGT, null, null, STCK, null),
            "Sword",
            ___ + PLT + ___,
            FIL + PLT + HAM,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, null, null, INGT, null, null, STCK, null, null),
            "Sword",
            ___ + PLT + ___,
            FIL + PLT + HAM,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, INGT, INGT, null, STCK, null, null, STCK, null),
            "Pickaxe",
            PLT + NGT + NGT,
            FIL + ROD + HAM,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(null, INGT, null, null, STCK, null, null, STCK, null),
            "Shovel",
            FIL + PLT + HAM,
            ___ + ROD + ___,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, INGT, STCK, null, null, STCK, null),
            "Axe",
            PLT + NGT + HAM,
            PLT + ROD + ___,
            FIL + ROD + ___),
        new RecipeReplacer(
            ST.array(null, INGT, INGT, null, STCK, INGT, null, STCK, null),
            "Axe",
            PLT + NGT + HAM,
            PLT + ROD + ___,
            FIL + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, null, STCK, null, null, STCK, null),
            "Hoe",
            PLT + NGT + HAM,
            FIL + ROD + ___,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, STCK, null, null, STCK, null, null),
            "Hoe",
            PLT + NGT + HAM,
            FIL + ROD + ___,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(null, INGT, INGT, null, STCK, null, null, STCK, null),
            "Hoe",
            PLT + NGT + HAM,
            FIL + ROD + ___,
            ___ + ROD + ___),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, INGT, null, null, INGT, INGT, STCK),
            "Sickle",
            PLT + PLT + ___,
            PLT + FIL + HAM,
            PLT + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, INGT, INGT, null, null, INGT, STCK, INGT, INGT),
            "Sickle",
            PLT + PLT + ___,
            PLT + FIL + HAM,
            PLT + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, INGT, null, INGT, null, null, null, INGT, STCK),
            "Sickle",
            ___ + PLT + ___,
            PLT + FIL + ___,
            HAM + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, INGT, null, null, null, INGT, STCK, INGT, null),
            "Sickle",
            ___ + PLT + ___,
            PLT + FIL + ___,
            HAM + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, INGT, null, INGT, null, INGT, null, null, STCK),
            "Sickle",
            ___ + PLT + ___,
            PLT + FIL + ___,
            HAM + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, INGT, null, INGT, null, INGT, STCK, null, null),
            "Sickle",
            ___ + PLT + ___,
            PLT + FIL + ___,
            HAM + PLT + ROD),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, INGT, null, null, INGT, null),
            "Sword",
            ___ + ROD + ___,
            FIL + PLT + HAM,
            ___ + PLT + ___),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, STCK, null, INGT, INGT, INGT),
            "Pickaxe",
            ___ + ROD + ___,
            FIL + ROD + HAM,
            PLT + NGT + NGT),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, STCK, null, null, INGT, null),
            "Shovel",
            ___ + ROD + ___,
            ___ + ROD + ___,
            FIL + PLT + HAM),
        new RecipeReplacer(
            ST.array(null, STCK, null, INGT, STCK, null, INGT, INGT, null),
            "Axe",
            FIL + ROD + ___,
            PLT + ROD + ___,
            PLT + NGT + HAM),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, STCK, INGT, null, INGT, INGT),
            "Axe",
            FIL + ROD + ___,
            PLT + ROD + ___,
            PLT + NGT + HAM),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, STCK, null, INGT, INGT, null),
            "Hoe",
            ___ + ROD + ___,
            FIL + ROD + ___,
            PLT + NGT + HAM),
        new RecipeReplacer(
            ST.array(null, STCK, null, null, STCK, null, null, INGT, INGT),
            "Hoe",
            ___ + ROD + ___,
            FIL + ROD + ___,
            PLT + NGT + HAM),
        new RecipeReplacer(
            ST.array(null, INGT, INGT, null, STCK, INGT, STCK, null, null),
            "Halberd",
            PLT + PLT + HAM,
            PLT + ROD + ___,
            FIL + ___ + ROD),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, INGT, STCK, null, null, null, STCK),
            "Halberd",
            PLT + PLT + HAM,
            PLT + ROD + ___,
            FIL + ___ + ROD),
        new RecipeReplacer(
            ST.array(null, null, INGT, null, STCK, null, STCK, null, null),
            "Spear",
            PLT + HAM + ___,
            FIL + ROD + ___,
            ___ + ___ + ROD),
        new RecipeReplacer(
            ST.array(INGT, null, null, null, STCK, null, null, null, STCK),
            "Spear",
            PLT + HAM + ___,
            FIL + ROD + ___,
            ___ + ___ + ROD),
        new RecipeReplacer(
            ST.array(null, null, INGT, null, INGT, null, STCK, null, null),
            "LongSword",
            PLT + HAM + ___,
            FIL + PLT + ___,
            ___ + ___ + ROD),
        new RecipeReplacer(
            ST.array(INGT, null, null, null, INGT, null, null, null, STCK),
            "LongSword",
            PLT + HAM + ___,
            FIL + PLT + ___,
            ___ + ___ + ROD),
        new RecipeReplacer(
            ST.array(null, null, INGT, null, STCK, null, null, null, null),
            "Knife",
            HAM + PLT,
            ROD + FIL),
        new RecipeReplacer(
            ST.array(INGT, STCK, null, null, null, null, null, null, null),
            "Knife",
            FIL + HAM,
            PLT + ROD),
        new RecipeReplacer(
            ST.array(STCK, INGT, null, null, null, null, null, null, null),
            "Knife",
            FIL + HAM,
            PLT + ROD),
        new RecipeReplacer(
            ST.array(INGT, null, null, STCK, null, null, null, null, null),
            "Knife",
            PLT + FIL,
            ROD + HAM),
        new RecipeReplacer(
            ST.array(STCK, null, null, INGT, null, null, null, null, null),
            "Knife",
            PLT + FIL,
            ROD + HAM),
        new RecipeReplacer(ST.array(INGT, INGT, INGT, INGT, INGT, INGT, null, STCK, null)),
        new RecipeReplacer(ST.array(INGT, INGT, null, INGT, INGT, STCK, INGT, INGT, null)),
        new RecipeReplacer(ST.array(null, INGT, INGT, STCK, INGT, INGT, null, INGT, INGT)),
        new RecipeReplacer(ST.array(null, STCK, null, INGT, INGT, INGT, INGT, INGT, INGT)),
        new RecipeReplacer(
            ST.array(INGT, INGT, INGT, INGT, STCK, INGT, INGT, STCK, INGT),
            "Paxel",
            PLT + NGT + PLT,
            NGT + ROD + NGT,
            PLT + ROD + PLT),
        new RecipeReplacer(
            ST.array(INGT, INGT, INGT, INGT, STCK, INGT, null, STCK, null),
            "WarAxe",
            PLT + PLT + PLT,
            PLT + ROD + PLT,
            FIL + ROD + HAM),
        new RecipeReplacer(
            ST.array(INGT, null, INGT, INGT, STCK, INGT, null, STCK, null),
            "WarAxe",
            PLT + ___ + PLT,
            PLT + ROD + PLT,
            FIL + ROD + HAM),
        new RecipeReplacer(ST.array(INGT, INGT, null, INGT, STCK, STCK, INGT, INGT, null)),
        new RecipeReplacer(ST.array(null, INGT, INGT, STCK, STCK, INGT, null, INGT, INGT), null),
        new RecipeReplacer(ST.array(null, STCK, null, INGT, STCK, INGT, INGT, INGT, INGT)),
        new RecipeReplacer(
            ST.array(INGT, STCK, INGT, INGT, STCK, INGT, null, STCK, null),
            "WarHammer",
            PLT + ROD + PLT,
            PLT + ROD + PLT,
            FIL + ROD + HAM),
        new RecipeReplacer(ST.array(INGT, INGT, null, STCK, STCK, STCK, INGT, INGT, null)),
        new RecipeReplacer(ST.array(null, INGT, INGT, STCK, STCK, STCK, null, INGT, INGT)),
        new RecipeReplacer(ST.array(null, STCK, null, INGT, STCK, INGT, INGT, STCK, INGT)),
        new RecipeReplacer(
            ST.array(INGT, INGT, null, STCK, null, INGT, STCK, null, null),
            "Scythe",
            NGT + PLT + HAM,
            ROD + FIL + PLT,
            ROD + ___ + ___),
        new RecipeReplacer(
            ST.array(null, INGT, INGT, INGT, null, STCK, null, null, STCK),
            "Scythe",
            HAM + PLT + NGT,
            PLT + FIL + ROD,
            ___ + ___ + ROD),
        new RecipeReplacer(
            ST.array(INGT, INGT, STCK, null, STCK, null, STCK, null, null),
            "Scythe",
            PLT + PLT + ROD,
            FIL + ROD + HAM,
            ROD + ___ + ___) };
