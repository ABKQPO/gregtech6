                        ...
                        if (!tRecipeMaps.isEmpty()) {
                            ...
                            for (OreDictMaterialStack tMaterial : tComponents.getUndividedComponents()) {
                                ...
                                if (tMaterial.mMaterial.mMeltingPoint <= DEF_ENV_TEMP
                                    && tFluidOutputs.add(tMaterial.mMaterial.fluid(tMaterial.mAmount, F))) continue;
                                if (tStackOutputs.add(
                                    OM.dust(
                                        tMaterial.mMaterial.mTargetPulver.mMaterial,
                                        UT.Code.units(
                                            tMaterial.mAmount,
                                            U,
                                            tMaterial.mMaterial.mTargetPulver.mAmount,
                                            F))))
                                    continue;
                            }

                            if (tStackOutputs.size() > 0 || tFluidOutputs.size() > 0)
                                for (RecipeMap tRecipeMap : tRecipeMaps) {
                                    boolean temp = T;

                                    if (temp && tRecipeMap.mInputFluidCount > 0) {
                                        FluidStack aFluid = aMaterial.liquid(tComponents.getCommonDivider() * U, T);
                                        if (FL.nonzero(aFluid)) {
                                            ...
                                            if (tRecipeMap == RM.Electrolyzer) {
                                                tRecipeMap.addRecipe1(
                                                    ...
                                                    F,
                                                    F,
                                                    Math.max(16, (tAmount * 14) / U),
                                                    UT.Code.units(tAmount, U, 292, T),
                                                    ST.tag(0),
                                                    FL.array(aFluid),
                                                    UT.Code.makeArray(
                                                        new FluidStack[tRecipeMap.mOutputFluidCount],
                                                        tFluidOutputs.toArray(ZL_FS)),
                                                    UT.Code.makeArray(
                                                        new ItemStack[tRecipeMap.mOutputItemsCount],
                                                        tStackOutputs.toArray(ZL_IS)));
                                            } else {
                                                tRecipeMap.addRecipe0(
                                                    T,
                                                    T,
                                                    F,
                                                    F,
                                    ...
                                    if ((temp || aMaterial.mReRegistrations.contains(ANY.Glowstone))
                                        && tRecipeMap.mInputItemsCount > 0) {
                                        ItemStack aStack = OP.dust.mat(aMaterial, tComponents.getCommonDivider());
                                        if (aStack != null) {
                                            temp = F;
                                            if (tRecipeMap == RM.Electrolyzer) {
                                                tRecipeMap.addRecipe2(
                                                    T,
                                                    T,
                                                    F,
                                                    F,
