...
public class Loader_Fuels implements Runnable {
    ...
    @Override
    public void run() {
        for (OreDictMaterial tMat : new OreDictMaterial[] { MT.Charcoal, MT.Coal, MT.CoalCoke, MT.Lignite,
            MT.LigniteCoke, MT.Anthracite, MT.Prismane, MT.Lonsdaleite, MT.PetCoke, MT.Peat, MT.PeatBituminous }) {
            FM.FluidBed.addRecipe1(
                T,
                -1,
                (tMat.mFurnaceBurnTime * 3 * EU_PER_FURNACE_TICK) * 9,
                OP.blockDust.mat(tMat, 1),
                FL.Calcite.make(648),
                NF,
                OM.dust(tMat.mTargetBurning.mMaterial, UT.Code.units(tMat.mTargetBurning.mAmount, U * 2, U * 9, F)));
            FM.FluidBed.addRecipe1(
                T,
                -1,
                (tMat.mFurnaceBurnTime * 3 * EU_PER_FURNACE_TICK),
                OP.dust.mat(tMat, 1),
                FL.Calcite.make(72),
                NF,
                OM.dust(tMat.mTargetBurning.mMaterial, UT.Code.units(tMat.mTargetBurning.mAmount, U * 2, U, F)));
            FM.FluidBed.addRecipe1(
                T,
                -1,
                (tMat.mFurnaceBurnTime * 3 * EU_PER_FURNACE_TICK) / 4,
                OP.dustSmall.mat(tMat, 1),
                FL.Calcite.make(18),
                NF,
                OM.dust(tMat.mTargetBurning.mMaterial, UT.Code.units(tMat.mTargetBurning.mAmount, U * 2, U4, F)));
            FM.FluidBed.addRecipe1(
                T,
                -1,
                (tMat.mFurnaceBurnTime * 3 * EU_PER_FURNACE_TICK) / 9,
                OP.dustTiny.mat(tMat, 1),
                FL.Calcite.make(8),
                NF,
                OM.dust(tMat.mTargetBurning.mMaterial, UT.Code.units(tMat.mTargetBurning.mAmount, U * 2, U9, F)));
            FM.FluidBed.addRecipe1(
                T,
                -1,
                (tMat.mFurnaceBurnTime * 3 * EU_PER_FURNACE_TICK) / 72,
                OP.dustDiv72.mat(tMat, 1),
                FL.Calcite.make(1),
                NF,
                OM.dust(tMat.mTargetBurning.mMaterial, UT.Code.units(tMat.mTargetBurning.mAmount, U * 2, U72, F)));
        }
        ...
        for (FluidStack tFuel : FL.Tar.list(1)) FM.Burn.addRecipe0(T, -16, 4, tFuel, FL.CarbonDioxide.make(1), ZL_IS);
        ...
        for (String tAlcohol : new String[] { "potion.dragonblood", "potion.goldencider", "potion.notchesbrew" })
        ...
        if (FL.Hot_Water.exists()) FM.Hot.addRecipe0(T, -2, 1, FL.Hot_Water.make(1), FL.Water.make(1), ZL_IS);
        if (FL.Water_Hot.exists()) FM.Hot.addRecipe0(T, -2, 1, FL.Water_Hot.make(1), FL.Water.make(1), ZL_IS);
        if (FL.Water_Boiling.exists()) FM.Hot.addRecipe0(T, -2, 1, FL.Water_Boiling.make(1), FL.Water.make(1), ZL_IS);
        ...
        FM.Hot.addRecipe0(
            T,
            -1,
            EU_PER_SEMI_HEAVY_WATER,
            FL.Hot_Semi_Heavy_Water.make(1),
            FL.amount(MT.HDO.mLiquid, 1),
            ZL_IS);
        FM.Hot.addRecipe0(
            T,
            -1,
            EU_PER_TRITIATED_WATER,
            FL.Hot_Tritiated_Water.make(1),
            FL.amount(MT.T2O.mLiquid, 1),
            ZL_IS);
        ...
        // new Recipe(new ItemStack(Items.lava_bucket), new ItemStack(Blocks.obsidian), OM.get(OP.ingot, MT.Copper, 1),
        // OM.get(OP.ingot, MT.Tin, 1), OM.get(OP.ingot, MT.Electrum, 1), 30, 2);

        // RecipeMap.sSmallNaquadahReactorFuels.addRecipe(T, ST.array(OM.get(OP.bolt , MT.Nq_528, 1)},
        // ST.array(OM.get(OP.bolt , MT.Nq , 1)}, null, null, null, null, 0, 0, 25000);
        // RecipeMap.sLargeNaquadahReactorFuels.addRecipe(T, ST.array(OM.get(OP.ingot, MT.Nq_528, 1)},
        // ST.array(OM.get(OP.ingot, MT.Nq , 1)}, null, null, null, null, 0, 0, 200000);
        // RecipeMap.sFluidNaquadahReactorFuels.addRecipe(T, ST.array(OM.get(OP.cell , MT.Nq_528, 1)},
        // ST.array(OM.get(OP.cell , MT.Empty , 1)}, null, null, null, null, 0, 0, 200000);
