    ...
    public void onUpdate() {
        ...
        if (tBlock.getMaterial() != Material.air) {
            ...
            AxisAlignedBB axisalignedbb = tBlock
                .getCollisionBoundingBoxFromPool(worldObj, mHitBlockX, mHitBlockY, mHitBlockZ);
        ...
        if (inGround) {
            ...
            List tAllPotentiallyHitEntities = worldObj.getEntitiesWithinAABBExcludingEntity(
                this,
            ...
            if (tVector != null && tHitEntity != null && tHitEntity instanceof EntityPlayer) {
                if (((EntityPlayer) tHitEntity).capabilities.disableDamage || (tShootingEntity instanceof EntityPlayer
                    && !((EntityPlayer) tShootingEntity).canAttackPlayer((EntityPlayer) tHitEntity))) tVector = null;
            }
            ...
            if (tVector != null) {
                if (tHitEntity != null) {
                    ...
                    if (tShootingEntity instanceof EntityPlayer) MinecraftForge.EVENT_BUS
                        .post(new AttackEntityEvent((EntityPlayer) tShootingEntity, tHitEntity));

                    float tMagicDamage = tHitEntity instanceof EntityLivingBase
                        ? EnchantmentHelper
                            .func_152377_a(mArrow, ((EntityLivingBase) tHitEntity).getCreatureAttribute())
                        : 0,
                        tDamage = UT.Code
                            .roundUp(
                                MathHelper.sqrt_double(motionX * motionX + motionY * motionY + motionZ * motionZ)
                                    * (getDamage() + Math.max(
                                        ...
                                        tData != null && tData.validMaterial()
                                            ? tData.mMaterial.mMaterial.mToolQuality - 1
                                            : 0)));

                    if (getIsCritical()) tDamage += rand.nextInt((int) (tDamage / 2.0 + 2.0));

                    int tImplosion = UT.NBT.getEnchantmentLevelImplosion(mArrow),
                        tFireDamage = (isBurning() ? 5 : 0)
                            + 4 * UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, mArrow),
                    ...
                    if (tImplosion > 0 && UT.Entities.isExplosiveCreature(tHitEntity)
                        && !EntityCreeper.class.isInstance(tHitEntity)) tMagicDamage += 1.5F * tImplosion;

                    int[] tDamages = onHitEntity(
                        tHitEntity,
                        tShootingEntity == null ? this : tShootingEntity,
                        mArrow == null ? ST.make(Items.arrow, 1, 0) : mArrow,
                        (int) (tDamage * 2),
                        (int) (tMagicDamage * 2),
                        tKnockback,
                        tFireDamage,
                    ...
                    if (tDamages != null) {
                        ...
                        if (!(tHitEntity instanceof EntityPlayer)
                            && UT.NBT.getEnchantmentLevel(Enchantment.looting, mArrow) > 0) {
                            EntityPlayer tPlayer = null;
                            if (worldObj instanceof WorldServer) tPlayer = FakePlayerFactory.get(
                            ...
                            if (tPlayer != null) {
                                ...
                                // Bypasses Twilight Forest Progression Checks. Yeah this is needed or else any Looting
                                // Arrow would do ZERO Damage.
                        ...
                        if (tDamage + tMagicDamage > 0 && tHitEntity
                            .attackEntityFrom(tDamageSource, (tDamage + tMagicDamage) * TFC_DAMAGE_MULTIPLIER)) {
                            if (tHitEntity instanceof EntityLivingBase) {
                                if (tHitTimer >= 0) tHitEntity.hurtResistantTime = tHitTimer;

                                if (tHitEntity instanceof EntityCreeper
                                    && UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, mArrow) > 0
                                    && tImplosion <= 0) ((EntityCreeper) tHitEntity).func_146079_cb();

                                EntityLivingBase tHitLivingEntity = (EntityLivingBase) tHitEntity;

                                if (!worldObj.isRemote) tHitLivingEntity
                                ...
                                if (tKnockback > 0) {
                                    ...
                                    if (tKnockbackDivider > 0.0F) tHitLivingEntity.addVelocity(
                                        motionX * tKnockback * 0.6000000238418579D / tKnockbackDivider,
                                        0.1D,
                                        motionZ * tKnockback * 0.6000000238418579D / tKnockbackDivider);
                                }

                                Enchantments.applyBullshitA(
                                    tHitLivingEntity,
                                    tShootingEntity == null ? this : tShootingEntity,
                                    mArrow);
                                Enchantments.applyBullshitB(
                            ...
                            if (!(tHitEntity instanceof EntityEnderman)
                                || ((EntityEnderman) tHitEntity).getActivePotionEffect(Potion.weakness) != null) {
                    ...
                    if (mHitBlock.getMaterial() != Material.air)
                        mHitBlock.onEntityCollidedWithBlock(worldObj, mHitBlockX, mHitBlockY, mHitBlockZ, this);

                    if (!worldObj.isRemote && UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, mArrow) > 2)
                        WD.burn(worldObj, mHitBlockX, mHitBlockY, mHitBlockZ, T, F);

                    if (breaksOnImpact()) setDead();
                }
            }

            if (getIsCritical()) for (int i = 0; i < 4; ++i) worldObj.spawnParticle(
                "crit",
                posX + motionX * i / 4.0D,
                posY + motionY * i / 4.0D,
                posZ + motionZ * i / 4.0D,
            ...
            for (rotationPitch = (float) (Math
                .atan2(motionY, MathHelper.sqrt_double(motionX * motionX + motionZ * motionZ)) * 180.0D
                / Math.PI); rotationPitch - prevRotationPitch < -180.0F; prevRotationPitch -= 360.0F) {/**/}
            ...
            if (isInWater()) {
                for (int l = 0; l < 4; ++l) worldObj.spawnParticle(
                    "bubble",
                    posX - motionX * 0.25,
                    posY - motionY * 0.25,
                    posZ - motionZ * 0.25,
