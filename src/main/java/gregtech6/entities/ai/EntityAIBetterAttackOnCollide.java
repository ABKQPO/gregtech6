...
public class EntityAIBetterAttackOnCollide extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (--mPathCoolDown <= 0) {
            mPath = mCreature.getNavigator()
                .getPathToEntityLiving(entitylivingbase);
            mPathCoolDown = 4 + mCreature.getRNG()
                .nextInt(7);
            return mPath != null;
        }
        return T;
    ...
    public boolean continueExecuting() {
        ...
        return tTarget != null && tTarget.isEntityAlive()
            && (!mLastingMemory ? !mCreature.getNavigator()
                .noPath()
                : mCreature.isWithinHomeDistance(
                    MathHelper.floor_double(tTarget.posX),
                    MathHelper.floor_double(tTarget.posY),
                    MathHelper.floor_double(tTarget.posZ)));
    }

    @Override
    public void startExecuting() {
        mCreature.getNavigator()
            .setPath(mPath, mSpeedToTarget);
        mPathCoolDown = 0;
    }
    ...
    public void updateTask() {
        EntityLivingBase tTarget = mCreature.getAttackTarget();
        mCreature.getLookHelper()
            .setLookPositionWithEntity(tTarget, 30, 30);
        ...
        mPathCoolDown--;
        if ((mLastingMemory || mCreature.getEntitySenses()
            .canSee(tTarget))
            && mPathCoolDown <= 0
            && ((mX == 0 && mY == 0 && mZ == 0) || tTarget.getDistanceSq(mX, mY, mZ) >= 1
                || mCreature.getRNG()
                    .nextFloat() < 0.05F)) {
            mX = tTarget.posX;
            mY = tTarget.boundingBox.minY;
            mZ = tTarget.posZ;

            mPathCoolDown = mFailedPathFindingPenalty + 4
                + mCreature.getRNG()
                    .nextInt(7);
            if (mCreature.getNavigator()
                .getPath() != null) {
                PathPoint tPathPoint = mCreature.getNavigator()
                    .getPath()
                    .getFinalPathPoint();
                if (tPathPoint != null
                    && tTarget.getDistanceSq(tPathPoint.xCoord, tPathPoint.yCoord, tPathPoint.zCoord) < 1) {
            ...
            if (!mCreature.getNavigator()
                .tryMoveToEntityLiving(tTarget, mSpeedToTarget)) {
                mPathCoolDown += 15;
            }
        }
        ...
        if (tTargetDistance <= tLookRadius && mAttackCoolDown <= 0) {
            ...
            if (tHeld != null) {
                ...
                } else if (ZOMBIES_DIG_WITH_TOOLS) {
                    // TODO: Handle tools to break things
                    // 1. figure out what the `held` item can work on.
                    // 2. Get nearby block that the tool works on adjust up/down or even if the target is high or low in
                    // comparison, skip TE's and such perhaps with ZOMBIES_DIG_TILEENTITIES.
                    // 3. Use up some of the `held` tool and break that block then set `attackTick` to something hig
                    // based on toughness of that block or so (or add another counter to take 'time' to break
                    // something?)
                }
            }
