...
public class GT_Tool_Scissors extends ToolStats {
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        ...
        return (tTool != null && (tTool.equalsIgnoreCase(TOOL_scissors) || tTool.equalsIgnoreCase(TOOL_shears)))
            || aBlock.getMaterial() == Material.cloth
            || aBlock.getMaterial() == Material.web
            || aBlock == Blocks.vine
            || IL.TF_Mazehedge.equal(aBlock)
            || IL.EBXL_Vines.equal(aBlock)
            || IL.BoP_Vines.equal(aBlock);
    }
    ...
    public void onStatsAddedToTool(MultiItemTool aItem, int aID) {
        aItem.addItemBehavior(
            ...
            new Behavior_Tool(
                TOOL_knife,
                SFX.MC_DIG_CLOTH,
                getToolDamagePerContainerCraft(),
                !canBlock(),
                SFX.RANDOM_PITCH));
