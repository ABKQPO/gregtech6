...
public class GT_Tool_BranchCutter extends ToolStats {
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        if (aBlock.getMaterial() == Material.leaves) aEvent.dropChance = Math.min(
            1.0F,
            Math.max(
                aEvent.dropChance,
                (UT.Code.bind4(
        ...
        if (aBlock == Blocks.leaves) {
            aDrops.clear();
            if ((aMetaData & 3) == 0 && RNGSUS.nextInt(9) <= aFortune * 2) aDrops.add(IL.Food_Apple_Red.get(1));
            else aDrops.add(ST.make(Blocks.sapling, 1, aMetaData & 3));
            ...
            if (RNGSUS.nextInt(9) <= aFortune * 2) aDrops.add(IL.AETHER_Apple.get(1));
            else aDrops.add(IL.AETHER_Skyroot_Sapling_Purple.get(1));
        }
        return 0;
    }
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        return "grafter".equalsIgnoreCase(aBlock.getHarvestTool(aMetaData)) || aBlock == Blocks.vine
