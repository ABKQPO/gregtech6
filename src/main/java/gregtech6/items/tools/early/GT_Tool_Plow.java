...
public class GT_Tool_Plow extends ToolStats {
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        return TOOL_plow.equalsIgnoreCase(aBlock.getHarvestTool(aMetaData)) || aBlock.getMaterial() == Material.fire
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        if (sIsHarvestingRightNow.get() == null && aPlayer instanceof EntityPlayerMP) {
            ...
            for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++)
                for (int k = -1; k < 2; k++) if (i != 0 || j != 0 || k != 0) if (aStack.getItem()
                    .getDigSpeed(
                        aStack,
    ...
    public IIconContainer getIcon(boolean aIsToolHead, ItemStack aStack) {
        return aIsToolHead
            ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mTextureSetsItems.get(OP.toolHeadPlow.mIconIndexItem)
