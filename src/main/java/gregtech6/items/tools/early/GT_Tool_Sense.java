...
public class GT_Tool_Sense extends ToolStats {
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        ...
        return (tTool != null && (tTool.equalsIgnoreCase(TOOL_sense) || tTool.equalsIgnoreCase(TOOL_scythe)))
            || aBlock.getMaterial() == Material.plants
            || aBlock.getMaterial() == Material.leaves
            || aBlock.getMaterial() == Material.vine
            || IL.TF_Mazehedge.equal(aBlock)
            || IL.NeLi_Wart_Block_Crimson.equal(aBlock);
    }
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        if (sIsHarvestingRightNow.get() == null && aPlayer instanceof EntityPlayerMP) {
            ...
            for (int i = -1; i < 2; i++)
                for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++) if (i != 0 || j != 0 || k != 0) {
                    if (aStack.getItem()
                        .getDigSpeed(
                            aStack,
                        ...
                        > 0) {
                        if (((EntityPlayerMP) aPlayer).theItemInWorldManager.tryHarvestBlock(aX + i, aY + j, aZ + k))
                            rConversions++;
                    }
                }
    ...
    public IIconContainer getIcon(boolean aIsToolHead, ItemStack aStack) {
        return aIsToolHead
            ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mTextureSetsItems.get(OP.toolHeadSense.mIconIndexItem)
