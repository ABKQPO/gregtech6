...
public class GT_Tool_HardHammer extends ToolStats {
    ...
    public static final List<String> mEffectiveList = new ArrayListNoNulls<>(
        F,
    ...
    public float getNormalDamageAgainstEntity(float aOriginalDamage, Entity aEntity, ItemStack aStack,
        ...
        return aEntity instanceof EntityIronGolem || mEffectiveList.contains(tName) || tName.contains("golem")
            ? aOriginalDamage * 2
            : aOriginalDamage;
    }
    ...
    public String getCraftingSound() {
        return SFX.MC_ANVIL_LAND;
    }

    @Override
    public String getBreakingSound() {
        return SFX.MC_ANVIL_BREAK;
    }
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        ...
        return (tTool != null && (tTool.equalsIgnoreCase(TOOL_hammer) || tTool.equalsIgnoreCase(TOOL_pickaxe)))
            || aBlock instanceof BlockSilverfish
            ...
            || RM.Hammer.containsInput(ST.make(aBlock, 1, aMetaData), null, NI);
    }
    ...
    public int convertBlockDrops(List<ItemStack> aDrops, ItemStack aStack, EntityPlayer aPlayer, Block aBlock,
        ...
        int rConversions = 0;
        Recipe tRecipe;
        if (aBlock.hasTileEntity(aMetaData) || null == (tRecipe = RM.Hammer
            .findRecipe(null, null, T, Integer.MAX_VALUE, null, ZL_FS, ST.make(aBlock, 1, aMetaData)))) {
            List<ItemStack> tDrops = ST.arraylist();
            for (int i = 0; i < aDrops.size(); i++) {
                tRecipe = RM.Hammer
                    .findRecipe(null, null, T, Integer.MAX_VALUE, null, ZL_FS, ST.amount(1, aDrops.get(i)));
                if (tRecipe != null) {
            ...
            for (int k = 0; k < tHammeringOutput.length; k++)
                if (tHammeringOutput[k] != null) aDrops.add(tHammeringOutput[k]);
            rConversions++;
        }
        return rConversions;
    }
    ...
    public IIconContainer getIcon(boolean aIsToolHead, ItemStack aStack) {
        return aIsToolHead
            ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mTextureSetsItems.get(OP.toolHeadHammer.mIconIndexItem)
