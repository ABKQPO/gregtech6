...
public class GT_Tool_Screwdriver extends ToolStats {
    ...
    public boolean isMinableBlock(Block aBlock, byte aMetaData) {
        return TOOL_screwdriver.equalsIgnoreCase(aBlock.getHarvestTool(aMetaData))
            || aBlock.getMaterial() == Material.circuits;
    }

    @Override
    public IIconContainer getIcon(boolean aIsToolHead, ItemStack aStack) {
        return !aIsToolHead ? MultiItemTool.getPrimaryMaterial(aStack, MT.Steel).mTextureSetsItems
            .get(OP.toolHeadScrewdriver.mIconIndexItem) : Textures.ItemIcons.HANDLE_SCREWDRIVER;
    }
    ...
    public void onStatsAddedToTool(MultiItemTool aItem, int aID) {
        aItem.addItemBehavior(
            aID,
            new Behavior_Tool(TOOL_screwdriver, SFX.GT_SCREWDRIVER, 100, !canBlock(), SFX.RANDOM_PITCH));
    }
