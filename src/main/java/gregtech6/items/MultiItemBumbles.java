...
public class MultiItemBumbles extends MultiItemRandomWithCompat implements IItemBumbleBee {
    ...
    public void addItems() {
        ...
        make(500, "Stoned Bumblebee", "Duuuude");// Forgot to remove one reference last year. I need a new thing for the
                                                 // Stoner Bumble to say...
        ...
        make(20230, "Aero Bumble", "According to the Laws of Fluid Dynamics, this Bumblebee CAN Fly!");// TODO Change
                                                                                                       // Bumble Texture
    ...
    public ItemStack bumbleProductStack(ItemStack aBumbleBee, short aMetaData, long aStacksize, int aProductIndex) {
        switch (aMetaData / 100) {
            ...
            case 1:
                return IL.Comb_Water.get(aStacksize);
            case 2:
                return IL.Comb_Magic.get(aStacksize);
            case 3:
                return IL.Comb_Nether.get(aStacksize);
            case 4:
                return IL.Comb_End.get(aStacksize);
            case 5:
                return IL.Comb_Rock.get(aStacksize);
            case 6:
                return IL.Comb_Jungle.get(aStacksize);
            case 7:
                return IL.Comb_Frozen.get(aStacksize);
            case 8:
                return IL.Comb_Shroom.get(aStacksize);
            case 9:
                return IL.Comb_Sandy.get(aStacksize);
            case 100:
                return IL.Comb_Clay.get(aStacksize);
            case 101:
                return IL.Comb_Sticky.get(aStacksize);
            case 102:
                return IL.Comb_Royal.get(aStacksize);
            case 103:
                return IL.Comb_Soul.get(aStacksize);
            case 104:
                return IL.Comb_Amnesic.get(aStacksize);
            case 105:
                return IL.Comb_Military.get(aStacksize);
            case 200:
                return IL.Comb_Pyro.get(aStacksize);
            case 201:
                return IL.Comb_Cryo.get(aStacksize);
            case 202:
                return IL.Comb_Aero.get(aStacksize);
            case 203:
                return IL.Comb_Tera.get(aStacksize);
            default:
                return IL.Comb_Honey.get(aStacksize);
        }
    }
    ...
    @Override
    public ChunkCoordinates bumbleCanProduce(World aWorld, int aX, int aY, int aZ, ItemStack aBumbleBee,
        short aMetaData, int aDistance) {
        boolean temp = T;
        ...
        int[] tOrderX = RNGSUS.nextBoolean()
            ? aDistance < SCANS_POS.length ? SCANS_POS[aDistance] : SCANS_POS[SCANS_POS.length - 1]
            : aDistance < SCANS_NEG.length ? SCANS_NEG[aDistance] : SCANS_NEG[SCANS_NEG.length - 1];
        int[] tOrderY = RNGSUS.nextBoolean()
            ? aDistance < SCANS_POS.length ? SCANS_POS[aDistance] : SCANS_POS[SCANS_POS.length - 1]
            : aDistance < SCANS_NEG.length ? SCANS_NEG[aDistance] : SCANS_NEG[SCANS_NEG.length - 1];
        int[] tOrderZ = RNGSUS.nextBoolean()
            ? aDistance < SCANS_POS.length ? SCANS_POS[aDistance] : SCANS_POS[SCANS_POS.length - 1]
            : aDistance < SCANS_NEG.length ? SCANS_NEG[aDistance] : SCANS_NEG[SCANS_NEG.length - 1];
        switch (aMetaData / 100) {
            case 1:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    if (WD.anywater(aWorld, aX + i, aY + j, aZ + k))
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 2:
                if (BIOMES_MAGICAL.contains(aWorld.getBiomeGenForCoords(aX, aZ).biomeName))
                ...
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    ...
                    if (tBlock == BlocksGT.Leaves_AB && (WD.meta(aWorld, aX + i, aY + j, aZ + k, F) & 7) == 7)
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 3:
            ...
            case 202:
                ...
                if (tChorusFlower == null) {
                    if (aWorld.provider.dimensionId == 1
                        || BIOMES_END.contains(aWorld.getBiomeGenForCoords(aX, aZ).biomeName))
                        return new ChunkCoordinates(aX, aY, aZ);
                    for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                        Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                        if (tBlock == Blocks.end_portal || tBlock == Blocks.dragon_egg)
                            return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                    }
                    return null;
                }
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                    if (tBlock == tChorusFlower || tBlock == Blocks.dragon_egg)
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 5:
            ...
            case 6:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    if (WD.block(aWorld, aX + i, aY + j, aZ + k, F) == Blocks.cocoa)
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 7:
            case 201:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                    if (tBlock == Blocks.ice || tBlock == Blocks.snow_layer
                        || tBlock == Blocks.snow
                        || tBlock == Blocks.packed_ice) return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 8:
                ...
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                    if (tBlock == Blocks.mycelium || tBlock == Blocks.red_mushroom
                        || tBlock == Blocks.brown_mushroom
                        || tBlock == tMushroom
                        || tBlock == Blocks.red_mushroom_block
                        || tBlock == Blocks.brown_mushroom_block) return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 9:
            case 105:
                ...
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                    if (tBlock == Blocks.cactus || tBlock == tCactus2 || tBlock == BlocksGT.FlowersB)
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                    if (tBlock == tCactus1) {
                        if (WD.meta(aWorld, aX + i, aY + j, aZ + k) == 12)
                            return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                        continue;
                    }
                    if (tBlock == Blocks.flower_pot) {
                        ...
                        if (tTileEntity instanceof TileEntityFlowerPot) {
                            if (Block.getBlockFromItem(((TileEntityFlowerPot) tTileEntity).getFlowerPotItem())
                                == Blocks.cactus) return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                        }
                        continue;
                    }
                }
            ...
            case 100:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    Block tBlock = WD.block(aWorld, aX + i, aY + j, aZ + k, F);
                    if (tBlock == Blocks.clay || (tBlock == BlocksGT.Diggables
                        && BlockDiggable.IS_CLAY[WD.meta(aWorld, aX + i, aY + j, aZ + k)]))
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 101:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    ...
                    if (tBlock == IL.IC2_Log_Rubber.block()) {
                        if (WD.meta(aWorld, aX + i, aY + j, aZ + k) == 0) continue;
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                    }
                    if (tBlock instanceof MultiTileEntityBlock
                        && WD.te(aWorld, aX + i, aY + j, aZ + k, F) instanceof MultiTileEntityResinHoleRubber)
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            case 103:
                for (int j : tOrderY) for (int i : tOrderX) for (int k : tOrderZ) {
                    if (OM.is(OD.soulsand, ST.make(WD.block(aWorld, aX + i, aY + j, aZ + k, F), 1, W)))
                        return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
            default:
                if (RNGSUS.nextBoolean()) {
                    for (int j : tOrderY)
                        for (int i : tOrderX) for (int k : tOrderZ) if (checkFlowers(aWorld, aX + i, aY + j, aZ + k))
                            return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                } else {
                    for (int j : tOrderY)
                        for (int k : tOrderZ) for (int i : tOrderX) if (checkFlowers(aWorld, aX + i, aY + j, aZ + k))
                            return new ChunkCoordinates(aX + i, aY + j, aZ + k);
                }
                return null;
        }
    ...
    @Override
    public ItemStack bumbleCombine(ItemStack aBumbleBeeA, short aMetaDataA, ItemStack aBumbleBeeB, short aMetaDataB,
        byte aBumbleType, Random aRandom) {
        // Special Case to give even chances to Addon Bumbles.
        if (aBumbleBeeB.getItem() != ItemsGT.BUMBLEBEES) return ((IItemBumbleBee) aBumbleBeeB.getItem())
            .bumbleCombine(aBumbleBeeA, aMetaDataA, aBumbleBeeB, aMetaDataB, aBumbleType, aRandom);
        if (aBumbleBeeA.getItem() != ItemsGT.BUMBLEBEES) return ((IItemBumbleBee) aBumbleBeeA.getItem())
            .bumbleCombine(aBumbleBeeA, aMetaDataA, aBumbleBeeB, aMetaDataB, aBumbleType, aRandom);
        // The Normal Case if the other Bumble does not match. Should not be possible to call this one in my own class
        // anymore though, because above.
        ...
        switch (aMetaDataA / 10) {
            case 3:
                switch (aMetaDataB / 10) {
                    case 13:
                        return ST.make(this, 1, 10100 + aBumbleType);
                    case 53:
                        return ST.make(this, 1, 10000 + aBumbleType);
                    case 93:
                        return ST.make(this, 1, 10200 + aBumbleType);
                }
                break;
            case 13:
                switch (aMetaDataB / 10) {
                    case 3:
                        return ST.make(this, 1, 10100 + aBumbleType);
                    case 53:
                        return ST.make(this, 1, 10400 + aBumbleType);
                }
                break;
            case 33:
                switch (aMetaDataB / 10) {
                    case 43:
                        return ST.make(this, 1, 10300 + aBumbleType);
                    case 1053:
                        return ST.make(this, 1, 20000 + aBumbleType);
                }
                break;
            case 43:
                switch (aMetaDataB / 10) {
                    case 33:
                        return ST.make(this, 1, 10300 + aBumbleType);
                    case 1053:
                        return ST.make(this, 1, 20200 + aBumbleType);
                }
                break;
            case 53:
                switch (aMetaDataB / 10) {
                    ...
                    case 13:
                        return ST.make(this, 1, 10400 + aBumbleType);
                    case 1053:
                        return ST.make(this, 1, 20300 + aBumbleType);
                }
                break;
            case 63:
                switch (aMetaDataB / 10) {
                    case 93:
                        return ST.make(this, 1, 10500 + aBumbleType);
                }
                break;
            case 73:
                switch (aMetaDataB / 10) {
                    case 1053:
                        return ST.make(this, 1, 20100 + aBumbleType);
                }
                break;
            case 93:
                switch (aMetaDataB / 10) {
    ...
    public boolean bumbleAttack(ItemStack aBumbleBee, short aMetaData, EntityLivingBase aAttacked) {
        ...
        boolean tSkeleton = (aAttacked instanceof EntitySkeleton
            || (aAttacked instanceof EntityHorse && ((EntityHorse) aAttacked).getHorseType() == 4)),
            tSnowGolem = (aAttacked.getClass() == EntitySnowman.class),
            tIronGolem = (aAttacked instanceof EntityIronGolem), tPlayer = (aAttacked instanceof EntityPlayer);
        switch (aMetaData / 100) {
            default:
                return !tSkeleton && !tSnowGolem
                    && !tIronGolem
                    && aAttacked.attackEntityFrom(DamageSources.getBumbleDamage(), (1 + ((aMetaData / 10) % 10)));
            case 9:
                return !tSkeleton && !tSnowGolem
                    && !tIronGolem
                    && aAttacked.attackEntityFrom(DamageSources.getBumbleDamage(), (1 + ((aMetaData / 10) % 10)) * 2);
            case 6:
                return !tSkeleton && !tSnowGolem
                    && !tIronGolem
                    && aAttacked.attackEntityFrom(DamageSources.getBumbleDamage(), (1 + ((aMetaData / 10) % 10)) * 4);
            case 8:
                return F;
            case 3:
                if (!tSkeleton && !tIronGolem
                    && aAttacked.attackEntityFrom(
                        DamageSources.getBumbleDamage()
                            .setFireDamage(),
                        (1 + ((aMetaData / 10) % 10)) * 2)) {
            ...
            case 203:
                return !tPlayer
                    && aAttacked.attackEntityFrom(DamageSources.getBumbleDamage(), (1 + ((aMetaData / 10) % 10)) * 10);
        }
    }
    ...
    @Override
    public ChunkCoordinates bumbleCanProduct(World aWorld, int aX, int aY, int aZ, ItemStack aBumbleBee,
        short aMetaData, int aProductIndex) {
        return new ChunkCoordinates(aX, aY, aZ);
    }
    ...
    public void addAdditionalToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (aStack.hasTagCompound()) aBumbleTag = aStack.getTagCompound()
            .getCompoundTag("gt.bumble");
        if (aBumbleTag == null || aBumbleTag.hasNoTags()) {
            ...
            if (aMeta % 10 < 5) {
                ...
                aList.add(
                    LH.Chat.CYAN + "Humidity:"
                        + LH.Chat._WHITE
                        + Util.getHumidityMin(aBumbleTag)
                        + " to "
                        + Util.getHumidityMax(aBumbleTag)
                        + LH.Chat.RED
                        + "   Temp:"
                        + LH.Chat._WHITE
                        + Util.getTemperatureMin(aBumbleTag)
                        + "K to "
                        + Util.getTemperatureMax(aBumbleTag)
                        + "K");
                aList.add(
                    LH.Chat.GREEN + "Offspring:"
                        + LH.Chat._WHITE
                        + Util.getOffspring(aBumbleTag)
                        + LH.Chat.ORANGE
                        + "   Life:"
                        + LH.Chat._WHITE
                        + Util.getLifeSpan(aBumbleTag)
                        + " ticks");
                aList.add(
                    LH.Chat.YELLOW + "Eff:"
                        + LH.Chat._WHITE
                        + LH.percent(Util.getWorkForce(aBumbleTag))
                        + "%"
                        + LH.Chat.RED
                        + "   Aggro:"
                        + LH.Chat._WHITE
                        + LH.percent(Util.getAggressiveness(aBumbleTag))
                        + "%");
    ...
    public void make(int aSpeciesID, String aName, String aTooltip) {
        for (int i : new int[] { 0, 1, 4 }) {
            for (String tFluid : FluidsGT.HONEY) if (FL.exists(tFluid)) RM.Bumblelyzer.addFakeRecipe(
                F,
                ST.array(ST.make(this, 1, aSpeciesID + i), OP.plateTiny.mat(MT.Paper, 1)),
                ST.array(ST.make(this, 1, aSpeciesID + i + 5)),
                null,
                null,
                FL.array(FL.make(tFluid, 10)),
                null,
                64,
                16,
                0);
            ...
            RM.Bumblelyzer.addFakeRecipe(
                F,
                ST.array(ST.make(this, 1, aSpeciesID + i + 5, "Was already scanned, auto-skipping")),
                ST.array(ST.make(this, 1, aSpeciesID + i + 5, "Just passed to the Output")),
                null,
                null,
                null,
                null,
        ...
        addItem(
            ...
            new OreDictItemData(MT.Paper, U9));
        addItem(
            aSpeciesID + 6,
            aName + " Princess (Scanned)",
            aTooltip,
            "gt:bumbleprincess",
            TC.stack(TC.BESTIA, 2),
            TC.stack(TC.COGNITIO, 1),
        ...
        RM.BumbleQueens.addFakeRecipe(
            F,
            ST.array(ST.make(this, 1, aSpeciesID + 2), ST.make(this, 1, aSpeciesID + 7)),
            tOutputs,
            null,
            tChances,
            null,
    ...
    public IIcon getIconFromDamage(int aMetaData) {
        aMetaData /= 10;
        aMetaData *= 10;
        return UT.Code.exists(aMetaData, mIconList) && mIconList[aMetaData][0] != null ? mIconList[aMetaData][0]
            : Textures.ItemIcons.RENDERING_ERROR.getIcon(0);
    }

    @Override
    public IIcon getIcon(ItemStack aStack, int aRenderPass, EntityPlayer aPlayer, ItemStack aUsedStack,
        int aUseRemaining) {
