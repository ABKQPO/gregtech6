...
public class Behavior_Sonictron extends AbstractBehaviorDefault {
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        /*
         * if (!aWorld.isRemote && aWorld.getBlock(aX, aY, aZ) == GregTech_API.sBlockMachines &&
         * aWorld.getBlockMetadata(aX, aY, aZ) == 6) {
    ...
    public void onUpdate(MultiItem aItem, ItemStack aStack, World aWorld, Entity aPlayer, int aTimer,
        boolean aIsInHand) {
        int tTickTimer = getTickTimer(aStack), tCurrentIndex = getCurrentIndex(aStack);

        if (tTickTimer++ % 2 == 0 && tCurrentIndex > -1) {
            // ItemStack[] tInventory = getNBTInventory(aStack);
            // GT.doSonictronSound(tInventory[tCurrentIndex], aPlayer.worldObj, aPlayer.posX, aPlayer.posY,
            // aPlayer.posZ);
    ...
    public static void copyInventory(ItemStack aInventory[], ItemStack aNewContent[], int aIndexlength) {
        for (int i = 0; i < aIndexlength; i++) {
            if (aNewContent[i] == null) aInventory[i] = null;
            else aInventory[i] = ST.copy(aNewContent[i]);
        }
    }
}
