...
public class Behavior_Arrow_Potion extends Behavior_Arrow {
    ...
    public Behavior_Arrow_Potion(float aSpeed, float aPrecision, Enchantment aEnchantment, int aLevel,
        int... aPotions) {
    ...
    public boolean onLeftClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        if (aEntity instanceof EntityLivingBase)
            for (int i = 3; i < mPotions.length; i += 4) if (RNGSUS.nextInt(100) < mPotions[i])
                UT.Entities.applyPotion(aEntity, mPotions[i - 3], mPotions[i - 2], mPotions[i - 1], F);
