...
public class Behavior_Gun extends AbstractBehaviorDefault {
    ...
    public boolean shoot(ItemStack aGun, ItemStack aBullet, EntityPlayer aPlayer) {
        ...
        int tFireAspect = UT.NBT.getEnchantmentLevel(Enchantment.flame, aGun)
            + UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, aBullet);

        // Make a List of all possible Targets.
        List tEntities = aPlayer.worldObj.getEntitiesWithinAABBExcludingEntity(
            aPlayer,
            AxisAlignedBB.getBoundingBox(
                Math.min(tPos.xCoord, tAim.xCoord) - 2,
                Math.min(tPos.yCoord, tAim.yCoord) - 2,
                Math.min(tPos.zCoord, tAim.zCoord) - 2,
                Math.max(tPos.xCoord, tAim.xCoord) + 2,
                Math.max(tPos.yCoord, tAim.yCoord) + 2,
                Math.max(tPos.zCoord, tAim.zCoord) + 2));
        ...
        for (int i = 1, ii = aCoords.size() - 1; i < ii; i++) {
            ...
            for (int j = 0; j < tTargets.size(); j++) {
                if (tTargets.get(j)
                    .getDistanceSq(aCoord.posX + 0.5, aCoord.posY + 0.5, aCoord.posZ + 0.5)
                    < tTargets.get(j)
                        .getDistanceSq(nCoord.posX + 0.5, nCoord.posY + 0.5, nCoord.posZ + 0.5)) {
            ...
            if (aBlock instanceof BlockPumpkin
                || WD.te(aPlayer.worldObj, aCoord, T) instanceof MultiTileEntityGregOLantern) {
                if (RNGSUS.nextInt(3) == 0) {
                    ST.drop(
                        aPlayer.worldObj,
                        aCoord.posX + 0.2 + RNGSUS.nextFloat() * 0.6,
                        aCoord.posY + 0.1 + RNGSUS.nextFloat() * 0.5,
                        aCoord.posZ + 0.2 + RNGSUS.nextFloat() * 0.6,
                        ST.make(Blocks.pumpkin, 1, 0));
                } else {
                    ST.drop(
                        aPlayer.worldObj,
                        aCoord.posX + 0.2 + RNGSUS.nextFloat() * 0.6,
                        aCoord.posY + 0.1 + RNGSUS.nextFloat() * 0.5,
                        aCoord.posZ + 0.2 + RNGSUS.nextFloat() * 0.6,
                        ST.make(Items.pumpkin_seeds, 1 + RNGSUS.nextInt(3), 0));
                }
            ...
            if (aBlock == Blocks.melon_block) {
                ST.drop(
                    ...
                    aCoord.posY + 0.1 + RNGSUS.nextFloat() * 0.5,
                    aCoord.posZ + 0.2 + RNGSUS.nextFloat() * 0.6,
                    ST.make(Items.melon, 1 + RNGSUS.nextInt(6), 0));
                ST.drop(
                    aPlayer.worldObj,
                    aCoord.posX + 0.2 + RNGSUS.nextFloat() * 0.6,
                    aCoord.posY + 0.1 + RNGSUS.nextFloat() * 0.5,
                    aCoord.posZ + 0.2 + RNGSUS.nextFloat() * 0.6,
                    ST.make(Items.melon_seeds, 1 + RNGSUS.nextInt(3), 0));
                WD.set(aPlayer.worldObj, aCoord.posX, aCoord.posY, aCoord.posZ, NB, 0, 3);
                if (tFireAspect > 1) WD.fire(aPlayer.worldObj, aCoord, F);
                UT.Sounds.send(aBlock.stepSound.getBreakSound(), aPlayer.worldObj, aCoord);
                tPower -= 3000;
            ...
            if (aBlock.getMaterial() == Material.glass || aBlock == Blocks.ice
                || aBlock == Blocks.redstone_lamp
                || aBlock == Blocks.lit_redstone_lamp) {
                ...
                for (OreDictMaterialStack tMaterial : tData.getAllMaterialStacks()) {
                    ...
                    while (tAmount-- > 0) {
                        ST.drop(
                            ...
                            aCoord.posY + 0.1 + RNGSUS.nextFloat() * 0.5,
                            aCoord.posZ + 0.2 + RNGSUS.nextFloat() * 0.6,
                            OP.scrapGt.mat(tMaterial.mMaterial, 1));
                    }
                }
                WD.set(aPlayer.worldObj, aCoord.posX, aCoord.posY, aCoord.posZ, NB, 0, 3);
                UT.Sounds.send(aBlock.stepSound.getBreakSound(), aPlayer.worldObj, aCoord);
            ...
            if (aBlock instanceof BlockFence || aBlock instanceof BlockFenceGate
                || aBlock == Blocks.web
                || aBlock == Blocks.mob_spawner
                || aBlock instanceof BlockPane
                || aBlock instanceof BlockRail
                || aBlock instanceof BlockTorch
                || aBlock instanceof BlockBaseBars
                || aBlock instanceof BlockBaseSpike
                || aBlock.getMaterial() == Material.cactus
                || aBlock.getMaterial() == Material.fire
                || aBlock.getMaterial() == Material.air
            ...
            if (aBlock instanceof BlockStairs
                || WD.opq(aPlayer.worldObj, aCoord.posX, aCoord.posY, aCoord.posZ, T, F)) {
                UT.Sounds.send(aBlock.stepSound.getBreakSound(), aPlayer.worldObj, aCoord);
                tPower = 0;
                continue;
            }
            if (aBlock.canCollideCheck(aMeta, F) || aBlock.canCollideCheck(aMeta, T)) {
                AxisAlignedBB tBox = aBlock
                    .getCollisionBoundingBoxFromPool(aPlayer.worldObj, aCoord.posX, aCoord.posY, aCoord.posZ);
    ...
    public boolean hit(ItemStack aGun, ItemStack aBullet, EntityPlayer aPlayer, Entity aTarget, long aPower,
        Vec3 aDir) {
        try {
            ...
            if (aTarget instanceof EntityPlayer && (((EntityPlayer) aTarget).capabilities.disableDamage
                || !aPlayer.canAttackPlayer((EntityPlayer) aTarget))) return F;
            ...
            if (!(aTarget instanceof EntityLivingBase || aTarget instanceof EntityDragonPart
                || aTarget instanceof EntityEnderCrystal)) return F;
            ...
            float tMassFactor = (tData != null && tData.nonemptyMaterial() ? (float) tData.mMaterial.weight() / 50.0F
                : 1),
            ...
            int tImplosion = UT.NBT.getEnchantmentLevelImplosion(aBullet),
                tFireDamage = 4 * (UT.NBT.getEnchantmentLevel(Enchantment.flame, aGun)
                    + UT.NBT.getEnchantmentLevel(Enchantment.fireAspect, aBullet)),
                tKnockback = (UT.NBT.getEnchantmentLevel(Enchantment.punch, aGun)
                    + UT.NBT.getEnchantmentLevel(Enchantment.knockback, aBullet));
            ...
            if (aTarget instanceof EntityPlayer) {
                ...
                // Bigger Bullets deal more Magic Damage just like they already do for Normal Damage, but not against
                // Players.
                // The Reason I didn't just up the Enchantment Level like I did with Looting is because that would
                // increase the Side Effects too.
                tMagicDamage *= mMagic;
                ...
                if (aPlayer.worldObj instanceof WorldServer) {
                    if (UT.NBT.getEnchantmentLevel(Enchantment.looting, aBullet) > 0) {
                        tPlayer = FakePlayerFactory.get(
                            (WorldServer) aPlayer.worldObj,
                            new GameProfile(new UUID(0, 0), ((EntityLivingBase) aPlayer).getCommandSenderName()));
                        tPlayer.inventory.currentItem = 0;
                        tPlayer.inventory.setInventorySlotContents(0, aBullet);
                        tPlayer.setPositionAndRotation(
                            aPlayer.posX,
                            aPlayer.posY,
                            aPlayer.posZ,
                            aPlayer.rotationYaw,
                            aPlayer.rotationPitch);
                        // Bypasses Twilight Forest Progression Checks. Yeah this is needed or else any Looting Bullet
                        // would do ZERO Damage.
            ...
            DamageSource tDamageSource = DamageSources
                .getCombatDamage(
                    "player",
                    tPlayer,
                    DamageSources.getDeathMessage(
                        aPlayer,
                        aTarget,
                        (tData != null && tData.validMaterial()
                            ? "[VICTIM] got killed by [KILLER] shooting a Bullet made of "
                                + tData.mMaterial.mMaterial.getLocal()
                            : "[VICTIM] got shot by [KILLER]")),
                    F)
                .setProjectile();
            // Extremely Fast Bullets will penetrate Armor. You need a Rifle with the Power Enchantment for this. A
            // Power 5 Carbine at point-blank could do too though.
            ...
            if (MD.TF.mLoaded && aTarget instanceof EntityTFLich
                && UT.NBT.getEnchantmentLevel(Enchantment.smite, aBullet) > 0) tDamageSource.setDamageBypassesArmor();

            if (aTarget.attackEntityFrom(tDamageSource, (tDamage + tMagicDamage) * TFC_DAMAGE_MULTIPLIER)) {
                aTarget.hurtResistantTime = (aTarget instanceof EntityLivingBase
                    ? ((EntityLivingBase) aTarget).maxHurtResistantTime
                    : 20);
                if (aTarget instanceof EntityCreeper && tFireDamage > 0 && tImplosion <= 0)
                    ((EntityCreeper) aTarget).func_146079_cb();
                if (tKnockback > 0) aTarget.addVelocity(
                    aDir.xCoord * tKnockback * aPower / 50000.0,
                    0.05,
                    aDir.zCoord * tKnockback * aPower / 50000.0);
                ...
                if (aTarget instanceof EntityPlayer && aPlayer instanceof EntityPlayerMP)
                    ((EntityPlayerMP) aPlayer).playerNetServerHandler.sendPacket(new S2BPacketChangeGameState(6, 0.0F));
            ...
            UT.Entities.sendchat(aPlayer, "See gregtech.log for details: " + e.toString());
            aTarget.setDead();
            return T;
        }
        // Just pretend we miss the Target if it was in its Invulnerability Frames, this will end up hitting whatever is
    ...
    // @Override public boolean onRightClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity
    // aEntity) {onItemRightClick(aItem, aStack, aPlayer.worldObj, aPlayer); return T;}
    @Override
    public boolean onItemUse(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        onItemRightClick(aItem, aStack, aPlayer.worldObj, aPlayer);
        return T;
    }

    @Override
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        int aZ, byte aSide, float hitX, float hitY, float hitZ) {
    ...
    public ItemStack onItemRightClick(MultiItem aItem, ItemStack aGun, World aWorld, EntityPlayer aPlayer) {
        ...
        if (!UT.Entities.hasInfiniteItems(aPlayer)
            && RNGSUS.nextInt(1 + UT.NBT.getEnchantmentLevel(Enchantment.infinity, aGun)) == 0) {
            OreDictItemData tData = OM.anydata(aBullet);
            aBullet.stackSize--;
            ST.save(aNBT, NBT_AMMO, aBullet.stackSize > 0 ? aBullet : NI);
            for (OreDictMaterialStack tMat : tData.mByProducts) if (tMat.mAmount >= OP.scrapGt.mAmount
                && !tMat.mMaterial.containsAny(TD.Properties.EXPLOSIVE, TD.Properties.FLAMMABLE))
                ST.give(aPlayer, OP.scrapGt.mat(tMat.mMaterial, tMat.mAmount / OP.scrapGt.mAmount));
        }
    ...
    public boolean isProjectile(ItemStack aStack) {
        return ST.item(aStack) instanceof IItemProjectile
            && ((IItemProjectile) ST.item(aStack)).hasProjectile(mBulletType, aStack);
    }
    ...
    public boolean reloadGun(ItemStack aGun, EntityPlayer aPlayer, boolean aOnlyCheckHeld) {
        ...
        if (isProjectile(aPlayer.inventory.mainInventory[aPlayer.inventory.currentItem])) {
            int tConsumed = Math
                .min(mAmmoPerMag, aPlayer.inventory.mainInventory[aPlayer.inventory.currentItem].stackSize);
            UT.Sounds.send(SFX.MC_CLICK, 16, aPlayer);
            ST.save(
                aNBT,
                NBT_AMMO,
                ST.amount(tConsumed, aPlayer.inventory.mainInventory[aPlayer.inventory.currentItem]));
        ...
        for (int i = 0; i < aPlayer.inventory.mainInventory.length; i++)
            if (aPlayer.inventory.mainInventory[i] == aGun) {
        ...
        for (int i = aPlayer.inventory.mainInventory.length - 1; i >= 0; i--)
            if (isProjectile(aPlayer.inventory.mainInventory[i])) {
