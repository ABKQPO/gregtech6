...
public class Behavior_Spray_Extinguisher extends AbstractBehaviorDefault {
    ...
    public long extinguish(World aWorld, int aX, int aY, int aZ, byte aSide, long aUses, EntityPlayer aPlayer,
        ItemStack aStack, float aHitX, float aHitY, float aHitZ) {
        if (aPlayer == null || SIDES_INVALID[aSide]
            || aPlayer instanceof FakePlayer
            || !WD.obstructed(aWorld, aX, aY, aZ, aSide)) {
        ...
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++) {
            ...
            if (aWorld.getBlock(aX + i, aY + j, aZ + k) == Blocks.fire
                && aWorld.setBlock(aX + i, aY + j, aZ + k, NB, 0, 3)) rUses += 10;
        }

        for (Object tEntity : aWorld.getEntitiesWithinAABB(
            Entity.class,
            AxisAlignedBB.getBoundingBox(aX - 2, aY - 2, aZ - 2, aX + 3, aY + 3, aZ + 3))) {
            if (rUses + 10 > aUses) return rUses;
