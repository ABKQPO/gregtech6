...
public class Behavior_FlintAndTinder extends AbstractBehaviorDefault {
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (aPlayer != null && SIDES_VALID[aSide]
            && !(aPlayer instanceof FakePlayer)
            && WD.obstructed(aWorld, aX, aY, aZ, aSide)) return F;
        List<String> tChatReturn = new ArrayListNoNulls<>();
        long tDamage = 5000;
        if (MultiItemTool.getPrimaryMaterial(aStack).mToolDurability <= 1) {
            tDamage = IBlockToolable.Util.onToolClick(
                TOOL_igniter,
                Long.MAX_VALUE,
                1,
                aPlayer,
                tChatReturn,
                aPlayer == null ? null : aPlayer.inventory,
                ...
                aHitY,
                aHitZ);
        } else if (MultiItemTool.getPrimaryMaterial(aStack)
            .containsAny(TD.Properties.FLAMMABLE, TD.Properties.BURNING)
            && RNGSUS.nextInt(100)
                < UT.Code.bind(1, 100, GT6_Main.gt_proxy.mFlintChance + (100 - GT6_Main.gt_proxy.mFlintChance) / 2)) {
                    tDamage = IBlockToolable.Util.onToolClick(
                        TOOL_igniter,
                        Long.MAX_VALUE,
                        1,
            ...
            if (RNGSUS.nextInt(100) < UT.Code.bind(1, 100, GT6_Main.gt_proxy.mFlintChance)) {
                tDamage = IBlockToolable.Util.onToolClick(
                    TOOL_igniter,
                    Long.MAX_VALUE,
                    1,
    ...
    public List<String> getAdditionalToolTips(MultiItem aItem, List<String> aList, ItemStack aStack) {
        if (MultiItemTool.getPrimaryMaterial(aStack).mToolDurability <= 1) {
            ...
            .containsAny(TD.Properties.FLAMMABLE, TD.Properties.BURNING)) {
                aList.add(
                    LH.get("gt.behaviour.flintandtinder")
                        + UT.Code
                            .bind(1, 100, GT6_Main.gt_proxy.mFlintChance + (100 - GT6_Main.gt_proxy.mFlintChance) / 2)
                        + "%");
            } else {
                aList.add(
                    LH.get("gt.behaviour.flintandtinder") + UT.Code.bind(1, 100, GT6_Main.gt_proxy.mFlintChance) + "%");
            }
        return aList;
    }
}
