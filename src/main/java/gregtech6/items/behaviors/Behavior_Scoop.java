        ...
        if (aEntity instanceof IEntityButterfly) {
            ...
            if (((MultiItemTool) aItem).doDamage(aStack, mCosts, aPlayer, F)) {
                Object tButterfly = ((IEntityButterfly) aEntity).getButterfly(),
                    tRoot = ((IButterfly) tButterfly).getGenome()
                        .getPrimary()
                        .getRoot();
                ((IButterflyRoot) tRoot).getBreedingTracker(aEntity.worldObj, aPlayer.getGameProfile())
                    .registerCatch(((IButterfly) tButterfly));
                ST.give(
                    aPlayer,
                    ((IButterflyRoot) tRoot)
                        .getMemberStack(((IButterfly) tButterfly).copy(), EnumFlutterType.BUTTERFLY.ordinal()),
                    F);
                aEntity.setDead();
            }
            return T;
