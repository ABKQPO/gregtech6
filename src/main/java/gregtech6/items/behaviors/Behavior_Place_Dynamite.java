...
public class Behavior_Place_Dynamite extends AbstractBehaviorDefault {
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (tBlock instanceof BlockStones) {
            if (tMeta >= 3) return F;
        } else if (!BlocksGT.drillableDynamite.contains(tBlock) && !StoneLayer.REPLACEABLE_BLOCKS.contains(tBlock)
            && !WD.ore_stone(tBlock, tMeta)) return F;

        for (int i = 0; i < aPlayer.inventory.mainInventory.length; i++) {
            ItemStack tStack = aPlayer.inventory.mainInventory[aPlayer.inventory.mainInventory.length - i - 1];
            if (IL.Boomstick.equal(tStack, F, T) || IL.Dynamite.equal(tStack, F, T)
                || IL.Dynamite_Strong.equal(tStack, F, T)) {
                NBTTagCompound tOldTag = tStack.getTagCompound();
                if (tStack.hasTagCompound()) {
                    tStack.setTagCompound(
                ...
                if (tStack.tryPlaceItemIntoWorld(aPlayer, aWorld, aX, aY, aZ, aSide, aHitX, aHitY, aHitZ)) {
                    ...
                    for (int j = 0; j < InventoryPlayer.getHotbarSize(); j++)
                        if (IL.Tool_Remote_Activator.equal(aPlayer.inventory.mainInventory[j], F, T)) {
                            if (Behavior_Remote.addCoords(
                                aPlayer.inventory.mainInventory[j],
                                aPlayer,
                                aWorld,
                                aX + OFFX[aSide],
                                aY + OFFY[aSide],
