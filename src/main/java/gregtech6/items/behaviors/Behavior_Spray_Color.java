...
public class Behavior_Spray_Color extends AbstractBehaviorDefault {
    ...
    private boolean colorize(World aWorld, int aX, int aY, int aZ, byte aSide) {
        ...
        if (aBlock != NB && (mAllowedVanillaBlocks.contains(aBlock) || aBlock instanceof BlockColored
            || IL.TE_Rockwool.block() == aBlock
            || aBlock == BlocksGT.Grass)) {
            if (aBlock == Blocks.hardened_clay)
                return aWorld.setBlock(aX, aY, aZ, Blocks.stained_hardened_clay, ~mColor & 15, 3);
            if (aBlock == Blocks.glass_pane)
                return aWorld.setBlock(aX, aY, aZ, Blocks.stained_glass_pane, ~mColor & 15, 3);
            ...
            if (aBlock == Blocks.grass || aBlock == BlocksGT.Grass) {
                switch (mColor) {
                    case DYE_INDEX_Green:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 0, 3);
                    case DYE_INDEX_Lime:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 1, 3);
                    case DYE_INDEX_Black:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 2, 3);
                    case DYE_INDEX_LightGray:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 3, 3);
                    case DYE_INDEX_Yellow:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 4, 3);
                    case DYE_INDEX_Brown:
                        return aWorld.setBlock(aX, aY, aZ, BlocksGT.Grass, 5, 3);
                    default:
                        return F;
                }
            }
            return aWorld.getBlockMetadata(aX, aY, aZ) != (~mColor & 15)
