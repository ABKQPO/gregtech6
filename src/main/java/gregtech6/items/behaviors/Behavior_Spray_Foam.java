...
public class Behavior_Spray_Foam extends AbstractBehaviorDefault {
    ...
    public Behavior_Spray_Foam(ItemStack aEmpty, ItemStack aUsed, ItemStack aFull, long aUses, byte aColor,
        boolean aOwned) {
        mEmpty = aEmpty;
        mUsed = aUsed;
        mFull = aFull;
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (aWorld.isRemote || aStack.stackSize != 1
            || aPlayer.isSneaking()
            || !aPlayer.canPlayerEdit(aX, aY, aZ, aSide, aStack)) return F;
        ...
        if (ST.equal(aStack, mUsed, T)) {
            long tFoamed = foam(
                ...
                aZ,
                aSide,
                UT.Entities.hasInfiniteItems(aPlayer) ? mUses : tUses,
                aPlayer,
                aStack);
            if (tFoamed > 0) {
    ...
    public long foam(World aWorld, int aX, int aY, int aZ, byte aSide, long aUses, EntityPlayer aPlayer,
        ItemStack aStack) {
        if (aUses < 1) return 0;
        long rUses = 0;

        DelegatorTileEntity<TileEntity> aDelegator = WD.te(aWorld, aX, aY, aZ, aSide, T);
        if (aDelegator.mTileEntity instanceof ITileEntityFoamable
            && !((ITileEntityFoamable) aDelegator.mTileEntity).hasFoam(aDelegator.mSideOfTileEntity))
            return ((ITileEntityFoamable) aDelegator.mTileEntity)
                .applyFoam(aDelegator.mSideOfTileEntity, aPlayer, DYES[mColor], mColor, mOwned) ? 10 : 0;
        ...
        if (aBlock instanceof IBlockFoamable
            && !((IBlockFoamable) aBlock).hasFoam(aWorld, aX, aY, aZ, aDelegator.mSideOfTileEntity))
            return ((IBlockFoamable) aBlock)
                .applyFoam(aWorld, aX, aY, aZ, aDelegator.mSideOfTileEntity, DYES[mColor], mColor) ? 10 : 0;

        try {
            if (UT.Reflection.getClassName(aDelegator.mTileEntity)
                .startsWith("TileEntityCable")
                && UT.Reflection.getPublicField(aDelegator.mTileEntity, "foamed")
                    .getByte(aDelegator.mTileEntity) == 0) {
                if (aUses >= 10) {
        ...
        if (IL.IC2_Scaffold.equal(aBlock)) return aUses >= 10 && MultiTileEntityCFoam
            .setBlock(aWorld, aX, aY, aZ, aDelegator.mSideOfTileEntity, aPlayer, aStack, DYES[mColor], mOwned) ? 10 : 0;
        ...
        if (BlocksGT.CFoamFresh != null) {
            ...
            switch ((int) getMode(aStack)) {
                case 0:
                    if (aUses >= 10 && WD.air(aWorld, aX, aY, aZ)
                        && (mOwned
                            ? MultiTileEntityCFoam
                                .setBlock(aWorld, aX, aY, aZ, aSide, aPlayer, aStack, DYES[mColor], mOwned)
                            : aWorld.setBlock(aX, aY, aZ, BlocksGT.CFoamFresh, mColor, 3))) {
                        aUses -= 10;
                        rUses += 10;
                    }
                    return rUses;
                ...
                case 2:
                    ...
                    for (byte i = 0; i < 3; i++) for (byte j = 0; j < 3; j++) {
                        if (aUses >= 10) {
                            if (WD.air(
                                aWorld,
                                aX + (SIDES_AXIS_X[tSide] ? 0 : i),
                                aY + (SIDES_AXIS_X[tSide] ? i : 0) + (SIDES_AXIS_Z[tSide] ? j : 0),
                                aZ + (SIDES_AXIS_Z[tSide] ? 0 : j))) {
                                if (mOwned
                                    ? MultiTileEntityCFoam.setBlock(
                                        aWorld,
                                        aX + (SIDES_AXIS_X[tSide] ? 0 : i),
                                        aY + (SIDES_AXIS_X[tSide] ? i : 0) + (SIDES_AXIS_Z[tSide] ? j : 0),
                                        aZ + (SIDES_AXIS_Z[tSide] ? 0 : j),
                    ...
                    return rUses;
                case 3:
                    if (aUses >= 5 && WD.air(aWorld, aX, aY, aZ)
                        && aWorld.setBlock(
                            aX,
                            aY,
                            aZ,
                            ((BlockCFoamFresh) BlocksGT.CFoamFresh).mSlabs[OPOS[aSide]],
                            mColor,
                            3)) {
                        aUses -= 5;
                        rUses += 5;
                        ...
                        if (aUses >= 5) {
                            if (WD.air(
                                ...
                                if (aWorld.setBlock(
                                    ...
                                    aY + (SIDES_AXIS_X[tSide] ? i : 0) + (SIDES_AXIS_Z[tSide] ? j : 0),
                                    aZ + (SIDES_AXIS_Z[tSide] ? 0 : j),
                                    ((BlockCFoamFresh) BlocksGT.CFoamFresh).mSlabs[OPOS[tSide]],
                                    mColor,
                                    3)) {
                                    aUses -= 5;
                                    rUses += 5;
    ...
    public void switchMode(ItemStack aStack, EntityPlayer aPlayer) {
        ...
        switch ((int) getMode(aStack)) {
            case 0:
                UT.Entities.sendchat(aPlayer, "Single Block Mode");
                break;
            case 1:
                UT.Entities.sendchat(aPlayer, "4m Line Mode");
                break;
            case 2:
                UT.Entities.sendchat(aPlayer, "3mx3m Area Mode");
                break;
            case 3:
                UT.Entities.sendchat(aPlayer, "Single Slab Mode");
                break;
            case 4:
                UT.Entities.sendchat(aPlayer, "3mx3m Slab Mode");
                break;
        }
    }
