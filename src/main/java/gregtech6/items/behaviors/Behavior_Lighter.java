...
public class Behavior_Lighter extends AbstractBehaviorDefault {
    ...
    public Behavior_Lighter(ItemStack aEmptyLighter, ItemStack aUsedLighter, ItemStack aFullLighter, long aFuelAmount,
        long aChance) {
    ...
    public boolean onLeftClickEntity(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, Entity aEntity) {
        if (aPlayer.worldObj.isRemote || (aStack.stackSize != 1 && (mFuelAmount != 1 || mEmptyLighter != null)))
            return F;
    ...
    public List<String> getAdditionalToolTips(MultiItem aItem, List<String> aList, ItemStack aStack) {
        ...
        if (mFuelAmount > 1) {
            long tFuelAmount = (ST.invalid(mFullLighter) ? 1
                : tNBT == null ? ST.equal(aStack, mFullLighter, T) ? mFuelAmount : 0 : UT.NBT.getLighterFuel(aStack));
            aList.add(LH.get("gt.behaviour.lighter.uses") + " " + tFuelAmount);
        } else {
            aList.add(LH.get("gt.behaviour.singleuse"));
        }
        if (mChance < 10000)
            aList.add(LH.get("gt.behaviour.lighter.chance") + " " + (mChance / 100) + "." + (mChance % 100) + "%");
        if (aStack.stackSize != 1 && (mFuelAmount != 1 || ST.valid(mEmptyLighter)))
            aList.add(LH.Chat.RED + LH.get(LH.REQUIREMENT_UNSTACKED));
        return aList;
    }
}
