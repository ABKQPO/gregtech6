...
public class Behavior_Cropnalyzer extends AbstractBehaviorDefault {
    ...
    public boolean onItemUseFirst(MultiItem aItem, ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY,
        ...
        if (aPlayer instanceof EntityPlayerMP) {
            ...
            if (aItem.useEnergy(TD.Energy.EU, aStack, tUsedEnergy, aPlayer, aPlayer.inventory, aWorld, aX, aY, aZ, T))
    ...
    public long getCropScan(ArrayList<String> aList, World aWorld, int aX, int aY, int aZ) {
        ...
        if (tTileEntity instanceof ic2.api.crops.ICropTile) {
            ...
            rList.add(
                "Type -- Name: " + LH.get(
                    ((ic2.api.crops.ICropTile) tTileEntity).getCrop()
                        .displayName())
                    + "   Growth: "
            ...
            rList.add(
                "Plant -- Fertilizer: " + ((ic2.api.crops.ICropTile) tTileEntity).getNutrientStorage()
                    + "   Water: "
            ...
            rList.add(
                "Environment -- Nutrients: " + ((ic2.api.crops.ICropTile) tTileEntity).getNutrients()
                    + "   Humidity: "
                    + ((ic2.api.crops.ICropTile) tTileEntity).getHumidity()
                    + "   Air-Quality: "
                    + ((ic2.api.crops.ICropTile) tTileEntity).getAirQuality());
            String tString = "";
            for (String tAttribute : ((ic2.api.crops.ICropTile) tTileEntity).getCrop()
                .attributes()) tString += ", " + tAttribute;
            rList.add("Attributes:" + tString.replaceFirst(",", ""));
            rList.add(
                "Discovered by: " + ((ic2.api.crops.ICropTile) tTileEntity).getCrop()
                    .discoveredBy());
