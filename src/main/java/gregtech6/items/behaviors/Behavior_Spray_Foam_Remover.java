...
public class Behavior_Spray_Foam_Remover extends AbstractBehaviorDefault {
    ...
    public long remove(World aWorld, int aX, int aY, int aZ, byte aSide, long aUses, EntityPlayer aPlayer,
        ItemStack aStack) {
        if (aUses < 1) return 0;
        ...
        if (aTileEntity.mTileEntity instanceof ITileEntityFoamable)
            return ((ITileEntityFoamable) aTileEntity.mTileEntity).removeFoam(aTileEntity.mSideOfTileEntity, aPlayer)
                ? 10
                : 0;
        ...
        if (aBlock instanceof IBlockFoamable)
            return ((IBlockFoamable) aBlock).removeFoam(aWorld, aX, aY, aZ, aTileEntity.mSideOfTileEntity)
                ? aBlock instanceof BlockCFoamFresh && SIDES_VALID[((BlockMetaType) aBlock).mSide] ? 5 : 10
                : 0;

        try {
            if (UT.Reflection.getClassName(aTileEntity.mTileEntity)
                .startsWith("TileEntityCable")) {
                if (aUses >= 10 && UT.Reflection.getPublicField(aTileEntity.mTileEntity, "foamed")
                    .getByte(aTileEntity.mTileEntity) >= 1) {
