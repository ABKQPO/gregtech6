...
public class MultiTileEntityUSBSwitch extends TileEntityBase08DataSwitch {
    ...
    static {
        LH.add(
            "gt.multitileentity.usb.switch.tooltip",
            "Switches between 16 different USB Sticks using Selector Covers");
    }
    ...
    public NBTTagCompound getUSBData(byte aSide, int aUSBTier) {
        ...
        if (OM.is(OD_USB_STICKS[aUSBTier], tUSB) && tUSB.hasTagCompound()
            && tUSB.getTagCompound()
                .getByte(NBT_USB_TIER) <= aUSBTier) {
            return tUSB.getTagCompound()
    ...
    public boolean setUSBData(byte aSide, int aUSBTier, NBTTagCompound aData) {
        ...
        if (OM.is(OD_USB_STICKS[aUSBTier], tUSB)) {
            ...
            if (aData == null || aData.hasNoTags()) {
                tUSB.getTagCompound()
                    .removeTag(NBT_USB_DATA);
                tUSB.getTagCompound()
                    .removeTag(NBT_USB_TIER);
            } else {
                tUSB.getTagCompound()
                    .setTag(NBT_USB_DATA, aData);
                tUSB.getTagCompound()
                    .setByte(NBT_USB_TIER, (byte) aUSBTier);
            }
            if (tUSB.getTagCompound()
                .hasNoTags()) tUSB.setTagCompound(null);
            return T;
        }
        return F;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? BlockTextureMulti.get(
            BlockTextureDefault.get(sColoreds[(int) UT.Code.bind_(0, 2, aSide)], mRGBa),
            BlockTextureDefault.get(sOverlays[(int) UT.Code.bind_(0, 2, aSide)])) : null;
    }
    ...
    public static IIconContainer sColoreds[] = new IIconContainer[] {
        ...
        sOverlays[] = new IIconContainer[] { new Textures.BlockIcons.CustomIcon("machines/usb/switch/overlay/bottom"),
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return OM.is(OD_USB_STICKS[0], aStack);
    }

    @Override
    public Object getGUIClient2(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerClientDefault(aPlayer.inventory, this, aGUIID, RES_PATH_GUI + "machines/USBSwitch.png");
    }
