...
public class MultiTileEntityHDDSwitch extends TileEntityBase08DataSwitch {
    ...
    public NBTTagCompound getUSBData(byte aSide, int aUSBTier) {
        ...
        if (OM.is(OD_USB_DRIVES[aUSBTier], tDrive) && tDrive.hasTagCompound()) {
            NBTTagCompound tDriveData = tDrive.getTagCompound()
                .getCompoundTag(NBT_USB_DRIVE);
            if (tDriveData.getByte(NBT_USB_TIER + mMode) <= aUSBTier)
                return tDriveData.hasKey(NBT_USB_DATA + mMode) ? tDriveData.getCompoundTag(NBT_USB_DATA + mMode) : null;
        }
        return null;
    }
    ...
    public boolean setUSBData(byte aSide, int aUSBTier, NBTTagCompound aData) {
        ...
        if (OM.is(OD_USB_DRIVES[aUSBTier], tDrive)) {
            ...
            if (tDriveData.hasNoTags()) {
                tDrive.getTagCompound()
                    .removeTag(NBT_USB_DRIVE);
            } else {
                tDrive.getTagCompound()
                    .setTag(NBT_USB_DRIVE, tDriveData);
            }
            if (tDrive.getTagCompound()
                .hasNoTags()) tDrive.setTagCompound(null);
            return T;
        }
        return F;
    }
    ...
    public static IIconContainer sColoreds[] = new IIconContainer[] {
        ...
        sOverlays[] = new IIconContainer[] { new Textures.BlockIcons.CustomIcon("machines/hdd/switch/overlay/bottom"),
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return OM.is(OD_USB_DRIVES[0], aStack);
    }

    @Override
    public Object getGUIClient2(int aGUIID, EntityPlayer aPlayer) {
        return new ContainerClientDefault(aPlayer.inventory, this, aGUIID, RES_PATH_GUI + "machines/HDDSwitch.png");
    }
