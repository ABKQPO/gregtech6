 ...
 */
public class MultiTileEntityBathingPot extends TileEntityBase07Paintable
    ...
    protected boolean canOutput(Recipe aRecipe) {
        for (int i = 0; i < 6; i++) if (slot(i + 6) != null) {
            if (aRecipe.mNeedsEmptyOutput || (aRecipe.mOutputs.length > 0 && aRecipe.mOutputs[i] != null
                && (!ST.equal(slot(i + 6), aRecipe.mOutputs[i], F)
                    || slot(i + 6).stackSize + aRecipe.mOutputs[i].stackSize > slot(i + 6).getMaxStackSize()))) {
                return F;
            }
        }
        for (int i = 0; i < mTanksOutput.length && i < aRecipe.mFluidOutputs.length; i++) if (mTanksOutput[i].has()) {
            if (aRecipe.mNeedsEmptyOutput
                || (aRecipe.mFluidOutputs[i] != null && (!mTanksOutput[i].contains(aRecipe.mFluidOutputs[i])
                    ...
                    || FL.gas(aRecipe.mFluidOutputs[i])
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide()) {
            ...
            if (slot(6) == null && slot(7) == null
                && slot(8) == null
                && slot(9) == null
                && slot(10) == null
                && slot(11) == null) for (int i = 0; i < 6; i++) if (ST.add(aPlayer, slot(i), T)) {
                    playCollect();
                    slotKill(i);
                    return T;
                }
    ...
    public static IIconContainer sTextureSides = new Textures.BlockIcons.CustomIcon(
        "machines/tools/bathing_pot/colored/sides"),
