 ...
 */
public class MultiTileEntityDustFunnel extends TileEntityBase07Paintable
    implements ITileEntityAdjacentInventoryUpdatable, IMTE_AddToolTips, IMTE_SyncDataShort {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            ...
            if (slotHas(0) && (mContent == null || mContent.mAmount < DUST_TYPES[mMode].mAmount)) {
                ...
                if (OM.prefixcontainsmaterialmatches(
                    tData,
                    mContent == null || mContent.mMaterial == MT.NULL ? null : mContent.mMaterial,
                    TD.Prefix.DUST_BASED)) {
                    int tSize = (int) Math.min(
                        slot(0).stackSize,
                        UT.Code.divup(
                            DUST_TYPES[mMode].mAmount - (mContent == null ? 0 : mContent.mAmount),
                            tData.mMaterial.mAmount));
                    mContent = OM.stack(
                        tData.mMaterial.mMaterial,
                        tData.mMaterial.mAmount * tSize + (mContent == null ? 0 : mContent.mAmount));
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide() && SIDES_TOP[aSide] && canInsertItem2(0, aPlayer.inventory.getCurrentItem(), aSide))
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        if (aSendAll) return getClientDataPacketByteArray(
            aSendAll,
            UT.Code.toByteS(mDust, 0),
            UT.Code.toByteS(mDust, 1),
            mMode,
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                box(aBlock, PX_P[0], PX_P[8], PX_P[0], PX_N[0], PX_N[0], PX_N[0]);
                return T;
            case 1:
                box(aBlock, PX_P[2 + mMode], PX_P[0], PX_P[2 + mMode], PX_N[2 + mMode], PX_N[8], PX_N[2 + mMode]);
                return T;
            case 2:
                box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_P[2], PX_P[8], PX_P[2]);
                return T;
            case 3:
                box(aBlock, PX_P[0], PX_P[0], PX_N[2], PX_P[2], PX_P[8], PX_N[0]);
                return T;
            case 4:
                box(aBlock, PX_N[2], PX_P[0], PX_P[0], PX_N[0], PX_P[8], PX_P[2]);
                return T;
            case 5:
                box(aBlock, PX_N[2], PX_P[0], PX_N[2], PX_N[0], PX_P[8], PX_N[0]);
                return T;
        }
        return F;
    }

    public static IIconContainer sTextureTop = new Textures.BlockIcons.CustomIcon(
        "machines/tools/dust_funnel/colored/top"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0 && aSide == 0) {
            ...
            mTextureSides = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureSides, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlaySides));
            mTextureBottom = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureBottom, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayBottom));

            if (mDust == 0) {
                mTextureInput = BlockTextureMulti.get(
                    BlockTextureDefault.get(sTextureTop, mRGBa, F, tGlow, F, F),
                    BlockTextureDefault.get(sOverlayTop),
                    BlockTextureDefault.get(sTextureHole, mRGBa, F, tGlow, F, F),
                    BlockTextureDefault.get(sOverlayHole));
            } else {
                if (UT.Code.exists(mDust, OreDictMaterial.MATERIAL_ARRAY)) {
                    mTextureInput = BlockTextureMulti.get(
                        OreDictMaterial.MATERIAL_ARRAY[mDust].getTextureDust(),
                        BlockTextureDefault.get(sTextureHole, mRGBa, F, tGlow, F, F),
                        BlockTextureDefault.get(sOverlayHole));
                } else {
                    mTextureInput = BlockTextureMulti.get(
                        BlockTextureDefault.get(MT.NULL, OP.blockDust, CA_GRAY_64, F),
                        BlockTextureDefault.get(sTextureHole, mRGBa, F, tGlow, F, F),
                        BlockTextureDefault.get(sOverlayHole));
                }
            }
        ...
        return SIDES_TOP[aSide] ? aRenderPass == 0 ? mTextureInput : null
            : SIDES_BOTTOM[aSide] ? mTextureBottom : mTextureSides;
    }

    public boolean isTopVisible() {
        return !hasCovers() || mCovers.mBehaviours[SIDE_TOP] == null
            || !mCovers.mBehaviours[SIDE_TOP].isOpaque(SIDE_TOP, mCovers);
    }
    ...
    public float getSurfaceSize(byte aSide) {
        return SIDES_TOP[aSide] ? PX_P[16] : SIDES_BOTTOM[aSide] ? PX_P[12 - 2 * mMode] : 0.0F;
    }

    @Override
    public float getSurfaceSizeAttachable(byte aSide) {
        return SIDES_TOP[aSide] ? PX_P[12] : PX_P[12 - 2 * mMode];
    }

    @Override
    public float getSurfaceDistance(byte aSide) {
        return SIDES_HORIZONTAL[aSide] ? PX_P[2 + mMode] : 0.0F;
    }
    ...
    public void adjacentInventoryUpdated(byte aSide, IInventory aTileEntity) {
        if (SIDES_VERTICAL[aSide]) updateInventory();
    }
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        if (aSlot == 0 && !slotHas(0) && SIDES_TOP_HORIZONTAL[aSide]) {
            ...
            return OM.prefixcontainsmaterialmatches(
                tData,
                mContent == null ? null : mContent.mMaterial,
                TD.Prefix.DUST_BASED) && DUST_TYPES[mMode].mat(tData.mMaterial.mMaterial, 1) != null;
        }
        return F;
    }
