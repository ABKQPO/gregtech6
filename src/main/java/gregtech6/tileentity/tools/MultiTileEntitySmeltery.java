 ...
 */
public class MultiTileEntitySmeltery extends TileEntityBase07Paintable
    implements ITileEntityCrucible, ITileEntityEnergy, ITileEntityGibbl, ITileEntityWeight, ITileEntityTemperature,
    ITileEntityMold, ITileEntityServerTickPost, IMTE_RemovedByPlayer, IMTE_OnEntityCollidedWithBlock,
            ...
            if (tBiome.rainfall > 0 && tBiome.temperature >= 0.2) {
                addMaterialStacks(
                    Arrays.asList(
                        OM.stack(
                            MT.Water,
                            U1000 * (long) Math.max(1, tBiome.rainfall * 100) * (worldObj.isThundering() ? 2 : 1))),
                    tTemperature);
            }
        }

        if (!slotHas(0))
            slot(0, WD.suck(worldObj, xCoord + PX_P[2], yCoord + PX_P[2], zCoord + PX_P[2], PX_N[4], 1, PX_N[4]));
                    ...
                    && !tMaterial.mMaterial.containsAny(TD.Properties.UNBURNABLE, TD.Processing.MELTING))) {
                        ...
                        if (tMaterial.mMaterial.mBoilingPoint >= 320) try {
                            for (EntityLivingBase tLiving : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                                ...
                                UT.Entities.applyTemperatureDamage(tLiving, tMaterial.mMaterial.mBoilingPoint, 2);
                        } catch (Throwable e) {
        ...
        if (mTemperature > getTemperatureMax(SIDE_INSIDE)) {
            ...
            if (mTemperature >= 320) try {
                for (EntityLivingBase tLiving : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    box(-GAS_RANGE, -1, -GAS_RANGE, GAS_RANGE + 1, GAS_RANGE + 1, GAS_RANGE + 1)))
                    UT.Entities.applyTemperatureDamage(tLiving, mTemperature, 2);
            } catch (Throwable e) {
                e.printStackTrace(ERR);
            }
        ...
        if (mTemperature >= 1300 && isServerSide() && !UT.Entities.isCreative(aPlayer)) {
            ...
            try {
                for (EntityLivingBase tLiving : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    box(-GAS_RANGE, -1, -GAS_RANGE, GAS_RANGE + 1, GAS_RANGE + 1, GAS_RANGE + 1)))
                    UT.Entities.applyTemperatureDamage(tLiving, mTemperature);
            } catch (Throwable e) {
                e.printStackTrace(ERR);
            }
            for (int j = 0, k = UT.Code.bindInt(mTemperature / 25); j < k; j++) WD.fire(
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_thermometer)) {
            if (aChatReturn != null) aChatReturn.add(
                "Temperature: " + mTemperature + (mTemperature >= 1300 ? "K (too hot to pick it up right now!)" : "K"));
            return 10000;
        }
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        if (aSendAll) return getClientDataPacketByteArray(
            ...
            (byte) (mMeltDown ? 1 : 0));
        if (mDisplayedFluid != oDisplayedFluid) return getClientDataPacketByteArray(
            F,
            mDisplayedHeight,
            UT.Code.toByteS(mDisplayedFluid, 0),
            UT.Code.toByteS(mDisplayedFluid, 1));
    ...
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        ...
        if (aData.length >= 6) mRGBa = UT.Code
            .getRGBInt(new short[] { UT.Code.unsignB(aData[3]), UT.Code.unsignB(aData[4]), UT.Code.unsignB(aData[5]) });
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[14], PX_N[0], PX_N[0]);
                return T;
            case 1:
                box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[0], PX_N[14]);
                return T;
            case 2:
                box(aBlock, PX_P[14], PX_P[0], PX_P[0], PX_N[0], PX_N[0], PX_N[0]);
                return T;
            case 3:
                box(aBlock, PX_P[0], PX_P[0], PX_P[14], PX_N[0], PX_N[0], PX_N[0]);
                return T;
            case 4:
                box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[14], PX_N[0]);
                return T;
            case 5:
                box(
                    aBlock,
                    ...
                    PX_P[0],
                    PX_N[0],
                    0.125F + (UT.Code.unsignB(mDisplayedHeight) / 292.571428F),
                    PX_N[0]);
                return T;
        }
        return F;
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
            case 2:
                return SIDES_AXIS_Z[aSide] || aSide == SIDE_BOTTOM ? null : mTexture;
            case 1:
            case 3:
                return SIDES_AXIS_X[aSide] || aSide == SIDE_BOTTOM ? null : mTexture;
            case 4:
                return SIDES_VERTICAL[aSide] ? mTexture : null;
            case 5:
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(0.125, 0.125, 0.125, 0.875, 0.875, 0.875);
    }
