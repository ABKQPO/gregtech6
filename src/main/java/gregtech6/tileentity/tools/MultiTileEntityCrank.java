...
public class MultiTileEntityCrank extends TileEntityBase11AttachmentSmall implements ITileEntityEnergy {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            ...
            if (oActive) for (EntityPlayer tPlayer : UT.Entities.getPlayersWithLastTarget(this)) {
                mActive = T;
                if (ITileEntityEnergy.Util.emitEnergyToSide(
                    TD.Energy.RU,
                    mFacing,
                    -UT.Code.divup(8L * UT.Entities.pot2Strength(tPlayer), UT.Entities.pot1Weakness(tPlayer)),
                    UT.Entities.pot1Haste(tPlayer),
                    this) > 0) UT.Entities.exhaust(tPlayer, 0.025);
                tPlayer.swingItem();
            }
            ...
            if (!mActive) {
                ...
                worldObj.getChunkFromBlockCoords(xCoord, zCoord)
                    .getEntitiesOfTypeWithinAAAB(EntityVillager.class, box(), tList, null);
                for (EntityVillager tVillager : tList)
                    if (UT.Code.roundDown(tVillager.posY + tVillager.getEyeHeight()) == yCoord
                        && UT.Code.roundDown(tVillager.posX) == xCoord
                        && UT.Code.roundDown(tVillager.posZ) == zCoord) {
                            mActive = T;
                            ITileEntityEnergy.Util.emitEnergyToSide(
                                TD.Energy.RU,
                                mFacing,
                                -UT.Code.divup(
                                    8L * UT.Entities.pot2Strength(tVillager),
                                    UT.Entities.pot1Weakness(tVillager)),
                                UT.Entities.pot1Haste(tVillager),
                                this);
                            // Multiple Players can use one Crank but multiple Villagers cannot (Collision Lag
                            // prevention)
                            break;
                        }
            }
    ...
    public int getRenderPasses2(Block aBlock, boolean[] aShouldSideBeRendered) {
        mTextureFront = BlockTextureMulti.get(
            BlockTextureDefault.get(mActive ? sTextureFrontSpin : sTextureFront, mRGBa),
            BlockTextureDefault.get(mActive ? sOverlayFrontSpin : sOverlayFront));
        mTextureBacks = BlockTextureDefault
            .get(mActive ? Textures.BlockIcons.AXLE_COUNTERCLOCKWISE : Textures.BlockIcons.AXLE, mRGBa);
        mTextureSides = BlockTextureDefault
            .get(mActive ? Textures.BlockIcons.AXLE_CLOCKWISE : Textures.BlockIcons.AXLE, mRGBa);
        return 2;
    }
    ...
    public static IIconContainer sTextureFront = new Textures.BlockIcons.CustomIcon(
        "machines/tools/crank/colored/front"),
    ...
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return aSide == mFacing && aEnergyType == TD.Energy.RU;
    }
    ...
    public byte getDirectionData() {
        return (byte) ((mFacing & 7) | (mActive ? 8 : 0));
    }
