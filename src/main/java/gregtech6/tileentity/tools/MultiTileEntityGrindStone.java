 ...
 */
public class MultiTileEntityGrindStone extends TileEntityBase09FacingSingle implements IMTE_SetBlockBoundsBasedOnState,
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (isServerSide()) {
            if (SIDES_TOP[aSide]) {
                ...
                if (tCoords[0] <= PX_P[SIDES_AXIS_Z[mFacing] ? 8 : 4]
                    && tCoords[1] <= PX_P[SIDES_AXIS_X[mFacing] ? 8 : 4]) return T;
            }
                ...
                if (ST.invalid(aStack) || mRecipes == null || !UT.Entities.isPlayer(aPlayer)) {
                    ...
                    if (tXP > 0) {
                        ...
                        worldObj.spawnEntityInWorld(
                            new EntityXPOrb(worldObj, xCoord + 0.5, yCoord + 1.25, zCoord + 0.5, tXP));
                    } else {
            ...
            if (SIDES_TOP[aSide] && mStone != 0) {
                float[] tCoords = UT.Code.getFacingCoordsClicked(aSide, aHitX, aHitY, aHitZ);
                if (tCoords[0] <= PX_P[SIDES_AXIS_Z[mFacing] ? 8 : 4]
                    && tCoords[1] <= PX_P[SIDES_AXIS_X[mFacing] ? 8 : 4]) {
                    mRecipes.openNEI();
                    return T;
                }
            }
        ...
        switch (aRenderPass) {
            case 0:
                box(
                    ...
                    PX_P[SIDES_AXIS_Z[mFacing] ? 8 : 4],
                    PX_N[1] + 0.0001F,
                    PX_P[SIDES_AXIS_X[mFacing] ? 8 : 4]);
                return T;
            case 1:
            ...
            case 2:
                box(
                    ...
                    PX_P[0],
                    PX_P[5],
                    SIDES_AXIS_Z[mFacing] ? PX_P[6] : PX_N[5],
                    PX_N[5],
                    SIDES_AXIS_X[mFacing] ? PX_P[6] : PX_N[5]);
                return T;
            case 3:
                box(
                    aBlock,
                    SIDES_AXIS_Z[mFacing] ? PX_N[6] : PX_P[5],
                    PX_P[0],
                    SIDES_AXIS_X[mFacing] ? PX_N[6] : PX_P[5],
                    PX_N[5],
                    PX_N[5],
                    PX_N[5]);
                return T;
            case 4:
                box(
                    aBlock,
                    PX_P[SIDES_AXIS_X[mFacing] ? 7 : 3],
                    PX_P[8],
                    PX_P[SIDES_AXIS_Z[mFacing] ? 7 : 3],
                    PX_N[SIDES_AXIS_X[mFacing] ? 7 : 3],
                    PX_N[6],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 7 : 3]);
                return T;
            case 5:
                box(
                    aBlock,
    ...
    public static IIconContainer sTextureLegs = new Textures.BlockIcons.CustomIcon(
        "machines/tools/grindstone/colored/legs"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0 && aSide == 0) {
            ...
            mTextureLegs = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureLegs, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayLegs));
            mTextureAxle = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureAxle, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayAxle));
            mTextureStone = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureStone, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayStone));
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[1], PX_N[2]);
    }
    ...
    public void setBlockBoundsBasedOnState(Block aBlock) {
        box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[1], PX_N[2]);
    }

    @Override
    public float getSurfaceSize(byte aSide) {
        return SIDES_BOTTOM[aSide] ? 1.0F : 0.0F;
    }
