 ...
 */
public class MultiTileEntityDynamite extends TileEntityBase09FacingSingle implements ITileEntityRemoteActivateable,
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (mFortune > 0) aList.add(LH.Chat.PINK + LH.get(LH.TOOLTIP_BLASTFORTUNE) + LH.Chat.WHITE + mFortune);
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_igniter) && mCountDown == 0 && WD.oxygen(worldObj, xCoord, yCoord, zCoord)) {
            mCountDown = 100;
            updateClientData();
            causeBlockUpdate();
            UT.Sounds.send(SFX.MC_TNT_IGNITE, 1.0F, 0.5F, this, F);
            return 10000;
        }
        if (aTool.equals(TOOL_extinguisher) && mCountDown != 0) {
            mCountDown = 0;
            updateClientData();
            causeBlockUpdate();
            UT.Sounds.send(SFX.MC_FIZZ, 1.0F, 0.5F, this, F);
            return 10000;
        }
        return 0;
    }
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            if (mBlockUpdated || aTimer == 2) {
                if ((mCountDown == 0 && hasRedstoneIncoming()) || WD.burning(worldObj, xCoord, yCoord, zCoord))
                    remoteActivate();

                while (mSunk) {
                    Block tBlock = getBlockAtSide(OPOS[mFacing]);
                    if (BlocksGT.drillableDynamite.contains(tBlock)) break;
                    if (tBlock.getBlockHardness(
                        worldObj,
                        xCoord + OFFX[OPOS[mFacing]],
                        yCoord + OFFY[OPOS[mFacing]],
                        zCoord + OFFZ[OPOS[mFacing]]) >= 0) {
    ...
    public byte getVisualData() {
        return (byte) ((mCountDown << 1) | (mSunk ? 1 : 0));
    }
    ...
    public void explode(boolean aInstant) {
        ...
        Explosion tExplosion = mSunk
            ? new DynamiteExplosion(
                worldObj,
                getOffsetXN(mFacing) + 0.5,
                getOffsetYN(mFacing) + 0.5,
                getOffsetZN(mFacing) + 0.5,
                mMaxExplosionResistance,
                mFortune)
            : new DynamiteExplosion(
                worldObj,
                xCoord + 0.5,
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        box(
            aBlock,
            PX_P[SIDE_X_NEG == mFacing ? mSunk ? 14 : 0 : SIDE_X_POS == mFacing ? 0 : 5],
            PX_P[SIDE_Y_NEG == mFacing ? mSunk ? 14 : 0 : SIDE_Y_POS == mFacing ? 0 : 5],
            PX_P[SIDE_Z_NEG == mFacing ? mSunk ? 14 : 0 : SIDE_Z_POS == mFacing ? 0 : 5],
            PX_N[SIDE_X_POS == mFacing ? mSunk ? 14 : 0 : SIDE_X_NEG == mFacing ? 0 : 5],
            PX_N[SIDE_Y_POS == mFacing ? mSunk ? 14 : 0 : SIDE_Y_NEG == mFacing ? 0 : 5],
            PX_N[SIDE_Z_POS == mFacing ? mSunk ? 14 : 0 : SIDE_Z_NEG == mFacing ? 0 : 5]);
        return T;
    }

    public static final IIconContainer sTextureFront = new Textures.BlockIcons.CustomIcon(
        "machines/tools/dynamite/colored/front"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aSide == mFacing) return BlockTextureMulti.get(
            BlockTextureDefault.get(mCountDown != 0 ? sTextureFrontActive : sTextureFront, mRGBa, F, F, F, F),
            BlockTextureDefault.get(mCountDown != 0 ? sOverlayFrontActive : sOverlayFront));
        if (aSide == OPOS[mFacing]) return BlockTextureMulti.get(
            BlockTextureDefault.get(mCountDown != 0 ? sTextureBackActive : sTextureBack, mRGBa, F, F, F, F),
            BlockTextureDefault.get(mCountDown != 0 ? sOverlayBackActive : sOverlayBack));
        return BlockTextureMulti.get(
            BlockTextureDefault.get(mCountDown != 0 ? sTextureSideActive : sTextureSide, mRGBa, F, F, F, F),
            BlockTextureDefault.get(mCountDown != 0 ? sOverlaySideActive : sOverlaySide));
    }
    ...
    public static class DynamiteExplosion extends Explosion {
        ...
        public DynamiteExplosion(World aWorld, double aX, double aY, double aZ, float aMaxExplosionResistance,
            byte aFortune) {
        ...
        public void doExplosionA() {
            for (int tX = UT.Code.roundDown(explosionX) - 1; tX <= UT.Code.roundDown(explosionX) + 1; tX++)
                for (int tY = UT.Code.roundDown(explosionY) - 1; tY <= UT.Code.roundDown(explosionY) + 1; tY++)
                    for (int tZ = UT.Code.roundDown(explosionZ) - 1; tZ <= UT.Code.roundDown(explosionZ) + 1; tZ++) {
                        Block tBlock = mWorld.getBlock(tX, tY, tZ);
                        if (tBlock == Blocks.mob_spawner || WD.bedrock(tBlock)) continue;
                        if (tBlock
                            .getExplosionResistance(exploder, mWorld, tX, tY, tZ, explosionX, explosionY, explosionZ)
                            <= mMaxExplosionResistance) affectedBlockPositions.add(new ChunkPosition(tX, tY, tZ));
                    }
            List tList = mWorld.getEntitiesWithinAABBExcludingEntity(
                exploder,
            ...
            for (Object tEntity : tList) if (!(tEntity instanceof EntityItem))
                ((Entity) tEntity).attackEntityFrom(tSource, 2 * mMaxExplosionResistance * TFC_DAMAGE_MULTIPLIER);
            explosionSize = 1F;
        }
        ...
        public void doExplosionB(boolean aEffects) {
            mWorld.playSoundEffect(
                explosionX,
                explosionY,
                explosionZ,
                "random.explode",
                4.0F,
                (1.0F + (RNGSUS.nextFloat() - RNGSUS.nextFloat()) * 0.2F) * 0.7F);
            ...
            if (isSmoking) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (aEffects) {
                        double d0 = (i + RNGSUS.nextFloat()), d1 = (j + RNGSUS.nextFloat()),
                            d2 = (k + RNGSUS.nextFloat()), d3 = d0 - explosionX, d4 = d1 - explosionY,
                            d5 = d2 - explosionZ, d6 = Math.sqrt(d3 * d3 + d4 * d4 + d5 * d5);
                        d3 /= d6;
                        d4 /= d6;
                        d5 /= d6;
                    ...
                    if (tBlock.getMaterial() != Material.air) {
                        ...
                        if (tBlock.canDropFromExplosion(this))
                            tBlock.dropBlockAsItemWithChance(mWorld, i, j, k, tMeta, 1, mFortune);
                    }
                }
            }
        }
