 ...
 */
public class MultiTileEntityMoldCoinage extends TileEntityBase07Paintable
    implements ITileEntityAnvil, IMTE_SetBlockBoundsBasedOnState, IMTE_GetCollisionBoundingBoxFromPool,
    IMTE_GetSelectedBoundingBoxFromPool, IMTE_AddToolTips, IMTE_SyncDataShort {
        ...
        if (aTool.equals(TOOL_hammer) && SIDES_TOP[aSide]) {
            if (aRemainingDurability >= 2000) {
                ...
                if (tData != null && tData.mPrefix == OP.plateTiny) {
                    slot(
                        0,
                        MultiTileEntityCoin.getCoin(slot(0).stackSize, tData.mMaterial.mMaterial, mIsUnique, mShape));
                    return 2000;
                }
                return 0;
            }
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        if (aSendAll) return getClientDataPacketByteArray(
            T,
            UT.Code.toByteS(mDisplayedMetal, 0),
            UT.Code.toByteS(mDisplayedMetal, 1),
    ...
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        ...
        if (aData.length >= 5) mRGBa = UT.Code
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        ...
        if (aRenderPass == 1) {
            box(aBlock, PX_P[4], PX_P[12], PX_P[4], PX_N[4], PX_N[2], PX_N[4]);
            return T;
        }
        if (aRenderPass == 2) {
            box(aBlock, PX_P[5], PX_P[11], PX_P[5], PX_N[5], PX_N[3], PX_N[5]);
            return T;
        }

        if (aRenderPass == 3) {
            box(aBlock, PX_P[11], PX_P[12], PX_P[5], PX_N[5], PX_N[2], PX_N[5]);
            return T;
        }
        if (aRenderPass == 4) {
            box(aBlock, PX_P[5], PX_P[12], PX_P[11], PX_N[5], PX_N[2], PX_N[5]);
            return T;
        }
        if (aRenderPass == 5) {
            box(aBlock, PX_P[5], PX_P[12], PX_P[5], PX_N[11], PX_N[2], PX_N[5]);
            return T;
        }
        if (aRenderPass == 6) {
            box(aBlock, PX_P[5], PX_P[12], PX_P[5], PX_N[5], PX_N[2], PX_N[11]);
            return T;
        }
        return F;
    }
    ...
    public static IIconContainer sTextureTop = new Textures.BlockIcons.CustomIcon(
        "machines/tools/mold_coinage/colored/top"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0 && aSide == 0) {
            ...
            mTextureHole = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureHole, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayHole));
            mTextureSides[0] = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureBottom, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlayBottom));
            mTextureSides[1] = BlockTextureMulti
                .get(BlockTextureDefault.get(sTextureTop, mRGBa, F, tGlow, F, F), BlockTextureDefault.get(sOverlayTop));
            mTextureSides[2] = BlockTextureMulti.get(
                BlockTextureDefault.get(sTextureSide, mRGBa, F, tGlow, F, F),
                BlockTextureDefault.get(sOverlaySide));
    ...
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        OreDictItemData tData = OM.anyassociation(slot(aSlot));
        return tData == null || tData.mPrefix != OP.plateTiny;
    }
