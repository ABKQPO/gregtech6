 ...
 */
public class MultiTileEntityMixingBowl extends TileEntityBase07Paintable
    implements IFluidHandler, ITileEntityConnectedTank, ITileEntityTapFillable, IMTE_SetBlockBoundsBasedOnState,
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < mTanksInput.length; i++)
            mTanksInput[i] = new FluidTankGT(tCapacity).readFromNBT(aNBT, NBT_TANK + ".in." + i);
        mTanksOutput = new FluidTankGT[mRecipes.mOutputFluidCount];
        for (int i = 0; i < mTanksOutput.length; i++)
            mTanksOutput[i] = new FluidTankGT(tCapacity).readFromNBT(aNBT, NBT_TANK + ".out." + i);
    }
        ...
        if (SIDES_TOP[aSide] && aTool.equals(TOOL_mixer)) {
            ...
            if (tRecipe != null) {
                ...
                if (canOutput(tRecipe) && tRecipe.isRecipeInputEqual(T, F, mTanksInput, tInputItems)) {
                    ...
                    for (int i = 0; i < mRecipes.mOutputItemsCount && i < tOutputItems.length; i++)
                        addStackToSlot(i + 6, tOutputItems[i]);
                    for (int i = 0; i < mTanksOutput.length && i < tOutputFluids.length; i++)
                        mTanksOutput[i].fill(tOutputFluids[i], T);
        ...
        if (aIsServerSide) {
            if (SERVER_TIME % 600 == 10 && worldObj.isRaining() && getRainOffset(0, 1, 0)) {
                ...
                if (tBiome.rainfall > 0 && tBiome.temperature >= 0.2) {
                    ...
                    if (!(tInFront instanceof BlockLiquid) && !(tInFront instanceof IFluidBlock)
                        && !tInFront.isSideSolid(worldObj, xCoord, yCoord + 1, zCoord, FORGE_DIR_OPPOSITES[SIDE_TOP])
                        && !tInFront.isSideSolid(worldObj, xCoord, yCoord + 1, zCoord, FORGE_DIR[SIDE_TOP])) {
                        FluidStack tWater = FL.Water
                            .make((long) Math.max(1, tBiome.rainfall * 200) * (worldObj.isThundering() ? 2 : 1));
                        if (tWater != null) {
            ...
            for (FluidTankGT tTank : mTanksOutput) if (tTank.has()) {
                mDisplay = (short) (-2 - tTank.getFluid()
                    .getFluidID());
                tBreak = T;
                break;
            }
    ...
    protected boolean canOutput(Recipe aRecipe) {
        if (slot(6) != null) {
            if (aRecipe.mNeedsEmptyOutput || (aRecipe.mOutputs.length > 0 && aRecipe.mOutputs[0] != null
                && (!ST.equal(slot(6), aRecipe.mOutputs[0], F)
                    || slot(6).stackSize + aRecipe.mOutputs[0].stackSize > slot(6).getMaxStackSize()))) {
                return F;
            }
        }
        for (int i = 0; i < mTanksOutput.length && i < aRecipe.mFluidOutputs.length; i++) if (mTanksOutput[i].has()) {
            if (aRecipe.mNeedsEmptyOutput
                || (aRecipe.mFluidOutputs[i] != null && (!mTanksOutput[i].contains(aRecipe.mFluidOutputs[i])
                    || FL.temperature(aRecipe.mFluidOutputs[i]) >= mMaterial.mMeltingPoint - 100
                    || FL.lighter(aRecipe.mFluidOutputs[i])
                    || !FL.simple(aRecipe.mFluidOutputs[i])
                    || mTanksOutput[i].has(Math.max(1000, 1 + aRecipe.mFluidOutputs[i].amount))))) {
                return F;
            }
        }
        return T;
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide()) {
            ...
            if (SIDES_TOP[aSide] && aHitX > PX_P[2] && aHitX < PX_N[2] && aHitZ > PX_P[2] && aHitZ < PX_N[2]) {
                ...
                if (aStack != null) for (FluidTankGT tTank : mTanksOutput)
                    if ((tStack = FL.fill(tTank, ST.amount(1, aStack), T, T, T, T)) != null) {
                        aStack.stackSize--;
                        ST.give(aPlayer, tStack, T);
                        return T;
                    }
                if (aStack != null) for (FluidTankGT tTank : mTanksInput)
                    if ((tStack = FL.fill(tTank, ST.amount(1, aStack), T, T, T, T)) != null) {
                        aStack.stackSize--;
                        ST.give(aPlayer, tStack, T);
                        return T;
    ...
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        ...
        if (aData.length > 4) mRGBa = UT.Code
    ...
    protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        ...
        if (FL.temperature(aFluidToFill) >= mMaterial.mMeltingPoint - 100 || !FL.heavier(aFluidToFill)
            || !FL.simple(aFluidToFill)) return null;
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[14], PX_N[8], PX_N[0]);
            case 1:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[8], PX_N[14]);
            case 2:
                return box(aBlock, PX_P[14], PX_P[0], PX_P[0], PX_N[0], PX_N[8], PX_N[0]);
            case 3:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[14], PX_N[0], PX_N[8], PX_N[0]);
            case 4:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[14], PX_N[0]);
            case 5:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[9], PX_N[0]);
            case 6:
                return box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[14], PX_N[8] + 0.001F, PX_N[14]);
        }
        return F;
    }

    public static IIconContainer sTextureSides = new Textures.BlockIcons.CustomIcon(
        "machines/tools/mixing_bowl/colored/sides"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                return SIDE_X_POS == aSide
                    ? BlockTextureMulti
                        .get(BlockTextureDefault.get(sTextureInsides, mRGBa), BlockTextureDefault.get(sOverlayInsides))
                    : SIDE_X_NEG == aSide
                        ? BlockTextureMulti
                            .get(BlockTextureDefault.get(sTextureSides, mRGBa), BlockTextureDefault.get(sOverlaySides))
                        : SIDE_TOP == aSide
                            ? BlockTextureMulti
                                .get(BlockTextureDefault.get(sTextureTop, mRGBa), BlockTextureDefault.get(sOverlayTop))
                            : null;
            case 2:
                return SIDE_X_NEG == aSide
                    ? BlockTextureMulti
                        .get(BlockTextureDefault.get(sTextureInsides, mRGBa), BlockTextureDefault.get(sOverlayInsides))
                    : SIDE_X_POS == aSide
                        ? BlockTextureMulti
                ...
                return SIDE_Z_NEG == aSide
                    ...
                    : SIDE_Z_POS == aSide
                        ...
                        : SIDE_TOP == aSide
                            ...
                            : null;
            case 4:
                return SIDE_TOP == aSide
                    ? BlockTextureMulti
                        .get(BlockTextureDefault.get(sTextureTop, mRGBa), BlockTextureDefault.get(sOverlayTop))
                    : SIDE_BOTTOM == aSide ? BlockTextureMulti
                        .get(BlockTextureDefault.get(sTextureBottom, mRGBa), BlockTextureDefault.get(sOverlayBottom))
                        : null;
            case 5:
                if (mDisplay == 0 || SIDE_TOP != aSide) return null;
                if (mDisplay < -1) {
                    Fluid tFluid = FluidRegistry.getFluid(-mDisplay - 2);
                    return tFluid == null ? BlockTextureCopied.get(Blocks.water, SIDE_ANY, 0, UNCOLOURED, F, F, F)
                        : BlockTextureFluid.get(FL.make(tFluid, 1000));
                }
                if (UT.Code.exists(mDisplay, OreDictMaterial.MATERIAL_ARRAY))
                    return OreDictMaterial.MATERIAL_ARRAY[mDisplay].getTextureDust();
                return BlockTextureDefault.get(MT.NULL, OP.blockDust, CA_GRAY_128, F);
            case 6:
                return SIDE_TOP == aSide ? BI.nei() : null;
            case 7:
                return SIDE_TOP != aSide ? SIDE_BOTTOM == aSide
                    ? BlockTextureMulti.get(
                        BlockTextureDefault.get(sTextureTableBottom, mRGBa),
                        BlockTextureDefault.get(sOverlayTableBottom))
                    : BlockTextureMulti.get(
                        BlockTextureDefault.get(sTextureTableSide, mRGBa),
                        BlockTextureDefault.get(sOverlayTableSide))
                    : null;
        }
        return null;
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[8], PX_N[0]);
    }
    ...
    public void setBlockBoundsBasedOnState(Block aBlock) {
        box(aBlock, PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[8], PX_N[0]);
    }
    ...
    public float getSurfaceDistance(byte aSide) {
        return SIDES_TOP[aSide] ? PX_N[8] : 0.0F;
    }
    ...
    @Override
    public int addFluidToConnectedTank(byte aSide, FluidStack aFluid,
        boolean aOnlyAddIfItAlreadyHasFluidsOfThatTypeOrIsDedicated) {
        return 0;
    }
    ...
    public int removeFluidFromConnectedTank(byte aSide, FluidStack aFluid, boolean aOnlyRemoveIfItCanRemoveAllAtOnce) {
        ...
        for (FluidTankGT tTank : mTanksInput)
            if (tTank.contains(aFluid)) if (tTank.has(aOnlyRemoveIfItCanRemoveAllAtOnce ? aFluid.amount : 1))
