 ...
 */
public class MultiTileEntityJuicer extends TileEntityBase07Paintable implements ITileEntityQuickObstructionCheck,
    IFluidHandler, ITileEntityConnectedTank, IMTE_SetBlockBoundsBasedOnState, IMTE_GetCollisionBoundingBoxFromPool,
    IMTE_GetSelectedBoundingBoxFromPool, IMTE_AddToolTips {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < mTanks.length; i++)
            mTanks[i] = new FluidTankGT(1000000).readFromNBT(aNBT, NBT_TANK + "." + i);
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) for (FluidTankGT tTank : mTanks) {
    ...
    protected boolean canOutput(Recipe aRecipe) {
        for (int i = 0; i < mTanks.length && i < aRecipe.mFluidOutputs.length; i++) if (mTanks[i].has()) {
            if (aRecipe.mNeedsEmptyOutput
                || (aRecipe.mFluidOutputs[i] != null && (!mTanks[i].contains(aRecipe.mFluidOutputs[i])
                    || FL.temperature(aRecipe.mFluidOutputs[i]) >= mMaterial.mMeltingPoint - 100
                    || FL.lighter(aRecipe.mFluidOutputs[i])
                    || mTanks[i].has(Math.max(1000, 1 + aRecipe.mFluidOutputs[i].amount))))) {
                return F;
            }
        }
        return T;
        ...
        if (isServerSide()) {
            ...
            if (tRecipe != null) {
                ...
                if (canOutput(tRecipe) && tRecipe.isRecipeInputEqual(T, F, ZL_FS, aStack)) {
                    ...
                    for (int i = 0; i < mTanks.length && i < tOutputFluids.length; i++)
                        mTanks[i].fill(tOutputFluids[i], T);
            ...
            if (aStack != null) for (FluidTankGT tTank : mTanks)
    ...
    protected IFluidTank getFluidTankDrainable2(byte aSide, FluidStack aFluidToDrain) {
        if (aFluidToDrain == null) {
            for (int i = 0, j; i < mTanks.length; i++)
                if (mTanks[j = ((int) (SERVER_TIME / 20) + i) % mTanks.length].has()) return mTanks[j];
        } else {
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[12], PX_N[12], PX_N[2]);
                return T;
            case 1:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[12], PX_N[12]);
                return T;
            case 2:
                box(aBlock, PX_P[12], PX_P[0], PX_P[2], PX_N[2], PX_N[12], PX_N[2]);
                return T;
            case 3:
                box(aBlock, PX_P[2], PX_P[0], PX_P[12], PX_N[2], PX_N[12], PX_N[2]);
                return T;
            case 4:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[15], PX_N[2]);
                return T;
            case 5:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[13], PX_N[2]);
                return T;
            case 6:
                box(aBlock, PX_P[6], PX_P[0], PX_P[6], PX_N[6], PX_N[9], PX_N[6]);
                return T;
            case 7:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[12], PX_N[12] + 0.001F, PX_N[12]);
                return T;
        }
        return F;
    }
    ...
    public static IIconContainer sTextureSides = new Textures.BlockIcons.CustomIcon(
        "machines/tools/juicer/colored/sides"),
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            ...
            case 6:
                return SIDE_TOP == aSide ? BlockTextureMulti
                    .get(BlockTextureDefault.get(sTextureMiddleTop, mRGBa), BlockTextureDefault.get(sOverlayMiddleTop))
                    : SIDE_BOTTOM == aSide ? null
                        : BlockTextureMulti.get(
                            BlockTextureDefault.get(sTextureMiddleSide, mRGBa),
    ...
    public float getSurfaceDistance(byte aSide) {
        return SIDES_TOP[aSide] ? PX_P[12] : PX_P[2];
    }
