...
public class MultiTileEntityCoin extends TileEntityBase04MultiTileEntities
    implements IMTE_OnDespawn, IMTE_GetLifeSpan, IMTE_GetSubItems, IMTE_OnRegistration, IMTE_OnRegistrationClient,
    IMTE_IgnorePlayerCollisionWhenPlacing, IMTE_GetExplosionResistance, IMTE_GetBlockHardness,
    IMTE_SetBlockBoundsBasedOnState, IMTE_AddCollisionBoxesToList, IMTE_GetCollisionBoundingBoxFromPool,
    IMTE_GetSelectedBoundingBoxFromPool, IMTE_GetLightOpacity, IMTE_AddToolTips, IMTE_OnPlaced, IMTE_SyncDataByteArray {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < mShape[0].length; i++)
            mShape[0][i] = UT.Code.getBitsS(aNBT.getShort("gt.coin.shape.0." + i));
        for (int i = 0; i < mShape[1].length; i++)
            mShape[1][i] = UT.Code.getBitsS(aNBT.getShort("gt.coin.shape.1." + i));
    ...
    public void writeToNBT2(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < mShape[0].length; i++)
            aNBT.setShort("gt.coin.shape.0." + i, (short) UT.Code.getBits(mShape[0][i]));
        for (int i = 0; i < mShape[1].length; i++)
            aNBT.setShort("gt.coin.shape.1." + i, (short) UT.Code.getBits(mShape[1][i]));
    ...
    public ArrayListNoNulls<ItemStack> getDrops(int aFortune, boolean aSilkTouch) {
        ...
        while (tCoinAmount > 0) {
            ...
            rList.add(
                getCoin(
                    tStackAmount,
                    MultiTileEntityRegistry.getRegistry(getMultiTileEntityRegistryID()),
    ...
    public boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide() && SIDES_TOP[aSide]) {
            ItemStack aStack = aPlayer.getCurrentEquippedItem(), tStack = getCoin(
                1,
                MultiTileEntityRegistry.getRegistry(getMultiTileEntityRegistryID()),
                getMultiTileEntityID());
            int tIndex = (int) (Math.min(0.99F, Math.max(0, aHitX)) * 4) * 4
                + (int) (Math.min(0.99F, Math.max(0, aHitZ)) * 4);
            if (tStack != null) {
                if (aStack == null) {
                    if (mCoinStackSizes[tIndex] > 0
                        && (UT.Entities.hasInfiniteItems(aPlayer) || ST.add(aPlayer, tStack, F))) {
            ...
            boolean temp = T;
            for (int i = 0; i < mCoinStackSizes.length; i++) if (mCoinStackSizes[i] > 0) {
                temp = F;
                break;
            }
    ...
    public int onDespawn(EntityItem aEntity, ItemStack aStack) {
        ...
        if (aNBT != null && !aEntity.worldObj.isRemote && aEntity.onGround) {
            if (aStack.stackSize > 0) for (byte tSide : ALL_SIDES_MIDDLE_DOWN) if (aStack.stackSize > 0) {
                TileEntity tTileEntity = WD.te(
                    aEntity.worldObj,
                    UT.Code.roundDown(aEntity.posX) + OFFX[tSide],
                    UT.Code.roundDown(aEntity.posY) + OFFY[tSide],
                    UT.Code.roundDown(aEntity.posZ) + OFFZ[tSide],
                    T);
                if (tTileEntity instanceof MultiTileEntityCoin) {
                    ...
                    if (tNBT.equals(aNBT)) {
                        for (int i = 0; i < mCoinStackSizes.length; i++) {
                            int tDifference = Math.min(
                                aStack.stackSize,
                                COIN_STACKSIZE - ((MultiTileEntityCoin) tTileEntity).mCoinStackSizes[i]);
                            if (tDifference > 0) {
            ...
            if (aStack.stackSize > 0) for (byte tSide : ALL_SIDES_MIDDLE_DOWN) if (aStack.stackSize > 0) {
                if (aEntity.worldObj.canPlaceEntityOnSide(
                    MTE_REGISTRY.mBlock,
                    UT.Code.roundDown(aEntity.posX) + OFFX[tSide],
                    UT.Code.roundDown(aEntity.posY) + OFFY[tSide],
                    UT.Code.roundDown(aEntity.posZ) + OFFZ[tSide],
                    ...
                    if (tUsedAmount > 0 && MTE_REGISTRY.mBlock.placeBlock(
                        aEntity.worldObj,
                        UT.Code.roundDown(aEntity.posX) + OFFX[tSide],
                        UT.Code.roundDown(aEntity.posY) + OFFY[tSide],
                        UT.Code.roundDown(aEntity.posZ) + OFFZ[tSide],
                        SIDE_UNKNOWN,
                        INSTANCE.getMultiTileEntityID(),
                        tNBT,
                        T,
                        F)) {
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        mCoinStackSizes[(int) (Math.min(0.99F, Math.max(0, aHitX)) * 4) * 4
            + (int) (Math.min(0.99F, Math.max(0, aHitZ)) * 4)] = 1;
        return T;
    }
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        if (aSendAll) {
            ...
            for (int i = 0; i < 32; i++)
                tPacket[i + 32] = UT.Code.toByteS((short) UT.Code.getBits(mShape[1][i / 2]), i % 2);
    ...
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        ...
        for (int i = 0; i < 16; i++)
            mShape[1][i] = UT.Code.getBitsS(UT.Code.combine(aData[32 + i * 2], aData[33 + i * 2]));
        ...
        if (UT.Code.exists(tMaterial, OreDictMaterial.MATERIAL_ARRAY))
            mMaterial = OreDictMaterial.MATERIAL_ARRAY[tMaterial];
        return T;
    }
    ...
    public void onRegistration(MultiTileEntityRegistry aRegistry, short aID) {
        ...
        for (OreDictMaterial tMaterial : OreDictMaterial.MATERIAL_ARRAY) if (tMaterial != null) {
            if ((tMaterial.mHidden || !OP.plateTiny.canGenerateItem(tMaterial)) && tMaterial != MT.Cu
                && tMaterial != MT.Ag
                && tMaterial != MT.Au
                && tMaterial != MT.Pt) continue;

            mMaterial = tMaterial;
            mIsUnique = T;
            mShape = new boolean[][][] { { { T, T, T, T, T, T, F, F, F, F, T, T, T, T, T, T },
                { T, T, T, T, F, F, F, F, F, F, F, F, T, T, T, T }, { T, T, F, F, F, F, F, F, F, F, F, F, F, F, T, T },
                { T, T, F, F, F, F, F, T, T, F, F, F, F, F, T, T }, { T, F, F, F, F, T, F, T, T, F, T, F, F, F, F, T },
                { T, F, F, F, T, T, T, T, T, T, T, T, F, F, F, T }, { F, F, F, F, F, T, T, F, F, T, T, F, F, F, F, F },
                { F, F, F, T, T, T, F, F, F, F, T, T, T, F, F, F }, { F, F, F, T, T, T, F, F, F, F, T, T, T, F, F, F },
                { F, F, F, F, F, T, T, F, F, T, T, F, F, F, F, F }, { T, F, F, F, T, T, T, T, T, T, T, T, F, F, F, T },
                { T, F, F, F, F, T, F, T, T, F, T, F, F, F, F, T }, { T, T, F, F, F, F, F, T, T, F, F, F, F, F, T, T },
                { T, T, F, F, F, F, F, F, F, F, F, F, F, F, T, T }, { T, T, T, T, F, F, F, F, F, F, F, F, T, T, T, T },
    ...
    public boolean setBlockBounds(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        if (mUseWorldRendering) {
            if (mUseNormalRendering) {
                if (aRenderPass % 256 == 0)
                    while (mCoinStackSizes[(aRenderPass + mRenderPassIndex) / 256] <= 0) mRenderPassIndex += 256;
        ...
        if (mUseNormalRendering) {
            box(
                ...
                PX_P[9] - (mShape[0][(aRenderPass % 256) / 16][aRenderPass % 16] ? DIST : 0)
                    - (mShape[1][(aRenderPass % 256) / 16][aRenderPass % 16] ? DIST * 2 : 0),
                PX_P[1 + (aRenderPass % 16)]);
            return T;
        }
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0) {
            if (!mUseNormalRendering && mUseWorldRendering) {
                mTextures[1] = BlockTextureDefault.get(
                    Textures.BlockIcons.COIN_TOP,
                    mMaterial == null ? UNCOLOURED : mMaterial.fRGBaSolid,
                    F,
                    F,
                    T,
                    !mUseNormalRendering);
                mTextures[0] = BlockTextureDefault.get(
                    Textures.BlockIcons.COIN_BOTTOM,
                    mMaterial == null ? UNCOLOURED : mMaterial.fRGBaSolid,
                    F,
                    F,
                    T,
                ...
                mTextures[0] = mTextures[1] = BlockTextureDefault.get(
                    Textures.BlockIcons.COIN,
                    mMaterial == null ? UNCOLOURED : mMaterial.fRGBaSolid,
                    F,
                    F,
                    T,
            ...
            mTextures[2] = mTextures[3] = mTextures[4] = mTextures[5] = BlockTextureDefault.get(
                Textures.BlockIcons.COIN_SIDE,
                mMaterial == null ? UNCOLOURED : mMaterial.fRGBaSolid,
                F,
                F,
                T,
        ...
        if (mUseNormalRendering) {
            ...
            switch (aSide) {
                case SIDE_X_NEG:
                    if (aX > 0 && aHeight <= 3 - (mShape[0][aX - 1][aZ] ? 1 : 0) - (mShape[1][aX - 1][aZ] ? 2 : 0))
                        return null;
                    break;
                case SIDE_X_POS:
                    if (aX < 15 && aHeight <= 3 - (mShape[0][aX + 1][aZ] ? 1 : 0) - (mShape[1][aX + 1][aZ] ? 2 : 0))
                        return null;
                    break;
                case SIDE_Z_NEG:
                    if (aZ > 0 && aHeight <= 3 - (mShape[0][aX][aZ - 1] ? 1 : 0) - (mShape[1][aX][aZ - 1] ? 2 : 0))
                        return null;
                    break;
                case SIDE_Z_POS:
                    if (aZ < 15 && aHeight <= 3 - (mShape[0][aX][aZ + 1] ? 1 : 0) - (mShape[1][aX][aZ + 1] ? 2 : 0))
                        return null;
                    break;
            }
        }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(PX_P[2], PX_P[2], PX_P[2], PX_N[2], PX_N[2], PX_N[2]);
    }

    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool() {
        return box(PX_P[0], PX_P[0], PX_P[0], PX_N[0], PX_N[14], PX_N[0]);
    }
    ...
    public void addCollisionBoxesToList(AxisAlignedBB aAABB, List<AxisAlignedBB> aList, Entity aEntity) {
        for (byte i = 0; i < 16; i++) if (mCoinStackSizes[i] > 0) box(
            aAABB,
            aList,
            PX_P[i / 4] * 4,
            0,
            PX_P[i % 4] * 4,
            PX_P[i / 4] * 4 + PX_P[4],
            mCoinStackSizes[i] / (float) COIN_STACKSIZE,
            PX_P[i % 4] * 4 + PX_P[4]);
    }
