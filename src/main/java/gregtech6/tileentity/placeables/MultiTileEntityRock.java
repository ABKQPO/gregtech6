 ...
 */
public class MultiTileEntityRock extends TileEntityBase03MultiTileEntities
    implements IMTE_CanPlaceSnowLayerOnRemoval, IMTE_CanEntityDestroy, IMTE_IgnorePlayerCollisionWhenPlacing,
    IMTE_OnToolClick, IMTE_OnNeighborBlockChange, IMTE_GetBlockHardness, IMTE_IsSideSolid, IMTE_GetLightOpacity,
    IMTE_GetExplosionResistance, ITileEntityQuickObstructionCheck, IMTE_GetCollisionBoundingBoxFromPool,
    ...
    public long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (isServerSide() && aTool.equals(TOOL_magnifyingglass)) {
            ...
            if (OD.itemFlint.is_(mRock)) {
                aChatReturn.add(
                    LH.Chat.GRAY + (APRIL_FOOLS || rng(WOODMANS_BDAY ? 10 : XMAS_IN_JULY ? 100 : 1000) == 0
            ...
            if (tData != null && tData.nonemptyMaterial()) {
                ...
                if (tData.mMaterial.mMaterial.contains(TD.Properties.STONE)) {
                    aChatReturn.add(
                        LH.Chat.GRAY + "This Rock consists out of "
                            + LH.Chat.WHITE
                            + tData.mMaterial.mMaterial.getLocal());
                    return 1;
                }
                aChatReturn
                    .add(LH.Chat.GRAY + "This Rock is bearing " + LH.Chat.CYAN + tData.mMaterial.mMaterial.getLocal());
                return 1;
            }
            return 1;
        }
    ...
    public void onNeighborBlockChange(World aWorld, Block aBlock) {
        if (isServerSide()) {
            if (!worldObj.getBlock(xCoord, yCoord - 1, zCoord)
                .isSideSolid(worldObj, xCoord, yCoord - 1, zCoord, FORGE_DIR[SIDE_TOP])) {
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] || SIDES_TOP_HORIZONTAL[aSide] ? mTexture : null;
    }

    @Override
    public int getRenderPasses(Block aBlock, boolean[] aShouldSideBeRendered) {
        if (worldObj == null) {
            mTexture = sStoneTexture;
            return 1;
        }
        if (hasSnow()) {
            mTexture = SNOW_TEXTURE;
            return 2;
        }

        Block tBlock = getBlockAtSide(SIDE_BOTTOM);
        if (tBlock == BlocksGT.Diggables) {
            mTexture = BlockTextureCopied.get(BlocksGT.Kimberlite, SIDE_ANY, 0);
            return 1;
        }
        if (tBlock instanceof BlockStones || tBlock == Blocks.snow
            || tBlock == Blocks.stone
            || tBlock == Blocks.end_stone
            || tBlock == Blocks.obsidian) {
        ...
        if (IL.NeLi_Gravel.equal(tBlock)) {
            mTexture = BlockTextureCopied.get(BlocksGT.GraniteBlack, SIDE_ANY, 0);
            return 1;
        }
        ...
        if (WD.dimALF(worldObj)) {
            mTexture = BlockTextureCopied.get(BlocksGT.Marble);
            return 1;
        }
        if (WD.dimATUM(worldObj)) {
            mTexture = BlockTextureCopied.get(BlocksGT.Limestone);
            return 1;
        }
        if (WD.dimAETHER(worldObj)) {
            mTexture = BlockTextureCopied.get(BlocksGT.Andesite);
            return 1;
        }
        if (WD.dimTROPIC(worldObj)) {
            mTexture = BlockTextureCopied.get(BlocksGT.Basalt);
            return 1;
        }
    ...
    public boolean setBlockBounds(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        box(
            aBlock,
            aRenderPass == 0 ? mMinX : 0,
            0,
            aRenderPass == 0 ? mMinZ : 0,
            aRenderPass == 0 ? mMaxX : 1,
            aRenderPass == 0 ? mMaxY : PX_P[1],
            aRenderPass == 0 ? mMaxZ : 1);
        return T;
    }

    @Override
    public void setBlockBoundsBasedOnState(Block aBlock) {
        box(aBlock, mMinX, 0, mMinZ, mMaxX, mMaxY, mMaxZ);
    }

    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool() {
        return box(mMinX, 0, mMinZ, mMaxX, mMaxY, mMaxZ);
    }
