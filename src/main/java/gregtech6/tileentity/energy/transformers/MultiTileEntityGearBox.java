 ...
 */
public class MultiTileEntityGearBox extends TileEntityBase07Paintable implements ITileEntityEnergy,
    ITileEntityRunningActively, ITileEntitySwitchableOnOff, IMTE_GetOreDictItemData, IMTE_AddToolTips {
    ...
    public byte mInputtedSides = 0, oInputtedSides = 0, mOrder = 0, mRotationData = 0, oRotationData = 0,
        mIgnorePower = 0;
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        if (!mGearsWork) aList.add(Chat.BLINKING_RED + LH.get("gt.tooltip.gearbox.custom.1"));
        aList
            .add(Chat.CYAN + LH.get(LH.AXLE_STATS_SPEED) + mMaxThroughPut + " " + TD.Energy.RU.getLocalisedNameShort());
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_wrench)) {
            ...
            if (aPlayerInventory != null) for (int i = 0, j = aPlayerInventory.getSizeInventory(); i < j; i++) {
                ...
                if (tData != null && tData.mPrefix == OP.gearGt
                    && tData.mMaterial != null
                    && (tData.mMaterial.mMaterial == mMaterial
                        || mMaterial.mToThis.contains(tData.mMaterial.mMaterial))) {
                    if (aPlayer == null) aPlayerInventory.decrStackSize(i, 1);
                    else ST.use(aPlayer, T, aPlayerInventory.getStackInSlot(i));
                    mAxleGear |= B[tSide];
                    mJammed = F;
                    mGearsWork = checkGears();
                    updateClientData();
        ...
        if (aTool.equals(TOOL_monkeywrench)) {
            if (SIDES_INVALID[aSide]) return 0;
            byte tSide = UT.Code.getSideWrenching(aSide, aHitX, aHitY, aHitZ);
            if (SIDES_AXIS_X[tSide]) if (((mAxleGear >>> 6) & 3) != 1) mAxleGear = (byte) ((mAxleGear & 63) | (1 << 6));
            else mAxleGear &= 63;
            if (SIDES_AXIS_Y[tSide]) if (((mAxleGear >>> 6) & 3) != 2) mAxleGear = (byte) ((mAxleGear & 63) | (2 << 6));
            else mAxleGear &= 63;
            if (SIDES_AXIS_Z[tSide]) if (((mAxleGear >>> 6) & 3) != 3) mAxleGear = (byte) ((mAxleGear & 63) | (3 << 6));
            else mAxleGear &= 63;
            mJammed = F;
            mGearsWork = checkGears();
            updateClientData();
        ...
        if (aTool.equals(TOOL_tachometer)) {
            if (aChatReturn != null) {
                if (FACE_CONNECTED[0][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][0])
                    aChatReturn.add(FACE_CONNECTED[0][mInputtedSides] ? "Accepts from Bottom" : "Emits to Bottom");
                if (FACE_CONNECTED[1][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][1])
                    aChatReturn.add(FACE_CONNECTED[1][mInputtedSides] ? "Accepts from Top" : "Emits to Top");
                if (FACE_CONNECTED[2][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][2])
                    aChatReturn.add(FACE_CONNECTED[2][mInputtedSides] ? "Accepts from North" : "Emits to North");
                if (FACE_CONNECTED[3][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][3])
                    aChatReturn.add(FACE_CONNECTED[3][mInputtedSides] ? "Accepts from South" : "Emits to South");
                if (FACE_CONNECTED[4][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][4])
                    aChatReturn.add(FACE_CONNECTED[4][mInputtedSides] ? "Accepts from West" : "Emits to West");
                if (FACE_CONNECTED[5][mAxleGear & 63] || AXIS_XYZ[(mAxleGear >>> 6) & 3][5])
                    aChatReturn.add(FACE_CONNECTED[5][mInputtedSides] ? "Accepts from East" : "Emits to East");
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            ...
            if (aChatReturn != null) aChatReturn.add(
                mGearsWork
                    ? mJammed ? "Gears interlocked properly, but they are jammed!" : "Gears interlocked properly."
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
                ...
                while (temp) {
                    ...
                    for (byte i = 0; i < 6; i++) {
                        ...
                        if (!FACE_CONNECTED[tSide][mInputtedSides]) {
                            if (FACE_CONNECTED[tSide][mAxleGear & 63]) {
                                long tUsed = ITileEntityEnergy.Util.insertEnergyInto(
                                    TD.Energy.RU,
                                    (mRotationData & B[tSide]) != 0 ? +mCurrentSpeed : -mCurrentSpeed,
                                    tUsable,
                                    this,
                                    getAdjacentTileEntity(tSide));
                                if (tUsed > 0) {
                                    mCurrentPower -= tUsed;
                                    if (mCurrentPower <= 0) {
                                        temp = F;
                            ...
                            } else if (AXIS_XYZ[(mAxleGear >>> 6) & 3][tSide]
                                && FACE_CONNECTED[OPOS[tSide]][mAxleGear & 63]) {
                                    long tUsed = ITileEntityEnergy.Util.insertEnergyInto(
                                        TD.Energy.RU,
                                        (mRotationData & B[OPOS[tSide]]) == 0 ? +mCurrentSpeed : -mCurrentSpeed,
                                        tUsable,
                                        this,
                                        getAdjacentTileEntity(tSide));
                                    if (tUsed > 0) {
            ...
            if ((mRotationData & B[6]) != 0 && WD.random(this, 20, CLIENT_TIME))
                UT.Sounds.play(SFX.MC_MINECART, 1, 0.1F, getCoords());
        }
    }
    ...
    public byte getRotations(byte aSide, boolean aNegative) {
        ...
        if (AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide]) {
            ...
            if (FACE_CONNECTED[aSide][mAxleGear & 63]) {
                ...
                if (!aNegative) for (byte tSide : ALL_SIDES_VALID_BUT_AXIS[aSide])
                    if (FACE_CONNECTED[tSide][mAxleGear & 63]) rRotationData |= B[tSide];
            ...
            // There is no Gears on that Axle, this should actually not get this far, because the Passthrough takes
            // over, before this gets called.
            ERR.print(
                "Something went wrong with the free Axle inside the Gearbox at " + getCoords()
                    + " receiving power from Side: "
                    + aSide);
        ...
        ERR.print(
            "Something went wrong with the Gearbox at " + getCoords()
                + " receiving power from Side: "
                + aSide
                + " even though there is neither a Gear nor an Axle at that Side");
        // Returning the current Rotation Data to make sure nothing breaks too badly.
        return mRotationData;
    }
    ...
    public boolean checkGears() {
        ...
        switch (FACE_CONNECTION_COUNT[mAxleGear & 63]) {
            ...
            case 2:
                ...
                // But also Rotate when both Gears are on the same Axle, as dumb and useless as that is, it is what
                // would happen.
                switch ((mAxleGear >>> 6) & 3) {
                    case 1:
                        return (mAxleGear & 48) != 0;
                    case 2:
                        return (mAxleGear & 3) != 0;
                    case 3:
                        return (mAxleGear & 12) != 0;
                }
                return F;
            case 3:
            case 4:
                ...
                switch ((mAxleGear >>> 6) & 3) {
                    case 1:
                        if ((mAxleGear & 48) == 48) return F;
                        break;
                    case 2:
                        if ((mAxleGear & 3) == 3) return F;
                        break;
                    case 3:
                        if ((mAxleGear & 12) == 12) return F;
                        break;
                }
    ...
    public boolean onTickCheck(long aTimer) {
        return mRotationData != oRotationData || super.onTickCheck(aTimer);
    }

    @Override
    public void onTickResetChecks(long aTimer, boolean aIsServerSide) {
        super.onTickResetChecks(aTimer, aIsServerSide);
        oRotationData = mRotationData;
    }
    ...
    public int getRenderPasses2(Block aBlock, boolean[] aShouldSideBeRendered) {
        ...
        mTextureGearA = BlockTextureMulti.get(
            mTexture,
            BlockTextureDefault.get(
                (mRotationData & B[6]) == 0 ? Textures.BlockIcons.GEAR : Textures.BlockIcons.GEAR_CLOCKWISE,
                mRGBa));
        mTextureAxleGearA = BlockTextureMulti.get(
            mTextureAxle,
            BlockTextureDefault.get(
                (mRotationData & B[6]) == 0 ? Textures.BlockIcons.GEAR : Textures.BlockIcons.GEAR_CLOCKWISE,
                mRGBa));
        mTextureGearB = BlockTextureMulti.get(
            mTexture,
            BlockTextureDefault.get(
                (mRotationData & B[6]) == 0 ? Textures.BlockIcons.GEAR : Textures.BlockIcons.GEAR_COUNTERCLOCKWISE,
                mRGBa));
        mTextureAxleGearB = BlockTextureMulti.get(
            mTextureAxle,
            BlockTextureDefault.get(
                (mRotationData & B[6]) == 0 ? Textures.BlockIcons.GEAR : Textures.BlockIcons.GEAR_COUNTERCLOCKWISE,
                mRGBa));
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? FACE_CONNECTED[aSide][mAxleGear & 63]
            ? FACE_CONNECTED[aSide][mRotationData & 63]
                ? AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide] ? mTextureAxleGearA : mTextureGearA
                : AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide] ? mTextureAxleGearB : mTextureGearB
            : AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide] ? mTextureAxle : mTexture : null;
    }
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSpeed, long aPower, boolean aDoInject) {
        ...
        if (tSpeed > mMaxThroughPut) {
            ...
            if (tCount > 0) {
                ...
                if (tCount > 1) ST.drop(getWorld(), getCoords(), OP.gearGt.mat(mMaterial, tCount - 1));
            }
            mAxleGear = 0;
            updateClientData();
            mGearsWork = checkGears();
        ...
        if (AXIS_XYZ[(mAxleGear >>> 6) & 3][aSide] && !FACE_CONNECTED[aSide][mAxleGear & 63]
            && !FACE_CONNECTED[OPOS[aSide]][mAxleGear & 63]) {
            return ITileEntityEnergy.Util
                .insertEnergyInto(TD.Energy.RU, aSpeed, aPower, this, getAdjacentTileEntity(OPOS[aSide]));
        }
        ...
        if ((mRotationData = getRotations(aSide, aSpeed < 0)) != 0) {
            ...
            if (mCurrentPower > 0) mIgnorePower++;
            else mIgnorePower = 0;
            // If ignoring further Inputs, keep the old values.
            if (mIgnorePower != 0) return 0;
    ...
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mJammed) && super.isEnergyAcceptingFrom(aEnergyType, aSide, aTheoretical);
    }
    ...
    public boolean isUsingWrenchingOverlay(ItemStack aStack, byte aSide) {
        return super.isUsingWrenchingOverlay(aStack, aSide) || ToolsGT.contains(TOOL_wrench, aStack)
            || ToolsGT.contains(TOOL_monkeywrench, aStack);
    }

    @Override
    public boolean isConnectedWrenchingOverlay(ItemStack aStack, byte aSide) {
        return FACE_CONNECTED[aSide][mAxleGear & 63];
    }
    ...
    public boolean getStateRunningPassively() {
        return (mRotationData & B[6]) != 0 || (oRotationData & B[6]) != 0;
    }
    ...
    public List<OreDictItemData> getOreDictItemData(List<OreDictItemData> aList) {
        if (FACE_CONNECTION_COUNT[mAxleGear & 63] > 0)
            aList.add(new OreDictItemData(mMaterial, OP.gearGt.mAmount * FACE_CONNECTION_COUNT[mAxleGear & 63]));
        return aList;
    }
}
