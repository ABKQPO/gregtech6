...
public class MultiTileEntityLongDistanceTransformer extends TileEntityBase09FacingSingle
    implements IMTE_HasMultiBlockMachineRelevantData, ITileEntityCanDelegate, ITileEntityMachineBlockUpdateable,
    ITileEntityRunningActively, ITileEntityEnergyElectricityAcceptor, ITileEntitySwitchableOnOff {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_ACTIVE_DATA)) {
            mActiveData = aNBT.getLong(NBT_ACTIVE_DATA);
        }
        if (aNBT.hasKey(NBT_INPUT)) {
            mInput = aNBT.getLong(NBT_INPUT);
        }
        if (aNBT.hasKey(NBT_OUTPUT)) {
            mOutput = aNBT.getLong(NBT_OUTPUT);
        }
        if (aNBT.hasKey(NBT_DISTANCE)) {
            mDistance = aNBT.getLong(NBT_DISTANCE);
        }
        if (aNBT.hasKey(NBT_THROUGHPUT)) {
            mThroughput = aNBT.getLong(NBT_THROUGHPUT);
        }
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        LH.addEnergyToolTips(
            this,
            aList,
            mEnergyTypeAccepted,
            mEnergyTypeEmitted,
            getLocalisedInputSide(),
            getLocalisedOutputSide());
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) {
                if (mSender != null && !mSender.isDead() && mSender.mTarget == this) {
                    ...
                    aChatReturn
                        .add("Sender is at: X: " + mSender.xCoord + " Y: " + mSender.yCoord + " Z: " + mSender.zCoord);
                } else {
                    aChatReturn.add(checkTarget() ? "Has Target" : "Has no loaded Target");
                    if (mTargetPos != null) aChatReturn.add(
    ...
    public boolean onTickCheck(long aTimer) {
        ...
        if (mStopped) {
            mActiveState = 0;
        } else if ((mTarget != null && (mTarget.mSender == null || mTarget.mSender.isDead()
            || mTarget.mSender.mTarget == null
            || mTarget.mSender.mTarget.isDead()))
            && (mTargetPos != null && (mTarget == null || mTarget.isDead()
                || !worldObj.blockExists(mTargetPos.posX, mTargetPos.posY, mTargetPos.posZ)))) {
                    mActiveState = 3;
                } else {
                    mActiveData <<= 1;
    ...
    public boolean checkTarget() {
        ...
        if (mTarget.mSender == null || mTarget.mSender.isDead()
            || mTarget.mSender.mTarget == null
            || mTarget.mSender.mTarget.isDead()) mTarget.mSender = this;
    ...
    private void scanWires(boolean aBurnWires) {
        ...
        if (aBlock instanceof BlockLongDistWire) {
            ...
            HashSetNoNulls<ChunkCoordinates> tNewChecks = new HashSetNoNulls<>(),
                tOldChecks = new HashSetNoNulls<>(F, getCoords()),
                tToCheck = new HashSetNoNulls<>(F, getOffsetN(mFacing, 1)), tWires = new HashSetNoNulls<>();
            ...
            while (!tToCheck.isEmpty()) {
                ...
                for (ChunkCoordinates aCoords : tToCheck) {
                    if (getBlock(aCoords) == aBlock && getMetaData(aCoords) == aMetaData) {
                        ...
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX + 1, aCoords.posY, aCoords.posZ)))
                            tNewChecks.add(tCoords);
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX - 1, aCoords.posY, aCoords.posZ)))
                            tNewChecks.add(tCoords);
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY + 1, aCoords.posZ)))
                            tNewChecks.add(tCoords);
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY - 1, aCoords.posZ)))
                            tNewChecks.add(tCoords);
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY, aCoords.posZ + 1)))
                            tNewChecks.add(tCoords);
                        if (tOldChecks
                            .add(tCoords = new ChunkCoordinates(aCoords.posX, aCoords.posY, aCoords.posZ - 1)))
                            tNewChecks.add(tCoords);
                        ...
                        if (tTileEntity != this && tTileEntity instanceof MultiTileEntityLongDistanceTransformer) {
                            if (tWires.contains(
                                ((MultiTileEntityLongDistanceTransformer) tTileEntity)
                                    .getOffset(((MultiTileEntityLongDistanceTransformer) tTileEntity).mFacing, 1))) {
                                mTarget = (MultiTileEntityLongDistanceTransformer) tTileEntity;
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        ...
        if (aDoInject) {
            long tConsumed = ITileEntityEnergy.Util
                .emitEnergyToNetwork(mTarget.mEnergyTypeEmitted, aSize * aSign, aAmount, mTarget);
    ...
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || checkTarget()) && (SIDES_INVALID[aSide] || isInput(aSide))
            && super.isEnergyAcceptingFrom(aEnergyType, aSide, aTheoretical);
    }

    @Override
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (SIDES_INVALID[aSide] || isOutput(aSide)) && super.isEnergyEmittingTo(aEnergyType, aSide, aTheoretical);
    }

    @Override
    public Collection<TagData> getEnergyTypes(byte aSide) {
        return new ArrayListNoNulls<>(F, mEnergyTypeAccepted, mEnergyTypeEmitted);
    }

    @Override
    public double getDemandedEnergy() {
        return checkTarget() ? super.getDemandedEnergy() : 0;
    }
    ...
    public boolean getStateRunningActively() {
        return mActive && ((mSender != null && !mSender.isDead() && mSender.mTarget == this)
            || (mTarget != null && !mTarget.isDead()));
    }
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return aEmitting ? aEnergyType == mEnergyTypeEmitted : aEnergyType == mEnergyTypeAccepted;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return BlockTextureMulti.get(
            BlockTextureDefault.get(sColoreds[aIndex], mRGBa),
            BlockTextureDefault.get(sOverlays[mActiveState][aIndex]));
    }
    ...
    public static IIconContainer sColoreds[] = new IIconContainer[] {
        ...
        sOverlays[][] = new IIconContainer[][] { {
            ...
            new Textures.BlockIcons.CustomIcon(
                "machines/transformers/longdistancetransformer_electric/overlay/side"), },
            { new Textures.BlockIcons.CustomIcon(
                "machines/transformers/longdistancetransformer_electric/overlay_active/front"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_active/back"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_active/side"), },
            { new Textures.BlockIcons.CustomIcon(
                "machines/transformers/longdistancetransformer_electric/overlay_blinking/front"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_blinking/back"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_blinking/side"), },
            { new Textures.BlockIcons.CustomIcon(
                "machines/transformers/longdistancetransformer_electric/overlay_unloaded/front"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_unloaded/back"),
                new Textures.BlockIcons.CustomIcon(
                    "machines/transformers/longdistancetransformer_electric/overlay_unloaded/side"), } };
