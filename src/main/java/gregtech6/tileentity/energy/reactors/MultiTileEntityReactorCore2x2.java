...
public class MultiTileEntityReactorCore2x2 extends MultiTileEntityReactorCore implements ITileEntitySwitchableMode {
    ...
    public void onServerTickPost(boolean aFirst) {
        if (aFirst) {
            ...
            if (SERVER_TIME % 20 == 19) {
                if (!mStopped) {
                    DelegatorTileEntity<MultiTileEntityReactorCore> tAdjacents[] = new DelegatorTileEntity[4],
                        tAdjacent;
                    DelegatorTileEntity tAdjacentTE = getAdjacentTileEntity(SIDE_Z_NEG);
                    if (tAdjacentTE.mTileEntity instanceof MultiTileEntityReactorCore
                        && SIDES_HORIZONTAL[tAdjacentTE.mSideOfTileEntity]) tAdjacents[0] = tAdjacentTE;
                    tAdjacentTE = getAdjacentTileEntity(SIDE_Z_POS);
                    if (tAdjacentTE.mTileEntity instanceof MultiTileEntityReactorCore
                        && SIDES_HORIZONTAL[tAdjacentTE.mSideOfTileEntity]) tAdjacents[1] = tAdjacentTE;
                    tAdjacentTE = getAdjacentTileEntity(SIDE_X_NEG);
                    if (tAdjacentTE.mTileEntity instanceof MultiTileEntityReactorCore
                        && SIDES_HORIZONTAL[tAdjacentTE.mSideOfTileEntity]) tAdjacents[2] = tAdjacentTE;
                    tAdjacentTE = getAdjacentTileEntity(SIDE_X_POS);
                    if (tAdjacentTE.mTileEntity instanceof MultiTileEntityReactorCore
                        && SIDES_HORIZONTAL[tAdjacentTE.mSideOfTileEntity]) tAdjacents[3] = tAdjacentTE;
                    ...
                    if (tNeutronCount != 0 || tModerated) {
                        ...
                        if (tAdjacent != null)
                            mNeutronCounts[0] += tAdjacent.mTileEntity.getReactorRodNeutronReflection(
                                S2103[tAdjacent.mSideOfTileEntity],
                                tNeutronCount,
                                tModerated);
                        tAdjacent = tAdjacents[SIDE_X_NEG - 2];
                        if (tAdjacent != null)
                            mNeutronCounts[0] += tAdjacent.mTileEntity.getReactorRodNeutronReflection(
                                S0312[tAdjacent.mSideOfTileEntity],
                                tNeutronCount,
                                tModerated);
                    }
                    ...
                    if (tNeutronCount != 0 || tModerated) {
                        ...
                        if (tAdjacent != null)
                            mNeutronCounts[1] += tAdjacent.mTileEntity.getReactorRodNeutronReflection(
                                S0312[tAdjacent.mSideOfTileEntity],
                                tNeutronCount,
                                tModerated);
                        tAdjacent = tAdjacents[SIDE_X_NEG - 2];
                    ...
                    if (tNeutronCount != 0 || tModerated) {
                        ...
                        tAdjacent = tAdjacents[SIDE_Z_NEG - 2];
                        if (tAdjacent != null)
                            mNeutronCounts[2] += tAdjacent.mTileEntity.getReactorRodNeutronReflection(
                                S0312[tAdjacent.mSideOfTileEntity],
                                tNeutronCount,
                                tModerated);
                    ...
                    if (tNeutronCount != 0 || tModerated) {
                        ...
                        tAdjacent = tAdjacents[SIDE_Z_POS - 2];
                        if (tAdjacent != null)
                            mNeutronCounts[3] += tAdjacent.mTileEntity.getReactorRodNeutronReflection(
                                S2103[tAdjacent.mSideOfTileEntity],
                                tNeutronCount,
                                tModerated);
                        tAdjacent = tAdjacents[SIDE_X_POS - 2];
            ...
            long tCalc = UT.Code.divup(
                (oNeutronCounts[0] = mNeutronCounts[0]) + (oNeutronCounts[1] = mNeutronCounts[1])
                    + (oNeutronCounts[2] = mNeutronCounts[2])
                    + (oNeutronCounts[3] = mNeutronCounts[3]),
                256);
            ...
            if (tCalc > 0 && SERVER_TIME % 20 == 10) {
                for (Object tEntity : worldObj.loadedEntityList) if (tEntity instanceof EntityLivingBase) {
                    ...
                    int tStrength = UT.Code
                        .bindInt((long) (tCalc - ((EntityLivingBase) tEntity).getDistance(xCoord, yCoord, zCoord)));
                    if (tStrength > 0) UT.Entities
                        .applyRadioactivity((EntityLivingBase) tEntity, (int) UT.Code.divup(tStrength, 10), tStrength);
                }
            }
            ...
            if (mEnergy > 0) {
                ...
                if (tIsExploding && !invempty()) {
                    ...
                    for (Object tEntity : worldObj.loadedEntityList) if (tEntity instanceof EntityLivingBase) {
                        ...
                        int tStrength = UT.Code
                            .bindInt((long) (tCalc - ((EntityLivingBase) tEntity).getDistance(xCoord, yCoord, zCoord)));
                        if (tStrength > 0) UT.Entities.applyRadioactivity(
                            (EntityLivingBase) tEntity,
                            (int) UT.Code.divup(tStrength, 10),
                            tStrength);
                    }
                    updateClientData();
                }
    ...
    public int getReactorRodNeutronReflection(int aSlot, int aNeutrons, boolean aModerated) {
        if (!mStopped && (mMode & B[aSlot]) == 0 && slotHas(aSlot) && ST.item(slot(aSlot)) instanceof IItemReactorRod)
            return ((IItemReactorRod) ST.item(slot(aSlot)))
                .getReactorRodNeutronReflection(this, aSlot, slot(aSlot), aNeutrons, aModerated);
        return 0;
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_pincers) && SIDES_TOP[aSide]) {
            ...
            if (slotHas(tSlot)
                && ST.give(aPlayer, slot(tSlot), T, worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5)) {
                slotKill(tSlot);
                updateClientData();
                return 10000;
            }
        ...
        if (aTool.equals(TOOL_geigercounter)) {
            if (aChatReturn != null) {
                aChatReturn.add(
                    "Neutron Levels: " + oNeutronCounts[0]
                        + "n; "
                        + oNeutronCounts[1]
                        + "n; "
                        + oNeutronCounts[2]
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide() && SIDES_TOP[aSide]) {
            ...
            if (ST.item(aStack) instanceof IItemReactorRod
                && ((IItemReactorRod) ST.item_(aStack)).isReactorRod(aStack)) {
                int tSlot = aHitX < 0.5 ? aHitZ < 0.5 ? 0 : 1 : aHitZ < 0.5 ? 2 : 3;
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        if (aSendAll) return getClientDataPacketByteArray(
            ...
            getVisualData(),
            getDirectionData(),
            UT.Code.toByteS(FL.id_(mTanks[0]), 0),
            UT.Code.toByteS(FL.id_(mTanks[0]), 1),
            UT.Code.toByteS(ST.id(slot(0)), 0),
            UT.Code.toByteS(ST.id(slot(0)), 1),
            UT.Code.toByteS(ST.meta(slot(0)), 0),
            UT.Code.toByteS(ST.meta(slot(0)), 1),
            UT.Code.toByteS(ST.id(slot(1)), 0),
            UT.Code.toByteS(ST.id(slot(1)), 1),
            UT.Code.toByteS(ST.meta(slot(1)), 0),
            UT.Code.toByteS(ST.meta(slot(1)), 1),
            UT.Code.toByteS(ST.id(slot(2)), 0),
            UT.Code.toByteS(ST.id(slot(2)), 1),
            UT.Code.toByteS(ST.meta(slot(2)), 0),
            UT.Code.toByteS(ST.meta(slot(2)), 1),
            UT.Code.toByteS(ST.id(slot(3)), 0),
            UT.Code.toByteS(ST.id(slot(3)), 1),
            UT.Code.toByteS(ST.meta(slot(3)), 0),
            UT.Code.toByteS(ST.meta(slot(3)), 1));
    ...
    public int getRenderPasses2(Block aBlock, boolean[] aShouldSideBeRendered) {
        mTextures[0] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[0], mRGBa), BlockTextureDefault.get(sOverlays[0]));
        mTextures[1] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[1], mRGBa), BlockTextureDefault.get(sOverlays[1]));
        mTextures[2] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[2], mRGBa), BlockTextureDefault.get(sOverlays[2]));
        mTextures[3] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[3], mRGBa), BlockTextureDefault.get(sOverlays[3]));
        mTextures[4] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[4], mRGBa), BlockTextureDefault.get(sOverlays[4]));
        mTextures[5] = BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[5], mRGBa), BlockTextureDefault.get(sOverlays[5]));
        mTextures[10] = (mTanks[0].has() ? BlockTextureFluid.get(mTanks[0]) : null);
        ItemStack aStack = slot(0);
        if (ST.item(aStack) instanceof IItemReactorRod) {
            mTextures[6] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureSides(this, 0, aStack, !mStopped && (mMode & B[0]) == 0);
            mTextures[11] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureTop(this, 0, aStack, !mStopped && (mMode & B[0]) == 0);
        } else {
            mTextures[6] = null;
            mTextures[11] = null;
        }
        aStack = slot(1);
        if (ST.item(aStack) instanceof IItemReactorRod) {
            mTextures[7] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureSides(this, 1, aStack, !mStopped && (mMode & B[1]) == 0);
            mTextures[12] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureTop(this, 1, aStack, !mStopped && (mMode & B[1]) == 0);
        } else {
            mTextures[7] = null;
            mTextures[12] = null;
        }
        aStack = slot(2);
        if (ST.item(aStack) instanceof IItemReactorRod) {
            mTextures[8] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureSides(this, 2, aStack, !mStopped && (mMode & B[2]) == 0);
            mTextures[13] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureTop(this, 2, aStack, !mStopped && (mMode & B[2]) == 0);
        } else {
            mTextures[8] = null;
            mTextures[13] = null;
        }
        aStack = slot(3);
        if (ST.item(aStack) instanceof IItemReactorRod) {
            mTextures[9] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureSides(this, 3, aStack, !mStopped && (mMode & B[3]) == 0);
            mTextures[14] = ((IItemReactorRod) ST.item_(aStack))
                .getReactorRodTextureTop(this, 3, aStack, !mStopped && (mMode & B[3]) == 0);
        } else {
            mTextures[9] = null;
            mTextures[14] = null;
        }
    ...
    public boolean usesRenderPass2(int aRenderPass, boolean[] aShouldSideBeRendered) {
        return aRenderPass < 6 || (aRenderPass == 6 && slotHas(0))
            || (aRenderPass == 7 && slotHas(1))
            || (aRenderPass == 8 && slotHas(2))
            || (aRenderPass == 9 && slotHas(3))
            || (aRenderPass == 10 && mTanks[0].has());
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            ...
            case 6:
                return box(aBlock, PX_P[2], PX_P[1], PX_P[2], PX_N[10], PX_P[17], PX_N[10]);
            case 7:
                return box(aBlock, PX_P[2], PX_P[1], PX_P[10], PX_N[10], PX_P[17], PX_N[2]);
            case 8:
                return box(aBlock, PX_P[10], PX_P[1], PX_P[2], PX_N[2], PX_P[17], PX_N[10]);
            case 9:
                return box(aBlock, PX_P[10], PX_P[1], PX_P[10], PX_N[2], PX_P[17], PX_N[2]);
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aRenderPass < 6
            && !ALONG_AXIS[aRenderPass][aSide]
                ? null
                : aRenderPass == mFacing ? mTextures[4]
                    : aRenderPass == mSecondFacing ? mTextures[5]
                        : aRenderPass >= 6 || aRenderPass < 2
                            ? mTextures[SIDES_VERTICAL[aSide] && aRenderPass != 10 && aRenderPass > 1 ? aRenderPass + 5
                                : aRenderPass]
                            : mTextures[aRenderPass < 6 && isCovered((byte) aRenderPass) ? 3 : 2];
    }
    ...
    public int[] getAccessibleSlotsFromSide2(byte aSide) {
        return aSide == SIDE_DOWN || aSide == SIDE_TOP ? UT.Code.getAscendingArray(4) : ZL_INTEGER;
    }

    @Override
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return (mStopped || (mMode & B[aSlot]) != 0) && aStack != null
            && !slotHas(aSlot)
            && ST.item(aStack) instanceof IItemReactorRod
            && ((IItemReactorRod) ST.item_(aStack)).isReactorRod(aStack);
    }
    ...
    } // Yeah Naming Convention doesn't work on the first one I added, just imagine a ".2x2" at the end of this String.
}
