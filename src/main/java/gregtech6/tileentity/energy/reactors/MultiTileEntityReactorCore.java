 ...
 */
public abstract class MultiTileEntityReactorCore extends TileEntityBase10FacingDouble
    implements ITileEntityServerTickPost, IFluidHandler, ITileEntityTapAccessible, ITileEntityFunnelAccessible,
    ...
    public FluidTankGT[] mTanks = { new FluidTankGT(64000),
        new FluidTankGT(64000).setCapacity(FL.Steam.mName, 64000L * STEAM_PER_WATER) };
    ...
    static {
        ...
        LH.add(
            "gt.tooltip.reactorcore.2",
            "Accessing a reactor slot is only possible when the slot or the reactor is off.");
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (mTanks[0].overHalf() && mSecondFacing != mFacing)
                FL.move(mTanks[0], getAdjacentTank(mSecondFacing), mTanks[0].amount() - mTanks[0].capacity() / 2);
    ...
    public byte getVisualData() {
        return (byte) (mStopped ? mRunning ? mMode | 48 : mMode | 32 : mRunning ? mMode | 16 : mMode);
    }
    ...
    protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        return FL.Coolant_IC2.is(aFluidToFill) || FL.distw(aFluidToFill)
    ...
    public int funnelFill(byte aSide, FluidStack aFluid, boolean aDoFill) {
        if (!(FL.Coolant_IC2.is(aFluid) || FL.distw(aFluid)
    ...
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mRunning) {
            UT.Entities.applyHeatDamage(aEntity, 5);
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(PX_P[1], PX_P[1], PX_P[1], PX_N[1], PX_N[1], PX_N[1]);
    }
