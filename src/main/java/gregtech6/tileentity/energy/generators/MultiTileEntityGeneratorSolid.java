 ...
 */
public abstract class MultiTileEntityGeneratorSolid extends TileEntityBase09FacingSingle implements ITileEntityEnergy,
    ITileEntityRunningActively, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {
        ...
        if (aIsServerSide) {
            if (mBurning) {
                ...
                if (mEnergy < mRate * 2) {
                    ...
                    if (mOutput1 == null && slotHas(0)
                        && !WD.hasCollide(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing))
                        && !getBlockAtSide(mFacing).getMaterial()
                            .isLiquid()
                        ...
                        if (IL.RC_Firestone_Refined.equal(slot(0), T, T)) {
                            ...
                            if (ST.invalid(tStack)) {
                                // Just dump the empty Firestone to the Output. This should not happen, unless you
                                // insert an empty Firestone.
                                if (addStackToSlot(1, slot(0))) slotKill(0);
                            } else if (ST.invalid(ST.container(tStack, F))) {
                                // 80% of whatever Heat Energy you get from Lava. This is over 10 times the normal
                                // Firestone Furnace burn Value.
                            ...
                            Recipe tRecipe = mRecipes
                                .findRecipe(this, mLastRecipe, T, Long.MAX_VALUE, null, ZL_FS, slot(0));
                ...
                if (rng(200) == 0
                    && WD.flaming(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing))) {
                    mBurning = T;
                }
            }
            // Out of Fuel I guess.
            ...
            if (mBurning && rng(4) == 0) spawnBurningParticles(
                xCoord + 0.5 + OFFX[mFacing] * 0.55 + (SIDES_AXIS_X[mFacing] ? 0 : RNGSUS.nextFloat() * 0.6F - 0.3F),
                yCoord + RNGSUS.nextFloat() * 0.375F,
                zCoord + 0.5 + OFFZ[mFacing] * 0.55 + (SIDES_AXIS_Z[mFacing] ? 0 : RNGSUS.nextFloat() * 0.6F - 0.3F));
        }
    }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_igniter) && (aSide == mFacing || aPlayer == null)) {
            mBurning = T;
            return 10000;
        }
        if (aTool.equals(TOOL_extinguisher) && (aSide == mFacing || aPlayer == null)) {
            mBurning = F;
            return 10000;
        }
    ...
    public boolean[] getValidSides() {
        return mBurning ? SIDES_THIS[mFacing] : SIDES_HORIZONTAL;
    }

    @Override
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mBurning) UT.Entities.applyHeatDamage(aEntity, Math.min(10.0F, mRate / 10.0F));
    }

    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(0, 0, 0, 1, 0.875, 1);
    }
    ...
    public int[] getAccessibleSlotsFromSide2(byte aSide) {
        return aSide == mFacing ? ZL_INTEGER : ACCESSABLE_SLOTS;
    }

    @Override
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        return aStack != null && aSlot == 0
            && aSide != mFacing
            && (mRecipes.containsInput(aStack, this, NI) || IL.RC_Firestone_Refined.equal(aStack, T, T)
                || IL.RC_Firestone_Cracked.equal(aStack, T, T));
    }

    @Override
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        return aStack != null && aSlot == 1 && aSide != mFacing;
    }
    ...
    public boolean isEnergyEmittingTo(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return SIDES_TOP[aSide] && super.isEnergyEmittingTo(aEnergyType, aSide, aTheoretical);
    }
    ...
    public boolean getStateRunningPossible() {
        return mBurning || (mOutput1 == null && slotHas(0)
            && !WD.hasCollide(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing))
            && !getBlockAtSide(mFacing).getMaterial()
                .isLiquid()
            && WD.oxygen(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing)));
    ...
    public float getBlockHardness() {
        return mBurning ? super.getBlockHardness() * 16 : super.getBlockHardness();
    }
