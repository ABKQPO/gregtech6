 ...
 */
public class MultiTileEntityGeneratorHotFluid extends TileEntityBase09FacingSingle
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            // Emit buffered Energy. And yes if you use a strong enough Fuel, that Energy would stay buffered even while
            // the Box is Off. This is very intended and represents partially used Fuel.
            if (mEnergy > 0) {
            ...
            if (mEnergy < mRate * 2) {
                ...
                if (!mTanks[1].has(mRate * 20)) {
                    ...
                    Recipe tRecipe = mRecipes
                        .findRecipe(this, mLastRecipe, T, Long.MAX_VALUE, NI, mTanks[0].AS_ARRAY, ZL_IS);
                    if (tRecipe != null) {
                        ...
                        // set remaining Fluid to null, because it is not valid Fuel anymore for whatever reason.
                        // MineTweaker happens to live Modpacks too sometimes. ;)
                        mTanks[0].setEmpty();
                    }
                }
            }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? SIDES_TOP[aSide]
            ? BlockTextureMulti.get(
                BlockTextureDefault.get(sColoreds[1], mRGBa),
                BlockTextureDefault.get((mActivity.mState > 0 ? sOverlaysActive : sOverlays)[1]))
            : aSide == mFacing
                ? BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[3], mRGBa),
                    BlockTextureDefault.get((mActivity.mState > 0 ? sOverlaysActive : sOverlays)[3]))
                : BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[2], mRGBa),
                    BlockTextureDefault.get((mActivity.mState > 0 ? sOverlaysActive : sOverlays)[2]))
            : null;
    }

    @Override
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mActivity.mState > 0) UT.Entities.applyHeatDamage(aEntity, Math.min(10.0F, mRate / 10.0F));
    }
    ...
    public boolean getStateRunningPossible() {
        return mActivity.mActive || mTanks[0].has();
    }
