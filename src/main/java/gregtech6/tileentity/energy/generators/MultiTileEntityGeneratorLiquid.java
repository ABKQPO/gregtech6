 ...
 */
public class MultiTileEntityGeneratorLiquid extends TileEntityBase09FacingSingle
    implements IFluidHandler, ITileEntityTapAccessible, ITileEntityEnergy, ITileEntityRunningActively,
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            ...
            if (mBurning || mCooldown > 0) {
                // Emit buffered Energy. And yes if you use a strong enough Fuel, that Energy would stay buffered even
                // while the Burning Box is Off. This is very intended and represents partially used Fuel.
                if (mEnergy >= mRate) {
                ...
                if (mEnergy < mRate * 2) {
                    ...
                    if (!WD.hasCollide(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing))
                        ...
                        Recipe tRecipe = mRecipes
                            .findRecipe(this, mLastRecipe, T, Long.MAX_VALUE, NI, mTank.AS_ARRAY, ZL_IS);
            ...
            if (mBurning && rng(5) == 0) spawnBurningParticles(
                xCoord + 0.5 + OFFX[mFacing] * 0.55 + (SIDES_AXIS_X[mFacing] ? 0 : RNGSUS.nextFloat() * 0.6 - 0.3),
                yCoord + RNGSUS.nextFloat() * 0.375,
                zCoord + 0.5 + OFFZ[mFacing] * 0.55 + (SIDES_AXIS_Z[mFacing] ? 0 : RNGSUS.nextFloat() * 0.6 - 0.3));
        }
    }
    ...
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mBurning || mCooldown > 0) UT.Entities.applyHeatDamage(aEntity, Math.min(10.0F, mRate / 10.0F));
    }
    ...
    public boolean getStateRunningPossible() {
        return mBurning
            || (mTank.has() && !WD.hasCollide(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing))
                && !getBlockAtSide(mFacing).getMaterial()
                    .isLiquid()
                && WD.oxygen(worldObj, getOffsetX(mFacing), getOffsetY(mFacing), getOffsetZ(mFacing)));
    }
