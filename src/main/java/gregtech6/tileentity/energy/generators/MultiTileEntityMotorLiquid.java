 ...
 */
public class MultiTileEntityMotorLiquid extends TileEntityBase09FacingSingle
    implements IFluidHandler, ITileEntityFunnelAccessible, ITileEntityTapAccessible, ITileEntityEnergy,
    ITileEntityRunningActively, ITileEntityAdjacentOnOff {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
                    ...
                    if (tRecipe.mFluidOutputs.length <= 0 || mTanks[1].canFillAll(tRecipe.mFluidOutputs[0])) {
                        if (tRecipe.isRecipeInputEqual(T, F, mTanks[0].AS_ARRAY, ZL_IS)) {
                            ...
                            while (mEnergy < mRate * 2
                                && (tRecipe.mFluidOutputs.length <= 0 || mTanks[1].canFillAll(tRecipe.mFluidOutputs[0]))
                            ...
                            // set remaining Fluid to null, in case the Fuel Type needs to be swapped out. But only if
                            // it was inactive for 64 ticks.
            ...
            if (mActivity.mState != 0 && WD.random(this, 20, CLIENT_TIME))
                UT.Sounds.play(SFX.MC_MINECART, 1, 0.5F, getCoords());
        }
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        if (aSide == mFacing) return BlockTextureMulti.get(
            BlockTextureDefault.get(sColoreds[0], mRGBa),
            BlockTextureDefault.get((mActivity.mState > 0 ? sOverlaysActive : sOverlays)[0]));
    ...
    public boolean getStateRunningPossible() {
        return mActivity.mActive || (mTanks[0].has() && !mTanks[1].isFull());
    }
