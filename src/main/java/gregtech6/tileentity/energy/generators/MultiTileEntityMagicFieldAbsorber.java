...
public class MultiTileEntityMagicFieldAbsorber extends TileEntityBase09FacingSingle
    implements ITileEntityEnergy, ITileEntityRunningActively, ITileEntitySwitchableOnOff {
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            LH.Chat.RED + LH.get(
                LH.ENERGY_OUTPUT) + ": " + LH.Chat.WHITE + mOutput + LH.Chat.RAINBOW + " ?U" + LH.Chat.WHITE + "/t");
        ...
        if (aIsServerSide && !mStopped) {
            if ((mCheck || mBlockUpdated || aTimer % 600 == 5)) {
                ...
                if (tBlock == Blocks.dragon_egg) {
                    mActive = T;
                    mOutput = 64;
                    mEnergyTypeEmitted = TD.Energy.QU;
                } else if (tBlock == Blocks.skull) {
                    mActive = T;
                    mOutput = 1;
                    mEnergyTypeEmitted = TD.Energy.TU; // I can't forsee this getting OP as heck. XD
                } else if (IL.TF_Trophy.equal(tBlock)) {
                    switch (tBlock.getDamageValue(worldObj, xCoord, yCoord + 1, zCoord)) {
                        case 1:
                            mActive = T;
                            mOutput = 64;
                            mEnergyTypeEmitted = TD.Energy.KU;
                            break; // Naga
                        case 2:
                            mActive = T;
                            mOutput = 64;
                        ...
                        default:
                            mActive = T;
                            mOutput = 64;
                            mEnergyTypeEmitted = TD.Energy.HU;
                            break; // Hydra
                        case 3:
                            mActive = T;
                            mOutput = 64;
                            mEnergyTypeEmitted = TD.Energy.LU;
                            break; // Ur-Ghast
                        case 4:
                            mActive = T;
                            mOutput = 64;
                            mEnergyTypeEmitted = TD.Energy.CU;
                            break; // Snow Queen
                    }
                }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        int aIndex = SIDES_TOP[aSide] ? 4
            : aSide == mFacing ? SIDES_HORIZONTAL[aSide] ? 0 : 2 : SIDES_HORIZONTAL[aSide] ? 1 : 3;
        return BlockTextureMulti.get(
            BlockTextureDefault.get(sColoreds[aIndex], mRGBa),
            BlockTextureDefault.get((mActive ? sOverlaysActive : sOverlays)[aIndex]));
    }
