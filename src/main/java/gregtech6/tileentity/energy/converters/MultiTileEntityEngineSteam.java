...
public class MultiTileEntityEngineSteam extends TileEntityBase09FacingSingle
    implements ITileEntityAdjacentOnOff, ITileEntityEnergyFluxHandler, ITileEntityRunningActively, IFluidHandler {

    /** The Array containing the different Engine State Colours from Blue over Green to Red */
    public static final int sEngineColors[] = { 0x0000ff, 0x0011ee, 0x0022dd, 0x0033cc, 0x0044bb, 0x0055aa, 0x006699,
        0x007788, 0x008877, 0x009966, 0x00aa55, 0x00bb44, 0x00cc33, 0x00dd22, 0x00ee11, 0x00ff00, 0x00ff00, 0x11ee00,
        0x22dd00, 0x33cc00, 0x44bb00, 0x55aa00, 0x669900, 0x778800, 0x887700, 0x996600, 0xaa5500, 0xbb4400, 0xcc3300,
        0xdd2200, 0xee1100, 0xff0000 };
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        mTank.readFromNBT(aNBT, NBT_TANK + "." + 0)
            .setCapacity(STEAM_PER_WATER * mOutput * 2);
    }
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        aList.add(
            Chat.CYAN + LH.get(LH.CONVERTS_FROM_X)
                ...
                + STEAM_PER_WATER
                + " L "
                + FL.name(FL.Steam.make(0), T)
                + " "
                + LH.get(LH.CONVERTS_TO_Y)
                + " "
                + (STEAM_PER_WATER / STEAM_PER_EU)
                + " "
                + mEnergyTypeEmitted.getLocalisedNameShort());
        aList.add(LH.getToolTipEfficiency(mEfficiency));
        aList.add(
            Chat.GREEN + LH.get(LH.ENERGY_INPUT)
                + ": "
                + Chat.WHITE
                + UT.Code.units(mOutput * STEAM_PER_EU, mEfficiency * 2, 10000, F)
                + " - "
                + UT.Code.units(mOutput * 2 * STEAM_PER_EU, mEfficiency, 10000, F)
                + " "
                + TD.Energy.STEAM.getLocalisedChatNameLong()
                + Chat.WHITE
                + "/t ("
                + LH.get(LH.FACE_BACK)
                + ")");
        aList.add(
            Chat.GREEN + LH.get(LH.ENERGY_CAPACITY)
                + ": "
                + Chat.WHITE
                + mTank.capacity()
                + " "
        ...
        aList.add(
            Chat.RED + LH.get(LH.ENERGY_OUTPUT)
                ...
                + (mOutput * 2)
                + " "
                + mEnergyTypeEmitted.getLocalisedChatNameShort()
                + Chat.WHITE
                + "/t ("
                + LH.get(LH.FACE_FRONT)
                + ")");
        aList.add(
            Chat.RED + LH.get(LH.ENERGY_CAPACITY)
                + ": "
                + Chat.WHITE
                + mCapacity
                + " "
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            // Emit Energy
            if (mActive) {
                mEmitsEnergy = (ITileEntityEnergy.Util
                    .emitEnergyToNetwork(mEnergyTypeEmitted, mPiston > 1 ? -tOutput : tOutput, 1, this) > 0);
                mEnergy -= tOutput;
                if (mTimer % 600 == 5) doDefaultStructuralChecks();
            }

            // Well the Engine stops if it has too much Steam and just vents everything, unless it freshly entered the
            // State.
    ...
    public byte getVisualData() {
        return (byte) ((mPiston & 3) | (mActive ? 4 : 0) | (mState << 3));
    }

    @Override
    public float getSurfaceSizeAttachable(byte aSide) {
        return ALONG_AXIS[aSide][mFacing] ? 0.5F : 0.25F;
    }
    ...
    public boolean isEnergyCapacitorType(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeEmitted;
    }
    ...
    public long getEnergyStored(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeEmitted ? mEnergy : 0;
    }

    @Override
    public long getEnergyCapacity(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeEmitted ? mCapacity : 0;
    }
    ...
    protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        return !mStopped && aSide == OPOS[mFacing] && FL.Steam.is(aFluidToFill) ? mTank : null;
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                return box(aBlock, PX_P[1], PX_P[1], PX_P[1], PX_N[1], PX_N[1], PX_N[1]);
            case 1:
                return box(
                    aBlock,
                    PX_P[0],
                    PX_P[0],
                    PX_P[0],
                    PX_N[SIDES_AXIS_X[mFacing] ? 14 : 0],
                    PX_N[SIDES_AXIS_Y[mFacing] ? 14 : 0],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 14 : 0]);
            case 2:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_X[mFacing] ? 14 : 0],
                    PX_P[SIDES_AXIS_Y[mFacing] ? 14 : 0],
                    PX_P[SIDES_AXIS_Z[mFacing] ? 14 : 0],
                    PX_N[0],
                    PX_N[0],
                    PX_N[0]);
            case 3:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_Y[mFacing] ? 0 : 6],
                    PX_P[SIDES_AXIS_Z[mFacing] ? 0 : 6],
                    PX_P[SIDES_AXIS_X[mFacing] ? 0 : 6],
                    PX_N[SIDES_AXIS_Y[mFacing] ? 0 : 6],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 0 : 6],
                    PX_N[SIDES_AXIS_X[mFacing] ? 0 : 6]);
            case 4:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_Z[mFacing] ? 0 : 6],
            ...
            case 5:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_X[mFacing] ? 0 : 4],
                    PX_P[SIDES_AXIS_Y[mFacing] ? 0 : 4],
                    PX_P[SIDES_AXIS_Z[mFacing] ? 0 : 4],
                    PX_N[SIDES_AXIS_X[mFacing] ? 0 : 4],
                    PX_N[SIDES_AXIS_Y[mFacing] ? 0 : 4],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 0 : 4]);
            case 6:
                return box(
                    aBlock,
                    PX_P[SIDES_AXIS_X[mFacing] ? 0 : 3],
                    PX_P[SIDES_AXIS_Y[mFacing] ? 0 : 3],
                    PX_P[SIDES_AXIS_Z[mFacing] ? 0 : 3],
                    PX_N[SIDES_AXIS_X[mFacing] ? 0 : 3],
                    PX_N[SIDES_AXIS_Y[mFacing] ? 0 : 3],
                    PX_N[SIDES_AXIS_Z[mFacing] ? 0 : 3]);
        }
        return F;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                return ALONG_AXIS[aSide][mFacing] ? null
                    : BlockTextureMulti
                        .get(BlockTextureDefault.get(sColoreds[3], mRGBa), BlockTextureDefault.get(sOverlays[3]));
            case 1:
            case 2:
                return BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[aSide == mFacing ? 0 : aSide == OPOS[mFacing] ? 1 : 2], mRGBa),
                    BlockTextureDefault.get(sOverlays[aSide == mFacing ? 0 : aSide == OPOS[mFacing] ? 1 : 2]));
            case 3:
                return BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[ALONG_AXIS_1[aSide][mFacing] ? 4 : 5], mRGBa),
                    BlockTextureDefault.get(sOverlays[ALONG_AXIS_1[aSide][mFacing] ? 4 : 5]));
            case 4:
                return BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[ALONG_AXIS_2[aSide][mFacing] ? 4 : 5], mRGBa),
                    BlockTextureDefault.get(sOverlays[ALONG_AXIS_2[aSide][mFacing] ? 4 : 5]));
            case 5:
                return aSide == OPOS[mFacing] ? null
                    : BlockTextureMulti.get(
                        BlockTextureDefault.get(sColoreds[6], sEngineColors[mState]),
                        BlockTextureDefault.get(sOverlays[6]));
            case 6:
                return ALONG_AXIS[aSide][mFacing] ? null
                    : BlockTextureMulti
                        .get(BlockTextureDefault.get(sColoreds[7], mRGBa), BlockTextureDefault.get(sOverlays[7]));
        }
        return null;
    }
     ...
     * if (tileentity != null) {
     * // render(tileentity.progress, tileentity.orientation, tileentity.getBaseTexture(),
     * tileentity.getChamberTexture(), tileentity.getTrunkTexture(tileentity.getEnergyStage()), x, y, z);
     * }
     * }
     * private void render(float progress, ForgeDirection orientation, ResourceLocation baseTexture, ResourceLocation
     * chamberTexture, ResourceLocation trunkTexture, double x, double y, double z) {
     * GL11.glPushMatrix();
