 ...
 */
public class MultiTileEntityBoilerTank extends TileEntityBase09FacingSingle
    implements ITileEntityEnergy, ITileEntityFunnelAccessible, ITileEntityGibbl, ITileEntityEnergyDataCapacitor,
    IFluidHandler, IMTE_RemovedByPlayer, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            // Emit Steam
            if (tAmount > 0) FL.move(
                mTanks[1],
                getAdjacentTank(SIDE_UP),
                Math.min(tAmount > mTanks[1].capacity() / 4 ? mOutput * 2 : mOutput, tAmount));
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? BlockTextureMulti.get(
            ...
            aSide != mFacing ? null : BlockTextureDefault.get(BI.BAROMETER),
            aSide != mFacing ? null : BlockTextureDefault.get(BI.BAROMETER_SCALE[mBarometer], CA_RED_64)) : null;
    }

    @Override
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mEnergy + mTanks[1].amount() / STEAM_PER_EU > 2000)
            UT.Entities.applyHeatDamage(aEntity, Math.min(10.0F, (mEnergy + mTanks[1].amount() / 2) / 2000.0F));
    }
    ...
    protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        return SIDES_BOTTOM_HORIZONTAL[aSide] && FL.water(aFluidToFill) ? mTanks[0] : null;
    }
