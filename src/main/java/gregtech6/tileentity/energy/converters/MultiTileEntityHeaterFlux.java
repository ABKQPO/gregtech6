...
public class MultiTileEntityHeaterFlux extends TileEntityBase10EnergyConverter
    implements ITileEntityEnergyFluxHandler, ITileEntityAdjacentOnOff, IMTE_GetCollisionBoundingBoxFromPool,
    IMTE_OnEntityCollidedWithBlock, ITileEntitySwitchableMode {

    @Override
    public void onEntityCollidedWithBlock(Entity aEntity) {
        if (mActivity.mState > 0)
            UT.Entities.applyHeatDamage(aEntity, Math.min(10.0F, mConverter.mEnergyOUT.mRec / 10.0F));
    }

    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(
            (mFacing == SIDE_X_NEG ? PX_P[2] : 0),
            (mFacing == SIDE_Y_NEG ? PX_P[2] : 0),
            (mFacing == SIDE_Z_NEG ? PX_P[2] : 0),
            (mFacing == SIDE_X_POS ? PX_N[2] : 1),
            (mFacing == SIDE_Y_POS ? PX_N[2] : 1),
            (mFacing == SIDE_Z_POS ? PX_N[2] : 1));
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return BlockTextureMulti.get(
            ...
            BlockTextureDefault.get((mActivity.mState > 0 ? sOverlaysActive : sOverlays)[aIndex]));
    }
    ...
    public String getTileEntityName() {
        return "gt.multitileentity.heater.heat_flux";
    }
}
