...
public class MultiTileEntityEngineFlux extends TileEntityBase09FacingSingle implements ITileEntityAdjacentOnOff,
    ITileEntityEnergyFluxHandler, ITileEntityRunningActively, ITileEntitySwitchableMode {
        ...
        if (TD.Energy.ALL_EU.contains(mEnergyTypeAccepted)) {
            if (TD.Energy.ALL_EU.contains(mEnergyTypeEmitted)) {
                ...
                if (mEnergyTypeEmitted == TD.Energy.RF)
                    aList.add(LH.getToolTipEfficiency(UT.Code.units(10000, mInput * 4, mOutput * 2, F)));
            }
        } else {
            if (TD.Energy.ALL_EU.contains(mEnergyTypeEmitted) && mEnergyTypeAccepted == TD.Energy.RF)
                aList.add(LH.getToolTipEfficiency(UT.Code.units(10000, mInput, mOutput * 8, F)));
        }
    }
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            long tOutput = (mOutput * (mState + 1)) / 16,
                tInput = ((mInput * (mState + 1)) / 16) + ((mInput * (mState + 1)) % 16 == 0 ? 0 : 1);
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null) {
                ...
                aChatReturn
                    .add("Input: " + (((mInput * (mState + 1)) / 16) + ((mInput * (mState + 1)) % 16 == 0 ? 0 : 1)));
                aChatReturn.add("Output: " + ((mOutput * (mState + 1)) / 16));
            }
            return 1;
        }
    ...
    public float getSurfaceSizeAttachable(byte aSide) {
        return ALONG_AXIS[aSide][mFacing] ? 0.5F : 1.0F;
    }
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        ...
        long tInput = Math.min(tInputMax - mEnergy, aSize * aAmount),
    ...
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mStopped) && aSide == OPOS[mFacing]
    ...
    public Collection<TagData> getEnergyTypes(byte aSide) {
        return new ArrayListNoNulls<>(F, mEnergyTypeEmitted, mEnergyTypeAccepted);
    }
            ...
            case 3:
                return aSide == OPOS[mFacing] ? null
                    : BlockTextureMulti.get(
                        BlockTextureDefault.get(sColoreds[4], sEngineColors[mState]),
                        BlockTextureDefault.get(sOverlays[4]));
            case 4:
                return ALONG_AXIS[aSide][mFacing] ? null
                    : BlockTextureMulti
    ...
    public String getTileEntityName() {
        return "gt.multitileentity.engine.kinetic_flux";
    }
}
