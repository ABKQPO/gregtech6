...
public class MultiTileEntityLaserBuildcraft extends TileEntityBase10EnergyConverter
    implements ITileEntityAdjacentOnOff, ITileEntityEnergyElectricityAcceptor, IMTE_SetBlockBoundsBasedOnState,
    ...
    public void doConversion(long aTimer) {
        if (EnergyCompat.BC_LASER) {
            ...
            mActivity.mActive = (tOutput >= mConverter.mEnergyOUT.mMin && tLaser.mTileEntity instanceof ILaserTarget
                && ((ILaserTarget) tLaser.mTileEntity).requiresLaserEnergy());

            if (mActivity.mActive) {
                if (tOutput > mConverter.mEnergyOUT.mMax && mConverter.mLimitConsumption)
                    tOutput = mConverter.mEnergyOUT.mMax;
    ...
    public float getSurfaceSize(byte aSide) {
        return aSide == OPOS[mFacing] ? 1.0F : 0.0F;
    }
    ...
    public float getSurfaceDistance(byte aSide) {
        return aSide == mFacing ? 0.5F : 0.0F;
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(
            PX_P[mFacing == SIDE_X_NEG ? 8 : 0],
            PX_P[mFacing == SIDE_Y_NEG ? 8 : 0],
            PX_P[mFacing == SIDE_Z_NEG ? 8 : 0],
            PX_N[mFacing == SIDE_X_POS ? 8 : 0],
            PX_N[mFacing == SIDE_Y_POS ? 8 : 0],
            PX_N[mFacing == SIDE_Z_POS ? 8 : 0]);
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                ...
                return T;
            case 1:
                box(aBlock, PX_P[4], PX_P[4], PX_P[4], PX_N[4], PX_N[4], PX_N[4]);
                return T;
            case 2:
                box(
                    aBlock,
                    mFacing == SIDE_X_NEG ? -0.99F : PX_P[5],
                    mFacing == SIDE_Y_NEG ? -0.99F : PX_P[5],
                    mFacing == SIDE_Z_NEG ? -0.99F : PX_P[5],
                    mFacing == SIDE_X_POS ? +1.99F : PX_N[5],
                    mFacing == SIDE_Y_POS ? +1.99F : PX_N[5],
                    mFacing == SIDE_Z_POS ? +1.99F : PX_N[5]);
                return T;
            case 3:
                box(
                    aBlock,
                    mFacing == SIDE_X_NEG ? -0.99F : PX_P[6],
                    mFacing == SIDE_Y_NEG ? -0.99F : PX_P[6],
                    mFacing == SIDE_Z_NEG ? -0.99F : PX_P[6],
                    mFacing == SIDE_X_POS ? +1.99F : PX_N[6],
                    mFacing == SIDE_Y_POS ? +1.99F : PX_N[6],
                    mFacing == SIDE_Z_POS ? +1.99F : PX_N[6]);
                return T;
            case 4:
                box(
                    aBlock,
                    mFacing == SIDE_X_NEG ? -0.99F : PX_P[7],
                    mFacing == SIDE_Y_NEG ? -0.99F : PX_P[7],
                    mFacing == SIDE_Z_NEG ? -0.99F : PX_P[7],
                    mFacing == SIDE_X_POS ? +1.99F : PX_N[7],
                    mFacing == SIDE_Y_POS ? +1.99F : PX_N[7],
                    mFacing == SIDE_Z_POS ? +1.99F : PX_N[7]);
                return T;
            // case 3: box(aBlock, mFacing==SIDE_X_POS?+1.99F:PIXELS_NEG[6], mFacing==SIDE_Y_POS?+1.99F:PIXELS_NEG[6],
            // mFacing==SIDE_Z_POS?+1.99F:PIXELS_NEG[6], mFacing==SIDE_X_NEG?-0.99F:PIXELS_POS[6],
            // mFacing==SIDE_Y_NEG?-0.99F:PIXELS_POS[6], mFacing==SIDE_Z_NEG?-0.99F:PIXELS_POS[6]); return T;
        }
        return F;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                int tIndex = aSide == mFacing ? 0 : aSide == OPOS[mFacing] ? 1 : 2;
                return BlockTextureMulti
                    .get(BlockTextureDefault.get(sColoreds[tIndex], mRGBa), BlockTextureDefault.get(sOverlays[tIndex]));
            case 1:
                if (aSide != OPOS[mFacing]) return BlockTextureMulti.get(
                    BlockTextureDefault.get(sColoreds[aSide == mFacing ? 3 : 4], MT.Diamond.fRGBaSolid),
                    BlockTextureDefault.get(sOverlays[aSide == mFacing ? 3 : 4]));
            default:
