 ...
 */
public class MultiTileEntityPump extends TileEntityBase09FacingSingle
    implements IFluidHandler, ITileEntityEnergy, ITileEntityRunningActively, ITileEntitySwitchableOnOff {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            if (mTank.has()) for (byte tSide : ALL_SIDES_VALID_BUT_AXIS[mFacing])
                if (mTank.has()) FL.move(mTank, getAdjacentTileEntity(tSide));
                else break;
            ...
            if (mTimer % 600 == 5) if (mActiveData != 0) doDefaultStructuralChecks();
    ...
    private boolean drainFluid(ChunkCoordinates aCoords) {
        ...
        // Somehow this Block is completely surrounded by pumpable Fluid, this should not be possible unless it is the
        // literal Cornercase! Scan again!
        return !(mPumpedFluids.contains(getBlock(aCoords.posX + 1, aCoords.posY, aCoords.posZ))
    ...
    public boolean onTickCheck(long aTimer) {
        ...
        if (mActiveData == 0 || mStopped) mActiveState = 0;
        else if (mActiveData == ~0L) mActiveState = 1;
        else mActiveState = 2;
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        ...
        long tInput = Math.min(8192 - mEnergy, aSize * aAmount),
    ...
    public boolean isEnergyType(TagData aEnergyType, byte aSide, boolean aEmitting) {
        return !aEmitting && aEnergyType == mEnergyType;
    }

    @Override
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return (aTheoretical || !mStopped) && (SIDES_INVALID[aSide] || aSide == OPOS[mFacing])
    ...
    public boolean getStateRunningPossible() {
        return (mActiveData != 0 || !mTank.has()) && (!mPumpList.isEmpty() || WD.liquid(getBlockAtSide(mFacing)));
    }

    @Override
    public boolean getStateRunningPassively() {
        return mActiveData != 0;
    }

    @Override
    public boolean getStateRunningActively() {
        return mActiveData != 0 && !mPumpedFluids.isEmpty();
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return BlockTextureMulti.get(
            ...
            BlockTextureDefault.get((mActiveState > 0 ? sOverlaysActive : sOverlays)[aIndex]));
    }
    ...
    public static IIconContainer sColoreds[] = new IIconContainer[] {
        ...
        sOverlays[] = new IIconContainer[] { new Textures.BlockIcons.CustomIcon("machines/pump/rotation/overlay/front"),
