...
public class MultiTileEntityLongDistancePipelineFluid extends TileEntityBase09FacingSingle
    implements IFluidHandler, ITileEntityCanDelegate, IMTE_HasMultiBlockMachineRelevantData,
    ITileEntityMachineBlockUpdateable, ITileEntitySwitchableOnOff {
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        if (aNBT.hasKey(NBT_TEMPERATURE)) {
            mTemperature = aNBT.getLong(NBT_TEMPERATURE);
        }
                ...
                for (ChunkCoordinates aCoords : tToCheck) {
                    if (getBlock(aCoords) == aBlock && getMetaData(aCoords) == aMetaData) {
                        ...
                        if (tTileEntity != this && tTileEntity instanceof MultiTileEntityLongDistancePipelineFluid) {
                            if (tWires.contains(
                                ((MultiTileEntityLongDistancePipelineFluid) tTileEntity)
                                    .getOffset(((MultiTileEntityLongDistancePipelineFluid) tTileEntity).mFacing, 1))) {
                                mTarget = (MultiTileEntityLongDistancePipelineFluid) tTileEntity;
