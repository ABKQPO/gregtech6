 ...
 */
public class MultiTileEntitySandwich extends TileEntityBase03MultiTileEntities implements IMTE_SyncDataByteArray,
    IMTE_IsProvidingWeakPower, IMTE_IsProvidingStrongPower, IMTE_GetComparatorInputOverride, IMTE_GetBlockHardness,
    IMTE_IsSideSolid, IMTE_GetLightOpacity, IMTE_GetExplosionResistance, ITileEntityQuickObstructionCheck,
    IMTE_GetCollisionBoundingBoxFromPool, IMTE_GetSelectedBoundingBoxFromPool, IMTE_SetBlockBoundsBasedOnState,
    IMTE_GetMaxStackSize, IMTE_OnlyPlaceableWhenSneaking, IMTE_CanPlace, IMTE_OnItemRightClick, IMTE_AddToolTips,
    IMTE_GetFoodValues, IMTE_OnEaten, IMTE_GetItemUseAction, IMTE_GetMaxItemUseDuration, IItemRottable {

    public ItemStack[] mStacks = new ItemStack[16];
    public byte[] mDisplay = { (byte) 254, (byte) 255, (byte) 255, (byte) 255, (byte) 255, (byte) 255, (byte) 255,
    ...
    public ArrayListNoNulls<ItemStack> getDrops(int aFortune, boolean aSilkTouch) {
        ...
        if (rList.isEmpty()) {
            ...
            if (tRegistry != null)
                rList.add(tRegistry.getItem(getMultiTileEntityID(), mStacks[0].stackSize, writeItemNBT(UT.NBT.make())));
        }
        return rList;
    }
    ...
    public int addIngredient(ItemStack aStack) {
        ...
        if (ST.valid(mStacks[mSize]) || mSize == 0) {
            ERR.println(
                "ERROR: Attempted to add Sandwich Ingredient to already occupied Slot: " + mSize
                    + " ; Clientside: "
                    + worldObj.isRemote);
            for (byte i = 0; i < mStacks.length; i++) if (ST.valid(mStacks[i])) {
                ERR.println(
                    i + ": " + ST.regName(mStacks[i]) + ":" + ST.meta(mStacks[i]) + " ; Display: " + mDisplay[i]);
            } else {
        ...
        if (tID == null) tID = Sandwiches.INGREDIENTS.get(aStack, W);
        // Check if the ingredient exists and fits onto the Sandwich.
        if (tID == null || mSize + Math.max(1, Sandwiches.INGREDIENT_MODEL_THICKNESS[UT.Code.unsignB(tID)]) > 16)
            return 0;
        // Add the Ingredient.
    ...
    public float getTotalSaturation() {
        float rSaturation = 0;
        for (ItemStack aStack : mStacks)
            if (ST.valid(aStack)) rSaturation = Math.max(rSaturation, ST.saturation(aStack));
    ...
    public void updateSandwich() {
        if (worldObj == null || isServerSide()) {
            for (byte i = 0; i < mStacks.length; i++) {
                ...
                if (tID == null) tID = Sandwiches.INGREDIENTS.get(mStacks[i], W);
    ...
    public void updateSandwichSize() {
        for (byte i = 0; i < mDisplay.length; i++) if (mDisplay[i] != (byte) 255) {
            mSize = (byte) UT.Code
                .bind(1, 16, i + Math.max(1, Sandwiches.INGREDIENT_MODEL_THICKNESS[UT.Code.unsignB(mDisplay[i])]));
        }
    }

    @Override
    public int getComparatorInputOverride(byte aSide) {
        return UT.Code.bind4(mSize - 1);
    }

    @Override
    public int isProvidingStrongPower(byte aOppositeSide) {
        return mRedstone ? UT.Code.bind4(mSize - 1) : 0;
    }
    ...
    public boolean canPlace(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte aSide,
        ...
        return WD.block(aWorld, aX, aY - 1, aZ)
            .isSideSolid(aWorld, aX, aY - 1, aZ, FORGE_DIR[SIDE_TOP]);
    }
    ...
    public ItemStack onEaten(MultiTileEntityItemInternal aItem, ItemStack aStack, World aWorld, EntityPlayer aPlayer) {
        if (MD.APC.mLoaded) {
            ...
            aPlayer.getFoodStats()
                .addStats(getTotalFood(), getTotalSaturation());
        }
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return (SIDES_VERTICAL[aSide] ? Sandwiches.INGREDIENT_TEXTURES_TOP
            : Sandwiches.INGREDIENT_TEXTURES_SIDES)[UT.Code.unsignB(mDisplay[aRenderPass / 4])];
    }
    ...
    public boolean setBlockBounds(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        short tID = UT.Code.unsignB(mDisplay[aRenderPass / 4]),
            tModel = UT.Code.unsignB(Sandwiches.INGREDIENT_MODEL_IDS[tID]),
            tThickness = Sandwiches.INGREDIENT_MODEL_THICKNESS[tID];
            ...
            switch (UT.Code.unsignB(Sandwiches.INGREDIENT_MODEL_IDS[UT.Code.unsignB(mDisplay[aRenderPass - 1])])) {
                ...
                case 252:
                    return box(
                        ...
                        PX_P[2] - tOffset,
                        PX_N[2] + tOffset,
                        PX_P[aRenderPass + tThickness],
                        PX_N[2] + tOffset);
                default:
                    return box(
                        aBlock,
                        PX_P[1] - tOffset,
                        PX_P[aRenderPass] - PX_P[1] / 2,
                        PX_P[1] - tOffset,
                        PX_N[1] + tOffset,
                        PX_P[aRenderPass + tThickness],
                        PX_N[1] + tOffset);
    ...
    public void setBlockBoundsBasedOnState(Block aBlock) {
        box(aBlock, PX_P[1], 0, PX_P[1], PX_N[1], PX_P[Math.max(1, mSize)], PX_N[1]);
    }

    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool() {
        return box(PX_P[1], 0, PX_P[1], PX_N[1], PX_P[Math.max(1, mSize)], PX_N[1]);
    }
