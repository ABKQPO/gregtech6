...
public class MultiTileEntityAutoToolHammer extends TileEntityBase09FacingSingle implements ITileEntityEnergy {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        if (aIsServerSide) {
            if (mPullingBack && mEnergy > 0) {
                ...
                if (IBlockToolable.Util.onToolClick(
                    TOOL_hammer,
                    mEnergy * 10,
                    mQuality,
                    null,
                    ...
                    if (!(tD.mTileEntity instanceof ITileEntityAnvil)
                        || !((ITileEntityAnvil) tD.mTileEntity).isAnvil(tD.mSideOfTileEntity)) {
                        ...
                        if (GT_Tool_HardHammer.INSTANCE.isMinableBlock(tBlock, tMetaData)) {
                            ...
                            if (tBlock.getHarvestLevel(tMetaData) <= mQuality) {
                                ...
                                if (tHardness >= 0 && tHardness * 50 <= mEnergy) {
                                    List<ItemStack> tDrops = tBlock
                                        .getDrops(tD.mWorld, tD.mX, tD.mY, tD.mZ, tMetaData, 0);
                                    GT_Tool_HardHammer.INSTANCE.convertBlockDrops(
                                        tDrops,
                                        null,
                                        null,
                                        tBlock,
                                    ...
                                    for (ItemStack tStack : tDrops)
                                        ST.drop(tD.mWorld, tD.mX + 0.5, tD.mY + 0.5, tD.mZ + 0.5, tStack);
                                    mSendSound = 1;
                                }
                            }
                        }
            ...
            if (mSendSound != 0) {
                UT.Sounds
                    .play(mSendSound == 1 ? SFX.MC_ANVIL_USE : SFX.MC_ANVIL_BREAK, 20, 1.0F, xCoord, yCoord, zCoord);
                mSendSound = 0;
            }
        }
    }
    ...
    public boolean isEnergyAcceptingFrom(TagData aEnergyType, byte aSide, boolean aTheoretical) {
        return aSide == OPOS[mFacing] && super.isEnergyAcceptingFrom(aEnergyType, aSide, aTheoretical);
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return BlockTextureMulti
            .get(BlockTextureDefault.get(sColoreds[aIndex], mRGBa), BlockTextureDefault.get(sOverlays[aIndex]));
    }
