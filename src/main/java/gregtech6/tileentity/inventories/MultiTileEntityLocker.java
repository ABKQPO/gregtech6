...
public class MultiTileEntityLocker extends TileEntityBase09FacingSingle implements ITileEntityRunningSuccessfully {
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (UT.Entities.isPlayer(aPlayer) && isServerSide()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (tStack == null || tStack.getItem()
                    .isValidArmor(tStack, 3 - i, aPlayer)) {
                    if (!IL.BTRS_Backpack.equal(aPlayer.inventory.armorInventory[i], T, T)
                        && !IL.BTRS_Thaumpack.equal(aPlayer.inventory.armorInventory[i], T, T)) {
    ...
    public boolean canInsertItem2(int aSlot, ItemStack aStack, byte aSide) {
        if (ST.valid(aStack)) try {
            return aStack.getItem()
                .isValidArmor(aStack, 3 - aSlot, null);
        } catch (Throwable e) {
