...
public class MultiTileEntityStorageInserter extends TileEntityBase07Paintable implements ITileEntityConnectedInventory {
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        boolean tDirectionsToGo[] = new boolean[] { T, T, T, T },
            tOnlyHand = (aPlayer.inventory.getCurrentItem() != null);
    ...
    public boolean checkColumn(EntityPlayer aPlayer, int aX, int aY, int aZ,
        ArrayListNoNulls<MultiTileEntityMassStorage> aList, boolean aOnlyHand) {
    ...
    public void tryInsert(EntityPlayer aPlayer, MultiTileEntityMassStorage aStorage, boolean aOnlyHand) {
        if (aPlayer.inventory.getCurrentItem() != null)
            aPlayer.inventory.mainInventory[aPlayer.inventory.currentItem] = aStorage
                .insertItems(aPlayer.inventory.mainInventory[aPlayer.inventory.currentItem], T);
        if (!aOnlyHand) for (int i = 9; i < aPlayer.inventory.mainInventory.length; i++) {
            if (aPlayer.inventory.mainInventory[i] != null && !ST.nonautoinsert(aPlayer.inventory.mainInventory[i])
                && (aStorage.slotHas(1) || aPlayer.inventory.mainInventory[i].getMaxStackSize() > 1)) {
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? BlockTextureMulti.get(
            BlockTextureDefault.get(sColored, mRGBa, mMaterial.contains(TD.Properties.GLOWING)),
            BlockTextureDefault.get(sOverlay)) : null;
    }
    ...
    // Icons
    public static IIconContainer sColored = new Textures.BlockIcons.CustomIcon(
        "machines/massstorage/inserter/colored/sides"),
