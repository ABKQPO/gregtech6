 ...
 */
public class MultiTileEntityHopper extends TileEntityBase09FacingSingle
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (mMode > 0) aList.add(Chat.CYAN + LH.get("gt.multitileentity.hopper.tooltip.2") + mMode);
        if (mExactMode) aList.add(Chat.CYAN + LH.get("gt.multitileentity.hopper.tooltip.3"));
    ...
    public boolean onPlaced(ItemStack aStack, EntityPlayer aPlayer, MultiTileEntityContainer aMTEContainer,
        ...
        if (isServerSide() && SIDES_BOTTOM_HORIZONTAL[mFacing]) {
            ...
            if (tDelegator.mTileEntity instanceof ITileEntityConnector && SIDES_VALID[tDelegator.mSideOfTileEntity]
                && ((ITileEntityConnector) tDelegator.mTileEntity).allowInteraction(aPlayer)
                && UT.Code.haveOneCommonElement(
                    ((ITileEntityConnector) tDelegator.mTileEntity).getConnectorTypes(tDelegator.mSideOfTileEntity),
                    TD.Connectors.ALL_ITEM_TRANSPORT)) {
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_screwdriver)) {
            ...
            if (aChatReturn != null) aChatReturn.add(
                mMode <= 0 ? (mExactMode ? "Emits up to 1 Stack" : "Emits up to 64 Items")
    ...
    public void onWalkOver2(EntityLivingBase aEntity) {
        if (isServerSide() && (aEntity.getClass() == EntitySnowman.class
            || "EntityNewSnowGolem".equalsIgnoreCase(UT.Reflection.getLowercaseClass(aEntity)))) {
            int i = invsize();
            while (--i >= 0) if (addStackToSlot(i, ST.make(Items.snowball, 1, 0))) break;
        }
    }
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (mCheck > 0) {
                mCheck--;
            } else if ((mCheck == 0 || mInventoryChanged || mBlockUpdated || SYNC_SECOND)
                && !hasRedstoneIncomingFromNonRail()) {
                    if (!SIDES_TOP[mFacing] && !invempty()) {
                        ...
                        if (tDelegator.getBlock() instanceof BlockRailBase) {
                            List tList = worldObj.getEntitiesWithinAABBExcludingEntity(
                                null,
                                tDelegator.box(0, 0, 0, 1, 1, 1),
                                IEntitySelector.selectInventories);
                            if (tList != null && !tList.isEmpty())
                                tDelegator = new DelegatorTileEntity<>((IInventory) tList.get(0), tDelegator);
                        }
                    ...
                    if (mInventoryChanged) {
                        for (int i = 0, k = invsize(), l = getInventoryStackLimit(); i < k; i++)
                            for (int j = i + 1; j < k; j++) if (slotHas(j)) {
                    ...
                    if (tMovedItems > 0) {
                        for (byte tSide : ALL_SIDES_BUT_TOP) if (tSide != mFacing) {
                            ...
                            if (tDelegatorUpdate.mTileEntity instanceof ITileEntityAdjacentInventoryUpdatable) {
                                ((ITileEntityAdjacentInventoryUpdatable) tDelegatorUpdate.mTileEntity)
                                    .adjacentInventoryUpdated(tDelegatorUpdate.mSideOfTileEntity, this);
                            }
                        }
                    }
                }
    ...
    public float getSurfaceSize(byte aSide) {
        return SIDES_TOP[aSide] ? PX_N[0] : PX_N[8];
    }

    @Override
    public float getSurfaceSizeAttachable(byte aSide) {
        return SIDES_TOP[aSide] ? PX_N[2] : PX_N[8];
    }
    ...
    public void adjacentInventoryUpdated(byte aSide, IInventory aTileEntity) {
        if (SIDES_TOP[aSide] || aSide == mFacing) if (mCheck < 0) mCheck = 0;
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
                ...
                return T;
            case 1:
                box(aBlock, PX_P[4], PX_P[4], PX_P[4], PX_N[4], PX_N[6], PX_N[4]);
                return T;
            case 2:
                switch (mFacing) {
                    case SIDE_Y_NEG:
                        box(aBlock, PX_P[6], PX_P[0], PX_P[6], PX_N[6], PX_N[12], PX_N[6]);
                        return T;
                    case SIDE_Z_NEG:
                        box(aBlock, PX_P[6], PX_P[4], PX_P[0], PX_N[6], PX_N[8], PX_N[12]);
                        return T;
                    case SIDE_Z_POS:
                        box(aBlock, PX_P[6], PX_P[4], PX_P[12], PX_N[6], PX_N[8], PX_N[0]);
                        return T;
                    case SIDE_X_NEG:
                        box(aBlock, PX_P[0], PX_P[4], PX_P[6], PX_N[12], PX_N[8], PX_N[6]);
                        return T;
                    case SIDE_X_POS:
                        box(aBlock, PX_P[12], PX_P[4], PX_P[6], PX_N[0], PX_N[8], PX_N[6]);
                        return T;
                }
        }
        return T;
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return (aRenderPass == 1 ? !SIDES_TOP[aSide]
            : aShouldSideBeRendered[aSide] || (aRenderPass == 0 && SIDES_BOTTOM[aSide])
                || (aRenderPass == 2 && aSide != mFacing))
                    ? BlockTextureMulti.get(
                        BlockTextureDefault
                            .get(sColoreds[FACES_TBS[aSide]], mRGBa, mMaterial.contains(TD.Properties.GLOWING)),
