 ...
 */
public class MultiTileEntityLockerCharging extends MultiTileEntityLocker
    implements ITileEntityEnergyElectricityAcceptor, ITileEntityEnergyFluxHandler {

    @Override
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        long rReturn = 0;
        for (int i = 0; i < 4; i++) if (slotHas(i)) rReturn += IItemEnergy.Utility.inject(
            aEnergyType,
            slot(i),
            aSize,
            aAmount - rReturn,
    ...
    public boolean getStateRunningActively() {
        for (int i = 0; i < 4; i++) if (!IItemEnergy.Utility.full(slot(i), T)) return F;
        return UT.Code.containsSomething(getInventory());
    }
