 ...
 */
public class MultiTileEntityEnderGarbageDump extends TileEntityBase07Paintable
    implements IFluidHandler, ITileEntityTapAccessible, IMTE_AddToolTips {

    static {
        LH.add(
            "gt.multitileentity.ender.garbage.dump.tooltip.1",
            "This is where all the Trash Bin Items & Fluids go to");
        LH.add("gt.multitileentity.ender.garbage.dump.tooltip.2", "Accesses the Garbage Dimension to retrieve Trash.");
        LH.add(
            "gt.multitileentity.ender.garbage.dump.tooltip.3",
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (!(tDelegate.mTileEntity instanceof MultiTileEntityEnderGarbageBin)) {
                ...
                if (!GarbageGT.GARBAGE_FLUIDS.isEmpty())
                    if (tDelegate.mTileEntity instanceof IFluidHandler) FL.move_(GarbageGT.GARBAGE_FLUIDS, tDelegate);
            }
        }
    }
    ...
    protected IFluidTank getFluidTankDrainable2(byte aSide, FluidStack aFluidToDrain) {
        if (aFluidToDrain == null) {
            for (int i = 0; i < GarbageGT.GARBAGE_FLUIDS.size(); i++) if (GarbageGT.GARBAGE_FLUIDS.get(i)
                .has()) return GarbageGT.GARBAGE_FLUIDS.get(i);
        } else {
            for (int i = 0; i < GarbageGT.GARBAGE_FLUIDS.size(); i++) if (GarbageGT.GARBAGE_FLUIDS.get(i)
                .contains(aFluidToDrain)) return GarbageGT.GARBAGE_FLUIDS.get(i);
        }
        return null;
    }
    ...
    protected IFluidTank[] getFluidTanks2(byte aSide) {
        return GarbageGT.GARBAGE_FLUIDS.isEmpty() ? new FluidTankGT().AS_ARRAY
            : GarbageGT.GARBAGE_FLUIDS.toArray(ZL_FT);
    }
    ...
    public boolean canExtractItem2(int aSlot, ItemStack aStack, byte aSide) {
        return GarbageGT.GARBAGE_ITEMS.get(aSlot).stackSize > 0;
    }

    @Override
    public FluidStack tapDrain(byte aSide, int aMaxDrain, boolean aDoDrain) {
        for (FluidTankGT tTank : GarbageGT.GARBAGE_FLUIDS)
            if (tTank.has() && !FL.gas(tTank)) return tTank.drain(aMaxDrain, aDoDrain);
