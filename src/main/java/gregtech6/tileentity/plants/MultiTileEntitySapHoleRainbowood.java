...
public class MultiTileEntitySapHoleRainbowood extends MultiTileEntityTreeHole {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            if (!mHasResin && aTimer % 600 == 0) {
                ...
                for (int i = 1; i < 12; i++) {
                    if (getBlockAtSideAndDistance(SIDE_TOP, i) != BlocksGT.LogB
                        && getMetaDataAtSideAndDistance(SIDE_TOP, i) != 3) break;
                    tTreeHeight++;
                }
    ...
    private boolean checkLeaves(int aX, int aY, int aZ) {
        return getBlock(aX, aY, aZ) == BlocksGT.Leaves_AB && getMetaData(aX, aY, aZ) == 15;
    }

    @Override
    public ArrayListNoNulls<ItemStack> getDrops(int aFortune, boolean aSilkTouch) {
        return isClientSide() ? super.getDrops(aFortune, aSilkTouch) : ST.arraylist(ST.make(BlocksGT.LogB, 1, 3));
    }
    ...
    public FluidStack getResinFluid(byte aSide) {
        return FL.Sap_Rainbow.make(250);
    }

    @Override
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide]
            ? aSide != mFacing ? BlockTextureCopied.get(BlocksGT.LogB, SIDE_ANY, 3)
                : BlockTextureDefault
                    .get(mHasResin ? Textures.BlockIcons.LOG_SAP_RAINBOWOOD : Textures.BlockIcons.LOG_HOLE_RAINBOWOOD)
            : null;
    }

    @Override
    public float getExplosionResistance2() {
        return BlocksGT.LogB.getExplosionResistance((byte) 3);
    }
