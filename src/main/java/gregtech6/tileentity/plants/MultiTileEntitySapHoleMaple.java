...
public class MultiTileEntitySapHoleMaple extends MultiTileEntityTreeHole {
            ...
            if (!mHasResin && aTimer % 600 == 0) {
                ...
                for (int i = 1; i < 12; i++) {
                    if (getBlockAtSideAndDistance(SIDE_TOP, i) != BlocksGT.LogA
                        && getMetaDataAtSideAndDistance(SIDE_TOP, i) != 1) break;
                    tTreeHeight++;
                }
    ...
    private boolean checkLeaves(int aX, int aY, int aZ) {
        return getBlock(aX, aY, aZ) == BlocksGT.Leaves_AB && getMetaData(aX, aY, aZ) == 9;
    }

    @Override
    public ArrayListNoNulls<ItemStack> getDrops(int aFortune, boolean aSilkTouch) {
        return isClientSide() ? super.getDrops(aFortune, aSilkTouch) : ST.arraylist(ST.make(BlocksGT.LogA, 1, 1));
    }
    ...
    public FluidStack getResinFluid(byte aSide) {
        return FL.Sap_Maple.make(250);
    }

    @Override
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? aSide != mFacing ? BlockTextureCopied.get(BlocksGT.LogA, SIDE_ANY, 1)
            : BlockTextureDefault
                .get(mHasResin ? Textures.BlockIcons.LOG_SAP_MAPLE : Textures.BlockIcons.LOG_HOLE_MAPLE)
            : null;
    }

    @Override
    public float getExplosionResistance2() {
        return BlocksGT.LogA.getExplosionResistance((byte) 1);
    }
