 ...
 */
public class MultiTileEntityBush extends TileEntityBase09FacingSingle
    implements IMTE_CanPlaceSnowLayerOnRemoval, ITileEntityQuickObstructionCheck, IMTE_OnOxygenRemoved, IMTE_CanPlace,
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        aList.add(
            LH.Chat.CYAN + (ST.valid(mBerry) ? LH.get(mBerry.getUnlocalizedName(), mBerry.getDisplayName())
                : "Rightclick with a Berry to set Output"));
    }
        ...
        if (aIsServerSide) {
            if (mBlockUpdated || SERVER_TIME % 128 == 64) {
                ...
                if (SIDES_VALID[mFacing]) {
                    ...
                    mSpeed = (byte) (IL.AETHER_Grass_Enchanted.equal(tBlock)
                        || IL.AETHER_Grass_Enchanted_Vanilla.equal(tBlock)
                            ? 2
                            : BlocksGT.plantableGreens.contains(tBlock) || tBlock.canSustainPlant(
                                worldObj,
                                xCoord,
                                yCoord - 1,
                                zCoord,
                                FORGE_DIR[SIDE_UP],
                                Blocks.yellow_flower) ? 1 : 0);
                }
            }
            ...
            if (mStage < 3 && mSpeed > 0 && SERVER_TIME % 128 == 0 && ST.valid(mBerry)) {
                ...
                if (getSkyOffset(0, 1, 0)) {
                    ...
                    if (worldObj.isRaining() && getRainOffset(OFFX[mFacing], SIDES_TOP[mFacing] ? 1 : 2, OFFZ[mFacing]))
                        for (int i = 0; i < mSpeed; i++) if (++mGrowth == 0) mStage++;
                } else {
                    if (getLightLevelOffset(OFFX[mFacing], SIDES_TOP[mFacing] ? 1 : 2, OFFZ[mFacing]) > 9)
                        for (int i = 0; i < mSpeed; i++) if (++mGrowth == 0) mStage++;
                }
            }
        }
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_magnifyingglass)) {
            if (aChatReturn != null && ST.valid(mBerry))
                aChatReturn.add("Grows " + (CODE_CLIENT ? mBerry.getDisplayName() : LH.get(mBerry.getDisplayName())));
            return 1;
        }
    ...
    @Override
    public boolean canPlace(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, byte aSide,
        float aHitX, float aHitY, float aHitZ) {
        TileEntity tTileEntity = aWorld.getTileEntity(aX - OFFX[aSide], aY - OFFY[aSide], aZ - OFFZ[aSide]);
        if (tTileEntity instanceof MultiTileEntityBush && SIDES_INVALID[((MultiTileEntityBush) tTileEntity).mFacing]
            && (ST.invalid(mBerry) || ST.equal(((MultiTileEntityBush) tTileEntity).mBerry, mBerry, F))) {
            mFacing = OPOS[aSide];
            mBerry = ((MultiTileEntityBush) tTileEntity).mBerry;
            updateClientData();
            return T;
        ...
        if (BlocksGT.plantableGreens.contains(tBlock)
            || tBlock.canSustainPlant(aWorld, aX, aY - 1, aZ, FORGE_DIR[SIDE_UP], Blocks.yellow_flower)) {
    ...
    public static final IIconContainer sTextureBush = new Textures.BlockIcons.CustomIcon(
        "machines/plants/bush/colored/bush"),
        ...
        if (ST.valid(mBerry)) {
            ...
            if (tData != null && tData.mPrefix == OP.plantGtBerry) {
                if (aShouldSideBeRendered == SIDES_ITEM_RENDER) {
                    mTexture = BlockTextureMulti.get(
                        BlockTextureDefault.get(sTextureBush, 0x009000),
                        BlockTextureDefault.get(sOverlayBush),
                        BlockTextureDefault.get(sTextureBerry, tData.mMaterial.mMaterial.fRGBaSolid),
                        BlockTextureDefault.get(sOverlayBerry));
                } else {
                    switch (mStage) {
                        case 0:
                            mTexture = BlockTextureMulti.get(
                        ...
                        case 1:
                            mTexture = BlockTextureMulti.get(
                                BlockTextureDefault.get(sTextureBush, 0x009000),
                                BlockTextureDefault.get(sOverlayBush),
                                BlockTextureDefault.get(sTextureImmature, 0xff9090),
                                BlockTextureDefault.get(sOverlayImmature));
                            break;
                        case 2:
                            mTexture = BlockTextureMulti.get(
                                BlockTextureDefault.get(sTextureBush, 0x009000),
                                BlockTextureDefault.get(sOverlayBush),
                                BlockTextureDefault.get(sTextureBerry, 0x80ff80),
                                BlockTextureDefault.get(sOverlayBerry));
                            break;
                        case 3:
                            mTexture = BlockTextureMulti.get(
                ...
                if (aShouldSideBeRendered == SIDES_ITEM_RENDER) {
                    mTexture = BlockTextureMulti.get(
                        BlockTextureDefault.get(sTextureBush, tBerryColor[0]),
                        BlockTextureDefault.get(sOverlayBush),
                        BlockTextureDefault.get(sTextureBerry, tBerryColor[2]),
                        BlockTextureDefault.get(sOverlayBerry));
                } else {
                    switch (mStage) {
                        case 0:
                        ...
                        case 1:
                            mTexture = BlockTextureMulti.get(
                                BlockTextureDefault.get(sTextureBush, tBerryColor[0]),
                                BlockTextureDefault.get(sOverlayBush),
                                BlockTextureDefault.get(sTextureImmature, tBerryColor[1]),
                                BlockTextureDefault.get(sOverlayImmature));
                            break;
                        case 2:
                            mTexture = BlockTextureMulti.get(
                        ...
                        case 3:
                            mTexture = BlockTextureMulti.get(
                                BlockTextureDefault.get(sTextureBush, tBerryColor[0]),
                                BlockTextureDefault.get(sOverlayBush),
                                BlockTextureDefault.get(sTextureBerry, tBerryColor[3]),
                                BlockTextureDefault.get(sOverlayBerry));
                            break;
                    }
                }
            ...
            mTexture = BlockTextureMulti
                .get(BlockTextureDefault.get(sTextureBush, 0xff00ff), BlockTextureDefault.get(sOverlayBush));
        }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 1) {
            box(aBlock, 0, 0, 0, 1, PX_P[1], 1);
            return T;
        }
        switch (mFacing) {
            case SIDE_X_POS:
                box(aBlock, PX_P[12], PX_P[2], PX_P[2], PX_N[0], PX_N[2], PX_N[2]);
                return T;
            case SIDE_Y_POS:
                box(aBlock, PX_P[2], PX_P[12], PX_P[2], PX_N[2], PX_N[0], PX_N[2]);
                return T;
            case SIDE_Z_POS:
                box(aBlock, PX_P[2], PX_P[2], PX_P[12], PX_N[2], PX_N[2], PX_N[0]);
                return T;
            case SIDE_X_NEG:
                box(aBlock, PX_P[0], PX_P[2], PX_P[2], PX_N[12], PX_N[2], PX_N[2]);
                return T;
            case SIDE_Y_NEG:
                box(aBlock, PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[12], PX_N[2]);
                return T;
            case SIDE_Z_NEG:
                box(aBlock, PX_P[2], PX_P[2], PX_P[0], PX_N[2], PX_N[2], PX_N[12]);
                return T;
        }
        return F;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aRenderPass == 0 ? aShouldSideBeRendered[aSide] || SIDES_VALID[mFacing] ? mTexture : null
            : aShouldSideBeRendered[aSide] ? SNOW_TEXTURE : null;
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        switch (mFacing) {
            case SIDE_X_POS:
                return box(PX_P[14], PX_P[2], PX_P[2], PX_N[0], PX_N[2], PX_N[2]);
            case SIDE_Y_POS:
                return box(PX_P[2], PX_P[14], PX_P[2], PX_N[2], PX_N[0], PX_N[2]);
            case SIDE_Z_POS:
                return box(PX_P[2], PX_P[2], PX_P[14], PX_N[2], PX_N[2], PX_N[0]);
            case SIDE_X_NEG:
                return box(PX_P[0], PX_P[2], PX_P[2], PX_N[14], PX_N[2], PX_N[2]);
            case SIDE_Y_NEG:
                return box(PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[14], PX_N[2]);
            case SIDE_Z_NEG:
                return box(PX_P[2], PX_P[2], PX_P[0], PX_N[2], PX_N[2], PX_N[14]);
        }
        return box();
    }
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool() {
        switch (mFacing) {
            case SIDE_X_POS:
                return box(PX_P[12], PX_P[2], PX_P[2], PX_N[0], PX_N[2], PX_N[2]);
            case SIDE_Y_POS:
                return box(PX_P[2], PX_P[12], PX_P[2], PX_N[2], PX_N[0], PX_N[2]);
            case SIDE_Z_POS:
                return box(PX_P[2], PX_P[2], PX_P[12], PX_N[2], PX_N[2], PX_N[0]);
            case SIDE_X_NEG:
                return box(PX_P[0], PX_P[2], PX_P[2], PX_N[12], PX_N[2], PX_N[2]);
            case SIDE_Y_NEG:
                return box(PX_P[2], PX_P[0], PX_P[2], PX_N[2], PX_N[12], PX_N[2]);
            case SIDE_Z_NEG:
                return box(PX_P[2], PX_P[2], PX_P[0], PX_N[2], PX_N[2], PX_N[12]);
        }
        return box();
    }
    ...
    public int getLightOpacity() {
        return SIDES_INVALID[mFacing] ? LIGHT_OPACITY_LEAVES : LIGHT_OPACITY_NONE;
    }
