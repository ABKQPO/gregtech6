...
public class MultiTileEntityResinHoleRubber extends MultiTileEntityTreeHole {
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            if (!mHasResin && aTimer % 600 == 0) {
                ...
                for (int i = 1; i < 10; i++) {
                    if (getBlockAtSideAndDistance(SIDE_TOP, i) != BlocksGT.LogA
                        && getMetaDataAtSideAndDistance(SIDE_TOP, i) != 0) break;
                    tTreeHeight++;
                }
    ...
    private boolean checkLeaves(int aX, int aY, int aZ) {
        return getBlock(aX, aY, aZ) == BlocksGT.Leaves_AB && getMetaData(aX, aY, aZ) == 8;
    }

    @Override
    public ArrayListNoNulls<ItemStack> getDrops(int aFortune, boolean aSilkTouch) {
        return isClientSide() ? super.getDrops(aFortune, aSilkTouch) : ST.arraylist(ST.make(BlocksGT.LogA, 1, 0));
    }

    @Override
    public ItemStack getResinItem(byte aSide) {
        return IL.IC2_Resin.get(1, IL.Resin.get(1));
    }

    @Override
    public FluidStack getResinFluid(byte aSide) {
        return FL.Resin_Rubber.make(250);
    }

    @Override
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? aSide != mFacing ? BlockTextureCopied.get(BlocksGT.LogA, SIDE_ANY, 0)
            : BlockTextureDefault
                .get(mHasResin ? Textures.BlockIcons.LOG_RESIN_RUBBER : Textures.BlockIcons.LOG_HOLE_RUBBER)
            : null;
    }

    @Override
    public float getExplosionResistance2() {
        return BlocksGT.LogA.getExplosionResistance((byte) 0);
    }
