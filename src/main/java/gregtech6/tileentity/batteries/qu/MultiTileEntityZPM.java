...
public class MultiTileEntityZPM extends TileEntityBase08Battery implements IMTE_GetLightValue {
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return BlockTextureDefault.get(
            SIDES_VERTICAL[aSide] ? SIDES_TOP[aSide] ? BlockIcons.ZPM_TOP : BlockIcons.ZPM_BOTTOM
                : BlockIcons.ZPM_SIDES,
            mDisplayedEnergy == 0
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        if (aRenderPass == 0) box(aBlock, PX_P[5], PX_P[4], PX_P[5], PX_N[5], PX_N[4], PX_N[5]);
        else box(aBlock, PX_P[4], PX_P[0], PX_P[4], PX_N[4], PX_N[12], PX_N[4]);
        return T;
    }

    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool() {
        return box(PX_P[4], PX_P[0], PX_P[4], PX_N[4], PX_N[4], PX_N[4]);
    }
    ...
    public void setBlockBoundsBasedOnState(Block aBlock) {
        box(aBlock, PX_P[4], PX_P[0], PX_P[4], PX_N[4], PX_N[4], PX_N[4]);
    }
