 ...
 */
public class MultiTileEntityBedrockDrill extends TileEntityBase10MultiBlockBase
    implements ITileEntityEnergy, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockFluidHandler,
    IFluidHandler, ITileEntityRunningPossible {
    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        ...
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) {
            ...
            if (tBlock == BlocksGT.oreBedrock) {
                OreDictMaterialStack tMaterial = BlocksGT.oreBedrock
                    .getMaterialAtSide(worldObj, xCoord + i, yCoord - 5, zCoord + j, SIDE_TOP);
                mList.add(tMaterial.mMaterial);
                mList.add(tMaterial.mMaterial);
            } else if (tBlock == BlocksGT.oreSmallBedrock) {
                OreDictMaterialStack tMaterial = BlocksGT.oreSmallBedrock
                    .getMaterialAtSide(worldObj, xCoord + i, yCoord - 5, zCoord + j, SIDE_TOP);
                mList.add(tMaterial.mMaterial);
    ...
    static {
        LH.add(
            "gt.tooltip.multiblock.bedrockdrill.1",
            "3x3 Base of Bedrock Mining Drill Heads centered ontop Bedrock Ores");
        ...
        LH.add(
            "gt.tooltip.multiblock.bedrockdrill.4",
            "Requires Lubricant, Power and a 3x3 place on Bedrock(-Ore) to mine");
        LH.add(
            "gt.tooltip.multiblock.bedrockdrill.5",
            "Also works on HBM's Bedrock Ores, even if it's not a full 3x3 of Bedrock");
    }
    ...
    public boolean isInsideStructure(int aX, int aY, int aZ) {
        return aX >= xCoord - 1 && aY >= yCoord - 5
            && aZ >= zCoord - 1
            && aX <= xCoord + 1
            && aY <= yCoord
            && aZ <= zCoord + 1;
        ...
        if (aIsServerSide) {
            ...
            if (mEnergy >= 32768 && !slotHas(0) && checkStructure(F) && mTank.drainAll(100)) {
                ...
                if (tSelector < mList.size()) {
                    ...
                    OreDictMaterial tMaterial = (rng(32) == 0
                        ? UT.Code.select(mList.get(tSelector), mList.get(tSelector).mByProducts)
                        : mList.get(tSelector));
                    ...
                    if (worldObj.provider.dimensionId == DIM_NETHER) {
                        ...
                        Object tBlock = BlocksGT.stoneToBrokenOres
                            .get(new ItemStackContainer(IL.ERE_Umberstone.get(1)));
                        slot(
                            0,
                            ST.make((Block) (tBlock instanceof Block ? tBlock : BlocksGT.oreBroken), 1, tMaterial.mID));
                    } else if (WD.dimATUM(worldObj)) {
                        // Atum Limestone Ore.
                        Object tBlock = BlocksGT.stoneToBrokenOres
                            .get(new ItemStackContainer(IL.ATUM_Limestone.get(1)));
                        slot(
                            0,
                            ST.make((Block) (tBlock instanceof Block ? tBlock : BlocksGT.oreBroken), 1, tMaterial.mID));
                    } else if (WD.dimBTL(worldObj)) {
                        // Betweenlands Stone Ores.
                        Object tBlock = BlocksGT.stoneToBrokenOres.get(
                            new ItemStackContainer((mType % 2 == 0 ? IL.BTL_Pitstone : IL.BTL_Betweenstone).get(1)));
                        slot(
                            0,
                            ST.make((Block) (tBlock instanceof Block ? tBlock : BlocksGT.oreBroken), 1, tMaterial.mID));
                    ...
                    if (rng(1000) == 0) {
                        // 0.1% Chance to get Bedrock Dust. Only really useful for the Byproducts it has, and
                        // Rotarycraft.
                        ...
                        switch (mType) {
                            case 0:
                                slot(0, ST.make(Blocks.obsidian, 1, 0));
                                break;
                            case 1:
                                slot(0, OP.blockDust.mat(MT.STONES.Mazestone, 1));
                                break;
                            case 2:
                                slot(0, IL.TF_Trollsteinn.get(1));
                                break;
                            case 3:
                                slot(0, OP.blockDust.mat(MT.STONES.Castlerock, 1));
                                break;
                            case 4:
                                slot(0, IL.TF_Deadrock.get(1));
                                break;
                        }
                    } else if (WD.dimERE(worldObj)) {
                        // Erebus Umberstone.
                        ...
                        if (mType == 0) {
                            ...
                            case 0:
                                slot(0, IL.BTL_Cragrock.get(1));
                                break;
                            case 1:
                                slot(0, IL.BTL_Limestone.get(1));
                                break;
                            case 2:
                                slot(0, IL.BTL_Pitstone.get(1));
                                break;
                            case 3:
                                slot(0, IL.BTL_Betweenstone.get(1));
                                break;
                        }
                    } else if (mType < BlocksGT.stones.length) {
                        // This might be the Overworld or some Overworld alike Dimension.
    ...
    public long getEnergyCapacity(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeAccepted ? 40000 : 0;
    }
    ...
    protected IFluidTank getFluidTankFillable(MultiTileEntityMultiBlockPart aPart, byte aSide,
        ...
        return FluidsGT.LUBRICANT.contains(
            aFluidToFill.getFluid()
                .getName()) ? mTank : null;
    }
    ...
    public boolean getStateRunningPossible() {
        return mStructureOkay && mTank.has(100);
    }
}
