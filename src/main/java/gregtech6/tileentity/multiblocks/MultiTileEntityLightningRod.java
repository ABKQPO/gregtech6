 ...
 */
public class MultiTileEntityLightningRod extends TileEntityBase10MultiBlockBase
    implements ITileEntityEnergy, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy {
    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        ...
        if (tSuccess) while (ITileEntityMultiBlockController.Util.checkAndSetTargetOffset(
            this,
            0,
            5 + mSize,
            0,
    ...
    static {
        ...
        LH.add(
            "gt.tooltip.multiblock.lightningrod.9",
            "Reduced Efficiency if too close to another Lightning Rod (256m)");
    }
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.GREEN + LH.get(LH.ENERGY_OUTPUT)
                + ": "
                + Chat.WHITE
                + VREC[6]
                + " "
                ...
                + "/p (up to 16 Amps)");
        aList.add(
            Chat.WHITE + mCapacity
                + " "
                + mEnergyTypeEmitted.getLocalisedChatNameShort()
                + Chat.GRAY
                + " per Lightning Strike");
        super.addToolTips(aList, aStack, aF3_H);
    }

    @Override
    public boolean isInsideStructure(int aX, int aY, int aZ) {
        return aY >= yCoord && aX >= xCoord - 1
            && aZ >= zCoord - 1
            && aX <= xCoord + 1
            && aZ <= zCoord + 1
            && (aY < yCoord + 5 || (aX == xCoord && aZ == zCoord && aY <= yCoord + mSize + 4));
    }
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            // Required to use force sometimes, because of variable Rod Size, and newly placed Rod Blocks don't cause
            // Multiblock Updates due to Lag.
            if (checkStructure(
                mSize < 100 && aTimer % 1200 == 300 && (worldObj.isRaining() || worldObj.isThundering()))) {
                if (mEnergy >= VREC[6]) {
                    mEnergy -= Math.max(
                        1,
                        ITileEntityEnergy.Util.emitEnergyToSide(mEnergyTypeEmitted, SIDE_BOTTOM, VREC[6], 16, this))
                        * VREC[6];
                } else {
                    mEnergy = 0;
                    if (mSize > 0 && yCoord + mSize >= 100
                        && rng(1000000) < Math.min(100, mSize)
                        && (worldObj.isThundering() || (worldObj.isRaining() && rng(10) == 0))) {
                        int tCount = 1;
                        for (MultiTileEntityLightningRod tLightningRod : ALL_LIGHTNING_RODS)
                            if (tLightningRod != this && tLightningRod.mSize > 0
                                && tLightningRod.getWorld() == worldObj
                                && Math.abs(tLightningRod.xCoord - xCoord) < 256
                                && Math.abs(tLightningRod.zCoord - zCoord) < 256) tCount++;
                        if (rng(tCount) == 0) {
                            boolean temp = T;
                            ...
                            if (temp) {
                                worldObj.addWeatherEffect(
                                    new EntityLightningBolt(worldObj, xCoord, yCoord + mSize + 4, zCoord));
                                mEnergy = mCapacity;
                            }
                        }
                    }
