...
public class MultiTileEntityCryoDistillationTower extends TileEntityBase10MultiBlockMachine {
    ...
    static {
        LH.add(
            "gt.tooltip.multiblock.cryodistillationtower.1",
            "3x3 Base of Heat Transmitters. They can also transfer Cold. ;)");
        LH.add("gt.tooltip.multiblock.cryodistillationtower.2", "3x3x8 of Distillation Tower Parts");
        LH.add(
            "gt.tooltip.multiblock.cryodistillationtower.3",
            "Main centered on Side-Bottom of Tower facing outwards");
    ...
    public void addToolTipsSided(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        if (mEnergyTypeAccepted != TD.Energy.TU) {
            for (byte tSide : ALL_SIDES_VALID) if (FACE_CONNECTED[tSide][mEnergyInputs]) {
                tSideNames += (temp ? ", " : "") + LH.get(LH.FACES[tSide]);
                temp = T;
            }
    ...
    public void doOutputItems() {
        ST.moveAll(
            delegator(FACING_TO_SIDE[mFacing][mItemAutoOutput]),
            WD.te(worldObj, getOffsetXN(mFacing, 3), yCoord, getOffsetZN(mFacing, 3), mFacing, F));
    }
        ...
        for (FluidTankGT tTank : mTanksOutput) {
            ...
            if (tFluid != null && tTank.has()) {
                ...
                if (FL.is(tFluid, "helium")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 7, getOffsetZN(mFacing, 3), mFacing, F);
                } else if (FL.is(tFluid, "neon")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 6, getOffsetZN(mFacing, 3), mFacing, F);
                } else if (FL.is(tFluid, "nitrogen")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 5, getOffsetZN(mFacing, 3), mFacing, F);
                } else if (FL.is(tFluid, "oxygen")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 4, getOffsetZN(mFacing, 3), mFacing, F);
                } else if (FL.is(tFluid, "argon")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 3, getOffsetZN(mFacing, 3), mFacing, F);
                } else if (FL.is(tFluid, "carbondioxide", "sulfurdioxide")) {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 2, getOffsetZN(mFacing, 3), mFacing, F);
                } else {
                    tDelegator = WD
                        .te(worldObj, getOffsetXN(mFacing, 3), yCoord + 1, getOffsetZN(mFacing, 3), mFacing, F);
                }
    ...
    public String getTileEntityName() {
        return "gt.multitileentity.multiblock.cryodistillationtower";
    }
}
