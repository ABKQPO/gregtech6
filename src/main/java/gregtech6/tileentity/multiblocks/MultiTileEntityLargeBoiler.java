 ...
 */
public class MultiTileEntityLargeBoiler extends TileEntityBase10MultiBlockBase
    implements ITileEntityEnergy, ITileEntityGibbl, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy,
    IMultiBlockFluidHandler, IFluidHandler, IMTE_RemovedByPlayer {
    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        ...
        if (worldObj.blockExists(tX - 1, tY, tZ - 1) && worldObj.blockExists(tX + 1, tY, tZ - 1)
            && worldObj.blockExists(tX - 1, tY, tZ + 1)
            && worldObj.blockExists(tX + 1, tY, tZ + 1)) {
            boolean tSuccess = T;

            if (getAir(tX, tY + 1, tZ)) worldObj.setBlockToAir(tX, tY + 1, tZ);
            else tSuccess = F;
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.RED + LH.get(LH.ENERGY_OUTPUT)
                + ": "
                + Chat.WHITE
                + UT.Code.units(mOutput, 10000, mEfficiency, F)
                + " "
    ...
    public void onTick2(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (tConversions > 0) {
                ...
                mTanks[1].setFluid(
                    FL.Steam.make(mTanks[1].amount() + UT.Code.units(tConversions, 10000, mEfficiency * 160, F)));
            ...
            // Emit Steam
            if (tAmount > 0) {
                FluidStack tDrainableSteam = mTanks[1].drain(
                    UT.Code.bindInt(Math.min(tAmount > mTanks[1].capacity() / 4 ? mOutput * 2 : mOutput, tAmount)),
                    F);
                ...
                if (tDrainableSteam != null) {
                    ...
                    DelegatorTileEntity<TileEntity>[] tDelegators = new DelegatorTileEntity[] {
                        ...
                        WD.te(
                            worldObj,
                            getOffsetXN(mFacing, 1) - 2,
                            yCoord + 1,
                            getOffsetZN(mFacing, 1),
                            SIDE_X_POS,
                            F),
                        WD.te(
                            worldObj,
                        ...
                        WD.te(
                            worldObj,
                            getOffsetXN(mFacing, 1),
                            yCoord + 1,
                            getOffsetZN(mFacing, 1) - 2,
                            SIDE_Z_POS,
                            F),
                        WD.te(
                            worldObj,
                    ...
                    for (int i = 0; i < tDelegators.length; i++) if (tDelegators[i].mTileEntity instanceof IFluidHandler
                        && (tTargetAmounts[i] = FL.fill_(tDelegators[i], tDrainableSteam, F)) > 0) tTargets++;
                    else tDelegators[i] = null;
                        ...
                        if (UT.Code.sum(tTargetAmounts) > tDrainableSteam.amount) {
                            ...
                            for (int i = 0; i < tDelegators.length; i++) if (tDelegators[i] != null) {
                                if (tTargetAmounts[i] <= tDrainableSteam.amount / tOriginalTargets) {
                                    tMoveable -= FL
                                        .move_(mTanks[1], tDelegators[i], tDrainableSteam.amount / tOriginalTargets);
                                    tDelegators[i] = null;
                                    if (--tTargets < 2) break;
                                }
                            }
                            ...
                            for (int i = 0; i < tDelegators.length; i++)
                                if (tDelegators[i] != null) FL.move_(mTanks[1], tDelegators[i], tTargetAmounts[i]);
                        }
                    }
                }
            }
        ...
        if (aTool.equals(TOOL_chisel)) {
            ...
            if (rResult > 0) {
                if (mBarometer > 15) {
                    ...
                    if (mEnergy + mTanks[1].amount() / STEAM_PER_EU > 2000)
                        UT.Entities.applyHeatDamage(aPlayer, (mEnergy + mTanks[1].amount() / 2) / 2000.0F);
                    mTanks[1].setEmpty();
                    mEfficiency = 10000;
                    mEnergy = 0;
                    return rResult;
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return aSide != mFacing || rTexture == null ? rTexture
            : BlockTextureMulti.get(
                rTexture,
                BlockTextureDefault.get(BI.BAROMETER),
                BlockTextureDefault.get(BI.BAROMETER_SCALE[mBarometer], CA_RED_64));
    }
    ...
    public long getEnergyCapacity(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeAccepted ? mCapacity : 0;
    }
    ...
    protected IFluidTank getFluidTankFillable(MultiTileEntityMultiBlockPart aPart, byte aSide,
        ...
        return FL.water(aFluidToFill) ? mTanks[0] : null;
    }
    ...
    public long getGibblValue(byte aSide) {
        return mTanks[1].amount();
    }

    @Override
    public long getGibblMax(byte aSide) {
        return mTanks[1].capacity();
    }
