 ...
 */
public class MultiTileEntityCrucible extends TileEntityBase10MultiBlockBase
    implements ITileEntityCrucible, ITileEntityEnergy, ITileEntityGibbl, ITileEntityWeight, ITileEntityTemperature,
    ITileEntityMold, ITileEntityServerTickPost, ITileEntityEnergyDataCapacitor, IMultiBlockEnergy, IMultiBlockInventory,
    IMultiBlockFluidHandler, IFluidHandler {
    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        ...
        if (getAir(xCoord, yCoord + 1, zCoord)) worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord);
        else tSuccess = F;
        if (getAir(xCoord, yCoord + 2, zCoord)) worldObj.setBlockToAir(xCoord, yCoord + 2, zCoord);
        else tSuccess = F;

        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) if (i != 0 || j != 0) {
            if (!ITileEntityMultiBlockController.Util.checkAndSetTargetOffset(
                this,
                i,
                0,
                j,
            ...
            if (!ITileEntityMultiBlockController.Util.checkAndSetTargetOffset(
                ...
                getMultiTileEntityRegistryID(),
                0,
                MultiTileEntityMultiBlockPart.ONLY_CRUCIBLE,
                aCoordinates,
                aPlayer,
                aInventory)) tSuccess = F;
            if (!ITileEntityMultiBlockController.Util.checkAndSetTargetOffset(
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.CYAN + LH.get(LH.CONVERTS_FROM_X)
                + " 1 "
                + TD.Energy.HU.getLocalisedNameShort()
                + " "
                + LH.get(LH.CONVERTS_TO_Y)
                + " +1 K "
                + LH.get(LH.CONVERTS_PER_Z)
                + " "
                + KG_PER_ENERGY
                + "kg (at least "
                + getEnergySizeInputMin(TD.Energy.HU, SIDE_ANY)
                + " Units per Tick required!)");
    ...
    public void onServerTickPost(boolean aFirst) {
            ...
            if (tBiome.rainfall > 0 && tBiome.temperature >= 0.2) {
                addMaterialStacks(
                    Arrays.asList(
                        OM.stack(
                            MT.Water,
                            U100 * (long) Math.max(1, tBiome.rainfall * 100) * (worldObj.isThundering() ? 2 : 1))),
                    tTemperature);
            }
        }
        ...
        if (ST.valid(tStack)) {
            ...
            if (tData == null) {
                ...
                for (OreDictMaterialStack tMaterial : tData.getAllMaterialStacks())
                    if (tMaterial.mAmount > 0) tList.add(tMaterial.clone());
                if (addMaterialStacks(tList, tTemperature)) decrStackSize(0, 1);
            } else if (tData.mPrefix == OP.oreRaw) {
                if (addMaterialStacks(
                    Arrays.asList(
                        OM.stack(
                            tData.mMaterial.mMaterial.mTargetCrushing.mMaterial,
                            tData.mMaterial.mMaterial.mTargetCrushing.mAmount
                                * tData.mMaterial.mMaterial.mOreMultiplier)),
                    tTemperature)) decrStackSize(0, 1);
            } else if (tData.mPrefix == OP.blockRaw) {
                if (addMaterialStacks(
                    Arrays.asList(
                        OM.stack(
                            tData.mMaterial.mMaterial.mTargetCrushing.mMaterial,
                            tData.mMaterial.mMaterial.mTargetCrushing.mAmount * tData.mMaterial.mMaterial.mOreMultiplier
                                * 9)),
                    tTemperature)) decrStackSize(0, 1);
        ...
        for (OreDictMaterialStack tMaterial : mContent) {
            if (mTemperature >= tMaterial.mMaterial.mMeltingPoint) {
                for (OreDictMaterial tAlloy : tMaterial.mMaterial.mAlloyComponentReferences)
                    if (tAlreadyCheckedAlloys.add(tAlloy) && mTemperature >= tAlloy.mMeltingPoint) {
                            ...
                            if (!tNeededStuff.isEmpty()) {
                                ...
                                for (OreDictMaterialStack tComponent : tNeededStuff) {
                                    ...
                                    for (OreDictMaterialStack tContent : mContent) {
                                        if (tContent.mMaterial == tComponent.mMaterial) {
                                            tConversions = Math
                                                .min(tConversions, tContent.mAmount / tComponent.mAmount);
                                            tBreak = F;
                                            break;
                                        }
                                    }
                                ...
                                if (!tBreak && tNonMolten <= 1 && tConversions > 0) {
                                    if (tPreferredAlloy == null || tPreferredRecipe == null
                                        || tConversions * tAlloyRecipe.getCommonDivider()
                                            > tMaxConversions * tPreferredRecipe.getCommonDivider()) {
        ...
        if (tPreferredAlloy != null && tPreferredRecipe != null) {
            ...
            OM.stack(tPreferredAlloy, tPreferredRecipe.getCommonDivider() * tMaxConversions)
                .addToList(mContent);
        }
        ...
        for (int i = 0; i < mContent.size(); i++) {
            ...
            if (tMaterial == null || tMaterial.mMaterial == MT.NULL
                || tMaterial.mMaterial == MT.Air
                || tMaterial.mAmount <= 0) {
                mContent.remove(i--);
            } else if (tMaterial.mMaterial.mGramPerCubicCentimeter <= WEIGHT_AIR_G_PER_CUBIC_CENTIMETER) {
                GarbageGT.trash(mContent.remove(i--));
                UT.Sounds.send(SFX.MC_FIZZ, this, F);
            } else if (mTemperature >= tMaterial.mMaterial.mBoilingPoint
                || (mTemperature > C + 40 && tMaterial.mMaterial.contains(TD.Properties.FLAMMABLE)
                    && !tMaterial.mMaterial.containsAny(TD.Properties.UNBURNABLE, TD.Processing.MELTING))) {
                        GarbageGT.trash(mContent.remove(i--));
                        UT.Sounds.send(SFX.MC_FIZZ, this, F);
                        if (tMaterial.mMaterial.mBoilingPoint >= 320) try {
                            for (EntityLivingBase tLiving : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                                EntityLivingBase.class,
                                box(-GAS_RANGE, -1, -GAS_RANGE, GAS_RANGE + 1, GAS_RANGE + 1, GAS_RANGE + 1)))
                                UT.Entities.applyTemperatureDamage(tLiving, tMaterial.mMaterial.mBoilingPoint, 4);
                        } catch (Throwable e) {
                            e.printStackTrace(ERR);
                        }
                        if (tMaterial.mMaterial.mBoilingPoint >= 2000)
                            for (int j = 0, k = Math.max(1, UT.Code.bindInt((9 * tMaterial.mAmount) / U)); j < k; j++)
                                WD.fire(
                                    worldObj,
                                    xCoord - FLAME_RANGE + rng(2 * FLAME_RANGE + 1),
                                    yCoord - 1 + rng(2 + FLAME_RANGE),
                                    zCoord - FLAME_RANGE + rng(2 * FLAME_RANGE + 1),
                                    rng(3) != 0);
                ...
                if (!mAcidProof && tMaterial.mMaterial.contains(TD.Properties.ACID)) {
                    GarbageGT.trash(mContent);
                    GarbageGT.trash(tToBeAdded);
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                    setToAir();
                    return;
                } else if (mTemperature >= tMaterial.mMaterial.mMeltingPoint
                    && (oTemperature < tMaterial.mMaterial.mMeltingPoint || tNewContent)) {
                        mContent.remove(i--);
                        OM.stack(
                            tMaterial.mMaterial.mTargetSmelting.mMaterial,
                            UT.Code.units_(tMaterial.mAmount, U, tMaterial.mMaterial.mTargetSmelting.mAmount, F))
                            .addToList(tToBeAdded);
                    } else if (mTemperature < tMaterial.mMaterial.mMeltingPoint
                        && (oTemperature >= tMaterial.mMaterial.mMeltingPoint || tNewContent)) {
                            mContent.remove(i--);
                            OM.stack(
                                tMaterial.mMaterial.mTargetSolidifying.mMaterial,
                                UT.Code.units_(tMaterial.mAmount, U, tMaterial.mMaterial.mTargetSolidifying.mAmount, F))
                                .addToList(tToBeAdded);
                        }
        }
        ...
        for (OreDictMaterialStack tMaterial : mContent) {
            if (tLightest == null
                || tMaterial.mMaterial.mGramPerCubicCentimeter < tLightest.mMaterial.mGramPerCubicCentimeter)
                tLightest = tMaterial;
        ...
        mDisplayedFluid = (tLightest == null || tLightest.mMaterial.mMeltingPoint > mTemperature ? -1
            : tLightest.mMaterial.mID);
        ...
        if (mTemperature > getTemperatureMax(SIDE_INSIDE)) {
            UT.Sounds.send(SFX.MC_FIZZ, this, F);
            GarbageGT.trash(mContent);
            if (mTemperature >= 320) try {
                for (EntityLivingBase tLiving : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    box(-GAS_RANGE, -1, -GAS_RANGE, GAS_RANGE + 1, GAS_RANGE + 1, GAS_RANGE + 1)))
                    UT.Entities.applyTemperatureDamage(tLiving, mTemperature, 4);
            } catch (Throwable e) {
                e.printStackTrace(ERR);
            }
            for (int j = 0, k = UT.Code.bindInt(mTemperature / 25); j < k; j++) WD.fire(
                worldObj,
                xCoord - FLAME_RANGE + rng(2 * FLAME_RANGE + 1),
                yCoord - 1 + rng(2 + FLAME_RANGE),
                zCoord - FLAME_RANGE + rng(2 * FLAME_RANGE + 1),
    ...
    public boolean addMaterialStacks(List<OreDictMaterialStack> aList, long aTemperature) {
        if (checkStructure(F) && OM.total(mContent) + OM.total(aList) <= MAX_AMOUNT) {
            ...
            if (tWeight1 + tWeight2 > 0) mTemperature = aTemperature + (mTemperature > aTemperature ? +1 : -1) * UT.Code
                .units(Math.abs(mTemperature - aTemperature), (long) (tWeight1 + tWeight2), (long) tWeight1, F);
    ...
    public long fillMold(OreDictMaterialStack aMaterial, long aTemperature, byte aSide) {
        if (isMoldInputSide(aSide)) {
            ...
            if (aMaterial.mAmount > U
                && addMaterialStacks(Arrays.asList(OM.stack(aMaterial.mMaterial, U)), aTemperature)) return U;
        }
        return 0;
    }
    ...
    public boolean breakBlock() {
        GarbageGT.trash(mContent);
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) if (i != 0 || j != 0) {
            ITileEntityMultiBlockController.Util.checkAndSetTargetOffset(
                this,
                i,
                0,
                j,
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        ...
        if (SIDES_TOP[aSide]) {
            if (isServerSide() && aPlayer != null) {
                ...
                OreDictMaterialStack tLightest = null;
                for (OreDictMaterialStack tMaterial : mContent) if (tLightest == null
                    || tMaterial.mMaterial.mGramPerCubicCentimeter < tLightest.mMaterial.mGramPerCubicCentimeter)
                    tLightest = tMaterial;
                ...
                if (aStack != null) {
                    ...
                    if (tFluid == null) {
                        if (tLightest != null && tLightest.mMaterial.mLiquid != null) {
                            ...
                            if (mTemperature >= tLightest.mMaterial.mMeltingPoint
                                && (tTemperature < 320 || mTemperature >= tTemperature)) {
                                ...
                                if (FL.nonzero(tFluid)) {
                                    ...
                                    if (ST.valid(tStack)) {
                                        tLightest.mAmount -= UT.Code.units(
                                            tAmount - tFluid.amount,
                                            tLightest.mMaterial.mLiquid.amount,
                                            tLightest.mMaterial.mLiquidUnit,
                                            T);
                                        aStack.stackSize--;
                        ...
                        if (!FL.gas(tFluid, T) && !FL.acid(tFluid)) {
                            ...
                            OreDictMaterialStack tFluidData = OreDictMaterial.FLUID_MAP.get(
                                tFluid.getFluid()
                                    .getName());
                            if (tFluidData != null) {
                                if (FL.equal(tFluidData.mMaterial.mLiquid, tFluid)) {
                                    if (addMaterialStacks(
                                        new ArrayListNoNulls<>(
                                            F,
                                            OM.stack(
                                                tFluidData.mMaterial,
                                                UT.Code.units(
                                                    tFluid.amount,
                                                    tFluidData.mMaterial.mLiquid.amount,
                                                    tFluidData.mMaterial.mLiquidUnit,
                                                    F))),
                                        UT.Code.bind(
                                            FL.temperature(tFluid),
                                            tFluidData.mMaterial.mMeltingPoint + 25,
                                            tFluidData.mMaterial.mBoilingPoint - 1))) {
                                        aStack.stackSize--;
                                        ST.give(aPlayer, tStack, T);
                                        return T;
                                    }
                                    ...
                                    if (addMaterialStacks(
                                        new ArrayListNoNulls<>(
                                            ...
                                            OM.stack(
                                                tFluidData.mMaterial,
                                                UT.Code.units(tFluid.amount, tFluidData.mAmount, U, F))),
                                        UT.Code.bind(
                                            FL.temperature(tFluid),
                                            tFluidData.mMaterial.mMeltingPoint + 25,
                                            tFluidData.mMaterial.mBoilingPoint - 1))) {
    ...
    public boolean fillMoldAtSide(ITileEntityMold aMold, byte aSide, byte aSideOfMold) {
        if (checkStructure(F)) for (OreDictMaterialStack tContent : mContent)
    ...
    public long onToolClick2(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_thermometer)) {
            if (aChatReturn != null) aChatReturn.add("Temperature: " + mTemperature + "K");
            return 10000;
        }
        if (aTool.equals(TOOL_shovel) && SIDES_TOP[aSide] && checkStructure(F) && aPlayer instanceof EntityPlayer) {
            ...
            if (tLightest != null && mTemperature < tLightest.mMaterial.mMeltingPoint) {
                ...
                ItemStack tOutputStack = OP.scrapGt
                    .mat(tLightest.mMaterial, UT.Code.bindStack(tLightest.mAmount / OP.scrapGt.mAmount));
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        return getClientDataPacketByteArray(
            T,
            ...
            getDirectionData(),
            mDisplayedHeight,
            UT.Code.toByteS(mDisplayedFluid, 0),
            UT.Code.toByteS(mDisplayedFluid, 1),
            (byte) (mMeltDown ? 1 : 0));
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        if (mStructureOkay) switch (aRenderPass) {
            case 0:
                box(aBlock, -0.999, 0.0, -0.999, -0.500, 3.000, 1.999);
                return T;
            case 1:
                box(aBlock, -0.999, 0.0, -0.999, 1.999, 3.000, -0.500);
                return T;
            case 2:
                box(aBlock, 1.500, 0.0, -0.999, 1.999, 3.000, 1.999);
                return T;
            case 3:
                box(aBlock, -0.999, 0.0, 1.500, 1.999, 3.000, 1.999);
                return T;
            case 4:
                box(aBlock, -0.999, 0.0, -0.999, 1.999, 1.125, 1.999);
                return T;
            case 5:
                box(aBlock, -0.999, 0.0, -0.999, 1.999, 1.125 + (UT.Code.unsignB(mDisplayedHeight) / 150.0), 1.999);
                return T;
        }
        return T;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        switch (aRenderPass) {
            case 0:
            case 2:
                return SIDES_AXIS_Z[aSide] || aSide == SIDE_BOTTOM ? null
                    : BlockTextureMulti.get(
                        BlockTextureDefault
                            .get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide]], mRenderedRGBA, T),
                        BlockTextureDefault
                            .get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide] + 3], T));
            case 1:
            case 3:
                return SIDES_AXIS_X[aSide] || aSide == SIDE_BOTTOM ? null
                    : BlockTextureMulti.get(
                        BlockTextureDefault
                            .get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide]], mRenderedRGBA, T),
                        BlockTextureDefault
                            .get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide] + 3], T));
            case 4:
                return SIDES_VERTICAL[aSide] ? BlockTextureMulti.get(
                    BlockTextureDefault
                        .get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide]], mRenderedRGBA, T),
                    BlockTextureDefault.get((aSide == mFacing ? mTexturesFront : mTextures)[FACES_TBS[aSide] + 3], T))
                    : null;
            case 5:
                return mDisplayedHeight != 0 && SIDES_TOP[aSide] ? mTextureMolten : null;
        }
        ...
        if (UT.Entities.applyTemperatureDamage(aEntity, mTemperature) && mTemperature > 320) {
            if (!aEntity.isEntityAlive()) {
                if (aEntity instanceof EntityVillager || aEntity instanceof EntityWitch) {
                    ...
                    addMaterialStacks(
                        new ArrayListNoNulls<>(F, OM.stack(4 * U, MT.Fe)),
                        WD.envTemp(worldObj, xCoord, yCoord, zCoord));
                    ...
                    addMaterialStacks(
                        new ArrayListNoNulls<>(
                            ...
                            OM.stack(
                                1 * U,
                                ((EntitySkeleton) aEntity).getSkeletonType() == 1 ? MT.BoneWither : MT.Bone),
                            ((EntitySkeleton) aEntity).getSkeletonType() == 1 ? OM.stack(1 * U, MT.Coal) : null),
                        WD.envTemp(worldObj, xCoord, yCoord, zCoord));
                } else if (aEntity instanceof EntityZombie) {
                    addMaterialStacks(
                        new ArrayListNoNulls<>(F, OM.stack(1 * U, MT.MeatRotten)),
                        WD.envTemp(worldObj, xCoord, yCoord, zCoord));
                } else if (aEntity instanceof EntityMooshroom || aEntity instanceof EntityCow
                    || aEntity instanceof EntityHorse) {
                        addMaterialStacks(new ArrayListNoNulls<>(F, OM.stack(3 * U, MT.MeatRaw)), C + 37);
                    } else if (aEntity instanceof EntityPig || aEntity instanceof EntitySheep
                        || aEntity instanceof EntityWolf
                        || aEntity instanceof EntitySquid) {
                            addMaterialStacks(new ArrayListNoNulls<>(F, OM.stack(2 * U, MT.MeatRaw)), C + 37);
                        } else if (aEntity instanceof EntityChicken || aEntity instanceof EntityOcelot
                            || aEntity instanceof EntitySpider
                            || aEntity instanceof EntitySilverfish) {
                                ...
                                if ("GregoriusT".equalsIgnoreCase(aEntity.getCommandSenderName()))
                                    for (int i = 0; i < 16; i++)
                                        addMaterialStacks(new ArrayListNoNulls<>(F, OM.stack(1 * U, MT.Tc)), C + 20);
                            }
            }
        }
    }
    ...
    public long getGibblValue(byte aSide) {
        return UT.Code.divup(OM.total(mContent) * 1000, U9);
    }

    @Override
    public long getGibblMax(byte aSide) {
        return UT.Code.divup(MAX_AMOUNT * 1000, U9);
    }
    ...
    public static final List<TagData> ENERGYTYPES = new ArrayListNoNulls<>(
        F,
        TD.Energy.KU,
        TD.Energy.HU,
        TD.Energy.CU,
    ...
    public long doInject(TagData aEnergyType, byte aSide, long aSize, long aAmount, boolean aDoInject) {
        if (aDoInject) {
            if (aEnergyType == TD.Energy.KU) {
                if (aSize * aAmount > 0 && WD.oxygen(worldObj, xCoord, yCoord + 1, zCoord)) addMaterialStacks(
                    new ArrayListNoNulls<>(
                        F,
                        OM.stack(Math.min(MAX_AMOUNT - OM.total(mContent), aSize * aAmount * U1000), MT.Air)),
                    mTemperature);
            } else if (aEnergyType == TD.Energy.CU) mEnergy -= Math.abs(aAmount * aSize);
            else mEnergy += Math.abs(aAmount * aSize);
        }
        return aAmount;
    }
    ...
    public float getBlockHardness() {
        return mDisplayedHeight != 0 ? super.getBlockHardness() * 100 : super.getBlockHardness();
    }

    @Override
    public String getTileEntityName() {
        return "gt.multitileentity.multiblock.crucible";
    }
}
