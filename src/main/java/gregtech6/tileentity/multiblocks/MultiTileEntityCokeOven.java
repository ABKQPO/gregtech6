            ...
            for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++) {
                if (i == 0 && j == 0 && k == 0) {
                    ...
                    if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(
                        ...
                        0,
                        MultiTileEntityMultiBlockPart.ONLY_ITEM_FLUID_ENERGY,
                        aCoordinates,
                        aPlayer,
    ...
    public DelegatorTileEntity<IFluidHandler> getFluidOutputTarget(byte aSide, Fluid aOutput) {
        ...
        if (aOutput != null) {
            ...
            for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) {
                ...
                if (tTarget.mTileEntity instanceof IFluidHandler
                    && ((IFluidHandler) tTarget.mTileEntity).canFill(tTarget.getForgeSideOfTileEntity(), aOutput)) {
