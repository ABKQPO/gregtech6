...
public class MultiTileEntitySluice extends TileEntityBase10MultiBlockMachine {
    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        int tMinX = xCoord - (SIDE_X_NEG == mFacing ? 0 : SIDE_X_POS == mFacing ? 6 : 1),
        ...
        if (worldObj.blockExists(tMinX, yCoord, tMinZ) && worldObj.blockExists(tMaxX, yCoord + 2, tMaxZ)) {
            ...
            for (int tX = tMinX; tX <= tMaxX; tX++) for (int tZ = tMinZ; tZ <= tMaxZ; tZ++) {
                ...
                if (SIDES_AXIS_X[mFacing] ? Math.abs(tX - xCoord) == 5 && tZ != zCoord
                    : Math.abs(tZ - zCoord) == 5 && tX != xCoord) {
                    if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(
                        this,
                        tX,
                        yCoord + 1,
    ...
    public boolean isInsideStructure(int aX, int aY, int aZ) {
        return aX >= xCoord - (SIDE_X_NEG == mFacing ? 0 : SIDE_X_POS == mFacing ? 6 : 1) && aY >= yCoord
