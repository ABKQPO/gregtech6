    ...
    public boolean checkStructure2(ChunkCoordinates aCoordinates, Entity aPlayer, IInventory aInventory) {
        ...
        if (worldObj.blockExists(tX - 1, tY, tZ - 1) && worldObj.blockExists(tX + 1, tY, tZ - 1)
            ...
            for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++) {
                if (i == 0 && j == 0 && k == 0) {
                    if (getAir(tX + i, tY + j, tZ + k)) worldObj.setBlockToAir(tX + i, tY + j, tZ + k);
                    else tSuccess = F;
                } else {
                    if (!ITileEntityMultiBlockController.Util.checkAndSetTarget(
                        ...
                        0,
                        MultiTileEntityMultiBlockPart.ONLY_FLUID,
                        aCoordinates,
                        aPlayer,
        ...
        if (aIsServerSide && checkStructure(F)) {
            ...
            if (tFluid != null && tFluid.amount > 0) {
                ...
                if (!mPlasmaProof && FL.plasma(mTank)) {
                    GarbageGT.trash(mTank);
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                } else if (!mGasProof && FL.gas(mTank)) {
                    GarbageGT.trash(mTank);
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                } else if (!allowFluid(tFluid)) {
                    GarbageGT.trash(mTank);
                    UT.Sounds.send(SFX.MC_FIZZ, this, F);
                } else if (SIDES_HORIZONTAL[mFacing] || FL.gas(mTank)
                    || (FL.lighter(tFluid) ? SIDES_TOP : SIDES_BOTTOM)[mFacing]) {
