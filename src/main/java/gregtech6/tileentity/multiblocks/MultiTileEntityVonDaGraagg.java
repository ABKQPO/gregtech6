 ...
 */
public class MultiTileEntityVonDaGraagg extends TileEntityBase10MultiBlockBase implements ITileEntityEnergy,
    ITileEntityMobSpawnInhibitor, IMultiBlockEnergy, ITileEntityProgress, ITileEntityGibbl {
    ...
    static {
        LH.add(
            "gt.tooltip.multiblock.von.da.graagg.1",
            "cornerless 5x5x2 Base of 41 Dense Galvanized Steel Wall with Main at bottom Center");
        LH.add(
            "gt.tooltip.multiblock.von.da.graagg.2",
            "A 5m long Pole of Large Copper Coil with +1 Dense Steel Wall ontop");
        LH.add(
            "gt.tooltip.multiblock.von.da.graagg.3",
            "Wrap a cornerless 3x3x3 of 16 Dense Steel Wall around the Top");
        LH.add(
            "gt.tooltip.multiblock.von.da.graagg.4",
            "Prevents Mob Spawns except on Mossy Cobblestone, Range depends on Input.");
    }
    ...
    public boolean inhibitMobSpawn(LivingSpawnEvent.CheckSpawn aEvent, World aWorld, int aX, int aY, int aZ) {
        if (mCurrentRange <= 0 || aWorld != worldObj
            || Math.abs(aX - xCoord) > mCurrentRange
            || Math.abs(aZ - zCoord) > mCurrentRange) return F;
    ...
    public long getEnergyCapacity(TagData aEnergyType, byte aSide) {
        return aEnergyType == mEnergyTypeAccepted ? 4096 : 0;
    }
