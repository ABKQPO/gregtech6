...
public class MultiTileEntityLargeTurbineGas extends MultiTileEntityLargeTurbine {
    ...
    public FluidTankGT mInputTank = new FluidTankGT(),
        mTanksOutput[] = new FluidTankGT[] { new FluidTankGT(), new FluidTankGT(), new FluidTankGT() };
    ...
    public void readFromNBT2(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < mTanksOutput.length; i++) mTanksOutput[i].readFromNBT(aNBT, NBT_TANK + "." + i)
            .setCapacity(mEnergyIN.mMax * 16);
        mInputTank.readFromNBT(aNBT, NBT_TANK)
            .setCapacity(mEnergyIN.mMax * 4);
    }
    ...
    public void addToolTips(List<String> aList, ItemStack aStack, boolean aF3_H) {
        ...
        aList.add(
            Chat.WHITE + LH.get("gt.tooltip.multiblock.gasturbine.1")
                + MultiTileEntityRegistry.getRegistry(getMultiTileEntityRegistryID())
                    .getLocal(mTurbineWalls));
    ...
    public void doConversion(long aTimer) {
        ...
        if (!mStopped && mInputTank.has()
            && mTanksOutput[0].underHalf()
            && mTanksOutput[1].underHalf()
            && mTanksOutput[2].underHalf()) {
            ...
            if (tRecipe != null) {
                ...
                if (tRecipe.mEUt < 0 && tRecipe.mDuration > 0) {
                    int tMax = UT.Code
                        .bindInt(UT.Code.divup(mEnergyIN.mMax - mStorage.mEnergy, -tRecipe.mEUt * tRecipe.mDuration)),
                        tParallel = tRecipe.isRecipeInputEqual(tMax, mInputTank.AS_ARRAY, ZL_IS);
    ...
    protected IFluidTank getFluidTankFillable2(byte aSide, FluidStack aFluidToFill) {
        return !mStopped && mRecipes.containsInput(aFluidToFill, this, NI) ? mInputTank : null;
    }
