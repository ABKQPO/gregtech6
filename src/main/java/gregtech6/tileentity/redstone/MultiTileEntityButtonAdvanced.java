 ...
 */
public class MultiTileEntityButtonAdvanced extends TileEntityBase09FacingSingle
    implements ITileEntityQuickObstructionCheck, IMTE_IgnorePlayerCollisionWhenPlacing, ITileEntityRemoteActivateable,
    ...
    public boolean receiveDataByteArray(byte[] aData, INetworkHandler aNetworkHandler) {
        ...
        mType = aData[5];
        mIndex = aData[6];
        if (mType < 0 || mIndex < 0 || mType >= sTextures.length || mIndex >= sTextures[mType].length)
            mType = mIndex = 0;
        return T;
    }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        box(
            aBlock,
            PX_P[SIDE_X_NEG == mFacing ? 14 : SIDE_X_POS == mFacing ? 0 : 4],
            PX_P[SIDE_Y_NEG == mFacing ? 14 : SIDE_Y_POS == mFacing ? 0 : 4],
            PX_P[SIDE_Z_NEG == mFacing ? 14 : SIDE_Z_POS == mFacing ? 0 : 4],
            PX_N[SIDE_X_POS == mFacing ? 14 : SIDE_X_NEG == mFacing ? 0 : 4],
            PX_N[SIDE_Y_POS == mFacing ? 14 : SIDE_Y_NEG == mFacing ? 0 : 4],
            PX_N[SIDE_Z_POS == mFacing ? 14 : SIDE_Z_NEG == mFacing ? 0 : 4]);
        return T;
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        ...
        return BlockTextureMulti.get(
            BlockTextureDefault.get(sTextures[mType][mIndex][tOffset + 0], mRGBa),
            BlockTextureDefault.get(sTextures[mType][mIndex][tOffset + 1]),
            BlockTextureDefault.get(sTextures[mType][mIndex][tOffset + 2], mRGBa, F, T, T, F),
            BlockTextureDefault.get(sTextures[mType][mIndex][tOffset + 3], UNCOLOURED, F, T, T, F));
    }
    ...
    public float getSurfaceSize(byte aSide) {
        return ALONG_AXIS[aSide][mFacing] ? PX_P[14] : 0.0F;
    }
    ...
    public float getSurfaceDistance(byte aSide) {
        return aSide == mFacing ? PX_N[1] : 0.0F;
    }
    ...
    public byte getVisualData() {
        return (byte) ((mActive ? 1 : 0) | (mInverted ? 2 : 0) | (mGlowInverted ? 4 : 0) | (mLampMode ? 8 : 0));
    }
    ...
    public void setVisualData(byte aData) {
        mActive = ((aData & 1) != 0);
        mInverted = ((aData & 2) != 0);
        mGlowInverted = ((aData & 4) != 0);
        mLampMode = ((aData & 8) != 0);
    }

    public static final IIconContainer[][][] sTextures = new IIconContainer[][][] { new IIconContainer[1][8],
        new IIconContainer[10][8], new IIconContainer[26][8], new IIconContainer[11][8], new IIconContainer[8][8],
        new IIconContainer[16][8], new IIconContainer[22][8] };

    static {
        for (int i = 0; i < sTextures.length; i++) {
            for (int j = 0; j < sTextures[i].length; j++) {
                sTextures[i][j][0] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/colored_off");
                sTextures[i][j][1] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/overlay_off");
                sTextures[i][j][2] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/glowing_off");
                sTextures[i][j][3] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/glowverlay_off");
                sTextures[i][j][4] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/colored_on");
                sTextures[i][j][5] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/overlay_on");
                sTextures[i][j][6] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/glowing_on");
                sTextures[i][j][7] = new Textures.BlockIcons.CustomIcon(
                    "machines/redstone/buttons/advanced/" + i + "/" + j + "/glowverlay_on");
            }
        }
    }
