...
public class MultiTileEntityItemometer extends MultiTileEntitySensorTE {
    ...
    static {
        LH.add("gt.tooltip.sensor.itemometer", "Measures the precise amount of Items");
    }

    @Override
    public String getSensorDescription() {
        return LH.get("gt.tooltip.sensor.itemometer");
    }
    ...
    public long getCurrentValue(DelegatorTileEntity<TileEntity> aDelegator) {
        if (aDelegator.mTileEntity instanceof IInventory) {
            ...
            if (aDelegator.mTileEntity instanceof ISidedInventory) {
                int[] tSlots = ((ISidedInventory) aDelegator.mTileEntity)
                    .getAccessibleSlotsFromSide(aDelegator.mSideOfTileEntity);
                if (tSlots == null || tSlots.length <= 0) try {
                    tSlots = ((ISidedInventory) aDelegator.mTileEntity).getAccessibleSlotsFromSide(SIDE_ANY);
                } catch (Throwable e) {
                    tSlots = null;
                }
    ...
    public long getCurrentMax(DelegatorTileEntity<TileEntity> aDelegator) {
        if (aDelegator.mTileEntity instanceof IInventory) {
            if (aDelegator.mTileEntity instanceof ISidedInventory)
                return (long) ((ISidedInventory) aDelegator.mTileEntity)
                    .getAccessibleSlotsFromSide(aDelegator.mSideOfTileEntity).length
                    * ((IInventory) aDelegator.mTileEntity).getInventoryStackLimit();
            return (long) ((IInventory) aDelegator.mTileEntity).getSizeInventory()
                * ((IInventory) aDelegator.mTileEntity).getInventoryStackLimit();
        }
        return 0;
    }
    ...
    public static IIconContainer sTextureFront = new Textures.BlockIcons.CustomIcon(
        "machines/redstone/sensors/itemometer/colored/front"),
    ...
    public String getTileEntityName() {
        return "gt.multitileentity.redstone.sensors.itemometer";
    }
}
