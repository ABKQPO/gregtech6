...
public class MultiTileEntityBucketometer extends MultiTileEntitySensorTE {
    ...
    static {
        LH.add("gt.tooltip.sensor.bucketometer", "Measures Fluids (In Cubic Meters)");
    }

    @Override
    public String getSensorDescription() {
        return LH.get("gt.tooltip.sensor.bucketometer");
    }

    @Override
    public long getCurrentValue(DelegatorTileEntity<TileEntity> aDelegator) {
        if (aDelegator.mTileEntity instanceof IFluidHandler) {
            FluidTankInfo[] tInfo = ((IFluidHandler) aDelegator.mTileEntity)
                .getTankInfo(FORGE_DIR[aDelegator.mSideOfTileEntity]);
            if (tInfo != null) {
                long rFluid = 0;
                for (FluidTankInfo tTank : tInfo)
                    if (tTank != null && tTank.fluid != null) rFluid += tTank.fluid.amount;
                return rFluid / 1000;
            }
        }
        ...
        if (tBlock instanceof IFluidBlock) {
            FluidStack tFluid = ((IFluidBlock) tBlock)
                .drain(aDelegator.mWorld, aDelegator.mX, aDelegator.mY, aDelegator.mZ, F);
    ...
    public static IIconContainer sTextureFront = new Textures.BlockIcons.CustomIcon(
        "machines/redstone/sensors/bucketometer/colored/front"),
    ...
    public String getTileEntityName() {
        return "gt.multitileentity.redstone.sensors.bucketometer";
    }
}
