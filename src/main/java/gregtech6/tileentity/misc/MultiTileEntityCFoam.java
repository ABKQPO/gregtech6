 ...
 */
public class MultiTileEntityCFoam extends TileEntityBase07Paintable implements ITileEntityFoamable,
    IMTE_GetPlayerRelativeBlockHardness, IMTE_AddToolTips, IMTE_OnPlaced, IMTE_OnRegistration {
    ...
    public static boolean setBlock(World aWorld, int aX, int aY, int aZ, byte aSide, EntityPlayer aPlayer,
        ItemStack aStack, short[] aRGB, boolean aOwned) {
        return MTE_REGISTRY.mBlock.placeBlock(
            aWorld,
            aX,
            aY,
            aZ,
            ...
            UT.NBT.make(
                NBT_COLOR,
                UT.Code.getRGBInt(aRGB),
                NBT_PAINTED,
                T,
                NBT_OWNABLE,
                aOwned,
                NBT_OWNER,
                aOwned && aPlayer != null ? aPlayer.getUniqueID()
                    .toString() : null),
            T,
            F);
    }
    ...
    public ITexture getTexture2(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide] ? BlockTextureDefault.get(
            mFoamDried ? mOwnable ? Textures.BlockIcons.CFOAM_HARDENED_OWNED : Textures.BlockIcons.CFOAM_HARDENED
                : mOwnable ? Textures.BlockIcons.CFOAM_FRESH_OWNED : Textures.BlockIcons.CFOAM_FRESH,
            mRGBa) : null;
    }

    @Override
    public int getLightOpacity() {
        return mFoamDried ? LIGHT_OPACITY_MAX : LIGHT_OPACITY_WATER;
    }

    @Override
    public float getBlockHardness() {
        return (mFoamDried ? BlocksGT.CFoam : BlocksGT.CFoamFresh).getBlockHardness(worldObj, xCoord, yCoord, zCoord);
    }

    @Override
    public float getExplosionResistance2() {
        return (mFoamDried ? BlocksGT.CFoam : BlocksGT.CFoamFresh).getExplosionResistance(null);
    }

    @Override
    public byte getVisualData() {
        return (byte) ((mFoamDried ? 1 : 0) | (mOwnable ? 2 : 0));
    }

    @Override
    public void setVisualData(byte aData) {
        mFoamDried = ((aData & 1) != 0);
        mOwnable = ((aData & 2) != 0);
    }
