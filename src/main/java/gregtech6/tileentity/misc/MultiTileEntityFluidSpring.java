 ...
 */
public class MultiTileEntityFluidSpring extends TileEntityBase04MultiTileEntities
    implements IMTE_OnRegistration, ITileEntitySurface, IMTE_IsSideSolid, IMTE_GetExplosionResistance,
    IMTE_GetBlockHardness, IMTE_GetLightOpacity, IMTE_SyncDataShort {
    ...
    public IPacket getClientDataPacket(boolean aSendAll) {
        return getClientDataPacketShort(
            aSendAll,
            (short) mFluid.getFluid()
                .getID());
    }
    ...
    public void onTick(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (tProduce) {
                Block tBlock = FL.BLOCKS.get(
                    mFluid.getFluid()
                        .getName()),
                    tAbove = getBlockAtSide(SIDE_UP);
                if (ST.invalid(tBlock)) tBlock = mFluid.getFluid()
                    .getBlock();
                if (ST.valid(tBlock)) {
                    if (tBlock instanceof BlockFluidFinite) {
                        if (tAbove == tBlock) {
                            worldObj.setBlock(
                                ...
                                zCoord,
                                tBlock,
                                UT.Code.bind4(getMetaDataAtSide(SIDE_UP) + 8),
                                3);
                            ...
                            if (getMetaDataAtSide(SIDE_UP) == 0) {
                                for (byte tSide : ALL_SIDES_HORIZONTAL) {
                                    ...
                                    if (tAbove == tBlock) {
                                        ...
                                        .isAir(worldObj, xCoord + OFFX[tSide], yCoord + 1, zCoord + OFFZ[tSide])) {
                                            worldObj.setBlock(
                                                xCoord + OFFX[tSide],
                                                yCoord + 1,
                                                zCoord + OFFZ[tSide],
    ...
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return aShouldSideBeRendered[aSide]
            ? BlockTextureMulti
                .get(BlockTextureFluid.get(mFluid), BlockTextureDefault.get(Textures.BlockIcons.FLUID_SPRING))
            : null;
    }
    ...
    public float getExplosionResistance2() {
        return Blocks.bedrock.getExplosionResistance(null);
    }
