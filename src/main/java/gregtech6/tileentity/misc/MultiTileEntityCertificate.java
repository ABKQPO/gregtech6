 ...
 */
public class MultiTileEntityCertificate extends TileEntityBase09FacingSingle implements IMTE_OnRegistration,
    IMTE_OnDespawn, IMTE_GetLifeSpan, IMTE_IgnorePlayerCollisionWhenPlacing, IMTE_OnServerLoad, IMTE_OnServerSave,
    ...
    public static ItemStack getCertificate(int aAmount, String aName) {
        ...
        if (UT.Code.stringValid(aName))
            tNBT.setTag("display", UT.NBT.makeString(tNBT.getCompoundTag("display"), "Name", aName));
    ...
    public boolean onBlockActivated3(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide()) {
            ...
            if (D1) for (ItemStack tStack : aPlayer.inventory.mainInventory) if (ST.valid(tStack)) {
                DEB.println(
                    "ST.make(\"" + ST.regName(tStack)
                        + "\"      , 1, "
                        + ST.meta_(tStack)
                        + ");                            // "
                        + ST.namesAndSizes(tStack));
            }
            // Now for the actual thing that this needs to do on Rightclick.
            if (mGold) {
                UT.Entities.sendchat(aPlayer, "This Certificate is owned by" + LH.Chat._YELLOW + getCustomName());
                return T;
            }
            if (mSilver) {
                UT.Entities.sendchat(aPlayer, "This Certificate is owned by" + LH.Chat._CYAN + getCustomName());
                return T;
            }
    ...
    public boolean setBlockBounds2(Block aBlock, int aRenderPass, boolean[] aShouldSideBeRendered) {
        box(
            aBlock,
            PX_P[SIDE_X_NEG == mFacing ? 15 : SIDE_X_POS == mFacing ? 0 : 1],
            PX_P[SIDE_Y_NEG == mFacing ? 15 : SIDE_Y_POS == mFacing ? 0 : 1],
            PX_P[SIDE_Z_NEG == mFacing ? 15 : SIDE_Z_POS == mFacing ? 0 : 1],
            PX_N[SIDE_X_POS == mFacing ? 15 : SIDE_X_NEG == mFacing ? 0 : 1],
            PX_N[SIDE_Y_POS == mFacing ? 15 : SIDE_Y_NEG == mFacing ? 0 : 1],
            PX_N[SIDE_Z_POS == mFacing ? 15 : SIDE_Z_NEG == mFacing ? 0 : 1]);
        return T;
    }
    ...
    public float getSurfaceSize(byte aSide) {
        return ALONG_AXIS[aSide][mFacing] ? PX_P[15] : 0.0F;
    }
    ...
    @Override
    public void onExploded(Explosion aExplosion) {/* Doesn't get removed after Explosions. */}
    ...
    public byte getVisualData() {
        return (byte) (mGold ? 2 : mSilver ? 1 : 0);
    }

    @Override
    public void setVisualData(byte aData) {
        if (aData == 2) mGold = T;
        if (aData == 1) mSilver = T;
    }

    public static IIconContainer sTextureBack = new Textures.BlockIcons.CustomIcon("certificate/BACK"),
