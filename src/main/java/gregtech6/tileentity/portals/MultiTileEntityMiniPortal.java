 ...
 */
public abstract class MultiTileEntityMiniPortal extends TileEntityBase04MultiTileEntities
    implements ITileEntitySurface, ITileEntityDelegating, IFluidHandler, ISidedInventory, IMTE_OnServerStart,
    IMTE_OnServerStop, IMTE_OnServerLoad, IMTE_OnToolClick, IMTE_IsProvidingWeakPower, IMTE_GetComparatorInputOverride,
    IMTE_GetExplosionResistance, IMTE_GetBlockHardness, IMTE_GetLightOpacity, IMTE_AddToolTips, IMTE_SyncDataByte {

    protected boolean mActive = F;

    public MultiTileEntityMiniPortal mTarget = null;
    public final byte[] mRedstone = new byte[] { 0, 0, 0, 0, 0, 0 }, mComparator = new byte[] { 0, 0, 0, 0, 0, 0 },
        xRedstone = new byte[] { 0, 0, 0, 0, 0, 0 }, xComparator = new byte[] { 0, 0, 0, 0, 0, 0 },
        wRedstone = new byte[] { 0, 0, 0, 0, 0, 0 }, wComparator = new byte[] { 0, 0, 0, 0, 0, 0 };
    ...
    public void onTick(long aTimer, boolean aIsServerSide) {
        ...
        if (aIsServerSide) {
            ...
            if (mTarget != null) for (byte tSide : ALL_SIDES_VALID) {
                mTarget.xRedstone[OPOS[tSide]] = (byte) UT.Code
                    .bind_(mTarget.xRedstone[OPOS[tSide]], 15, getRedstoneIncoming(tSide));
                mTarget.xComparator[OPOS[tSide]] = (byte) UT.Code
                    .bind_(mTarget.xComparator[OPOS[tSide]], 15, getComparatorIncoming(tSide));
            }
        }
    }
    ...
    public void removeThisPortalFromLists() {
        if (getPortalListA().remove(this)) for (MultiTileEntityMiniPortal tPortal : getPortalListB())
            if (tPortal.mTarget == this) tPortal.findTargetPortal();
        if (getPortalListB().remove(this)) for (MultiTileEntityMiniPortal tPortal : getPortalListA())
            if (tPortal.mTarget == this) tPortal.findTargetPortal();
    }
    ...
    public void onServerStart() {
        getPortalListA().clear();
        getPortalListB().clear();
    }
    ...
    public float getBlockHardness() {
        return Blocks.stone.getBlockHardness(worldObj, xCoord, yCoord, zCoord);
    }
    ...
    protected static float[][] sBlockBounds = { { PX_P[1], PX_P[1], PX_P[1], PX_N[1], PX_N[1], PX_N[1] },
    ...
    protected static boolean[][] sRenderedSides = { { T, T, T, T, T, T },

        { T, T, T, T, F, F }, { F, F, T, T, T, T }, { T, T, F, F, T, T }, { T, T, F, F, T, T }, { F, F, T, T, T, T },
        { T, T, F, F, T, T }, { T, T, T, T, F, F }, { F, F, T, T, T, T }, { T, T, T, T, F, F }, { T, T, T, T, F, F },
        { F, F, T, T, T, T }, { T, T, F, F, T, T } };

    @Override
    public ITexture getTexture(Block aBlock, int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered) {
        return sRenderedSides[aRenderPass][aSide]
            ? aRenderPass == 0 ? mActive || worldObj == null ? aShouldSideBeRendered[aSide] ? getPortalTexture() : null
                : getInactiveTexture() : getFrameTexture()
            : null;
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (mTarget != null) {
            ...
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.fill(tTileEntity.getForgeSideOfTileEntity(), resource, doFill);
        }
        return 0;
    }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        if (mTarget != null) {
            DelegatorTileEntity<IFluidHandler> tTileEntity = mTarget.getAdjacentTank(OPOS[UT.Code.side(from)]);
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.drain(tTileEntity.getForgeSideOfTileEntity(), resource, doDrain);
        }
        return null;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        if (mTarget != null) {
            DelegatorTileEntity<IFluidHandler> tTileEntity = mTarget.getAdjacentTank(OPOS[UT.Code.side(from)]);
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.canFill(tTileEntity.getForgeSideOfTileEntity(), fluid);
        }
        return F;
    }
    ...
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        if (mTarget != null) {
            DelegatorTileEntity<IFluidHandler> tTileEntity = mTarget.getAdjacentTank(OPOS[UT.Code.side(from)]);
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.canDrain(tTileEntity.getForgeSideOfTileEntity(), fluid);
        }
        return F;
    }
