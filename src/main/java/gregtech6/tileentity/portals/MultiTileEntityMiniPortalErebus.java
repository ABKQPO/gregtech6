...
public class MultiTileEntityMiniPortalErebus extends MultiTileEntityMiniPortal {
    ...
    public static List<MultiTileEntityMiniPortal> sListErebusSide = new ArrayListNoNulls<>(),
    ...
    public boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide()) {
            ...
            if (ST.valid(aStack) && aStack.stackSize > 0
                && (IL.ERE_Gaean_Gem.equal(aStack, F, T) || IL.ERE_Gaean_Staff.equal(aStack, F, T))) {
                setPortalActive();
    ...
    public ITexture sErebusPortal = BlockTextureCopied
        .get(ST.block(MD.ERE, "portal", Blocks.portal), SIDE_ANY, 0, UNCOLOURED, F, T, T),
        sErebusPortalFrame = BlockTextureCopied
            .get(ST.block(MD.ERE, "umberstone", Blocks.stone), SIDE_ANY, 0, UNCOLOURED, F, F, F),
        sErebusPortalInactive = BlockTextureCopied.get(Blocks.leaves, SIDE_ANY, 0, DYE_Green, F, F, F);

    @Override
    public ITexture getPortalTexture() {
        return sErebusPortal;
    }

    @Override
    public ITexture getFrameTexture() {
        return sErebusPortalFrame;
    }

    @Override
    public ITexture getInactiveTexture() {
        return sErebusPortalInactive;
    }

    @Override
    public String getTileEntityName() {
        return "gt.multitileentity.portal.erebus";
    }
}
