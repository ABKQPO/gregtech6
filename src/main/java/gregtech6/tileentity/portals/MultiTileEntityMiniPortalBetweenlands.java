...
public class MultiTileEntityMiniPortalBetweenlands extends MultiTileEntityMiniPortal {
    ...
    public static List<MultiTileEntityMiniPortal> sListBetweenlandsSide = new ArrayListNoNulls<>(),
    ...
    public void findTargetPortal() {
        ...
        if (MD.BTL.mLoaded && worldObj != null && isServerSide()) {
            if (worldObj.provider.dimensionId == DIM_OVERWORLD) {
                ...
                for (MultiTileEntityMiniPortal tTarget : sListBetweenlandsSide)
                    if (tTarget != this && !tTarget.isDead()) {
                        long tXDifference = xCoord - tTarget.xCoord, tZDifference = zCoord - tTarget.zCoord;
                        long tTempDist = tXDifference * tXDifference + tZDifference * tZDifference;
                        if (tTempDist < tShortestDistance) {
                            tShortestDistance = tTempDist;
                            mTarget = tTarget;
                        } else if (tTempDist == tShortestDistance && (mTarget == null
                            || Math.abs(tTarget.yCoord - yCoord) < Math.abs(mTarget.yCoord - yCoord))) {
                                mTarget = tTarget;
                            }
                    }
                ...
                for (MultiTileEntityMiniPortal tTarget : sListWorldSide) if (tTarget != this && !tTarget.isDead()) {
                    ...
                    if (tTempDist < tShortestDistance) {
                        tShortestDistance = tTempDist;
                        mTarget = tTarget;
                    } else if (tTempDist == tShortestDistance
                        && (mTarget == null || Math.abs(tTarget.yCoord - yCoord) < Math.abs(mTarget.yCoord - yCoord))) {
                            mTarget = tTarget;
                        }
                }
            }
    ...
    public boolean onBlockActivated2(EntityPlayer aPlayer, byte aSide, float aHitX, float aHitY, float aHitZ) {
        if (isServerSide()) {
            ...
            if (ST.valid(aStack) && aStack.stackSize > 0 && IL.BTL_Swamp_Talisman.equal(aStack, F, T)) {
                setPortalActive();
                if (mTarget != null) UT.Entities
                    .sendchat(aPlayer, "X: " + mTarget.xCoord + "   Y: " + mTarget.yCoord + "   Z: " + mTarget.zCoord);
    ...
    public ITexture sBetweenlandsPortal = BlockTextureCopied
        .get(ST.block(MD.BTL, "treePortalBlock", Blocks.portal), SIDE_ANY, 0, UNCOLOURED, F, T, T),
        sBetweenlandsPortalFrame = BlockTextureCopied
            .get(ST.block(MD.BTL, "portalBark", Blocks.stone), SIDE_ANY, 0, UNCOLOURED, F, F, F),
        sBetweenlandsPortalInactive = BlockTextureCopied.get(Blocks.leaves, SIDE_ANY, 0, DYE_Green, F, F, F);

    @Override
    public ITexture getPortalTexture() {
        return sBetweenlandsPortal;
    }

    @Override
    public ITexture getFrameTexture() {
        return sBetweenlandsPortalFrame;
    }

    @Override
    public ITexture getInactiveTexture() {
        return sBetweenlandsPortalInactive;
    }
