...
public class MultiTileEntityMiniPortalNether extends MultiTileEntityMiniPortal implements IMTE_RandomDisplayTick {
    ...
    public static List<MultiTileEntityMiniPortal> sListNetherSide = new ArrayListNoNulls<>(),
    ...
    static {
        LH.add(
            "gt.tileentity.portal.nether.tooltip.1",
    ...
    public void randomDisplayTick(Random aRandom) {
        if (mActive) for (int i = 0; i < 4; ++i) worldObj.spawnParticle(
            "portal",
            xCoord + aRandom.nextFloat(),
            yCoord + aRandom.nextFloat(),
            zCoord + aRandom.nextFloat(),
            (aRandom.nextFloat() - 0.5D) * 0.5D,
            (aRandom.nextFloat() - 0.5D) * 0.5D,
            (aRandom.nextFloat() - 0.5D) * 0.5D);
    }
    ...
    public long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (isClientSide()) return super.onToolClick(
            aTool,
        ...
        if (aTool.equals(TOOL_igniter)) {
            ...
            else setPortalActive();
            if (mTarget != null && aChatReturn != null)
    ...
    public float getBlockHardness() {
        return Blocks.obsidian.getBlockHardness(worldObj, xCoord, yCoord, zCoord);
    }

    @Override
    public float getExplosionResistance2() {
        return Blocks.obsidian.getExplosionResistance(null);
    }

    public ITexture sNetherPortal = BlockTextureCopied.get(Blocks.portal, SIDE_ANY, 0, UNCOLOURED, F, T, T),
        sNetherPortalFrame = BlockTextureCopied.get(Blocks.obsidian, SIDE_ANY, 0);

    @Override
    public ITexture getPortalTexture() {
        return sNetherPortal;
    }

    @Override
    public ITexture getFrameTexture() {
        return sNetherPortalFrame;
    }
