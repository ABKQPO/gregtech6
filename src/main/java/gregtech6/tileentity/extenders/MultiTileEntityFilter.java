...
public class MultiTileEntityFilter extends MultiTileEntityExtender implements ITileEntityLogisticsSemiFilteredItem {
    ...
    public int getSizeInventoryGUI() {
        return mFilter == null ? 0 : mFilter.length;
    }

    @Override
    public ItemStack getStackInSlotGUI(int aSlot) {
        return mFilter[aSlot];
    }

    @Override
    public ItemStack decrStackSizeGUI(int aSlot, int aDecrement) {
        mInventoryChanged = T;
        if (mFilter[aSlot] != null) {
            if (mFilter[aSlot].stackSize <= aDecrement) {
                ItemStack tStack = mFilter[aSlot];
                mFilter[aSlot] = null;
                return tStack;
            }
            ItemStack rStack = mFilter[aSlot].splitStack(aDecrement);
            if (mFilter[aSlot].stackSize <= 0) mFilter[aSlot] = null;
            return rStack;
        }
        return null;
    }
    ...
    public ItemStack getStackInSlotOnClosingGUI(int aSlot) {
        ItemStack rStack = mFilter[aSlot];
        mFilter[aSlot] = null;
        return rStack;
    }
    ...
    public boolean allowInput(ItemStack aStack) {
        for (ItemStack tStack : mFilter)
            if (ST.valid(tStack) && ST.equal_(tStack, aStack, !tStack.hasTagCompound() || ST.meta_(tStack) == W))
                return !mInverted;
        return mInverted;
    }
    ...
    public boolean allowInput(Fluid aFluid) {
        ...
        for (ItemStack tStack : mFilter)
            if (IL.Display_Fluid.equal(tStack, T, T) && FL.id(aFluid) == ST.meta_(tStack)) return !mInverted;
        return mInverted;
    }
    ...
    public int fill(ForgeDirection aDirection, FluidStack aFluid, boolean aDoFill) {
        ...
        if ((mModes & EXTENDER_TANK) != 0 && (aSide == mFacing || allowInput(aFluid))) {
            ...
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.fill(tTileEntity.getForgeSideOfTileEntity(), aFluid, aDoFill);
        }
        return 0;
    }
    ...
    public FluidStack drain(ForgeDirection aDirection, FluidStack aFluid, boolean aDoDrain) {
        ...
        if ((mModes & EXTENDER_TANK) != 0 && (aSide == mFacing || allowInput(aFluid))) {
            ...
            DelegatorTileEntity<IFluidHandler> tTileEntity = getAdjacentTank(getExtenderTargetSide(aSide), F, T);
            if (tTileEntity.mTileEntity != null)
                return tTileEntity.mTileEntity.drain(tTileEntity.getForgeSideOfTileEntity(), aFluid, aDoDrain);
        }
        return null;
    }
    ...
    public FluidStack drain(ForgeDirection aDirection, int aToDrain, boolean aDoDrain) {
        if ((mModes & EXTENDER_TANK) != 0) {
            ...
            DelegatorTileEntity<IFluidHandler> tTileEntity = getAdjacentTank(getExtenderTargetSide(aSide), F, T);
            if (tTileEntity.mTileEntity != null) return (aSide == mFacing
                || allowInput(tTileEntity.mTileEntity.drain(tTileEntity.getForgeSideOfTileEntity(), aToDrain, F)))
                    ? tTileEntity.mTileEntity.drain(tTileEntity.getForgeSideOfTileEntity(), aToDrain, aDoDrain)
                    : null;
        }
        return null;
    }
    ...
    public class MultiTileEntityGUICommonFilter extends ContainerCommon {
        ...
        public int addSlots(InventoryPlayer aInventoryPlayer) {
            ...
            for (int y = 0, i = 0; y < tRows; y++) for (int x = 0; x < 9 && i < tSize; x++)
                addSlotToContainer(new Slot_Holo(mTileEntity, i++, 8 + x * 18, 18 + y * 18, T, F, 1));
        ...
        public ItemStack slotClick(int aSlotIndex, int aMouseclick, int aShifthold, EntityPlayer aPlayer) {
            if (aSlotIndex < 0 || aSlotIndex >= inventorySlots.size())
                ...
                if (tSlot != null) {
                    ...
                    if (tStack != null) {
                        if ((((MultiTileEntityFilter) mTileEntity).mModes & EXTENDER_INV) == 0) {
                            ...
                            if (tFluid != null && ((MultiTileEntityFilter) mTileEntity).allowInput(tFluid)
                                == ((MultiTileEntityFilter) mTileEntity).mInverted) {
                                for (int i = 0; i < ((MultiTileEntityFilter) mTileEntity).mFilter.length; i++)
                                    if (ST.invalid(((MultiTileEntityFilter) mTileEntity).mFilter[i])) {
                                        ((MultiTileEntityFilter) mTileEntity).mFilter[i] = FL
                                            .display(tFluid.getFluid());
                                        detectAndSendChanges();
                                        return null;
                                    }
                            }
                        } else {
                            if (((MultiTileEntityFilter) mTileEntity).allowInput(tStack)
                                == ((MultiTileEntityFilter) mTileEntity).mInverted) {
                                for (int i = 0; i < ((MultiTileEntityFilter) mTileEntity).mFilter.length; i++)
                                    if (ST.invalid(((MultiTileEntityFilter) mTileEntity).mFilter[i])) {
                    ...
                    if (aMouseclick == 0) {
                        ...
                        if ((((MultiTileEntityFilter) mTileEntity).mModes & EXTENDER_TANK) != 0) {
                            ...
                            if (tFluid == null) {
                                ...
                                if (tData != null && tData.mPrefix.contains(TD.Prefix.IS_CONTAINER)
                                    && !tData.mPrefix.contains(TD.Prefix.IS_CRATE)) {
    ...
    public class MultiTileEntityGUIClientFilter extends ContainerClient {
        ...
        public MultiTileEntityGUIClientFilter(InventoryPlayer aInventoryPlayer, MultiTileEntityFilter aTileEntity,
            int aGUIID) {
            super(
                new MultiTileEntityGUICommonFilter(aInventoryPlayer, aTileEntity, aGUIID),
                RES_PATH_GUI + "machines/Filter.png");
            mRows = mContainer.mTileEntity.getSizeInventoryGUI() / 9
                + (mContainer.mTileEntity.getSizeInventoryGUI() % 9 == 0 ? 0 : 1);
            ySize = 114 + mRows * 18;
        }
