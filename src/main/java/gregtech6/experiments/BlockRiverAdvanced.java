...
public class BlockRiverAdvanced extends BlockWaterlike {
    ...
    public void updateTick(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        for (byte tSide : ALL_SIDES_VALID) {
            ...
            if (aBlocks[tSide] == Blocks.bedrock) aSource = tSide; // TODO: Remove this if this River Block ever gets
                                                                   // used!
        }
        ...
        for (byte tSide : ALL_SIDES_VALID) if (aBlocks[tSide] == BlocksGT.Ocean || aBlocks[tSide] == BlocksGT.River
            || aBlocks[tSide] == BlocksGT.Swamp) {
                if (SIDES_INVALID[aSource]) WD.set(aWorld, aX, aY, aZ, NB, 0, 3);
                return;
            }

        // No Source for this River Block, so remove it.
        if (SIDES_INVALID[aSource]) {
        ...
        if (SIDES_BOTTOM[aFlow] && SIDES_HORIZONTAL[aSource]) {
            ...
            if (tBlock == Blocks.dirt || tBlock == Blocks.grass
                || tBlock == Blocks.mycelium
                || tBlock == Blocks.sand
                || tBlock == Blocks.gravel
                || tBlock == Blocks.snow) {
        ...
        if (SIDES_HORIZONTAL[aSource] && SIDES_HORIZONTAL[aFlow] && aBlocks[aSource] == this) {
            ...
            if (!ALONG_AXIS[aFlow][aSource] && aMetas[aFlow] == aMeta
                && goThisWay(aWorld, aX + OFFX[aSource], aY, aZ + OFFZ[aSource], aFlow)) {
                WD.set(aWorld, aX, aY, aZ, NB, 0, 3, T);
                WD.set(
                    aWorld,
                    aX + OFFX[aSource] + OFFX[aFlow],
                    aY,
                    aZ + OFFZ[aSource] + OFFZ[aFlow],
                    this,
                    aMetas[aSource],
                    3,
                    T);
        ...
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[aY % ALL_SIDES_HORIZONTAL_ORDER.length])
            if (aBlocks[tSide] != this && canDisplace(aWorld, aX + OFFX[tSide], aY - 1, aZ + OFFZ[tSide])
                && goThisWay(aWorld, aX, aY, aZ, tSide)) return;

        // Try flowing into a direction that is likely to lead downwards.
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[aY % ALL_SIDES_HORIZONTAL_ORDER.length])
            if (aBlocks[tSide] != this && canDisplace(aWorld, aX + OFFX[tSide] * 2, aY - 1, aZ + OFFZ[tSide] * 2)
                && goThisWay(aWorld, aX, aY, aZ, tSide)) return;

        // Try flowing the same direction as surrounding River Blocks
        for (byte tSide : ALL_SIDES_HORIZONTAL)
            if (aBlocks[tSide] == this && aMetas[tSide] != 0 && aMetas[tSide] <= 6) {
        ...
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[aY % ALL_SIDES_HORIZONTAL_ORDER.length])
            if (aBlocks[tSide] != this && goThisWay(aWorld, aX, aY, aZ, tSide)) return;
    ...
    public boolean goThisWay(World aWorld, int aX, int aY, int aZ, byte aSide) {
        return displaceIfPossible(aWorld, aX + OFFX[aSide], aY + OFFY[aSide], aZ + OFFZ[aSide])
            && WD.set(aWorld, aX + OFFX[aSide], aY + OFFY[aSide], aZ + OFFZ[aSide], this, 0, 3, T)
            && WD.set(aWorld, aX, aY, aZ, this, aSide + 1, 3, T);
    }
