        ...
        for (int i = 0; i < 16; i++) for (int j = 0; j < 16; j++) {
            ...
            for (int tY = 1; tY < tMaxHeight; tY++) {
                ...
                if (aBlock == Blocks.bedrock) {
                    ...
                    if (tCanPlaceRocks && aRandom.nextInt(128) == 0) tRegistry.mBlock.placeBlock(
                        aWorld,
                        tX,
                        tY,
                        tZ,
                        ...
                        ST.save(
                            NBT_VALUE,
                            OP.rockGt.mat(
                                aRandom.nextBoolean() && tLastOre != null ? tLastOre.mTargetCrushing.mMaterial
                                    : tLastRock,
                                1)),
                        F,
                        T);
                ...
                } else if (aBlock == Blocks.stone
                    || (aBlock == Blocks.monster_egg && aStorage.getExtBlockMetadata(i, tY & 15, j) == 0)) {
                        tCanPlaceRocks = T;
                        boolean temp = T;
                        if (tScan[5] == tScan[1]) {
                            for (StoneLayerOres tOres : tScan[3].mOres) if (tOres.mMaterial.mID > 0
                                && tOres.check(tScan[3], aWorld, tX, tY, tZ, aBiome, aRandom)
                                && (tScan[6] == tScan[0] ? tOres.normal(tScan[3], aWorld, tX, tY, tZ, aBiome)
                                    : tOres.small(tScan[3], aWorld, tX, tY, tZ, aBiome))) {
                                        if (tOres.mGenerateIndicators) tLastOre = tOres.mMaterial;
                                        temp = F;
                                        break;
                                    }
                        } else {
                            for (StoneLayerOres tOres : StoneLayer.get(tScan[5], tScan[1])) if (tOres.mMaterial.mID > 0
                                && tOres.check(tScan[3], aWorld, tX, tY, tZ, aBiome, aRandom)
                                && tOres.set(tScan[3], aWorld, tX, tY, tZ, aBiome, aRandom)) {
                                    if (tOres.mGenerateIndicators) tLastOre = tOres.mMaterial;
                                    temp = F;
                                    break;
                                }
                        }
                        if (temp && tScan[4] != tScan[2]
                            && tScan[3].mOreSmall != null
                            && !StoneLayer.RANDOM_SMALL_GEM_ORES.isEmpty()
                            && aRandom.nextInt(100) == 0) {
                            if (tScan[3].mOreSmall.placeBlock(
                                aWorld,
                                tX,
                                tY,
                                tZ,
                                SIDE_UNKNOWN,
                                UT.Code.select(MT.Emerald, StoneLayer.RANDOM_SMALL_GEM_ORES).mID,
                                null,
                                F,
                                T)) {
                                temp = F;
                        ...
                        // Check for the GT6 Stone being natural. Unlikely case due to GT6 Stone being the thing that is
                        // supposed to generate this very moment and not before. But Villages would otherwise see their
                        // House Materials replaed.
                        ...
                        if (tCanPlaceRocks && !aBlock.getMaterial()
                            .isLiquid() && aRandom.nextInt(128) == 0)
                        ...
                        if (aBlock.isOpaqueCube()) {
                            tCanPlaceRocks = (aBlock.getMaterial() == Material.clay
                ...
                // Ores that should not generate too deeply will be replaced by (Deep)Slate. This prevents flammable
                // Ores near Lava in most cases.
                if (tY - 2 + tScanMinusOne < 24 && tScan[tScanMinusOne].mNoDeep)
