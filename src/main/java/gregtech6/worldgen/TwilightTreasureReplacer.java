...
public class TwilightTreasureReplacer extends TFTreasure {
    ...
    /**
     * needed simply because Hill 3 has way too many Chests and Hill 2 exclusively has the Peacock Fan. So I'm
     * downgrading Hill 3 a little.
     */
    ...
    public static TFTreasure create(TFTreasure aTreasure, int aIndex, String aCategory, String aVanillacategory,
        long aChestID) {
        return new TwilightTreasureReplacer(aTreasure, aIndex, aCategory, aVanillacategory, aChestID);
    }

    public TwilightTreasureReplacer(TFTreasure aTreasure, int aIndex, String aCategory, String aVanillacategory,
        long aChestID) {
        super(aIndex);
        ...
        if (aIndex == 4) {
            ...
            if (MD.ChocoCraft.mLoaded) {
                ItemStack tStack = ST.make(MD.ChocoCraft, "gysahlGreenBlock", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
            }
            // Harvestcraft Gardens
            if (MD.HaC.mLoaded) {
                ItemStack tStack = ST.make(MD.HaC, "tropicalgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "watergarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "herbgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "grassgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "textilegarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "groundgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "berrygarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "mushroomgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "leafygarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "desertgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "stalkgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
                tStack = ST.make(MD.HaC, "gourdgarden", 4, 0);
                if (ST.valid(tStack)) useless.add(tStack);
            }
            // Seeds for Stuff.
        ...
        if (aIndex == 7) {
            ...
            if (IL.EtFu_Magmatic_Netherrack.exists()) useless.add(IL.EtFu_Magmatic_Netherrack.get(24));
        }
        ...
        if (aIndex == 14) {
            ...
            mRares = 4;
            // Thaumcraft Saplings
            if (IL.TC_Greatwood_Sapling.exists()) rare.add(IL.TC_Greatwood_Sapling.get(4));
            if (IL.TC_Silverwood_Sapling.exists()) ultrarare.add(IL.TC_Silverwood_Sapling.get(1));
        ...
        if (aIndex == 21) {
            ...
            if (IL.EtFu_Chorus_Fruit.exists()) useless.add(IL.EtFu_Chorus_Fruit.get(24));
            if (IL.EtFu_Chorus_Flower.exists()) uncommon.add(IL.EtFu_Chorus_Flower.get(8));
        ...
        if (aIndex == 22) {
            ...
            mUncommons = 12;
            // Dimension Stuff that is nowhere else to be found.
            if (IL.EtFu_Dragon_Breath.exists()) useless.add(IL.EtFu_Dragon_Breath.get(36));
    ...
    public boolean generate(World aWorld, Random aRandom, int aX, int aY, int aZ) {
        return generate(aWorld, aRandom, aX, aY, aZ, Blocks.chest);
    }
    ...
    public boolean generate(World aWorld, Random aRandom, int aX, int aY, int aZ, Block aChest) {
        ...
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[RNGSUS.nextInt(ALL_SIDES_HORIZONTAL_ORDER.length)]) {
            ...
            if (tBlock == Blocks.chest || tBlock == Blocks.trapped_chest) {
                // replace adjacent vanilla Chests with Firefly Jars. Should help with the Maze Vault randomly exploding
                // from Mob spawns.
                WD.set(aWorld, aX + OFFX[tSide], aY, aZ + OFFZ[tSide], IL.TF_Firefly_Jar.block(), 0, 3);
                // face away from close Wall.
                for (byte tFace : (SIDES_AXIS_Z[tSide] ? ALL_SIDES_X : ALL_SIDES_Z))
                    if (WD.opq(aWorld, aX + OFFX[tFace], aY, aZ + OFFZ[tFace], T, T)) {
                        return tRegistry.mBlock.placeBlock(
                            aWorld,
                            aX,
                            aY,
                            aZ,
                            SIDE_UNKNOWN,
                            mChestID,
                            UT.NBT.make(NBT_FACING, OPOS[tFace], NBT_TRAPPED, T, "gt.dungeonloot", mCategory),
                            F,
                            T);
                    }
                // face away from further Wall then.
                for (byte tFace : (SIDES_AXIS_Z[tSide] ? ALL_SIDES_X : ALL_SIDES_Z))
                    if (WD.opq(aWorld, aX + OFFX[tFace] * 2, aY, aZ + OFFZ[tFace] * 2, T, T)) {
                        return tRegistry.mBlock.placeBlock(
                            aWorld,
                            aX,
                            aY,
                ...
                return tRegistry.mBlock.placeBlock(
                    ...
                    UT.NBT.make(
                        NBT_FACING,
                        (SIDES_AXIS_Z[tSide] ? ALL_SIDES_X : ALL_SIDES_Z)[RNGSUS.nextInt(2)],
                        NBT_TRAPPED,
                        T,
                        "gt.dungeonloot",
                        mCategory),
        ...
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[RNGSUS.nextInt(ALL_SIDES_HORIZONTAL_ORDER.length)]) {
            if (WD.air(aWorld, aX + OFFX[tSide], aY, aZ + OFFZ[tSide])) return tRegistry.mBlock.placeBlock(
                aWorld,
                aX,
                aY,
                aZ,
                SIDE_UNKNOWN,
                mChestID,
                UT.NBT.make(NBT_FACING, tSide, NBT_TRAPPED, T, "gt.dungeonloot", mCategory),
                F,
                T);
        }
        // face away from close Wall.
        for (byte tSide : ALL_SIDES_HORIZONTAL_ORDER[RNGSUS.nextInt(ALL_SIDES_HORIZONTAL_ORDER.length)]) {
            if (WD.opq(aWorld, aX + OFFX[tSide], aY, aZ + OFFZ[tSide], T, T)) return tRegistry.mBlock.placeBlock(
                aWorld,
                aX,
                aY,
                aZ,
                SIDE_UNKNOWN,
                mChestID,
                UT.NBT.make(NBT_FACING, OPOS[tSide], NBT_TRAPPED, T, "gt.dungeonloot", mCategory),
                F,
                T);
        }
        // face away from further Wall then.
        ...
        return tRegistry.mBlock.placeBlock(
            ...
            UT.NBT.make(
                NBT_FACING,
                ALL_SIDES_HORIZONTAL[RNGSUS.nextInt(ALL_SIDES_HORIZONTAL.length)],
                NBT_TRAPPED,
                T,
                "gt.dungeonloot",
                mCategory),
    ...
    public boolean generate(IInventory aInventory) {
        boolean rReturn = T;
        // About twice as much Loot as normal TF because the Loot is quite lackluster compared to the time investment
        // otherwise.
        ...
        // Some Extra Loot from a fitting Vanilla Category in order to make most Modded Loot Items available in TF, if
        // you can't find the few Vanilla Dungeons.
        if (UT.Code.stringValid(mVanillacategory))
            for (int i = 0, j = mVanillas + RNGSUS.nextInt(mVanillaRNG); i < j; i++)
                rReturn &= addToInventory(aInventory, ChestGenHooks.getOneItem(mVanillacategory, RNGSUS));
        return rReturn;
    }
    ...
    public boolean addToInventory(IInventory aInventory, ItemStack aStack) {
        ...
        aInventory.setInventorySlotContents(
            tSlot,
            IL.TF_Uncrafting.equal(aStack, T, T) ? IL.TF_Transformation_Powder.get(12 + RNGSUS.nextInt(13))
                : ST.item(aStack) == Items.potionitem ? IL.Bottle_Loot.get(1 + RNGSUS.nextInt(2)) : aStack);
        return T;
    }
    ...
    public int findEmptySlot(IInventory aInventory) {
        ...
        for (int i = 0; i < 100; i++) {
            int k = RNGSUS.nextInt(j);
            if (aInventory.getStackInSlot(k) == null) return k;
        }
