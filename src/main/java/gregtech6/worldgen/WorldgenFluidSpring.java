...
public class WorldgenFluidSpring extends WorldgenObject {
    ...
    @SafeVarargs
    public WorldgenFluidSpring(String aName, boolean aDefault, Block aBlock, int aMeta, int aProbability,
        FluidStack aSpringFluid, List<WorldgenObject>... aLists) {
        this(aName, aDefault, aBlock, aMeta, aProbability, 0, aSpringFluid, aLists);
    }

    @SafeVarargs
    public WorldgenFluidSpring(String aName, boolean aDefault, Block aBlock, int aMeta, int aProbability,
        int aIndicatorType, FluidStack aSpringFluid, List<WorldgenObject>... aLists) {
    ...
    public boolean generate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        if (GENERATING_SPECIAL || !WorldgenOresBedrock.GENERATED_NO_BEDROCK_ORE
            || !WorldgenOresBedrock.CAN_GENERATE_BEDROCK_ORE
            || aRandom.nextInt(mProbability) != 0) return F;
        ...
        for (int i = 0; i <= 6; i++)
            for (int tX = aMinX + i; tX <= aMaxX - i; tX++) for (int tZ = aMinZ + i; tZ <= aMaxZ - i; tZ++) {
            ...
            case 3:
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    for (int tY = tMaxHeight; tY > tMinHeight; tY--) {
                        ...
                        for (int a = -1; a <= 1; a++) for (int b = -1; b <= 1; b++) if (aRandom.nextBoolean())
                            if (BlocksGT.plantableGrass.contains(aWorld.getBlock(tX + a, tY, tZ + b))) {
                                WD.set(
                                    aWorld,
                                    tX + a,
                                    tY,
                                    tZ + b,
                                    BlocksGT.Grass,
                                    mIndicatorType == 3 ? 0 : 3 + mIndicatorType,
                                    0);
                            }
                        break;
                    }
