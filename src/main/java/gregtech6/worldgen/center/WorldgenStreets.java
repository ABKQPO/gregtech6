...
public class WorldgenStreets extends WorldgenObject {
    ...
    public WorldgenStreets(String aName, boolean aDefault, List<WorldgenObject>... aLists) {
        ...
        mHeight = getConfigFile()
            .get(mCategory, "Height", getConfigFile().get("worldgenerator.streets", "Height", WD.waterLevel() + 4));
        ...
        if (aMinX == -16 || aMinX == 0) {
            if (aMinZ == -16 || aMinZ == 0) {
                    ...
                    if (i <= -29 || j <= -29 || i >= 28 || j >= 28) {
                        if (UT.Code.inside(-12, 11, i) || UT.Code.inside(-12, 11, j)) {
                            aWorld.setBlock(
                                ...
                                j,
                                BlocksGT.Asphalt,
                                i == -31 || j == -31 || i == 30 || j == 30 ? DYE_INDEX_White : DYE_INDEX_Gray,
                                0);
                            aWorld.setBlock(i, mHeight + 1, j, NB, 0, 0);
                        } else {
                            aWorld.setBlock(i, mHeight, j, BlocksGT.Concrete, DYE_INDEX_Gray, 0);
                            if (!GENERATE_BIOMES && (i == -32 || j == -32 || i == 31 || j == 31)
                                && (!GENERATE_NEXUS || i < 0 || i == 31 || j != -32)
                                && (!GENERATE_TESTING || j > 0 || i != 31 || j == -32)) {
                                aWorld.setBlock(
                                    i,
                                    mHeight + 1,
                                    j,
                                    WD.even(i, 1, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                                    DYE_INDEX_LightGray,
                                    0);
                                aWorld.setBlock(
                                    i,
                                    mHeight + 2,
                                    j,
                                    WD.even(i, 2, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                                    DYE_INDEX_LightGray,
                                    0);
                                aWorld.setBlock(
                                    i,
                                    mHeight + 3,
                                    j,
                                    WD.even(i, 3, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                                    DYE_INDEX_LightGray,
                                    0);
                                aWorld.setBlock(
                                    i,
                                    mHeight + 4,
                                    j,
                                    WD.even(i, 4, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                                    DYE_INDEX_LightGray,
                                    0);
                                aWorld.setBlock(
                                    i,
                                    mHeight + 5,
                                    j,
                                    WD.even(i, 5, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                                    DYE_INDEX_LightGray,
                                    0);
                            } else {
                                aWorld.setBlock(
                                    ...
                                    mHeight + 1,
                                    j,
                                    ((BlockMetaType) BlocksGT.CFoam).mSlabs[SIDE_Y_NEG],
                                    DYE_INDEX_LightGray,
                                    0);
                            }
                        }
                        ...
                        if (UT.Code.inside(-12, 11, i) && UT.Code.inside(-12, 11, j)) {
                            ...
                            aWorld.setBlock(
                                ...
                                mHeight + 1,
                                j,
                                UT.Code.inside(-11, 10, i) && UT.Code.inside(-11, 10, j) ? BlocksGT.Concrete
                                    : BlocksGT.CFoam,
                                DYE_INDEX_LightGray,
                                0);
                        } else {
                ...
                aWorld.setBlock(
                    ...
                    mHeight + 1,
                    -2,
                    ((BlockMetaType) BlocksGT.CFoam).mSlabs[SIDE_Z_POS],
                    DYE_INDEX_White,
                    0);
                aWorld.setBlock(
                    -31,
                ...
                aWorld.setBlock(
                    ...
                    mHeight + 1,
                    -1,
                    ((BlockMetaType) BlocksGT.CFoam).mSlabs[SIDE_X_NEG],
                    DYE_INDEX_White,
                    0);
                aWorld.setBlock(
                    -30,
                ...
                aWorld.setBlock(
                    ...
                    mHeight + 1,
                    1,
                    ((BlockMetaType) BlocksGT.CFoam).mSlabs[SIDE_Z_NEG],
                    DYE_INDEX_White,
                    0);
                aWorld.setBlock(
                    -32,
                ...
                aWorld.setBlock(
                    ...
                    mHeight + 1,
                    -1,
                    ((BlockMetaType) BlocksGT.CFoam).mSlabs[SIDE_X_POS],
                    DYE_INDEX_Red,
                    0);
                aWorld.setBlock(
                    29,
                ...
                if (GENERATE_BEACON) {
                    for (int i = -5; i < 5; i++)
                        for (int j = -5; j < 5; j++) WD.set(aWorld, i, mHeight - 3, j, Blocks.iron_block, 0, 0);
                    for (int i = -4; i < 4; i++)
                        for (int j = -4; j < 4; j++) WD.set(aWorld, i, mHeight - 2, j, Blocks.iron_block, 0, 0);
                    for (int i = -3; i < 3; i++)
                        for (int j = -3; j < 3; j++) WD.set(aWorld, i, mHeight - 1, j, Blocks.iron_block, 0, 0);
                    for (int i = -2; i < 2; i++)
                        for (int j = -2; j < 2; j++) WD.set(aWorld, i, mHeight, j, Blocks.iron_block, 0, 0);
            ...
            if (aMinZ < -96 || aMinZ > 80) {
                ...
                for (int tOpaqueCount = 0, i = -16; i < 16; i++) for (int j = 0; j < 16; j++) {
                    ...
                    if (tBlock.getMaterial()
                        .isLiquid() || WD.anywater(tBlock)
                        || (WD.opq(tBlock) && tBlock.getMaterial() != Material.wood
                            && !tBlock.isWood(aWorld, i, mHeight + 9, aMinZ + j)
                            && !tBlock.isLeaves(aWorld, i, mHeight + 9, aMinZ + j))) {
                        if (tOpaqueCount++ > 128) {
                ...
                for (int i = aMinZ; i <= aMaxZ; i++)
                    for (int j = (aMinZ < 0 ? 0 : -16), k = (aMinZ < 0 ? 16 : 0); j < k; j++) {
            ...
            if (aMinX < -96 || aMinX > 80) {
                ...
                for (int tOpaqueCount = 0, i = -16; i < 16; i++) for (int j = 0; j < 16; j++) {
                    ...
                    if (tBlock.getMaterial()
                        .isLiquid() || WD.anywater(tBlock)
                        || (WD.opq(tBlock) && tBlock.getMaterial() != Material.wood
                            && !tBlock.isWood(aWorld, aMinX + j, mHeight + 9, i)
                            && !tBlock.isLeaves(aWorld, aMinX + j, mHeight + 9, i))) {
                        if (tOpaqueCount++ > 128) {
    ...
    public final boolean generateRoadX(World aWorld, int aMinZ, boolean aLand, boolean aKillSky, boolean aTunnel,
        boolean aBridge, boolean aSideWalls) {
        for (int i = 0; i < 16; i++) {
            if (aLand) {
                for (int j = mHeight + 1; j > 0; j--) if (!WD.opq(aWorld, -13, j, aMinZ + i, T, T))
                    WD.set(aWorld, -13, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight + 1; j > 0; j--) if (!WD.opq(aWorld, 12, j, aMinZ + i, T, T))
                    WD.set(aWorld, 12, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight; j > 0; j--) if (!WD.opq(aWorld, -14, j, aMinZ + i, T, T))
                    WD.set(aWorld, -14, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight; j > 0; j--) if (!WD.opq(aWorld, 13, j, aMinZ + i, T, T))
                    WD.set(aWorld, 13, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 1; j > 0; j--) if (!WD.opq(aWorld, -15, j, aMinZ + i, T, T))
                    WD.set(aWorld, -15, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 1; j > 0; j--) if (!WD.opq(aWorld, 14, j, aMinZ + i, T, T))
                    WD.set(aWorld, 14, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, -16, j, aMinZ + i, T, T))
                    WD.set(aWorld, -16, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, 15, j, aMinZ + i, T, T))
                    WD.set(aWorld, 15, j, aMinZ + i, Blocks.gravel, 1, 0, T);
                else break;
            }
            if (aTunnel) {
                ...
                for (int j = 0; j < 7; j++) {
                    aWorld.setBlock(
                        ...
                        aMinZ + i,
                        BlocksGT.Concrete,
                        j == 3 ? DYE_INDEX_LightGray : DYE_INDEX_White,
                        0);
                    aWorld.setBlock(
                        12,
                        mHeight + j,
            ...
            if (aKillSky) {
                for (int j = -13; j < 13; j++)
                    for (int k = 2; k < 64; k++) aWorld.setBlock(j, mHeight + k, aMinZ + i, NB, 0, 0);
            } else {
                for (int j = -12; j < 12; j++)
                    for (int k = 2; k < 7; k++) aWorld.setBlock(j, mHeight + k, aMinZ + i, NB, 0, 0);
            }
            for (int j = -12; j < 2; j++) {
                ...
                if (aLand) {
                    ...
                    for (int k = mHeight - 3; k > 0; k--) if (!WD.opq(aWorld, j, k, aMinZ + i, T, T))
                        WD.set(aWorld, j, k, aMinZ + i, Blocks.cobblestone, 0, 0, T);
                    else break;
                } else {
            ...
            aWorld.setBlock(-11, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, 0, 0);
            ...
            aWorld.setBlock(-7, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, ((i + 2) / 4) % 2 == 0 ? 8 : 0, 0);
            ...
            aWorld.setBlock(-3, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, 0, 0);
            ...
            if (aLand) {
                ...
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, -1, j, aMinZ + i, T, T))
                    WD.set(aWorld, -1, j, aMinZ + i, Blocks.cobblestone, 0, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, 0, j, aMinZ + i, T, T))
                    WD.set(aWorld, 0, j, aMinZ + i, Blocks.cobblestone, 0, 0, T);
                else break;
            } else {
            ...
            aWorld.setBlock(
                ...
                0);
            aWorld.setBlock(2, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(2, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, 0, 0);
            aWorld.setBlock(3, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(4, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(5, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(6, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(6, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, ((i + 2) / 4) % 2 == 0 ? 8 : 0, 0);
            aWorld.setBlock(7, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(8, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(9, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(10, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(10, mHeight + 1, aMinZ + i, (Block) BlocksGT.RailRoad, 0, 0);
            aWorld.setBlock(11, mHeight, aMinZ + i, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(
                11,
                mHeight + 1,
        ...
        if (aSideWalls) {
            for (int i = 0; i < 8; i++) {
                Block tBlock = WD.block(aWorld, 13, mHeight + 4, aMinZ + i, T);
                if (tBlock.getMaterial()
                    .isLiquid() || WD.opq(tBlock)) {
                    for (int j = 0; j < 8; j++) for (int k = 2; k < 6; k++) WD.set(
                        aWorld,
                        12,
                        mHeight + k,
                        aMinZ + j,
                        WD.even(0, k, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                        DYE_INDEX_LightGray,
                        0,
                        T);
                    break;
            ...
            for (int i = 0; i < 8; i++) {
                Block tBlock = WD.block(aWorld, -14, mHeight + 4, aMinZ + i, T);
                if (tBlock.getMaterial()
                    .isLiquid() || WD.opq(tBlock)) {
                    for (int j = 0; j < 8; j++) for (int k = 2; k < 6; k++) WD.set(
                        aWorld,
                        ...
                        mHeight + k,
                        aMinZ + j,
                        WD.even(1, k, j) ? BlocksGT.CFoam : BlocksGT.Concrete,
                        DYE_INDEX_LightGray,
                        0,
                        T);
                    break;
            ...
            for (int i = 8; i < 16; i++) {
                ...
                if (tBlock.getMaterial()
                    .isLiquid() || WD.opq(tBlock)) {
                    for (int j = 8; j < 16; j++) for (int k = 2; k < 6; k++) WD.set(
                        aWorld,
                        12,
                        mHeight + k,
                        aMinZ + j,
        ...
        if (aMinZ >> 9 != (aMinZ - 16) >> 9) {
            for (int i = 5; i < 11; i++) for (int j = 1; j < 6; j++) {
                WD.set(
                    ...
                    aMinZ + i,
                    BlocksGT.Concrete,
                    i == 5 || i == 10 || j == 1 || j == 5 ? aTunnel ? DYE_INDEX_Black : DYE_INDEX_White
                        : aMinZ < 0 ? DYE_INDEX_Yellow : DYE_INDEX_Green,
                    0,
                    j == 1);
                WD.set(
                    aWorld,
            ...
            if (aTunnel) {
                for (int i = 0; i < 7; i++) {
                    aWorld.setBlock(
                        ...
                        aMinZ + 0,
                        BlocksGT.Concrete,
                        i == 3 ? DYE_INDEX_LightGray : DYE_INDEX_White,
                        0);
                    aWorld.setBlock(
                        -1,
                        mHeight + i,
        ...
        if (aBridge) {
            ...
            for (int k = mHeight - 4; k > 0; k--)
                if (!(WD.opq(aWorld, -10, k, aMinZ + 10, T, T) && WD.opq(aWorld, -5, k, aMinZ + 10, T, T)
                    && WD.opq(aWorld, -10, k, aMinZ + 5, T, T)
                    && WD.opq(aWorld, -5, k, aMinZ + 5, T, T))) {
            ...
            for (int k = mHeight - 4; k > 0; k--)
                if (!(WD.opq(aWorld, 9, k, aMinZ + 10, T, T) && WD.opq(aWorld, 4, k, aMinZ + 10, T, T)
                    && WD.opq(aWorld, 9, k, aMinZ + 5, T, T)
                    && WD.opq(aWorld, 4, k, aMinZ + 5, T, T))) {
        ...
        for (EntityLivingBase tEntity : (List<EntityLivingBase>) aWorld.getEntitiesWithinAABB(
            EntityLivingBase.class,
            AxisAlignedBB.getBoundingBox(-16, mHeight, aMinZ, +16, mHeight + 8, aMinZ + 16)))
            if (!(tEntity instanceof EntityPlayer)) tEntity.setDead();
        return T;
    }

    @SuppressWarnings("unchecked")
    public final boolean generateRoadZ(World aWorld, int aMinX, boolean aLand, boolean aKillSky, boolean aTunnel,
        boolean aBridge, boolean aSideWalls) {
        for (int i = 0; i < 16; i++) {
            if (aLand) {
                for (int j = mHeight + 1; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, -13, T, T))
                    WD.set(aWorld, aMinX + i, j, -13, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight + 1; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, 12, T, T))
                    WD.set(aWorld, aMinX + i, j, 12, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, -14, T, T))
                    WD.set(aWorld, aMinX + i, j, -14, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, 13, T, T))
                    WD.set(aWorld, aMinX + i, j, 13, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 1; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, -15, T, T))
                    WD.set(aWorld, aMinX + i, j, -15, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 1; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, 14, T, T))
                    WD.set(aWorld, aMinX + i, j, 14, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, -16, T, T))
                    WD.set(aWorld, aMinX + i, j, -16, Blocks.gravel, 1, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, 15, T, T))
                    WD.set(aWorld, aMinX + i, j, 15, Blocks.gravel, 1, 0, T);
                else break;
            }
            if (aTunnel) {
            ...
            if (aKillSky) {
                for (int j = -13; j < 13; j++)
                    for (int k = 2; k < 32; k++) aWorld.setBlock(aMinX + i, mHeight + k, j, NB, 0, 0);
            } else {
                for (int j = -12; j < 12; j++)
                    for (int k = 2; k < 7; k++) aWorld.setBlock(aMinX + i, mHeight + k, j, NB, 0, 0);
            }
            for (int j = -12; j < 2; j++) {
                ...
                if (aLand) {
                    ...
                    for (int k = mHeight - 3; k > 0; k--) if (!WD.opq(aWorld, aMinX + i, k, j, T, T))
                        WD.set(aWorld, aMinX + i, k, j, Blocks.cobblestone, 0, 0, T);
                    else break;
                } else {
            ...
            aWorld.setBlock(aMinX + i, mHeight + 1, -11, (Block) BlocksGT.RailRoad, 1, 0);
            ...
            aWorld.setBlock(aMinX + i, mHeight + 1, -7, (Block) BlocksGT.RailRoad, ((i + 2) / 4) % 2 == 0 ? 9 : 1, 0);
            ...
            aWorld.setBlock(aMinX + i, mHeight + 1, -3, (Block) BlocksGT.RailRoad, 1, 0);
            ...
            if (aLand) {
                ...
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, -1, T, T))
                    WD.set(aWorld, aMinX + i, j, -1, Blocks.cobblestone, 0, 0, T);
                else break;
                for (int j = mHeight - 2; j > 0; j--) if (!WD.opq(aWorld, aMinX + i, j, 0, T, T))
                    WD.set(aWorld, aMinX + i, j, 0, Blocks.cobblestone, 0, 0, T);
                else break;
            } else {
            ...
            aWorld.setBlock(
                ...
                0);
            aWorld.setBlock(aMinX + i, mHeight, 2, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight + 1, 2, (Block) BlocksGT.RailRoad, 1, 0);
            aWorld.setBlock(aMinX + i, mHeight, 3, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 4, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 5, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 6, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight + 1, 6, (Block) BlocksGT.RailRoad, ((i + 2) / 4) % 2 == 0 ? 9 : 1, 0);
            aWorld.setBlock(aMinX + i, mHeight, 7, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 8, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 9, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight, 10, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(aMinX + i, mHeight + 1, 10, (Block) BlocksGT.RailRoad, 1, 0);
            aWorld.setBlock(aMinX + i, mHeight, 11, BlocksGT.Asphalt, DYE_INDEX_Gray, 0);
            aWorld.setBlock(
                aMinX + i,
                mHeight + 1,
        ...
        if (aSideWalls) {
            for (int i = 0; i < 8; i++) {
                Block tBlock = WD.block(aWorld, aMinX + i, mHeight + 4, 13, T);
                if (tBlock.getMaterial()
                    .isLiquid() || WD.opq(tBlock)) {
                    for (int j = 0; j < 8; j++) for (int k = 2; k < 6; k++) WD.set(
                        aWorld,
            ...
            for (int i = 0; i < 8; i++) {
                Block tBlock = WD.block(aWorld, aMinX + i, mHeight + 4, -14, T);
                if (tBlock.getMaterial()
                    .isLiquid() || WD.opq(tBlock)) {
                    for (int j = 0; j < 8; j++) for (int k = 2; k < 6; k++) WD.set(
                        aWorld,
        ...
        if (aMinX >> 9 != (aMinX - 16) >> 9) {
            for (int i = 5; i < 11; i++) for (int j = 1; j < 6; j++) {
                WD.set(
                    ...
                    BlocksGT.Concrete,
                    i == 5 || i == 10 || j == 1 || j == 5 ? aTunnel ? DYE_INDEX_Black : DYE_INDEX_White
                        : aMinX < 0 ? DYE_INDEX_Blue : DYE_INDEX_Red,
                    0,
                    j == 1);
                WD.set(
                    aWorld,
        ...
        if (aBridge) {
            ...
            for (int k = mHeight - 4; k > 0; k--)
                if (!(WD.opq(aWorld, aMinX + 10, k, -10, T, T) && WD.opq(aWorld, aMinX + 10, k, -5, T, T)
                    && WD.opq(aWorld, aMinX + 5, k, -10, T, T)
                    && WD.opq(aWorld, aMinX + 5, k, -5, T, T))) {
            ...
            for (int k = mHeight - 4; k > 0; k--)
                if (!(WD.opq(aWorld, aMinX + 10, k, 9, T, T) && WD.opq(aWorld, aMinX + 10, k, 4, T, T)
                    && WD.opq(aWorld, aMinX + 5, k, 9, T, T)
                    && WD.opq(aWorld, aMinX + 5, k, 4, T, T))) {
        ...
        for (EntityLivingBase tEntity : (List<EntityLivingBase>) aWorld.getEntitiesWithinAABB(
            EntityLivingBase.class,
            AxisAlignedBB.getBoundingBox(aMinX, mHeight, -16, aMinX + 16, mHeight + 8, +16)))
            if (!(tEntity instanceof EntityPlayer)) tEntity.setDead();
        return T;
    }
}
