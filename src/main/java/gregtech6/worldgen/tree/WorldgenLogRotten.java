...
public class WorldgenLogRotten extends WorldgenOnSurface {
    ...
    @SafeVarargs
    public WorldgenLogRotten(String aName, boolean aDefault, int aAmount, int aProbability,
    ...
    public int canGenerate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        for (String tName : aBiomeNames)
            if (BIOMES_SWAMP.contains(tName) || BIOMES_JUNGLE.contains(tName)) return mAmount;
        return 0;
    }

    @Override
    public boolean tryPlaceStuff(World aWorld, int aX, int aY, int aZ, Random aRandom, Block aContact) {
        if (WD.anywater(aWorld, aX, aY, aZ, aContact)) aY--;
        else {
        ...
        switch (aRandom.nextInt(3)) {
            case 0:
                ...
                if (aY < tY - 2 && aRandom.nextBoolean())
                    WD.set(aWorld, aX, aY + 5, aZ, BlocksGT.Log1, PILLARS_Y[1], 2);
                return T;
            case 1:
