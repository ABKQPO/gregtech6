...
public class WorldgenLogMossy extends WorldgenOnSurface {
    ...
    @SafeVarargs
    public WorldgenLogMossy(String aName, boolean aDefault, int aAmount, int aProbability,
    ...
    public int canGenerate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        for (String tName : aBiomeNames)
            if (BIOMES_PLAINS.contains(tName) || BIOMES_WOODS.contains(tName) || BIOMES_SWAMP.contains(tName))
                return mAmount;
        return 0;
    }
    ...
    public boolean setMushroom(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        switch (aRandom.nextInt(MD.HaC.mLoaded ? 3 : 2)) {
            case 0:
                return WD.set(aWorld, aX, aY, aZ, Blocks.red_mushroom, 0, 2);
            case 1:
                return WD.set(aWorld, aX, aY, aZ, Blocks.brown_mushroom, 0, 2);
            case 2:
                return WD.set(aWorld, aX, aY, aZ, ST.block(MD.HaC, "mushroomgarden"), 0, 2);
        }
        return F;
    }
}
