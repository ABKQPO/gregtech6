...
public class WorldgenLogDry extends WorldgenOnSurface {
    ...
    @SafeVarargs
    public WorldgenLogDry(String aName, boolean aDefault, int aAmount, int aProbability,
    ...
    public int canGenerate(World aWorld, Chunk aChunk, int aDimType, int aMinX, int aMinZ, int aMaxX, int aMaxZ,
        ...
        for (String tName : aBiomeNames) if (BIOMES_PLAINS.contains(tName) || BIOMES_WOODS.contains(tName)
            || BIOMES_SAVANNA.contains(tName)
            || BIOMES_DESERT.contains(tName)
            || BIOMES_MESA.contains(tName)
