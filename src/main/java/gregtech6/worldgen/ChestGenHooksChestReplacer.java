...
public class ChestGenHooksChestReplacer extends ChestGenHooks {
    ...
    public WeightedRandomChestContent[] getItems(Random aRandom) {
        ...
        for (int i = 0; i < rReturn.length; i++)
            rReturn[i] = new WeightedRandomChestContentChestReplacer(rReturn[i], mCategory, mChestID);
        return rReturn;
    }

    @Override
    public void addItem(WeightedRandomChestContent aItem) {
        mHookToReplaceChestsOf.addItem(aItem);
    }

    @Override
    public void removeItem(ItemStack aStack) {
        mHookToReplaceChestsOf.removeItem(aStack);
    }

    @Override
    public int getCount(Random aRandom) {
        return mHookToReplaceChestsOf.getCount(aRandom);
    }

    @Override
    public ItemStack getOneItem(Random aRandom) {
        return mHookToReplaceChestsOf.getOneItem(aRandom);
    }

    @Override
    public int getMin() {
        return mHookToReplaceChestsOf.getMin();
    }

    @Override
    public int getMax() {
        return mHookToReplaceChestsOf.getMax();
    }

    @Override
    public void setMin(int aValue) {
        mHookToReplaceChestsOf.setMin(aValue);
        super.setMin(aValue);
    }

    @Override
    public void setMax(int aValue) {
        mHookToReplaceChestsOf.setMax(aValue);
        super.setMax(aValue);
    }
    ...
    public static class WeightedRandomChestContentChestReplacer extends WeightedRandomChestContent {
        ...
        public WeightedRandomChestContentChestReplacer(WeightedRandomChestContent aContent, String aCategory,
            ...
            super(
                aContent.theItemId,
                aContent.theMinimumChanceToGenerateItem,
                aContent.theMaximumChanceToGenerateItem,
                aContent.itemWeight);
            mCategory = aCategory;
            mContent = aContent;
            mChestID = aChestID;
        ...
        protected ItemStack[] generateChestContent(Random aRandom, IInventory aInventory) {
            ...
            int aX = ((TileEntityChest) aInventory).xCoord, aY = ((TileEntityChest) aInventory).yCoord,
                aZ = ((TileEntityChest) aInventory).zCoord;
            ...
            tRegistry.mBlock.placeBlock(
                ...
                SIDE_UNKNOWN,
                mChestID,
                UT.NBT.make(NBT_FACING, tFacing, NBT_TRAPPED, T, "gt.dungeonloot", mCategory),
                F,
                T);
        ...
        protected ItemStack[] generateChestContent2(Random aRandom, IInventory aInventory) {
            try {
                Method tMethod = mContent.getClass()
                    .getDeclaredMethod("generateChestContent", Random.class, IInventory.class);
