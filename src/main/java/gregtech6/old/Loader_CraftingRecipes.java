...
public class Loader_CraftingRecipes implements Runnable {
    ...
    public void run() {
         ...
         * tStack = null;
         * }
         * tStack = GT_Utility.getWrittenBook("GregTech Machine Safety (Edition 2)", "Gregorius Techneticies", new
         * String[] {
        ...
        CR.shaped(
            ST.mkic("reactorVent", 1),
            DEF,
            "AIA",
            "I I",
            "AIA",
            'I',
            ST.make(Blocks.iron_bars, 1, 0),
            'A',
            OP.plate.dat(MT.Al));
        CR.shapeless(
            ST.mkic("reactorPlatingExplosive", 1),
            DEF,
            new Object[] { ST.mkic("reactorPlating", 1), OP.plate.dat(MT.Pb) });

        CR.shaped(
            ST.mkic("glassFiberCableItem", 1),
            DEF,
            "GGG",
            "EDE",
            "GGG",
        ...
        CR.shaped(
            ...
            OD_CIRCUITS[3],
            'S',
            ST.mkic("energyCrystal", 1, W),
            'L',
            OP.dust.dat(MT.Lazurite));
        CR.shaped(
            ST.mkic("lapotronCrystal", 1),
            DEF,
            "LCL",
            ...
            ST.mkic("energyCrystal", 1, W),
            'L',
            OP.dust.dat(MT.Lapis));
        /*
         * CR.shaped(ST.makeIC2("luminator", 16), DEFAULT, "RTR", "GHG", "GGG", 'H', OP.cell.dat(MT.He), 'T',
         * OP.ingot.dat(MT.Sn), 'R', OP.ingot.dat(ANY.Fe), 'G', ST.make(Blocks.glass, 1, 0));
         * CR.shaped(ST.makeIC2("luminator", 16), DEFAULT, "RTR", "GHG", "GGG", 'H', OP.cell.dat(MT.Hg), 'T',
         * OP.ingot.dat(MT.Sn), 'R', OP.ingot.dat(ANY.Fe), 'G', ST.make(Blocks.glass, 1, 0));
         */
        ...
        if (ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "beryliumreflector", T)) {
            if (CR.remout(ST.mkic("reactorReflectorThick", 1))) CR.shaped(
                ST.mkic("reactorReflectorThick", 1),
                DEF,
                " N ",
                "NBN",
                " N ",
                ...
                OP.plate.dat(MT.Be),
                'N',
                ST.mkic("reactorReflector", 1));
        }

        if (OP.gear.mat(MT.Diamond, 1) != null) {
            ItemStack tStack = CR.get(
                OP.gear.mat(MT.Fe, 1),
            ...
            if (ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "quarry", T)) {
                ...
                CR.shaped(
                    ...
                    OD_CIRCUITS[3],
                    'D',
                    OP.gear.dat(ANY.Diamond),
                    'G',
                    OP.gear.dat(MT.Au),
                    'I',
                    OP.gear.dat(ANY.Steel),
                    'P',
                    ST.mkic("diamondDrill", 1, W));
            }
        ...
        if (ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "nanosaber", T)) {
            if (CR.remout(ST.mkic("nanoSaber", 1))) CR.shaped(
                ST.mkic("nanoSaber", 1),
                DEF,
                "PI ",
                "PI ",
                "CLC",
                'L',
                OP.battery.dat(MT.Master),
                'I',
                ST.mkic("iridiumPlate", 1),
                'P',
                OP.plate.dat(MT.Pt),
                'C',
                OD_CIRCUITS[5]);
        ...
        if (CR.remout(ST.mkic("diamondDrill", 1))) CR.shaped(
            ST.mkic("diamondDrill", 1),
            DEF,
            " D ",
            "DMD",
            "TAT",
            'M',
            ST.mkic("miningDrill", 1, W),
            'D',
            OP.gem.dat(MT.DiamondIndustrial),
            'T',
            OP.plate.dat(MT.Ti),
            'A',
            OD_CIRCUITS[3]);
        if (CR.remout(ST.mkic("miningDrill", 1))) CR.shaped(
            ST.mkic("miningDrill", 1),
            DEF,
            " S ",
            "SCS",
            "SBS",
            ...
            OD_CIRCUITS[1],
            'B',
            OP.battery.dat(MT.Basic),
            'S',
            ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "electricsteeltools", T)
                ? OP.plate.dat(MT.StainlessSteel)
                : OP.plate.dat(ANY.Fe));
        if (CR.remout(ST.mkic("chainsaw", 1))) CR.shaped(
            ST.mkic("chainsaw", 1),
            DEF,
            "BS ",
            "SCS",
            ...
            ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "electricsteeltools", T)
                ? OP.plate.dat(MT.StainlessSteel)
                : OP.plate.dat(ANY.Fe));
        if (CR.remout(ST.mkic("electricHoe", 1))) CR.shaped(
            ST.mkic("electricHoe", 1),
            DEF,
            "SS ",
            " C ",
            " B ",
            ...
            ConfigsGT.RECIPES.get(ConfigCategories.Recipes.harderrecipes, "electricsteeltools", T)
                ? OP.plate.dat(MT.StainlessSteel)
                : OP.plate.dat(ANY.Fe));
        if (CR.remout(ST.mkic("electricTreetap", 1))) CR.shaped(
            ST.mkic("electricTreetap", 1),
            DEF,
            " B ",
            "SCS",
            "S  ",
