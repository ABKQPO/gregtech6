...
public class BlockDiggable extends BlockBaseMeta implements IBlockOnWalkOver {
    ...
    public boolean canSustainPlant(IBlockAccess aWorld, int aX, int aY, int aZ, ForgeDirection aSide,
        ...
        return tType == EnumPlantType.Plains || tType == EnumPlantType.Water
            || tType == EnumPlantType.Desert
            || tType == EnumPlantType.Beach;
    }
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        switch (aMeta) {
            case 0:
                return ST.arraylist(IL.Mud_Ball.get(4));
            case 1:
                return ST.arraylist(IL.Clay_Ball_Brown.get(4));
            case 2:
                return ST.arraylist(OP.ingot.mat(MT.Peat, 4));
            case 3:
                return ST.arraylist(IL.Clay_Ball_Red.get(4));
            case 4:
                return ST.arraylist(IL.Clay_Ball_Yellow.get(4));
            case 5:
                return ST.arraylist(IL.Clay_Ball_Blue.get(4));
            case 6:
                return ST.arraylist(IL.Clay_Ball_White.get(4));
            default:
                return ST.arraylist(ST.make(this, 1, aMeta));
        }
    }

    @Override
    public void onWalkOver(EntityLivingBase aEntity, World aWorld, int aX, int aY, int aZ) {
        if (doesWalkSpeed(WD.meta(aWorld, aX, aY, aZ))) {
            aEntity.motionX *= 0.5;
            aEntity.motionZ *= 0.5;
        }
    }
    ...
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        return Blocks.dirt.getBlockHardness(aWorld, aX, aY, aZ);
    }

    @Override
    public float getExplosionResistance(byte aMeta) {
        return Blocks.dirt.getExplosionResistance(null);
    }
}
