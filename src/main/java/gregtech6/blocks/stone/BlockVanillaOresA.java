...
public class BlockVanillaOresA extends BlockBaseMeta {
    ...
    public static float[] HARDNESS_LEVELS = { 0.5F, 0.5F, 1.5F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.5F, 1.0F, 1.5F, 1.5F,
        1.5F, 0.5F, 2.0F, 0.5F };
    public static OreDictMaterial[] ORE_MATERIALS = { MT.S, MT.Apatite, MT.Ruby, MT.Amber, MT.Amethyst,
        MT.OREMATS.Galena, MT.OREMATS.Tetrahedrite, MT.OREMATS.Cassiterite, MT.OREMATS.Cooperite,
        MT.OREMATS.Pentlandite, MT.OREMATS.Scheelite, MT.TiO2, MT.OREMATS.Bastnasite, MT.Graphite,
        MT.OREMATS.Pitchblende, MT.OREMATS.Borax };
    ...
    public BlockVanillaOresA(String aUnlocalised) {
        super(
            ...
            Material.rock,
            soundTypeStone,
            ORE_MATERIALS.length,
            Textures.BlockIcons.VANILLA_ORES_A);
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        return ST.arraylist(OP.oreRaw.mat(ORE_MATERIALS[aMeta], aFortune > 0 ? 1 + RNGSUS.nextInt(aFortune + 1) : 1));
    }
    ...
    public int getExpDrop(IBlockAccess aWorld, int aMeta, int aFortune) {
        switch (aMeta) {
            case 0:
            case 1:
                return 0 + RNGSUS.nextInt(3);
            case 2:
            case 3:
            case 4:
                return 3 + RNGSUS.nextInt(5);
            default:
                return 2 + RNGSUS.nextInt(4);
        }
    }
    ...
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        return Blocks.stone.getBlockHardness(aWorld, aX, aY, aZ) * HARDNESS_LEVELS[WD.meta(aWorld, aX, aY, aZ)];
    }
