...
public class BlockSpikeSharp extends BlockBaseSpike {
    ...
    public void onEntityCollidedWithBlock(World aWorld, int aX, int aY, int aZ, Entity aEntity) {
        ...
        if (aEntity instanceof EntityLivingBase) {
            if (aMeta < 8) {
                if (!(aEntity instanceof EntityIronGolem || aEntity instanceof EntitySkeleton
                    || aEntity instanceof EntitySlime))
                    aEntity.attackEntityFrom(
                        DamageSources.getSpikeDamage(),
                        TFC_DAMAGE_MULTIPLIER * ((aMeta & 7) < 6 ? 5.0F : 2.5F));
            } else {
                aEntity.attackEntityFrom(
                    DamageSources.getSpikeDamage(),
                    TFC_DAMAGE_MULTIPLIER * ((aMeta & 7) < 6 ? 10.0F : 5.0F));
            }
        }
    }
}
