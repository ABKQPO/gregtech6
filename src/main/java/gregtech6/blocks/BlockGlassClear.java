...
public class BlockGlassClear extends BlockColored {
    ...
    public BlockGlassClear(String aUnlocalised) {
        super(
            ...
            0.5F,
            0,
            Textures.BlockIcons.GLASSES_CLEAR);
    ...
    protected BlockGlassClear(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial,
        SoundType aVanillaSoundType, String aName, String aDefaultLocalised, OreDictMaterial aMaterial,
        float aResistanceMultiplier, float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons,
        byte aSlabType, BlockMetaType aBlock) {
        super(
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        return ST.arraylist(OP.scrapGt.mat(MT.Glass, mBlock == this ? 80 : 40));
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess aWorld, int aX, int aY, int aZ, int aSide) {
        ...
        return aBlock instanceof BlockMetaType && ((BlockMetaType) aBlock).mBlock == mBlock
            ? WD.meta(aWorld, aX, aY, aZ)
                != aWorld.getBlockMetadata(aX - OFFX[aSide], aY - OFFY[aSide], aZ - OFFZ[aSide])
                || ((((BlockMetaType) aBlock).mSide != mSide || aSide == mSide)
                    && ((BlockMetaType) aBlock).mSide != OPOS[aSide]
                    && ((BlockMetaType) aBlock).mSide != SIDE_ANY)
