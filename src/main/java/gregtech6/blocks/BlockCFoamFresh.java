...
public class BlockCFoamFresh extends BlockColored implements IBlockFoamable {
    ...
    public BlockCFoamFresh(String aUnlocalised) {
        super(
            ...
            0.0F,
            0,
            Textures.BlockIcons.CFOAMS_FRESH);
    }
    ...
    protected BlockCFoamFresh(Class<? extends ItemBlock> aItemClass, Material aVanillaMaterial,
        SoundType aVanillaSoundType, String aName, String aDefaultLocalised, OreDictMaterial aMaterial,
        float aResistanceMultiplier, float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons,
        byte aSlabType, BlockMetaType aBlock) {
        super(
    ...
    public boolean dryFoam(World aWorld, int aX, int aY, int aZ, byte aSide) {
        return aWorld.setBlock(
            aX,
            aY,
            aZ,
            SIDES_VALID[mSide] ? ((BlockMetaType) BlocksGT.CFoam).mSlabs[mSide] : BlocksGT.CFoam,
