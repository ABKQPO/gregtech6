...
public class BlockBaleGrass extends BlockBaseBale {
    ...
    public void updateTick2(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (aRandom.nextInt(3) > 0
            && !(aWorld.isDaytime() && !aWorld.isRaining() && aWorld.canBlockSeeTheSky(aX, aY + 2, aZ))) return;
        ...
        if (!tWet) for (byte tSide : ALL_SIDES_VALID)
            if (WD.anywater(aWorld, aX + OFFX[tSide], aY + OFFY[tSide], aZ + OFFZ[tSide])) {
                tWet = T;
                break;
            }
        if ((aMeta & PILLAR_DATA) == 0) {
            if (tWet
                || (aWorld.isRaining() && tBiome.rainfall > 0 && aWorld.getPrecipitationHeight(aX, aZ) <= aY + 2)) {
        ...
        if ((aMeta & 3) == 0) {
            ...
            if (aPlayer != null) {
                if (aPlayer.worldObj.provider.isHellWorld) {
                    ...
                    int aX = UT.Code.roundDown(aPlayer.posX), aY = UT.Code.roundDown(aPlayer.posY),
                        aZ = UT.Code.roundDown(aPlayer.posZ);
                    BiomeGenBase tBiome = aPlayer.worldObj.getBiomeGenForCoords(aX, aZ);
                    if (tBiome.rainfall > 0.8F || (aPlayer.worldObj.isRaining() && tBiome.rainfall > 0
                        && aPlayer.worldObj.getPrecipitationHeight(aX, aZ) <= aY + 2)) {
