...
public class BlockTreeSaplingAB extends BlockBaseSapling {
    ...
    public boolean canBlockStay(World aWorld, int aX, int aY, int aZ) {
        ...
        // Coconut Trees should be able to grow on Sand, not because realism, but because it makes it easier to plant
        // them in Deserts and Beaches.
        // Weirdly this Line of Code does not work, so maybe such a Hybrid Plant is not possible thanks to Forge
        // fuckery.
        return (tBlock == Blocks.sand
            || tBlock.canSustainPlant(aWorld, aX, aY - 1, aZ, ForgeDirection.UP, (IPlantable) Blocks.cactus))
            && (aWorld.getBlockMetadata(aX, aY, aZ) & 7) == 6;
    }
        ...
        switch (aMeta & 7) {
            case 0:
                ...
                for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++)
                    if (i != 0 || j != 0) if (!canPlaceTree(aWorld, aX + i, tMaxHeight - 5, aZ + j)) return F;
                ...
                for (int tY = aY + 1; tY < tMaxHeight; tY++) {
                    if (tCanPlaceResinHole && tMaxHeight - tY > 5
                        && (aRandom.nextInt(2) == 0 || !MultiTileEntityResinHoleRubber.nearby(aWorld, aX, aY, aZ))) {
                        ...
                        if (tRegistry != null) {
                            tRegistry.mBlock.placeBlock(
                                ...
                                SIDE_UNKNOWN,
                                (short) 32762,
                                UT.NBT.make(NBT_FACING, ALL_SIDES_HORIZONTAL[aRandom.nextInt(4)]),
                                T,
                                T);
                            continue;
                        }
            ...
            case 1:
                ...
                for (int i = -2; i <= 2; i++) for (int j = -2; j <= 2; j++)
                    if (i != 0 || j != 0) if (!canPlaceTree(aWorld, aX + i, tMaxHeight - 4, aZ + j)) return F;
                if (aWorld.isRemote) return T;
            ...
            case 2:
                ...
                for (int i = -3; i <= 3; i++) for (int j = -3; j <= 3; j++)
                    if (i != 0 || j != 0) if (!canPlaceTree(aWorld, aX + i, tMaxHeight - 2, aZ + j)) return F;
                if (aWorld.isRemote) return T;
            ...
            case 4:
                ...
                for (int i = -2; i <= 2; i++) for (int j = -2; j <= 2; j++)
                    if (i != 0 || j != 0) if (!canPlaceTree(aWorld, aX + i, aY + 2, aZ + j)) return F;
                if (aWorld.isRemote) return T;
                ...
                for (int i = -3; i <= 3; i++) for (int j = -3; j <= 3; j++) {
                    if (Math.abs(i * j) < 9 && (i != 0 || j != 0))
                        placeTree(aWorld, aX + i, aY + 1, aZ + j, BlocksGT.Leaves_AB, 12);
                }
                return T;
            case 5:
            ...
            case 6:
                ...
                for (int i = -3; i <= 3; i++) for (int j = -3; j <= 3; j++)
                    if (i != 0 || j != 0) if (!canPlaceTree(aWorld, aX + i, tMaxHeight, aZ + j)) return F;
                if (aWorld.isRemote) return T;
