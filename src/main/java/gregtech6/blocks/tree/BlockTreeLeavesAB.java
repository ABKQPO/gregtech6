...
public class BlockTreeLeavesAB extends BlockBaseLeaves implements Runnable {
    ...
    public BlockTreeLeavesAB(String aUnlocalised, Block aSaplings) {
        super(
            ...
            soundTypeGrass,
            8,
            Textures.BlockIcons.LEAVES_AB,
            aSaplings,
            new Block[] { BlocksGT.LogA, BlocksGT.LogA, BlocksGT.LogA, BlocksGT.LogA, BlocksGT.LogB, BlocksGT.LogB,
                BlocksGT.LogB, BlocksGT.LogB },
    ...
    public int getLeavesRangeYPos(byte aMeta) {
        return 0;
    } // There is no instance where Leaves are below the Logs for these Trees.
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        ...
        if (RNGSUS.nextInt(tChance) < ((aMeta & 7) == 6 ? 2 : 1)) {
            ...
            switch (aMeta & 7) {
                case 2:
                    if (RNGSUS.nextInt(tChance) < 2) rDrops.add(OP.stick.mat(MT.WOODS.Willow, 1));
                    break;
                case 3:
                    if (RNGSUS.nextInt(tChance) < 2) rDrops.add(OP.stick.mat(MT.WOODS.BlueMahoe, 1));
                    break;
                case 4:
                    if (RNGSUS.nextInt(tChance) < 2) rDrops.add(OP.stick.mat(MT.WOODS.Hazel, 1));
                    break;
            }
        }
    ...
    public int colorMultiplier(IBlockAccess aWorld, int aX, int aY, int aZ) {
        return (aWorld.getBlockMetadata(aX, aY, aZ) & 7) == 7
            ? RAINBOW_ARRAY[(Math.abs(aX) + Math.abs(aY) + Math.abs(aZ)) % RAINBOW_ARRAY.length]
            : UNCOLORED;
    }
}
