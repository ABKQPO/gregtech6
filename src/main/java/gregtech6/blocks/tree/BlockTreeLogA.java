...
public class BlockTreeLogA extends BlockBaseLogFlammable implements IBlockToolable {
    ...
    public int getLeavesRangeSide(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeSide((byte) (aMeta & 3));
    }

    @Override
    public int getLeavesRangeYPos(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeYNeg((byte) (aMeta & 3));
    } // Yes it has to be the Negative Range of the Leaves here

    @Override
    public int getLeavesRangeYNeg(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeYPos((byte) (aMeta & 3));
    ...
    public long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (SIDES_HORIZONTAL[aSide] && aTool.equals(TOOL_drill) && WD.meta(aWorld, aX, aY, aZ) == 1) {
            ...
            if (tRegistry != null) {
                tRegistry.mBlock
                    .placeBlock(aWorld, aX, aY, aZ, SIDE_UNKNOWN, (short) 32761, UT.NBT.make(NBT_FACING, aSide), T, T);
                return aTool.equals(TOOL_axe) ? 500 : 1000;
            }
        }
