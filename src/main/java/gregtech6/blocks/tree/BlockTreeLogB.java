...
public class BlockTreeLogB extends BlockBaseLogFlammable implements IBlockToolable {
    ...
    public int getLeavesRangeSide(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeSide((byte) ((aMeta & 3) | 4));
    }

    @Override
    public int getLeavesRangeYPos(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeYNeg((byte) ((aMeta & 3) | 4));
    } // Yes it has to be the Negative Range of the Leaves here

    @Override
    public int getLeavesRangeYNeg(byte aMeta) {
        return ((BlockBaseLeaves) BlocksGT.Leaves_AB).getLeavesRangeYPos((byte) ((aMeta & 3) | 4));
    } // Yes it has to be the Positive Range of the Leaves here
    ...
    public long onToolClick(String aTool, long aRemainingDurability, long aQuality, Entity aPlayer,
        ...
        if (aTool.equals(TOOL_axe) || aTool.equals(TOOL_saw) || aTool.equals(TOOL_knife)) {
            ...
            ST.give(
                aPlayer,
                (aMeta & 3) == 1 ? IL.HaC_Cinnamon.get(1, IL.Food_Cinnamon.get(1, OM.dust(MT.Cinnamon)))
                    : OM.dust(MT.Bark),
                aWorld,
                aX + OFFX[aSide],
                aY + OFFY[aSide],
        ...
        if (SIDES_HORIZONTAL[aSide] && aTool.equals(TOOL_drill) && WD.meta(aWorld, aX, aY, aZ) == 3) {
            ...
            if (tRegistry != null) {
                tRegistry.mBlock
                    .placeBlock(aWorld, aX, aY, aZ, SIDE_UNKNOWN, (short) 32760, UT.NBT.make(NBT_FACING, aSide), T, T);
                return 1000;
            }
        }
