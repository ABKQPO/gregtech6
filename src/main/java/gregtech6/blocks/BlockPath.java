...
public class BlockPath extends BlockBaseMeta implements IBlockOnWalkOver, IRenderedBlock {
    ...
    public ArrayList<ItemStack> getDrops(World aWorld, int aX, int aY, int aZ, int aMeta, int aFortune) {
        switch (aMeta) {
            case 1:
                return ST.arraylist(IL.AETHER_Dirt.get(1));
            case 2:
                return ST.arraylist(IL.BoP_Dirt_Loamy.get(1));
            case 3:
                return ST.arraylist(IL.BoP_Dirt_Sandy.get(1));
            case 4:
                return ST.arraylist(IL.BoP_Dirt_Silty.get(1));
            case 5:
                return ST.arraylist(IL.EB_Dirt_Alfisol.get(1));
            case 6:
                return ST.arraylist(IL.EB_Dirt_Andisol.get(1));
            case 7:
                return ST.arraylist(IL.EB_Dirt_Gelisol.get(1));
            case 8:
                return ST.arraylist(IL.EB_Dirt_Histosol.get(1));
            case 9:
                return ST.arraylist(IL.EB_Dirt_Inceptisol.get(1));
            case 10:
                return ST.arraylist(IL.EB_Dirt_Mollisol.get(1));
            case 11:
                return ST.arraylist(IL.EB_Dirt_Oxisol.get(1));
            default:
                return ST.arraylist(ST.make(Blocks.dirt, 1, 0));
        }
    }
    ...
    public int getRenderType() {
        return RendererBlockTextured.INSTANCE == null ? 0 : RendererBlockTextured.INSTANCE.mRenderID;
    }
    ...
    public ITexture getTexture(int aRenderPass, byte aSide, ItemStack aStack) {
        ...
        return SIDES_BOTTOM[aSide] ? tDirt
            : BlockTextureMulti.get(tDirt, BlockTextureDefault.get(Textures.BlockIcons.PATH_SIDE));
    }
    ...
    public ITexture getTexture(int aRenderPass, byte aSide, boolean[] aShouldSideBeRendered, IBlockAccess aWorld,
        ...
        return BlockTextureMulti.get(
            BlockTextureDefault.get(mIcons[WD.meta(aWorld, aX, aY, aZ) % 16]),
            BlockTextureDefault
                .get(isHalfBlock(aWorld, aX, aY, aZ) ? Textures.BlockIcons.PATH_SLAB : Textures.BlockIcons.PATH_SIDE));
    }

    public boolean isHalfBlock(IBlockAccess aWorld, int aX, int aY, int aZ) {
        return aWorld.getBlock(aX + 1, aY - 1, aZ) == this || aWorld.getBlock(aX, aY - 1, aZ + 1) == this
            || aWorld.getBlock(aX - 1, aY - 1, aZ) == this
            || aWorld.getBlock(aX, aY - 1, aZ - 1) == this;
    }
    ...
    public void onWalkOver(EntityLivingBase aEntity, World aWorld, int aX, int aY, int aZ) {
        if ((aEntity.motionX != 0 || aEntity.motionZ != 0) && !aEntity.isInWater() && !aEntity.isSneaking()) {
            double tSpeed = (aWorld.getBlock(aX, aY - 1, aZ).slipperiness >= 0.8 && isHalfBlock(aWorld, aX, aY, aZ)
                ? 1.05
                : 1.1);
    ...
    public boolean setBlockBounds(int aRenderPass, ItemStack aStack) {
        setBlockBounds(0, 0, 0, 1, PX_N[1], 1);
        return T;
    }
    ...
    public boolean setBlockBounds(int aRenderPass, IBlockAccess aWorld, int aX, int aY, int aZ,
        boolean[] aShouldSideBeRendered) {
        setBlockBounds(0, 0, 0, 1, (isHalfBlock(aWorld, aX, aY, aZ) ? PX_N[9] : PX_N[1]), 1);
        return T;
    }
    ...
    public IIcon getIcon(int aSide, int aMeta) {
        return (SIDES_TOP[aSide] ? Textures.BlockIcons.PATH_TOP : Textures.BlockIcons.DIRTS[aMeta % 16]).getIcon(0);
    }

    @Override
    @SuppressWarnings({ "unchecked" })
    public void addCollisionBoxesToList(World aWorld, int aX, int aY, int aZ, AxisAlignedBB aAABB, List aList,
        Entity aEntity) {
        AxisAlignedBB tAABB = AxisAlignedBB.getBoundingBox(aX, aY, aZ, aX + 1, aY + 0.5, aZ + 1);
        if (tAABB.intersectsWith(aAABB)) aList.add(tAABB);
        if (isHalfBlock(aWorld, aX, aY, aZ)) return;
        tAABB = AxisAlignedBB.getBoundingBox(aX, aY, aZ, aX + 1, aY + 0.875, aZ + 1);
        if (tAABB.intersectsWith(aAABB)) aList.add(tAABB);
        tAABB = AxisAlignedBB.getBoundingBox(aX, aY, aZ, aX + 1, aY + 1, aZ + 1);
        if (tAABB.intersectsWith(aAABB)) aList.add(tAABB);
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World aWorld, int aX, int aY, int aZ) {
        return AxisAlignedBB
            .getBoundingBox(aX, aY, aZ, aX + 1, aY + (isHalfBlock(aWorld, aX, aY, aZ) ? 0.5 : 1), aZ + 1);
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess aWorld, int aX, int aY, int aZ) {
        setBlockBounds(0, 0, 0, 1, (isHalfBlock(aWorld, aX, aY, aZ) ? 0.5F : 1), 1);
    }
    ...
    public float getBlockHardness(World aWorld, int aX, int aY, int aZ) {
        return Blocks.grass.getBlockHardness(aWorld, aX, aY, aZ) * 2;
    }

    @Override
    public float getExplosionResistance(byte aMeta) {
        return Blocks.grass.getExplosionResistance(null) * 1.5F;
    }
