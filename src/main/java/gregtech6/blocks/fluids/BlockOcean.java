...
public class BlockOcean extends BlockWaterlike {
    ...
    public void onNeighborBlockChange(World aWorld, int aX, int aY, int aZ, Block aBlock) {
        if (aBlock == Blocks.dirt && aWorld.getBlock(aX, aY - 1, aZ) == Blocks.grass)
    ...
    public void updateTick(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        if (WD.meta(aWorld, aX, aY, aZ) != 0) {
            if (tOceanCounter >= 2 || (SPREAD_TO_AIR && tHasOceanBiome)
                || (aWorld.getBlock(aX, aY + 1, aZ) == this && WD.meta(aWorld, aX, aY + 1, aZ) == 0)) {
    ...
    public int getLightOpacity(IBlockAccess aWorld, int aX, int aY, int aZ) {
        // TODO FIX THIS SHIT
        return aWorld.getBlockMetadata(aX, aY, aZ) == 0 && WD.air(aWorld.getBlock(aX, aY + 1, aZ))
            && WD.air(aWorld.getBlock(aX, aY + 2, aZ))
            && aWorld.getBlock(aX, aY - 1, aZ)
                .getLightOpacity(aWorld, aX, aY - 1, aZ) < LIGHT_OPACITY_MAX ? 16 : LIGHT_OPACITY_NONE;
    }
