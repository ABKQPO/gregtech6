...
public abstract class BlockWaterlike extends BlockFluidClassic implements IBlock, IItemGT, IBlockOnHeadInside {
    ...
    public void updateFlow(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        // Here was an else Block that only caused huge amounts of Network Lag with no purpose. Forge, just what the
        // fuck, setting Metadata from 0 to 0 and updating that "change" to Clients? There was no change that needed to
        // be updated!

        if (canDisplace(aWorld, aX, aY + densityDir, aZ)) {
            if (displaceIfPossible(aWorld, aX, aY + densityDir, aZ))
                aWorld.setBlock(aX, aY + densityDir, aZ, this, 1, WATER_UPDATE_FLAGS | 1);
            return;
        }

        int tFlowMeta = (aWorld.getBlock(aX, aY - densityDir, aZ) instanceof BlockWaterlike ? 1
            : quantaPerBlock - quantaRemaining + 1);
        if (tFlowMeta >= quantaPerBlock) return;

        if (aWorld.blockExists(aX, aY, aZ - 1) && displaceIfPossible(aWorld, aX, aY, aZ - 1))
            aWorld.setBlock(aX, aY, aZ - 1, this, tFlowMeta, WATER_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX, aY, aZ + 1) && displaceIfPossible(aWorld, aX, aY, aZ + 1))
            aWorld.setBlock(aX, aY, aZ + 1, this, tFlowMeta, WATER_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX - 1, aY, aZ) && displaceIfPossible(aWorld, aX - 1, aY, aZ))
            aWorld.setBlock(aX - 1, aY, aZ, this, tFlowMeta, WATER_UPDATE_FLAGS | 1);
        if (aWorld.blockExists(aX + 1, aY, aZ) && displaceIfPossible(aWorld, aX + 1, aY, aZ))
            aWorld.setBlock(aX + 1, aY, aZ, this, tFlowMeta, WATER_UPDATE_FLAGS | 1);
    }
    ...
    public Vec3 getFlowVector(IBlockAccess aWorld, int aX, int aY, int aZ) {
        ...
        if (aWorld.getBlock(aX, aY + 1, aZ) instanceof BlockWaterlike
            && (isBlockSolid(aWorld, aX, aY, aZ - 1, SIDE_Z_NEG) || isBlockSolid(aWorld, aX, aY, aZ + 1, SIDE_Z_POS)
    ...
    public final String getUnlocalizedName() {
        return FL.name(mFluid, F);
    }

    @Override
    public String getLocalizedName() {
        return FL.name(mFluid, T);
    }
    ...
    public void onHeadInside(EntityLivingBase aEntity, World aWorld, int aX, int aY, int aZ) {
        if (!aWorld.isRemote && !mEffects.isEmpty()
            && (FL.gas(mFluid) ? !UT.Entities.isImmuneToBreathingGases(aEntity)
                : !UT.Entities.isWearingFullChemHazmat(aEntity))) {
