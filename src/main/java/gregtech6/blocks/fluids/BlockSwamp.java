...
public class BlockSwamp extends BlockWaterlike {
    ...
    public void updateTick(World aWorld, int aX, int aY, int aZ, Random aRandom) {
        ...
        for (byte tSide : ALL_SIDES_BUT_TOP) {
            ...
            if (tBlock != NB) {
                ...
                if (tBlock == this) {
                    ...
                    if (tMeta == 0) tSwampCounter++;
                } else if (tBlock == Blocks.sand || tBlock == Blocks.dirt
                    || tBlock == Blocks.grass
                    || tBlock == Blocks.mycelium
                    || tBlock == BlocksGT.Grass
                    || tBlock == BlocksGT.Diggables
                    || tBlock == BlocksGT.Sands
                    || tBlock == BlocksGT.oreSand
                    || tBlock == BlocksGT.oreRedSand
                    || tBlock == BlocksGT.oreMud
                    || tBlock == BlocksGT.oreSmallSand
                    || tBlock == BlocksGT.oreSmallRedSand
                    || tBlock == BlocksGT.oreSmallMud
                    || IL.EtFu_Dirt.equal(tBlock)) {
                        tDirt = T;
                        ...
                        else if (WD.anywater(aWorld, aX, aY + 1, aZ))
                            aWorld.setBlock(aX, aY + 1, aZ, Blocks.brown_mushroom_block, 10, 3);
                        if (WD.air(aWorld, aX, aY + 2, aZ))
                            aWorld.setBlock(aX, aY + 2, aZ, Blocks.brown_mushroom_block, 10, 3);
                        else if (WD.anywater(aWorld, aX, aY + 2, aZ))
                            aWorld.setBlock(aX, aY + 2, aZ, Blocks.brown_mushroom_block, 10, 3);
                        if (WD.air(aWorld, aX, aY + 3, aZ))
                            aWorld.setBlock(aX, aY + 3, aZ, Blocks.brown_mushroom_block, 10, 3);
                        else if (WD.anywater(aWorld, aX, aY + 3, aZ))
                            aWorld.setBlock(aX, aY + 3, aZ, Blocks.brown_mushroom_block, 10, 3);
                        if (WD.air(aWorld, aX, aY + 4, aZ))
                            aWorld.setBlock(aX, aY + 4, aZ, Blocks.brown_mushroom_block, 10, 3);
                        else if (WD.anywater(aWorld, aX, aY + 4, aZ))
                            aWorld.setBlock(aX, aY + 4, aZ, Blocks.brown_mushroom_block, 10, 3);
                        for (int i = -2; i <= 2; i++) for (int j = -2; j <= 2; j++) {
                            // Plug the holes.
                            if (Math.abs(i * j) < 4) {
                                if (WD.air(aWorld, aX + i, aY, aZ + j))
                                    aWorld.setBlock(aX + i, aY, aZ + j, Blocks.brown_mushroom_block, 5, 3);
                                else if (WD.anywater(aWorld, aX + i, aY, aZ + j))
                                    aWorld.setBlock(aX + i, aY, aZ + j, Blocks.brown_mushroom_block, 5, 3);
                                if (WD.air(aWorld, aX + i, aY + 5, aZ + j))
                                    aWorld.setBlock(aX + i, aY + 5, aZ + j, Blocks.brown_mushroom_block, 5, 3);
                                else if (WD.anywater(aWorld, aX + i, aY + 5, aZ + j))
                                    aWorld.setBlock(aX + i, aY + 5, aZ + j, Blocks.brown_mushroom_block, 5, 3);
                            } else {
                                if (WD.anywater(aWorld, aX + i, aY, aZ + j))
                                    aWorld.setBlock(aX + i, aY, aZ + j, NB, 0, 3);
                                if (WD.anywater(aWorld, aX + i, aY + 5, aZ + j))
                                    aWorld.setBlock(aX + i, aY + 5, aZ + j, NB, 0, 3);
                            }
                            // Soak it all up.
                            if (WD.anywater(aWorld, aX + i, aY + 4, aZ + j))
                                aWorld.setBlock(aX + i, aY + 4, aZ + j, NB, 0, 3);
                            if (WD.anywater(aWorld, aX + i, aY + 3, aZ + j))
                                aWorld.setBlock(aX + i, aY + 3, aZ + j, NB, 0, 3);
                            if (WD.anywater(aWorld, aX + i, aY + 2, aZ + j))
                                aWorld.setBlock(aX + i, aY + 2, aZ + j, NB, 0, 3);
                            if (WD.anywater(aWorld, aX + i, aY + 1, aZ + j))
                                aWorld.setBlock(aX + i, aY + 1, aZ + j, NB, 0, 3);
                            if (WD.anywater(aWorld, aX + i, aY - 1, aZ + j))
                                aWorld.setBlock(aX + i, aY - 1, aZ + j, NB, 0, 3);
                            if (WD.anywater(aWorld, aX + i, aY - 2, aZ + j))
                                aWorld.setBlock(aX + i, aY - 1, aZ + j, NB, 0, 3);
        ...
        if (tDirt) for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++) {
            ...
            if (tBlock == Blocks.sand || tBlock == Blocks.dirt
                || tBlock == Blocks.grass
                || tBlock == Blocks.mycelium
                || IL.EtFu_Dirt.equal(tBlock)) {
                aWorld.setBlock(aX + i, aY + j, aZ + k, BlocksGT.Diggables, 0, 2);
                continue;
            }
            if (tBlock == BlocksGT.oreSand || tBlock == BlocksGT.oreRedSand) {
                BlocksGT.oreMud.placeBlock(
                    aWorld,
                    aX + i,
                    aY + j,
                    aZ + k,
                    SIDE_UNKNOWN,
                    ((IBlockExtendedMetaData) tBlock).getExtendedMetaData(aWorld, aX + i, aY + j, aZ + k),
                    null,
                    T,
                    T);
                continue;
            }
            if (tBlock == BlocksGT.oreSmallSand || tBlock == BlocksGT.oreSmallRedSand) {
                BlocksGT.oreSmallMud.placeBlock(
                    aWorld,
                    aX + i,
                    aY + j,
                    aZ + k,
        ...
        if (BIOMES_INFINITE_WATER.contains(tBiome.biomeName)) {
            ...
            for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) if (i != 0 && j != 0) {
                if (aWorld.getBlock(aX + i, aY, aZ + j) instanceof BlockSwamp
                    && aWorld.getBlockMetadata(aX + i, aY, aZ + j) == 0) {
                    tSwampCounter++;
                }
            }
            if (tSwampCounter < 3) {
        ...
        for (ChunkCoordinates tCoords : tList) {
            if (aWorld.setBlock(tCoords.posX, tCoords.posY, tCoords.posZ, this, 0, WATER_UPDATE_FLAGS))
                for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) {
                    if (aWorld.blockExists(tCoords.posX + i, tCoords.posY, tCoords.posZ + j)) {
                        ...
                        if (tBlock instanceof BlockSwamp) aWorld
                            .scheduleBlockUpdate(tCoords.posX + i, tCoords.posY, tCoords.posZ + j, this, tickRate);
                    }
                }
        }
    ...
    public static boolean water(Block aBlock) {
        return aBlock == Blocks.water || aBlock == Blocks.flowing_water
            || aBlock == BlocksGT.Ocean
            || aBlock == BlocksGT.River;
    }
}
