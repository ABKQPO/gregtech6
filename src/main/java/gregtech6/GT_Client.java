...
public class GT_Client extends GT_Proxy {
    ...
    private final PlayerModelRenderer mPlayerRenderer = new PlayerModelRenderer(
        mSupporterListSilver,
        mSupporterListGold);
            ...
            if (aEvent.player == GT_API.api_proxy.getThePlayer()) {
                if (FIRST_CLIENT_PLAYER_TICK) {
                        ...
                        tLink.getChatStyle()
                            .setChatClickEvent(
                                new ClickEvent(
                                    ...
                                    ConfigsGT.CLIENT.mConfig.getConfigFile()
                                        .getAbsolutePath()));
                        aEvent.player.addChatComponentMessage(tLink);
                    }
                    if (mVersionOutdated) {
                        aEvent.player.addChatComponentMessage(
                            new ChatComponentText("Major GT6 Update released, for details visit"));
                        tLink = new ChatComponentText(LH.Chat.BLUE + "https://gregtech.mechaenetia.com/1.7.10");
                        tLink.getChatStyle()
                            .setChatClickEvent(
                                new ClickEvent(ClickEvent.Action.OPEN_URL, "https://gregtech.mechaenetia.com/1.7.10"));
                        aEvent.player.addChatComponentMessage(tLink);
                    ...
                    if (MD.IC2.mLoaded && !MD.IC2C.mLoaded) {
                        try {
                            int tVersion = Integer.parseInt(
                                ((String) Class.forName("ic2.core.IC2")
                                    .getField("VERSION")
                                    .get(null)).substring(4, 7));
                            if (tVersion < 827) {
                                aEvent.player.addChatComponentMessage(
                                    new ChatComponentText(LH.Chat.RED + "Please update IndustrialCraft!"));
                                // IC2 Site doesn't support https.
                                tLink = new ChatComponentText(
                                    LH.Chat.BLUE + "http://ic2api.player.to:8080/job/IC2_experimental/827/");
                                tLink.getChatStyle()
                                    .setChatClickEvent(
                                        new ClickEvent(
                                            ClickEvent.Action.OPEN_URL,
                                            "http://ic2api.player.to:8080/job/IC2_experimental/827/"));
                                aEvent.player.addChatComponentMessage(tLink);
                            }
                        } catch (Throwable e) {/**/}
                    }
                            ...
                            if (Class.forName("com.chocohead.patcher.ThaumicFixer") != null) {
                                aEvent.player.addChatComponentMessage(
                                    new ChatComponentText(
                                        LH.Chat.RED + "Warning! Chocoheads ThaumicFixer needs to be uninstalled!"));
                                aEvent.player.addChatComponentMessage(
                                    new ChatComponentText(
                                        LH.Chat.ORANGE
                                            + "Not uninstalling it can lead to crashes when viewing Aspects."));
                                aEvent.player.addChatComponentMessage(
                                    new ChatComponentText(
                                        LH.Chat.ORANGE
                                            + "Lag is already fixed with a better Version of the ASM Code,"));
                                aEvent.player.addChatComponentMessage(
                                    new ChatComponentText(
                                        LH.Chat.ORANGE + "that doesn't obliterate the Thaumcraft API for no reason."));
                            }
                        } catch (Throwable e) {/**/}
                    }
                    if (MD.COG.mLoaded && !MD.PFAA.mLoaded
                        && ConfigsGT.CLIENT.get(ConfigCategories.general, "warnings_customoregen", T)) {
                        aEvent.player.addChatComponentMessage(
                            new ChatComponentText(
                                LH.Chat.RED
                                    + "Warning! CustomOreGen will screw up all GregTech Worldgen with its Default Configs!"));
                        aEvent.player.addChatComponentMessage(
                            new ChatComponentText(
                                LH.Chat.ORANGE
                                    + "If you don't even use CustomOreGen, I would highly recommend you to remove it."));
                    ...
                    if (WOODMANS_BDAY) {
                        aEvent.player.addChatComponentMessage(
                            new ChatComponentText(
                                LH.Chat.WHITE + "<" + LH.Chat.GREEN + ">:]" + LH.Chat.WHITE + "> Have a nice day!"));
                    }
                    if (APRIL_FOOLS) {
                        aEvent.player
                            .addChatComponentMessage(new ChatComponentText(CHAT_GREG + "Watch your Calendar!"));
                    }
                }
            }
        }
     ...
     * for (int i = 0, j = mSoundItems.size(); i < j; i++) if (UT.Stacks.equal(mSoundItems.get(i), aStack)) {tString =
     * mSoundNames.get(i); break;}
     * if (tString.startsWith("random.explode")) if (aStack.stackSize==3) tString = "random.fuse"; else if
     * (aStack.stackSize==2) tString = "random.old_explode";
     ...
     * }
     * }
     * if (tString.startsWith("streaming.")) aWorld.playRecord(tString.substring(10, tString.length()), (int)aX,
     * (int)aY, (int)aZ); else aWorld.playSound(aX, aY, aZ, tString, 3.0F,
     * tString.startsWith("note.")?(float)Math.pow(2.0D, (aStack.stackSize - 13) / 12.0D):1.0F, false);
     * }
     */
